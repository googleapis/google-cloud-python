        ...
        for pipeline_state_entry in pipeline_state["libraries"]:
            ...
            for api_path in pipeline_state_entry["apiPaths"]:
                ...
                if pipeline_state_entry["id"] == "google-cloud-trace":
                    ...
                    reference_doc_includes_forced='executions_v1+executions_v1beta+workflows_v1beta'
                ...
                if not reference_doc_includes_forced:
                    reference_doc_includes[pipeline_state_entry["id"]] = "+".join([api.replace(os.path.commonprefix(["".join(default_version.split("/")[:-1]), api]) + "/", "").replace("/", "_") for api in pipeline_state_entry["apiPaths"] if api != default_version])
                else:
