            ...
            for individual_api in apis_dir:
                ...
                if "id" in api_entry:
                    with open(PACKAGES_DIR / api_entry["id"] / ".repo-metadata.json", "r") as repo_metadata_json_file:
                        ...
                        api_entry["documentation-name"] = repo_metadata["name"]
                        ...
                        if "service-yaml" in api_entry:
                            with open(individual_api / api_entry["service-yaml"], "r") as service_yaml_file:
                                ...
                                if "documentation_uri:" not in service_yaml:
                                    api_entry["documentation-uri"] = repo_metadata["product_documentation"]
                                if "summary:" not in service_yaml and "api_description" in repo_metadata:
                                    api_entry["api-description"] = repo_metadata.get("api_description", repo_metadata.get("name_pretty"))
                        # if there is no service yaml, backfill as much as possible
                        else:
                            api_entry["title"] = repo_metadata.get("name_pretty")
                            api_entry["documentation-uri"] = repo_metadata.get("product_documentation")
                            api_entry["api-description"] = repo_metadata.get("api_description", repo_metadata.get("name_pretty"))
