    ...
    with open(pipeline_state_json, "r") as pipeline_state_json_file:
        ...
        for pipeline_state_entry in pipeline_state["libraries"]:
            ...
            for api_path in pipeline_state_entry["apiPaths"]:
                ...
                # For this API, determine if we need to explicitly set it to the default version
                ...
                if pipeline_state_entry["id"] == "google-cloud-trace":
                    default_versions[pipeline_state_entry["id"]] = "v2"
                default_versions[pipeline_state_entry["id"]] = default_version
                non_default_versions[pipeline_state_entry["id"]] = "+".join([api for api in pipeline_state_entry["apiPaths"] if api != default_version])
            ...
            for individual_api in apis_dir:
                ...
                if "id" in api_entry:
                    ...
                    if api_entry["id"] in default_versions:
                        api_entry["default-version"] = default_versions[api_entry["id"]]
                        api_entry["non-default-versions"] = non_default_versions[api_entry["id"]]
