...
HttpRequest = _reflection.GeneratedProtocolMessageType(
    ...
    dict(
        ...
        __doc__="""A common proto for logging HTTP requests. Only contains semantics
  ...
  Attributes:
      ...
      request_size:
          The size of the HTTP request message in bytes, including the
          request headers and the request body.
      status:
          The response code indicating the status of response. Examples:
          200, 404.
      response_size:
          The size of the HTTP response message sent back to the client,
          in bytes, including the response headers and the response
          body.
      user_agent:
          The user agent sent by the client. Example: ``"Mozilla/4.0
          (compatible; MSIE 6.0; Windows 98; Q312461; .NET CLR
          1.0.3705)"``.
      remote_ip:
      ...
      server_ip:
          The IP address (IPv4 or IPv6) of the origin server that the
          request was sent to.
      referer:
          The referer URL of the request, as defined in `HTTP/1.1 Header
          Field Definitions
          <http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html>`__.
      latency:
          The request processing latency on the server, from the time
          the request was received until the response was sent.
      cache_lookup:
          Whether or not a cache lookup was attempted.
      cache_hit:
          Whether or not an entity was served from cache (with or
          without validation).
      ...
      cache_fill_bytes:
          The number of HTTP response bytes inserted into cache. Set
