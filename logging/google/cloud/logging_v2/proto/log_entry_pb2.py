...
_LOGENTRY = _descriptor.Descriptor(
  ...
  fields=[
    ...
    _descriptor.FieldDescriptor(
      ...
      options=None),
    _descriptor.FieldDescriptor(
      name='source_location', full_name='google.logging.v2.LogEntry.source_location', index=14,
...
LogEntry = _reflection.GeneratedProtocolMessageType('LogEntry', (_message.Message,), dict(
  ...
  Attributes:
      ...
      timestamp:
          ...
          current time.  Incoming log entries should have timestamps
          that are no more than the `logs retention period
          </logging/quota-policy>`__ in the past, and no more than 24
          hours in the future. See the ``entries.write`` API method for
          more information.
      receive_timestamp:
      ...
      span_id:
          Optional. Id of the span within the trace associated with the
          log entry. e.g. "0000000000000042" For Stackdriver trace
          spans, this is the same format that the Stackdriver trace API
          uses. The ID is a 16-character hexadecimal encoding of an
          8-byte array.
      source_location:
