...
class ApiConfig(proto.Message):
    ...
    Attributes:
        ...
        openapi_documents (MutableSequence[google.cloud.apigateway_v1.types.ApiConfig.OpenApiDocument]):
            Optional. OpenAPI specification documents. If
            specified, grpc_services and
            managed_service_configs must not be included.
        grpc_services (MutableSequence[google.cloud.apigateway_v1.types.ApiConfig.GrpcServiceDefinition]):
            Optional. gRPC service definition files. If
            specified, openapi_documents must not be
            included.
    ...
    class GrpcServiceDefinition(proto.Message):
        ...
        Attributes:
            file_descriptor_set (google.cloud.apigateway_v1.types.ApiConfig.File):
                Input only. File descriptor set, generated by
                protoc.
                To generate, use protoc with imports and source
                info included. For an example test.proto file,
                the following command would put the value in a
                new file named out.pb.

                $ protoc --include_imports --include_source_info
                test.proto -o out.pb
            source (MutableSequence[google.cloud.apigateway_v1.types.ApiConfig.File]):
                Optional. Uncompiled proto files associated with
                the descriptor set, used for display purposes
                (server-side compilation is not supported).
                These should match the inputs to 'protoc'
...
class UpdateGatewayRequest(proto.Message):
    ...
    Attributes:
        update_mask (google.protobuf.field_mask_pb2.FieldMask):
            ...
            overwritten in the Gateway resource by the
...
class UpdateApiRequest(proto.Message):
    ...
    Attributes:
        update_mask (google.protobuf.field_mask_pb2.FieldMask):
            Field mask is used to specify the fields to be
            overwritten in the Api resource by the update.
...
class UpdateApiConfigRequest(proto.Message):
    ...
    Attributes:
        update_mask (google.protobuf.field_mask_pb2.FieldMask):
            Field mask is used to specify the fields to be
            overwritten in the ApiConfig resource by the
            update. The fields specified in the update_mask
            are relative to the resource, not the full
            request. A field will be overwritten if it is in
            the mask. If the user does not provide a mask
