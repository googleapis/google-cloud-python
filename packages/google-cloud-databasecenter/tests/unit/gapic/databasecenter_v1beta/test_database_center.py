...
from google.cloud.databasecenter_v1beta.services.database_center import DatabaseCenterAsyncClient, DatabaseCenterClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert DatabaseCenterClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert DatabaseCenterClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert DatabaseCenterClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert DatabaseCenterClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert DatabaseCenterClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert DatabaseCenterClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert DatabaseCenterClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert DatabaseCenterClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert DatabaseCenterClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert DatabaseCenterClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert DatabaseCenterClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert DatabaseCenterClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert DatabaseCenterClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(DatabaseCenterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatabaseCenterClient))
@mock.patch.object(DatabaseCenterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatabaseCenterAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DatabaseCenterClient._DEFAULT_UNIVERSE
    default_endpoint = DatabaseCenterClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = DatabaseCenterClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert DatabaseCenterClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        DatabaseCenterClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DatabaseCenterClient.DEFAULT_MTLS_ENDPOINT
    )
    assert DatabaseCenterClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert DatabaseCenterClient._get_api_endpoint(None, None, default_universe, "always") == DatabaseCenterClient.DEFAULT_MTLS_ENDPOINT
    assert (
        DatabaseCenterClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == DatabaseCenterClient.DEFAULT_MTLS_ENDPOINT
    )
    assert DatabaseCenterClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert DatabaseCenterClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        DatabaseCenterClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert DatabaseCenterClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert DatabaseCenterClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert DatabaseCenterClient._get_universe_domain(None, None) == DatabaseCenterClient._DEFAULT_UNIVERSE
...
def test_database_center_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "databasecenter.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://databasecenter.googleapis.com"
        )
...
def test_database_center_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DatabaseCenterAsyncClient, transports.DatabaseCenterGrpcAsyncIOTransport, "grpc_asyncio"),
        (DatabaseCenterClient, transports.DatabaseCenterRestTransport, "rest"),
    ],
)
@mock.patch.object(DatabaseCenterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatabaseCenterClient))
@mock.patch.object(DatabaseCenterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatabaseCenterAsyncClient))
def test_database_center_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DatabaseCenterAsyncClient, transports.DatabaseCenterGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (DatabaseCenterClient, transports.DatabaseCenterGrpcTransport, "grpc", "false"),
        (DatabaseCenterAsyncClient, transports.DatabaseCenterGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_database_center_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [DatabaseCenterClient, DatabaseCenterAsyncClient])
@mock.patch.object(DatabaseCenterClient, "DEFAULT_ENDPOINT", modify_default_endpoint(DatabaseCenterClient))
@mock.patch.object(DatabaseCenterAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(DatabaseCenterAsyncClient))
...
def test_database_center_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        (DatabaseCenterClient, transports.DatabaseCenterGrpcTransport, "grpc", grpc_helpers),
        (DatabaseCenterAsyncClient, transports.DatabaseCenterGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (DatabaseCenterClient, transports.DatabaseCenterRestTransport, "rest", None),
    ],
)
def test_database_center_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_database_center_client_client_options_from_dict():
    with mock.patch("google.cloud.databasecenter_v1beta.services.database_center.transports.DatabaseCenterGrpcTransport.__init__") as grpc_transport:
        grpc_transport.return_value = None
        client = DatabaseCenterClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
def test_database_center_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
@pytest.mark.asyncio
async def test_query_products_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.query_products in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.query_products] = mock_rpc
...
@pytest.mark.asyncio
async def test_query_products_async(transport: str = "grpc_asyncio", request_type=service.QueryProductsRequest):
    client = DatabaseCenterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_query_products_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.query_products), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            service.QueryProductsResponse(
                products=[
...
async def test_query_products_async_pages():
    ...
    with mock.patch.object(type(client.transport.query_products), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.query_products(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_query_products_rest_required_fields(request_type=service.QueryProductsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).query_products._get_unset_required_fields(jsonified_request)
...
def test_query_products_rest_unset_required_fields():
    transport = transports.DatabaseCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = DatabaseCenterClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = DatabaseCenterClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = DatabaseCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = DatabaseCenterAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = DatabaseCenterAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = DatabaseCenterClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_query_products_rest_bad_request(request_type=service.QueryProductsRequest):
    client = DatabaseCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_query_products_rest_interceptors(null_interceptor):
    transport = transports.DatabaseCenterRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.DatabaseCenterRestInterceptor(),
    )
    client = DatabaseCenterClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DatabaseCenterRestInterceptor, "post_query_products") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = service.QueryProductsResponse.to_json(service.QueryProductsResponse())
...
def test_database_center_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.DatabaseCenterTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_database_center_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.databasecenter_v1beta.services.database_center.transports.DatabaseCenterTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.DatabaseCenterGrpcTransport, grpc_helpers), (transports.DatabaseCenterGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.DatabaseCenterGrpcTransport, transports.DatabaseCenterGrpcAsyncIOTransport])
...
def test_database_center_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.DatabaseCenterRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_database_center_host_no_port(transport_name):
    client = DatabaseCenterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="databasecenter.googleapis.com"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "databasecenter.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://databasecenter.googleapis.com"
...
def test_database_center_host_with_port(transport_name):
    client = DatabaseCenterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="databasecenter.googleapis.com:8000"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "databasecenter.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://databasecenter.googleapis.com:8000"
    )
...
@pytest.mark.parametrize("transport_class", [transports.DatabaseCenterGrpcTransport, transports.DatabaseCenterGrpcAsyncIOTransport])
def test_database_center_transport_channel_mtls_with_client_cert_source(transport_class):
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.DatabaseCenterTransport, "_prep_wrapped_messages") as prep:
        client = DatabaseCenterClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = DatabaseCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
