...
import mock
import pytest
from google.api_core import client_options, grpc_helpers
...
class TestBigtableDataClient:
    ...
    def test_get_api_surface(self, method):
        ...
        if method == "get_table":
            ...
            surface = client.get_materialized_view(
                expected_instance_id, "view_id", expected_app_profile_id
            )
            assert isinstance(
                surface, CrossSync._Sync_Impl.TestMaterializedView._get_target_class()
            )
            assert (
    ...
    def test_api_surface_arg_passthrough(self, method):
        ...
        if method == "get_table":
            ...
            surface_type = CrossSync._Sync_Impl.TestMaterializedView._get_target_class()
        else:
            raise TypeError(f"unexpected method: {method}")
        with self._make_client(project="project-id") as client:
...
class TestTable:
    ...
    def test_ctor_invalid_timeout_values(self):
...
class TestMaterializedView(CrossSync._Sync_Impl.TestTable):
    ...
    def test_mutation_methods_unsupported(self, fn_name, fn_args):
        ...
        with pytest.raises(NotImplementedError) as e:
            view.__getattribute__(fn_name)(*fn_args)
        assert "not supported for materialized views" in str(e.value)
        client.close()

    def test_mutations_batcher_unsupported(self):
        ...
        class mock_stream:
            ...
            def __next__(self):
                ...
                if len(self.chunk_list) > self.idx:
                    if sleep_time:
                        CrossSync._Sync_Impl.sleep(self.sleep_time)
...
class TestReadRowsSharded:
        ...
        with self._make_client() as client:
            with client.get_table("instance", "table") as table:
                with mock.patch.object(table, "read_rows") as read_rows:
                    ...
                    table.read_rows_sharded(queries, operation_timeout=starting_timeout)
    ...
    def test_read_rows_sharded_negative_batch_timeout(self):
        ...
        with self._make_client(project=project) as client:
            with client.get_table(instance, table_id) as table:
                with mock.patch.object(
                    ...
                    table.read_modify_write_row(row_key, IncrementRule("f", "q"))
                    assert mock_gapic.call_count == 1
            ...
            with client.get_table("instance", "table_id") as table:
                with mock.patch.object(
                    ...
                    with mock.patch.object(Row, "_from_pb") as constructor_mock:
                        ...
                        table.read_modify_write_row("key", IncrementRule("f", "q"))
