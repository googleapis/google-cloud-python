...
class PolicyBasedRoutingServiceClient(metaclass=PolicyBasedRoutingServiceClientMeta):
    ...
    def policy_based_route_path(
        ...
        return "projects/{project}/{location}/global/PolicyBasedRoutes/{policy_based_route}".format(
            project=project,
            location=location,
    ...
    def parse_policy_based_route_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/(?P<location>.+?)/global/PolicyBasedRoutes/(?P<policy_based_route>.+?)$",
            path,
        )
        return m.groupdict() if m else {}
