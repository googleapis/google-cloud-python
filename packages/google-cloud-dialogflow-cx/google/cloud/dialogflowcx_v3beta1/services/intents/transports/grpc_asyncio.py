...
class IntentsGrpcAsyncIOTransport(IntentsTransport):
    ...
    @property
    def create_intent(self) -> Callable[[gcdc_intent.CreateIntentRequest], Awaitable[gcdc_intent.Intent]]:
    ...
    @property
    def update_intent(self) -> Callable[[gcdc_intent.UpdateIntentRequest], Awaitable[gcdc_intent.Intent]]:
    ...
    @property
    def import_intents(self) -> Callable[[intent.ImportIntentsRequest], Awaitable[operations_pb2.Operation]]:
        ...
        if "import_intents" not in self._stubs:
            self._stubs["import_intents"] = self._logged_channel.unary_unary(
    ...
    @property
    def export_intents(self) -> Callable[[intent.ExportIntentsRequest], Awaitable[operations_pb2.Operation]]:
        ...
        if "export_intents" not in self._stubs:
            self._stubs["export_intents"] = self._logged_channel.unary_unary(
