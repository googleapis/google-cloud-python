...
class ToolsGrpcAsyncIOTransport(ToolsTransport):
    ...
    def create_tool(
        ...
        if "create_tool" not in self._stubs:
            self._stubs["create_tool"] = self._logged_channel.unary_unary(
                ...
                request_serializer=gcdc_tool.CreateToolRequest.serialize,
                response_deserializer=gcdc_tool.Tool.deserialize,
            )
    ...
    def list_tools(
        ...
        Returns a list of
        [Tools][google.cloud.dialogflow.cx.v3beta1.Tool] in the
        specified agent.
    ...
    def export_tools(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "export_tools" not in self._stubs:
            self._stubs["export_tools"] = self._logged_channel.unary_unary(
                "/google.cloud.dialogflow.cx.v3beta1.Tools/ExportTools",
                request_serializer=tool.ExportToolsRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["export_tools"]
    ...
    def update_tool(
        ...
        if "update_tool" not in self._stubs:
            self._stubs["update_tool"] = self._logged_channel.unary_unary(
                ...
                request_serializer=gcdc_tool.UpdateToolRequest.serialize,
                response_deserializer=gcdc_tool.Tool.deserialize,
            )
    ...
    def list_tool_versions(
        ...
        Returns:
            Callable[[~.ListToolVersionsRequest],
                    Awaitable[~.ListToolVersionsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def restore_tool_version(
        ...
        Returns:
            Callable[[~.RestoreToolVersionRequest],
                    Awaitable[~.RestoreToolVersionResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            ...
            self.list_tool_versions: self._wrap_method(
                self.list_tool_versions,
                default_timeout=None,
                client_info=client_info,
            ),
            self.create_tool_version: self._wrap_method(
                self.create_tool_version,
                default_timeout=None,
                client_info=client_info,
            ),
            self.get_tool_version: self._wrap_method(
                self.get_tool_version,
                default_timeout=None,
                client_info=client_info,
            ),
            self.delete_tool_version: self._wrap_method(
                self.delete_tool_version,
                default_timeout=None,
                client_info=client_info,
            ),
            self.restore_tool_version: self._wrap_method(
