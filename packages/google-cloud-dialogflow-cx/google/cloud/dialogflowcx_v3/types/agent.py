...
class Agent(proto.Message):
    r"""Agents are best described as Natural Language Understanding
    (NLU) modules that transform user requests into actionable data.
    You can include agents in your app, product, or service to
    determine user intent and respond to the user in a natural way.

    After you create an agent, you can add
    `Intents <google.cloud.dialogflow.cx.v3.Intent>`__, `Entity
    Types <google.cloud.dialogflow.cx.v3.EntityType>`__, `Flows
    <google.cloud.dialogflow.cx.v3.Flow>`__,
    `Fulfillments <google.cloud.dialogflow.cx.v3.Fulfillment>`__,
    `Webhooks <google.cloud.dialogflow.cx.v3.Webhook>`__,
    `TransitionRouteGroups
    <google.cloud.dialogflow.cx.v3.TransitionRouteGroup>`__ and so
    on to manage the conversation flows.
    ...
    Attributes:
        ...
        default_language_code (str):
            Required. Immutable. The default language of the
            agent as a language tag. See `Language
            Support
            <https://cloud.google.com/dialogflow/cx/docs/reference/language>`__
            for a list of the currently supported language
            codes. This field cannot be set by the
            `Agents.UpdateAgent
            <google.cloud.dialogflow.cx.v3.Agents.UpdateAgent>`__
            method.
        ...
        avatar_uri (str):
            ...
            <https://cloud.google.com/dialogflow/docs/integrations/web-demo>`__
            integration.
        speech_to_text_settings (google.cloud.dialogflowcx_v3.types.SpeechToTextSettings):
            Speech recognition related settings.
        start_flow (str):
            Immutable. Name of the start flow in this agent.
            A start flow will be automatically created when
            the agent is created, and can only be deleted by
        ...
        enable_stackdriver_logging (bool):
            Indicates if stackdriver logging is enabled for
            the agent. Please use
            `agent.advanced_settings
            <google.cloud.dialogflow.cx.v3.AdvancedSettings.LoggingSettings>`__
            instead.
        ...
        enable_multi_language_training (bool):
            ...
            agent.
        locked (bool):
            Indicates whether the agent is locked for
            changes. If the agent is locked, modifications
            to the agent will be rejected except for
            [RestoreAgent][].
    ...
    class GenAppBuilderSettings(proto.Message):
        ...
        Attributes:
            engine (str):
                Required. The full name of the Gen App Builder
                engine related to this agent if there is one.
                Format: ``projects/{Project
                ID}/locations/{Location
                ID}/collections/{Collection ID}/engines/{Engine
                ID}``
        """
    ...
    class AnswerFeedbackSettings(proto.Message):
        ...
        Attributes:
            enable_answer_feedback (bool):
                Optional. If enabled, end users will be able to
                provide `answer feedback
                <google.cloud.dialogflow.cx.v3.AnswerFeedback>`__
                to Dialogflow responses. Feature works only if
                interaction logging is enabled in the Dialogflow
                agent.
        """
    ...
    class PersonalizationSettings(proto.Message):
        ...
        Attributes:
            default_end_user_metadata (google.protobuf.struct_pb2.Struct):
                Optional. Default end user metadata, used when
                processing DetectIntent requests. Recommended to
                be filled as a template instead of hard-coded
                value, for example { "age":
                "$session.params.age" }. The data will be merged
                with the
                `QueryParameters.end_user_metadata
                <google.cloud.dialogflow.cx.v3.QueryParameters.end_user_metadata>`__
                in
                `DetectIntentRequest.query_params
                <google.cloud.dialogflow.cx.v3.DetectIntentRequest.query_params>`__
    ...
    class ClientCertificateSettings(proto.Message):
        ...
        Attributes:
            ...
            private_key (str):
                Required. The name of the SecretManager secret
                version resource storing the private key encoded
                in PEM format. Format:

                ``projects/{project}/secrets/{secret}/versions/{version}``
            passphrase (str):
                Optional. The name of the SecretManager secret
                version resource storing the passphrase.
                'passphrase' should be left unset if the private
...
class ExportAgentRequest(proto.Message):
    ...
    Attributes:
        ...
        agent_uri (str):
            ...
            to export the agent to. The format of this URI
            must be ``gs://<bucket-name>/<object-name>``. If
            left unspecified, the serialized agent is
            returned inline.
        ...
        data_format (google.cloud.dialogflowcx_v3.types.ExportAgentRequest.DataFormat):
            Optional. The data format of the exported agent.
...
class ExportAgentResponse(proto.Message):
    ...
    Attributes:
        agent_uri (str):
            ...
            This field is populated if ``agent_uri`` is
            specified in `ExportAgentRequest
            <google.cloud.dialogflow.cx.v3.ExportAgentRequest>`__.

            This field is a member of `oneof`_ ``agent``.
        agent_content (bytes):
            Uncompressed raw byte content for agent. This
            field is populated if none of ``agent_uri`` and
            ...
            This field is a member of `oneof`_ ``agent``.
        commit_sha (str):
            Commit SHA of the git push. This field is
            populated if ``git_destination`` is specified in
            `ExportAgentRequest
            <google.cloud.dialogflow.cx.v3.ExportAgentRequest>`__.
...
class RestoreAgentRequest(proto.Message):
    ...
    Attributes:
        ...
        agent_uri (str):
            ...
            <https://cloud.google.com/storage/docs/>`__ URI
            to restore agent from. The format of this URI
...
class AgentValidationResult(proto.Message):
    ...
    Attributes:
        name (str):
            The unique identifier of the agent validation
            result. Format:
...
class UpdateGenerativeSettingsRequest(proto.Message):
    ...
    generative_settings: generative_settings.GenerativeSettings = proto.Field(
        proto.MESSAGE,
        number=1,
        message=generative_settings.GenerativeSettings,
    )
    update_mask: field_mask_pb2.FieldMask = proto.Field(
        proto.MESSAGE,
        number=2,
