...
class Page(proto.Message):
    ...
    For each flow, you define many pages, where your combined pages
    can handle a complete conversation on the topics the flow is
    designed for. At any given moment, exactly one page is the
    current page, the current page is considered active, and the
    flow associated with that page is considered active. Every flow
    has a special start page. When a flow initially becomes active,
    the start page page becomes the current page. For each
    conversational turn, the current page will either stay the same
    ...
    Attributes:
        ...
        transition_route_groups (MutableSequence[str]):
            ...
            added to the page. Transition route groups must
            be unique within a page. If the page links both
            flow-level transition route groups and
            agent-level transition route groups, the
            flow-level ones will have higher priority and
            will be put before the agent-level ones.

            *   If multiple transition routes within a page
            scope refer to the same     intent, then the
            precedence order is: page's transition route ->
            page's     transition route group -> flow's
            transition routes.

            *   If multiple transition route groups within a
            page contain the same     intent, then the first
            group in the ordered list takes precedence.
        ...
        transition_routes (MutableSequence[google.cloud.dialogflowcx_v3.types.TransitionRoute]):
            A list of transitions for the transition rules
            of this page. They route the conversation to
            another page in the same flow, or another flow.

            When we are in a certain page, the
            TransitionRoutes are evaluated in the following
            order:

            *   TransitionRoutes defined in the page with
            intent specified. *   TransitionRoutes defined
            in the
                `transition route
                groups
            <google.cloud.dialogflow.cx.v3.Page.transition_route_groups>`__
            with intent specified.

            *   TransitionRoutes defined in flow with intent
            specified. *   TransitionRoutes defined in the
                `transition route
                groups
            <google.cloud.dialogflow.cx.v3.Flow.transition_route_groups>`__
            with intent specified.

            *   TransitionRoutes defined in the page with
            only condition specified. *   TransitionRoutes
            defined in the
                `transition route
                groups
            <google.cloud.dialogflow.cx.v3.Page.transition_route_groups>`__
...
class Form(proto.Message):
    r"""A form is a data model that groups related parameters that can
    be collected from the user. The process in which the agent
    prompts the user and collects parameter values from the user is
    called form filling. A form can be added to a `page
    <google.cloud.dialogflow.cx.v3.Page>`__. When form filling is
    ...
    class Parameter(proto.Message):
        ...
        Attributes:
            ...
            redact (bool):
                ...
                be redacted in log.  If redaction is enabled,
                the parameter content will be replaced by
        ...
        class FillBehavior(proto.Message):
            ...
            Attributes:
                ...
                reprompt_event_handlers (MutableSequence[google.cloud.dialogflowcx_v3.types.EventHandler]):
                    The handlers for parameter-level events, used to
                    provide reprompt for the parameter or transition
                    to a different page/flow. The supported events
                    are:

                    *   ``sys.no-match-<N>``, where N can be from 1
                    to 6 *   ``sys.no-match-default``

                    *   ``sys.no-input-<N>``, where N can be from 1
                    to 6 *   ``sys.no-input-default``

                    *   ``sys.invalid-parameter``

                    ``initial_prompt_fulfillment`` provides the
                    first prompt for the parameter.

                    If the user's response does not fill the
                    parameter, a no-match/no-input event will be
                    triggered, and the fulfillment associated with
                    the ``sys.no-match-1``/``sys.no-input-1``
                    handler (if defined) will be called to provide a
                    prompt. The
                    ``sys.no-match-2``/``sys.no-input-2`` handler
                    (if defined) will respond to the next
                    no-match/no-input event, and so on.

                    A ``sys.no-match-default`` or
                    ``sys.no-input-default`` handler will be used to
                    handle all following no-match/no-input events
                    after all numbered no-match/no-input handlers
                    for the parameter are consumed.

                    A ``sys.invalid-parameter`` handler can be
                    defined to handle the case where the parameter
                    values have been ``invalidated`` by webhook. For
                    example, if the user's response fill the
                    parameter, however the parameter was invalidated
                    by webhook, the fulfillment associated with the
                    ``sys.invalid-parameter`` handler (if defined)
                    will be called to provide a prompt.

                    If the event handler for the corresponding event
                    can't be found on the parameter,
                    ``initial_prompt_fulfillment`` will be
                    re-prompted.
            """
...
class EventHandler(proto.Message):
    ...
    `event <google.cloud.dialogflow.cx.v3.EventHandler.event>`__
    that can be handled during a session. When the specified event
    happens, the following actions are taken in order:

    *   If there is a
    ```trigger_fulfillment``
    <google.cloud.dialogflow.cx.v3.EventHandler.trigger_fulfillment>`__
    associated with the event, it will be called.

    *   If there is a
    ```target_page``
    <google.cloud.dialogflow.cx.v3.EventHandler.target_page>`__
    associated with the event, the session will transition into the
    specified page.

    *   If there is a
    ```target_flow``
    <google.cloud.dialogflow.cx.v3.EventHandler.target_flow>`__
    associated with the event, the session will transition into the
    specified flow.
...
class TransitionRoute(proto.Message):
    r"""A transition route specifies a `intent
    <google.cloud.dialogflow.cx.v3.Intent>`__ that can be matched
    and/or a data condition that can be evaluated during a session.
    When a specified transition is matched, the following actions
    are taken in order:

    *   If there is a
    ```trigger_fulfillment``
    <google.cloud.dialogflow.cx.v3.TransitionRoute.trigger_fulfillment>`__
    associated with the transition, it will be called.

    *   If there is a
    ```target_page``
    <google.cloud.dialogflow.cx.v3.TransitionRoute.target_page>`__
    associated with the transition, the session will transition into
    the specified page.

    *   If there is a
    ```target_flow``
    <google.cloud.dialogflow.cx.v3.TransitionRoute.target_flow>`__
    associated with the transition, the session will transition into
    the specified flow.
    ...
    Attributes:
        ...
        intent (str):
            ...
            Indicates that the transition can only happen
            when the given intent is matched.
            At least one of ``intent`` or ``condition`` must
            be specified. When both ``intent`` and
            ``condition`` are specified, the transition can
            only happen when both are fulfilled.
        condition (str):
            The condition to evaluate against `form
            parameters
            <google.cloud.dialogflow.cx.v3.Form.parameters>`__
            ...
            only happen when both are fulfilled.
        trigger_fulfillment (google.cloud.dialogflowcx_v3.types.Fulfillment):
            The fulfillment to call when the condition is
            satisfied. At least one of
            ``trigger_fulfillment`` and ``target`` must be
            specified. When both are defined,
...
class ListPagesResponse(proto.Message):
    ...
    Attributes:
        pages (MutableSequence[google.cloud.dialogflowcx_v3.types.Page]):
...
class GetPageRequest(proto.Message):
    r"""The request message for
    `Pages.GetPage <google.cloud.dialogflow.cx.v3.Pages.GetPage>`__.
    ...
    Attributes:
        name (str):
            ...
            ``projects/<ProjectID>/locations/<LocationID>/agents/<AgentID>/flows/<FlowID>/pages/<PageID>``.
        language_code (str):
            The language to retrieve the page for. The
...
class DeletePageRequest(proto.Message):
    ...
    Attributes:
        name (str):
            ...
            ``projects/<ProjectID>/locations/<LocationID>/agents/<AgentID>/Flows/<flowID>/pages/<PageID>``.
        force (bool):
            This field has no effect for pages with no
            incoming transitions. For pages with incoming
            transitions:
            ...
            *  If ``force`` is set to true, Dialogflow will
            remove the page, as well as    any transitions
            to the page (i.e. `Target
               page <EventHandler.target_page>`__ in event
            handlers or `Target    page
            <TransitionRoute.target_page>`__ in transition
            routes that point to    this page will be
            cleared).
    """
...
class KnowledgeConnectorSettings(proto.Message):
    ...
    Attributes:
        ...
        trigger_fulfillment (google.cloud.dialogflowcx_v3.types.Fulfillment):
            ...
            When the answers from the Knowledge Connector
            are selected by Dialogflow, you can utitlize the
            request scoped parameter
            ``$request.knowledge.answers`` (contains up to
            the 5 highest confidence answers) and
            ``$request.knowledge.questions`` (contains the
            corresponding questions) to construct the
            fulfillment.
        target_page (str):
            The target page to transition to.
            Format:
        ...
        target_flow (str):
            ...
            This field is a member of `oneof`_ ``target``.
        data_store_connections (MutableSequence[google.cloud.dialogflowcx_v3.types.DataStoreConnection]):
