...
class ResponseMessage(proto.Message):
    ...
    * If the OutputAudioText responses are a mixture of text and
    SSML, then the   concatenated result is treated as SSML;
    otherwise, the result is treated as   either text or SSML as
    appropriate. The agent designer should ideally use   either text
    or SSML consistently throughout the bot design.

    * Otherwise, all Text responses are linearly concatenated, and
    the result is   used for output audio synthesis.

    This approach allows for more sophisticated user experience
    scenarios, where the text displayed to the user may differ from
    what is heard.
    ...
    Attributes:
        ...
        end_interaction (google.cloud.dialogflowcx_v3.types.ResponseMessage.EndInteraction):
            Output only. A signal that indicates the
            interaction with the Dialogflow agent has ended.
            This message is generated by Dialogflow only
            when the conversation reaches ``END_SESSION``
            page. It is not supposed to be defined by the
            user.

            It's guaranteed that there is at most one such
            message in each response.

            This field is a member of `oneof`_ ``message``.
        play_audio (google.cloud.dialogflowcx_v3.types.ResponseMessage.PlayAudio):
            Signal that the client should play an audio clip
            hosted at a client-specific URI. Dialogflow uses
            this to construct `mixed_audio
            <google.cloud.dialogflow.cx.v3.ResponseMessage.mixed_audio>`__.
            However, Dialogflow itself does not try to read
            or process the URI in any way.

            This field is a member of `oneof`_ ``message``.
        mixed_audio (google.cloud.dialogflowcx_v3.types.ResponseMessage.MixedAudio):
            Output only. An audio response message composed
            of both the synthesized Dialogflow agent
            responses and responses defined via `play_audio
            <google.cloud.dialogflow.cx.v3.ResponseMessage.play_audio>`__.
            This message is generated by Dialogflow only and
        ...
        channel (str):
            ...
            with. Clients can specify the channel via
            `QueryParameters.channel
            <google.cloud.dialogflow.cx.v3.QueryParameters.channel>`__,
            and only associated channel response will be
            returned.
    """
    ...
    class ResponseType(proto.Enum):
        ...
        Values:
            ...
            ENTRY_PROMPT (1):
                ...
                prompt
                <google.cloud.dialogflow.cx.v3.Page.entry_fulfillment>`__
                in the page.
            PARAMETER_PROMPT (2):
                The response is from `form-filling
                prompt
                <google.cloud.dialogflow.cx.v3.Form.Parameter.fill_behavior>`__
                in the page.
            HANDLER_PROMPT (3):
                The response is from a `transition
                route
                <google.cloud.dialogflow.cx.v3.TransitionRoute>`__
                or an `event handler <EventHandler>`__ in the
    ...
    class LiveAgentHandoff(proto.Message):
        ...
        handed off to a human agent for measurement purposes. What else
        to do with this signal is up to you and your handoff procedures.

        You may set this, for example:

        * In the
        `entry_fulfillment
        <google.cloud.dialogflow.cx.v3.Page.entry_fulfillment>`__ of a
        `Page <google.cloud.dialogflow.cx.v3.Page>`__ if   entering the
        page indicates something went extremely wrong in the
        conversation.

        * In a webhook response when you determine that the customer
    ...
    class ConversationSuccess(proto.Message):
        r"""Indicates that the conversation succeeded, i.e., the bot handled
        the issue that the customer talked to it about.

        Dialogflow only uses this to determine which conversations
        should be counted as successful and doesn't process the metadata
        in this message in any way. Note that Dialogflow also considers
        conversations that get to the conversation end page as
        successful even if they don't return `ConversationSuccess
        <google.cloud.dialogflow.cx.v3.ResponseMessage.ConversationSuccess>`__.
    ...
    class OutputAudioText(proto.Message):
        ...
        Attributes:
            ...
            ssml (str):
                The SSML text to be synthesized. For more
                information, see `SSML
                </speech/text-to-speech/docs/ssml>`__.
    ...
    class TelephonyTransferCall(proto.Message):
        ...
        Attributes:
            phone_number (str):
                ...
                in `E.164 format
                <https://en.wikipedia.org/wiki/E.164>`__.
