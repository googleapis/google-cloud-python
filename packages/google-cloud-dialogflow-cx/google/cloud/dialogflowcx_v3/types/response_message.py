...
class ResponseMessage(proto.Message):
    ...
    Attributes:
        text (~.response_message.ResponseMessage.Text):
            Returns a text response.
        payload (~.struct.Struct):
            Returns a response containing a custom,
            platform-specific payload.
        conversation_success (~.response_message.ResponseMessage.ConversationSuccess):
            Indicates that the conversation succeeded.
        output_audio_text (~.response_message.ResponseMessage.OutputAudioText):
            A text or ssml response that is
            preferentially used for TTS output audio
            synthesis, as described in the comment on the
            ResponseMessage message.
        live_agent_handoff (~.response_message.ResponseMessage.LiveAgentHandoff):
            Hands off conversation to a human agent.
        end_interaction (~.response_message.ResponseMessage.EndInteraction):
            ...
            ``END_SESSION`` or ``END_PAGE`` page. It is not supposed to
            be defined by the user. It's guaranteed that there is at
            most one such message in each response.
        play_audio (~.response_message.ResponseMessage.PlayAudio):
            ...
            the URI in any way.
        mixed_audio (~.response_message.ResponseMessage.MixedAudio):
    ...
    class LiveAgentHandoff(proto.Message):
        ...
        metadata = proto.Field(proto.MESSAGE, number=1, message=struct.Struct,)
    ...
    class MixedAudio(proto.Message):
        ...
        Attributes:
            segments (Sequence[~.response_message.ResponseMessage.MixedAudio.Segment]):
    ...
    payload = proto.Field(
        proto.MESSAGE, number=2, oneof="message", message=struct.Struct,
    )
