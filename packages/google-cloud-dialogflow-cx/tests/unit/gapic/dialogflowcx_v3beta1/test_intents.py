...
from google.cloud.dialogflowcx_v3beta1.services.intents import IntentsAsyncClient, IntentsClient, pagers, transports
...
def test_list_intents_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v3beta1/{parent=projects/*/locations/*/agents/*}/intents" % client.transport._host, args[1])
...
def test_get_intent_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{name=projects/*/locations/*/agents/*/intents/*}" % client.transport._host, args[1])
...
def test_update_intent_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{intent.name=projects/*/locations/*/agents/*/intents/*}" % client.transport._host, args[1])
