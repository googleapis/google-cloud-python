...
from google.cloud.dialogflowcx_v3beta1.services.pages import PagesAsyncClient, PagesClient, pagers, transports
from google.cloud.dialogflowcx_v3beta1.types import advanced_settings, data_store_connection, fulfillment, gcs
...
def test__get_default_mtls_endpoint():
    ...
    assert PagesClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert PagesClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert PagesClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__get_client_cert_source():
    ...
    assert PagesClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert PagesClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert PagesClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(PagesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PagesClient))
@mock.patch.object(PagesAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PagesAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = PagesClient._DEFAULT_UNIVERSE
    default_endpoint = PagesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = PagesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert PagesClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert PagesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == PagesClient.DEFAULT_MTLS_ENDPOINT
    assert PagesClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert PagesClient._get_api_endpoint(None, None, default_universe, "always") == PagesClient.DEFAULT_MTLS_ENDPOINT
    assert PagesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == PagesClient.DEFAULT_MTLS_ENDPOINT
    assert PagesClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert PagesClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        PagesClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert PagesClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert PagesClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (PagesAsyncClient, transports.PagesGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (PagesClient, transports.PagesGrpcTransport, "grpc", "false"),
        (PagesAsyncClient, transports.PagesGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_pages_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(PagesClient, "DEFAULT_ENDPOINT", modify_default_endpoint(PagesClient))
@mock.patch.object(PagesAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(PagesAsyncClient))
...
def test_pages_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (PagesAsyncClient, transports.PagesGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (PagesClient, transports.PagesRestTransport, "rest", None),
    ],
)
def test_pages_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_pages_client_client_options_from_dict():
    with mock.patch("google.cloud.dialogflowcx_v3beta1.services.pages.transports.PagesGrpcTransport.__init__") as grpc_transport:
...
def test_pages_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
async def test_list_pages_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_pages in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_pages] = mock_rpc
...
async def test_list_pages_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_pages), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(page.ListPagesResponse())
        await client.list_pages(request)
...
async def test_list_pages_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_pages), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            page.ListPagesResponse(
                pages=[
...
async def test_list_pages_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_pages), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_pages(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
async def test_get_page_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.get_page in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_page] = mock_rpc
...
@pytest.mark.asyncio
async def test_create_page_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = PagesAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_page in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_page] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_page_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = PagesAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_page in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_page] = mock_rpc
...
@pytest.mark.asyncio
async def test_delete_page_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = PagesAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_page in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_page] = mock_rpc
...
def test_list_pages_rest_required_fields(request_type=page.ListPagesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_pages._get_unset_required_fields(jsonified_request)
...
def test_list_pages_rest_unset_required_fields():
    transport = transports.PagesRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_pages_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v3beta1/{parent=projects/*/locations/*/agents/*/flows/*}/pages" % client.transport._host, args[1])
...
def test_get_page_rest_required_fields(request_type=page.GetPageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_page._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_page_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/agents/sample3/flows/sample4/pages/sample5"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{name=projects/*/locations/*/agents/*/flows/*/pages/*}" % client.transport._host, args[1])
...
def test_create_page_rest_required_fields(request_type=gcdc_page.CreatePageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_page._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_page_rest_required_fields(request_type=gcdc_page.UpdatePageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_page._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_page_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"page": {"name": "projects/sample1/locations/sample2/agents/sample3/flows/sample4/pages/sample5"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{page.name=projects/*/locations/*/agents/*/flows/*/pages/*}" % client.transport._host, args[1])
...
def test_delete_page_rest_required_fields(request_type=page.DeletePageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_page._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = PagesClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = PagesClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = PagesClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = PagesAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = PagesAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = PagesClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_pages_rest_bad_request(request_type=page.ListPagesRequest):
    client = PagesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_pages_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.PagesRestInterceptor, "post_list_pages") as post, mock.patch.object(
...
def test_get_page_rest_bad_request(request_type=page.GetPageRequest):
    client = PagesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/agents/sample3/flows/sample4/pages/sample5"}
    request = request_type(**request_init)
...
def test_get_page_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PagesRestInterceptor, "post_get_page") as post, mock.patch.object(
...
def test_create_page_rest_call_success(request_type):
    ...
    request_init["page"] = {
            ...
            "messages": [
                {
                    ...
                    "knowledge_info_card": {},
                    "tool_call": {"tool": "tool_value", "action": "action_value", "input_parameters": {}},
        ...
        "transition_route_groups": ["transition_route_groups_value1", "transition_route_groups_value2"],
        "transition_routes": [
            {
                "name": "name_value",
                "description": "description_value",
        ...
        "knowledge_connector_settings": {
            ...
            "target_page": "target_page_value",
            "target_flow": "target_flow_value",
            "data_store_connections": [{"data_store_type": 1, "data_store": "data_store_value", "document_processing_mode": 1}],
        },
    }
...
def test_create_page_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PagesRestInterceptor, "post_create_page") as post, mock.patch.object(
...
def test_update_page_rest_bad_request(request_type=gcdc_page.UpdatePageRequest):
    client = PagesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"page": {"name": "projects/sample1/locations/sample2/agents/sample3/flows/sample4/pages/sample5"}}
    request = request_type(**request_init)
...
def test_update_page_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PagesRestInterceptor, "post_update_page") as post, mock.patch.object(
...
def test_delete_page_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PagesRestInterceptor, "pre_delete_page") as pre:
        pre.assert_not_called()
...
def test_pages_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.PagesTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_pages_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.dialogflowcx_v3beta1.services.pages.transports.PagesTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.PagesGrpcTransport, grpc_helpers), (transports.PagesGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.PagesGrpcTransport, transports.PagesGrpcAsyncIOTransport])
...
def test_pages_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.PagesRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_tool_path():
    ...
    agent = "oyster"
    tool = "nudibranch"
    expected = "projects/{project}/locations/{location}/agents/{agent}/tools/{tool}".format(
        project=project,
        location=location,
        agent=agent,
        tool=tool,
...
def test_transition_route_group_path():
    ...
    actual = PagesClient.transition_route_group_path(project, location, agent, flow, transition_route_group)
    assert expected == actual
...
def test_client_ctx():
    ...
    for transport in transports:
        client = PagesClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
