...
from google.cloud.dialogflowcx_v3beta1.services.agents import AgentsAsyncClient, AgentsClient, pagers, transports
from google.cloud.dialogflowcx_v3beta1.types import audio_config, bigquery_export, flow, gcs
...
def test__get_client_cert_source():
    ...
    assert AgentsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert AgentsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert AgentsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert AgentsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(AgentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AgentsClient))
@mock.patch.object(AgentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AgentsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = AgentsClient._DEFAULT_UNIVERSE
    default_endpoint = AgentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = AgentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert AgentsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert AgentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == AgentsClient.DEFAULT_MTLS_ENDPOINT
    assert AgentsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert AgentsClient._get_api_endpoint(None, None, default_universe, "always") == AgentsClient.DEFAULT_MTLS_ENDPOINT
    assert AgentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == AgentsClient.DEFAULT_MTLS_ENDPOINT
    assert AgentsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert AgentsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        AgentsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert AgentsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert AgentsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert AgentsClient._get_universe_domain(None, None) == AgentsClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (AgentsAsyncClient, transports.AgentsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (AgentsClient, transports.AgentsRestTransport, "rest", None),
    ],
)
def test_agents_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_agents_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
@pytest.mark.asyncio
async def test_list_agents_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_agents in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_agents] = mock_rpc
...
async def test_list_agents_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_agents), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            agent.ListAgentsResponse(
                agents=[
...
async def test_list_agents_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_agents), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_agents(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
async def test_get_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.get_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_agent] = mock_rpc
...
def test_create_agent_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_agent in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_agent] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_agent] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_agent] = mock_rpc
...
@pytest.mark.asyncio
async def test_delete_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_agent] = mock_rpc
...
@pytest.mark.asyncio
async def test_export_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.export_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.export_agent] = mock_rpc
...
@pytest.mark.asyncio
async def test_restore_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.restore_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.restore_agent] = mock_rpc
...
@pytest.mark.asyncio
async def test_validate_agent_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.validate_agent in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.validate_agent] = mock_rpc
...
def test_get_agent_validation_result_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_agent_validation_result in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_agent_validation_result] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_agent_validation_result_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_agent_validation_result in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_agent_validation_result] = mock_rpc
...
def test_get_generative_settings_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_generative_settings in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_generative_settings] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_generative_settings_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_generative_settings in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_generative_settings] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_generative_settings_async(transport: str = "grpc_asyncio", request_type=agent.GetGenerativeSettingsRequest):
    client = AgentsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_get_generative_settings_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_generative_settings), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(generative_settings.GenerativeSettings())
        await client.get_generative_settings(request)
...
def test_update_generative_settings_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_generative_settings in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_generative_settings] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_generative_settings_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AgentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_generative_settings in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_generative_settings] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_generative_settings_async(transport: str = "grpc_asyncio", request_type=agent.UpdateGenerativeSettingsRequest):
    client = AgentsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_update_generative_settings_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.update_generative_settings), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(gcdc_generative_settings.GenerativeSettings())
        await client.update_generative_settings(request)
...
def test_update_generative_settings_flattened():
    ...
    with mock.patch.object(type(client.transport.update_generative_settings), "__call__") as call:
        ...
        client.update_generative_settings(
            generative_settings=gcdc_generative_settings.GenerativeSettings(name="name_value"),
            update_mask=field_mask_pb2.FieldMask(paths=["paths_value"]),
        )
...
def test_list_agents_rest_required_fields(request_type=agent.ListAgentsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_agents._get_unset_required_fields(jsonified_request)
...
def test_list_agents_rest_unset_required_fields():
    transport = transports.AgentsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_agents_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v3beta1/{parent=projects/*/locations/*}/agents" % client.transport._host, args[1])
...
def test_get_agent_rest_required_fields(request_type=agent.GetAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_agent_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{name=projects/*/locations/*/agents/*}" % client.transport._host, args[1])
...
def test_create_agent_rest_required_fields(request_type=gcdc_agent.CreateAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_agent_rest_required_fields(request_type=gcdc_agent.UpdateAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_agent_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"agent": {"name": "projects/sample1/locations/sample2/agents/sample3"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{agent.name=projects/*/locations/*/agents/*}" % client.transport._host, args[1])
...
def test_delete_agent_rest_required_fields(request_type=agent.DeleteAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_export_agent_rest_required_fields(request_type=agent.ExportAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).export_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_restore_agent_rest_required_fields(request_type=agent.RestoreAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).restore_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_validate_agent_rest_required_fields(request_type=agent.ValidateAgentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).validate_agent._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_agent_validation_result_rest_required_fields(request_type=agent.GetAgentValidationResultRequest):
    transport_class = transports.AgentsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_agent_validation_result._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_get_agent_validation_result_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/agents/sample3/validationResult"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{name=projects/*/locations/*/agents/*/validationResult}" % client.transport._host, args[1])
...
def test_get_generative_settings_rest_required_fields(request_type=agent.GetGenerativeSettingsRequest):
    transport_class = transports.AgentsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_generative_settings._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_get_generative_settings_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/agents/sample3/generativeSettings"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{name=projects/*/locations/*/agents/*/generativeSettings}" % client.transport._host, args[1])
...
def test_update_generative_settings_rest_required_fields(request_type=agent.UpdateGenerativeSettingsRequest):
    transport_class = transports.AgentsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_generative_settings._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_update_generative_settings_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"generative_settings": {"name": "projects/sample1/locations/sample2/agents/sample3/generativeSettings"}}
        ...
        assert path_template.validate(
            "%s/v3beta1/{generative_settings.name=projects/*/locations/*/agents/*/generativeSettings}" % client.transport._host, args[1]
        )
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = AgentsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = AgentsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = AgentsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = AgentsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = AgentsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_agents_rest_bad_request(request_type=agent.ListAgentsRequest):
    client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_agents_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_list_agents") as post, mock.patch.object(
...
def test_get_agent_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_get_agent") as post, mock.patch.object(
...
def test_create_agent_rest_call_success(request_type):
    ...
    request_init["agent"] = {
        ...
        "advanced_settings": {
            ...
            "logging_settings": {"enable_stackdriver_logging": True, "enable_interaction_logging": True, "enable_consent_based_redaction": True},
        },
        ...
        "bigquery_export_settings": {"enabled": True, "bigquery_table": "bigquery_table_value"},
...
def test_create_agent_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_create_agent") as post, mock.patch.object(
...
def test_update_agent_rest_bad_request(request_type=gcdc_agent.UpdateAgentRequest):
    client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"agent": {"name": "projects/sample1/locations/sample2/agents/sample3"}}
    request = request_type(**request_init)
...
def test_update_agent_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_update_agent") as post, mock.patch.object(
...
def test_delete_agent_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "pre_delete_agent") as pre:
        pre.assert_not_called()
...
def test_validate_agent_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_validate_agent") as post, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = agent.AgentValidationResult.to_json(agent.AgentValidationResult())
        req.return_value.content = return_value
...
def test_get_agent_validation_result_rest_bad_request(request_type=agent.GetAgentValidationResultRequest):
    client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/agents/sample3/validationResult"}
    request = request_type(**request_init)
...
def test_get_agent_validation_result_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_get_agent_validation_result") as post, mock.patch.object(
        transports.AgentsRestInterceptor, "post_get_agent_validation_result_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = agent.GetAgentValidationResultRequest.pb(agent.GetAgentValidationResultRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_get_generative_settings_rest_bad_request(request_type=agent.GetGenerativeSettingsRequest):
    client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/agents/sample3/generativeSettings"}
    request = request_type(**request_init)
...
def test_get_generative_settings_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_get_generative_settings") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = agent.GetGenerativeSettingsRequest.pb(agent.GetGenerativeSettingsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = generative_settings.GenerativeSettings.to_json(generative_settings.GenerativeSettings())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = generative_settings.GenerativeSettings(), metadata
...
def test_update_generative_settings_rest_bad_request(request_type=agent.UpdateGenerativeSettingsRequest):
    client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"generative_settings": {"name": "projects/sample1/locations/sample2/agents/sample3/generativeSettings"}}
    request = request_type(**request_init)
...
def test_update_generative_settings_rest_call_success(request_type):
    ...
    request_init["generative_settings"] = {
        ...
        "fallback_settings": {
            ...
            "prompt_templates": [{"display_name": "display_name_value", "prompt_text": "prompt_text_value", "frozen": True}],
        },
        "generative_safety_settings": {
            "default_banned_phrase_match_strategy": 1,
            "banned_phrases": [{"text": "text_value", "language_code": "language_code_value"}],
        ...
        "llm_model_settings": {
            ...
            "parameters": {"temperature": 0.1198, "input_token_limit": 1, "output_token_limit": 1},
        },
    }
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = agent.UpdateGenerativeSettingsRequest.meta.fields["generative_settings"]
...
def test_update_generative_settings_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_update_generative_settings") as post, mock.patch.object(
        transports.AgentsRestInterceptor, "post_update_generative_settings_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = agent.UpdateGenerativeSettingsRequest.pb(agent.UpdateGenerativeSettingsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcdc_generative_settings.GenerativeSettings.to_json(gcdc_generative_settings.GenerativeSettings())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = gcdc_generative_settings.GenerativeSettings(), metadata
...
def test_agents_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.AgentsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.AgentsGrpcTransport, grpc_helpers), (transports.AgentsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_agents_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.AgentsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_ctx():
    ...
    for transport in transports:
        client = AgentsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
