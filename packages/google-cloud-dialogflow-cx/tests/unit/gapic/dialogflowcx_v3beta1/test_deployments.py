...
from google.cloud.dialogflowcx_v3beta1.services.deployments import DeploymentsAsyncClient, DeploymentsClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert DeploymentsClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert DeploymentsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert DeploymentsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert DeploymentsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert DeploymentsClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi


def test__read_environment_variables():
    assert DeploymentsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert DeploymentsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                DeploymentsClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert DeploymentsClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert DeploymentsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert DeploymentsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert DeploymentsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert DeploymentsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert DeploymentsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                DeploymentsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert DeploymentsClient._get_client_cert_source(None, False) is None
    assert DeploymentsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert DeploymentsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert DeploymentsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert DeploymentsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(DeploymentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsClient))
@mock.patch.object(DeploymentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DeploymentsClient._DEFAULT_UNIVERSE
    default_endpoint = DeploymentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = DeploymentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert DeploymentsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert DeploymentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DeploymentsClient.DEFAULT_MTLS_ENDPOINT
    assert DeploymentsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert DeploymentsClient._get_api_endpoint(None, None, default_universe, "always") == DeploymentsClient.DEFAULT_MTLS_ENDPOINT
    assert DeploymentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == DeploymentsClient.DEFAULT_MTLS_ENDPOINT
    assert DeploymentsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert DeploymentsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        DeploymentsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert DeploymentsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert DeploymentsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert DeploymentsClient._get_universe_domain(None, None) == DeploymentsClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        DeploymentsClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = DeploymentsClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (transports.DeploymentsRestTransport, "rest"),
    ],
)
def test_deployments_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DeploymentsAsyncClient, transports.DeploymentsGrpcAsyncIOTransport, "grpc_asyncio"),
        (DeploymentsClient, transports.DeploymentsRestTransport, "rest"),
    ],
)
@mock.patch.object(DeploymentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsClient))
@mock.patch.object(DeploymentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsAsyncClient))
def test_deployments_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DeploymentsAsyncClient, transports.DeploymentsGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (DeploymentsClient, transports.DeploymentsGrpcTransport, "grpc", "false"),
        (DeploymentsAsyncClient, transports.DeploymentsGrpcAsyncIOTransport, "grpc_asyncio", "false"),
        (DeploymentsClient, transports.DeploymentsRestTransport, "rest", "true"),
        (DeploymentsClient, transports.DeploymentsRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(DeploymentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsClient))
@mock.patch.object(DeploymentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsAsyncClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_deployments_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(DeploymentsClient, "DEFAULT_ENDPOINT", modify_default_endpoint(DeploymentsClient))
@mock.patch.object(DeploymentsAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(DeploymentsAsyncClient))
...
@mock.patch.object(DeploymentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsClient))
@mock.patch.object(DeploymentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DeploymentsAsyncClient))
def test_deployments_client_client_api_endpoint(client_class):
...
def test_deployments_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DeploymentsAsyncClient, transports.DeploymentsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (DeploymentsClient, transports.DeploymentsRestTransport, "rest", None),
    ],
)
def test_deployments_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_deployments_client_client_options_from_dict():
    with mock.patch("google.cloud.dialogflowcx_v3beta1.services.deployments.transports.DeploymentsGrpcTransport.__init__") as grpc_transport:
...
def test_deployments_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
@pytest.mark.asyncio
async def test_list_deployments_async(transport: str = "grpc_asyncio", request_type=deployment.ListDeploymentsRequest):
    client = DeploymentsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_list_deployments_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_deployments), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(deployment.ListDeploymentsResponse())
        await client.list_deployments(request)
...
@pytest.mark.asyncio
async def test_get_deployment_async(transport: str = "grpc_asyncio", request_type=deployment.GetDeploymentRequest):
    client = DeploymentsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_get_deployment_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_deployment), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(deployment.Deployment())
        await client.get_deployment(request)
...
def test_list_deployments_rest_required_fields(request_type=deployment.ListDeploymentsRequest):
...
def test_list_deployments_rest_unset_required_fields():
    transport = transports.DeploymentsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_deployments_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v3beta1/{parent=projects/*/locations/*/agents/*/environments/*}/deployments" % client.transport._host, args[1]
        )
...
def test_list_deployments_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(deployment.ListDeploymentsResponse.to_json(x) for x in response)
...
def test_get_deployment_rest_required_fields(request_type=deployment.GetDeploymentRequest):
    transport_class = transports.DeploymentsRestTransport
...
def test_get_deployment_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/agents/sample3/environments/sample4/deployments/sample5"}
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v3beta1/{name=projects/*/locations/*/agents/*/environments/*/deployments/*}" % client.transport._host, args[1]
        )
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = DeploymentsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = DeploymentsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = DeploymentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = DeploymentsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = DeploymentsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = DeploymentsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_deployments_rest_bad_request(request_type=deployment.ListDeploymentsRequest):
    client = DeploymentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_deployments_rest_interceptors(null_interceptor):
    transport = transports.DeploymentsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.DeploymentsRestInterceptor(),
    )
    client = DeploymentsClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DeploymentsRestInterceptor, "post_list_deployments") as post, mock.patch.object(
        transports.DeploymentsRestInterceptor, "post_list_deployments_with_metadata"
        ...
        pb_message = deployment.ListDeploymentsRequest.pb(deployment.ListDeploymentsRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = deployment.ListDeploymentsResponse.to_json(deployment.ListDeploymentsResponse())
        ...
        post_with_metadata.return_value = deployment.ListDeploymentsResponse(), metadata
...
def test_get_deployment_rest_bad_request(request_type=deployment.GetDeploymentRequest):
    client = DeploymentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/agents/sample3/environments/sample4/deployments/sample5"}
    request = request_type(**request_init)
...
def test_get_deployment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DeploymentsRestInterceptor, "post_get_deployment") as post, mock.patch.object(
        transports.DeploymentsRestInterceptor, "post_get_deployment_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = deployment.GetDeploymentRequest.pb(deployment.GetDeploymentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        post_with_metadata.return_value = deployment.Deployment(), metadata
...
def test_deployments_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.DeploymentsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_deployments_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.dialogflowcx_v3beta1.services.deployments.transports.DeploymentsTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.DeploymentsGrpcTransport, grpc_helpers), (transports.DeploymentsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.DeploymentsGrpcTransport, transports.DeploymentsGrpcAsyncIOTransport])
...
def test_deployments_http_transport_client_cert_source_for_mtls():
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.DeploymentsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_deployments_client_transport_session_collision(transport_name):
    creds1 = ga_credentials.AnonymousCredentials()
    creds2 = ga_credentials.AnonymousCredentials()
    client1 = DeploymentsClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = DeploymentsClient(
        credentials=creds2,
        transport=transport_name,
    )
...
def test_deployment_path():
    ...
    actual = DeploymentsClient.deployment_path(project, location, agent, environment, deployment)
...
def test_experiment_path():
    ...
    actual = DeploymentsClient.experiment_path(project, location, agent, environment, experiment)
    assert expected == actual
...
def test_test_case_result_path():
    ...
    actual = DeploymentsClient.test_case_result_path(project, location, agent, test_case, result)
    assert expected == actual
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.DeploymentsTransport, "_prep_wrapped_messages") as prep:
        client = DeploymentsClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = DeploymentsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
