...
def test__get_default_mtls_endpoint():
    ...
    assert (
        DeploymentsClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        DeploymentsClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        DeploymentsClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                DeploymentsClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(DeploymentsClient),
)
@mock.patch.object(
    DeploymentsAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(DeploymentsAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DeploymentsClient._DEFAULT_UNIVERSE
    default_endpoint = DeploymentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = DeploymentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        DeploymentsClient._get_api_endpoint(
    ...
    assert (
        ...
        == DeploymentsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        DeploymentsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        DeploymentsClient._get_api_endpoint(None, None, default_universe, "always")
        == DeploymentsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        DeploymentsClient._get_api_endpoint(
    ...
    assert (
        DeploymentsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        DeploymentsClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        DeploymentsClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        DeploymentsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        DeploymentsClient._get_universe_domain(None, None)
        == DeploymentsClient._DEFAULT_UNIVERSE
    )
...
def test_list_deployments_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v3beta1/{parent=projects/*/locations/*/agents/*/environments/*}/deployments"
...
def test_list_deployments_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(
            deployment.ListDeploymentsResponse.to_json(x) for x in response
        )
...
def test_get_deployment_rest_required_fields(
    request_type=deployment.GetDeploymentRequest,
):
    transport_class = transports.DeploymentsRestTransport
...
def test_get_deployment_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v3beta1/{name=projects/*/locations/*/agents/*/environments/*/deployments/*}"
            % client.transport._host,
            args[1],
        )
