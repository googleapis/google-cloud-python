...
from google.cloud.dialogflowcx_v3beta1.services.deployments.transports.base import (
...
@requires_google_auth_lt_1_25_0
def test_deployments_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        DeploymentsClient()
...
@requires_google_auth_lt_1_25_0
def test_deployments_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
...
def test_deployments_host_no_port():
    client = DeploymentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_deployments_host_with_port():
    client = DeploymentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(
