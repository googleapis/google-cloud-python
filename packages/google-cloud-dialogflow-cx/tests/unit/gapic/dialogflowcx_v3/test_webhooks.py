...
from google.cloud.dialogflowcx_v3.services.webhooks import WebhooksAsyncClient, WebhooksClient, pagers, transports
...
def test__read_environment_variables():
    assert WebhooksClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert WebhooksClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                WebhooksClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert WebhooksClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert WebhooksClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert WebhooksClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert WebhooksClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert WebhooksClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert WebhooksClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                WebhooksClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert WebhooksClient._get_client_cert_source(None, False) is None
    assert WebhooksClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert WebhooksClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert WebhooksClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert WebhooksClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(WebhooksClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksClient))
@mock.patch.object(WebhooksAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = WebhooksClient._DEFAULT_UNIVERSE
    default_endpoint = WebhooksClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = WebhooksClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert WebhooksClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert WebhooksClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == WebhooksClient.DEFAULT_MTLS_ENDPOINT
    assert WebhooksClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert WebhooksClient._get_api_endpoint(None, None, default_universe, "always") == WebhooksClient.DEFAULT_MTLS_ENDPOINT
    assert WebhooksClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == WebhooksClient.DEFAULT_MTLS_ENDPOINT
    assert WebhooksClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert WebhooksClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        WebhooksClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert WebhooksClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert WebhooksClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert WebhooksClient._get_universe_domain(None, None) == WebhooksClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize(
    ...
    [
        (WebhooksClient, "grpc"),
        (WebhooksAsyncClient, "grpc_asyncio"),
        (WebhooksClient, "rest"),
    ],
)
def test_webhooks_client_from_service_account_info(client_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (transports.WebhooksRestTransport, "rest"),
    ],
)
def test_webhooks_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_webhooks_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WebhooksClient, transports.WebhooksRestTransport, "rest"),
    ],
)
@mock.patch.object(WebhooksClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksClient))
@mock.patch.object(WebhooksAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksAsyncClient))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WebhooksClient, transports.WebhooksRestTransport, "rest", "true"),
        (WebhooksClient, transports.WebhooksRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(WebhooksClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksClient))
@mock.patch.object(WebhooksAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksAsyncClient))
...
@mock.patch.object(WebhooksClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksClient))
@mock.patch.object(WebhooksAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebhooksAsyncClient))
def test_webhooks_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WebhooksAsyncClient, transports.WebhooksGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (WebhooksClient, transports.WebhooksRestTransport, "rest", None),
    ],
)
def test_webhooks_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_webhooks_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_list_webhooks_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[client._transport.list_webhooks] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_webhooks_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_webhooks in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_webhooks] = mock_rpc
...
async def test_list_webhooks_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_webhooks), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            webhook.ListWebhooksResponse(
                webhooks=[
...
async def test_list_webhooks_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_webhooks), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_webhooks(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_webhook_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_webhook in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_webhook] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_webhook_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebhooksAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_webhook in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_webhook] = mock_rpc
...
@pytest.mark.asyncio
async def test_create_webhook_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebhooksAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_webhook in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_webhook] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_webhook_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebhooksAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_webhook in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_webhook] = mock_rpc
...
@pytest.mark.asyncio
async def test_delete_webhook_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebhooksAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_webhook in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_webhook] = mock_rpc
...
def test_list_webhooks_rest_required_fields(request_type=webhook.ListWebhooksRequest):
    transport_class = transports.WebhooksRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_webhooks._get_unset_required_fields(jsonified_request)
...
def test_list_webhooks_rest_unset_required_fields():
    transport = transports.WebhooksRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.list_webhooks._get_unset_required_fields({})
...
def test_list_webhooks_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.list_webhooks(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3/{parent=projects/*/locations/*/agents/*}/webhooks" % client.transport._host, args[1])


def test_list_webhooks_rest_flattened_error(transport: str = "rest"):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_webhooks_rest_pager(transport: str = "rest"):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(webhook.ListWebhooksResponse.to_json(x) for x in response)
        ...
        pages = list(client.list_webhooks(request=sample_request).pages)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_webhook_rest_required_fields(request_type=webhook.GetWebhookRequest):
    transport_class = transports.WebhooksRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_webhook._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_webhook_rest_unset_required_fields():
    ...
    unset_fields = transport.get_webhook._get_unset_required_fields({})
...
def test_get_webhook_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/agents/sample3/webhooks/sample4"}
        ...
        client.get_webhook(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3/{name=projects/*/locations/*/agents/*/webhooks/*}" % client.transport._host, args[1])


def test_get_webhook_rest_flattened_error(transport: str = "rest"):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_webhook_rest_required_fields(request_type=gcdc_webhook.CreateWebhookRequest):
    transport_class = transports.WebhooksRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_webhook._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_create_webhook_rest_unset_required_fields():
    ...
    unset_fields = transport.create_webhook._get_unset_required_fields({})
    assert set(unset_fields) == (
        set(())
        & set(
            (
...
def test_create_webhook_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.create_webhook(**mock_args)
...
def test_create_webhook_rest_flattened_error(transport: str = "rest"):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_webhook_rest_required_fields(request_type=gcdc_webhook.UpdateWebhookRequest):
    transport_class = transports.WebhooksRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_webhook._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_webhook_rest_unset_required_fields():
    ...
    unset_fields = transport.update_webhook._get_unset_required_fields({})
...
def test_update_webhook_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"webhook": {"name": "projects/sample1/locations/sample2/agents/sample3/webhooks/sample4"}}
        ...
        client.update_webhook(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3/{webhook.name=projects/*/locations/*/agents/*/webhooks/*}" % client.transport._host, args[1])


def test_update_webhook_rest_flattened_error(transport: str = "rest"):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_webhook_rest_required_fields(request_type=webhook.DeleteWebhookRequest):
    transport_class = transports.WebhooksRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_webhook._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_delete_webhook_rest_unset_required_fields():
    ...
    unset_fields = transport.delete_webhook._get_unset_required_fields({})
...
def test_delete_webhook_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.delete_webhook(**mock_args)
...
def test_delete_webhook_rest_flattened_error(transport: str = "rest"):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = WebhooksClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = WebhooksClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = WebhooksClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = WebhooksAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = WebhooksAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = WebhooksClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_webhooks_rest_bad_request(request_type=webhook.ListWebhooksRequest):
    client = WebhooksClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_webhooks_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.WebhooksRestInterceptor, "post_list_webhooks") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = webhook.ListWebhooksResponse.to_json(webhook.ListWebhooksResponse())
...
def test_get_webhook_rest_bad_request(request_type=webhook.GetWebhookRequest):
    client = WebhooksClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/agents/sample3/webhooks/sample4"}
    request = request_type(**request_init)
...
def test_get_webhook_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebhooksRestInterceptor, "post_get_webhook") as post, mock.patch.object(
...
def test_create_webhook_rest_bad_request(request_type=gcdc_webhook.CreateWebhookRequest):
    client = WebhooksClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2/agents/sample3"}
    request = request_type(**request_init)
...
def test_create_webhook_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebhooksRestInterceptor, "post_create_webhook") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcdc_webhook.CreateWebhookRequest.pb(gcdc_webhook.CreateWebhookRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_webhook_rest_bad_request(request_type=gcdc_webhook.UpdateWebhookRequest):
    client = WebhooksClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"webhook": {"name": "projects/sample1/locations/sample2/agents/sample3/webhooks/sample4"}}
    request = request_type(**request_init)
...
def test_update_webhook_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebhooksRestInterceptor, "post_update_webhook") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcdc_webhook.UpdateWebhookRequest.pb(gcdc_webhook.UpdateWebhookRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_webhook_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebhooksRestInterceptor, "pre_delete_webhook") as pre:
        pre.assert_not_called()
...
def test_webhooks_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.WebhooksTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.WebhooksGrpcTransport, grpc_helpers), (transports.WebhooksGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_webhooks_http_transport_client_cert_source_for_mtls():
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.WebhooksRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_webhooks_host_no_port(transport_name):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_webhooks_host_with_port(transport_name):
    client = WebhooksClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_webhooks_client_transport_session_collision(transport_name):
    ...
    client2 = WebhooksClient(
        credentials=creds2,
        transport=transport_name,
    )
    session1 = client1.transport.list_webhooks._session
    session2 = client2.transport.list_webhooks._session
    assert session1 != session2
    session1 = client1.transport.get_webhook._session
    session2 = client2.transport.get_webhook._session
    assert session1 != session2
    session1 = client1.transport.create_webhook._session
    session2 = client2.transport.create_webhook._session
    assert session1 != session2
    session1 = client1.transport.update_webhook._session
    session2 = client2.transport.update_webhook._session
    assert session1 != session2
    session1 = client1.transport.delete_webhook._session
    session2 = client2.transport.delete_webhook._session
    assert session1 != session2
...
def test_client_ctx():
    ...
    for transport in transports:
        client = WebhooksClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
