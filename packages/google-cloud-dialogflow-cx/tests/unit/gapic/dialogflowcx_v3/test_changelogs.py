...
from google.cloud.dialogflowcx_v3.services.changelogs import ChangelogsAsyncClient, ChangelogsClient, pagers, transports
...
def test_changelogs_client_client_options_from_dict():
    with mock.patch("google.cloud.dialogflowcx_v3.services.changelogs.transports.ChangelogsGrpcTransport.__init__") as grpc_transport:
...
async def test_list_changelogs_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_changelogs), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_changelogs(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_changelogs_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v3/{parent=projects/*/locations/*/agents/*}/changelogs" % client.transport._host, args[1])
...
def test_get_changelog_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3/{name=projects/*/locations/*/agents/*/changelogs/*}" % client.transport._host, args[1])
...
def test_transport_kind_grpc():
    transport = ChangelogsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = ChangelogsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ChangelogsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ChangelogsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_list_changelogs_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.ChangelogsRestInterceptor, "post_list_changelogs") as post, mock.patch.object(
...
def test_get_changelog_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ChangelogsRestInterceptor, "post_get_changelog") as post, mock.patch.object(
...
def test_changelogs_base_transport():
    ...
    with mock.patch("google.cloud.dialogflowcx_v3.services.changelogs.transports.ChangelogsTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.ChangelogsGrpcTransport, grpc_helpers), (transports.ChangelogsGrpcAsyncIOTransport, grpc_helpers_async)],
)
