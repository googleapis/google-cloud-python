...
def test__get_default_mtls_endpoint():
    ...
    assert (
        LookupServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        LookupServiceClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        LookupServiceClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                LookupServiceClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                LookupServiceClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(LookupServiceClient),
)
@mock.patch.object(
    LookupServiceAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(LookupServiceAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = LookupServiceClient._DEFAULT_UNIVERSE
    default_endpoint = LookupServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = LookupServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        LookupServiceClient._get_api_endpoint(
    ...
    assert (
        ...
        == LookupServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        LookupServiceClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        LookupServiceClient._get_api_endpoint(None, None, default_universe, "always")
        == LookupServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        LookupServiceClient._get_api_endpoint(
    ...
    assert (
        LookupServiceClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        LookupServiceClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        LookupServiceClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        LookupServiceClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        LookupServiceClient._get_universe_domain(None, None)
        == LookupServiceClient._DEFAULT_UNIVERSE
    )
...
def test_lookup_service_client_mtls_env_auto(
    client_class, transport_class, transport_name, use_client_cert_env
):
...
def test_lookup_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.servicedirectory_v1.services.lookup_service.transports.LookupServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_lookup_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
@pytest.mark.asyncio
async def test_resolve_service_async_use_cached_wrapped_rpc(
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.resolve_service
...
def test_resolve_service_rest_required_fields(
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).resolve_service._get_unset_required_fields(jsonified_request)
...
def test_transport_kind_grpc_asyncio():
    transport = LookupServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = LookupServiceClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
    assert transport.kind == "rest"


def test_resolve_service_rest_bad_request(
    request_type=lookup_service.ResolveServiceRequest,
):
    client = LookupServiceClient(
...
def test_resolve_service_rest_interceptors(null_interceptor):
    transport = transports.LookupServiceRestTransport(
        ...
        interceptor=None
        if null_interceptor
        else transports.LookupServiceRestInterceptor(),
    )
    client = LookupServiceClient(transport=transport)
    ...
    with (
        ...
        mock.patch.object(
            transports.LookupServiceRestInterceptor, "post_resolve_service"
        ) as post,
        mock.patch.object(
            transports.LookupServiceRestInterceptor,
        ...
        pb_message = lookup_service.ResolveServiceRequest.pb(
            lookup_service.ResolveServiceRequest()
        )
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = lookup_service.ResolveServiceResponse.to_json(
            lookup_service.ResolveServiceResponse()
        )
...
def test_lookup_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.servicedirectory_v1.services.lookup_service.transports.LookupServiceTransport.__init__"
    ) as Transport:
...
def test_endpoint_path():
    ...
    actual = LookupServiceClient.endpoint_path(
