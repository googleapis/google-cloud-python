...
@pytest.mark.asyncio
async def test_resolve_service_empty_call_async():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    ...
    with mock.patch.object(type(client.transport.resolve_service), "__call__") as call:
        ...
        response = await client.resolve_service()
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        assert args[0] == lookup_service.ResolveServiceRequest()
...
def test_resolve_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        req.return_value._content = lookup_service.ResolveServiceResponse.to_json(
            lookup_service.ResolveServiceResponse()
        )
...
def test_resolve_service_rest_bad_request(
    transport: str = "rest", request_type=lookup_service.ResolveServiceRequest
):
    client = LookupServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_transport_kind(transport_name):
    transport = LookupServiceClient.get_transport_class(transport_name)(
        credentials=ga_credentials.AnonymousCredentials(),
    )
