...
class VerifyAttestationRequest(proto.Message):
    ...
    Attributes:
        ...
        gcp_credentials (google.cloud.confidentialcomputing_v1.types.GcpCredentials):
            Optional. Credentials used to populate the
            "emails" claim in the claims_token.
...
class SevSnpAttestation(proto.Message):
    ...
    Attributes:
        report (bytes):
            ...
            Format is in revision 1.55, §7.3 Attestation,
            Table 22. ATTESTATION_REPORT Structure in this
            document:

            https://www.amd.com/content/dam/amd/en/documents/epyc-technical-docs/specifications/56860.pdf
        aux_blob (bytes):
            Optional. Certificate bundle defined in the GHCB
            protocol definition Format is documented in GHCB
            revision 2.03, section 4.1.8.1 struct cert_table
            in this document:
...
class VerifyAttestationResponse(proto.Message):
    ...
    Attributes:
        oidc_claims_token (str):
            Output only. Same as claims_token, but as a
            string.
...
class TokenOptions(proto.Message):
    ...
    Attributes:
        ...
        nonce (MutableSequence[str]):
            Optional. Optional parameter to place one or
            more nonces in the eat_nonce claim in the output
            token. The minimum size for JSON-encoded EATs is
            10 bytes and the maximum size is 74 bytes.
...
class TpmAttestation(proto.Message):
    ...
    Attributes:
        quotes (MutableSequence[google.cloud.confidentialcomputing_v1.types.TpmAttestation.Quote]):
            TPM2 PCR Quotes generated by calling TPM2_Quote
            on each PCR bank.
        tcg_event_log (bytes):
            ...
            Specification".
        canonical_event_log (bytes):
            An Event Log containing additional events
            measured into the TPM that are not already
            present in the tcg_event_log. Formatted as
            described in the "Canonical Event Log Format"
            TCG Specification.
        ak_cert (bytes):
        ...
        cert_chain (MutableSequence[bytes]):
            List of DER-encoded X.509 certificates which,
            together with the ak_cert, chain back to a
    ...
    class Quote(proto.Message):
        ...
        Attributes:
            hash_algo (int):
                The hash algorithm of the PCR bank being quoted,
...
class ContainerImageSignature(proto.Message):
    ...
    Attributes:
        payload (bytes):
            Optional. The binary signature payload following
            the SimpleSigning format
            https://github.com/sigstore/cosign/blob/main/specs/SIGNATURE_SPEC.md#simple-signing.
            This payload includes the container image
            digest.
        signature (bytes):
...
class VerifyConfidentialSpaceRequest(proto.Message):
    ...
    Attributes:
        ...
        gcp_credentials (google.cloud.confidentialcomputing_v1.types.GcpCredentials):
            Optional. Credentials used to populate the
            "emails" claim in the claims_token. If not
            present, token will not contain the "emails"
            claim.
        signed_entities (MutableSequence[google.cloud.confidentialcomputing_v1.types.SignedEntity]):
            Optional. A list of signed entities
            containing container image signatures that can
        ...
        gce_shielded_identity (google.cloud.confidentialcomputing_v1.types.GceShieldedIdentity):
            Optional. Information about the associated
            Compute Engine instance. Required for td_ccel
            requests only - tpm_attestation requests will
            provide this information in the attestation.
    ...
    class ConfidentialSpaceOptions(proto.Message):
        ...
        Attributes:
            ...
            signature_type (google.cloud.confidentialcomputing_v1.types.SignatureType):
                ...
                the attestation token. Defaults to
                SIGNATURE_TYPE_OIDC if unspecified.
        """
...
class GceShieldedIdentity(proto.Message):
    ...
    Attributes:
        ...
        ak_cert_chain (MutableSequence[bytes]):
            Optional. List of DER-encoded X.509 certificates
            which, together with the ak_cert, chain back to
