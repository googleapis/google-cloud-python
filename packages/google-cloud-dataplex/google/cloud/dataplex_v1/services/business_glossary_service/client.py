...
class BusinessGlossaryServiceClient(metaclass=BusinessGlossaryServiceClientMeta):
    ...
    def parse_glossary_category_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/glossaries/(?P<glossary>.+?)/categories/(?P<glossary_category>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def parse_glossary_term_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/glossaries/(?P<glossary>.+?)/terms/(?P<glossary_term>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = BusinessGlossaryServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, BusinessGlossaryServiceTransport, Callable[..., BusinessGlossaryServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = BusinessGlossaryServiceClient._read_environment_variables()
        self._client_cert_source = BusinessGlossaryServiceClient._get_client_cert_source(
            self._client_options.client_cert_source, self._use_client_cert
        )
        self._universe_domain = BusinessGlossaryServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        self._api_endpoint = self._api_endpoint or BusinessGlossaryServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[BusinessGlossaryServiceTransport], Callable[..., BusinessGlossaryServiceTransport]] = (
