...
from google.cloud.dataplex_v1.types import data_discovery, data_documentation, data_profile, data_quality, datascans, processing, resources, service
...
class DataScanServiceClient(metaclass=DataScanServiceClientMeta):
    ...
    def parse_data_scan_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataScans/(?P<dataScan>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_data_scan_job_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataScans/(?P<dataScan>.+?)/jobs/(?P<job>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_entity_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/lakes/(?P<lake>.+?)/zones/(?P<zone>.+?)/entities/(?P<entity>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = DataScanServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, DataScanServiceTransport, Callable[..., DataScanServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = DataScanServiceClient._read_environment_variables()
        self._client_cert_source = DataScanServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = DataScanServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        self._api_endpoint = self._api_endpoint or DataScanServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[DataScanServiceTransport], Callable[..., DataScanServiceTransport]] = (
    ...
    def update_data_scan(
        ...
        *,
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("data_scan.name", request.data_scan.name),)),)
    ...
    def generate_data_quality_rules(
        self,
        request: Optional[Union[datascans.GenerateDataQualityRulesRequest, dict]] = None,
        *,
        name: Optional[str] = None,
        retry: OptionalRetry = gapic_v1.method.DEFAULT,
        timeout: Union[float, object] = gapic_v1.method.DEFAULT,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.generate_data_quality_rules]
