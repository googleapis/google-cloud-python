    ...
    * Stripping extraneous information from responses
    ...
    .. code-block:: python
        class MyCustomWorkstationsInterceptor(WorkstationsRestInterceptor):
            ...
            def pre_push_credentials(self, request, metadata):
                logging.log(f"Received request: {request}")
                return request, metadata

            def post_push_credentials(self, response):
                logging.log(f"Received response: {response}")
                return response
    ...
    def pre_push_credentials(
        self,
        request: workstations.PushCredentialsRequest,
        metadata: Sequence[Tuple[str, Union[str, bytes]]],
    ) -> Tuple[
        workstations.PushCredentialsRequest, Sequence[Tuple[str, Union[str, bytes]]]
    ]:
        """Pre-rpc interceptor for push_credentials
    ...
    def post_push_credentials(
        self, response: operations_pb2.Operation
    ) -> operations_pb2.Operation:
        """Post-rpc interceptor for push_credentials

        DEPRECATED. Please use the `post_push_credentials_with_metadata`
        interceptor instead.

        Override in a subclass to read or manipulate the response
        after it is returned by the Workstations server but before
        it is returned to user code. This `post_push_credentials` interceptor runs
        before the `post_push_credentials_with_metadata` interceptor.
        """
        return response

    def post_push_credentials_with_metadata(
        self,
        response: operations_pb2.Operation,
        metadata: Sequence[Tuple[str, Union[str, bytes]]],
    ) -> Tuple[operations_pb2.Operation, Sequence[Tuple[str, Union[str, bytes]]]]:
        ...
        We recommend only using this `post_push_credentials_with_metadata`
        interceptor in new development instead of the `post_push_credentials` interceptor.
        When both interceptors are used, this `post_push_credentials_with_metadata` interceptor runs after the
        `post_push_credentials` interceptor. The (possibly modified) response returned by
        `post_push_credentials` will be passed to
...
class WorkstationsRestTransport(_BaseWorkstationsRestTransport):
    ...
    class _PushCredentials(
        _BaseWorkstationsRestTransport._BasePushCredentials, WorkstationsRestStub
    ):
        def __hash__(self):
            return hash("WorkstationsRestTransport.PushCredentials")
        ...
        def __call__(
            ...
            *,
            ...
            r"""Call the push credentials method over HTTP.
            ...
            http_options = (
                _BaseWorkstationsRestTransport._BasePushCredentials._get_http_options()
            )

            request, metadata = self._interceptor.pre_push_credentials(
                request, metadata
            )
            transcoded_request = _BaseWorkstationsRestTransport._BasePushCredentials._get_transcoded_request(
                http_options, request
            )

            body = _BaseWorkstationsRestTransport._BasePushCredentials._get_request_body_json(
                transcoded_request
            )

            # Jsonify the query params
            query_params = _BaseWorkstationsRestTransport._BasePushCredentials._get_query_params_json(
                transcoded_request
            )
            ...
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                ...
                _LOGGER.debug(
                    f"Sending request for google.cloud.workstations_v1beta.WorkstationsClient.PushCredentials",
                    extra={
                        "serviceName": "google.cloud.workstations.v1beta.Workstations",
                        "rpcName": "PushCredentials",
                        "httpRequest": http_request,
                        "metadata": http_request["headers"],
                    },
                )
            ...
            resp = self._interceptor.post_push_credentials(resp)
            response_metadata = [(k, str(v)) for k, v in response.headers.items()]
            resp, _ = self._interceptor.post_push_credentials_with_metadata(
                resp, response_metadata
            )
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                logging.DEBUG
                ...
                _LOGGER.debug(
                    "Received response for google.cloud.workstations_v1beta.WorkstationsClient.push_credentials",
                    extra={
                        "serviceName": "google.cloud.workstations.v1beta.Workstations",
                        "rpcName": "PushCredentials",
                        "metadata": http_response["headers"],
    ...
    def push_credentials(
        ...
        # The return type is fine, but mypy isn't sophisticated enough to determine what's going on here.
        # In C++ this would require a dynamic_cast
        return self._PushCredentials(self._session, self._host, self._interceptor)  # type: ignore
