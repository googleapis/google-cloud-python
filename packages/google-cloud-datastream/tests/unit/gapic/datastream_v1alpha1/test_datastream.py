...
from google.cloud.datastream_v1alpha1.services.datastream.transports.base import (
...
def test_datastream_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_create_connection_profile_flattened():
    ...
    with mock.patch.object(
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == "parent_value"
        assert args[0].connection_profile == datastream_resources.ConnectionProfile(
            name="name_value"
        )
...
def test_discover_connection_profile_flattened():
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        client.discover_connection_profile(parent="parent_value",)
...
def test_discover_connection_profile_flattened_error():
    ...
    with pytest.raises(ValueError):
        client.discover_connection_profile(
            datastream.DiscoverConnectionProfileRequest(), parent="parent_value",
        )
...
async def test_discover_connection_profile_flattened_async():
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.discover_connection_profile(parent="parent_value",)
...
def test_create_private_connection_flattened():
    ...
    with mock.patch.object(
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == "parent_value"
        assert args[0].private_connection == datastream_resources.PrivateConnection(
            name="name_value"
        )
