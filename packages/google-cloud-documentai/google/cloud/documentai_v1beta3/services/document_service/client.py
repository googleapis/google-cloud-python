...
class DocumentServiceClient(metaclass=DocumentServiceClientMeta):
    ...
    def parse_dataset_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/processors/(?P<processor>.+?)/dataset$", path)
    ...
    def parse_dataset_schema_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/processors/(?P<processor>.+?)/dataset/datasetSchema$", path)
        return m.groupdict() if m else {}
    ...
    def parse_schema_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/schemas/(?P<schema>.+?)$", path)
        return m.groupdict() if m else {}
        ...
        *,
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.documentai_v1beta3.DocumentServiceClient`.",
                    extra={
