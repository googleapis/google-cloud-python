...
class MemorystoreClient(metaclass=MemorystoreClientMeta):
    ...
    def parse_backup_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/backupCollections/(?P<backup_collection>.+?)/backups/(?P<backup>.+?)$", path
        )
    ...
    def parse_backup_collection_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/backupCollections/(?P<backup_collection>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_certificate_authority_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/instances/(?P<instance>.+?)/certificateAuthority$", path)
        return m.groupdict() if m else {}
    ...
    def parse_forwarding_rule_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/regions/(?P<region>.+?)/forwardingRules/(?P<forwarding_rule>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = MemorystoreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, MemorystoreTransport, Callable[..., MemorystoreTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = MemorystoreClient._read_environment_variables()
        self._client_cert_source = MemorystoreClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = MemorystoreClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[MemorystoreTransport], Callable[..., MemorystoreTransport]] = (
    ...
    def get_certificate_authority(
        self,
        request: Optional[Union[memorystore.GetCertificateAuthorityRequest, dict]] = None,
        *,
        name: Optional[str] = None,
        retry: OptionalRetry = gapic_v1.method.DEFAULT,
        timeout: Union[float, object] = gapic_v1.method.DEFAULT,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.get_certificate_authority]
    ...
    def reschedule_maintenance(
        ...
        *,
        name: Optional[str] = None,
        reschedule_type: Optional[memorystore.RescheduleMaintenanceRequest.RescheduleType] = None,
