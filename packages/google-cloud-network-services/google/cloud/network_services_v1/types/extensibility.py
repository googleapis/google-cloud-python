    ...
    class VersionDetails(proto.Message):
        ...
        Attributes:
            ...
            plugin_config_uri (str):
                ...
                The URI can refer to one of the following repository
                formats:

                - Container images: the ``plugin_config_uri`` must point to
                  a container that contains a single file with the name
                  ``plugin.config``. When a new ``WasmPluginVersion``
                  resource is created, the digest of the image is saved in
                  the ``plugin_config_digest`` field. When pulling a
                  container image from Artifact Registry, the digest value
                  is used instead of an image tag.

                - Generic artifacts: the ``plugin_config_uri`` must be in
                  this format:
                  ``projects/{project}/locations/{location}/repositories/{repository}/ genericArtifacts/{package}:{version}``.
                  The specified package and version must contain a file with
                  the name ``plugin.config``. When a new
                  ``WasmPluginVersion`` resource is created, the checksum of
                  the contents of the file is saved in the
                  ``plugin_config_digest`` field.
            ...
            image_uri (str):
                Optional. URI of the image containing the Wasm module,
                stored in Artifact Registry.

                The URI can refer to one of the following repository
                formats:

                - Container images: the ``image_uri`` must point to a
                  container that contains a single file with the name
                  ``plugin.wasm``. When a new ``WasmPluginVersion`` resource
                  is created, the digest of the image is saved in the
                  ``image_digest`` field. When pulling a container image
                  from Artifact Registry, the digest value is used instead
                  of an image tag.

                - Generic artifacts: the ``image_uri`` must be in this
                  format:
                  ``projects/{project}/locations/{location}/repositories/{repository}/ genericArtifacts/{package}:{version}``.
                  The specified package and version must contain a file with
                  the name ``plugin.wasm``. When a new ``WasmPluginVersion``
                  resource is created, the checksum of the contents of the
                  file is saved in the ``image_digest`` field.
            image_digest (str):
                Output only. This field holds the digest (usually checksum)
                value for the plugin image. The value is calculated based on
                the ``image_uri`` field. If the ``image_uri`` field refers
                to a container image, the digest value is obtained from the
                container image. If the ``image_uri`` field refers to a
                generic artifact, the digest value is calculated based on
                the contents of the file.
            plugin_config_digest (str):
                Output only. This field holds the digest (usually checksum)
                value for the plugin configuration. The value is calculated
                based on the contents of ``plugin_config_data`` field or the
    ...
    class LogConfig(proto.Message):
        ...
        Attributes:
            ...
            min_log_level (google.cloud.network_services_v1.types.WasmPlugin.LogConfig.LogLevel):
                Non-empty default. Specifies the lowest level of the plugin
                logs that are exported to Cloud Logging. This setting
