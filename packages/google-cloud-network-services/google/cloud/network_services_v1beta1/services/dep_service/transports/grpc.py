...
class DepServiceGrpcTransport(DepServiceTransport):
    ...
    def list_extension_bindings(
        ...
        [dep.ListExtensionBindingsRequest], dep.ListExtensionBindingsResponse
    ]:
        r"""Return a callable for the list extension bindings method over gRPC.

        Lists ``ExtensionBinding`` resources in a given project and
        location.

        Returns:
            Callable[[~.ListExtensionBindingsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "list_extension_bindings" not in self._stubs:
            self._stubs["list_extension_bindings"] = self._logged_channel.unary_unary(
                "/google.cloud.networkservices.v1beta1.DepService/ListExtensionBindings",
                request_serializer=dep.ListExtensionBindingsRequest.serialize,
                response_deserializer=dep.ListExtensionBindingsResponse.deserialize,
            )
        return self._stubs["list_extension_bindings"]

    @property
    def get_extension_binding(
        self,
    ) -> Callable[[dep.GetExtensionBindingRequest], dep.ExtensionBinding]:
        r"""Return a callable for the get extension binding method over gRPC.

        Gets details of the specified ``ExtensionBinding`` resource.

        Returns:
            Callable[[~.GetExtensionBindingRequest],
                    ~.ExtensionBinding]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_extension_binding" not in self._stubs:
            self._stubs["get_extension_binding"] = self._logged_channel.unary_unary(
                "/google.cloud.networkservices.v1beta1.DepService/GetExtensionBinding",
                request_serializer=dep.GetExtensionBindingRequest.serialize,
                response_deserializer=dep.ExtensionBinding.deserialize,
            )
        return self._stubs["get_extension_binding"]

    @property
    def create_extension_binding(
        self,
    ) -> Callable[[dep.CreateExtensionBindingRequest], operations_pb2.Operation]:
        r"""Return a callable for the create extension binding method over gRPC.

        Creates a new ``ExtensionBinding`` resource in a given project
        and location.

        Returns:
            Callable[[~.CreateExtensionBindingRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "create_extension_binding" not in self._stubs:
            self._stubs["create_extension_binding"] = self._logged_channel.unary_unary(
                "/google.cloud.networkservices.v1beta1.DepService/CreateExtensionBinding",
                request_serializer=dep.CreateExtensionBindingRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["create_extension_binding"]

    @property
    def update_extension_binding(
        self,
    ) -> Callable[[dep.UpdateExtensionBindingRequest], operations_pb2.Operation]:
        r"""Return a callable for the update extension binding method over gRPC.

        Updates the parameters of the specified ``ExtensionBinding``
        resource.

        Returns:
            Callable[[~.UpdateExtensionBindingRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "update_extension_binding" not in self._stubs:
            self._stubs["update_extension_binding"] = self._logged_channel.unary_unary(
                "/google.cloud.networkservices.v1beta1.DepService/UpdateExtensionBinding",
                request_serializer=dep.UpdateExtensionBindingRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["update_extension_binding"]

    @property
    def delete_extension_binding(
        self,
    ) -> Callable[[dep.DeleteExtensionBindingRequest], operations_pb2.Operation]:
        r"""Return a callable for the delete extension binding method over gRPC.

        Deletes the specified ``ExtensionBinding`` resource.

        Returns:
            Callable[[~.DeleteExtensionBindingRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "delete_extension_binding" not in self._stubs:
            self._stubs["delete_extension_binding"] = self._logged_channel.unary_unary(
                "/google.cloud.networkservices.v1beta1.DepService/DeleteExtensionBinding",
                request_serializer=dep.DeleteExtensionBindingRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["delete_extension_binding"]
