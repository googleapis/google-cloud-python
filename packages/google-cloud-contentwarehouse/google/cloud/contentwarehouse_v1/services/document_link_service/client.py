...
class DocumentLinkServiceClient(metaclass=DocumentLinkServiceClientMeta):
    ...
    def parse_document_link_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/documents/(?P<document>.+?)/documentLinks/(?P<document_link>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def __init__(
        ...
        *,
        ...
        self._api_endpoint = self._api_endpoint or DocumentLinkServiceClient._get_api_endpoint(
