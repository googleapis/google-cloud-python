...
from google.cloud.contentwarehouse_v1.services.rule_set_service import RuleSetServiceAsyncClient, RuleSetServiceClient, pagers, transports
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert RuleSetServiceClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert RuleSetServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert RuleSetServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                RuleSetServiceClient._use_client_cert_effective()
...
def test__get_api_endpoint():
    ...
    assert (
        RuleSetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == RuleSetServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    ...
    assert (
        RuleSetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == RuleSetServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
@pytest.mark.parametrize("client_class", [RuleSetServiceClient, RuleSetServiceAsyncClient])
...
def test_rule_set_service_client_client_options_from_dict():
    with mock.patch("google.cloud.contentwarehouse_v1.services.rule_set_service.transports.RuleSetServiceGrpcTransport.__init__") as grpc_transport:
        ...
        client = RuleSetServiceClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
async def test_list_rule_sets_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_rule_sets), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            ruleset_service_request.ListRuleSetsResponse(
                rule_sets=[
...
async def test_list_rule_sets_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_rule_sets), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_rule_sets(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = RuleSetServiceClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = RuleSetServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = RuleSetServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = RuleSetServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = RuleSetServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = RuleSetServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_create_rule_set_rest_bad_request(request_type=ruleset_service_request.CreateRuleSetRequest):
    client = RuleSetServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
                ...
                "actions": [
                    {
                                ...
                                "audit_configs": [
                                    {
                                        ...
                                        "audit_log_configs": [
                                            {"log_type": 1, "exempted_members": ["exempted_members_value1", "exempted_members_value2"]}
                                        ],
                                    }
                                ],
                                "etag": b"etag_blob",
                        ...
                        "data_update": {"entries": {}},
                        "add_to_folder": {"folders": ["folders_value1", "folders_value2"]},
                        "publish_to_pub_sub": {"topic_id": "topic_id_value", "messages": ["messages_value1", "messages_value2"]},
                        "remove_from_folder_action": {"condition": "condition_value", "folder": "folder_value"},
...
def test_create_rule_set_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.RuleSetServiceRestInterceptor, "post_create_rule_set") as post, mock.patch.object(
...
def test_get_rule_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RuleSetServiceRestInterceptor, "post_get_rule_set") as post, mock.patch.object(
...
def test_update_rule_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RuleSetServiceRestInterceptor, "post_update_rule_set") as post, mock.patch.object(
...
def test_delete_rule_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RuleSetServiceRestInterceptor, "pre_delete_rule_set") as pre:
        pre.assert_not_called()
...
def test_list_rule_sets_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RuleSetServiceRestInterceptor, "post_list_rule_sets") as post, mock.patch.object(
        ...
        post_with_metadata.return_value = ruleset_service_request.ListRuleSetsResponse(), metadata
...
def test_rule_set_service_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.RuleSetServiceTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.RuleSetServiceGrpcTransport, grpc_helpers), (transports.RuleSetServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_rule_set_service_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.RuleSetServiceRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
@pytest.mark.parametrize("transport_class", [transports.RuleSetServiceGrpcTransport, transports.RuleSetServiceGrpcAsyncIOTransport])
def test_rule_set_service_transport_channel_mtls_with_client_cert_source(transport_class):
...
def test_client_ctx():
    ...
    for transport in transports:
        client = RuleSetServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
