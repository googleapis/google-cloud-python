...
def test__get_api_endpoint():
    ...
    assert (
        SynonymSetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        SynonymSetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == SynonymSetServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = SynonymSetServiceClient(credentials=cred)
...
def test_synonym_set_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "contentwarehouse.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://contentwarehouse.googleapis.com"
        )
...
async def test_list_synonym_sets_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_synonym_sets), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_synonym_sets(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_transport_kind_grpc():
    transport = SynonymSetServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = SynonymSetServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = SynonymSetServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = SynonymSetServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_create_synonym_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.SynonymSetServiceRestInterceptor, "post_create_synonym_set") as post, mock.patch.object(
        transports.SynonymSetServiceRestInterceptor, "post_create_synonym_set_with_metadata"
        ...
        post_with_metadata.return_value = synonymset.SynonymSet(), metadata
...
def test_get_synonym_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SynonymSetServiceRestInterceptor, "post_get_synonym_set") as post, mock.patch.object(
        transports.SynonymSetServiceRestInterceptor, "post_get_synonym_set_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_update_synonym_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SynonymSetServiceRestInterceptor, "post_update_synonym_set") as post, mock.patch.object(
        transports.SynonymSetServiceRestInterceptor, "post_update_synonym_set_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_delete_synonym_set_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SynonymSetServiceRestInterceptor, "pre_delete_synonym_set") as pre:
        pre.assert_not_called()
...
def test_list_synonym_sets_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SynonymSetServiceRestInterceptor, "post_list_synonym_sets") as post, mock.patch.object(
        transports.SynonymSetServiceRestInterceptor, "post_list_synonym_sets_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post_with_metadata.return_value = synonymset_service_request.ListSynonymSetsResponse(), metadata
...
@pytest.mark.parametrize(
    ...
    [(transports.SynonymSetServiceGrpcTransport, grpc_helpers), (transports.SynonymSetServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_synonym_set_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "contentwarehouse.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://contentwarehouse.googleapis.com:8000"
    )
