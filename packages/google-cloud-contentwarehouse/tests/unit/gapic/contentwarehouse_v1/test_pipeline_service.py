...
from google.cloud.contentwarehouse_v1.services.pipeline_service import (
...
def test_pipeline_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.contentwarehouse_v1.services.pipeline_service.transports.PipelineServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_run_pipeline_rest_required_fields(
    request_type=pipeline_service.RunPipelineRequest,
):
...
def test_run_pipeline_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1/{name=projects/*/locations/*}:runPipeline" % client.transport._host,
            args[1],
        )
...
def test_run_pipeline_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        pb_message = pipeline_service.RunPipelineRequest.pb(
            pipeline_service.RunPipelineRequest()
        )
...
def test_pipeline_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.contentwarehouse_v1.services.pipeline_service.transports.PipelineServiceTransport.__init__"
    ) as Transport:
