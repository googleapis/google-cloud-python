...
from google.cloud.contentwarehouse_v1.services.document_service import DocumentServiceAsyncClient, DocumentServiceClient, pagers, transports
from google.cloud.contentwarehouse_v1.types import document_service, document_service_request, filters, histogram, rule_engine
...
def test_search_documents_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.search_documents), "__call__") as call:
        ...
        assert all(isinstance(i, document_service.SearchDocumentsResponse.MatchingDocument) for i in results)
...
async def test_search_documents_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.search_documents), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            document_service.SearchDocumentsResponse(
                matching_documents=[
        ...
        assert all(isinstance(i, document_service.SearchDocumentsResponse.MatchingDocument) for i in responses)
...
async def test_search_documents_async_pages():
    ...
    with mock.patch.object(type(client.transport.search_documents), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.search_documents(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_create_document_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post_with_metadata.return_value = document_service.CreateDocumentResponse(), metadata
...
def test_get_document_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post_with_metadata.return_value = gcc_document.Document(), metadata
...
def test_update_document_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post_with_metadata.return_value = document_service.UpdateDocumentResponse(), metadata
...
def test_search_documents_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DocumentServiceRestInterceptor, "post_search_documents") as post, mock.patch.object(
        ...
        post_with_metadata.return_value = document_service.SearchDocumentsResponse(), metadata
...
def test_lock_document_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DocumentServiceRestInterceptor, "post_lock_document") as post, mock.patch.object(
...
def test_fetch_acl_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DocumentServiceRestInterceptor, "post_fetch_acl") as post, mock.patch.object(
        ...
        post_with_metadata.return_value = document_service.FetchAclResponse(), metadata
...
def test_set_acl_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DocumentServiceRestInterceptor, "post_set_acl") as post, mock.patch.object(
        ...
        post_with_metadata.return_value = document_service.SetAclResponse(), metadata
