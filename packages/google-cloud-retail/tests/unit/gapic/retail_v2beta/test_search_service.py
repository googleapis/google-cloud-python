...
from google.cloud.retail_v2beta.services.search_service import SearchServiceAsyncClient, SearchServiceClient, pagers, transports
...
def test_search_service_client_client_options_from_dict():
    with mock.patch("google.cloud.retail_v2beta.services.search_service.transports.SearchServiceGrpcTransport.__init__") as grpc_transport:
...
def test_search_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.search), "__call__") as call:
        ...
        retry = retries.Retry()
        timeout = 5
        expected_metadata = tuple(expected_metadata) + (gapic_v1.routing_header.to_grpc_metadata((("placement", ""),)),)
...
def test_search_service_base_transport():
    ...
    with mock.patch("google.cloud.retail_v2beta.services.search_service.transports.SearchServiceTransport.__init__") as Transport:
...
def test_search_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.cloud.retail_v2beta.services.search_service.transports.SearchServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
def test_experiment_path():
    ...
    catalog = "scallop"
    experiment = "abalone"
    expected = "projects/{project}/locations/{location}/catalogs/{catalog}/experiments/{experiment}".format(
        project=project,
        location=location,
        catalog=catalog,
