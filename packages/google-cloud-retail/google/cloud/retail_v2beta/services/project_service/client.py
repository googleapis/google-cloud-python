...
class ProjectServiceClient(metaclass=ProjectServiceClientMeta):
    ...
    def get_mtls_endpoint_and_cert_source(cls, client_options: Optional[client_options_lib.ClientOptions] = None):
        ...
        use_client_cert = ProjectServiceClient._use_client_cert_effective()
    ...
    def __init__(
        ...
        *,
        ...
        self._api_endpoint = self._api_endpoint or ProjectServiceClient._get_api_endpoint(
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.retail_v2beta.ProjectServiceClient`.",
                    extra={
    ...
    def update_alert_config(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [alert_config, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("alert_config.name", request.alert_config.name),)),)
