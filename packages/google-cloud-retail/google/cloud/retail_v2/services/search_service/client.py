...
class SearchServiceClient(metaclass=SearchServiceClientMeta):
    ...
    def parse_experiment_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/experiments/(?P<experiment>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_serving_config_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/servingConfigs/(?P<serving_config>.+?)$", path
        )
        return m.groupdict() if m else {}
        ...
        *,
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.retail_v2.SearchServiceClient`.",
                    extra={
    ...
    def search(
        ...
        *,
        ...
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("placement", request.placement),)),)
