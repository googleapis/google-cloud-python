...
from .base import DEFAULT_CLIENT_INFO, AppTopologyTransport
...
class AppTopologyGrpcTransport(AppTopologyTransport):
    """gRPC backend transport for AppTopology.
    ...
    def generate_discovered_resources_topology(
        ...
        r"""Return a callable for the generate discovered resources
        topology method over gRPC.

        Generate the topology for all resources in the given project. If
        the project represents an `app
        boundary <https://cloud.google.com/app-hub/docs/reference/rest/v1/Boundary>`__,
        the topology is generated for all resources in the boundary.

        Returns:
            Callable[[~.GenerateDiscoveredResourcesTopologyRequest],
        ...
        if "generate_discovered_resources_topology" not in self._stubs:
            self._stubs["generate_discovered_resources_topology"] = (
                self._logged_channel.unary_unary(
                    "/google.cloud.apptopology.v1.AppTopology/GenerateDiscoveredResourcesTopology",
                    request_serializer=service.GenerateDiscoveredResourcesTopologyRequest.serialize,
                    response_deserializer=service.GenerateDiscoveredResourcesTopologyResponse.deserialize,
                )
            )
        return self._stubs["generate_discovered_resources_topology"]
    ...
    def get_schema(self) -> Callable[[service.GetSchemaRequest], schema.Schema]:
        ...
        Retrieves the schema for the specified topology
        domain. The schema defines the NodeTypes and EdgeTypes
        that are supported in
        GenerateDiscoveredResourcesTopology requests and
        responses for a given domain.
        ...
        if "get_schema" not in self._stubs:
            self._stubs["get_schema"] = self._logged_channel.unary_unary(
                ...
                request_serializer=service.GetSchemaRequest.serialize,
    ...
    def get_domain(self) -> Callable[[service.GetDomainRequest], schema.Domain]:
        ...
        if "get_domain" not in self._stubs:
            self._stubs["get_domain"] = self._logged_channel.unary_unary(
                ...
                request_serializer=service.GetDomainRequest.serialize,
                response_deserializer=schema.Domain.deserialize,
            )
    ...
    def list_domains(
        ...
        if "list_domains" not in self._stubs:
            self._stubs["list_domains"] = self._logged_channel.unary_unary(
                ...
                request_serializer=service.ListDomainsRequest.serialize,
                response_deserializer=service.ListDomainsResponse.deserialize,
            )
...
__all__ = ("AppTopologyGrpcTransport",)
