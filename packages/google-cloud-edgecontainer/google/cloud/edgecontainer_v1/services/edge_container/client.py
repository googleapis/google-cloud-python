...
class EdgeContainerClient(metaclass=EdgeContainerClientMeta):
    ...
    _DEFAULT_ENDPOINT_TEMPLATE = "edgecontainer.{UNIVERSE_DOMAIN}"
    ...
    def parse_machine_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/machines/(?P<machine>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_node_pool_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/clusters/(?P<cluster>.+?)/nodePools/(?P<node_pool>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_vpn_connection_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/vpnConnections/(?P<vpn_connection>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def get_mtls_endpoint_and_cert_source(cls, client_options: Optional[client_options_lib.ClientOptions] = None):
        ...
        use_client_cert = EdgeContainerClient._use_client_cert_effective()
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = EdgeContainerClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = EdgeContainerClient._DEFAULT_UNIVERSE
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, EdgeContainerTransport, Callable[..., EdgeContainerTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = EdgeContainerClient._read_environment_variables()
        self._client_cert_source = EdgeContainerClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = EdgeContainerClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, EdgeContainerTransport)
        if transport_provided:
            ...
            self._transport = cast(EdgeContainerTransport, transport)
            self._api_endpoint = self._transport.host

        self._api_endpoint = self._api_endpoint or EdgeContainerClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[EdgeContainerTransport], Callable[..., EdgeContainerTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.edgecontainer_v1.EdgeContainerClient`.",
                    extra={
    ...
    def upgrade_cluster(
        ...
        *,
        ...
        Args:
            request (Union[google.cloud.edgecontainer_v1.types.UpgradeClusterRequest, dict]):
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [name, target_version, schedule]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
    ...
    def generate_access_token(
        ...
        *,
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("cluster", request.cluster),)),)
    ...
    def generate_offline_credential(
        ...
        *,
        ...
        Args:
            request (Union[google.cloud.edgecontainer_v1.types.GenerateOfflineCredentialRequest, dict]):
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.generate_offline_credential]
    ...
    def create_node_pool(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [parent, node_pool, node_pool_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
    ...
    def update_node_pool(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [node_pool, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("node_pool.name", request.node_pool.name),)),)
    ...
    def create_vpn_connection(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [parent, vpn_connection, vpn_connection_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
    ...
    def get_server_config(
        ...
        *,
        ...
        Args:
            request (Union[google.cloud.edgecontainer_v1.types.GetServerConfigRequest, dict]):
