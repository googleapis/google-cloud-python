...
async def test_list_replay_results_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_replay_results), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_replay_results(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_replay_results_rest_required_fields(request_type=simulator.ListReplayResultsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_replay_results._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_transport_kind_grpc():
    transport = SimulatorClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = SimulatorAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = SimulatorAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = SimulatorClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_get_replay_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.SimulatorRestInterceptor, "post_get_replay") as post, mock.patch.object(
...
def test_list_replay_results_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SimulatorRestInterceptor, "post_list_replay_results") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.SimulatorGrpcTransport, grpc_helpers), (transports.SimulatorGrpcAsyncIOTransport, grpc_helpers_async)],
)
