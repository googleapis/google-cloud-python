...
from google.ai.generativelanguage_v1beta.services.prediction_service import PredictionServiceAsyncClient, PredictionServiceClient, transports
...
def test_predict_long_running(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.predict_long_running), "__call__") as call:
        # Designate an appropriate return value for the call.
...
def test_predict_long_running_rest_required_fields(request_type=prediction_service.PredictLongRunningRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).predict_long_running._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_predict_long_running_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.PredictionServiceRestInterceptor, "post_predict_long_running_with_metadata"
    ) as post_with_metadata, mock.patch.object(
