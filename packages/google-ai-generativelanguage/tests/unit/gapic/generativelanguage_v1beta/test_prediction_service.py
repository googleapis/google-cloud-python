...
from google.ai.generativelanguage_v1beta.services.prediction_service import (
...
def test_prediction_service_client_client_options_from_dict():
    with mock.patch(
        "google.ai.generativelanguage_v1beta.services.prediction_service.transports.PredictionServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_predict_long_running(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client.transport.predict_long_running), "__call__"
    ) as call:
        # Designate an appropriate return value for the call.
...
def test_predict_long_running_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._transport.predict_long_running in client._transport._wrapped_methods
        )
        ...
        client._transport._wrapped_methods[client._transport.predict_long_running] = (
            mock_rpc
        )
        request = {}
...
@pytest.mark.asyncio
async def test_predict_long_running_async_use_cached_wrapped_rpc(
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.predict_long_running
            in client._client._transport._wrapped_methods
        )
...
@pytest.mark.asyncio
async def test_predict_long_running_async(
    transport: str = "grpc_asyncio",
    request_type=prediction_service.PredictLongRunningRequest,
):
    client = PredictionServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
...
def test_predict_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1beta/{model=models/*}:predict" % client.transport._host, args[1]
        )
...
def test_predict_long_running_rest_required_fields(
    request_type=prediction_service.PredictLongRunningRequest,
):
    transport_class = transports.PredictionServiceRestTransport
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).predict_long_running._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_predict_long_running_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1beta/{model=models/*}:predictLongRunning" % client.transport._host,
            args[1],
        )
...
def test_predict_long_running_rest_bad_request(
    request_type=prediction_service.PredictLongRunningRequest,
):
    client = PredictionServiceClient(
        credentials=ga_credentials.AnonymousCredentials(), transport="rest"
...
def test_predict_long_running_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.PredictionServiceRestInterceptor, "post_predict_long_running"
    ) as post, mock.patch.object(
        transports.PredictionServiceRestInterceptor,
        "post_predict_long_running_with_metadata",
    ) as post_with_metadata, mock.patch.object(
        transports.PredictionServiceRestInterceptor, "pre_predict_long_running"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = prediction_service.PredictLongRunningRequest.pb(
            prediction_service.PredictLongRunningRequest()
        )
        transcode.return_value = {
            "method": "post",
...
def test_prediction_service_base_transport():
    ...
    with mock.patch(
        "google.ai.generativelanguage_v1beta.services.prediction_service.transports.PredictionServiceTransport.__init__"
    ) as Transport:
...
def test_prediction_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.ai.generativelanguage_v1beta.services.prediction_service.transports.PredictionServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
