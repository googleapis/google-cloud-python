...
from google.ai.generativelanguage_v1alpha.services.file_service import (
...
def test__get_default_mtls_endpoint():
    ...
    assert (
        FileServiceClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    )
    assert (
        FileServiceClient._get_default_mtls_endpoint(api_mtls_endpoint)
        == api_mtls_endpoint
    )
    assert (
        FileServiceClient._get_default_mtls_endpoint(sandbox_endpoint)
        == sandbox_mtls_endpoint
    )
    assert (
        FileServiceClient._get_default_mtls_endpoint(sandbox_mtls_endpoint)
        == sandbox_mtls_endpoint
    )
    assert FileServiceClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi
    assert (
        FileServiceClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        FileServiceClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        FileServiceClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                FileServiceClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                FileServiceClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(FileServiceClient),
)
@mock.patch.object(
    FileServiceAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(FileServiceAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = FileServiceClient._DEFAULT_UNIVERSE
    default_endpoint = FileServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = FileServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        FileServiceClient._get_api_endpoint(
    ...
    assert (
        ...
        == FileServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        FileServiceClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        FileServiceClient._get_api_endpoint(None, None, default_universe, "always")
        == FileServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        FileServiceClient._get_api_endpoint(
    ...
    assert (
        FileServiceClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        FileServiceClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        FileServiceClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        FileServiceClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        FileServiceClient._get_universe_domain(None, None)
        == FileServiceClient._DEFAULT_UNIVERSE
    )
...
def test_file_service_client_service_account_always_use_jwt(
    transport_class, transport_name
):
    with mock.patch.object(
...
def test_file_service_client_client_options(
    client_class, transport_class, transport_name
):
...
def test_file_service_client_mtls_env_auto(
    client_class, transport_class, transport_name, use_client_cert_env
):
...
@mock.patch.object(
    FileServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(FileServiceClient)
)
@mock.patch.object(
    FileServiceAsyncClient,
    "DEFAULT_ENDPOINT",
    modify_default_endpoint(FileServiceAsyncClient),
)
...
def test_file_service_client_client_options_scopes(
    client_class, transport_class, transport_name
):
...
def test_file_service_client_client_options_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
...
def test_file_service_client_client_options_from_dict():
    with mock.patch(
        "google.ai.generativelanguage_v1alpha.services.file_service.transports.FileServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_file_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_get_file_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1alpha/{name=files/*}" % client.transport._host, args[1]
        )
...
def test_delete_file_rest_required_fields(request_type=file_service.DeleteFileRequest):
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).delete_file._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_transport_kind_rest():
    transport = FileServiceClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_create_file_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = file_service.CreateFileResponse.to_json(
            file_service.CreateFileResponse()
        )
...
def test_list_files_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = file_service.ListFilesResponse.to_json(
            file_service.ListFilesResponse()
        )
        req.return_value.content = return_value
...
def test_get_file_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(transports.FileServiceRestInterceptor, "pre_get_file") as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_delete_file_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(
            transports.FileServiceRestInterceptor, "pre_delete_file"
        ) as pre,
    ):
        pre.assert_not_called()
...
def test_file_service_base_transport():
    ...
    with mock.patch(
        "google.ai.generativelanguage_v1alpha.services.file_service.transports.FileServiceTransport.__init__"
    ) as Transport:
...
@pytest.mark.parametrize(
    ...
    [
        (transports.FileServiceGrpcTransport, grpc_helpers),
        (transports.FileServiceGrpcAsyncIOTransport, grpc_helpers_async),
    ],
)
...
@pytest.mark.parametrize(
    "transport_class",
    [transports.FileServiceGrpcTransport, transports.FileServiceGrpcAsyncIOTransport],
)
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(
        transports.FileServiceTransport, "_prep_wrapped_messages"
    ) as prep:
        client = FileServiceClient(
            credentials=ga_credentials.AnonymousCredentials(),
