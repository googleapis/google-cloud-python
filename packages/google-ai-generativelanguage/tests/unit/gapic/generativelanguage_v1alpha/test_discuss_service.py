...
from google.ai.generativelanguage_v1alpha.services.discuss_service import DiscussServiceAsyncClient, DiscussServiceClient, transports
from google.ai.generativelanguage_v1alpha.types import citation, discuss_service, safety
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert DiscussServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert DiscussServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                DiscussServiceClient._use_client_cert_effective()
...
def test__get_api_endpoint():
    ...
    assert (
        DiscussServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DiscussServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    ...
    assert (
        DiscussServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
...
def test_discuss_service_client_client_options_from_dict():
    with mock.patch(
        "google.ai.generativelanguage_v1alpha.services.discuss_service.transports.DiscussServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_count_message_tokens_rest_required_fields(request_type=discuss_service.CountMessageTokensRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).count_message_tokens._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_transport_kind_grpc():
    transport = DiscussServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = DiscussServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = DiscussServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = DiscussServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_generate_message_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.DiscussServiceRestInterceptor, "post_generate_message") as post, mock.patch.object(
        transports.DiscussServiceRestInterceptor, "post_generate_message_with_metadata"
...
def test_count_message_tokens_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DiscussServiceRestInterceptor, "post_count_message_tokens") as post, mock.patch.object(
        transports.DiscussServiceRestInterceptor, "post_count_message_tokens_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_discuss_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.ai.generativelanguage_v1alpha.services.discuss_service.transports.DiscussServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
@pytest.mark.parametrize(
    ...
    [(transports.DiscussServiceGrpcTransport, grpc_helpers), (transports.DiscussServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
