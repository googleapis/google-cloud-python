...
from google.ai.generativelanguage_v1beta2.services.text_service import (
...
def test_text_service_client_client_options_from_dict():
    with mock.patch(
        "google.ai.generativelanguage_v1beta2.services.text_service.transports.TextServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_generate_text_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1beta2/{model=models/*}:generateText" % client.transport._host, args[1]
        )
...
def test_embed_text_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1beta2/{model=models/*}:embedText" % client.transport._host, args[1]
        )
...
def test_text_service_base_transport():
    ...
    with mock.patch(
        "google.ai.generativelanguage_v1beta2.services.text_service.transports.TextServiceTransport.__init__"
    ) as Transport:
...
def test_text_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.ai.generativelanguage_v1beta2.services.text_service.transports.TextServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
