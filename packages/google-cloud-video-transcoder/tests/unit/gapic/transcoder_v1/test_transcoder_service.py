...
def test__read_environment_variables():
    assert TranscoderServiceClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert TranscoderServiceClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert TranscoderServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert TranscoderServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert TranscoderServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert TranscoderServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert TranscoderServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                TranscoderServiceClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert TranscoderServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert TranscoderServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert TranscoderServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert TranscoderServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(TranscoderServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(TranscoderServiceClient))
@mock.patch.object(TranscoderServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(TranscoderServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = TranscoderServiceClient._DEFAULT_UNIVERSE
    default_endpoint = TranscoderServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = TranscoderServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert TranscoderServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        TranscoderServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
        == TranscoderServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert TranscoderServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert TranscoderServiceClient._get_api_endpoint(None, None, default_universe, "always") == TranscoderServiceClient.DEFAULT_MTLS_ENDPOINT
    assert (
        TranscoderServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == TranscoderServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert TranscoderServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert TranscoderServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        TranscoderServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert TranscoderServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert TranscoderServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert TranscoderServiceClient._get_universe_domain(None, None) == TranscoderServiceClient._DEFAULT_UNIVERSE
...
def test_transcoder_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "transcoder.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://transcoder.googleapis.com"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (TranscoderServiceClient, transports.TranscoderServiceRestTransport, "rest", "true"),
        (TranscoderServiceClient, transports.TranscoderServiceRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(TranscoderServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(TranscoderServiceClient))
@mock.patch.object(TranscoderServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(TranscoderServiceAsyncClient))
...
@pytest.mark.parametrize(
    ...
    [
        (TranscoderServiceClient, transports.TranscoderServiceGrpcTransport, "grpc", grpc_helpers),
        (TranscoderServiceAsyncClient, transports.TranscoderServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (TranscoderServiceClient, transports.TranscoderServiceRestTransport, "rest", None),
    ],
)
...
def test_create_job_template_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_job_template in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_job_template] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_job_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = TranscoderServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_job_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_job_template] = mock_rpc
...
def test_list_job_templates_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.list_job_templates in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_job_templates] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_job_templates_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = TranscoderServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_job_templates in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_job_templates] = mock_rpc
...
async def test_list_job_templates_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_job_templates), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        # Workaround issue in python 3.9 related to code coverage by adding `# pragma: no branch`
        # See https://github.com/googleapis/gapic-generator-python/pull/1174#issuecomment-1025132372
        async for page_ in (await client.list_job_templates(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_job_template_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_job_template] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_job_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = TranscoderServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_job_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_job_template] = mock_rpc
...
def test_delete_job_template_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_job_template in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_job_template] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_job_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = TranscoderServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_job_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_job_template] = mock_rpc
...
def test_create_job_template_rest_required_fields(request_type=services.CreateJobTemplateRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_job_template._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_delete_job_template_rest_required_fields(request_type=services.DeleteJobTemplateRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_job_template._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_transport_kind_grpc():
    transport = TranscoderServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = TranscoderServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = TranscoderServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = TranscoderServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_create_job_rest_call_success(request_type):
    ...
    request_init["job"] = {
        ...
        "config": {
            "inputs": [
                {
                    ...
                    "preprocessing_config": {
                        "color": {"saturation": 0.10980000000000001, "contrast": 0.878, "brightness": 0.1081},
                        "denoise": {"strength": 0.879, "tune": "tune_value"},
                        "deblock": {"strength": 0.879, "enabled": True},
                        "audio": {"lufs": 0.442, "high_boost": True, "low_boost": True},
                        "crop": {"top_pixels": 1095, "bottom_pixels": 1417, "left_pixels": 1183, "right_pixels": 1298},
                        "pad": {"top_pixels": 1095, "bottom_pixels": 1417, "left_pixels": 1183, "right_pixels": 1298},
                        "deinterlace": {
                            "yadif": {
                                "mode": "mode_value",
                            ...
                            "bwdif": {"mode": "mode_value", "parity": "parity_value", "deinterlace_all_frames": True},
                        },
                    },
                    "attributes": {
                        "track_definitions": [
                            {
                                ...
                                "detected_languages": ["detected_languages_value1", "detected_languages_value2"],
                            }
                        ]
                    },
                }
            ...
            "elementary_streams": [
                {
                    ...
                    "audio_stream": {
                        ...
                        "bitrate_bps": 1167,
                        "channel_count": 1377,
                        "channel_layout": ["channel_layout_value1", "channel_layout_value2"],
                        "mapping_": [
                            {
                    ...
                    "text_stream": {
                        "codec": "codec_value",
                        "language_code": "language_code_value",
                        "mapping_": [{"atom_key": "atom_key_value", "input_key": "input_key_value", "input_track": 1188}],
                        "display_name": "display_name_value",
                    },
                }
            ],
            "mux_streams": [
                {
                    ...
                    "elementary_streams": ["elementary_streams_value1", "elementary_streams_value2"],
                    "segment_settings": {"segment_duration": {}, "individual_segments": True},
            ...
            "overlays": [
                {
                    "image": {"uri": "uri_value", "resolution": {"x": 0.12, "y": 0.121}, "alpha": 0.518},
                    "animations": [
                        {
                            "animation_static": {"xy": {}, "start_time_offset": {}},
                            "animation_fade": {"fade_type": 1, "xy": {}, "start_time_offset": {}, "end_time_offset": {}},
            ...
            "encryptions": [
                {
                    ...
                    "drm_systems": {"widevine": {}, "fairplay": {}, "playready": {}, "clearkey": {}},
                }
            ],
        },
        "state": 1,
...
def test_create_job_rest_interceptors(null_interceptor):
    transport = transports.TranscoderServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.TranscoderServiceRestInterceptor(),
    )
    client = TranscoderServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "post_create_job") as post, mock.patch.object(
...
def test_list_jobs_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "post_list_jobs") as post, mock.patch.object(
...
def test_get_job_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "post_get_job") as post, mock.patch.object(
...
def test_delete_job_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "pre_delete_job") as pre:
        pre.assert_not_called()
...
def test_create_job_template_rest_bad_request(request_type=services.CreateJobTemplateRequest):
    client = TranscoderServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_create_job_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "post_create_job_template") as post, mock.patch.object(
        transports.TranscoderServiceRestInterceptor, "post_create_job_template_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_list_job_templates_rest_bad_request(request_type=services.ListJobTemplatesRequest):
    client = TranscoderServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_list_job_templates_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "post_list_job_templates") as post, mock.patch.object(
        transports.TranscoderServiceRestInterceptor, "post_list_job_templates_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = services.ListJobTemplatesResponse.to_json(services.ListJobTemplatesResponse())
        req.return_value.content = return_value
...
def test_get_job_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "post_get_job_template") as post, mock.patch.object(
        transports.TranscoderServiceRestInterceptor, "post_get_job_template_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_delete_job_template_rest_bad_request(request_type=services.DeleteJobTemplateRequest):
    client = TranscoderServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/jobTemplates/sample3"}
    request = request_type(**request_init)
...
def test_delete_job_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TranscoderServiceRestInterceptor, "pre_delete_job_template") as pre:
        pre.assert_not_called()
...
@pytest.mark.parametrize(
    ...
    [(transports.TranscoderServiceGrpcTransport, grpc_helpers), (transports.TranscoderServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_transcoder_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "transcoder.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://transcoder.googleapis.com:8000"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (TranscoderServiceAsyncClient, transports.TranscoderServiceGrpcAsyncIOTransport),
    ],
)
