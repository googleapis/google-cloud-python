...
from google.cloud.filestore_v1.services.cloud_filestore_manager.transports.base import (
...
def test_create_instance_flattened():
    ...
    with mock.patch.object(type(client.transport.create_instance), "__call__") as call:
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == "parent_value"
        assert args[0].instance == cloud_filestore_service.Instance(name="name_value")
...
def test_create_backup_flattened():
    ...
    with mock.patch.object(type(client.transport.create_backup), "__call__") as call:
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == "parent_value"
        assert args[0].backup == cloud_filestore_service.Backup(name="name_value")
...
def test_cloud_filestore_manager_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
...
@requires_google_auth_lt_1_25_0
def test_cloud_filestore_manager_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        CloudFilestoreManagerClient()
...
@requires_google_auth_lt_1_25_0
def test_cloud_filestore_manager_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
