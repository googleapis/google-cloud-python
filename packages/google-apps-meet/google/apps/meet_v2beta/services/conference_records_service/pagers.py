...
class ListSmartNotesPager:
    """A pager for iterating through ``list_smart_notes`` requests.

    This class thinly wraps an initial
    :class:`google.apps.meet_v2beta.types.ListSmartNotesResponse` object, and
    provides an ``__iter__`` method to iterate through its
    ``smart_notes`` field.

    If there are more pages, the ``__iter__`` method will make additional
    ``ListSmartNotes`` requests and continue to iterate
    through the ``smart_notes`` field on the
    corresponding responses.

    All the usual :class:`google.apps.meet_v2beta.types.ListSmartNotesResponse`
    attributes are available on the pager. If multiple requests are made, only
    the most recent response is retained, and thus used for attribute lookup.
    """
    ...
    def __init__(
        self,
        method: Callable[..., service.ListSmartNotesResponse],
        ...
        *,
        ...
        Args:
            method (Callable): The method that was originally called, and
                which instantiated this pager.
            request (google.apps.meet_v2beta.types.ListSmartNotesRequest):
                The initial request object.
            response (google.apps.meet_v2beta.types.ListSmartNotesResponse):
                The initial response object.
            retry (google.api_core.retry.Retry): Designation of what errors,
                if any, should be retried.
            timeout (float): The timeout for this request.
        ...
        """
        self._method = method
        self._request = service.ListSmartNotesRequest(request)
        self._response = response
        self._retry = retry
        self._timeout = timeout
        self._metadata = metadata
    ...
    @property
    def pages(self) -> Iterator[service.ListSmartNotesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = self._method(
    ...
    def __iter__(self) -> Iterator[resource.SmartNote]:
        for page in self.pages:
            yield from page.smart_notes

    def __repr__(self) -> str:
        return "{0}<{1!r}>".format(self.__class__.__name__, self._response)


class ListSmartNotesAsyncPager:
    """A pager for iterating through ``list_smart_notes`` requests.
    ...
    def __init__(
        self,
        method: Callable[..., Awaitable[service.ListSmartNotesResponse]],
        request: service.ListSmartNotesRequest,
        response: service.ListSmartNotesResponse,
        *,
        retry: OptionalAsyncRetry = gapic_v1.method.DEFAULT,
    ...
    @property
    async def pages(self) -> AsyncIterator[service.ListSmartNotesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(
    ...
    def __aiter__(self) -> AsyncIterator[resource.SmartNote]:
        async def async_generator():
            async for page in self.pages:
                for response in page.smart_notes:
                    yield response
