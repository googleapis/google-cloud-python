...
class AggregatedListPager:
    ...
    @property
    def pages(self) -> Iterable[compute.NodeTemplateAggregatedList]:
    ...
    def __iter__(self) -> Iterable[Tuple[str, compute.NodeTemplatesScopedList]]:
...
class ListPager:
    ...
    @property
    def pages(self) -> Iterable[compute.NodeTemplateList]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = self._method(self._request, metadata=self._metadata)
    ...
    def __iter__(self) -> Iterable[compute.NodeTemplate]:
        for page in self.pages:
