...
class AggregatedListPager:
    ...
    @property
    def pages(self) -> Iterable[compute.AddressAggregatedList]:
    ...
    def __iter__(self) -> Iterable[Tuple[str, compute.AddressesScopedList]]:
...
class ListPager:
    ...
    @property
    def pages(self) -> Iterable[compute.AddressList]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = self._method(self._request, metadata=self._metadata)
    ...
    def __iter__(self) -> Iterable[compute.Address]:
        for page in self.pages:
