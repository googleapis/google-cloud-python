...
from google.cloud.compute_v1.services.snapshots import SnapshotsClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert SnapshotsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert SnapshotsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert SnapshotsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert SnapshotsClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi


def test__read_environment_variables():
    assert SnapshotsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert SnapshotsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                SnapshotsClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert SnapshotsClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert SnapshotsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert SnapshotsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert SnapshotsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert SnapshotsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert SnapshotsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                SnapshotsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert SnapshotsClient._get_client_cert_source(None, False) is None
    assert SnapshotsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert SnapshotsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert SnapshotsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert SnapshotsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(SnapshotsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SnapshotsClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = SnapshotsClient._DEFAULT_UNIVERSE
    default_endpoint = SnapshotsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = SnapshotsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert SnapshotsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert SnapshotsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == SnapshotsClient.DEFAULT_MTLS_ENDPOINT
    assert SnapshotsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert SnapshotsClient._get_api_endpoint(None, None, default_universe, "always") == SnapshotsClient.DEFAULT_MTLS_ENDPOINT
    assert SnapshotsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == SnapshotsClient.DEFAULT_MTLS_ENDPOINT
    assert SnapshotsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert SnapshotsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        SnapshotsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert SnapshotsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert SnapshotsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert SnapshotsClient._get_universe_domain(None, None) == SnapshotsClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        SnapshotsClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = SnapshotsClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        (transports.SnapshotsRestTransport, "rest"),
    ],
)
def test_snapshots_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_snapshots_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(SnapshotsClient, "DEFAULT_ENDPOINT", modify_default_endpoint(SnapshotsClient))
def test_snapshots_client_get_mtls_endpoint_and_cert_source(client_class):
    mock_client_cert_source = mock.Mock()
...
@pytest.mark.parametrize("client_class", [SnapshotsClient])
@mock.patch.object(SnapshotsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SnapshotsClient))
def test_snapshots_client_client_api_endpoint(client_class):
...
def test_snapshots_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_snapshots_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_delete_rest_unset_required_fields():
    transport = transports.SnapshotsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/compute/v1/projects/{project}/global/snapshots/{snapshot}" % client.transport._host, args[1])
...
def test_get_rest_required_fields(request_type=compute.GetSnapshotRequest):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = compute.Snapshot.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_get_iam_policy_rest_required_fields(request_type=compute.GetIamPolicySnapshotRequest):
    transport_class = transports.SnapshotsRestTransport
...
def test_get_iam_policy_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/v1/projects/{project}/global/snapshots/{resource}/getIamPolicy" % client.transport._host, args[1])
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/v1/projects/{project}/global/snapshots" % client.transport._host, args[1])
...
def test_list_rest_required_fields(request_type=compute.ListSnapshotsRequest):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = compute.SnapshotList.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_set_iam_policy_rest_required_fields(request_type=compute.SetIamPolicySnapshotRequest):
    transport_class = transports.SnapshotsRestTransport
...
def test_set_iam_policy_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/v1/projects/{project}/global/snapshots/{resource}/setIamPolicy" % client.transport._host, args[1])
...
def test_set_labels_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/v1/projects/{project}/global/snapshots/{resource}/setLabels" % client.transport._host, args[1])
...
def test_set_labels_unary_rest_required_fields(request_type=compute.SetLabelsSnapshotRequest):
    transport_class = transports.SnapshotsRestTransport
...
def test_test_iam_permissions_rest_required_fields(request_type=compute.TestIamPermissionsSnapshotRequest):
    transport_class = transports.SnapshotsRestTransport
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/global/snapshots/{resource}/testIamPermissions" % client.transport._host, args[1]
        )
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = SnapshotsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_rest():
    transport = SnapshotsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_delete_rest_bad_request(request_type=compute.DeleteSnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_delete_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_delete") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_delete_with_metadata"
        ...
        pb_message = compute.DeleteSnapshotRequest.pb(compute.DeleteSnapshotRequest())
...
def test_get_rest_bad_request(request_type=compute.GetSnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "snapshot": "sample2"}
    request = request_type(**request_init)
...
def test_get_rest_call_success(request_type):
    ...
    assert response.source_disk_for_recovery_checkpoint == "source_disk_for_recovery_checkpoint_value"
    ...
    assert response.source_snapshot_schedule_policy == "source_snapshot_schedule_policy_value"
    assert response.source_snapshot_schedule_policy_id == "source_snapshot_schedule_policy_id_value"
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_get") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_get_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.GetSnapshotRequest.pb(compute.GetSnapshotRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.Snapshot.to_json(compute.Snapshot())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.Snapshot(), metadata
...
def test_get_iam_policy_rest_bad_request(request_type=compute.GetIamPolicySnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_get_iam_policy") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_get_iam_policy_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.GetIamPolicySnapshotRequest.pb(compute.GetIamPolicySnapshotRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_insert_rest_bad_request(request_type=compute.InsertSnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_insert_rest_call_success(request_type):
    ...
    test_field = compute.InsertSnapshotRequest.meta.fields["snapshot_resource"]
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_insert") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_insert_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.InsertSnapshotRequest.pb(compute.InsertSnapshotRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_list_rest_bad_request(request_type=compute.ListSnapshotsRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1"}
    request = request_type(**request_init)
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_list") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_list_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.ListSnapshotsRequest.pb(compute.ListSnapshotsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.SnapshotList.to_json(compute.SnapshotList())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.SnapshotList(), metadata
...
def test_set_iam_policy_rest_bad_request(request_type=compute.SetIamPolicySnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "resource": "sample2"}
    request = request_type(**request_init)
...
def test_set_iam_policy_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.SetIamPolicySnapshotRequest.meta.fields["global_set_policy_request_resource"]
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_set_iam_policy") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_set_iam_policy_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.SetIamPolicySnapshotRequest.pb(compute.SetIamPolicySnapshotRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_set_labels_rest_bad_request(request_type=compute.SetLabelsSnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "resource": "sample2"}
    request = request_type(**request_init)
...
def test_set_labels_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.SetLabelsSnapshotRequest.meta.fields["global_set_labels_request_resource"]
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["global_set_labels_request_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_set_labels_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_set_labels") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_set_labels_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.SetLabelsSnapshotRequest.pb(compute.SetLabelsSnapshotRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsSnapshotRequest):
    client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "resource": "sample2"}
    request = request_type(**request_init)
...
def test_test_iam_permissions_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.TestIamPermissionsSnapshotRequest.meta.fields["test_permissions_request_resource"]
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SnapshotsRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
        transports.SnapshotsRestInterceptor, "post_test_iam_permissions_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.TestIamPermissionsSnapshotRequest.pb(compute.TestIamPermissionsSnapshotRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_snapshots_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.SnapshotsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_snapshots_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.compute_v1.services.snapshots.transports.SnapshotsTransport.__init__") as Transport:
...
def test_snapshots_http_transport_client_cert_source_for_mtls():
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.SnapshotsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_snapshots_client_transport_session_collision(transport_name):
    creds1 = ga_credentials.AnonymousCredentials()
    creds2 = ga_credentials.AnonymousCredentials()
    client1 = SnapshotsClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = SnapshotsClient(
        credentials=creds2,
        transport=transport_name,
    )
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.SnapshotsTransport, "_prep_wrapped_messages") as prep:
        client = SnapshotsClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = SnapshotsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
...
@pytest.mark.parametrize(
    ...
    [
        (SnapshotsClient, transports.SnapshotsRestTransport),
    ],
)
