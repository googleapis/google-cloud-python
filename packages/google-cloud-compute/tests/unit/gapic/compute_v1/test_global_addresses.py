...
from google.cloud.compute_v1.services.global_addresses.transports.base import (
...
def test_delete_rest_flattened():
    client = GlobalAddressesClient(credentials=ga_credentials.AnonymousCredentials(),)
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        client.insert(
            project="project_value", address_resource=address_resource,
        )
...
def test_global_addresses_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
...
@requires_google_auth_lt_1_25_0
def test_global_addresses_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        GlobalAddressesClient()
