...
from google.cloud.compute_v1.services.global_operations.transports.base import (
...
def test_global_operations_client_from_service_account_info(client_class):
...
def test_aggregated_list_rest(
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        return_value = compute.OperationAggregatedList(
            id="id_value",
            items={
                "key_value": compute.OperationsScopedList(
                    operations=[
                        compute.Operation(
...
def test_aggregated_list_rest_flattened():
    client = GlobalOperationsClient(credentials=ga_credentials.AnonymousCredentials(),)
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        # using the keyword arguments to the method.
        client.delete(
            project="project_value", operation="operation_value",
        )
...
def test_global_operations_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
...
@requires_google_auth_lt_1_25_0
def test_global_operations_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        GlobalOperationsClient()
...
def test_global_operations_host_no_port():
    client = GlobalOperationsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_global_operations_host_with_port():
    client = GlobalOperationsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(
