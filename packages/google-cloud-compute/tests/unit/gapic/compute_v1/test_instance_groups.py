...
def test__read_environment_variables():
    assert InstanceGroupsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert InstanceGroupsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert InstanceGroupsClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert InstanceGroupsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert InstanceGroupsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert InstanceGroupsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert InstanceGroupsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert InstanceGroupsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                InstanceGroupsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert InstanceGroupsClient._get_client_cert_source(None, False) is None
    assert InstanceGroupsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert InstanceGroupsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert InstanceGroupsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert InstanceGroupsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(InstanceGroupsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstanceGroupsClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = InstanceGroupsClient._DEFAULT_UNIVERSE
    default_endpoint = InstanceGroupsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = InstanceGroupsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert InstanceGroupsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        InstanceGroupsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == InstanceGroupsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert InstanceGroupsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert InstanceGroupsClient._get_api_endpoint(None, None, default_universe, "always") == InstanceGroupsClient.DEFAULT_MTLS_ENDPOINT
    assert (
        InstanceGroupsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == InstanceGroupsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert InstanceGroupsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert InstanceGroupsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        InstanceGroupsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert InstanceGroupsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert InstanceGroupsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert InstanceGroupsClient._get_universe_domain(None, None) == InstanceGroupsClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        InstanceGroupsClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = InstanceGroupsClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        (transports.InstanceGroupsRestTransport, "rest"),
    ],
)
...
def test_instance_groups_client_get_transport_class():
    ...
    available_transports = [
        transports.InstanceGroupsRestTransport,
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (InstanceGroupsClient, transports.InstanceGroupsRestTransport, "rest", "true"),
        (InstanceGroupsClient, transports.InstanceGroupsRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(InstanceGroupsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstanceGroupsClient))
...
@pytest.mark.parametrize("client_class", [InstanceGroupsClient])
@mock.patch.object(InstanceGroupsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstanceGroupsClient))
def test_instance_groups_client_client_api_endpoint(client_class):
...
def test_add_instances_rest_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.add_instances in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.add_instances] = mock_rpc
...
def test_add_instances_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/zones/{zone}/instanceGroups/{instance_group}/addInstances" % client.transport._host, args[1]
        )
...
def test_add_instances_unary_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = InstanceGroupsClient(
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/zones/{zone}/instanceGroups/{instance_group}" % client.transport._host, args[1]
        )
...
def test_list_instances_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/zones/{zone}/instanceGroups/{instance_group}/listInstances" % client.transport._host, args[1]
        )
...
def test_list_instances_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        sample_request = {"project": "sample1", "zone": "sample2", "instance_group": "sample3"}
        sample_request["instance_groups_list_instances_request_resource"] = compute.InstanceGroupsListInstancesRequest(
...
def test_remove_instances_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = InstanceGroupsClient(
        ...
        # Ensure method has been cached
        assert client._transport.remove_instances in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.remove_instances] = mock_rpc
...
def test_remove_instances_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/zones/{zone}/instanceGroups/{instance_group}/removeInstances" % client.transport._host, args[1]
        )
...
def test_remove_instances_unary_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = InstanceGroupsClient(
...
def test_set_named_ports_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = InstanceGroupsClient(
        ...
        # Ensure method has been cached
        assert client._transport.set_named_ports in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.set_named_ports] = mock_rpc
...
def test_set_named_ports_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/zones/{zone}/instanceGroups/{instance_group}/setNamedPorts" % client.transport._host, args[1]
        )
...
def test_set_named_ports_unary_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = InstanceGroupsClient(
...
def test_test_iam_permissions_rest_required_fields(request_type=compute.TestIamPermissionsInstanceGroupRequest):
    transport_class = transports.InstanceGroupsRestTransport
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/zones/{zone}/instanceGroups/{resource}/testIamPermissions" % client.transport._host, args[1]
        )
...
def test_transport_kind_rest():
    transport = InstanceGroupsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_add_instances_rest_bad_request(request_type=compute.AddInstancesInstanceGroupRequest):
...
def test_add_instances_rest_call_success(request_type):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    for field, value in request_init["instance_groups_add_instances_request_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_add_instances_rest_interceptors(null_interceptor):
    transport = transports.InstanceGroupsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.InstanceGroupsRestInterceptor(),
    )
    client = InstanceGroupsClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_add_instances") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_add_instances_with_metadata"
...
def test_aggregated_list_rest_bad_request(request_type=compute.AggregatedListInstanceGroupsRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_aggregated_list") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_aggregated_list_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceGroupAggregatedList.to_json(compute.InstanceGroupAggregatedList())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.InstanceGroupAggregatedList(), metadata
...
def test_delete_rest_bad_request(request_type=compute.DeleteInstanceGroupRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "zone": "sample2", "instance_group": "sample3"}
    request = request_type(**request_init)
...
def test_delete_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_delete") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_delete_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_get_rest_bad_request(request_type=compute.GetInstanceGroupRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "zone": "sample2", "instance_group": "sample3"}
    request = request_type(**request_init)
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_get") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_get_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceGroup.to_json(compute.InstanceGroup())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.InstanceGroup(), metadata
...
def test_insert_rest_bad_request(request_type=compute.InsertInstanceGroupRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_insert_rest_call_success(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["instance_group_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_insert") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_insert_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_list_rest_bad_request(request_type=compute.ListInstanceGroupsRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "zone": "sample2"}
    request = request_type(**request_init)
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_list") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_list_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceGroupList.to_json(compute.InstanceGroupList())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.InstanceGroupList(), metadata
...
def test_list_instances_rest_bad_request(request_type=compute.ListInstancesInstanceGroupsRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "zone": "sample2", "instance_group": "sample3"}
    request = request_type(**request_init)
...
def test_list_instances_rest_call_success(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["instance_groups_list_instances_request_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_list_instances_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_list_instances") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_list_instances_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceGroupsListInstances.to_json(compute.InstanceGroupsListInstances())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.InstanceGroupsListInstances(), metadata
...
def test_remove_instances_rest_bad_request(request_type=compute.RemoveInstancesInstanceGroupRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "zone": "sample2", "instance_group": "sample3"}
    request = request_type(**request_init)
...
def test_remove_instances_rest_call_success(request_type):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["instance_groups_remove_instances_request_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_remove_instances_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_remove_instances") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_remove_instances_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_set_named_ports_rest_bad_request(request_type=compute.SetNamedPortsInstanceGroupRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "zone": "sample2", "instance_group": "sample3"}
    request = request_type(**request_init)
...
def test_set_named_ports_rest_call_success(request_type):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["instance_groups_set_named_ports_request_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_set_named_ports_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_set_named_ports") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_set_named_ports_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsInstanceGroupRequest):
    client = InstanceGroupsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_test_iam_permissions_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.TestIamPermissionsInstanceGroupRequest.meta.fields["test_permissions_request_resource"]
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstanceGroupsRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "post_test_iam_permissions_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.InstanceGroupsRestInterceptor, "pre_test_iam_permissions"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.TestIamPermissionsInstanceGroupRequest.pb(compute.TestIamPermissionsInstanceGroupRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_add_instances_unary_empty_call_rest():
    client = InstanceGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport.add_instances), "__call__") as call:
        client.add_instances_unary(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_remove_instances_unary_empty_call_rest():
    client = InstanceGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(type(client.transport.remove_instances), "__call__") as call:
        client.remove_instances_unary(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_set_named_ports_unary_empty_call_rest():
    client = InstanceGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(type(client.transport.set_named_ports), "__call__") as call:
        client.set_named_ports_unary(request=None)
...
def test_instance_groups_client_transport_session_collision(transport_name):
    ...
    client1 = InstanceGroupsClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = InstanceGroupsClient(
        credentials=creds2,
        transport=transport_name,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (InstanceGroupsClient, transports.InstanceGroupsRestTransport),
    ],
)
