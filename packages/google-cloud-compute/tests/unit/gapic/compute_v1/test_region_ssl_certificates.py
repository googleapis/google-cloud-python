...
def test_delete_rest_flattened(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        assert path_template.validate(
            "https://%s/compute/v1/projects/{project}/regions/{region}/sslCertificates/{ssl_certificate}"
...
def test_insert_rest(
    ...
    request_init["ssl_certificate_resource"] = compute.SslCertificate(
        certificate="certificate_value"
    )
    request = request_type(request_init)
...
def test_insert_rest_flattened(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "https://%s/compute/v1/projects/{project}/regions/{region}/sslCertificates"
            % client.transport._host,
            args[1],
        )
