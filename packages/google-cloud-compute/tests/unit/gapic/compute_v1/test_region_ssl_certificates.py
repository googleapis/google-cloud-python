...
from google.cloud.compute_v1.services.region_ssl_certificates.transports.base import (
...
def test_region_ssl_certificates_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("client_class", [RegionSslCertificatesClient,])
def test_region_ssl_certificates_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        assert "ssl_certificate_value" in http_call[1] + str(body) + str(params)
...
def test_region_ssl_certificates_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
...
def test_region_ssl_certificates_host_no_port():
    client = RegionSslCertificatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_region_ssl_certificates_host_with_port():
    client = RegionSslCertificatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(
