...
def test_expand_ip_cidr_range_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseExpandIpCidrRange,
        "_BaseExpandIpCidrRange__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_set_private_ip_google_access_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseSetPrivateIpGoogleAccess,
        "_BaseSetPrivateIpGoogleAccess__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.SubnetworkAggregatedList.to_json(
            compute.SubnetworkAggregatedList()
        )
...
def test_list_usable_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.UsableSubnetworksAggregatedList.to_json(
            compute.UsableSubnetworksAggregatedList()
        )
        req.return_value.content = return_value
...
def test_expand_ip_cidr_range_unary_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
        type(client.transport.expand_ip_cidr_range), "__call__"
    ) as call:
...
def test_set_private_ip_google_access_unary_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
        type(client.transport.set_private_ip_google_access), "__call__"
    ) as call:
