...
def test__get_default_mtls_endpoint():
    ...
    assert (
        ServiceAttachmentsClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        ServiceAttachmentsClient._get_client_cert_source(
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                ServiceAttachmentsClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(ServiceAttachmentsClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ServiceAttachmentsClient._DEFAULT_UNIVERSE
    default_endpoint = ServiceAttachmentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = ServiceAttachmentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        ServiceAttachmentsClient._get_api_endpoint(
    ...
    assert (
        ServiceAttachmentsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        ServiceAttachmentsClient._get_api_endpoint(
    ...
    assert (
        ServiceAttachmentsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        ServiceAttachmentsClient._get_api_endpoint(
...
def test__get_universe_domain():
    ...
    assert (
        ServiceAttachmentsClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        ServiceAttachmentsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        ServiceAttachmentsClient._get_universe_domain(None, None)
        == ServiceAttachmentsClient._DEFAULT_UNIVERSE
    )
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.ServiceAttachmentAggregatedList.to_json(
            compute.ServiceAttachmentAggregatedList()
        )
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.ServiceAttachmentList.to_json(
            compute.ServiceAttachmentList()
        )
        req.return_value.content = return_value
