...
def test__get_client_cert_source():
    ...
    assert (
        VpnGatewaysClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        VpnGatewaysClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                VpnGatewaysClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                VpnGatewaysClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(VpnGatewaysClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = VpnGatewaysClient._DEFAULT_UNIVERSE
    default_endpoint = VpnGatewaysClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = VpnGatewaysClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        VpnGatewaysClient._get_api_endpoint(
    ...
    assert (
        ...
        == VpnGatewaysClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        VpnGatewaysClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        VpnGatewaysClient._get_api_endpoint(None, None, default_universe, "always")
        == VpnGatewaysClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        VpnGatewaysClient._get_api_endpoint(
    ...
    assert (
        VpnGatewaysClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        VpnGatewaysClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        VpnGatewaysClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        VpnGatewaysClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        VpnGatewaysClient._get_universe_domain(None, None)
        == VpnGatewaysClient._DEFAULT_UNIVERSE
    )
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.VpnGatewayAggregatedList.to_json(
            compute.VpnGatewayAggregatedList()
        )
...
def test_get_status_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.VpnGatewaysGetStatusResponse.to_json(
