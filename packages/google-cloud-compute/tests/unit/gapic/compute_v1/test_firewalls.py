...
def test__get_default_mtls_endpoint():
    ...
    assert (
        FirewallsClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        FirewallsClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        FirewallsClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                FirewallsClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                FirewallsClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(FirewallsClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = FirewallsClient._DEFAULT_UNIVERSE
    default_endpoint = FirewallsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = FirewallsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
    ...
    assert (
        ...
        == FirewallsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        FirewallsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        FirewallsClient._get_api_endpoint(None, None, default_universe, "always")
        == FirewallsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        FirewallsClient._get_api_endpoint(
    ...
    assert (
        FirewallsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        FirewallsClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        FirewallsClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        FirewallsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        FirewallsClient._get_universe_domain(None, None)
        == FirewallsClient._DEFAULT_UNIVERSE
    )
...
def test_test_iam_permissions_rest_required_fields(
    request_type=compute.TestIamPermissionsFirewallRequest,
):
    transport_class = transports.FirewallsRestTransport
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/v1/projects/{project}/global/firewalls/{resource}/testIamPermissions"
            % client.transport._host,
            args[1],
        )
...
def test_test_iam_permissions_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.TestIamPermissionsFirewallRequest.meta.fields[
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(
            transports.FirewallsRestInterceptor, "post_test_iam_permissions"
        ) as post,
        mock.patch.object(
            transports.FirewallsRestInterceptor,
            "post_test_iam_permissions_with_metadata",
        ) as post_with_metadata,
        mock.patch.object(
            transports.FirewallsRestInterceptor, "pre_test_iam_permissions"
        ) as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.TestIamPermissionsFirewallRequest.pb(
            compute.TestIamPermissionsFirewallRequest()
        )
        transcode.return_value = {
            "method": "post",
