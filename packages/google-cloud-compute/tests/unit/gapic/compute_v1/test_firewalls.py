...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert FirewallsClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert FirewallsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert FirewallsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                FirewallsClient._use_client_cert_effective()
...
def test_transport_kind_rest():
    transport = FirewallsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_delete_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.FirewallsRestInterceptor, "post_delete") as post, mock.patch.object(
        transports.FirewallsRestInterceptor, "post_delete_with_metadata"
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FirewallsRestInterceptor, "post_get") as post, mock.patch.object(
        transports.FirewallsRestInterceptor, "post_get_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_insert_rest_call_success(request_type):
    ...
    request_init["firewall_resource"] = {
        "allowed": [{"I_p_protocol": "I_p_protocol_value", "ports": ["ports_value1", "ports_value2"]}],
        "creation_timestamp": "creation_timestamp_value",
        "denied": [{"I_p_protocol": "I_p_protocol_value", "ports": ["ports_value1", "ports_value2"]}],
        "description": "description_value",
        "destination_ranges": ["destination_ranges_value1", "destination_ranges_value2"],
        "direction": "direction_value",
        "disabled": True,
        "id": 205,
        "kind": "kind_value",
        ...
        "source_service_accounts": ["source_service_accounts_value1", "source_service_accounts_value2"],
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FirewallsRestInterceptor, "post_insert") as post, mock.patch.object(
        transports.FirewallsRestInterceptor, "post_insert_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FirewallsRestInterceptor, "post_list") as post, mock.patch.object(
        transports.FirewallsRestInterceptor, "post_list_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_patch_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FirewallsRestInterceptor, "post_patch") as post, mock.patch.object(
        transports.FirewallsRestInterceptor, "post_patch_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_update_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FirewallsRestInterceptor, "post_update") as post, mock.patch.object(
        transports.FirewallsRestInterceptor, "post_update_with_metadata"
    ) as post_with_metadata, mock.patch.object(
