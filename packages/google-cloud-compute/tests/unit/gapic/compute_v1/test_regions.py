...
def test_regions_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize(
    "transport_class,transport_name", [(transports.RegionsRestTransport, "rest"),]
)
...
@pytest.mark.parametrize("client_class", [RegionsClient,])
def test_regions_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(
...
@pytest.mark.parametrize("request_type", [compute.GetRegionRequest, dict,])
...
def test_get_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("project", "region",)))
...
def test_get_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "https://%s/compute/v1/projects/{project}/regions/{region}"
...
def test_get_rest_flattened_error(transport: str = "rest"):
    ...
    with pytest.raises(ValueError):
        client.get(
            compute.GetRegionRequest(), project="project_value", region="region_value",
        )
...
@pytest.mark.parametrize("request_type", [compute.ListRegionsRequest, dict,])
...
def test_list_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "https://%s/compute/v1/projects/{project}/regions" % client.transport._host,
            args[1],
        )
...
def test_list_rest_flattened_error(transport: str = "rest"):
    ...
    with pytest.raises(ValueError):
        client.list(
            compute.ListRegionsRequest(), project="project_value",
        )
...
def test_list_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = (
            compute.RegionList(
                items=[compute.Region(), compute.Region(), compute.Region(),],
                next_page_token="abc",
            ),
            compute.RegionList(items=[], next_page_token="def",),
            compute.RegionList(items=[compute.Region(),], next_page_token="ghi",),
            compute.RegionList(items=[compute.Region(), compute.Region(),],),
        )
...
def test_credentials_transport_error():
    ...
    with pytest.raises(ValueError):
        client = RegionsClient(client_options=options, transport=transport,)
...
@pytest.mark.parametrize("transport_class", [transports.RegionsRestTransport,])
...
def test_regions_host_with_port():
    client = RegionsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(
...
@pytest.mark.parametrize(
    "client_class,transport_class", [(RegionsClient, transports.RegionsRestTransport),]
)
