...
def test__get_default_mtls_endpoint():
    ...
    assert RegionsClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
...
def test__get_client_cert_source():
    ...
    assert (
        RegionsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    )
    assert (
        RegionsClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                RegionsClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(RegionsClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = RegionsClient._DEFAULT_UNIVERSE
    default_endpoint = RegionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = RegionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        RegionsClient._get_api_endpoint(
    ...
    assert (
        ...
        == RegionsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        RegionsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        RegionsClient._get_api_endpoint(None, None, default_universe, "always")
        == RegionsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        RegionsClient._get_api_endpoint(
    ...
    assert (
        RegionsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        RegionsClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        RegionsClient._get_universe_domain(client_universe_domain, universe_domain_env)
        == client_universe_domain
    )
    assert (
        RegionsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        RegionsClient._get_universe_domain(None, None)
        == RegionsClient._DEFAULT_UNIVERSE
    )
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(transports.RegionsRestInterceptor, "post_get") as post,
        mock.patch.object(
            transports.RegionsRestInterceptor, "post_get_with_metadata"
        ) as post_with_metadata,
        mock.patch.object(transports.RegionsRestInterceptor, "pre_get") as pre,
    ):
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with (
        mock.patch.object(type(client.transport._session), "request") as req,
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(transports.RegionsRestInterceptor, "post_list") as post,
        mock.patch.object(
            transports.RegionsRestInterceptor, "post_list_with_metadata"
        ) as post_with_metadata,
        mock.patch.object(transports.RegionsRestInterceptor, "pre_list") as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
