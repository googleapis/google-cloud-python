...
def test_get_rest_flattened():
    client = RegionsClient(credentials=ga_credentials.AnonymousCredentials(),)
...
def test_list_rest(transport: str = "rest", request_type=compute.ListRegionsRequest):
    client = RegionsClient(
        credentials=ga_credentials.AnonymousCredentials(), transport=transport,
    )
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        return_value = compute.RegionList(
            id="id_value",
            items=[compute.Region(creation_timestamp="creation_timestamp_value")],
            kind="kind_value",
    ...
    assert response.items == [
        compute.Region(creation_timestamp="creation_timestamp_value")
    ]
    assert response.kind == "kind_value"
...
def test_regions_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(
        google.auth, "load_credentials_from_file", autospec=True
    ) as load_creds, mock.patch(
