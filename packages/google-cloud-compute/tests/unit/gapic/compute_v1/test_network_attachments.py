...
def test__get_default_mtls_endpoint():
    ...
    assert (
        NetworkAttachmentsClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        NetworkAttachmentsClient._get_client_cert_source(
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                NetworkAttachmentsClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(NetworkAttachmentsClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = NetworkAttachmentsClient._DEFAULT_UNIVERSE
    default_endpoint = NetworkAttachmentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = NetworkAttachmentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
    ...
    assert (
        NetworkAttachmentsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        NetworkAttachmentsClient._get_api_endpoint(
    ...
    assert (
        NetworkAttachmentsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        NetworkAttachmentsClient._get_api_endpoint(
...
def test__get_universe_domain():
    ...
    assert (
        NetworkAttachmentsClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        NetworkAttachmentsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        NetworkAttachmentsClient._get_universe_domain(None, None)
        == NetworkAttachmentsClient._DEFAULT_UNIVERSE
    )
...
def test_patch_unary_rest_required_fields(
    request_type=compute.PatchNetworkAttachmentRequest,
):
    transport_class = transports.NetworkAttachmentsRestTransport
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.NetworkAttachmentAggregatedList.to_json(
            compute.NetworkAttachmentAggregatedList()
        )
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.NetworkAttachmentList.to_json(
            compute.NetworkAttachmentList()
        )
        req.return_value.content = return_value
...
def test_patch_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.PatchNetworkAttachmentRequest.meta.fields[
        "network_attachment_resource"
    ]
