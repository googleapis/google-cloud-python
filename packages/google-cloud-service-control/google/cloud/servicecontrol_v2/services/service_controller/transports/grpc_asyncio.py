...
from .base import DEFAULT_CLIENT_INFO, ServiceControllerTransport
...
class ServiceControllerGrpcAsyncIOTransport(ServiceControllerTransport):
    ...
    @property
    def check(self) -> Callable[[service_controller.CheckRequest], Awaitable[service_controller.CheckResponse]]:
        ...
        if "check" not in self._stubs:
            self._stubs["check"] = self._logged_channel.unary_unary(
    ...
    @property
    def report(self) -> Callable[[service_controller.ReportRequest], Awaitable[service_controller.ReportResponse]]:
        ...
        if "report" not in self._stubs:
            self._stubs["report"] = self._logged_channel.unary_unary(
