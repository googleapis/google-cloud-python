...
class ExecutePipelineRequest(proto.Message):
    ...
    structured_pipeline: pipeline.StructuredPipeline = proto.Field(
        proto.MESSAGE,
        number=2,
        oneof="pipeline_type",
        message=pipeline.StructuredPipeline,
    )
    transaction: bytes = proto.Field(
        proto.BYTES,
        number=5,
...
class ExecutePipelineResponse(proto.Message):
    ...
    Attributes:
        transaction (bytes):
            ...
            This field is only specified as part of the first response
            from the server, alongside the ``results`` field when the
            original request specified
            [ExecuteRequest.new_transaction][].
        results (MutableSequence[google.cloud.firestore_v1.types.Document]):
            An ordered batch of results returned executing a pipeline.

            The batch size is variable, and can even be zero for when
    ...
    results: MutableSequence[gf_document.Document] = proto.RepeatedField(
        proto.MESSAGE,
        number=2,
        message=gf_document.Document,
    )
    ...
    explain_stats: gf_explain_stats.ExplainStats = proto.Field(
        proto.MESSAGE,
        number=4,
        message=gf_explain_stats.ExplainStats,
    )
