...
from google.cloud.firestore_v1._helpers import GeoPoint, decode_dict, decode_value
...
def _extract_canonical_value(val: Any) -> Any:
    ...
    if hasattr(val, "_pb") or hasattr(val, "WhichOneof"):
        ...
        if vtype == "null_value":
            ...
            decoded = decode_dict(value_pb.map_value.fields, client=None, decode_bson=True)
...
class Order(object):
    ...
    def _to_decimal_or_nan(val) -> Any:
        ...
        if isinstance(val, (int, BSONInt32)):
            return decimal.Decimal(int(val))
    ...
    def compare_resource_paths(left, right) -> int:
        p_left = left.path if hasattr(left, "path") else (left.value if hasattr(left, "value") else str(left))
        p_right = right.path if hasattr(right, "path") else (right.value if hasattr(right, "value") else str(right))
