...
def test_delete_document_rest_required_fields(
    ...
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("currentDocument",))
...
def test_execute_pipeline_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseExecutePipeline,
...
def test_partition_query_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BasePartitionQuery,
...
def test_list_collection_ids_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseListCollectionIds,
...
def test_batch_write_rest_required_fields(request_type=firestore.BatchWriteRequest):
    ...
    default_values = getattr(
        transport_class._BaseBatchWrite,
...
def test_batch_get_documents_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(
            transports.FirestoreRestInterceptor,
...
def test_commit_rest_interceptors(null_interceptor):
    ...
    with (
        mock.patch.object(type(client.transport._session), "request") as req,
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(transports.FirestoreRestInterceptor, "post_commit") as post,
        mock.patch.object(
            transports.FirestoreRestInterceptor, "post_commit_with_metadata"
        ) as post_with_metadata,
        mock.patch.object(transports.FirestoreRestInterceptor, "pre_commit") as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_rollback_rest_interceptors(null_interceptor):
    ...
    with (
        mock.patch.object(type(client.transport._session), "request") as req,
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(transports.FirestoreRestInterceptor, "pre_rollback") as pre,
    ):
        pre.assert_not_called()
...
def test_run_query_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(transports.FirestoreRestInterceptor, "pre_run_query") as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
