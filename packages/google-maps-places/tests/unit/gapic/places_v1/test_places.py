...
from google.maps.places_v1.services.places import PlacesAsyncClient, PlacesClient, transports
...
def test_places_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "places.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://places.googleapis.com"
        )
...
def test_autocomplete_places_rest_required_fields(request_type=places_service.AutocompletePlacesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).autocomplete_places._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = PlacesClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = PlacesClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = PlacesClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = PlacesAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = PlacesAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = PlacesClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_search_nearby_rest_bad_request(request_type=places_service.SearchNearbyRequest):
    client = PlacesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_search_nearby_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.PlacesRestInterceptor, "post_search_nearby") as post, mock.patch.object(
...
def test_search_text_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PlacesRestInterceptor, "post_search_text") as post, mock.patch.object(
...
def test_get_photo_media_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PlacesRestInterceptor, "post_get_photo_media") as post, mock.patch.object(
...
def test_get_place_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PlacesRestInterceptor, "post_get_place") as post, mock.patch.object(
...
def test_autocomplete_places_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PlacesRestInterceptor, "post_autocomplete_places") as post, mock.patch.object(
...
def test_places_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.PlacesTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.PlacesGrpcTransport, grpc_helpers), (transports.PlacesGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_places_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.PlacesRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_places_host_no_port(transport_name):
    ...
    assert client.transport._host == ("places.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://places.googleapis.com")
...
def test_places_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "places.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://places.googleapis.com:8000"
    )
...
def test_client_ctx():
    ...
    for transport in transports:
        client = PlacesClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
