...
class PlacesGrpcTransport(PlacesTransport):
    ...
    def search_nearby(
        ...
        if "search_nearby" not in self._stubs:
            self._stubs["search_nearby"] = self._logged_channel.unary_unary(
    ...
    def search_text(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "search_text" not in self._stubs:
            self._stubs["search_text"] = self._logged_channel.unary_unary(
                "/google.maps.places.v1.Places/SearchText",
                request_serializer=places_service.SearchTextRequest.serialize,
    ...
    def get_photo_media(
        ...
        if "get_photo_media" not in self._stubs:
            self._stubs["get_photo_media"] = self._logged_channel.unary_unary(
    ...
    def get_place(self) -> Callable[[places_service.GetPlaceRequest], place.Place]:
        ...
        if "get_place" not in self._stubs:
            self._stubs["get_place"] = self._logged_channel.unary_unary(
    ...
    def autocomplete_places(
        ...
        if "autocomplete_places" not in self._stubs:
            self._stubs["autocomplete_places"] = self._logged_channel.unary_unary(
