...
class DeveloperConnectClient(metaclass=DeveloperConnectClientMeta):
    ...
    def parse_user_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/accountConnectors/(?P<account_connector>.+?)/users/(?P<user>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def __init__(
        ...
        *,
        ...
        self._api_endpoint = self._api_endpoint or DeveloperConnectClient._get_api_endpoint(
