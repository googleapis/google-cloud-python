...
from google.cloud.language_v1.services.language_service import LanguageServiceAsyncClient, LanguageServiceClient, transports
...
def test_analyze_sentiment_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v1/documents:analyzeSentiment" % client.transport._host, args[1])
...
def test_analyze_entities_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/documents:analyzeEntities" % client.transport._host, args[1])
...
def test_analyze_entity_sentiment_rest_required_fields(request_type=language_service.AnalyzeEntitySentimentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).analyze_entity_sentiment._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_analyze_entity_sentiment_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/documents:analyzeEntitySentiment" % client.transport._host, args[1])
...
def test_analyze_syntax_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/documents:analyzeSyntax" % client.transport._host, args[1])
...
def test_classify_text_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/documents:classifyText" % client.transport._host, args[1])
...
def test_moderate_text_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/documents:moderateText" % client.transport._host, args[1])
...
def test_annotate_text_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/documents:annotateText" % client.transport._host, args[1])
...
def test_analyze_entity_sentiment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.LanguageServiceRestInterceptor, "post_analyze_entity_sentiment") as post, mock.patch.object(
        transports.LanguageServiceRestInterceptor, "post_analyze_entity_sentiment_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_analyze_syntax_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.LanguageServiceRestInterceptor, "post_analyze_syntax") as post, mock.patch.object(
