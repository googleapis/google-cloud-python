...
def test_language_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.language_v1.services.language_service.transports.LanguageServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_analyze_entity_sentiment_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._transport.analyze_entity_sentiment
...
@pytest.mark.asyncio
async def test_analyze_entity_sentiment_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.analyze_entity_sentiment
            in client._client._transport._wrapped_methods
        )
...
@pytest.mark.asyncio
async def test_analyze_syntax_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.analyze_syntax
            in client._client._transport._wrapped_methods
        )
...
def test_analyze_entity_sentiment_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseAnalyzeEntitySentiment,
        "_BaseAnalyzeEntitySentiment__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = language_service.AnalyzeEntitySentimentResponse.pb(
                return_value
            )
            json_return_value = json_format.MessageToJson(return_value)
...
def test_analyze_syntax_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseAnalyzeSyntax,
        "_BaseAnalyzeSyntax__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_moderate_text_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/documents:moderateText" % client.transport._host, args[1]
        )
...
def test_language_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.language_v1.services.language_service.transports.LanguageServiceTransport.__init__"
    ) as Transport:
