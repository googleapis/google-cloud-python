...
from google.cloud.eventarc_publishing_v1.services.publisher import PublisherAsyncClient, PublisherClient, transports
...
def test_publisher_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "eventarcpublishing.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://eventarcpublishing.googleapis.com"
        )
...
def test_publish_channel_connection_events(request_type, transport: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.publish_channel_connection_events), "__call__") as call:
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = PublisherClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = PublisherClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = PublisherClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = PublisherAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = PublisherAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = PublisherClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_publish_channel_connection_events_rest_bad_request(request_type=publisher.PublishChannelConnectionEventsRequest):
    client = PublisherClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_publish_channel_connection_events_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.PublisherRestInterceptor, "post_publish_channel_connection_events") as post, mock.patch.object(
        transports.PublisherRestInterceptor, "post_publish_channel_connection_events_with_metadata"
...
def test_publish_events_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PublisherRestInterceptor, "post_publish_events") as post, mock.patch.object(
...
def test_publish_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.PublisherRestInterceptor, "post_publish") as post, mock.patch.object(
...
def test_publisher_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.PublisherTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.PublisherGrpcTransport, grpc_helpers), (transports.PublisherGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_publisher_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.PublisherRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_publisher_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "eventarcpublishing.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://eventarcpublishing.googleapis.com:8000"
    )
...
def test_client_ctx():
    ...
    for transport in transports:
        client = PublisherClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
