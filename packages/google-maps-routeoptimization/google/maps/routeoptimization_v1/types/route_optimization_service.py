...
class OptimizeToursRequest(proto.Message):
    ...
    Attributes:
        ...
        populate_transition_polylines (bool):
            If true, polylines and route tokens will be populated in
            response
...
class OptimizeToursResponse(proto.Message):
    ...
    Attributes:
        ...
        processed_request (google.maps.routeoptimization_v1.types.OptimizeToursRequest):
            In some cases we modify the incoming request
            before solving it, i.e. adding costs. If
            solving_mode == TRANSFORM_AND_RETURN_REQUEST,
            the modified request is returned here.
    ...
    class Metrics(proto.Message):
        ...
        Attributes:
            ...
            costs (MutableMapping[str, float]):
                Cost of the solution, broken down by
                cost-related request fields. The keys are proto
                paths, relative to the input
                OptimizeToursRequest, e.g.
                "model.shipments.pickups.cost", and the values
                are the total cost generated by the
                corresponding cost field, aggregated over the
                whole solution. In other words,
                costs["model.shipments.pickups.cost"] is the sum
                of all pickup costs over the solution. All costs
                defined in the model are reported in detail here
                with the exception of costs related to
                TransitionAttributes that are only reported in
                an aggregated way as of 2022/01.
            total_cost (float):
...
class Shipment(proto.Message):
    ...
    Attributes:
        ...
        load_demands (MutableMapping[str, google.maps.routeoptimization_v1.types.Shipment.Load]):
            Load demands of the shipment (for example
            weight, volume, number of pallets etc). The keys
            in the map should be identifiers describing the
            type of the corresponding load, ideally also
            including the units. For example: "weight_kg",
            "volume_gallons", "pallet_count", etc. If a
            given key does not appear in the map, the
            corresponding load is considered as null.
        penalty_cost (float):
...
class ShipmentTypeIncompatibility(proto.Message):
    ...
    shipment_type. The appearance of incompatible shipments on the
    same route is restricted based on the incompatibility mode.
...
class ShipmentTypeRequirement(proto.Message):
    ...
    shipment_type. The specifics of the requirement are defined by
    the requirement mode.
...
class Vehicle(proto.Message):
    ...
    class LoadLimit(proto.Message):
        ...
        class LoadCost(proto.Message):
            ...
            Attributes:
                load_threshold (int):
                    Amount of load above which the cost of moving a
                    unit of load changes from
                    cost_per_unit_below_threshold to
    ...
    class DurationLimit(proto.Message):
        ...
        Attributes:
            max_duration (google.protobuf.duration_pb2.Duration):
                A hard limit constraining the duration to be at
                most max_duration.
...
class DistanceLimit(proto.Message):
    ...
    Attributes:
        max_meters (int):
            A hard limit constraining the distance to be at
            most max_meters. The limit must be nonnegative.

            This field is a member of `oneof`_ ``_max_meters``.
        soft_max_meters (int):
            A soft limit not enforcing a maximum distance
            limit, but when violated results in a cost which
            adds up to other costs defined in the model,
            with the same unit.

            If defined soft_max_meters must be less than
            max_meters and must be nonnegative.
...
class Waypoint(proto.Message):
    ...
    Attributes:
        ...
        vehicle_stopover (bool):
            Indicates that the waypoint is meant for
            vehicles to stop at, where the intention is to
            either pick up or drop off. This option works
            only for the 'DRIVING' travel mode, and when the
...
class ShipmentRoute(proto.Message):
    ...
    Attributes:
        ...
        visits (MutableSequence[google.maps.routeoptimization_v1.types.ShipmentRoute.Visit]):
            ...
            If this field is empty, the vehicle is
        ...
        route_costs (MutableMapping[str, float]):
            Cost of the route, broken down by cost-related
            request fields. The keys are proto paths,
            relative to the input OptimizeToursRequest, e.g.
            "model.shipments.pickups.cost", and the values
            are the total cost generated by the
            corresponding cost field, aggregated over the
            whole route. In other words,
            costs["model.shipments.pickups.cost"] is the sum
            of all pickup costs over the route. All costs
            defined in the model are reported in detail here
            with the exception of costs related to
            TransitionAttributes that are only reported in
            an aggregated way as of 2022/01.
    ...
    class Transition(proto.Message):
        ...
        Attributes:
            ...
            route_polyline (google.maps.routeoptimization_v1.types.ShipmentRoute.EncodedPolyline):
                ...
                is set to true.
            route_token (str):
                Output only. An opaque token that can be passed to
                `Navigation
                ...
                even if the service returns the exact same route. This field
...
class OptimizeToursValidationError(proto.Message):
    ...
    Attributes:
        code (int):
            ...
            The fields following this section provide more context about
            the error.
            ...
            *MULTIPLE ERRORS*: When there are multiple errors, the
            ...
            errors may cause the validation process to skip other
            errors.
