...
from .base import DEFAULT_CLIENT_INFO, ResourceSettingsServiceTransport
from .grpc import ResourceSettingsServiceGrpcTransport
...
class ResourceSettingsServiceGrpcAsyncIOTransport(ResourceSettingsServiceTransport):
    """gRPC AsyncIO backend transport for ResourceSettingsService.
    ...
    def list_settings(
        ...
        Returns:
            Callable[[~.ListSettingsRequest],
                    Awaitable[~.ListSettingsResponse]]:
        ...
        if "list_settings" not in self._stubs:
            self._stubs["list_settings"] = self._logged_channel.unary_unary(
    ...
    def get_setting(
        ...
        [resource_settings.GetSettingRequest], Awaitable[resource_settings.Setting]
    ]:
        ...
        if "get_setting" not in self._stubs:
            self._stubs["get_setting"] = self._logged_channel.unary_unary(
    ...
    def update_setting(
        ...
        [resource_settings.UpdateSettingRequest], Awaitable[resource_settings.Setting]
    ]:
        ...
        if "update_setting" not in self._stubs:
            self._stubs["update_setting"] = self._logged_channel.unary_unary(
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.list_settings: self._wrap_method(
                self.list_settings,
            ...
            self.get_setting: self._wrap_method(
                self.get_setting,
                default_retry=retries.AsyncRetry(
                    initial=1.0,
                    maximum=10.0,
            ...
            self.update_setting: self._wrap_method(
                self.update_setting,
                default_retry=retries.AsyncRetry(
                    initial=1.0,
                    maximum=10.0,
