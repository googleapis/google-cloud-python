...
from google.cloud.dialogflow_v2.services.versions import VersionsAsyncClient, VersionsClient, pagers, transports
...
def test__read_environment_variables():
    assert VersionsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert VersionsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                VersionsClient._read_environment_variables()
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert VersionsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert VersionsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert VersionsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert VersionsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert VersionsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                VersionsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert VersionsClient._get_client_cert_source(None, False) is None
    assert VersionsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert VersionsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert VersionsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert VersionsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(VersionsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(VersionsClient))
@mock.patch.object(VersionsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(VersionsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = VersionsClient._DEFAULT_UNIVERSE
    default_endpoint = VersionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = VersionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert VersionsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert VersionsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == VersionsClient.DEFAULT_MTLS_ENDPOINT
    assert VersionsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert VersionsClient._get_api_endpoint(None, None, default_universe, "always") == VersionsClient.DEFAULT_MTLS_ENDPOINT
    assert VersionsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == VersionsClient.DEFAULT_MTLS_ENDPOINT
    assert VersionsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert VersionsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        VersionsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert VersionsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert VersionsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert VersionsClient._get_universe_domain(None, None) == VersionsClient._DEFAULT_UNIVERSE
...
def test_versions_client_service_account_always_use_jwt(transport_class, transport_name):
...
@mock.patch.object(VersionsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(VersionsClient))
@mock.patch.object(VersionsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(VersionsAsyncClient))
def test_versions_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (VersionsAsyncClient, transports.VersionsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (VersionsClient, transports.VersionsRestTransport, "rest", None),
    ],
)
def test_versions_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_versions_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_list_versions_rest_required_fields(request_type=version.ListVersionsRequest):
...
def test_list_versions_rest_unset_required_fields():
    transport = transports.VersionsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_versions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v2/{parent=projects/*/agent}/versions" % client.transport._host, args[1])
...
def test_get_version_rest_required_fields(request_type=version.GetVersionRequest):
    transport_class = transports.VersionsRestTransport
...
def test_get_version_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/agent/versions/sample2"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2/{name=projects/*/agent/versions/*}" % client.transport._host, args[1])
...
def test_create_version_rest_required_fields(request_type=gcd_version.CreateVersionRequest):
    transport_class = transports.VersionsRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = gcd_version.Version()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, "request") as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = gcd_version.Version.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_update_version_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"version": {"name": "projects/sample1/agent/versions/sample2"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2/{version.name=projects/*/agent/versions/*}" % client.transport._host, args[1])
...
def test_delete_version_rest_required_fields(request_type=version.DeleteVersionRequest):
    transport_class = transports.VersionsRestTransport
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = VersionsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = VersionsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = VersionsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_create_version_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.create_version), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = gcd_version.CreateVersionRequest()
...
def test_update_version_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.update_version), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = gcd_version.UpdateVersionRequest()
...
def test_transport_kind_grpc_asyncio():
    transport = VersionsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = VersionsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = VersionsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_versions_rest_bad_request(request_type=version.ListVersionsRequest):
    client = VersionsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_versions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.VersionsRestInterceptor, "post_list_versions") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = version.ListVersionsResponse.to_json(version.ListVersionsResponse())
...
def test_get_version_rest_bad_request(request_type=version.GetVersionRequest):
    client = VersionsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/agent/versions/sample2"}
    request = request_type(**request_init)
...
def test_get_version_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.VersionsRestInterceptor, "post_get_version") as post, mock.patch.object(
...
def test_create_version_rest_call_success(request_type):
    ...
    test_field = gcd_version.CreateVersionRequest.meta.fields["version"]
...
def test_create_version_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.VersionsRestInterceptor, "post_create_version") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_version.CreateVersionRequest.pb(gcd_version.CreateVersionRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcd_version.Version.to_json(gcd_version.Version())
        req.return_value.content = return_value
...
def test_update_version_rest_bad_request(request_type=gcd_version.UpdateVersionRequest):
    client = VersionsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"version": {"name": "projects/sample1/agent/versions/sample2"}}
    request = request_type(**request_init)
...
def test_update_version_rest_call_success(request_type):
    ...
    request_init = {"version": {"name": "projects/sample1/agent/versions/sample2"}}
    request_init["version"] = {
        "name": "projects/sample1/agent/versions/sample2",
        "description": "description_value",
        "version_number": 1518,
        "create_time": {"seconds": 751, "nanos": 543},
        "status": 1,
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = gcd_version.UpdateVersionRequest.meta.fields["version"]
...
def test_update_version_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.VersionsRestInterceptor, "post_update_version") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_version.UpdateVersionRequest.pb(gcd_version.UpdateVersionRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_version_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.VersionsRestInterceptor, "pre_delete_version") as pre:
        pre.assert_not_called()
...
def test_versions_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.VersionsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.VersionsGrpcTransport, grpc_helpers), (transports.VersionsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_versions_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.VersionsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_ctx():
    ...
    for transport in transports:
        client = VersionsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
