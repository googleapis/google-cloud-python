...
from google.cloud.dialogflow_v2.services.versions import (
    VersionsAsyncClient,
    VersionsClient,
    pagers,
    transports,
...
def test__get_default_mtls_endpoint():
    ...
    assert VersionsClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
...
def test__get_client_cert_source():
    ...
    assert (
        VersionsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    )
    assert (
        VersionsClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                VersionsClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                VersionsClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(VersionsClient),
)
@mock.patch.object(
    VersionsAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(VersionsAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = VersionsClient._DEFAULT_UNIVERSE
    default_endpoint = VersionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = VersionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        VersionsClient._get_api_endpoint(
    ...
    assert (
        ...
        == VersionsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        VersionsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        VersionsClient._get_api_endpoint(None, None, default_universe, "always")
        == VersionsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        VersionsClient._get_api_endpoint(
    ...
    assert (
        VersionsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        VersionsClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        VersionsClient._get_universe_domain(client_universe_domain, universe_domain_env)
        == client_universe_domain
    )
    assert (
        VersionsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        VersionsClient._get_universe_domain(None, None)
        == VersionsClient._DEFAULT_UNIVERSE
    )
...
def test_list_versions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v2/{parent=projects/*/agent}/versions" % client.transport._host, args[1]
        )
...
def test_get_version_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v2/{name=projects/*/agent/versions/*}" % client.transport._host, args[1]
        )
...
def test_create_version_rest_required_fields(
    request_type=gcd_version.CreateVersionRequest,
):
    transport_class = transports.VersionsRestTransport
...
def test_update_version_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {
            "version": {"name": "projects/sample1/agent/versions/sample2"}
        }
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v2/{version.name=projects/*/agent/versions/*}" % client.transport._host,
            args[1],
        )
...
def test_get_version_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(transports.VersionsRestInterceptor, "pre_get_version") as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
