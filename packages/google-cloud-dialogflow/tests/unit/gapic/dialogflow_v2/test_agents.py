...
from google.cloud.dialogflow_v2.services.agents import AgentsAsyncClient, AgentsClient, pagers, transports
...
async def test_search_agents_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.search_agents), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            agent.SearchAgentsResponse(
                agents=[
...
async def test_search_agents_async_pages():
    ...
    with mock.patch.object(type(client.transport.search_agents), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.search_agents(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_validation_result(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_validation_result), "__call__") as call:
        # Designate an appropriate return value for the call.
...
def test_get_validation_result_rest_required_fields(request_type=agent.GetValidationResultRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_validation_result._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_set_agent_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_set_agent") as post, mock.patch.object(
...
def test_search_agents_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_search_agents") as post, mock.patch.object(
...
def test_get_validation_result_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AgentsRestInterceptor, "post_get_validation_result") as post, mock.patch.object(
