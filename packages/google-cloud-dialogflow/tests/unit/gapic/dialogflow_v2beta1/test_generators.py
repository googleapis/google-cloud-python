...
from google.cloud.dialogflow_v2beta1.services.generators import GeneratorsAsyncClient, GeneratorsClient, pagers, transports
...
from google.cloud.dialogflow_v2beta1.types import tool_call
...
async def test_list_generators_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_generators), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            generator.ListGeneratorsResponse(
                generators=[
...
async def test_list_generators_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_generators), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_generators(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = GeneratorsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = GeneratorsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = GeneratorsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = GeneratorsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = GeneratorsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = GeneratorsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_create_generator_rest_bad_request(request_type=gcd_generator.CreateGeneratorRequest):
    client = GeneratorsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
            ...
            "few_shot_examples": [
                {
                    "conversation_context": {
                        "message_entries": [
                            {"role": 1, "text": "text_value", "language_code": "language_code_value", "create_time": {"seconds": 751, "nanos": 543}}
                        ]
                    },
                    "extra_info": {},
...
def test_create_generator_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.GeneratorsRestInterceptor, "post_create_generator") as post, mock.patch.object(
...
def test_get_generator_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.GeneratorsRestInterceptor, "post_get_generator") as post, mock.patch.object(
...
def test_list_generators_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.GeneratorsRestInterceptor, "post_list_generators") as post, mock.patch.object(
...
def test_delete_generator_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.GeneratorsRestInterceptor, "pre_delete_generator") as pre:
        pre.assert_not_called()
...
def test_update_generator_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.GeneratorsRestInterceptor, "post_update_generator") as post, mock.patch.object(
...
def test_generators_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.GeneratorsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.GeneratorsGrpcTransport, grpc_helpers), (transports.GeneratorsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_generators_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.GeneratorsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_ctx():
    ...
    for transport in transports:
        client = GeneratorsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
