...
from google.cloud.dialogflow_v2beta1.services.environments import EnvironmentsAsyncClient, EnvironmentsClient, pagers, transports
from google.cloud.dialogflow_v2beta1.types import audio_config, environment, fulfillment
...
def test_get_environment_history(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_environment_history), "__call__") as call:
        # Designate an appropriate return value for the call.
...
async def test_get_environment_history_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_environment_history), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            environment.EnvironmentHistory(
                entries=[
        ...
        async_pager = await client.get_environment_history(
            request={},
        )
        assert async_pager.next_page_token == "abc"
        responses = []
        ...
        assert len(responses) == 6
        assert all(isinstance(i, environment.EnvironmentHistory.Entry) for i in responses)
...
async def test_get_environment_history_async_pages():
    ...
    with mock.patch.object(type(client.transport.get_environment_history), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        # Workaround issue in python 3.9 related to code coverage by adding `# pragma: no branch`
        # See https://github.com/googleapis/gapic-generator-python/pull/1174#issuecomment-1025132372
        async for page_ in (await client.get_environment_history(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_environments_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v2beta1/{parent=projects/*/agent}/environments" % client.transport._host, args[1])
...
def test_get_environment_history_rest_required_fields(request_type=environment.GetEnvironmentHistoryRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_environment_history._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_create_environment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.EnvironmentsRestInterceptor, "post_create_environment") as post, mock.patch.object(
...
def test_update_environment_rest_call_success(request_type):
    ...
    request_init = {"environment": {"name": "projects/sample1/agent/environments/sample2"}}
    request_init["environment"] = {
        "name": "projects/sample1/agent/environments/sample2",
        "description": "description_value",
        "agent_version": "agent_version_value",
        "state": 1,
        "update_time": {"seconds": 751, "nanos": 543},
...
def test_update_environment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.EnvironmentsRestInterceptor, "post_update_environment") as post, mock.patch.object(
...
def test_get_environment_history_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.EnvironmentsRestInterceptor, "post_get_environment_history") as post, mock.patch.object(
        transports.EnvironmentsRestInterceptor, "post_get_environment_history_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.EnvironmentsRestInterceptor, "pre_get_environment_history"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
