...
def test_get_environment_history_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._transport.get_environment_history
...
@pytest.mark.asyncio
async def test_get_environment_history_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.get_environment_history
            in client._client._transport._wrapped_methods
        )
...
def test_list_environments_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v2beta1/{parent=projects/*/agent}/environments"
...
def test_create_environment_rest_bad_request(
    transport: str = "rest", request_type=environment.CreateEnvironmentRequest
):
    client = EnvironmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_update_environment_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {
        "environment": {"name": "projects/sample1/agent/environments/sample2"}
    }
    request_init["environment"] = {
...
def test_update_environment_rest_bad_request(
    transport: str = "rest", request_type=environment.UpdateEnvironmentRequest
):
    client = EnvironmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_get_environment_history_rest_required_fields(
    request_type=environment.GetEnvironmentHistoryRequest,
):
    transport_class = transports.EnvironmentsRestTransport
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).get_environment_history._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_environment_history_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.EnvironmentsRestInterceptor, "post_get_environment_history"
    ) as post, mock.patch.object(
        transports.EnvironmentsRestInterceptor, "pre_get_environment_history"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = environment.GetEnvironmentHistoryRequest.pb(
            environment.GetEnvironmentHistoryRequest()
        )
        transcode.return_value = {
            "method": "post",
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = environment.EnvironmentHistory.to_json(
            environment.EnvironmentHistory()
        )
...
def test_get_environment_history_rest_bad_request(
    transport: str = "rest", request_type=environment.GetEnvironmentHistoryRequest
):
    client = EnvironmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
