...
from google.cloud.dialogflow_v2beta1.services.documents import DocumentsAsyncClient, DocumentsClient, pagers, transports
...
def test__read_environment_variables():
    assert DocumentsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert DocumentsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert DocumentsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert DocumentsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert DocumentsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert DocumentsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert DocumentsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                DocumentsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert DocumentsClient._get_client_cert_source(None, False) is None
    assert DocumentsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert DocumentsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert DocumentsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert DocumentsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(DocumentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DocumentsClient))
@mock.patch.object(DocumentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DocumentsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DocumentsClient._DEFAULT_UNIVERSE
    default_endpoint = DocumentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = DocumentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert DocumentsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert DocumentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DocumentsClient.DEFAULT_MTLS_ENDPOINT
    assert DocumentsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert DocumentsClient._get_api_endpoint(None, None, default_universe, "always") == DocumentsClient.DEFAULT_MTLS_ENDPOINT
    assert DocumentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == DocumentsClient.DEFAULT_MTLS_ENDPOINT
    assert DocumentsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert DocumentsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        DocumentsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert DocumentsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert DocumentsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert DocumentsClient._get_universe_domain(None, None) == DocumentsClient._DEFAULT_UNIVERSE
...
@mock.patch.object(DocumentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DocumentsClient))
@mock.patch.object(DocumentsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DocumentsAsyncClient))
def test_documents_client_client_api_endpoint(client_class):
...
def test_reload_document_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = DocumentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
    ...
    with mock.patch.object(type(client.transport.reload_document), "__call__") as call:
        call.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client.reload_document(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_reload_document_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.reload_document in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.reload_document] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_reload_document_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DocumentsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.reload_document in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.reload_document] = mock_rpc
...
def test_list_documents_rest_required_fields(request_type=document.ListDocumentsRequest):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            return_value = document.ListDocumentsResponse.pb(return_value)
...
def test_list_documents_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v2beta1/{parent=projects/*/knowledgeBases/*}/documents" % client.transport._host, args[1])
...
def test_get_document_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{name=projects/*/knowledgeBases/*/documents/*}" % client.transport._host, args[1])
...
def test_create_document_rest_required_fields(request_type=gcd_document.CreateDocumentRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_document._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("import_gcs_custom_metadata",))
    jsonified_request.update(unset_fields)
...
def test_update_document_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{document.name=projects/*/knowledgeBases/*/documents/*}" % client.transport._host, args[1])
...
def test_reload_document_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = DocumentsClient(
...
def test_reload_document_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{name=projects/*/knowledgeBases/*/documents/*}:reload" % client.transport._host, args[1])
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = DocumentsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = DocumentsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_list_documents_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.list_documents), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = document.ListDocumentsRequest()
...
def test_get_document_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.get_document), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = document.GetDocumentRequest()
...
def test_create_document_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.create_document), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = gcd_document.CreateDocumentRequest()
...
def test_update_document_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.update_document), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = gcd_document.UpdateDocumentRequest()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_reload_document_empty_call_grpc():
    client = DocumentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.reload_document), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.reload_document(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = document.ReloadDocumentRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = DocumentsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = DocumentsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_reload_document_empty_call_grpc_asyncio():
    client = DocumentsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.reload_document), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.reload_document(request=None)
...
def test_transport_kind_rest():
    transport = DocumentsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_documents_rest_bad_request(request_type=document.ListDocumentsRequest):
    client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_documents_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.DocumentsRestInterceptor, "post_list_documents") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = document.ListDocumentsResponse.to_json(document.ListDocumentsResponse())
...
def test_get_document_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DocumentsRestInterceptor, "post_get_document") as post, mock.patch.object(
...
def test_create_document_rest_bad_request(request_type=gcd_document.CreateDocumentRequest):
    client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/knowledgeBases/sample2"}
    request = request_type(**request_init)
...
def test_create_document_rest_call_success(request_type):
    ...
    test_field = gcd_document.CreateDocumentRequest.meta.fields["document"]
...
def test_import_documents_rest_bad_request(request_type=document.ImportDocumentsRequest):
    client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/knowledgeBases/sample2"}
    request = request_type(**request_init)
...
def test_update_document_rest_bad_request(request_type=gcd_document.UpdateDocumentRequest):
    client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_update_document_rest_call_success(request_type):
    ...
    request_init = {"document": {"name": "projects/sample1/knowledgeBases/sample2/documents/sample3"}}
    request_init["document"] = {
        "name": "projects/sample1/knowledgeBases/sample2/documents/sample3",
        "display_name": "display_name_value",
        "mime_type": "mime_type_value",
        "knowledge_types": [1],
        "content_uri": "content_uri_value",
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = gcd_document.UpdateDocumentRequest.meta.fields["document"]
...
def test_reload_document_rest_call_success(request_type):
    client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_reload_document_empty_call_rest():
    client = DocumentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_documents_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.DocumentsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.DocumentsGrpcTransport, grpc_helpers), (transports.DocumentsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_documents_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.DocumentsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
async def test_get_location_field_headers_async():
    client = DocumentsAsyncClient(credentials=async_anonymous_credentials())
...
def test_client_ctx():
    ...
    for transport in transports:
        client = DocumentsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
