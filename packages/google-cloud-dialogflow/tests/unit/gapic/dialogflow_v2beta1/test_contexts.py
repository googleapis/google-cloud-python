...
from google.cloud.dialogflow_v2beta1.services.contexts import ContextsAsyncClient, ContextsClient, pagers, transports
...
def test__read_environment_variables():
    assert ContextsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert ContextsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ContextsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ContextsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert ContextsClient._get_client_cert_source(None, False) is None
    assert ContextsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ContextsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert ContextsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ContextsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(ContextsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContextsClient))
@mock.patch.object(ContextsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContextsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ContextsClient._DEFAULT_UNIVERSE
    default_endpoint = ContextsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ContextsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ContextsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ContextsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ContextsClient.DEFAULT_MTLS_ENDPOINT
    assert ContextsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ContextsClient._get_api_endpoint(None, None, default_universe, "always") == ContextsClient.DEFAULT_MTLS_ENDPOINT
    assert ContextsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ContextsClient.DEFAULT_MTLS_ENDPOINT
    assert ContextsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ContextsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ContextsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ContextsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ContextsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ContextsClient._get_universe_domain(None, None) == ContextsClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize(
    ...
    [
        (ContextsClient, "grpc"),
        (ContextsAsyncClient, "grpc_asyncio"),
        (ContextsClient, "rest"),
    ],
)
def test_contexts_client_from_service_account_info(client_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.ContextsGrpcTransport, "grpc"),
        (transports.ContextsGrpcAsyncIOTransport, "grpc_asyncio"),
        (transports.ContextsRestTransport, "rest"),
    ],
)
def test_contexts_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_contexts_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ContextsClient, transports.ContextsRestTransport, "rest", "true"),
        (ContextsClient, transports.ContextsRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(ContextsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContextsClient))
@mock.patch.object(ContextsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContextsAsyncClient))
...
def test_contexts_client_get_mtls_endpoint_and_cert_source(client_class):
    mock_client_cert_source = mock.Mock()
...
@pytest.mark.parametrize(
    ...
    [
        (ContextsClient, transports.ContextsGrpcTransport, "grpc", grpc_helpers),
        (ContextsAsyncClient, transports.ContextsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (ContextsClient, transports.ContextsRestTransport, "rest", None),
    ],
)
def test_contexts_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_contexts_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_list_contexts_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_contexts), "__call__") as call:
        ...
        pager = client.list_contexts(request={}, retry=retry, timeout=timeout)
...
async def test_list_contexts_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_contexts), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            context.ListContextsResponse(
                contexts=[
...
def test_create_context_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_context in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_context] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_context_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ContextsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_context in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_context] = mock_rpc
...
def test_create_context_flattened():
    ...
    with mock.patch.object(type(client.transport.create_context), "__call__") as call:
        ...
        assert arg == mock_val
        arg = args[0].context
        mock_val = gcd_context.Context(name="name_value")
        assert arg == mock_val
...
def test_update_context_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_context in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_context] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_context_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ContextsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_context in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_context] = mock_rpc
...
def test_delete_context_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_context in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_context] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_context_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ContextsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_context in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_context] = mock_rpc
...
def test_delete_all_contexts_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_delete_all_contexts_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_all_contexts in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_all_contexts] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_all_contexts_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ContextsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_all_contexts in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_all_contexts] = mock_rpc
...
def test_list_contexts_rest_required_fields(request_type=context.ListContextsRequest):
    ...
    return_value = context.ListContextsResponse()
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            return_value = context.ListContextsResponse.pb(return_value)
...
def test_list_contexts_rest_unset_required_fields():
    transport = transports.ContextsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_contexts_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v2beta1/{parent=projects/*/agent/sessions/*}/contexts" % client.transport._host, args[1])
...
def test_list_contexts_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(context.ListContextsResponse.to_json(x) for x in response)
...
def test_get_context_rest_required_fields(request_type=context.GetContextRequest):
    transport_class = transports.ContextsRestTransport
...
def test_get_context_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/agent/sessions/sample2/contexts/sample3"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{name=projects/*/agent/sessions/*/contexts/*}" % client.transport._host, args[1])
...
def test_create_context_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ContextsClient(
...
def test_create_context_rest_required_fields(request_type=gcd_context.CreateContextRequest):
    transport_class = transports.ContextsRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = gcd_context.Context()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, "request") as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = gcd_context.Context.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_update_context_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ContextsClient(
...
def test_update_context_rest_required_fields(request_type=gcd_context.UpdateContextRequest):
    transport_class = transports.ContextsRestTransport
...
def test_update_context_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("updateMask",)) & set(("context",)))
...
def test_update_context_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"context": {"name": "projects/sample1/agent/sessions/sample2/contexts/sample3"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{context.name=projects/*/agent/sessions/*/contexts/*}" % client.transport._host, args[1])
...
def test_delete_context_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ContextsClient(
...
def test_delete_context_rest_required_fields(request_type=context.DeleteContextRequest):
    transport_class = transports.ContextsRestTransport
...
def test_delete_all_contexts_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ContextsClient(
...
def test_delete_all_contexts_rest_required_fields(request_type=context.DeleteAllContextsRequest):
    transport_class = transports.ContextsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_all_contexts._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_delete_all_contexts_rest_unset_required_fields():
    ...
    unset_fields = transport.delete_all_contexts._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("parent",)))


def test_delete_all_contexts_rest_flattened():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.delete_all_contexts(**mock_args)
...
def test_delete_all_contexts_rest_flattened_error(transport: str = "rest"):
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = ContextsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = ContextsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_context_empty_call_grpc():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.create_context), "__call__") as call:
        call.return_value = gcd_context.Context()
        client.create_context(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_context_empty_call_grpc():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.update_context), "__call__") as call:
        call.return_value = gcd_context.Context()
        client.update_context(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_context_empty_call_grpc():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.delete_context), "__call__") as call:
        call.return_value = None
        client.delete_context(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_all_contexts_empty_call_grpc():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.delete_all_contexts), "__call__") as call:
        call.return_value = None
        client.delete_all_contexts(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = ContextsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ContextsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_context_empty_call_grpc_asyncio():
    client = ContextsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.create_context), "__call__") as call:
        ...
        await client.create_context(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_update_context_empty_call_grpc_asyncio():
    client = ContextsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.update_context), "__call__") as call:
        ...
        await client.update_context(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_delete_context_empty_call_grpc_asyncio():
    client = ContextsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.delete_context), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.delete_context(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_delete_all_contexts_empty_call_grpc_asyncio():
    client = ContextsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.delete_all_contexts), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.delete_all_contexts(request=None)
...
def test_transport_kind_rest():
    transport = ContextsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_contexts_rest_bad_request(request_type=context.ListContextsRequest):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_contexts_rest_interceptors(null_interceptor):
    transport = transports.ContextsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ContextsRestInterceptor(),
    )
    client = ContextsClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ContextsRestInterceptor, "post_list_contexts") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = context.ListContextsResponse.to_json(context.ListContextsResponse())
...
def test_get_context_rest_bad_request(request_type=context.GetContextRequest):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/agent/sessions/sample2/contexts/sample3"}
    request = request_type(**request_init)
...
def test_get_context_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ContextsRestInterceptor, "post_get_context") as post, mock.patch.object(
...
def test_create_context_rest_call_success(request_type):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")

    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/agent/sessions/sample2"}
    request_init["context"] = {"name": "name_value", "lifespan_count": 1498, "parameters": {"fields": {}}}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748

    # Determine if the message type is proto-plus or protobuf
    test_field = gcd_context.CreateContextRequest.meta.fields["context"]
...
def test_create_context_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ContextsRestInterceptor, "post_create_context") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_context.CreateContextRequest.pb(gcd_context.CreateContextRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcd_context.Context.to_json(gcd_context.Context())
        req.return_value.content = return_value
...
def test_update_context_rest_bad_request(request_type=gcd_context.UpdateContextRequest):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"context": {"name": "projects/sample1/agent/sessions/sample2/contexts/sample3"}}
    request = request_type(**request_init)
...
def test_update_context_rest_call_success(request_type):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")

    # send a request that will satisfy transcoding
    request_init = {"context": {"name": "projects/sample1/agent/sessions/sample2/contexts/sample3"}}
    request_init["context"] = {
        "name": "projects/sample1/agent/sessions/sample2/contexts/sample3",
        "lifespan_count": 1498,
        "parameters": {"fields": {}},
    }
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = gcd_context.UpdateContextRequest.meta.fields["context"]
...
def test_update_context_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ContextsRestInterceptor, "post_update_context") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_context.UpdateContextRequest.pb(gcd_context.UpdateContextRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_context_rest_call_success(request_type):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_delete_context_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ContextsRestInterceptor, "pre_delete_context") as pre:
        pre.assert_not_called()
...
def test_delete_all_contexts_rest_bad_request(request_type=context.DeleteAllContextsRequest):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/agent/sessions/sample2"}
    request = request_type(**request_init)
...
def test_delete_all_contexts_rest_call_success(request_type):
    client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_delete_all_contexts_rest_interceptors(null_interceptor):
    transport = transports.ContextsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ContextsRestInterceptor(),
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ContextsRestInterceptor, "pre_delete_all_contexts") as pre:
        pre.assert_not_called()
        pb_message = context.DeleteAllContextsRequest.pb(context.DeleteAllContextsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_context_empty_call_rest():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_context_empty_call_rest():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_context_empty_call_rest():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_all_contexts_empty_call_rest():
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_contexts_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.ContextsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
def test_contexts_transport_auth_gdch_credentials(transport_class):
...
@pytest.mark.parametrize(
    ...
    [(transports.ContextsGrpcTransport, grpc_helpers), (transports.ContextsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_contexts_grpc_transport_client_cert_source_for_mtls(transport_class):
...
def test_contexts_http_transport_client_cert_source_for_mtls():
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.ContextsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_contexts_host_no_port(transport_name):
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_contexts_host_with_port(transport_name):
    client = ContextsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_contexts_client_transport_session_collision(transport_name):
    creds1 = ga_credentials.AnonymousCredentials()
    creds2 = ga_credentials.AnonymousCredentials()
    client1 = ContextsClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = ContextsClient(
        credentials=creds2,
        transport=transport_name,
    )
    ...
    assert session1 != session2
    session1 = client1.transport.delete_all_contexts._session
    session2 = client2.transport.delete_all_contexts._session
    assert session1 != session2
...
async def test_get_location_field_headers_async():
    client = ContextsAsyncClient(credentials=async_anonymous_credentials())
...
def test_client_ctx():
    ...
    for transport in transports:
        client = ContextsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
...
@pytest.mark.parametrize(
    ...
    [
        (ContextsClient, transports.ContextsGrpcTransport),
        (ContextsAsyncClient, transports.ContextsGrpcAsyncIOTransport),
    ],
)
