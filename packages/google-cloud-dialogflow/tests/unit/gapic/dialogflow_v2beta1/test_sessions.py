...
def test_detect_intent_rest_required_fields(
...
def test_detect_intent_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v2beta1/{session=projects/*/agent/sessions/*}:detectIntent"
...
def test_detect_intent_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        pb_message = gcd_session.DetectIntentRequest.pb(
            gcd_session.DetectIntentRequest()
        )
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcd_session.DetectIntentResponse.to_json(
            gcd_session.DetectIntentResponse()
        )
