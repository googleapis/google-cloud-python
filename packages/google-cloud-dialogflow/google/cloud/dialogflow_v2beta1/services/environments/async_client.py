...
class EnvironmentsAsyncClient:
    ...
    def __init__(
        ...
        *,
        ...
        if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
            _LOGGER.debug(
                "Created client `google.cloud.dialogflow_v2beta1.EnvironmentsAsyncClient`.",
                extra={
    ...
    async def get_environment_history(
        ...
        *,
        ...
        # - Use the request object if provided (there's no risk of modifying the input as
        #   there are no flattened fields), or create one.
        if not isinstance(request, environment.GetEnvironmentHistoryRequest):
            request = environment.GetEnvironmentHistoryRequest(request)

        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._client._transport._wrapped_methods[self._client._transport.get_environment_history]
