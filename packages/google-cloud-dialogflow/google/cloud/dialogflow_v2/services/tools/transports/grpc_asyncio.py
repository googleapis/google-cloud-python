...
from .grpc import ToolsGrpcTransport
...
class ToolsGrpcAsyncIOTransport(ToolsTransport):
    """gRPC AsyncIO backend transport for Tools.

    Tool Service for LLM powered Agent Assist. Tools can be used
    to interact with remote APIs (e.g. fetching orders) to retrieve
    additional information as input to LLM.
    ...
    def create_tool(
        self,
    ) -> Callable[[gcd_tool.CreateToolRequest], Awaitable[gcd_tool.Tool]]:
        ...
        if "create_tool" not in self._stubs:
            self._stubs["create_tool"] = self._logged_channel.unary_unary(
                ...
                request_serializer=gcd_tool.CreateToolRequest.serialize,
                response_deserializer=gcd_tool.Tool.deserialize,
            )
        return self._stubs["create_tool"]

    @property
    def get_tool(self) -> Callable[[tool.GetToolRequest], Awaitable[tool.Tool]]:
        ...
        if "get_tool" not in self._stubs:
            self._stubs["get_tool"] = self._logged_channel.unary_unary(
                ...
                request_serializer=tool.GetToolRequest.serialize,
    ...
    def list_tools(
        ...
        if "list_tools" not in self._stubs:
            self._stubs["list_tools"] = self._logged_channel.unary_unary(
                ...
                request_serializer=tool.ListToolsRequest.serialize,
                response_deserializer=tool.ListToolsResponse.deserialize,
            )
    ...
    def delete_tool(
        ...
        if "delete_tool" not in self._stubs:
            self._stubs["delete_tool"] = self._logged_channel.unary_unary(
                ...
                request_serializer=tool.DeleteToolRequest.serialize,
    ...
    def update_tool(
        self,
    ) -> Callable[[gcd_tool.UpdateToolRequest], Awaitable[gcd_tool.Tool]]:
        ...
        if "update_tool" not in self._stubs:
            self._stubs["update_tool"] = self._logged_channel.unary_unary(
                ...
                request_serializer=gcd_tool.UpdateToolRequest.serialize,
                response_deserializer=gcd_tool.Tool.deserialize,
            )
...
__all__ = ("ToolsGrpcAsyncIOTransport",)
