...
class AgentsGrpcAsyncIOTransport(AgentsTransport):
    ...
    @property
    def set_agent(self) -> Callable[[gcd_agent.SetAgentRequest], Awaitable[gcd_agent.Agent]]:
        ...
        if "set_agent" not in self._stubs:
            self._stubs["set_agent"] = self._logged_channel.unary_unary(
    ...
    @property
    def search_agents(self) -> Callable[[agent.SearchAgentsRequest], Awaitable[agent.SearchAgentsResponse]]:
    ...
    @property
    def train_agent(self) -> Callable[[agent.TrainAgentRequest], Awaitable[operations_pb2.Operation]]:
        ...
        if "train_agent" not in self._stubs:
            self._stubs["train_agent"] = self._logged_channel.unary_unary(
    ...
    @property
    def import_agent(self) -> Callable[[agent.ImportAgentRequest], Awaitable[operations_pb2.Operation]]:
        ...
        if "import_agent" not in self._stubs:
            self._stubs["import_agent"] = self._logged_channel.unary_unary(
    ...
    @property
    def get_validation_result(self) -> Callable[[agent.GetValidationResultRequest], Awaitable[validation_result.ValidationResult]]:
        ...
        if "get_validation_result" not in self._stubs:
            self._stubs["get_validation_result"] = self._logged_channel.unary_unary(
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            ...
            self.set_agent: self._wrap_method(
                self.set_agent,
                default_timeout=None,
                client_info=client_info,
            ),
            ...
            self.train_agent: self._wrap_method(
                self.train_agent,
                default_timeout=None,
                client_info=client_info,
            ),
            ...
            self.import_agent: self._wrap_method(
                self.import_agent,
                default_timeout=None,
                client_info=client_info,
            ),
            ...
            self.get_validation_result: self._wrap_method(
