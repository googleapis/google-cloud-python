...
def test_transport_kind_grpc_asyncio():
    transport = VpcAccessServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = VpcAccessServiceClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_list_connectors_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = vpc_access.ListConnectorsResponse.to_json(
            vpc_access.ListConnectorsResponse()
        )
        req.return_value.content = return_value
