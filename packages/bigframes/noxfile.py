...
def core_deps_from_source(session, protobuf_implementation):
    ...
    # We use install_unittest_dependencies to get standard deps and extras
    constraints_path = str(
        CURRENT_DIRECTORY / "testing" / f"constraints-{session.python}.txt"
    )
    install_unittest_dependencies(session, True, "-c", constraints_path)

    # Now override core dependencies with source installs from this repo
    ...
    # Run py.test against the unit tests (mimicking run_unit)
    scripts_path = "scripts"
    tests_path = os.path.join("tests", "unit")
    third_party_tests_path = os.path.join("third_party", "bigframes_vendored")
    session.run(
