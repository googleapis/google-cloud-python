...
def test_array_index(scalar_types_df: bpd.DataFrame, snapshot):
    ...
    sql = utils._apply_ops_to_sql(
        scalar_types_df, list(ops_map.values()), list(ops_map.keys())
    )
    snapshot.assert_match(sql, "out.sql")
...
def test_array_reduce_op(repeated_types_df: bpd.DataFrame, snapshot):
    ops_map = {
        ...
        "array_agg_str": ops.ArrayReduceOp(agg_ops.ArrayAggOp()).as_expr(
            "string_list_col"
        ),
    }
...
def test_array_slice(scalar_types_df: bpd.DataFrame, snapshot):
    array_expr = ops.ToArrayOp().as_expr("int64_col", "int64_too")
    ops_map = {
        "string_slice": ops.ArraySliceOp(start=1, stop=5).as_expr("string_col"),
        "slice_only_start": expression.OpExpression(
            ops.ArraySliceOp(start=1, stop=None),
            (array_expr,),
        ),
        "slice_start_stop": expression.OpExpression(
            ops.ArraySliceOp(start=1, stop=5),
            (array_expr,),
        ),
    }
