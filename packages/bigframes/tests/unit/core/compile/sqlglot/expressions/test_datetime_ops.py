...
def test_datetime_to_integer_label(scalar_types_df: bpd.DataFrame, snapshot):
    ...
    ops_map = {
        "fixed_freq": ops.DatetimeToIntegerLabelOp(
            freq=pd.tseries.offsets.Day(),  # type: ignore[arg-type]
            origin="start",
            closed="left",  # type: ignore
        ...
        "non_fixed_freq_weekly": ops.DatetimeToIntegerLabelOp(
            freq=pd.tseries.offsets.Week(weekday=6),  # type: ignore[arg-type]
            origin="start",
            closed="left",  # type: ignore
        ).as_expr("datetime_col", "timestamp_col"),
        "non_fixed_freq_monthly": ops.DatetimeToIntegerLabelOp(
            freq=pd.tseries.offsets.MonthEnd(),  # type: ignore[arg-type]
            origin="start",
            closed="left",  # type: ignore
        ).as_expr("datetime_col", "timestamp_col"),
        "non_fixed_freq_quarterly": ops.DatetimeToIntegerLabelOp(
            freq=pd.tseries.offsets.QuarterEnd(startingMonth=12),  # type: ignore[arg-type]
            origin="start",
            closed="left",  # type: ignore
        ).as_expr("datetime_col", "timestamp_col"),
        "non_fixed_freq_yearly": ops.DatetimeToIntegerLabelOp(
            freq=pd.tseries.offsets.YearEnd(),  # type: ignore[arg-type]
            origin="start",
            closed="left",  # type: ignore
        ).as_expr("datetime_col", "timestamp_col"),
    }
