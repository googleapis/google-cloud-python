WITH `bfcte_0` AS (
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`PART` AS `bft_4`
  WHERE
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`PARTSUPP` AS `bft_3`
), `bfcte_2` AS (
  SELECT
    `L_PARTKEY`,
    `L_SUPPKEY`,
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`NATION` AS `bft_1`
  WHERE
    `N_NAME` = 'CANADA'
), `bfcte_4` AS (
  SELECT
