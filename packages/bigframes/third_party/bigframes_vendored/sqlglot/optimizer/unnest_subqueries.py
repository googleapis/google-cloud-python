# Contains code from https://github.com/tobymao/sqlglot/blob/v28.5.0/sqlglot/optimizer/unnest_subqueries.py
...
def unnest(select, parent_select, next_alias_name):
    ...
    if not isinstance(predicate, (exp.In, exp.Any)):
        ...
        parent_select.join(
            select, on=on_clause, join_type=join_type, join_alias=alias, copy=False
        )
        return
