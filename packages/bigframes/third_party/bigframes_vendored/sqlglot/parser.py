# Contains code from https://github.com/tobymao/sqlglot/blob/v28.5.0/sqlglot/parser.py
...
class Parser(metaclass=_Parser):
    ...
    FUNCTIONS: t.Dict[str, t.Callable] = {
        ...
        **dict.fromkeys(("COALESCE", "IFNULL", "NVL"), build_coalesce),
        ...
        "UPPER": build_upper,
        "UUID": lambda args, dialect: exp.Uuid(
            is_string=dialect.UUID_IS_STRING_TYPE or None
        ),
    ...
    def _parse_property(self) -> t.Optional[exp.Expression]:
        ...
        if self._match_text_seq("SQL", "SECURITY"):
            return self.expression(
                ...
                this=self._match_texts(("DEFINER", "INVOKER"))
    ...
    def _parse_update(self) -> exp.Update:
        kwargs: t.Dict[str, t.Any] = {
            "this": self._parse_table(
                joins=True, alias_tokens=self.UPDATE_ALIAS_TOKENS
            ),
        }
        while self._curr:
    ...
    def _implicit_unnests_to_explicit(self, this: E) -> E:
        from bigframes_vendored.sqlglot.optimizer.normalize_identifiers import (
            normalize_identifiers as _norm,
        )

        refs = {
            _norm(this.args["from_"].this.copy(), dialect=self.dialect).alias_or_name
        }
    ...
    def _parse_cube_or_rollup(
        self, with_prefix: bool = False
    ) -> t.Optional[exp.Cube | exp.Rollup]:
        ...
        return self.expression(
            ...
            expressions=[]
            if with_prefix
            else self._parse_wrapped_csv(self._parse_bitwise),
        )
    ...
    def _parse_name_as_expression(self) -> t.Optional[exp.Expression]:
        ...
        if self._match(TokenType.ALIAS):
            this = self.expression(
                exp.Alias, alias=this, this=self._parse_disjunction()
            )
        return this
        ...
        if self._match_set(self.TYPE_TOKENS):
            ...
            if isinstance(identifier, exp.Identifier):
                ...
                if (
                    tokens
                    and len(tokens) == 1
                    and tokens[0].token_type in self.TYPE_TOKENS
                ):
    ...
    def _parse_vector_expressions(
        ...
        return [
            exp.DataType.build(expressions[0].name, dialect=self.dialect),
            *expressions[1:],
        ]
    ...
    def _parse_bracket_key_value(
        ...
        return self._parse_slice(
            self._parse_alias(self._parse_disjunction(), explicit=True)
        )
    ...
    def _parse_alter_table_alter(self) -> t.Optional[exp.Expression]:
        ...
        if self._match_pair(TokenType.SET, TokenType.DEFAULT):
            return self.expression(
                exp.AlterColumn, this=column, default=self._parse_disjunction()
            )
    ...
    def _parse_alter_session(self) -> exp.AlterSession:
        ...
        if self._match(TokenType.SET):
            ...
            return self.expression(
                exp.AlterSession, expressions=expressions, unset=False
            )

        self._match_text_seq("UNSET")
        expressions = self._parse_csv(
            lambda: self.expression(
                exp.SetItem, this=self._parse_id_var(any_token=True)
            )
        )
    ...
    def _parse_alter(self) -> exp.Alter | exp.Command:
        ...
        if alter_token.token_type == TokenType.SESSION:
            ...
            this = self._parse_table(
                schema=True, parse_partition=self.ALTER_TABLE_PARTITIONS
            )
        ...
        if parser:
            ...
            cascade = (
                ...
                and self._match_text_seq("CASCADE")
            )
    ...
    def _parse_when_matched(self) -> exp.Whens:
        ...
        while self._match(TokenType.WHEN):
            ...
            condition = (
                self._parse_disjunction() if self._match(TokenType.AND) else None
            )
    ...
    def _parse_copy_parameters(self) -> t.List[exp.CopyParameter]:
        ...
        while self._curr and not self._match(TokenType.R_PAREN, advance=False):
            ...
            if prev in self.COPY_INTO_VARLEN_OPTIONS and self._match(
                ...
                param.set(
                    "expression", self._parse_unquoted_field() or self._parse_bracket()
                )
    ...
    def _parse_max_min_by(self, expr_type: t.Type[exp.AggFunc]) -> exp.AggFunc:
        ...
        if self._match(TokenType.DISTINCT):
            args.append(
                self.expression(exp.Distinct, expressions=[self._parse_lambda()])
            )
            self._match(TokenType.COMMA)
    ...
    def _parse_initcap(self) -> exp.Initcap:
        ...
        if expr.args.get("expression") is None:
            expr.set(
                "expression",
                exp.Literal.string(self.dialect.INITCAP_DEFAULT_DELIMITER_CHARS),
            )
