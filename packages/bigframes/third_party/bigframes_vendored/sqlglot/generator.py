# Contains code from https://github.com/tobymao/sqlglot/blob/v28.5.0/sqlglot/generator.py
...
from bigframes_vendored.sqlglot.jsonpath import (
    ALL_JSON_PATH_PARTS,
...
class Generator(metaclass=_Generator):
    ...
    TRANSFORMS: t.Dict[t.Type[exp.Expression], t.Callable[..., str]] = {
        **JSON_PATH_PART_TRANSFORMS,
        ...
        exp.EnviromentProperty: lambda self,
        e: f"ENVIRONMENT ({self.expressions(e, flat=True)})",
        ...
        e: f"USING TEMPLATE {self.sql(e, 'this')}",
        exp.UsingData: lambda self, e: f"USING DATA {self.sql(e, 'this')}",
        exp.UppercaseColumnConstraint: lambda *_: "UPPERCASE",
        exp.UtcDate: lambda self, e: self.sql(
            exp.CurrentDate(this=exp.Literal.string("UTC"))
        ),
        exp.UtcTime: lambda self, e: self.sql(
            exp.CurrentTime(this=exp.Literal.string("UTC"))
        ),
    ...
    def __init__(
        ...
        self.pretty = (
            pretty if pretty is not None else bigframes_vendored.sqlglot.pretty
        )
    ...
    def preprocess(self, expression: exp.Expression) -> exp.Expression:
        ...
        if self.ENSURE_BOOLS:
            from bigframes_vendored.sqlglot.transforms import ensure_bools
    ...
    def _move_ctes_to_top_level(self, expression: E) -> E:
        if (
            ...
            from bigframes_vendored.sqlglot.transforms import move_ctes_to_top_level
    ...
    def unicodestring_sql(self, expression: exp.UnicodeString) -> str:
        ...
        if not self.dialect.UNICODE_START or (escape and not self.SUPPORTS_UESCAPE):
            this = escape_pattern.sub(
                self.UNICODE_SUBSTITUTE or escape_substitute, this
            )
    ...
    def extract_sql(self, expression: exp.Extract) -> str:
        from bigframes_vendored.sqlglot.dialects.dialect import map_date_part
    ...
    def concat_sql(self, expression: exp.Concat) -> str:
        if self.dialect.CONCAT_COALESCE and not expression.args.get("coalesce"):
            ...
            from bigframes_vendored.sqlglot.dialects.dialect import concat_to_dpipe_sql
    ...
    def matchagainst_sql(self, expression: exp.MatchAgainst) -> str:
        ...
        return f"{self.func('MATCH', *expressions)} AGAINST({self.sql(expression, 'this')}{modifier})"
    ...
    def alter_sql(self, expression: exp.Alter) -> str:
        ...
        cascade = (
            " CASCADE"
            if expression.args.get("cascade")
            and self.dialect.ALTER_TABLE_SUPPORTS_CASCADE
            else ""
        )
        this = self.sql(expression, "this")
        this = f" {this}" if this else ""
    ...
    def _like_sql(self, expression: exp.Like | exp.ILike) -> str:
        ...
        if isinstance(rhs, (exp.All, exp.Any)) and not self.SUPPORTS_LIKE_QUANTIFIERS:
            ...
            parent = expression.parent
            if not isinstance(parent, type(like_expr)) and isinstance(
    ...
    def when_sql(self, expression: exp.When) -> str:
        ...
        if isinstance(then_expression, exp.Insert):
            ...
            if isinstance(then_expression.args.get("expressions"), exp.Star):
                ...
                then = (
                    f"UPDATE SET{self.sep()}{expressions_sql}"
                    if expressions_sql
                    else "UPDATE"
                )
    ...
    def _ml_sql(self, expression: exp.Func, name: str) -> str:
        ...
        if expr:
            ...
            expr_sql = (
                f"TABLE {expr_sql}" if not isinstance(expr, exp.Subquery) else expr_sql
            )
        else:
            expr_sql = None
    ...
    def generateembedding_sql(self, expression: exp.GenerateEmbedding) -> str:
        name = (
            ...
            if expression.args.get("is_text")
    ...
    def dateadd_sql(self, expression: exp.DateAdd) -> str:
        from bigframes_vendored.sqlglot.dialects.dialect import unit_to_str
    ...
    def convert_sql(self, expression: exp.Convert) -> str:
        ...
        if isinstance(style, exp.Literal) and style.is_int:
            from bigframes_vendored.sqlglot.dialects.tsql import TSQL
    ...
    def copy_sql(self, expression: exp.Copy) -> str:
        ...
        kind = (
            self.seg("FROM" if expression.args.get("kind") else "TO") if files else ""
        )
    ...
    def arrayconcat_sql(
        ...
        if not self.ARRAY_CONCAT_IS_VAR_LEN:
            if len(exprs) == 0:
                ...
                rhs = reduce(
                    lambda x, y: exp.ArrayConcat(this=x, expressions=[y]), exprs
                )
        else:
    ...
    def semanticview_sql(self, expression: exp.SemanticView) -> str:
        ...
        facts = self.expressions(
            expression, "facts", dynamic=True, skip_first=True, skip_last=True
        )
