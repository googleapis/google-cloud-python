...
from google.cloud.lustre_v1.services.lustre import LustreAsyncClient, LustreClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert LustreClient._get_default_mtls_endpoint(None) is None
    assert LustreClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert LustreClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert LustreClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert LustreClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert LustreClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi


def test__read_environment_variables():
    assert LustreClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert LustreClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                LustreClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert LustreClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert LustreClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert LustreClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert LustreClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert LustreClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert LustreClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                LustreClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert LustreClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert LustreClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert LustreClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert LustreClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(LustreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LustreClient))
@mock.patch.object(LustreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LustreAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = LustreClient._DEFAULT_UNIVERSE
    default_endpoint = LustreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = LustreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert LustreClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert LustreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == LustreClient.DEFAULT_MTLS_ENDPOINT
    assert LustreClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert LustreClient._get_api_endpoint(None, None, default_universe, "always") == LustreClient.DEFAULT_MTLS_ENDPOINT
    assert LustreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == LustreClient.DEFAULT_MTLS_ENDPOINT
    assert LustreClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert LustreClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        LustreClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert LustreClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert LustreClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert LustreClient._get_universe_domain(None, None) == LustreClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        LustreClient._get_universe_domain("", None)
...
def test_lustre_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "lustre.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://lustre.googleapis.com"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (LustreAsyncClient, transports.LustreGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (LustreClient, transports.LustreGrpcTransport, "grpc", "false"),
        (LustreAsyncClient, transports.LustreGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_lustre_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(LustreClient, "DEFAULT_ENDPOINT", modify_default_endpoint(LustreClient))
@mock.patch.object(LustreAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(LustreAsyncClient))
...
def test_lustre_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (LustreAsyncClient, transports.LustreGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (LustreClient, transports.LustreRestTransport, "rest", None),
    ],
)
def test_lustre_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_lustre_client_client_options_from_dict():
    with mock.patch("google.cloud.lustre_v1.services.lustre.transports.LustreGrpcTransport.__init__") as grpc_transport:
...
def test_lustre_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_create_instance_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(type(client.transport.create_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        assert args[0] == gcl_instance.CreateInstanceRequest(
            parent="parent_value",
            instance_id="instance_id_value",
        )
...
@pytest.mark.asyncio
async def test_create_instance_async(transport: str = "grpc_asyncio", request_type=gcl_instance.CreateInstanceRequest):
    client = LustreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_update_instance_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(type(client.transport.update_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        assert args[0] == gcl_instance.UpdateInstanceRequest()
...
@pytest.mark.asyncio
async def test_update_instance_async(transport: str = "grpc_asyncio", request_type=gcl_instance.UpdateInstanceRequest):
    client = LustreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_delete_instance_async(transport: str = "grpc_asyncio", request_type=instance.DeleteInstanceRequest):
    client = LustreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_import_data_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(type(client.transport.import_data), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        assert args[0] == transfer.ImportDataRequest(
            name="name_value",
            service_account="service_account_value",
        )
...
@pytest.mark.asyncio
async def test_import_data_async(transport: str = "grpc_asyncio", request_type=transfer.ImportDataRequest):
    client = LustreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_export_data_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(type(client.transport.export_data), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        assert args[0] == transfer.ExportDataRequest(
            name="name_value",
            service_account="service_account_value",
        )
...
@pytest.mark.asyncio
async def test_export_data_async(transport: str = "grpc_asyncio", request_type=transfer.ExportDataRequest):
    client = LustreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_list_instances_rest_unset_required_fields():
    transport = transports.LustreRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_create_instance_rest_required_fields(request_type=gcl_instance.CreateInstanceRequest):
    transport_class = transports.LustreRestTransport
...
def test_update_instance_rest_required_fields(request_type=gcl_instance.UpdateInstanceRequest):
    transport_class = transports.LustreRestTransport
...
def test_delete_instance_rest_required_fields(request_type=instance.DeleteInstanceRequest):
    transport_class = transports.LustreRestTransport
...
def test_import_data_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=projects/*/locations/*/instances/*}:importData" % client.transport._host, args[1])
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = LustreClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = LustreClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = LustreClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = LustreAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = LustreAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = LustreClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_instances_rest_bad_request(request_type=instance.ListInstancesRequest):
    client = LustreClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_instances_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.LustreRestInterceptor, "post_list_instances") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = instance.ListInstancesResponse.to_json(instance.ListInstancesResponse())
...
def test_get_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.LustreRestInterceptor, "post_get_instance") as post, mock.patch.object(
        transports.LustreRestInterceptor, "post_get_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.LustreRestInterceptor, "pre_get_instance"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_create_instance_rest_bad_request(request_type=gcl_instance.CreateInstanceRequest):
    client = LustreClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_create_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcl_instance.CreateInstanceRequest.pb(gcl_instance.CreateInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_instance_rest_bad_request(request_type=gcl_instance.UpdateInstanceRequest):
    client = LustreClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_update_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcl_instance.UpdateInstanceRequest.pb(gcl_instance.UpdateInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_import_data_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.LustreRestInterceptor, "post_import_data"
    ) as post, mock.patch.object(
        transports.LustreRestInterceptor, "post_import_data_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.LustreRestInterceptor, "pre_import_data"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_export_data_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.LustreRestInterceptor, "post_export_data"
    ) as post, mock.patch.object(
        transports.LustreRestInterceptor, "post_export_data_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.LustreRestInterceptor, "pre_export_data"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_lustre_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.LustreTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_lustre_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.lustre_v1.services.lustre.transports.LustreTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.LustreGrpcTransport, grpc_helpers), (transports.LustreGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.LustreGrpcTransport, transports.LustreGrpcAsyncIOTransport])
...
def test_lustre_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.LustreRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_lustre_host_no_port(transport_name):
    client = LustreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="lustre.googleapis.com"),
        transport=transport_name,
    )
    assert client.transport._host == ("lustre.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://lustre.googleapis.com")
...
def test_lustre_host_with_port(transport_name):
    client = LustreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="lustre.googleapis.com:8000"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "lustre.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://lustre.googleapis.com:8000"
    )
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.LustreTransport, "_prep_wrapped_messages") as prep:
        client = LustreClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = LustreClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
