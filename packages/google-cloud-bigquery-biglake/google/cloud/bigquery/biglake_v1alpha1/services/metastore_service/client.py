...
class MetastoreServiceClient(metaclass=MetastoreServiceClientMeta):
    ...
    def lock_path(
        ...
        return "projects/{project}/locations/{location}/catalogs/{catalog}/databases/{database}/locks/{lock}".format(
            project=project,
            location=location,
            catalog=catalog,
            database=database,
    ...
    def parse_lock_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/databases/(?P<database>.+?)/locks/(?P<lock>.+?)$",
            path,
        )
        return m.groupdict() if m else {}
