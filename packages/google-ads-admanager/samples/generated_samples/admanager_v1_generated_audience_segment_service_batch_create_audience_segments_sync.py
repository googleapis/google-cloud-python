...
#
# Snippet for BatchCreateAudienceSegments
...
def sample_batch_create_audience_segments():
    # Create a client
    client = admanager_v1.AudienceSegmentServiceClient()

    # Initialize request argument(s)
    requests = admanager_v1.CreateAudienceSegmentRequest()
    requests.parent = "parent_value"

    request = admanager_v1.BatchCreateAudienceSegmentsRequest(
        parent="parent_value",
        requests=requests,
    )

    # Make the request
    response = client.batch_create_audience_segments(request=request)
