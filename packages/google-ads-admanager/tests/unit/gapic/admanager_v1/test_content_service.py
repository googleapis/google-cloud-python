...
from google.ads.admanager_v1.services.content_service import (
...
def test_get_content_rest_required_fields(
...
def test_get_content_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1/{name=networks/*/content/*}" % client.transport._host, args[1]
        )
...
def test_list_content_rest_required_fields(
    request_type=content_service.ListContentRequest,
):
    transport_class = transports.ContentServiceRestTransport
...
def test_list_content_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/{parent=networks/*}/content" % client.transport._host, args[1]
        )
...
def test_list_content_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(
            content_service.ListContentResponse.to_json(x) for x in response
        )
...
def test_get_content_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        pb_message = content_service.GetContentRequest.pb(
            content_service.GetContentRequest()
        )
...
def test_list_content_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = content_service.ListContentRequest.pb(
            content_service.ListContentRequest()
        )
        transcode.return_value = {
            "method": "post",
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = content_service.ListContentResponse.to_json(
            content_service.ListContentResponse()
        )
        req.return_value.content = return_value
...
def test_content_service_base_transport():
    ...
    with mock.patch(
        "google.ads.admanager_v1.services.content_service.transports.ContentServiceTransport.__init__"
    ) as Transport:
