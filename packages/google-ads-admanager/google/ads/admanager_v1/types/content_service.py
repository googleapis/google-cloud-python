...
from google.ads.admanager_v1.types import content_messages
...
class GetContentRequest(proto.Message):
    r"""Request object for ``GetContent`` method.

    Attributes:
        name (str):
            Required. The resource name of the Content. Format:
...
class ListContentRequest(proto.Message):
    r"""Request object for ``ListContent`` method.
    ...
    Attributes:
        ...
        page_size (int):
            Optional. The maximum number of ``Content`` to return. The
            service may return fewer than this value. If unspecified, at
            most 50 ``Content`` will be returned. The maximum value is
...
class ListContentResponse(proto.Message):
    r"""Response object for ``ListContentRequest`` containing matching
    ``Content`` objects.

    Attributes:
        content (MutableSequence[google.ads.admanager_v1.types.Content]):
        ...
        next_page_token (str):
            ...
            pages.
        total_size (int):
            Total number of ``Content`` objects. If a filter was
    ...
    content: MutableSequence[content_messages.Content] = proto.RepeatedField(
        proto.MESSAGE,
        number=1,
        message=content_messages.Content,
    )
