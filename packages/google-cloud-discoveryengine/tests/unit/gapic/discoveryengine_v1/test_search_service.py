...
from google.cloud.discoveryengine_v1.services.search_service import SearchServiceAsyncClient, SearchServiceClient, pagers, transports
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert SearchServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert SearchServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert SearchServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert SearchServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert SearchServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert SearchServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert SearchServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(SearchServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SearchServiceClient))
@mock.patch.object(SearchServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SearchServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = SearchServiceClient._DEFAULT_UNIVERSE
    default_endpoint = SearchServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = SearchServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert SearchServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert SearchServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == SearchServiceClient.DEFAULT_MTLS_ENDPOINT
    assert SearchServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert SearchServiceClient._get_api_endpoint(None, None, default_universe, "always") == SearchServiceClient.DEFAULT_MTLS_ENDPOINT
    assert (
        SearchServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == SearchServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert SearchServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert SearchServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        SearchServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert SearchServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert SearchServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert SearchServiceClient._get_universe_domain(None, None) == SearchServiceClient._DEFAULT_UNIVERSE
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = SearchServiceClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        (SearchServiceClient, transports.SearchServiceGrpcTransport, "grpc", grpc_helpers),
        (SearchServiceAsyncClient, transports.SearchServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_search_service_client_client_options_from_dict():
    with mock.patch("google.cloud.discoveryengine_v1.services.search_service.transports.SearchServiceGrpcTransport.__init__") as grpc_transport:
...
def test_search_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.search), "__call__") as call:
        ...
        retry = retries.Retry()
        timeout = 5
        expected_metadata = tuple(expected_metadata) + (gapic_v1.routing_header.to_grpc_metadata((("serving_config", ""),)),)
        pager = client.search(request={}, retry=retry, timeout=timeout)
...
def test_search_lite(request_type, transport: str = "grpc"):
    ...
    # Establish that the response is the type that we expect.
    assert isinstance(response, pagers.SearchLitePager)
    assert response.total_size == 1086
    assert response.attribution_token == "attribution_token_value"
    assert response.redirect_uri == "redirect_uri_value"
    assert response.next_page_token == "next_page_token_value"
...
def test_search_lite_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.search_lite in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.search_lite] = mock_rpc
        request = {}
        client.search_lite(request)
...
@pytest.mark.asyncio
async def test_search_lite_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = SearchServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.search_lite in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.search_lite] = mock_rpc

        request = {}
        await client.search_lite(request)
...
@pytest.mark.asyncio
async def test_search_lite_async(transport: str = "grpc_asyncio", request_type=search_service.SearchRequest):
    client = SearchServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    # Establish that the response is the type that we expect.
    assert isinstance(response, pagers.SearchLiteAsyncPager)
    assert response.total_size == 1086
    assert response.attribution_token == "attribution_token_value"
    assert response.redirect_uri == "redirect_uri_value"
    assert response.next_page_token == "next_page_token_value"
...
@pytest.mark.asyncio
async def test_search_lite_field_headers_async():
    client = SearchServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_search_lite_pager(transport_name: str = "grpc"):
    client = SearchServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport_name,
    )
    ...
    with mock.patch.object(type(client.transport.search_lite), "__call__") as call:
        ...
        timeout = 5
        expected_metadata = tuple(expected_metadata) + (gapic_v1.routing_header.to_grpc_metadata((("serving_config", ""),)),)
        pager = client.search_lite(request={}, retry=retry, timeout=timeout)
...
def test_search_lite_pages(transport_name: str = "grpc"):
    client = SearchServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport_name,
    )
    ...
    with mock.patch.object(type(client.transport.search_lite), "__call__") as call:
        ...
        pages = list(client.search_lite(request={}).pages)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token


@pytest.mark.asyncio
async def test_search_lite_async_pager():
    client = SearchServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    )

    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.search_lite), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            search_service.SearchResponse(
                results=[
        ...
        async_pager = await client.search_lite(
            request={},
        )
        assert async_pager.next_page_token == "abc"
        responses = []
...
@pytest.mark.asyncio
async def test_search_lite_async_pages():
    client = SearchServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    with mock.patch.object(type(client.transport.search_lite), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        # Workaround issue in python 3.9 related to code coverage by adding `# pragma: no branch`
        # See https://github.com/googleapis/gapic-generator-python/pull/1174#issuecomment-1025132372
        async for page_ in (await client.search_lite(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_search_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        sample_request = {"serving_config": "projects/sample1/locations/sample2/dataStores/sample3/servingConfigs/sample4"}
...
def test_search_lite_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = SearchServiceClient(
...
def test_search_lite_rest_required_fields(request_type=search_service.SearchRequest):
    transport_class = transports.SearchServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).search_lite._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_search_lite_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        pages = list(client.search_lite(request=sample_request).pages)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_transport_kind_grpc():
    transport = SearchServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_search_lite_empty_call_grpc():
    client = SearchServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.search_lite), "__call__") as call:
        call.return_value = search_service.SearchResponse()
        client.search_lite(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = SearchServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = SearchServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_search_lite_empty_call_grpc_asyncio():
    client = SearchServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
...
def test_transport_kind_rest():
    transport = SearchServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_search_rest_bad_request(request_type=search_service.SearchRequest):
...
def test_search_rest_interceptors(null_interceptor):
    transport = transports.SearchServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.SearchServiceRestInterceptor(),
    )
    client = SearchServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SearchServiceRestInterceptor, "post_search") as post, mock.patch.object(
        transports.SearchServiceRestInterceptor, "post_search_with_metadata"
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = search_service.SearchResponse.to_json(search_service.SearchResponse())
        ...
        post_with_metadata.return_value = search_service.SearchResponse(), metadata
...
def test_search_lite_rest_bad_request(request_type=search_service.SearchRequest):
    client = SearchServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"serving_config": "projects/sample1/locations/sample2/dataStores/sample3/servingConfigs/sample4"}
    request = request_type(**request_init)
...
def test_search_lite_rest_call_success(request_type):
    client = SearchServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_search_lite_rest_interceptors(null_interceptor):
    transport = transports.SearchServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.SearchServiceRestInterceptor(),
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SearchServiceRestInterceptor, "post_search_lite") as post, mock.patch.object(
        transports.SearchServiceRestInterceptor, "post_search_lite_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.SearchServiceRestInterceptor, "pre_search_lite"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_search_lite_empty_call_rest():
    client = SearchServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_search_service_base_transport():
    ...
    with mock.patch("google.cloud.discoveryengine_v1.services.search_service.transports.SearchServiceTransport.__init__") as Transport:
...
def test_search_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.cloud.discoveryengine_v1.services.search_service.transports.SearchServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
@pytest.mark.parametrize(
    ...
    [(transports.SearchServiceGrpcTransport, grpc_helpers), (transports.SearchServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_search_service_client_transport_session_collision(transport_name):
    ...
    session1 = client1.transport.search_lite._session
    session2 = client2.transport.search_lite._session
    assert session1 != session2
...
def test_chunk_path():
    ...
    actual = SearchServiceClient.chunk_path(project, location, data_store, branch, document, chunk)
    assert expected == actual
...
def test_document_path():
    ...
    actual = SearchServiceClient.document_path(project, location, data_store, branch, document)
    assert expected == actual
...
def test_serving_config_path():
    ...
    actual = SearchServiceClient.serving_config_path(project, location, data_store, serving_config)
    assert expected == actual
...
def test_session_path():
    ...
    actual = SearchServiceClient.session_path(project, location, data_store, session)
    assert expected == actual
...
def test_parse_session_path():
    ...
    # Check that the path construction is reversible.
    actual = SearchServiceClient.parse_session_path(path)
    assert expected == actual
