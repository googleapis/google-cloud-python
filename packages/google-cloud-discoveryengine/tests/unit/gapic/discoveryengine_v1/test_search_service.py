...
from google.cloud.discoveryengine_v1.services.search_service import transports
...
def test_search_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.discoveryengine_v1.services.search_service.transports.SearchServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
@pytest.mark.asyncio
async def test_search_lite_async_use_cached_wrapped_rpc(
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.search_lite
            in client._client._transport._wrapped_methods
        )
...
async def test_search_lite_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client.transport.search_lite), "__call__", new_callable=mock.AsyncMock
    ) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            search_service.SearchResponse(
...
def test_search_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = search_service.SearchResponse.to_json(
            search_service.SearchResponse()
        )
...
def test_search_lite_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(
            transports.SearchServiceRestInterceptor, "post_search_lite"
        ) as post,
        mock.patch.object(
...
def test_search_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.discoveryengine_v1.services.search_service.transports.SearchServiceTransport.__init__"
    ) as Transport:
