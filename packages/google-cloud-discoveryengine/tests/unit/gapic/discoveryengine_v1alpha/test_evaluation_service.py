...
from google.cloud.discoveryengine_v1alpha.services.evaluation_service import EvaluationServiceAsyncClient, EvaluationServiceClient, pagers, transports
from google.cloud.discoveryengine_v1alpha.types import evaluation_service, search_service
...
def test__get_api_endpoint():
    ...
    assert (
        EvaluationServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        EvaluationServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == EvaluationServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
@pytest.mark.parametrize("client_class", [EvaluationServiceClient, EvaluationServiceAsyncClient])
...
def test_evaluation_service_client_client_options_from_dict():
    with mock.patch(
        ...
        client = EvaluationServiceClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
def test_list_evaluation_results_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_evaluation_results), "__call__") as call:
        ...
        retry = retries.Retry()
        timeout = 5
        expected_metadata = tuple(expected_metadata) + (gapic_v1.routing_header.to_grpc_metadata((("evaluation", ""),)),)
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, evaluation_service.ListEvaluationResultsResponse.EvaluationResult) for i in results)
...
async def test_list_evaluation_results_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_evaluation_results), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            evaluation_service.ListEvaluationResultsResponse(
                evaluation_results=[
        ...
        assert len(responses) == 6
        assert all(isinstance(i, evaluation_service.ListEvaluationResultsResponse.EvaluationResult) for i in responses)
...
async def test_list_evaluation_results_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_evaluation_results), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        # Workaround issue in python 3.9 related to code coverage by adding `# pragma: no branch`
        # See https://github.com/googleapis/gapic-generator-python/pull/1174#issuecomment-1025132372
        async for page_ in (await client.list_evaluation_results(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_evaluation_results_rest_required_fields(request_type=evaluation_service.ListEvaluationResultsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_evaluation_results._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = EvaluationServiceClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = EvaluationServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = EvaluationServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = EvaluationServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = EvaluationServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = EvaluationServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_get_evaluation_rest_bad_request(request_type=evaluation_service.GetEvaluationRequest):
    client = EvaluationServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_evaluation_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.EvaluationServiceRestInterceptor, "post_get_evaluation") as post, mock.patch.object(
...
def test_list_evaluations_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.EvaluationServiceRestInterceptor, "post_list_evaluations") as post, mock.patch.object(
        transports.EvaluationServiceRestInterceptor, "post_list_evaluations_with_metadata"
    ) as post_with_metadata, mock.patch.object(
    ...
    request_init["evaluation"] = {
        ...
        "evaluation_spec": {
            "search_request": {
                ...
                "facet_specs": [
                    {
                        "facet_key": {
                            "key": "key_value",
                            "intervals": [
                                {"minimum": 0.764, "exclusive_minimum": 0.18430000000000002, "maximum": 0.766, "exclusive_maximum": 0.1845}
                            ],
                            "restricted_values": ["restricted_values_value1", "restricted_values_value2"],
                        ...
                        "limit": 543,
                        "excluded_filter_keys": ["excluded_filter_keys_value1", "excluded_filter_keys_value2"],
                    ...
                    "condition_boost_specs": [
                        {
                            ...
                            "boost_control_spec": {
                                ...
                                "control_points": [{"attribute_value": "attribute_value_value", "boost_amount": 0.1306}],
                            },
                        }
                    ]
                },
                "params": {},
                "query_expansion_spec": {"condition": 1, "pin_unexpanded_results": True},
                "spell_correction_spec": {"mode": 1},
                "user_pseudo_id": "user_pseudo_id_value",
                "content_search_spec": {
                    "snippet_spec": {"max_snippet_count": 1840, "reference_only": True, "return_snippet": True},
                    "summary_spec": {
                ...
                "embedding_spec": {"embedding_vectors": [{"field_path": "field_path_value", "vector": [0.66, 0.661]}]},
                ...
                "session_spec": {"query_id": "query_id_value", "search_result_persistence_count": 3328},
                "relevance_threshold": 1,
            },
            "query_set_spec": {"sample_query_set": "sample_query_set_value"},
        },
        "quality_metrics": {
            "doc_recall": {"top_1": 0.483, "top_3": 0.485, "top_5": 0.487, "top_10": 0.531},
            "doc_precision": {},
            "doc_ndcg": {},
            "page_recall": {},
            "page_ndcg": {},
...
def test_create_evaluation_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.EvaluationServiceRestInterceptor, "post_create_evaluation_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_list_evaluation_results_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.EvaluationServiceRestInterceptor, "post_list_evaluation_results") as post, mock.patch.object(
        transports.EvaluationServiceRestInterceptor, "post_list_evaluation_results_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_evaluation_service_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.EvaluationServiceTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.EvaluationServiceGrpcTransport, grpc_helpers), (transports.EvaluationServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_evaluation_service_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.EvaluationServiceRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
@pytest.mark.parametrize("transport_class", [transports.EvaluationServiceGrpcTransport, transports.EvaluationServiceGrpcAsyncIOTransport])
def test_evaluation_service_transport_channel_mtls_with_client_cert_source(transport_class):
...
def test_evaluation_path():
    ...
    location = "octopus"
    evaluation = "oyster"
    expected = "projects/{project}/locations/{location}/evaluations/{evaluation}".format(
        project=project,
        location=location,
        evaluation=evaluation,
    )
...
def test_client_ctx():
    ...
    for transport in transports:
        client = EvaluationServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
