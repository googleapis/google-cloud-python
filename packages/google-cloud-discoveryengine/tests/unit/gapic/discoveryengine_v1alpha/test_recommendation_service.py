...
from google.cloud.discoveryengine_v1alpha.types import common, recommendation_service, user_event
...
def test__get_api_endpoint():
    ...
    assert (
        RecommendationServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        RecommendationServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == RecommendationServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_transport_kind_grpc():
    transport = RecommendationServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = RecommendationServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = RecommendationServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = RecommendationServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_recommend_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.RecommendationServiceRestInterceptor, "post_recommend") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.RecommendationServiceGrpcTransport, grpc_helpers), (transports.RecommendationServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
