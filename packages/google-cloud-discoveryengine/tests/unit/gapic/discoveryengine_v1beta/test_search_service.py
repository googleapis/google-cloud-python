...
from google.cloud.discoveryengine_v1beta.services.search_service import SearchServiceAsyncClient
from google.cloud.discoveryengine_v1beta.services.search_service import SearchServiceClient
from google.cloud.discoveryengine_v1beta.services.search_service import pagers
from google.cloud.discoveryengine_v1beta.services.search_service import transports
...
def test_search_service_client_client_options_from_dict():
    with mock.patch('google.cloud.discoveryengine_v1beta.services.search_service.transports.SearchServiceGrpcTransport.__init__') as grpc_transport:
...
def test_search_lite(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.search_lite),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = search_service.SearchResponse(
            total_size=1086,
...
async def test_search_lite_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.search_lite(request={},)
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager: # pragma: no branch
            responses.append(response)
...
def test_search_lite_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.SearchServiceRestInterceptor, "post_search_lite") as post, \
        mock.patch.object(transports.SearchServiceRestInterceptor, "post_search_lite_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SearchServiceRestInterceptor, "pre_search_lite") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = search_service.SearchRequest.pb(search_service.SearchRequest())
        ...
        client.search_lite(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_search_service_base_transport():
    ...
    with mock.patch('google.cloud.discoveryengine_v1beta.services.search_service.transports.SearchServiceTransport.__init__') as Transport:
...
def test_search_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.discoveryengine_v1beta.services.search_service.transports.SearchServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_search_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.discoveryengine_v1beta.services.search_service.transports.SearchServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.SearchServiceTransport()
        adc.assert_called_once()
