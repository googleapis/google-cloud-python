...
from google.cloud.discoveryengine_v1beta.services.control_service import ControlServiceAsyncClient, ControlServiceClient, pagers, transports
...
def test__get_api_endpoint():
    ...
    assert (
        ControlServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ControlServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    ...
    assert (
        ControlServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
...
async def test_list_controls_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_controls), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_controls(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_transport_kind_grpc():
    transport = ControlServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = ControlServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ControlServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ControlServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_create_control_rest_call_success(request_type):
    ...
    request_init["control"] = {
        "boost_action": {"boost": 0.551, "filter": "filter_value", "data_store": "data_store_value"},
        ...
        "name": "name_value",
        "display_name": "display_name_value",
        "associated_serving_config_ids": ["associated_serving_config_ids_value1", "associated_serving_config_ids_value2"],
        "solution_type": 1,
        "use_cases": [1],
        "conditions": [
            {
                "query_terms": [{"value": "value_value", "full_match": True}],
                "active_time_range": [{"start_time": {"seconds": 751, "nanos": 543}, "end_time": {}}],
...
def test_create_control_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.ControlServiceRestInterceptor, "post_create_control") as post, mock.patch.object(
...
def test_delete_control_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ControlServiceRestInterceptor, "pre_delete_control") as pre:
        pre.assert_not_called()
...
def test_update_control_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ControlServiceRestInterceptor, "post_update_control") as post, mock.patch.object(
...
def test_get_control_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ControlServiceRestInterceptor, "post_get_control") as post, mock.patch.object(
...
def test_list_controls_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ControlServiceRestInterceptor, "post_list_controls") as post, mock.patch.object(
...
def test_control_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.cloud.discoveryengine_v1beta.services.control_service.transports.ControlServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
@pytest.mark.parametrize(
    ...
    [(transports.ControlServiceGrpcTransport, grpc_helpers), (transports.ControlServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
