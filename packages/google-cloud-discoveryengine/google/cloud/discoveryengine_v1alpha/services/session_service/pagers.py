...
from google.cloud.discoveryengine_v1alpha.types import conversational_search_service, session, session_service
...
class ListFilesPager:
    ...
    This class thinly wraps an initial
    :class:`google.cloud.discoveryengine_v1alpha.types.ListFilesResponse` object, and
    provides an ``__iter__`` method to iterate through its
    ``files`` field.
    ...
    All the usual :class:`google.cloud.discoveryengine_v1alpha.types.ListFilesResponse`
    attributes are available on the pager. If multiple requests are made, only
    the most recent response is retained, and thus used for attribute lookup.
    """
    ...
    def __init__(
        self,
        method: Callable[..., session_service.ListFilesResponse],
        ...
        *,
        ...
        Args:
            method (Callable): The method that was originally called, and
                which instantiated this pager.
            request (google.cloud.discoveryengine_v1alpha.types.ListFilesRequest):
                The initial request object.
            response (google.cloud.discoveryengine_v1alpha.types.ListFilesResponse):
                The initial response object.
            retry (google.api_core.retry.Retry): Designation of what errors,
                if any, should be retried.
            timeout (float): The timeout for this request.
        ...
        """
        self._method = method
        self._request = session_service.ListFilesRequest(request)
        self._response = response
        self._retry = retry
        self._timeout = timeout
        self._metadata = metadata
    ...
    @property
    def pages(self) -> Iterator[session_service.ListFilesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
    ...
    def __iter__(self) -> Iterator[session.FileMetadata]:
        for page in self.pages:
...
class ListFilesAsyncPager:
    ...
    def __init__(
        self,
        method: Callable[..., Awaitable[session_service.ListFilesResponse]],
        request: session_service.ListFilesRequest,
        response: session_service.ListFilesResponse,
        *,
        retry: OptionalAsyncRetry = gapic_v1.method.DEFAULT,
    ...
    @property
    async def pages(self) -> AsyncIterator[session_service.ListFilesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
    ...
    def __aiter__(self) -> AsyncIterator[session.FileMetadata]:
        async def async_generator():
            async for page in self.pages:
