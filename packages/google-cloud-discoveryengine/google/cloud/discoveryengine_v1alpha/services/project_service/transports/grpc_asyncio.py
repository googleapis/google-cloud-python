...
class ProjectServiceGrpcAsyncIOTransport(ProjectServiceTransport):
    ...
    def provision_project(
        ...
        if "provision_project" not in self._stubs:
            self._stubs["provision_project"] = self.grpc_channel.unary_unary(
    ...
    def report_consent_change(
        ...
        if "report_consent_change" not in self._stubs:
            self._stubs["report_consent_change"] = self.grpc_channel.unary_unary(
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.get_project: gapic_v1.method_async.wrap_method(
                self.get_project,
                default_timeout=None,
                client_info=client_info,
            ),
            self.provision_project: gapic_v1.method_async.wrap_method(
                self.provision_project,
                default_timeout=None,
                client_info=client_info,
            ),
            self.report_consent_change: gapic_v1.method_async.wrap_method(
