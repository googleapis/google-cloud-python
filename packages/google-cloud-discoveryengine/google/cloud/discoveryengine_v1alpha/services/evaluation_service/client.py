...
class EvaluationServiceClient(metaclass=EvaluationServiceClientMeta):
    ...
    def evaluation_path(
        ...
        return "projects/{project}/locations/{location}/evaluations/{evaluation}".format(
            project=project,
            location=location,
    ...
    def parse_serving_config_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)/servingConfigs/(?P<serving_config>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def __init__(
        ...
        *,
        ...
        self._api_endpoint = self._api_endpoint or EvaluationServiceClient._get_api_endpoint(
    ...
    def list_evaluation_results(
        ...
        *,
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("evaluation", request.evaluation),)),)
