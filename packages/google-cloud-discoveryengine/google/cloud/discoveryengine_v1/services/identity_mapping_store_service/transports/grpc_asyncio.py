...
class IdentityMappingStoreServiceGrpcAsyncIOTransport(IdentityMappingStoreServiceTransport):
    """gRPC AsyncIO backend transport for IdentityMappingStoreService.
    ...
    def create_identity_mapping_store(
        self,
    ) -> Callable[[identity_mapping_store_service.CreateIdentityMappingStoreRequest], Awaitable[gcd_identity_mapping_store.IdentityMappingStore]]:
        r"""Return a callable for the create identity mapping store method over gRPC.

        Creates a new Identity Mapping Store.

        Returns:
            Callable[[~.CreateIdentityMappingStoreRequest],
                    Awaitable[~.IdentityMappingStore]]:
    ...
    def get_identity_mapping_store(
        self,
    ) -> Callable[[identity_mapping_store_service.GetIdentityMappingStoreRequest], Awaitable[identity_mapping_store.IdentityMappingStore]]:
        r"""Return a callable for the get identity mapping store method over gRPC.

        Gets the Identity Mapping Store.

        Returns:
            Callable[[~.GetIdentityMappingStoreRequest],
                    Awaitable[~.IdentityMappingStore]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
    ...
    def delete_identity_mapping_store(
        self,
    ) -> Callable[[identity_mapping_store_service.DeleteIdentityMappingStoreRequest], Awaitable[operations_pb2.Operation]]:
        r"""Return a callable for the delete identity mapping store method over gRPC.

        Deletes the Identity Mapping Store.

        Returns:
            Callable[[~.DeleteIdentityMappingStoreRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
    ...
    def import_identity_mappings(
        self,
    ) -> Callable[[identity_mapping_store_service.ImportIdentityMappingsRequest], Awaitable[operations_pb2.Operation]]:
        ...
        Returns:
            Callable[[~.ImportIdentityMappingsRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "import_identity_mappings" not in self._stubs:
            self._stubs["import_identity_mappings"] = self._logged_channel.unary_unary(
                "/google.cloud.discoveryengine.v1.IdentityMappingStoreService/ImportIdentityMappings",
                request_serializer=identity_mapping_store_service.ImportIdentityMappingsRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["import_identity_mappings"]

    @property
    def purge_identity_mappings(self) -> Callable[[identity_mapping_store_service.PurgeIdentityMappingsRequest], Awaitable[operations_pb2.Operation]]:
        ...
        Returns:
            Callable[[~.PurgeIdentityMappingsRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "purge_identity_mappings" not in self._stubs:
            self._stubs["purge_identity_mappings"] = self._logged_channel.unary_unary(
                "/google.cloud.discoveryengine.v1.IdentityMappingStoreService/PurgeIdentityMappings",
                request_serializer=identity_mapping_store_service.PurgeIdentityMappingsRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["purge_identity_mappings"]

    @property
    def list_identity_mappings(
        self,
    ) -> Callable[
        [identity_mapping_store_service.ListIdentityMappingsRequest], Awaitable[identity_mapping_store_service.ListIdentityMappingsResponse]
    ]:
        ...
        Returns:
            Callable[[~.ListIdentityMappingsRequest],
                    Awaitable[~.ListIdentityMappingsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "list_identity_mappings" not in self._stubs:
            self._stubs["list_identity_mappings"] = self._logged_channel.unary_unary(
                "/google.cloud.discoveryengine.v1.IdentityMappingStoreService/ListIdentityMappings",
                request_serializer=identity_mapping_store_service.ListIdentityMappingsRequest.serialize,
                response_deserializer=identity_mapping_store_service.ListIdentityMappingsResponse.deserialize,
            )
        return self._stubs["list_identity_mappings"]

    @property
    def list_identity_mapping_stores(
        self,
    ) -> Callable[
        [identity_mapping_store_service.ListIdentityMappingStoresRequest], Awaitable[identity_mapping_store_service.ListIdentityMappingStoresResponse]
    ]:
        ...
        Returns:
            Callable[[~.ListIdentityMappingStoresRequest],
                    Awaitable[~.ListIdentityMappingStoresResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.create_identity_mapping_store: self._wrap_method(
                self.create_identity_mapping_store,
                default_timeout=None,
                client_info=client_info,
            ),
            self.get_identity_mapping_store: self._wrap_method(
                self.get_identity_mapping_store,
                default_timeout=None,
                client_info=client_info,
            ),
            self.delete_identity_mapping_store: self._wrap_method(
                self.delete_identity_mapping_store,
                default_timeout=None,
                client_info=client_info,
            ),
            self.import_identity_mappings: self._wrap_method(
                self.import_identity_mappings,
                default_timeout=None,
                client_info=client_info,
            ),
            self.purge_identity_mappings: self._wrap_method(
                self.purge_identity_mappings,
                default_timeout=None,
                client_info=client_info,
            ),
            self.list_identity_mappings: self._wrap_method(
                self.list_identity_mappings,
                default_timeout=None,
                client_info=client_info,
            ),
            self.list_identity_mapping_stores: self._wrap_method(
                self.list_identity_mapping_stores,
                default_timeout=None,
                client_info=client_info,
            ),
...
__all__ = ("IdentityMappingStoreServiceGrpcAsyncIOTransport",)
