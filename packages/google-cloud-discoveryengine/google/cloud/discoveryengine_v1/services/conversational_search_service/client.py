...
from google.cloud.discoveryengine_v1.services.conversational_search_service import pagers
...
class ConversationalSearchServiceClient(metaclass=ConversationalSearchServiceClientMeta):
    ...
    @staticmethod
    def answer_path(project: str,location: str,data_store: str,session: str,answer: str,) -> str:
        """Returns a fully-qualified answer string."""
        return "projects/{project}/locations/{location}/dataStores/{data_store}/sessions/{session}/answers/{answer}".format(project=project, location=location, data_store=data_store, session=session, answer=answer, )

    @staticmethod
    def parse_answer_path(path: str) -> Dict[str,str]:
        """Parses a answer path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)/sessions/(?P<session>.+?)/answers/(?P<answer>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def assist_answer_path(project: str,location: str,collection: str,engine: str,session: str,assist_answer: str,) -> str:
        """Returns a fully-qualified assist_answer string."""
        return "projects/{project}/locations/{location}/collections/{collection}/engines/{engine}/sessions/{session}/assistAnswers/{assist_answer}".format(project=project, location=location, collection=collection, engine=engine, session=session, assist_answer=assist_answer, )

    @staticmethod
    def parse_assist_answer_path(path: str) -> Dict[str,str]:
        """Parses a assist_answer path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/collections/(?P<collection>.+?)/engines/(?P<engine>.+?)/sessions/(?P<session>.+?)/assistAnswers/(?P<assist_answer>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def chunk_path(project: str,location: str,data_store: str,branch: str,document: str,chunk: str,) -> str:
        """Returns a fully-qualified chunk string."""
        return "projects/{project}/locations/{location}/dataStores/{data_store}/branches/{branch}/documents/{document}/chunks/{chunk}".format(project=project, location=location, data_store=data_store, branch=branch, document=document, chunk=chunk, )

    @staticmethod
    def parse_chunk_path(path: str) -> Dict[str,str]:
        """Parses a chunk path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)/branches/(?P<branch>.+?)/documents/(?P<document>.+?)/chunks/(?P<chunk>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def conversation_path(project: str,location: str,data_store: str,conversation: str,) -> str:
        """Returns a fully-qualified conversation string."""
        return "projects/{project}/locations/{location}/dataStores/{data_store}/conversations/{conversation}".format(project=project, location=location, data_store=data_store, conversation=conversation, )

    @staticmethod
    def parse_conversation_path(path: str) -> Dict[str,str]:
        """Parses a conversation path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)/conversations/(?P<conversation>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def data_store_path(project: str,location: str,data_store: str,) -> str:
        """Returns a fully-qualified data_store string."""
        return "projects/{project}/locations/{location}/dataStores/{data_store}".format(project=project, location=location, data_store=data_store, )

    @staticmethod
    def parse_data_store_path(path: str) -> Dict[str,str]:
        """Parses a data_store path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    @staticmethod
    def serving_config_path(project: str,location: str,data_store: str,serving_config: str,) -> str:
        """Returns a fully-qualified serving_config string."""
        return "projects/{project}/locations/{location}/dataStores/{data_store}/servingConfigs/{serving_config}".format(project=project, location=location, data_store=data_store, serving_config=serving_config, )

    @staticmethod
    def parse_serving_config_path(path: str) -> Dict[str,str]:
        """Parses a serving_config path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)/servingConfigs/(?P<serving_config>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def session_path(project: str,location: str,data_store: str,session: str,) -> str:
        """Returns a fully-qualified session string."""
        return "projects/{project}/locations/{location}/dataStores/{data_store}/sessions/{session}".format(project=project, location=location, data_store=data_store, session=session, )

    @staticmethod
    def parse_session_path(path: str) -> Dict[str,str]:
        """Parses a session path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/dataStores/(?P<data_store>.+?)/sessions/(?P<session>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = ConversationalSearchServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, ConversationalSearchServiceTransport, Callable[..., ConversationalSearchServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = ConversationalSearchServiceClient._read_environment_variables()
        self._client_cert_source = ConversationalSearchServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = ConversationalSearchServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[ConversationalSearchServiceTransport], Callable[..., ConversationalSearchServiceTransport]] = (
                ConversationalSearchServiceClient.get_transport_class(transport)
                if isinstance(transport, str) or transport is None
                else cast(Callable[..., ConversationalSearchServiceTransport], transport)
            )
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.discoveryengine_v1.ConversationalSearchServiceClient`.",
                    extra = {
    ...
    def converse_conversation(self,
    ...
    def stream_answer_query(self,
            request: Optional[Union[conversational_search_service.AnswerQueryRequest, dict]] = None,
            *,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: Union[float, object] = gapic_v1.method.DEFAULT,
            metadata: Sequence[Tuple[str, Union[str, bytes]]] = (),
            ) -> Iterable[conversational_search_service.AnswerQueryResponse]:
        r"""Answer query method (streaming).
        ...
        .. code-block:: python
            ...
            def sample_stream_answer_query():
                # Create a client
                client = discoveryengine_v1.ConversationalSearchServiceClient()
                ...
                # Make the request
                stream = client.stream_answer_query(request=request)
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.stream_answer_query]
