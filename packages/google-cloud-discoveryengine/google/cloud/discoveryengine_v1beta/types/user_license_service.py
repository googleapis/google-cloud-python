...
from google.cloud.discoveryengine_v1beta.types import user_license
    ...
    Attributes:
        ...
        filter (str):
            ...
            - ``license_assignment_state != NO_LICENSE_ATTEMPTED_LOGIN``
              ...
              assigned.
            - ``user_principal = user1@example.com`` to list user
        ...
        order_by (str):
            ...
            are listed. The value must be a comma-separated list of
            fields. Default sorting order is ascending. To specify
            descending order for a field, append a " desc" suffix.
            Redundant space characters in the syntax are insignificant.

            Supported fields (only ``user_principal`` is supported for
            now):
            ...
            - ``user_principal`` to order by ``user_principal`` in
              ascending order.
            - ``user_principal desc`` to order by ``user_principal`` in
              descending order.
    """
...
class ListUserLicensesResponse(proto.Message):
    ...
    Attributes:
        user_licenses (MutableSequence[google.cloud.discoveryengine_v1beta.types.UserLicense]):
            All the customer's
            [UserLicense][google.cloud.discoveryengine.v1beta.UserLicense]s.
        next_page_token (str):
...
class ListLicenseConfigsUsageStatsRequest(proto.Message):
    r"""Request message for
...
class ListLicenseConfigsUsageStatsResponse(proto.Message):
    r"""Response message for
    [UserLicenseService.ListLicenseConfigsUsageStats][google.cloud.discoveryengine.v1beta.UserLicenseService.ListLicenseConfigsUsageStats]
    method.

    Attributes:
        license_config_usage_stats (MutableSequence[google.cloud.discoveryengine_v1beta.types.LicenseConfigUsageStats]):
            All the customer's
            [LicenseConfigUsageStats][google.cloud.discoveryengine.v1beta.LicenseConfigUsageStats].
    """

    license_config_usage_stats: MutableSequence[
        user_license.LicenseConfigUsageStats
    ] = proto.RepeatedField(
        proto.MESSAGE,
        number=1,
        message=user_license.LicenseConfigUsageStats,
    )
...
class BatchUpdateUserLicensesRequest(proto.Message):
    ...
    Attributes:
        inline_source (google.cloud.discoveryengine_v1beta.types.BatchUpdateUserLicensesRequest.InlineSource):
