...
class CmekConfig(proto.Message):
    ...
    Attributes:
        ...
        kms_key (str):
            Required. KMS key resource name which will be used to
            encrypt resources
            ``projects/{project}/locations/{location}/keyRings/{keyRing}/cryptoKeys/{keyId}``.
        kms_key_version (str):
            Output only. KMS key version resource name which will be
            used to encrypt resources
            ``<kms_key>/cryptoKeyVersions/{keyVersion}``.
        state (google.cloud.discoveryengine_v1beta.types.CmekConfig.State):
        ...
        last_rotation_timestamp_micros (int):
            ...
            rotation.
        single_region_keys (MutableSequence[google.cloud.discoveryengine_v1beta.types.SingleRegionKey]):
            Optional. Single-regional CMEKs that are
            required for some VAIS features.
        notebooklm_state (google.cloud.discoveryengine_v1beta.types.CmekConfig.NotebookLMState):
    ...
    class State(proto.Enum):
        ...
        Values:
            ...
            EXPIRED (9):
                The KMS key is expired, meaning the key has
                been disabled for 30+ days. The customer can
                call DeleteCmekConfig to change the state to
                DELETED.
        """
...
class ListCmekConfigsResponse(proto.Message):
    ...
    Attributes:
        cmek_configs (MutableSequence[google.cloud.discoveryengine_v1beta.types.CmekConfig]):
            All the customer's
            [CmekConfig][google.cloud.discoveryengine.v1beta.CmekConfig]s.
    """
