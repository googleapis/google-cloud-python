    ...
    Attributes:
        ...
        data_schema (str):
            ...
            - ``content``: Unstructured data (e.g. PDF,
              ...
              becomes a document, with the ID set to the
              first 128   bits of SHA256(URI) encoded as a
              hex string.

            - ``custom``: One custom data JSON per row in
              arbitrary format that conforms   to the
              defined
            ...
            - ``csv``: A CSV file with header conforming to
              the defined
              [Schema][google.cloud.discoveryengine.v1beta.Schema]
              of the   data store. Each entry after the
              header is imported as a Document.   This can
              only be used by the GENERIC Data Store
              vertical.
    ...
    class BigtableColumnFamily(proto.Message):
        ...
        Attributes:
            field_name (str):
                ...
                the document. The name has to match the pattern
                ``[a-zA-Z0-9][a-zA-Z0-9-_]*``. If not set, it is
                parsed from the family name with best effort.
            ...
            encoding (google.cloud.discoveryengine_v1beta.types.BigtableOptions.Encoding):
                ...
                - ``TEXT``: indicates values are alphanumeric
                  ...
                  are encoded using ``HBase Bytes.toBytes``
                  family of functions. This can be overridden
                  for a specific column by listing that column
                  in ``columns`` and specifying an encoding for
                  it.
    ...
    class BigtableColumn(proto.Message):
        ...
        Attributes:
            ...
            field_name (str):
                ...
                ``[a-zA-Z0-9][a-zA-Z0-9-_]*``. If not set, it is
                parsed from the qualifier bytes with best
    ...
    class InlineSource(proto.Message):
        ...
        Attributes:
            documents (MutableSequence[google.cloud.discoveryengine_v1beta.types.Document]):
                ...
                Each document must have a valid
                [Document.id][google.cloud.discoveryengine.v1beta.Document.id].
