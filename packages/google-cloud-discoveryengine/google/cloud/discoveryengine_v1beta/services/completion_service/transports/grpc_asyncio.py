...
class CompletionServiceGrpcAsyncIOTransport(CompletionServiceTransport):
    ...
    def advanced_complete_query(
        self,
    ) -> Callable[[completion_service.AdvancedCompleteQueryRequest], Awaitable[completion_service.AdvancedCompleteQueryResponse]]:
        ...
        Returns:
            Callable[[~.AdvancedCompleteQueryRequest],
                    Awaitable[~.AdvancedCompleteQueryResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def import_suggestion_deny_list_entries(
        self,
    ) -> Callable[[import_config.ImportSuggestionDenyListEntriesRequest], Awaitable[operations_pb2.Operation]]:
    ...
    def purge_suggestion_deny_list_entries(
        self,
    ) -> Callable[[purge_config.PurgeSuggestionDenyListEntriesRequest], Awaitable[operations_pb2.Operation]]:
    ...
    @property
    def import_completion_suggestions(self) -> Callable[[import_config.ImportCompletionSuggestionsRequest], Awaitable[operations_pb2.Operation]]:
    ...
    @property
    def purge_completion_suggestions(self) -> Callable[[purge_config.PurgeCompletionSuggestionsRequest], Awaitable[operations_pb2.Operation]]:
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            ...
            self.advanced_complete_query: self._wrap_method(
