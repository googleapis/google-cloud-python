    ...
    * Stripping extraneous information from responses
    ...
    .. code-block:: python
        class MyCustomStreamsServiceInterceptor(StreamsServiceRestInterceptor):
            ...
            def pre_get_stream_thumbnail(self, request, metadata):
                logging.log(f"Received request: {request}")
                return request, metadata

            def post_get_stream_thumbnail(self, response):
                logging.log(f"Received response: {response}")
                return response
    ...
    def pre_get_stream_thumbnail(
        self, request: streams_service.GetStreamThumbnailRequest, metadata: Sequence[Tuple[str, Union[str, bytes]]]
    ) -> Tuple[streams_service.GetStreamThumbnailRequest, Sequence[Tuple[str, Union[str, bytes]]]]:
    ...
    def post_get_stream_thumbnail(self, response: operations_pb2.Operation) -> operations_pb2.Operation:
...
class StreamsServiceRestTransport(_BaseStreamsServiceRestTransport):
    ...
    class _GetStreamThumbnail(_BaseStreamsServiceRestTransport._BaseGetStreamThumbnail, StreamsServiceRestStub):
        def __hash__(self):
            return hash("StreamsServiceRestTransport.GetStreamThumbnail")
        ...
        def __call__(
            ...
            *,
            ...
            Args:
                request (~.streams_service.GetStreamThumbnailRequest):
                    The request object. Message for getting the thumbnail of
                a Stream.
                retry (google.api_core.retry.Retry): Designation of what errors, if any,
                    should be retried.
                timeout (float): The timeout for this request.
            ...
            http_options = _BaseStreamsServiceRestTransport._BaseGetStreamThumbnail._get_http_options()

            request, metadata = self._interceptor.pre_get_stream_thumbnail(request, metadata)
            transcoded_request = _BaseStreamsServiceRestTransport._BaseGetStreamThumbnail._get_transcoded_request(http_options, request)

            body = _BaseStreamsServiceRestTransport._BaseGetStreamThumbnail._get_request_body_json(transcoded_request)

            # Jsonify the query params
            query_params = _BaseStreamsServiceRestTransport._BaseGetStreamThumbnail._get_query_params_json(transcoded_request)
            ...
            response_metadata = [(k, str(v)) for k, v in response.headers.items()]
            resp, _ = self._interceptor.post_get_stream_thumbnail_with_metadata(resp, response_metadata)
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(logging.DEBUG):  # pragma: NO COVER
                try:
                    response_payload = json_format.MessageToJson(resp)
                except:
    ...
    def get_stream_thumbnail(self) -> Callable[[streams_service.GetStreamThumbnailRequest], operations_pb2.Operation]:
        # The return type is fine, but mypy isn't sophisticated enough to determine what's going on here.
        # In C++ this would require a dynamic_cast
        return self._GetStreamThumbnail(self._session, self._host, self._interceptor)  # type: ignore
