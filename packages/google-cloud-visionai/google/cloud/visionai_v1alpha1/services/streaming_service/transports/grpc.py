...
class StreamingServiceGrpcTransport(StreamingServiceTransport):
    ...
    def send_packets(
        ...
        if "send_packets" not in self._stubs:
            self._stubs["send_packets"] = self.grpc_channel.stream_stream(
    ...
    def receive_packets(
        ...
        if "receive_packets" not in self._stubs:
            self._stubs["receive_packets"] = self.grpc_channel.stream_stream(
    ...
    def receive_events(
        ...
        if "receive_events" not in self._stubs:
            self._stubs["receive_events"] = self.grpc_channel.stream_stream(
    ...
    def acquire_lease(
        ...
        if "acquire_lease" not in self._stubs:
            self._stubs["acquire_lease"] = self.grpc_channel.unary_unary(
    ...
    def release_lease(
        ...
        if "release_lease" not in self._stubs:
            self._stubs["release_lease"] = self.grpc_channel.unary_unary(
