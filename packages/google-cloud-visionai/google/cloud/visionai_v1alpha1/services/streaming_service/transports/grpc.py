...
from .base import DEFAULT_CLIENT_INFO, StreamingServiceTransport
...
class StreamingServiceGrpcTransport(StreamingServiceTransport):
    """gRPC backend transport for StreamingService.
    ...
    def send_packets(
        ...
        if "send_packets" not in self._stubs:
            self._stubs["send_packets"] = self._logged_channel.stream_stream(
                "/google.cloud.visionai.v1alpha1.StreamingService/SendPackets",
                request_serializer=streaming_service.SendPacketsRequest.serialize,
                response_deserializer=streaming_service.SendPacketsResponse.deserialize,
            )
    ...
    def receive_packets(
        ...
        Returns:
            Callable[[~.ReceivePacketsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "receive_packets" not in self._stubs:
            self._stubs["receive_packets"] = self._logged_channel.stream_stream(
                "/google.cloud.visionai.v1alpha1.StreamingService/ReceivePackets",
                request_serializer=streaming_service.ReceivePacketsRequest.serialize,
                response_deserializer=streaming_service.ReceivePacketsResponse.deserialize,
            )
    ...
    def receive_events(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "receive_events" not in self._stubs:
            self._stubs["receive_events"] = self._logged_channel.stream_stream(
                "/google.cloud.visionai.v1alpha1.StreamingService/ReceiveEvents",
                request_serializer=streaming_service.ReceiveEventsRequest.serialize,
                response_deserializer=streaming_service.ReceiveEventsResponse.deserialize,
            )
    ...
    def acquire_lease(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "acquire_lease" not in self._stubs:
            self._stubs["acquire_lease"] = self._logged_channel.unary_unary(
                "/google.cloud.visionai.v1alpha1.StreamingService/AcquireLease",
                request_serializer=streaming_service.AcquireLeaseRequest.serialize,
                response_deserializer=streaming_service.Lease.deserialize,
            )
    ...
    def renew_lease(
        ...
        if "renew_lease" not in self._stubs:
            self._stubs["renew_lease"] = self._logged_channel.unary_unary(
                "/google.cloud.visionai.v1alpha1.StreamingService/RenewLease",
                request_serializer=streaming_service.RenewLeaseRequest.serialize,
                response_deserializer=streaming_service.Lease.deserialize,
            )
    ...
    def release_lease(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "release_lease" not in self._stubs:
            self._stubs["release_lease"] = self._logged_channel.unary_unary(
                "/google.cloud.visionai.v1alpha1.StreamingService/ReleaseLease",
                request_serializer=streaming_service.ReleaseLeaseRequest.serialize,
                response_deserializer=streaming_service.ReleaseLeaseResponse.deserialize,
            )
        return self._stubs["release_lease"]
...
__all__ = ("StreamingServiceGrpcTransport",)
