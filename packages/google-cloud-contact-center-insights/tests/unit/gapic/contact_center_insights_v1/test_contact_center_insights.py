...
from google.cloud.contact_center_insights_v1.services.contact_center_insights.transports.base import (
...
def test_contact_center_insights_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_create_conversation_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].conversation == resources.Conversation(
            call_metadata=resources.Conversation.CallMetadata(customer_channel=1706)
        )
