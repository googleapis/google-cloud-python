...
from google.cloud.orchestration.airflow.service_v1.services.image_versions import ImageVersionsAsyncClient, ImageVersionsClient, pagers, transports
...
def test__get_api_endpoint():
    ...
    assert (
        ImageVersionsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ImageVersionsClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_transport_kind_grpc():
    transport = ImageVersionsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = ImageVersionsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ImageVersionsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ImageVersionsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_list_image_versions_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.ImageVersionsRestInterceptor, "post_list_image_versions") as post, mock.patch.object(
        transports.ImageVersionsRestInterceptor, "post_list_image_versions_with_metadata"
...
@pytest.mark.parametrize(
    ...
    [(transports.ImageVersionsGrpcTransport, grpc_helpers), (transports.ImageVersionsGrpcAsyncIOTransport, grpc_helpers_async)],
)
