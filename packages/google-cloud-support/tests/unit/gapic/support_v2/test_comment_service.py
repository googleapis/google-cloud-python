...
def test__get_api_endpoint():
    ...
    assert (
        CommentServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == CommentServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    ...
    assert (
        CommentServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
...
def test_comment_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "cloudsupport.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudsupport.googleapis.com"
        )
...
async def test_list_comments_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_comments), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_comments(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_transport_kind_grpc():
    transport = CommentServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = CommentServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = CommentServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = CommentServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_list_comments_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.CommentServiceRestInterceptor, "post_list_comments") as post, mock.patch.object(
...
def test_create_comment_rest_call_success(request_type):
    ...
    request_init["comment"] = {
        ...
        "creator": {"display_name": "display_name_value", "email": "email_value", "google_support": True, "username": "username_value"},
        "body": "body_value",
        "plain_text_body": "plain_text_body_value",
    }
...
def test_create_comment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.CommentServiceRestInterceptor, "post_create_comment") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.CommentServiceGrpcTransport, grpc_helpers), (transports.CommentServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_comment_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "cloudsupport.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudsupport.googleapis.com:8000"
    )
