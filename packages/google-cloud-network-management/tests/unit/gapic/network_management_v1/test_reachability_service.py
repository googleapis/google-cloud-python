...
def test__read_environment_variables():
    assert ReachabilityServiceClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert ReachabilityServiceClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                ReachabilityServiceClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert ReachabilityServiceClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ReachabilityServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ReachabilityServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ReachabilityServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert ReachabilityServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert ReachabilityServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                ReachabilityServiceClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert ReachabilityServiceClient._get_client_cert_source(None, False) is None
    assert ReachabilityServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ReachabilityServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert ReachabilityServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ReachabilityServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(ReachabilityServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceClient))
@mock.patch.object(ReachabilityServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ReachabilityServiceClient._DEFAULT_UNIVERSE
    default_endpoint = ReachabilityServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ReachabilityServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ReachabilityServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        ReachabilityServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
        == ReachabilityServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert ReachabilityServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ReachabilityServiceClient._get_api_endpoint(None, None, default_universe, "always") == ReachabilityServiceClient.DEFAULT_MTLS_ENDPOINT
    assert (
        ReachabilityServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == ReachabilityServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert ReachabilityServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ReachabilityServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ReachabilityServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ReachabilityServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ReachabilityServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ReachabilityServiceClient._get_universe_domain(None, None) == ReachabilityServiceClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize(
    ...
    [
        (ReachabilityServiceClient, transports.ReachabilityServiceGrpcTransport, "grpc"),
        (ReachabilityServiceAsyncClient, transports.ReachabilityServiceGrpcAsyncIOTransport, "grpc_asyncio"),
        (ReachabilityServiceClient, transports.ReachabilityServiceRestTransport, "rest"),
    ],
)
@mock.patch.object(ReachabilityServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceClient))
@mock.patch.object(ReachabilityServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceAsyncClient))
...
@pytest.mark.parametrize(
    ...
    [
        (ReachabilityServiceClient, transports.ReachabilityServiceGrpcTransport, "grpc", "true"),
        (ReachabilityServiceAsyncClient, transports.ReachabilityServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (ReachabilityServiceClient, transports.ReachabilityServiceGrpcTransport, "grpc", "false"),
        (ReachabilityServiceAsyncClient, transports.ReachabilityServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
        (ReachabilityServiceClient, transports.ReachabilityServiceRestTransport, "rest", "true"),
        (ReachabilityServiceClient, transports.ReachabilityServiceRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(ReachabilityServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceClient))
@mock.patch.object(ReachabilityServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceAsyncClient))
...
@mock.patch.object(ReachabilityServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceClient))
@mock.patch.object(ReachabilityServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReachabilityServiceAsyncClient))
def test_reachability_service_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        (ReachabilityServiceClient, transports.ReachabilityServiceGrpcTransport, "grpc", grpc_helpers),
        (ReachabilityServiceAsyncClient, transports.ReachabilityServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (ReachabilityServiceClient, transports.ReachabilityServiceRestTransport, "rest", None),
    ],
)
...
def test_list_connectivity_tests_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.list_connectivity_tests in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_connectivity_tests] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_connectivity_tests_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_connectivity_tests in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_connectivity_tests] = mock_rpc
...
def test_list_connectivity_tests_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_connectivity_tests), "__call__") as call:
        ...
        pager = client.list_connectivity_tests(request={}, retry=retry, timeout=timeout)
...
async def test_list_connectivity_tests_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_connectivity_tests), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_connectivity_tests(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_connectivity_test_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_connectivity_test in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_connectivity_test] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_connectivity_test_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ReachabilityServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_connectivity_test in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_connectivity_test] = mock_rpc
...
def test_create_connectivity_test_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_connectivity_test in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_connectivity_test] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_connectivity_test_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ReachabilityServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_connectivity_test in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_connectivity_test] = mock_rpc
...
def test_update_connectivity_test_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_connectivity_test in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_connectivity_test] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_connectivity_test_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ReachabilityServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_connectivity_test in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_connectivity_test] = mock_rpc
...
def test_rerun_connectivity_test_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.rerun_connectivity_test in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.rerun_connectivity_test] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_rerun_connectivity_test_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ReachabilityServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.rerun_connectivity_test in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.rerun_connectivity_test] = mock_rpc
...
def test_delete_connectivity_test_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_connectivity_test in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_connectivity_test] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_connectivity_test_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ReachabilityServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_connectivity_test in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_connectivity_test] = mock_rpc
...
def test_list_connectivity_tests_rest_required_fields(request_type=reachability.ListConnectivityTestsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_connectivity_tests._get_unset_required_fields(
        jsonified_request
    )
...
def test_get_connectivity_test_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ReachabilityServiceClient(
...
def test_create_connectivity_test_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ReachabilityServiceClient(
...
def test_create_connectivity_test_rest_required_fields(request_type=reachability.CreateConnectivityTestRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_connectivity_test._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_update_connectivity_test_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ReachabilityServiceClient(
...
def test_update_connectivity_test_rest_required_fields(request_type=reachability.UpdateConnectivityTestRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_connectivity_test._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_rerun_connectivity_test_rest_required_fields(request_type=reachability.RerunConnectivityTestRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).rerun_connectivity_test._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_delete_connectivity_test_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ReachabilityServiceClient(
...
def test_delete_connectivity_test_rest_required_fields(request_type=reachability.DeleteConnectivityTestRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_connectivity_test._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_transport_kind_grpc():
    transport = ReachabilityServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_list_connectivity_tests_empty_call_grpc():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.list_connectivity_tests), "__call__") as call:
        call.return_value = reachability.ListConnectivityTestsResponse()
        client.list_connectivity_tests(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_connectivity_test_empty_call_grpc():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.get_connectivity_test), "__call__") as call:
        call.return_value = connectivity_test.ConnectivityTest()
        client.get_connectivity_test(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_connectivity_test_empty_call_grpc():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.create_connectivity_test), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.create_connectivity_test(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_connectivity_test_empty_call_grpc():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.update_connectivity_test), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.update_connectivity_test(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_rerun_connectivity_test_empty_call_grpc():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.rerun_connectivity_test), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.rerun_connectivity_test(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_connectivity_test_empty_call_grpc():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.delete_connectivity_test), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.delete_connectivity_test(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = ReachabilityServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_connectivity_tests_empty_call_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    ...
    with mock.patch.object(type(client.transport.list_connectivity_tests), "__call__") as call:
        ...
        await client.list_connectivity_tests(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_connectivity_test_empty_call_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.get_connectivity_test), "__call__") as call:
        ...
        await client.get_connectivity_test(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_connectivity_test_empty_call_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.create_connectivity_test), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.create_connectivity_test(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_update_connectivity_test_empty_call_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.update_connectivity_test), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.update_connectivity_test(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_rerun_connectivity_test_empty_call_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.rerun_connectivity_test), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.rerun_connectivity_test(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_delete_connectivity_test_empty_call_grpc_asyncio():
    client = ReachabilityServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.delete_connectivity_test), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.delete_connectivity_test(request=None)
...
def test_transport_kind_rest():
    transport = ReachabilityServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_connectivity_tests_rest_bad_request(request_type=reachability.ListConnectivityTestsRequest):
...
def test_list_connectivity_tests_rest_call_success(request_type):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_connectivity_tests_rest_interceptors(null_interceptor):
    transport = transports.ReachabilityServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ReachabilityServiceRestInterceptor(),
    )
    client = ReachabilityServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ReachabilityServiceRestInterceptor, "post_list_connectivity_tests") as post, mock.patch.object(
        transports.ReachabilityServiceRestInterceptor, "post_list_connectivity_tests_with_metadata"
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = reachability.ListConnectivityTestsResponse.to_json(reachability.ListConnectivityTestsResponse())
        ...
        post_with_metadata.return_value = reachability.ListConnectivityTestsResponse(), metadata
...
def test_get_connectivity_test_rest_bad_request(request_type=reachability.GetConnectivityTestRequest):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_connectivity_test_rest_call_success(request_type):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_connectivity_test_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ReachabilityServiceRestInterceptor, "post_get_connectivity_test") as post, mock.patch.object(
        transports.ReachabilityServiceRestInterceptor, "post_get_connectivity_test_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = connectivity_test.ConnectivityTest.to_json(connectivity_test.ConnectivityTest())
        req.return_value.content = return_value
...
def test_create_connectivity_test_rest_bad_request(request_type=reachability.CreateConnectivityTestRequest):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/global"}
    request = request_type(**request_init)
...
def test_create_connectivity_test_rest_call_success(request_type):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    request_init["resource"] = {
        ...
        "source": {
            ...
            "cloud_run_revision": {"uri": "uri_value", "service_uri": "service_uri_value"},
                    ...
                    "steps": [
                        {
                            ...
                            "route": {
                                ...
                                "network_uri": "network_uri_value",
                                "priority": 898,
                                "instance_tags": ["instance_tags_value1", "instance_tags_value2"],
                                "src_ip_range": "src_ip_range_value",
                                "dest_port_ranges": ["dest_port_ranges_value1", "dest_port_ranges_value2"],
                                "src_port_ranges": ["src_port_ranges_value1", "src_port_ranges_value2"],
                            ...
                            "endpoint": {},
                            "google_service": {"source_ip": "source_ip_value", "google_service_type": 1},
                            "forwarding_rule": {
                                "display_name": "display_name_value",
                                "uri": "uri_value",
                            ...
                            "hybrid_subnet": {"display_name": "display_name_value", "uri": "uri_value", "region": "region_value"},
                            "vpn_gateway": {
                                "display_name": "display_name_value",
                                "uri": "uri_value",
                                "network_uri": "network_uri_value",
                            ...
                            "vpc_connector": {"display_name": "display_name_value", "uri": "uri_value", "location": "location_value"},
                            ...
                            "serverless_external_connection": {"selected_ip_address": "selected_ip_address_value"},
                            "deliver": {
                                "target": 1,
                            ...
                            "forward": {"target": 1, "resource_uri": "resource_uri_value", "ip_address": "ip_address_value"},
                            "abort": {
                                "cause": 1,
                                "resource_uri": "resource_uri_value",
                                "ip_address": "ip_address_value",
                                "projects_missing_permission": ["projects_missing_permission_value1", "projects_missing_permission_value2"],
                            },
                            "drop": {
                                "cause": 1,
                                "resource_uri": "resource_uri_value",
                            ...
                            "nat": {
                                ...
                                "protocol": "protocol_value",
                                "network_uri": "network_uri_value",
                                "old_source_ip": "old_source_ip_value",
                                "new_source_ip": "new_source_ip_value",
                                "old_destination_ip": "old_destination_ip_value",
                                "new_destination_ip": "new_destination_ip_value",
                                "old_source_port": 1619,
                                "new_source_port": 1630,
                                "old_destination_port": 2148,
                            ...
                            "proxy_connection": {
                                ...
                                "old_destination_port": 2148,
                                "new_destination_port": 2159,
                                "subnet_uri": "subnet_uri_value",
                                "network_uri": "network_uri_value",
                            },
                            "load_balancer_backend_info": {
                                "name": "name_value",
                                "instance_uri": "instance_uri_value",
                                "backend_service_uri": "backend_service_uri_value",
                                "instance_group_uri": "instance_group_uri_value",
                                "network_endpoint_group_uri": "network_endpoint_group_uri_value",
                                "backend_bucket_uri": "backend_bucket_uri_value",
                                "psc_service_attachment_uri": "psc_service_attachment_uri_value",
                                "psc_google_api_target": "psc_google_api_target_value",
                                "health_check_uri": "health_check_uri_value",
        ...
        "probing_details": {
            ...
            "endpoint_info": {},
            "probing_latency": {"latency_percentiles": [{"percent": 753, "latency_micros": 1500}]},
            "destination_egress_location": {"metropolitan_area": "metropolitan_area_value"},
            "edge_responses": [
                {
                    "result": 1,
                    "sent_probe_count": 1721,
...
def test_create_connectivity_test_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.ReachabilityServiceRestInterceptor, "post_create_connectivity_test_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_update_connectivity_test_rest_bad_request(request_type=reachability.UpdateConnectivityTestRequest):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_update_connectivity_test_rest_call_success(request_type):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_update_connectivity_test_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.ReachabilityServiceRestInterceptor, "post_update_connectivity_test_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_rerun_connectivity_test_rest_bad_request(request_type=reachability.RerunConnectivityTestRequest):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/global/connectivityTests/sample2"}
    request = request_type(**request_init)
...
def test_rerun_connectivity_test_rest_call_success(request_type):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_rerun_connectivity_test_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.ReachabilityServiceRestInterceptor, "post_rerun_connectivity_test_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_delete_connectivity_test_rest_bad_request(request_type=reachability.DeleteConnectivityTestRequest):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/global/connectivityTests/sample2"}
    request = request_type(**request_init)
...
def test_delete_connectivity_test_rest_call_success(request_type):
    client = ReachabilityServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_delete_connectivity_test_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.ReachabilityServiceRestInterceptor, "post_delete_connectivity_test_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_cancel_operation_rest(request_type):
    ...
    request_init = {"name": "projects/sample1/locations/global/operations/sample2"}
    request = request_type(**request_init)
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, "request") as req:
        # Designate an appropriate value for the returned response.
...
def test_list_operations_rest(request_type):
    ...
    request_init = {"name": "projects/sample1/locations/global"}
    request = request_type(**request_init)
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, "request") as req:
        # Designate an appropriate value for the returned response.
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_connectivity_tests_empty_call_rest():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_connectivity_test_empty_call_rest():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_connectivity_test_empty_call_rest():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_connectivity_test_empty_call_rest():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_rerun_connectivity_test_empty_call_rest():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_connectivity_test_empty_call_rest():
    client = ReachabilityServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
@pytest.mark.parametrize(
    ...
    [(transports.ReachabilityServiceGrpcTransport, grpc_helpers), (transports.ReachabilityServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
async def test_get_location_field_headers_async():
    client = ReachabilityServiceAsyncClient(credentials=async_anonymous_credentials())
