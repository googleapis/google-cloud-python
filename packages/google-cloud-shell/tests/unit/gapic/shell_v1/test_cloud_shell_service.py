...
def test__get_api_endpoint():
    ...
    assert (
        CloudShellServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        CloudShellServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == CloudShellServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_cloud_shell_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "cloudshell.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudshell.googleapis.com"
        )
...
def test_transport_kind_grpc():
    transport = CloudShellServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = CloudShellServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = CloudShellServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = CloudShellServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_get_environment_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.CloudShellServiceRestInterceptor, "post_get_environment") as post, mock.patch.object(
        transports.CloudShellServiceRestInterceptor, "post_get_environment_with_metadata"
...
def test_start_environment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.CloudShellServiceRestInterceptor, "post_start_environment_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_authorize_environment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.CloudShellServiceRestInterceptor, "post_authorize_environment_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_remove_public_key_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.CloudShellServiceRestInterceptor, "post_remove_public_key_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.CloudShellServiceGrpcTransport, grpc_helpers), (transports.CloudShellServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_cloud_shell_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "cloudshell.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudshell.googleapis.com:8000"
    )
