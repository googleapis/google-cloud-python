...
def test__get_default_mtls_endpoint():
    ...
    assert (
        PolicyTroubleshooterClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        PolicyTroubleshooterClient._get_client_cert_source(
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                PolicyTroubleshooterClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(PolicyTroubleshooterClient),
)
@mock.patch.object(
    PolicyTroubleshooterAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(PolicyTroubleshooterAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = PolicyTroubleshooterClient._DEFAULT_UNIVERSE
    default_endpoint = PolicyTroubleshooterClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = PolicyTroubleshooterClient._DEFAULT_ENDPOINT_TEMPLATE.format(
    ...
    assert (
        PolicyTroubleshooterClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        PolicyTroubleshooterClient._get_api_endpoint(
...
def test__get_universe_domain():
    ...
    assert (
        PolicyTroubleshooterClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        PolicyTroubleshooterClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        PolicyTroubleshooterClient._get_universe_domain(None, None)
        == PolicyTroubleshooterClient._DEFAULT_UNIVERSE
    )
...
def test_troubleshoot_iam_policy_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(
            transports.PolicyTroubleshooterRestInterceptor,
            "post_troubleshoot_iam_policy",
        ) as post,
        mock.patch.object(
            transports.PolicyTroubleshooterRestInterceptor,
