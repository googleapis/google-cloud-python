...
from .transports.rest import FunctionServiceRestTransport
...
class FunctionServiceClientMeta(type):
    ...
    _transport_registry = OrderedDict()  # type: Dict[str, Type[FunctionServiceTransport]]
    ...
    _transport_registry["rest"] = FunctionServiceRestTransport
...
class FunctionServiceClient(metaclass=FunctionServiceClientMeta):
    ...
    def get_mtls_endpoint_and_cert_source(cls, client_options: Optional[client_options_lib.ClientOptions] = None):
        ...
        use_client_cert = FunctionServiceClient._use_client_cert_effective()
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            _default_universe = FunctionServiceClient._DEFAULT_UNIVERSE
            if universe_domain != _default_universe:
                raise MutualTLSChannelError(f"mTLS is not supported in any universe other than {_default_universe}.")
            api_endpoint = FunctionServiceClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = FunctionServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = FunctionServiceClient._DEFAULT_UNIVERSE
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, FunctionServiceTransport, Callable[..., FunctionServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = FunctionServiceClient._read_environment_variables()
        self._client_cert_source = FunctionServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = FunctionServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, FunctionServiceTransport)
        if transport_provided:
            ...
            self._transport = cast(FunctionServiceTransport, transport)
            self._api_endpoint = self._transport.host

        self._api_endpoint = self._api_endpoint or FunctionServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[FunctionServiceTransport], Callable[..., FunctionServiceTransport]] = (
                FunctionServiceClient.get_transport_class(transport)
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.functions_v2.FunctionServiceClient`.",
                    extra={
    ...
    def create_function(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [parent, function, function_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
    ...
    def update_function(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [function, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("function.name", request.function.name),)),)
