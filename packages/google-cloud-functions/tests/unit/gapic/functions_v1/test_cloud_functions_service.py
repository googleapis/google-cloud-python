...
def test__get_default_mtls_endpoint():
    ...
    assert (
        CloudFunctionsServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        CloudFunctionsServiceClient._get_client_cert_source(
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                CloudFunctionsServiceClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(CloudFunctionsServiceClient),
)
@mock.patch.object(
    CloudFunctionsServiceAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(CloudFunctionsServiceAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = CloudFunctionsServiceClient._DEFAULT_UNIVERSE
    default_endpoint = CloudFunctionsServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = CloudFunctionsServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        CloudFunctionsServiceClient._get_api_endpoint(
...
def test__get_universe_domain():
    ...
    assert (
        CloudFunctionsServiceClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        CloudFunctionsServiceClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        CloudFunctionsServiceClient._get_universe_domain(None, None)
        == CloudFunctionsServiceClient._DEFAULT_UNIVERSE
    )
...
@pytest.mark.asyncio
async def test_call_function_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
...
async def test_call_function_async(request_type, transport: str = "grpc_asyncio"):
    client = CloudFunctionsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_transport_kind_grpc_asyncio():
    transport = CloudFunctionsServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = CloudFunctionsServiceClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_call_function_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = functions.CallFunctionResponse.to_json(
            functions.CallFunctionResponse()
        )
        req.return_value.content = return_value
