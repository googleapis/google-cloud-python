...
from .base import DEFAULT_CLIENT_INFO, ProductsServiceTransport
...
class ProductsServiceGrpcAsyncIOTransport(ProductsServiceTransport):
    ...
    @property
    def get_product(self) -> Callable[[products.GetProductRequest], Awaitable[products.Product]]:
        r"""Return a callable for the get product method over gRPC.

        Retrieves the processed product from your Merchant
        Center account.
        ...
        if "get_product" not in self._stubs:
            self._stubs["get_product"] = self._logged_channel.unary_unary(
                ...
                request_serializer=products.GetProductRequest.serialize,
                response_deserializer=products.Product.deserialize,
            )
        return self._stubs["get_product"]

    @property
    def list_products(self) -> Callable[[products.ListProductsRequest], Awaitable[products.ListProductsResponse]]:
        ...
        if "list_products" not in self._stubs:
            self._stubs["list_products"] = self._logged_channel.unary_unary(
                ...
                request_serializer=products.ListProductsRequest.serialize,
                response_deserializer=products.ListProductsResponse.deserialize,
            )
...
__all__ = ("ProductsServiceGrpcAsyncIOTransport",)
