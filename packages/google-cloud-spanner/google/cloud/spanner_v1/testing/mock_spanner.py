...
class SpannerServicer(spanner_grpc.SpannerServicer):
    ...
    @property
    def requests_peers(self):
        """Returns the list of client peer addresses captured parallel to self.requests."""
        return self._requests_peers
    ...
    def _record_request(self, request, context=None):
        ...
        if context is not None and hasattr(context, "peer"):
            self._requests_peers.append(context.peer())
        else:
            self._requests_peers.append(None)
