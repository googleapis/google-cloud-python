...
class TestTransaction(OpenTelemetryBase):
    ...
    @CrossSync.pytest
    async def test_lazy_stream_execute_sql_followed_by_execute_update_no_deadlock(self):
        from google.cloud.spanner_v1 import (
            ResultSet,
            ResultSetMetadata,
            ResultSetStats,
        ...
        result_set = await transaction.execute_sql("SELECT 1")
        self.assertIsNotNone(result_set)
        self.assertIsNone(transaction._transaction_id)

        row_count = await transaction.execute_update("UPDATE table SET x = 1")
        self.assertEqual(row_count, 1)
        self.assertEqual(transaction._transaction_id, TRANSACTION_ID)

    @CrossSync.pytest
    async def test_reset_and_begin_resets_affinity(self):
        from google.cloud.spanner_v1 import Transaction as TransactionPB
        ...
        await transaction._reset_and_begin()
        self.assertIsNotNone(transaction._affinity)
        self.assertIsNot(transaction._affinity, initial_affinity)
        self.assertEqual(transaction._transaction_id, TRANSACTION_ID)

    @CrossSync.pytest
    async def test_transaction_affinity_attached_to_statements(self):
        from google.cloud.spanner_v1 import (
            ResultSet,
            ResultSetMetadata,
            ResultSetStats,
        ...
        # 1. execute_update
        await transaction.execute_update("UPDATE table SET x = 1")
        ...
        # 2. batch_update
        await transaction.batch_update(statements=["UPDATE table SET x = 2"])
    ...
    @CrossSync.pytest
    async def test_transaction_commit_and_rollback_affinity_lifecycle(self):
        ...
        with mock.patch.object(
            ...
            await transaction_rollback.rollback()
