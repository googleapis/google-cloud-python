...
from google.cloud.spanner_v1._async.channel_pool import (
    ...
    TransactionAffinity,
    _AsyncStreamWrapper,
    _AsyncUnaryStreamMultiCallable,
    _AsyncUnaryUnaryCall,
    _AsyncUnaryUnaryMultiCallable,
...
class MockGrpcChannel:
    """Mock gRPC channel simulating unary and streaming calls."""

    def __init__(self, name="mock-channel"):
        self.name = name
        self.closed = False
    ...
    def unary_stream(self, method, request_serializer=None, response_deserializer=None):
        def invoker(
            ...
            return MockAsyncStream(["row_1", "row_2"])

        return invoker


class TestChannelPoolOptions(IsolatedAsyncioTestCase):
    ...
    def test_validation_max_rpc_per_channel_invalid(self):
        with self.assertRaises(ValueError):
            ChannelPoolOptions(max_rpc_per_channel=0)

    def test_validation_scale_up_step_factor_invalid(self):
        with self.assertRaises(ValueError):
            ChannelPoolOptions(scale_up_step_factor=0.0).validate()
        with self.assertRaises(ValueError):
            ChannelPoolOptions(scale_up_step_factor=1.5).validate()
    ...
    def test_validation_scale_down_period_invalid(self):
        with self.assertRaises(ValueError):
            ChannelPoolOptions(scale_down_period_seconds=0.0).validate()
    ...
    def test_from_dict(self):
        ...
        self.assertEqual(options.min_channels, 3)
        self.assertEqual(options.max_channels, 8)
        self.assertEqual(options.max_rpc_per_channel, 30)
...
class TestChannelEntry(IsolatedAsyncioTestCase):
    def test_channel_entry_counters(self):
        mock_channel = MockGrpcChannel("ch-1")
...
class TestP2CSelection(IsolatedAsyncioTestCase):
    def test_p2c_picks_less_loaded_channel(self):
        entry_1 = ChannelEntry(1, 1, MockGrpcChannel("ch-1"))
        entry_2 = ChannelEntry(2, 2, MockGrpcChannel("ch-2"))
        entry_1.increment_in_flight()
        entry_1.increment_in_flight()
        entry_2.increment_in_flight()

        # Both entries chosen, entry_2 has 1 in-flight vs entry_1 with 2
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.random.randrange",
            side_effect=[0, 0],
        ):
            picked = pick_channel_p2c([entry_1, entry_2])
            self.assertEqual(picked.entry_id, 2)

    def test_p2c_tie_breaker_active_rw_transactions(self):
        entry_1 = ChannelEntry(1, 1, MockGrpcChannel("ch-1"))
        entry_2 = ChannelEntry(2, 2, MockGrpcChannel("ch-2"))
        entry_1.increment_rw_transaction()

        # Same in-flight (0), entry_2 has 0 RW transactions vs entry_1 with 1
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.random.randrange",
            side_effect=[0, 0],
        ):
    ...
    def test_p2c_tie_breaker_random_when_load_equal(self):
        ...
        with mock.patch(
            ...
            picked = pick_channel_p2c([entry_1, entry_2])
            self.assertEqual(picked.entry_id, 1)
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.random.randrange",
            side_effect=[1, 0],
        ):
    ...
    def test_p2c_single_candidate(self):
        entry_1 = ChannelEntry(1, 1, MockGrpcChannel("ch-1"))
        picked = pick_channel_p2c([entry_1])
        self.assertEqual(picked.entry_id, 1)
        self.assertIsNone(pick_channel_p2c([]))

    def test_p2c_random_tie_breaker(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
...
class TestTransactionAffinity(IsolatedAsyncioTestCase):
    def test_read_write_affinity_increments_and_decrements_rw_count(self):
        entry = ChannelEntry(1, 1, MockGrpcChannel("ch-1"))
        ...
        # First pin sets entry_id and increments active_rw_transactions
        affinity.ensure_rw_guard(entry)
        self.assertEqual(affinity.entry_id, 1)
        self.assertEqual(entry.active_rw_transactions, 1)

        # Subsequent calls for the same entry do not duplicate increment
        affinity.ensure_rw_guard(entry)
        self.assertEqual(entry.active_rw_transactions, 1)

        # Resetting affinity decrements active_rw_transactions
        affinity.reset()
        self.assertIsNone(affinity.entry_id)
        self.assertEqual(entry.active_rw_transactions, 0)
    ...
    def test_pin_if_unpinned(self):
        affinity = TransactionAffinity.new_read_write()
        entry1 = ChannelEntry(1, 1, MockGrpcChannel("ch-1"))
        entry2 = ChannelEntry(2, 2, MockGrpcChannel("ch-2"))
...
class TestChannelPoolLifecycleAndDuckTyping(IsolatedAsyncioTestCase):
    async def test_static_channel_pool_from_channels(self):
        channels = [MockGrpcChannel("c1"), MockGrpcChannel("c2")]
        ...
        # Unary call through pool
        caller = pool.unary_unary("/test.Service/TestMethod")
        result = await caller("ping", metadata=[("header-key", "header-val")])
    ...
    async def test_request_id_formatting(self):
        channels = [MockGrpcChannel("c1")]
        pool = ChannelPool.from_channels(channels)
        ...
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.get_current_span",
            return_value=mock_span,
        ):
            caller = pool.unary_unary("/test.Service/TestMethod")
            result = await caller("test_req", metadata=metadata)
        ...
        # The 4th segment (index 3) should be replaced with logical_channel_id "1"
    ...
    async def test_dynamic_pool_start_and_ensure_background_tasks(self):
        created = []

        def factory():
            ch = MockGrpcChannel(f"dyn-{len(created)}")
            created.append(ch)
            return ch
        ...
        failing_channel = MockGrpcChannel("failing")
    ...
    async def test_transaction_affinity_pinning_read_write(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        ...
        affinity = TransactionAffinity.new_read_write()
        caller = pool.unary_unary("/test.Service/TestMethod")
        result1 = await caller(
            "query1", metadata=[("x-goog-spanner-affinity", affinity)]
        )
        first_channel = result1["channel"]
        self.assertIsNotNone(affinity.entry_id)

        # Second call in same transaction affinity pins to the exact same channel
        result2 = await caller(
            "query2", metadata=[("x-goog-spanner-affinity", affinity)]
        )
    ...
    async def test_soft_affinity_re_pins_when_draining(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool.from_channels([c1, c2])
    ...
    async def test_hard_affinity_sticks_when_draining(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool.from_channels([c1, c2])
    ...
    async def test_streaming_rpc_lifetime_holds_lease(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        ...
        # In-flight RPC count is incremented while streaming is active
        self.assertEqual(entry.in_flight_rpcs, 1)
        ...
        # In-flight RPC count decrements after stream exhaustion
        self.assertEqual(entry.in_flight_rpcs, 0)

        await pool.close()

    async def test_dict_metadata_request_id_and_affinity_stripping(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
    ...
    async def test_stream_wrapper_stop_iteration_does_not_cancel(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        ...
        self.assertEqual(items, ["row_1", "row_2"])
        self.assertEqual(entry.in_flight_rpcs, 0)
        # Normal completion should not cancel the stream
        self.assertFalse(stream._stream.closed)

        await pool.close()

    async def test_stream_wrapper_del_reclaims_lease(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        entry = pool._active_entries[0]
        ...
        # Abandon stream early without consuming to completion
        del stream
        gc.collect()
    ...
    async def test_stream_wrapper_last_chunk_finishes_lease(self):
        class MockChunk:
    ...
    async def test_stream_wrapper_read_last_chunk_finishes_lease(self):
        class MockChunk:
            def __init__(self, last=False):
                self.last = last
        ...
        self.assertEqual(entry.in_flight_rpcs, 1)
        res1 = await stream.read()
        self.assertIs(res1, chunk1)
        self.assertEqual(entry.in_flight_rpcs, 1)
        self.assertFalse(stream._closed)

        res2 = await stream.read()
        self.assertIs(res2, chunk2)
        self.assertEqual(entry.in_flight_rpcs, 0)
        self.assertTrue(stream._closed)
    ...
    async def test_interceptor_sharing(self):
        c1 = MockGrpcChannel("c1")
    ...
    async def test_stream_wrapper_read_and_cancel_and_getattr(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])

        # Test read() returning EOF
        stream_caller = pool.unary_stream("/test.Service/StreamingMethod")
        stream = stream_caller("stream_query")
        ...
        stream._stream.items = ["item1"]
        res1 = await stream.read()
        self.assertEqual(res1, "item1")

        # Mock next read returns aio.EOF
        stream._stream.items = [aio.EOF]
        eof_res = await stream.read()
        self.assertEqual(eof_res, aio.EOF)
        ...
        stream2 = stream_caller("stream_query2")

        async def failing_read():
            raise RuntimeError("stream read error")

        stream2._stream.read = failing_read
        with self.assertRaises(RuntimeError):
            await stream2.read()

        # Test exception during __anext__()
        stream3 = stream_caller("stream_query3")

        async def failing_anext():
            raise RuntimeError("stream anext error")

        stream3._stream.__anext__ = failing_anext
        with self.assertRaises(RuntimeError):
        ...
        stream4._stream.cancel = failing_cancel
        stream4.close()  # should not raise

        await pool.close()

    async def test_resolve_affinity_unpinned_pins_channel(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool.from_channels([c1, c2])
    ...
    async def test_acquire_empty_pool_raises_runtime_error(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
    ...
    async def test_pool_unsupported_streaming_and_stubs(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        ...
        await pool.get_state()
        await pool.wait_for_state_change(None)
        await pool.channel_ready()
    ...
    async def test_interceptor_sharing_all_four_types(self):
        c1 = MockGrpcChannel("c1")
        c1._unary_unary_interceptors = []
    ...
    async def test_multicallable_error_handling_releases_lease(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        entry = pool._active_entries[0]
    ...
    async def test_resolve_affinity_all_edge_branches(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        ...
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.pick_channel_p2c",
            return_value=None,
        ):
            self.assertIsNone(pool.resolve_affinity(aff))
...
class TestChannelPoolScaling(IsolatedAsyncioTestCase):
    async def test_scale_up_trigger_and_priming(self):
        created_channels = []

        def factory():
            ch = MockGrpcChannel(f"dyn-ch-{len(created_channels) + 1}")
        ...
        # Set prime session
        await pool.set_prime_session("projects/p/instances/i/databases/d/sessions/s123")

        # Simulate load: in_flight_rpcs > max_rpc_per_channel (2)
        entry = pool._active_entries[0]
        entry.increment_in_flight()
        entry.increment_in_flight()
        entry.increment_in_flight()

        # Run one scale-up iteration directly
        await pool._execute_scale_up()

        # Pool should have scaled up by min_scale_up_step = 2 channels (from 1 to 3)
        self.assertEqual(pool.active_channels_count, 3)

        await pool.close()

    async def test_scale_down_debouncing_and_draining_cleanup(self):
        channels = [
            MockGrpcChannel("c1"),
            MockGrpcChannel("c2"),
            MockGrpcChannel("c3"),
        ...
        await pool._execute_scale_down_check()
        self.assertEqual(pool.active_channels_count, 4)
        self.assertEqual(pool._consecutive_low_load_checks, 2)

        # Debouncing check 3: triggers scale down to desired_channel_count (min_channels = 2)
        await pool._execute_scale_down_check()
        self.assertEqual(pool.active_channels_count, 2)
        self.assertEqual(pool.draining_channels_count, 2)

        # Draining entries with 0 in-flight and 0 active RW are closed during draining sweep
    ...
    async def test_scale_down_no_visibility_gap(self):
        channels = [
            MockGrpcChannel("c1"),
            MockGrpcChannel("c2"),
            MockGrpcChannel("c3"),
    ...
    async def test_draining_channel_idle_timeout_for_abandoned_transaction(self):
        c1 = MockGrpcChannel("c1")
        ...
        # Simulate old activity older than DRAINING_TRANSACTION_TIMEOUT (15s)
        entry.last_activity = 100.0

        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.time.monotonic",
            return_value=120.0,
        ):
            await pool._sweep_draining_channels()
    ...
    async def test_priming_sends_gfe_headers_and_uses_prime_timeout(self):
        recorded_calls = []

        class PrimingCapturingChannel(MockGrpcChannel):
            def unary_unary(
                self, method, request_serializer=None, response_deserializer=None
            ):
                async def invoker(request, timeout=None, metadata=None, **kwargs):
        ...
        channel = PrimingCapturingChannel("priming-ch")
        options = ChannelPoolOptions(
            min_channels=1,
            max_channels=2,
    ...
    async def test_scale_up_cooldown_waits_for_cooldown(self):
        pool = ChannelPool(
        ...
        async def fake_wait(event, timeout=None):
        ...
        with mock.patch(
            ...
            with mock.patch(
                "google.cloud.aio._cross_sync.CrossSync.event_wait",
                side_effect=fake_wait,
            ):
                pool._scale_up_event.set()
                await pool._scale_up_worker_loop()

        self.assertIn(8.0, waited_timeouts)
        await pool.close()

    async def test_scale_up_edge_cases(self):
        c1 = MockGrpcChannel("c1")
        ...
        await pool.set_prime_session("projects/p/instances/i/databases/d/sessions/s1")
        c2 = MockGrpcChannel("c2")
        ...
        await pool._dial_prime_and_publish_one()

        # 5. _publish_primed_channel when at max channels closes channel
        pool._active_entries = pool._active_entries + (ChannelEntry(2, 2, c2),)
        c3 = MockGrpcChannel("c3")
        published = await pool._publish_primed_channel(c3)
        ...
        c4 = MockGrpcChannel("c4")
        published = await pool._publish_primed_channel(c4)
    ...
    async def test_scale_down_edge_cases(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        options = ChannelPoolOptions(
        ...
        await pool._evaluate_scale_down()
        ...
        # total_in_flight is now 0, avg_load is 0 < 10
        await pool._evaluate_scale_down()
        self.assertEqual(pool._consecutive_low_load_checks, 1)

        await pool.close()

    async def test_lazy_init_affinity_bypass_prevention(self):
    ...
    async def test_aborted_retry_preserves_affinity(self):
        ...
        with mock.patch(
            "google.cloud.spanner_v1._async.session._delay_until_retry",
            new_callable=AsyncMock,
        ):
            with mock.patch.object(Transaction, "commit", new_callable=AsyncMock):
                result = await session.run_in_transaction(work)
    ...
    async def test_multicallable_caching(self):
        c1 = MockGrpcChannel("c1")
    ...
    async def test_draining_safe_closure_conditions(self):
        c1 = MockGrpcChannel("c1")
        options = ChannelPoolOptions(min_channels=1, max_channels=2)
        pool = ChannelPool.from_channels([c1], options=options)
    ...
    async def test_scale_down_custom_consecutive_checks(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        c3 = MockGrpcChannel("c3")
        options = ChannelPoolOptions(
    ...
    async def test_database_channel_pool_property_and_close(self):
        ...
        database = await instance.database("test-db", channel_pool_options=options)
        ...
        mock_pool = ChannelPool.from_channels([MockGrpcChannel("c1")])
    ...
    async def test_channel_pool_duck_typing_stubs(self):
        pool = ChannelPool.from_channels([MockGrpcChannel("c1")])
        callback = mock.Mock()
        ...
        await pool.channel_ready()
        await pool.wait_for_state_change(state)
        await pool.close()
    ...
    async def test_channel_lease_context_managers_and_properties(self):
        c1 = MockGrpcChannel("c1")
    ...
    async def test_prime_channel_failure_scenarios(self):
        pool = ChannelPool(auto_start_background_tasks=False)
        c1 = MockGrpcChannel("c1")

        self.assertFalse(await pool._prime_channel(c1))
    ...
    async def test_publish_primed_channel_overflow_and_shutdown(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool(
        ...
        c2 = MockGrpcChannel("c2")
        self.assertFalse(await pool._publish_primed_channel(c2))
        self.assertTrue(c2.closed)

        c3 = MockGrpcChannel("c3")
        pool._shutdown_event.set()
        self.assertFalse(await pool._publish_primed_channel(c3))
        self.assertTrue(c3.closed)

    async def test_sweep_draining_channels_with_active_in_flight(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool(
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
    ...
    async def test_stream_wrapper_async_edge_cases(self):
        c1 = MockGrpcChannel("c1")
        entry = ChannelEntry(id=1, logical_channel_id=1, channel=c1)
        lease = ChannelLease(entry)

        mock_stream = mock.AsyncMock()
        mock_stream.read.return_value = aio.EOF
        mock_stream.cancel = mock.Mock()
        mock_stream.extra_attr = "stream_prop"

        wrapper = _AsyncStreamWrapper(mock_stream, lease)
        self.assertIs(wrapper.__aiter__(), wrapper)
        self.assertEqual(wrapper.extra_attr, "stream_prop")

        res = await wrapper.read()
        self.assertIs(res, aio.EOF)
        self.assertTrue(lease._released)

        lease2 = ChannelLease(entry)
        mock_stream2 = mock.AsyncMock()
        mock_stream2.read.side_effect = RuntimeError("read error")
        mock_stream2.cancel = mock.Mock()
        wrapper2 = _AsyncStreamWrapper(mock_stream2, lease2)
        with self.assertRaises(RuntimeError):
            await wrapper2.read()
        ...
        mock_stream3.__anext__.side_effect = StopAsyncIteration
        mock_stream3.cancel = mock.Mock()
        wrapper3 = _AsyncStreamWrapper(mock_stream3, lease3)
        with self.assertRaises(StopAsyncIteration):
            await wrapper3.__anext__()
        self.assertTrue(lease3._released)

        lease4 = ChannelLease(entry)
        mock_stream4 = mock.AsyncMock()
        mock_stream4.__anext__.side_effect = RuntimeError("stream err")
        mock_stream4.cancel = mock.Mock()
        wrapper4 = _AsyncStreamWrapper(mock_stream4, lease4)
        with self.assertRaises(RuntimeError):
            await wrapper4.__anext__()
        self.assertTrue(lease4._released)

        lease5 = ChannelLease(entry)
        mock_stream_sync = mock.Mock()
        mock_stream_sync.cancel = mock.Mock()
        wrapper5 = _AsyncStreamWrapper(mock_stream_sync, lease5)
        wrapper5.cancel()
        mock_stream_sync.cancel.assert_called_once()
        self.assertTrue(lease5._released)

        lease6 = ChannelLease(entry)
        wrapper6 = _AsyncStreamWrapper(mock_stream_sync, lease6)
        wrapper6.__del__()
        self.assertTrue(lease6._released)
    ...
    async def test_scale_up_execution_early_exits(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
            channel_factory=lambda: MockGrpcChannel("c2"),
        )
        pool._prime_session = "projects/p/instances/i/databases/d/sessions/s1"
        c2 = MockGrpcChannel("c2")
        pool._active_entries = pool._active_entries + (ChannelEntry(2, 2, c2),)
        ...
        pool._active_entries = (
            ChannelEntry(1, 1, MockGrpcChannel("c1")),
            ChannelEntry(2, 2, MockGrpcChannel("c2")),
        )
    ...
    async def test_scale_up_worker_and_monitor_single_step(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
            options=ChannelPoolOptions(
        ...
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.CrossSync.event_wait",
            return_value=None,
        ):
            await pool._scale_down_monitor_loop()
        await pool.close()

    async def test_ensure_background_tasks_branches(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
    ...
    def test_default_options_validate(self):
        ...
        opts.validate()
        self.assertEqual(opts.initial_channels, opts.min_channels)
        opts.initial_channels = None
        opts.validate()

    async def test_stream_wrapper_edge_branches(self):
        c1 = MockGrpcChannel("c1")
        entry = ChannelEntry(1, 1, c1)
        lease = ChannelLease(entry)
        mock_stream = mock.AsyncMock()
        ...
        mock_stream_no_cancel = mock.AsyncMock(spec=[])
        wrapper_no_cancel = _AsyncStreamWrapper(
            mock_stream_no_cancel, ChannelLease(entry)
        )
        wrapper_no_cancel.close()
        await wrapper_no_cancel.aclose()

    async def test_verify_loop_runtime_error(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
        ...
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool.CrossSync.verify_async_event_loop",
            side_effect=RuntimeError("no loop"),
        ):
            pool._start_background_tasks()
            pool._ensure_background_tasks()
        await pool.close()

    async def test_make_lease_and_pick_channel_none(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
    ...
    async def test_resolve_affinity_read_only_pinned(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1])
        entry1 = pool._active_entries[0]
    ...
    async def test_resolve_affinity_make_lease_none(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool.from_channels([c1, c2])
        ro_affinity = TransactionAffinity.new_read_only()
    ...
    async def test_resolve_affinity_read_only_repins_when_channel_draining(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool.from_channels([c1, c2])
        entry1 = pool._active_entries[0]
    ...
    async def test_scale_up_cooldown_branches(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
            options=ChannelPoolOptions(
    ...
    async def test_dial_prime_and_publish_one_priming_failure(self):
        c = MockGrpcChannel("c_fail")
    ...
    async def test_publish_primed_channel_occupied_slots_and_error(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(min_channels=1, max_channels=3),
            initial_channels=[MockGrpcChannel("c1")],
            auto_start_background_tasks=False,
        )
        draining_closed = ChannelEntry(10, 5, MockGrpcChannel("d1"))
        draining_closed.set_state(ChannelState.CLOSED)
        pool._draining_entries = (draining_closed,)

        c_new = MockGrpcChannel("c_new")
        self.assertTrue(await pool._publish_primed_channel(c_new))

        pool._shutdown_event.set()
        c_bad = MockGrpcChannel("c_bad")
        c_bad.close = mock.AsyncMock(side_effect=RuntimeError("close fail"))
        self.assertFalse(await pool._publish_primed_channel(c_bad))
        await pool.close()

    async def test_evaluate_scale_down_zero_channels_to_remove(self):
        options = ChannelPoolOptions(
            min_channels=2,
            max_channels=5,
            min_rpc_per_channel=10,
        ...
        pool = ChannelPool.from_channels(
            [MockGrpcChannel("c1"), MockGrpcChannel("c2"), MockGrpcChannel("c3")],
            options=options,
            auto_start_background_tasks=False,
        )
    ...
    async def test_stream_wrapper_context_manager(self):
        c1 = MockGrpcChannel("c1")
        entry = ChannelEntry(1, 1, c1)
        entry.increment_in_flight()
        lease = ChannelLease(entry)
        ...
        mock_stream.__aexit__ = mock.AsyncMock(return_value=None)
        wrapper = _AsyncStreamWrapper(mock_stream, lease)
        self.assertEqual(entry.in_flight_rpcs, 1)
        async with wrapper as stream:
            self.assertIs(stream, wrapper)
        mock_stream.__aenter__.assert_awaited_once()
        ...
        entry.increment_in_flight()
        lease2 = ChannelLease(entry)
        wrapper2 = _AsyncStreamWrapper(mock_stream, lease2)
        self.assertEqual(entry.in_flight_rpcs, 1)
        with self.assertRaises(ValueError):
            async with wrapper2:
    ...
    async def test_dial_prime_and_publish_close_exception(self):
        c_bad = MockGrpcChannel("c_bad")
        c_bad.close = mock.AsyncMock(side_effect=RuntimeError("close failed"))
        pool = ChannelPool(
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
    ...
    async def test_publish_primed_channel_at_max_channels(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
            options=ChannelPoolOptions(min_channels=1, max_channels=1),
        ...
        c2 = MockGrpcChannel("c2")
        result = await pool._publish_primed_channel(c2)
    ...
    async def test_scale_up_worker_shutdown_on_wake(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
            initial_channels=[MockGrpcChannel("c1")],
            auto_start_background_tasks=False,
        ...
        pool._scale_up_event.set()
        await pool._run_scale_up_worker(pool_ref)

    async def test_scale_up_worker_shutdown_during_cooldown(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(
                min_channels=1, max_channels=2, scale_up_cooldown_seconds=0.01
            ),
    ...
    def test_resolve_caller_affinity_dict_metadata(self):
        pool = ChannelPool.from_channels(
            [MockGrpcChannel("c1")], auto_start_background_tasks=False
        )
        affinity = TransactionAffinity.new_read_write()
        resolved = pool._resolve_caller_affinity({"x-goog-spanner-affinity": affinity})
        self.assertIs(resolved, affinity)

    async def test_publish_primed_channel_at_max_channels_closes_channel(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(min_channels=1, max_channels=1),
            initial_channels=[MockGrpcChannel("c1")],
            auto_start_background_tasks=False,
        )
        extra_channel = MockGrpcChannel("extra")
        success = await pool._publish_primed_channel(extra_channel)
    ...
    def test_stream_wrapper_cancel_returns_bool(self):
        ...
        entry = ChannelEntry(1, 1, MockGrpcChannel("c1"))
        lease = ChannelLease(entry)
        from google.cloud.spanner_v1._async.channel_pool import _AsyncStreamWrapper

        wrapper = _AsyncStreamWrapper(mock_call, lease)
    ...
    def test_pin_entry_repin(self):
        affinity = TransactionAffinity.new_read_write()
        entry1 = ChannelEntry(1, 1, MockGrpcChannel("c1"))
        entry2 = ChannelEntry(2, 2, MockGrpcChannel("c2"))
    ...
    async def test_stream_wrapper_aenter_aexit_delegation(self):
        class MockAsyncStreamWithContext:
            def __init__(self):
                self.entered = False
                self.exited = False
        ...
        mock_stream = MockAsyncStreamWithContext()
        entry = ChannelEntry(1, 1, MockGrpcChannel("c1"))
        lease = ChannelLease(entry)
        from google.cloud.spanner_v1._async.channel_pool import _AsyncStreamWrapper
    ...
    async def test_publish_primed_channel_shutdown_while_priming(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(min_channels=1, max_channels=2),
            initial_channels=[MockGrpcChannel("c1")],
            auto_start_background_tasks=False,
...
class TestAsyncChannelPoolGrpcInterfaceAndGapicIntegration(IsolatedAsyncioTestCase):
    """Tests verifying async channel pool compliance with gRPC aio MultiCallable interfaces and GAPIC async clients."""

    async def test_unary_unary_is_multicallable_instance(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1], auto_start_background_tasks=False)
        callable_obj = pool.unary_unary("/google.spanner.v1.Spanner/Commit")
        self.assertIsInstance(callable_obj, aio.UnaryUnaryMultiCallable)
        self.assertIsInstance(callable_obj, _AsyncUnaryUnaryMultiCallable)

    async def test_unary_stream_is_multicallable_instance(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1], auto_start_background_tasks=False)
        callable_obj = pool.unary_stream(
            "/google.spanner.v1.Spanner/ExecuteStreamingSql"
        )
        self.assertIsInstance(callable_obj, aio.UnaryStreamMultiCallable)
        self.assertIsInstance(callable_obj, _AsyncUnaryStreamMultiCallable)

    async def test_unary_unary_call_is_aio_call_and_releases_lease(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1], auto_start_background_tasks=False)
        callable_obj = pool.unary_unary("/google.spanner.v1.Spanner/Commit")
        entry = pool._active_entries[0]
        ...
        call = callable_obj({"request": "data"})
        self.assertIsInstance(call, aio.UnaryUnaryCall)
        self.assertIsInstance(call, _AsyncUnaryUnaryCall)
        self.assertEqual(entry.in_flight_rpcs, 1)
    ...
    async def test_unary_unary_call_methods_and_cancellation(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1], auto_start_background_tasks=False)
        callable_obj = pool.unary_unary("/test")
        entry = pool._active_entries[0]

        call = callable_obj("test-req")
        self.assertEqual(entry.in_flight_rpcs, 1)
    ...
    async def test_unary_unary_call_error_releases_lease_and_attaches_request_id(
        self,
    ):
        class FailingAsyncChannel:
            def unary_unary(
                self, method, request_serializer=None, response_deserializer=None
            ):
                async def invoker(*args, **kwargs):
                    raise RuntimeError("RPC async error")
        ...
        pool = ChannelPool.from_channels(
            [FailingAsyncChannel()], auto_start_background_tasks=False
        )
        callable_obj = pool.unary_unary("/test")
        entry = pool._active_entries[0]
        with self.assertRaises(RuntimeError) as ctx:
            await callable_obj(
                "req",
                metadata=(("x-goog-spanner-request-id", "async-req-123"),),
            )
        self.assertEqual(entry.in_flight_rpcs, 0)
        self.assertEqual(
            getattr(ctx.exception, "_spanner_request_id", None),
            "async-req-123",
        )

    async def test_unary_stream_call_is_aio_call_and_iterates(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1], auto_start_background_tasks=False)
        ...
        self.assertIsInstance(stream, aio.UnaryStreamCall)
        self.assertIsInstance(stream, _AsyncStreamWrapper)
        self.assertEqual(entry.in_flight_rpcs, 1)
    ...
    async def test_unary_stream_read_and_cancellation(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels([c1], auto_start_background_tasks=False)
        stream_callable = pool.unary_stream(
            "/google.spanner.v1.Spanner/ExecuteStreamingSql"
        ...
        item1 = await stream.read()
        self.assertEqual(item1, "row_1")
        self.assertEqual(entry.in_flight_rpcs, 1)

        stream.cancel()
        self.assertEqual(entry.in_flight_rpcs, 0)

    async def test_spanner_grpc_asyncio_transport_and_client_end_to_end(self):
        ...
        class MockAsyncGapicChannel:
            def unary_unary(
                self, method, request_serializer=None, response_deserializer=None
            ):
                class MockUnaryCall(aio.UnaryUnaryCall):
                ...
                def invoker(request, *args, **kwargs):
                    ...
                    if "BatchCreateSessions" in method:
                        return MockUnaryCall(BatchCreateSessionsResponse())
                    return MockUnaryCall(mock.MagicMock())
            ...
            def unary_stream(
                ...
                class MockStreamCall(aio.UnaryStreamCall):
                    def __init__(self):
        ...
        mock_channel = MockAsyncGapicChannel()
        pool = ChannelPool.from_channels(
            [mock_channel], auto_start_background_tasks=False
        )
        transport = SpannerGrpcAsyncIOTransport(channel=pool)
        client = SpannerAsyncClient(transport=transport)

        # 1. Unary call through GAPIC async client
        commit_resp = await client.commit(
        ...
        # 2. Batch create sessions
        batch_resp = await client.batch_create_sessions(
            request={
        ...
        # 3. Streaming call through GAPIC async client
        stream = await client.execute_streaming_sql(
            request={
    ...
    async def test_scale_up_worker_respects_min_scale_up_step(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(
                min_channels=1,
                max_channels=10,
        ...
        pool._channel_factory = mock.AsyncMock(
            side_effect=[
                MockGrpcChannel("c2"),
                MockGrpcChannel("c3"),
                MockGrpcChannel("c4"),
        ...
        pool._channel_primer = mock.AsyncMock()
    ...
    async def test_unary_unary_call_done_callback_releases_lease_without_await(self):
        class MockCallWithDoneCallback:
            def __init__(self):
                self._callbacks = []
        ...
        mock_call = MockCallWithDoneCallback()
        mock_channel = mock.Mock()
        mock_channel.unary_unary.return_value = mock.Mock(return_value=mock_call)
        ...
        mock_call.trigger_done()
        self.assertEqual(entry.in_flight_rpcs, 0)

    async def test_stream_wrapper_releases_on_last_chunk(self):
        class MockChunk:
            def __init__(self, last=False):
                self.last = last
        ...
        class MockDrainStream:
            ...
            async def __anext__(self):
                stream_calls.append("__anext__")
                if not self.items:
                    raise StopAsyncIteration
                return self.items.pop(0)

            async def read(self):
                stream_calls.append("read")
                if not self.items:
                    raise StopAsyncIteration
                return self.items.pop(0)

        mock_stream = MockDrainStream([chunk1, chunk2])
        mock_channel = mock.Mock()
        ...
        res1 = await stream.__anext__()
        self.assertIs(res1, chunk1)
        self.assertEqual(stream_calls, ["__anext__"])

        res2 = await stream.__anext__()
        self.assertIs(res2, chunk2)
        self.assertEqual(stream_calls, ["__anext__", "__anext__"])
        self.assertEqual(entry.in_flight_rpcs, 0)
        self.assertTrue(stream._closed)
    ...
    async def test_scale_up_promotes_draining_channels_before_dialing(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        pool = ChannelPool.from_channels(
            [c1, c2],
    ...
    async def test_scale_up_triggered_by_max_entry_load_hotspot(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        c3 = MockGrpcChannel("c3")
        c4 = MockGrpcChannel("c4")
        mock_factory = mock.Mock(side_effect=lambda: MockGrpcChannel("c_new"))
        pool = ChannelPool.from_channels(
            [c1, c2, c3, c4],
            options=ChannelPoolOptions(
                min_channels=4,
    ...
    async def test_format_request_metadata_tuple_and_fast_path_edge_cases(self):
        pool = ChannelPool(auto_start_background_tasks=False)
    ...
    async def test_scale_up_partial_promotion_of_draining_entries(self):
        c1 = MockGrpcChannel("c1")
        c2 = MockGrpcChannel("c2")
        c3 = MockGrpcChannel("c3")
        pool = ChannelPool.from_channels(
    ...
    async def test_dial_prime_and_publish_failure_and_shutdown_paths(self):
        c1 = MockGrpcChannel("c1")
        pool = ChannelPool.from_channels(
            [c1],
        ...
        failing_channel = MockGrpcChannel("failing")
        failing_channel.close = mock.AsyncMock()
        pool._channel_factory = mock.AsyncMock(return_value=failing_channel)
        with mock.patch.object(
            pool, "_warm_channel", new=mock.AsyncMock(return_value=False)
        ):
            await pool._dial_prime_and_publish_one()
        ...
        failing_prime_channel = MockGrpcChannel("failing_prime")
        failing_prime_channel.close = mock.AsyncMock()
        pool._channel_factory = mock.AsyncMock(return_value=failing_prime_channel)
        pool._prime_session = "projects/p/instances/i/databases/d/sessions/s1"
        with mock.patch.object(
            pool, "_prime_channel", new=mock.AsyncMock(return_value=False)
        ):
            await pool._dial_prime_and_publish_one()
        ...
        shutdown_channel = MockGrpcChannel("shutdown_ch")
        shutdown_channel.close = mock.AsyncMock()

        async def dial_and_shutdown():
            pool._shutdown_event.set()
            return shutdown_channel

        pool._channel_factory = dial_and_shutdown
        with mock.patch.object(
            pool, "_warm_channel", new=mock.AsyncMock(return_value=True)
        ):
            await pool._dial_prime_and_publish_one()
    ...
    async def test_warm_channel_direct_coverage(self):
        pool = ChannelPool(
            options=ChannelPoolOptions(prime_timeout_seconds=0.1),
            auto_start_background_tasks=False,
        )

        # 1. Success with channel_ready
        mock_channel = mock.Mock()
        mock_channel.channel_ready = mock.AsyncMock(return_value=None)
        self.assertTrue(await pool._warm_channel(mock_channel))
        mock_channel.channel_ready.assert_called_once()

        # 2. Timeout/failure with channel_ready logs and returns False
        mock_failing_channel = mock.Mock()
        mock_failing_channel.channel_ready = mock.AsyncMock(
            side_effect=asyncio.TimeoutError("timeout")
        )
        with mock.patch(
            "google.cloud.spanner_v1._async.channel_pool._LOGGER.debug"
        ) as mock_debug:
            self.assertFalse(await pool._warm_channel(mock_failing_channel))
        ...
        self.assertTrue(await pool._warm_channel(mock_get_state_channel))
        ...
        with mock.patch(
            ...
            self.assertFalse(await pool._warm_channel(mock_failing_get_state))
            mock_debug.assert_called_once()
        await pool.close()
