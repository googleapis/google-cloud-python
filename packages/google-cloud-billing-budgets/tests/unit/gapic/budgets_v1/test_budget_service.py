...
from google.cloud.billing.budgets_v1.services.budget_service import BudgetServiceAsyncClient, BudgetServiceClient, pagers, transports
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert BudgetServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert BudgetServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                BudgetServiceClient._use_client_cert_effective()
...
def test__get_api_endpoint():
    ...
    assert (
        BudgetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == BudgetServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_budget_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "billingbudgets.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://billingbudgets.googleapis.com"
        )
...
@pytest.mark.parametrize("client_class", [BudgetServiceClient, BudgetServiceAsyncClient])
...
def test_budget_service_client_client_options_from_dict():
    with mock.patch("google.cloud.billing.budgets_v1.services.budget_service.transports.BudgetServiceGrpcTransport.__init__") as grpc_transport:
        ...
        client = BudgetServiceClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
async def test_list_budgets_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_budgets), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            budget_service.ListBudgetsResponse(
                budgets=[
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = BudgetServiceClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = BudgetServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = BudgetServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = BudgetServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = BudgetServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = BudgetServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_create_budget_rest_bad_request(request_type=budget_service.CreateBudgetRequest):
    client = BudgetServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_create_budget_rest_call_success(request_type):
    ...
    request_init["budget"] = {
        ...
        "budget_filter": {
            ...
            "resource_ancestors": ["resource_ancestors_value1", "resource_ancestors_value2"],
            ...
            "labels": {},
            "calendar_period": 1,
            "custom_period": {"start_date": {"year": 433, "month": 550, "day": 318}, "end_date": {}},
        },
        "amount": {"specified_amount": {"currency_code": "currency_code_value", "units": 563, "nanos": 543}, "last_period_amount": {}},
        "threshold_rules": [{"threshold_percent": 0.1821, "spend_basis": 1}],
        "notifications_rule": {
            "pubsub_topic": "pubsub_topic_value",
            "schema_version": "schema_version_value",
            "monitoring_notification_channels": ["monitoring_notification_channels_value1", "monitoring_notification_channels_value2"],
...
def test_create_budget_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.BudgetServiceRestInterceptor, "post_create_budget") as post, mock.patch.object(
...
def test_update_budget_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BudgetServiceRestInterceptor, "post_update_budget") as post, mock.patch.object(
...
def test_get_budget_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BudgetServiceRestInterceptor, "post_get_budget") as post, mock.patch.object(
...
def test_list_budgets_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BudgetServiceRestInterceptor, "post_list_budgets") as post, mock.patch.object(
...
def test_delete_budget_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BudgetServiceRestInterceptor, "pre_delete_budget") as pre:
        pre.assert_not_called()
...
def test_budget_service_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.BudgetServiceTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.BudgetServiceGrpcTransport, grpc_helpers), (transports.BudgetServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_budget_service_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.BudgetServiceRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_budget_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "billingbudgets.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://billingbudgets.googleapis.com:8000"
    )
...
def test_client_ctx():
    ...
    for transport in transports:
        client = BudgetServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
