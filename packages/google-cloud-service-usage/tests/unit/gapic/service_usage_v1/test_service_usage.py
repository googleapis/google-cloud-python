...
def test_service_usage_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "serviceusage.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://serviceusage.googleapis.com"
        )
...
def test_transport_kind_grpc():
    transport = ServiceUsageClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = ServiceUsageAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ServiceUsageAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ServiceUsageClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_get_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServiceUsageRestInterceptor, "post_get_service") as post, mock.patch.object(
...
def test_list_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServiceUsageRestInterceptor, "post_list_services") as post, mock.patch.object(
...
def test_batch_enable_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.ServiceUsageRestInterceptor, "post_batch_enable_services_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_batch_get_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServiceUsageRestInterceptor, "post_batch_get_services") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.ServiceUsageGrpcTransport, grpc_helpers), (transports.ServiceUsageGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_service_usage_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "serviceusage.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://serviceusage.googleapis.com:8000"
    )
