...
from google.cloud.service_usage_v1.services.service_usage.transports.base import (
    _API_CORE_VERSION,
)
from google.cloud.service_usage_v1.services.service_usage.transports.base import (
...
def test_get_service(
    ...
    with mock.patch.object(type(client.transport.get_service), "__call__") as call:
        ...
        call.return_value = resources.Service(
            name="name_value", parent="parent_value", state=resources.State.DISABLED,
        )
...
async def test_list_services_async(
    ...
    with mock.patch.object(type(client.transport.list_services), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(
            serviceusage.ListServicesResponse(next_page_token="next_page_token_value",)
        )
...
def test_list_services_pager():
    ...
    with mock.patch.object(type(client.transport.list_services), "__call__") as call:
        ...
        call.side_effect = (
            ...
            serviceusage.ListServicesResponse(services=[], next_page_token="def",),
            serviceusage.ListServicesResponse(
                services=[resources.Service(),], next_page_token="ghi",
            ),
            serviceusage.ListServicesResponse(
                services=[resources.Service(), resources.Service(),],
            ),
            RuntimeError,
        )
...
def test_service_usage_transport_create_channel_old_api_core(
    transport_class, grpc_helpers
):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
@requires_api_core_lt_1_26_0
def test_service_usage_transport_create_channel_user_scopes(
    transport_class, grpc_helpers
):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
