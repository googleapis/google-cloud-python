...
from google.cloud.speech_v1.services.speech import (
    SpeechAsyncClient,
    SpeechClient,
    transports,
)
from google.cloud.speech_v1.types import cloud_speech, resource
...
def test_long_running_recognize_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[client._transport.long_running_recognize] = (
            mock_rpc
        )
        request = {}
...
def test_recognize_rest_required_fields(request_type=cloud_speech.RecognizeRequest):
    ...
    default_values = getattr(
        transport_class._BaseRecognize,
...
def test_recognize_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1/speech:recognize" % client.transport._host, args[1]
        )
...
def test_long_running_recognize_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseLongRunningRecognize,
        "_BaseLongRunningRecognize__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_long_running_recognize_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/speech:longrunningrecognize" % client.transport._host, args[1]
        )
...
def test_long_running_recognize_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(
            transports.SpeechRestInterceptor,
            "post_long_running_recognize_with_metadata",
        ) as post_with_metadata,
        mock.patch.object(
            transports.SpeechRestInterceptor, "pre_long_running_recognize"
        ) as pre,
    ):
        pre.assert_not_called()
        post.assert_not_called()
