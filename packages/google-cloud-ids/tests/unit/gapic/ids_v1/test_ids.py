...
from google.cloud.ids_v1.services.ids import IDSAsyncClient, IDSClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert IDSClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert IDSClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert IDSClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert IDSClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert IDSClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                IDSClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert IDSClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert IDSClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert IDSClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(IDSClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(IDSClient))
@mock.patch.object(IDSAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(IDSAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = IDSClient._DEFAULT_UNIVERSE
    default_endpoint = IDSClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = IDSClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert IDSClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert IDSClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == IDSClient.DEFAULT_MTLS_ENDPOINT
    assert IDSClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert IDSClient._get_api_endpoint(None, None, default_universe, "always") == IDSClient.DEFAULT_MTLS_ENDPOINT
    assert IDSClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == IDSClient.DEFAULT_MTLS_ENDPOINT
    assert IDSClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert IDSClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        IDSClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert IDSClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert IDSClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = IDSClient(credentials=cred)
...
def test_ids_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == ("ids.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://ids.googleapis.com")
...
def test_ids_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(IDSAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(IDSAsyncClient))
...
def test_ids_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (IDSAsyncClient, transports.IDSGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (IDSClient, transports.IDSRestTransport, "rest", None),
    ],
)
def test_ids_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_ids_client_client_options_from_dict():
    with mock.patch("google.cloud.ids_v1.services.ids.transports.IDSGrpcTransport.__init__") as grpc_transport:
...
def test_ids_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
@pytest.mark.asyncio
async def test_list_endpoints_async(transport: str = "grpc_asyncio", request_type=ids.ListEndpointsRequest):
    client = IDSAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_list_endpoints_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_endpoints), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(ids.ListEndpointsResponse())
        await client.list_endpoints(request)
...
@pytest.mark.asyncio
async def test_get_endpoint_async(transport: str = "grpc_asyncio", request_type=ids.GetEndpointRequest):
    client = IDSAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_create_endpoint_async(transport: str = "grpc_asyncio", request_type=ids.CreateEndpointRequest):
    client = IDSAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_delete_endpoint_async(transport: str = "grpc_asyncio", request_type=ids.DeleteEndpointRequest):
    client = IDSAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_list_endpoints_rest_required_fields(request_type=ids.ListEndpointsRequest):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            return_value = ids.ListEndpointsResponse.pb(return_value)
...
def test_list_endpoints_rest_unset_required_fields():
    transport = transports.IDSRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_get_endpoint_rest_required_fields(request_type=ids.GetEndpointRequest):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = ids.Endpoint.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = IDSClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = IDSClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = IDSClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = IDSAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = IDSAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = IDSClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_endpoints_rest_bad_request(request_type=ids.ListEndpointsRequest):
    client = IDSClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_endpoints_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.IDSRestInterceptor, "post_list_endpoints") as post, mock.patch.object(
        ...
        post_with_metadata.return_value = ids.ListEndpointsResponse(), metadata
...
def test_get_endpoint_rest_bad_request(request_type=ids.GetEndpointRequest):
    client = IDSClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_endpoint_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.IDSRestInterceptor, "post_get_endpoint") as post, mock.patch.object(
        ...
        post_with_metadata.return_value = ids.Endpoint(), metadata
...
def test_create_endpoint_rest_bad_request(request_type=ids.CreateEndpointRequest):
    client = IDSClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_create_endpoint_rest_call_success(request_type):
    ...
    test_field = ids.CreateEndpointRequest.meta.fields["endpoint"]
...
def test_delete_endpoint_rest_bad_request(request_type=ids.DeleteEndpointRequest):
    client = IDSClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/endpoints/sample3"}
    request = request_type(**request_init)
...
def test_ids_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.IDSTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_ids_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.ids_v1.services.ids.transports.IDSTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.IDSGrpcTransport, grpc_helpers), (transports.IDSGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.IDSGrpcTransport, transports.IDSGrpcAsyncIOTransport])
...
def test_ids_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.IDSRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_ids_host_with_port(transport_name):
    client = IDSClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="ids.googleapis.com:8000"),
        transport=transport_name,
    )
    assert client.transport._host == ("ids.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://ids.googleapis.com:8000")
...
def test_client_ctx():
    ...
    for transport in transports:
        client = IDSClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
