...
def test_apply_software_update_rest_required_fields(request_type=cloud_memcache.ApplySoftwareUpdateRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).apply_software_update._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_apply_software_update_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1beta2/{instance=projects/*/locations/*/instances/*}:applySoftwareUpdate" % client.transport._host, args[1]
        )
...
def test_reschedule_maintenance_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1beta2/{instance=projects/*/locations/*/instances/*}:rescheduleMaintenance" % client.transport._host, args[1]
        )
...
def test_apply_software_update_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.CloudMemcacheRestInterceptor, "post_apply_software_update_with_metadata"
    ) as post_with_metadata, mock.patch.object(
