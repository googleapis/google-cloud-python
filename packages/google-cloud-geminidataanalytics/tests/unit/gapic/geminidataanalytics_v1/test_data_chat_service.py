...
from google.cloud.geminidataanalytics_v1.services.data_chat_service import (
...
def test_data_chat_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.geminidataanalytics_v1.services.data_chat_service.transports.DataChatServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_list_messages_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/{parent=projects/*/locations/*/conversations/*}/messages"
            % client.transport._host,
            args[1],
        )
...
def test_data_chat_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.geminidataanalytics_v1.services.data_chat_service.transports.DataChatServiceTransport.__init__"
    ) as Transport:
...
def test_data_chat_service_base_transport_with_credentials_file():
    ...
    with (
        ...
        mock.patch(
            "google.cloud.geminidataanalytics_v1.services.data_chat_service.transports.DataChatServiceTransport._prep_wrapped_messages"
        ) as Transport,
    ):
        Transport.return_value = None
