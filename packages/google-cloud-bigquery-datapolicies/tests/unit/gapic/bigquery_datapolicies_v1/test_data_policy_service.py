...
def test_rename_data_policy_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._transport.rename_data_policy in client._transport._wrapped_methods
        )
        ...
        client._transport._wrapped_methods[client._transport.rename_data_policy] = (
            mock_rpc
        )
        request = {}
...
@pytest.mark.asyncio
async def test_rename_data_policy_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.rename_data_policy
            in client._client._transport._wrapped_methods
        )
...
async def test_rename_data_policy_async(request_type, transport: str = "grpc_asyncio"):
    client = DataPolicyServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_rename_data_policy_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseRenameDataPolicy,
