...
class AssuredWorkloadsServiceGrpcAsyncIOTransport(AssuredWorkloadsServiceTransport):
    ...
    def restrict_allowed_resources(
        ...
        Returns:
            Callable[[~.RestrictAllowedResourcesRequest],
                    Awaitable[~.RestrictAllowedResourcesResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def analyze_workload_move(
        self,
    ) -> Callable[[assuredworkloads.AnalyzeWorkloadMoveRequest], Awaitable[assuredworkloads.AnalyzeWorkloadMoveResponse]]:
        ...
        Returns:
            Callable[[~.AnalyzeWorkloadMoveRequest],
                    Awaitable[~.AnalyzeWorkloadMoveResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            ...
            self.analyze_workload_move: self._wrap_method(
