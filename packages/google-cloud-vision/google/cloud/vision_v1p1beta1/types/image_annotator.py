...
class Feature(proto.Message):
    r"""Users describe the type of Google Cloud Vision API tasks to
    perform over images by using *Feature*s. Each Feature indicates
    a type of image detection task to perform. Features encode the
    Cloud Vision API vertical to operate on and the number of
...
class ImageSource(proto.Message):
    ...
    Attributes:
        gcs_image_uri (str):
            NOTE: For new code ``image_uri`` below is
            preferred. Google Cloud Storage image URI, which
            must be in the following form:

            ``gs://bucket_name/object_name`` (for details,
        ...
        image_uri (str):
            ...
            1) Google Cloud Storage image URI, which must be
            ...
            <https://cloud.google.com/storage/docs/reference-uris>`__).
            NOTE: Cloud Storage object versioning is not
            supported. 2) Publicly accessible image
            HTTP/HTTPS URL.
            This is preferred over the legacy
            ``gcs_image_uri`` above. When both
            ``gcs_image_uri`` and ``image_uri`` are
