    ...
    class Type(proto.Enum):
        ...
        Values:
            ...
            TEXT_DETECTION (5):
                ...
                larger image; if the image is  a document, use ``DOCUMENT_TEXT_DETECTION``
                instead.
...
class ImageSource(proto.Message):
    ...
    Attributes:
        ...
        image_uri (str):
            ...
            1. A Google Cloud Storage URI of the form     ``gs://bucket_name/object_name``.
            ...
            images from     HTTP/HTTPS URLs, Google cannot guarantee
            that the request will be     completed. Your request may
            fail if the specified host denies the     request (e.g. due
...
class FaceAnnotation(proto.Message):
    ...
    Attributes:
        bounding_poly (google.cloud.vision_v1.types.BoundingPoly):
            ...
            bounding box  are in the original image's scale.  The
            bounding box is computed to "frame" the face in accordance
            with human  expectations. It is based on the landmarker
            results.  Note that one or more x and/or y coordinates may
            not be generated in the  ``BoundingPoly`` (the polygon will be
            unbounded) if only a partial face  appears in the image to
            be annotated.
...
class TextDetectionParams(proto.Message):
    ...
    Attributes:
        ...
        advanced_ocr_options (MutableSequence[str]):
            ...
            - ``legacy_layout``: a heuristics layout detection algorithm, which
              serves as an alternative to the current ML-based layout
              detection algorithm. Customers can choose the best
...
class BatchAnnotateImagesRequest(proto.Message):
    ...
    Attributes:
        ...
        parent (str):
            ...
            Supported location-ids:      ``us``: USA country only,
