...
Product = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      product_labels:
          ...
          may have up to 500 product_labels.  Notice that the total
...
ImportProductSetsGcsSource = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      csv_file_uri:
          ...
          isplay_name][google.cloud.vision.v1p4beta1.Product.display_nam
          e], the ``product-category`` column refers to [product_categor
          y][google.cloud.vision.v1p4beta1.Product.product_category],
          and the ``labels`` column refers to [product_labels][google.cl
          oud.vision.v1p4beta1.Product.product_labels].  The ``image-
          id`` column is optional but must be unique if provided. If it
          is empty, the system will automatically assign a unique id to
          the image.  The ``product-display-name`` column is optional.
          If it is empty, the system sets the [display_name][google.clou
          d.vision.v1p4beta1.Product.display_name] field for the product
          to a space (" "). You can update the ``display_name`` later by
          using the API.  If a ``Product`` with the specified ``product-
          id`` already exists, then the system ignores the ``product-
          display-name``, ``product-category``, and ``labels`` columns.
          ...
          non-negative integers for absolute bounding polygons, and
          float values in [0, 1] for normalized bounding polygons.  The
          system will resize the image if the image resolution is too
