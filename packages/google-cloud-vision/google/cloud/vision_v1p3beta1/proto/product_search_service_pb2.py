...
ImportProductSetsGcsSource = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      csv_file_uri:
          ...
          the image.  The product\_display\_name field is optional. If
          it is empty, a space (" ") is used as the place holder for the
          product display\_name, which can be updated later through the
          realtime API.  If the Product with product\_id already exists,
          the fields product\_display\_name, product\_category and
          labels are ignored.  If a Product doesn't exist and needs to
          be created on the fly, the product\_display\_name field refers
          to [Product.display\_name][google.cloud.vision.v1p3beta1.Produ
          ct.display\_name], the product\_category field refers to [Prod
          uct.product\_category][google.cloud.vision.v1p3beta1.Product.p
          roduct\_category], and the labels field refers to
          [Product.labels][].  Labels (optional) should be a line
          containing a list of comma-separated key-value pairs, with the
          format "key\_1=value\_1,key\_2=value\_2,...,key\_n=value\_n".
          The bounding\_poly (optional) field is used to identify one
          region of interest from the image in the same manner as
          CreateReferenceImage. If no bounding\_poly is specified, the
          ...
          that at most one bounding\_poly is allowed per line. If the
          image contains multiple regions of interest, the csv should
          contain one line per region of interest.  The bounding\_poly
          column should contain an even number of comma-separated
          numbers, with the format
          "p1\_x,p1\_y,p2\_x,p2\_y,...,pn\_x,pn\_y". Nonnegative
          integers should be used for absolute bounding polygons, and
