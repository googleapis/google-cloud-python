...
class ImportProductSetsGcsSource(proto.Message):
    ...
    Attributes:
        csv_file_uri (str):
            ...
            optional. A new
            ProductSet/Product with the same id will be created on the
            fly
            if the ProductSet/Product specified by
            product_set_id/product_id does not
            exist.

            The image_id field is optional but has to be unique if
            provided. If it is
            empty, we will automatically assign an unique id to the
            image.

            The product_display_name field is optional. If it is empty,
            a space (" ")
            is used as the place holder for the product display_name,
            which can
            ...
            CreateReferenceImage. If no
            bounding_poly is specified, the system will try to detect
            regions of
            ...
            line. If the image
            contains multiple regions of interest, the csv should
            ...
            "p1_x,p1_y,p2_x,p2_y,...,pn_x,pn_y". Nonnegative
