...
def test_cloud_scheduler_client_from_service_account_file(client_class):
    ...
    with mock.patch.object(
        ...
        assert client._transport._host == "cloudscheduler.googleapis.com:443"
...
@pytest.mark.asyncio
async def test_pause_job_async(transport: str = "grpc_asyncio"):
    client = CloudSchedulerAsyncClient(
        credentials=credentials.AnonymousCredentials(), transport=transport,
    )
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client._client._transport.pause_job), "__call__"
    ) as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(
            job.Job(
...
@pytest.mark.asyncio
async def test_resume_job_async(transport: str = "grpc_asyncio"):
    client = CloudSchedulerAsyncClient(
        credentials=credentials.AnonymousCredentials(), transport=transport,
    )
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client._client._transport.resume_job), "__call__"
    ) as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(
            job.Job(
...
@pytest.mark.asyncio
async def test_run_job_async(transport: str = "grpc_asyncio"):
    client = CloudSchedulerAsyncClient(
        credentials=credentials.AnonymousCredentials(), transport=transport,
    )
...
def test_transport_grpc_default():
    ...
    client = CloudSchedulerClient(credentials=credentials.AnonymousCredentials(),)
    assert isinstance(client._transport, transports.CloudSchedulerGrpcTransport,)
...
def test_cloud_scheduler_host_with_port():
    ...
    assert client._transport._host == "cloudscheduler.googleapis.com:8000"
...
def test_cloud_scheduler_grpc_transport_channel_mtls_with_client_cert_source(
...
def test_cloud_scheduler_grpc_asyncio_transport_channel_mtls_with_client_cert_source(
    grpc_create_channel, grpc_ssl_channel_cred
):
    # Check that if channel is None, but api_mtls_endpoint and client_cert_source
    # are provided, then a mTLS channel will be created.
...
@mock.patch("google.api_core.grpc_helpers.create_channel", autospec=True)
def test_cloud_scheduler_grpc_transport_channel_mtls_with_adc(
...
@mock.patch("google.api_core.grpc_helpers_async.create_channel", autospec=True)
def test_cloud_scheduler_grpc_asyncio_transport_channel_mtls_with_adc(
    grpc_create_channel, api_mtls_endpoint
):
    # Check that if channel and client_cert_source are None, but api_mtls_endpoint
    # is provided, then a mTLS channel will be created with SSL ADC.
