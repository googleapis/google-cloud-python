...
def test__get_api_endpoint():
    ...
    assert (
        IAMCredentialsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == IAMCredentialsClient.DEFAULT_MTLS_ENDPOINT
    )
    ...
    assert (
        IAMCredentialsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
...
def test_iam_credentials_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "iamcredentials.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://iamcredentials.googleapis.com"
        )
...
def test_transport_kind_grpc():
    transport = IAMCredentialsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = IAMCredentialsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = IAMCredentialsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = IAMCredentialsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_generate_access_token_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.IAMCredentialsRestInterceptor, "post_generate_access_token") as post, mock.patch.object(
        transports.IAMCredentialsRestInterceptor, "post_generate_access_token_with_metadata"
...
def test_generate_id_token_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.IAMCredentialsRestInterceptor, "post_generate_id_token") as post, mock.patch.object(
...
def test_sign_blob_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.IAMCredentialsRestInterceptor, "post_sign_blob") as post, mock.patch.object(
...
def test_sign_jwt_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.IAMCredentialsRestInterceptor, "post_sign_jwt") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.IAMCredentialsGrpcTransport, grpc_helpers), (transports.IAMCredentialsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_iam_credentials_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "iamcredentials.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://iamcredentials.googleapis.com:8000"
    )
