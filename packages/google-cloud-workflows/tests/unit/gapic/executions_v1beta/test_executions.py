...
def test__get_default_mtls_endpoint():
    ...
    assert (
        ExecutionsClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        ExecutionsClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        ExecutionsClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                ExecutionsClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                ExecutionsClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(ExecutionsClient),
)
@mock.patch.object(
    ExecutionsAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(ExecutionsAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ExecutionsClient._DEFAULT_UNIVERSE
    default_endpoint = ExecutionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = ExecutionsClient._DEFAULT_ENDPOINT_TEMPLATE.format(
    ...
    assert (
        ...
        == ExecutionsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        ExecutionsClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        ExecutionsClient._get_api_endpoint(None, None, default_universe, "always")
        == ExecutionsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        ExecutionsClient._get_api_endpoint(
    ...
    assert (
        ExecutionsClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        ExecutionsClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        ExecutionsClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        ExecutionsClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        ExecutionsClient._get_universe_domain(None, None)
        == ExecutionsClient._DEFAULT_UNIVERSE
    )
...
async def test_create_execution_async(request_type, transport: str = "grpc_asyncio"):
    client = ExecutionsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_cancel_execution_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[client._transport.cancel_execution] = (
            mock_rpc
        )
        request = {}
...
@pytest.mark.asyncio
async def test_cancel_execution_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.cancel_execution
            in client._client._transport._wrapped_methods
        )
...
async def test_cancel_execution_async(request_type, transport: str = "grpc_asyncio"):
    client = ExecutionsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
