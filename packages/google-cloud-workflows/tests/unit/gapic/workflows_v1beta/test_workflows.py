...
from google.cloud.workflows_v1beta.services.workflows import WorkflowsAsyncClient, WorkflowsClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert WorkflowsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert WorkflowsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert WorkflowsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert WorkflowsClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi


def test__read_environment_variables():
    assert WorkflowsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert WorkflowsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert WorkflowsClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert WorkflowsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert WorkflowsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert WorkflowsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert WorkflowsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert WorkflowsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                WorkflowsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert WorkflowsClient._get_client_cert_source(None, False) is None
    assert WorkflowsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert WorkflowsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert WorkflowsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert WorkflowsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(WorkflowsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkflowsClient))
@mock.patch.object(WorkflowsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkflowsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = WorkflowsClient._DEFAULT_UNIVERSE
    default_endpoint = WorkflowsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = WorkflowsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert WorkflowsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert WorkflowsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == WorkflowsClient.DEFAULT_MTLS_ENDPOINT
    assert WorkflowsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert WorkflowsClient._get_api_endpoint(None, None, default_universe, "always") == WorkflowsClient.DEFAULT_MTLS_ENDPOINT
    assert WorkflowsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == WorkflowsClient.DEFAULT_MTLS_ENDPOINT
    assert WorkflowsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert WorkflowsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        WorkflowsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert WorkflowsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert WorkflowsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert WorkflowsClient._get_universe_domain(None, None) == WorkflowsClient._DEFAULT_UNIVERSE
...
def test_workflows_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "workflows.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://workflows.googleapis.com"
        )
...
def test_workflows_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WorkflowsAsyncClient, transports.WorkflowsGrpcAsyncIOTransport, "grpc_asyncio"),
...
def test_workflows_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(WorkflowsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkflowsClient))
@mock.patch.object(WorkflowsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkflowsAsyncClient))
def test_workflows_client_client_api_endpoint(client_class):
...
def test_workflows_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WorkflowsAsyncClient, transports.WorkflowsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (WorkflowsClient, transports.WorkflowsRestTransport, "rest", None),
    ],
)
def test_workflows_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_workflows_client_client_options_from_dict():
    with mock.patch("google.cloud.workflows_v1beta.services.workflows.transports.WorkflowsGrpcTransport.__init__") as grpc_transport:
...
def test_workflows_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_list_workflows_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.list_workflows in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_workflows] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_workflows_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_workflows in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_workflows] = mock_rpc
...
def test_list_workflows_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_workflows), "__call__") as call:
        ...
        pager = client.list_workflows(request={}, retry=retry, timeout=timeout)
...
async def test_list_workflows_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_workflows), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            workflows.ListWorkflowsResponse(
                workflows=[
...
async def test_list_workflows_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_workflows), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_workflows(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_workflow_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_workflow in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_workflow] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_workflow_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkflowsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_workflow in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_workflow] = mock_rpc
...
def test_create_workflow_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_create_workflow_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_workflow in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_workflow] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_workflow_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkflowsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_workflow in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_workflow] = mock_rpc
...
def test_delete_workflow_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_delete_workflow_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_workflow in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_workflow] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_workflow_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkflowsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_workflow in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_workflow] = mock_rpc
...
def test_update_workflow_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_update_workflow_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_workflow in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_workflow] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_workflow_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkflowsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_workflow in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_workflow] = mock_rpc
...
def test_list_workflows_rest_required_fields(request_type=workflows.ListWorkflowsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_workflows._get_unset_required_fields(jsonified_request)
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            return_value = workflows.ListWorkflowsResponse.pb(return_value)
...
def test_list_workflows_rest_unset_required_fields():
    transport = transports.WorkflowsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_workflows_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v1beta/{parent=projects/*/locations/*}/workflows" % client.transport._host, args[1])
...
def test_get_workflow_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WorkflowsClient(
...
def test_get_workflow_rest_required_fields(request_type=workflows.GetWorkflowRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_workflow._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = workflows.Workflow.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_get_workflow_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/workflows/sample3"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{name=projects/*/locations/*/workflows/*}" % client.transport._host, args[1])
...
def test_create_workflow_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WorkflowsClient(
...
def test_create_workflow_rest_required_fields(request_type=workflows.CreateWorkflowRequest):
    transport_class = transports.WorkflowsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_workflow._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_delete_workflow_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WorkflowsClient(
...
def test_delete_workflow_rest_required_fields(request_type=workflows.DeleteWorkflowRequest):
    transport_class = transports.WorkflowsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_workflow._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_workflow_rest_required_fields(request_type=workflows.UpdateWorkflowRequest):
    transport_class = transports.WorkflowsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_workflow._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_workflow_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"workflow": {"name": "projects/sample1/locations/sample2/workflows/sample3"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{workflow.name=projects/*/locations/*/workflows/*}" % client.transport._host, args[1])
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = WorkflowsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = WorkflowsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
    assert client is not None


# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_workflows_empty_call_grpc():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.list_workflows), "__call__") as call:
        call.return_value = workflows.ListWorkflowsResponse()
        client.list_workflows(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_workflow_empty_call_grpc():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.get_workflow), "__call__") as call:
        call.return_value = workflows.Workflow()
        client.get_workflow(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_workflow_empty_call_grpc():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.create_workflow), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.create_workflow(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_workflow_empty_call_grpc():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.delete_workflow), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.delete_workflow(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_workflow_empty_call_grpc():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.update_workflow), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.update_workflow(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = WorkflowsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = WorkflowsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_workflows_empty_call_grpc_asyncio():
    client = WorkflowsAsyncClient(
        credentials=async_anonymous_credentials(),
    ...
    with mock.patch.object(type(client.transport.list_workflows), "__call__") as call:
        ...
        await client.list_workflows(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_workflow_empty_call_grpc_asyncio():
    client = WorkflowsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_workflow_empty_call_grpc_asyncio():
    client = WorkflowsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.create_workflow), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.create_workflow(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_delete_workflow_empty_call_grpc_asyncio():
    client = WorkflowsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.delete_workflow), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.delete_workflow(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_update_workflow_empty_call_grpc_asyncio():
    client = WorkflowsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.update_workflow), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.update_workflow(request=None)
...
def test_transport_kind_rest():
    transport = WorkflowsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_workflows_rest_bad_request(request_type=workflows.ListWorkflowsRequest):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_workflows_rest_call_success(request_type):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_workflows_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.WorkflowsRestInterceptor, "post_list_workflows") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = workflows.ListWorkflowsResponse.to_json(workflows.ListWorkflowsResponse())
...
def test_get_workflow_rest_call_success(request_type):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_workflow_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkflowsRestInterceptor, "post_get_workflow") as post, mock.patch.object(
...
def test_create_workflow_rest_call_success(request_type):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    test_field = workflows.CreateWorkflowRequest.meta.fields["workflow"]
    ...
    for field, value in request_init["workflow"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
    ...
    for subfield_to_delete in subfields_not_in_runtime:  # pragma: NO COVER
        ...
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init["workflow"][field])):
                    del request_init["workflow"][field][i][subfield]
            else:
                del request_init["workflow"][field][subfield]
    request = request_type(**request_init)
...
def test_create_workflow_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = workflows.CreateWorkflowRequest.pb(workflows.CreateWorkflowRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_workflow_rest_call_success(request_type):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_delete_workflow_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = workflows.DeleteWorkflowRequest.pb(workflows.DeleteWorkflowRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_workflow_rest_bad_request(request_type=workflows.UpdateWorkflowRequest):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"workflow": {"name": "projects/sample1/locations/sample2/workflows/sample3"}}
    request = request_type(**request_init)
...
def test_update_workflow_rest_call_success(request_type):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = workflows.UpdateWorkflowRequest.meta.fields["workflow"]
...
def test_update_workflow_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = workflows.UpdateWorkflowRequest.pb(workflows.UpdateWorkflowRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_workflows_empty_call_rest():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_workflow_empty_call_rest():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_workflow_empty_call_rest():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_workflow_empty_call_rest():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_workflow_empty_call_rest():
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_workflows_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.WorkflowsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_workflows_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.workflows_v1beta.services.workflows.transports.WorkflowsTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.WorkflowsGrpcTransport, grpc_helpers), (transports.WorkflowsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.WorkflowsGrpcTransport, transports.WorkflowsGrpcAsyncIOTransport])
...
def test_workflows_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.WorkflowsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_workflows_host_no_port(transport_name):
    client = WorkflowsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="workflows.googleapis.com"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "workflows.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://workflows.googleapis.com"
...
def test_workflows_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "workflows.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://workflows.googleapis.com:8000"
    )
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.WorkflowsTransport, "_prep_wrapped_messages") as prep:
        client = WorkflowsClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
async def test_get_location_field_headers_async():
    client = WorkflowsAsyncClient(credentials=async_anonymous_credentials())
...
def test_client_ctx():
    ...
    for transport in transports:
        client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
