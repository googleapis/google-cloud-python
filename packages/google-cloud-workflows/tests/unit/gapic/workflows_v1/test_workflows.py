...
from google.cloud.workflows_v1.services.workflows import WorkflowsAsyncClient, WorkflowsClient, pagers, transports
...
def test_get_workflow(request_type, transport: str = "grpc"):
    ...
    assert response.execution_history_level == workflows.ExecutionHistoryLevel.EXECUTION_HISTORY_BASIC
...
def test_list_workflow_revisions(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_workflow_revisions), "__call__") as call:
        # Designate an appropriate return value for the call.
...
def test_list_workflow_revisions_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.list_workflow_revisions in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_workflow_revisions] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_workflow_revisions_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkflowsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_workflow_revisions in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_workflow_revisions] = mock_rpc
...
@pytest.mark.asyncio
async def test_list_workflow_revisions_async(transport: str = "grpc_asyncio", request_type=workflows.ListWorkflowRevisionsRequest):
    client = WorkflowsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_list_workflow_revisions_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_workflow_revisions), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(workflows.ListWorkflowRevisionsResponse())
        await client.list_workflow_revisions(request)
...
async def test_list_workflow_revisions_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_workflow_revisions), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            workflows.ListWorkflowRevisionsResponse(
                workflows=[
...
async def test_list_workflow_revisions_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_workflow_revisions), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        # Workaround issue in python 3.9 related to code coverage by adding `# pragma: no branch`
        # See https://github.com/googleapis/gapic-generator-python/pull/1174#issuecomment-1025132372
        async for page_ in (await client.list_workflow_revisions(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_workflows_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v1/{parent=projects/*/locations/*}/workflows" % client.transport._host, args[1])
...
def test_get_workflow_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=projects/*/locations/*/workflows/*}" % client.transport._host, args[1])
...
def test_update_workflow_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{workflow.name=projects/*/locations/*/workflows/*}" % client.transport._host, args[1])
...
def test_list_workflow_revisions_rest_required_fields(request_type=workflows.ListWorkflowRevisionsRequest):
    transport_class = transports.WorkflowsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_workflow_revisions._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_list_workflow_revisions_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        # Wrap the values into proper Response objs
        response = tuple(workflows.ListWorkflowRevisionsResponse.to_json(x) for x in response)
        return_values = tuple(Response() for i in response)
        for return_val, response_val in zip(return_values, response):
            return_val._content = response_val.encode("UTF-8")
            return_val.status_code = 200
...
def test_create_workflow_rest_call_success(request_type):
    ...
    request_init["workflow"] = {
        ...
        "all_kms_keys_versions": ["all_kms_keys_versions_value1", "all_kms_keys_versions_value2"],
        "crypto_key_version": "crypto_key_version_value",
        "tags": {},
    }
...
def test_list_workflow_revisions_rest_bad_request(request_type=workflows.ListWorkflowRevisionsRequest):
    client = WorkflowsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/workflows/sample3"}
    request = request_type(**request_init)
...
def test_list_workflow_revisions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkflowsRestInterceptor, "post_list_workflow_revisions") as post, mock.patch.object(
        transports.WorkflowsRestInterceptor, "post_list_workflow_revisions_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = workflows.ListWorkflowRevisionsRequest.pb(workflows.ListWorkflowRevisionsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = workflows.ListWorkflowRevisionsResponse.to_json(workflows.ListWorkflowRevisionsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = workflows.ListWorkflowRevisionsResponse(), metadata
...
def test_crypto_key_version_path():
    ...
    actual = WorkflowsClient.crypto_key_version_path(project, location, keyRing, cryptoKey, cryptoKeyVersion)
    assert expected == actual
