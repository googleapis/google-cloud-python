...
class LineageClient(metaclass=LineageClientMeta):
    ...
    def parse_process_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/processes/(?P<process>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def parse_run_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/processes/(?P<process>.+?)/runs/(?P<run>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = LineageClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = LineageClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = LineageClient._DEFAULT_UNIVERSE
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, LineageTransport, Callable[..., LineageTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = LineageClient._read_environment_variables()
        self._client_cert_source = LineageClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = LineageClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, LineageTransport)
        ...
        self._api_endpoint = self._api_endpoint or LineageClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[LineageTransport], Callable[..., LineageTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.datacatalog.lineage_v1.LineageClient`.",
                    extra={
    ...
    def process_open_lineage_run_event(
        ...
        *,
        ...
        rpc = self._transport._wrapped_methods[self._transport.process_open_lineage_run_event]
    ...
    def batch_search_link_processes(
        ...
        *,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.batch_search_link_processes]
