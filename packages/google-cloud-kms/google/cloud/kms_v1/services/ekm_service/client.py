...
class EkmServiceClient(metaclass=EkmServiceClientMeta):
    ...
    def ekm_config_path(
        ...
        """Returns a fully-qualified ekm_config string."""
        return "projects/{project}/locations/{location}/ekmConfig".format(
            project=project,
            location=location,
        )

    @staticmethod
    def parse_ekm_config_path(path: str) -> Dict[str, str]:
        """Parses a ekm_config path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/ekmConfig$", path)
    ...
    def parse_ekm_connection_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/ekmConnections/(?P<ekm_connection>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def get_mtls_endpoint_and_cert_source(cls, client_options: Optional[client_options_lib.ClientOptions] = None):
        ...
        use_client_cert = EkmServiceClient._use_client_cert_effective()
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = EkmServiceClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = EkmServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = EkmServiceClient._DEFAULT_UNIVERSE
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, EkmServiceTransport, Callable[..., EkmServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = EkmServiceClient._read_environment_variables()
        self._client_cert_source = EkmServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = EkmServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, EkmServiceTransport)
        if transport_provided:
            ...
            self._transport = cast(EkmServiceTransport, transport)
            self._api_endpoint = self._transport.host

        self._api_endpoint = self._api_endpoint or EkmServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[EkmServiceTransport], Callable[..., EkmServiceTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.kms_v1.EkmServiceClient`.",
                    extra={
    ...
    def update_ekm_connection(
        ...
        *,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [ekm_connection, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("ekm_connection.name", request.ekm_connection.name),)),)
    ...
    def get_ekm_config(
        ...
        *,
        ...
        Args:
            request (Union[google.cloud.kms_v1.types.GetEkmConfigRequest, dict]):
                The request object. Request message for
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.get_ekm_config]
    ...
    def update_ekm_config(
        ...
        *,
        ...
        Args:
            request (Union[google.cloud.kms_v1.types.UpdateEkmConfigRequest, dict]):
                The request object. Request message for
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [ekm_config, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of " "the individual field arguments should be set.")
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.update_ekm_config]

        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("ekm_config.name", request.ekm_config.name),)),)
