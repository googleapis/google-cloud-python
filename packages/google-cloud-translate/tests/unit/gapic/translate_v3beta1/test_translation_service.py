...
def test__get_default_mtls_endpoint():
    ...
    assert (
        TranslationServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
@pytest.mark.asyncio
async def test_refine_text_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.refine_text
            in client._client._transport._wrapped_methods
        )
...
def test_refine_text_rest_required_fields(
    request_type=translation_service.RefineTextRequest,
):
    transport_class = transports.TranslationServiceRestTransport
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).refine_text._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_refine_text_rest_bad_request(
    request_type=translation_service.RefineTextRequest,
):
    client = TranslationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(), transport="rest"
...
def test_refine_text_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(path_template, "transcode") as transcode,
        mock.patch.object(
            transports.TranslationServiceRestInterceptor, "post_refine_text"
        ) as post,
        mock.patch.object(
            transports.TranslationServiceRestInterceptor,
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = translation_service.RefineTextRequest.pb(
            translation_service.RefineTextRequest()
        )
        transcode.return_value = {
            "method": "post",
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = translation_service.RefineTextResponse.to_json(
            translation_service.RefineTextResponse()
        )
        req.return_value.content = return_value
