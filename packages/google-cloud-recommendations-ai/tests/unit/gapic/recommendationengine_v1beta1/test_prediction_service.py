...
from google.cloud.recommendationengine_v1beta1.types import catalog, common, prediction_service
...
def test__get_api_endpoint():
    ...
    assert (
        PredictionServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        PredictionServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == PredictionServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_predict_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.predict), "__call__") as call:
        ...
        pager = client.predict(request={}, retry=retry, timeout=timeout)
        ...
        assert all(isinstance(i, prediction_service.PredictResponse.PredictionResult) for i in results)
...
async def test_predict_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.predict), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            prediction_service.PredictResponse(
                results=[
        ...
        assert all(isinstance(i, prediction_service.PredictResponse.PredictionResult) for i in responses)
...
def test_predict_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1beta1/{name=projects/*/locations/*/catalogs/*/eventStores/*/placements/*}:predict" % client.transport._host, args[1]
        )
...
def test_transport_kind_grpc():
    transport = PredictionServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = PredictionServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = PredictionServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = PredictionServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_predict_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.PredictionServiceRestInterceptor, "post_predict") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.PredictionServiceGrpcTransport, grpc_helpers), (transports.PredictionServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
