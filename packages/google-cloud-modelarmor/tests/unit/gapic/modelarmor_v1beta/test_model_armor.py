...
from google.cloud.modelarmor_v1beta.services.model_armor import ModelArmorAsyncClient, ModelArmorClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert ModelArmorClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert ModelArmorClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert ModelArmorClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert ModelArmorClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ModelArmorClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert ModelArmorClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ModelArmorClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert ModelArmorClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ModelArmorClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(ModelArmorClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ModelArmorClient))
@mock.patch.object(ModelArmorAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ModelArmorAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ModelArmorClient._DEFAULT_UNIVERSE
    default_endpoint = ModelArmorClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ModelArmorClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ModelArmorClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ModelArmorClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ModelArmorClient.DEFAULT_MTLS_ENDPOINT
    assert ModelArmorClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ModelArmorClient._get_api_endpoint(None, None, default_universe, "always") == ModelArmorClient.DEFAULT_MTLS_ENDPOINT
    assert ModelArmorClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ModelArmorClient.DEFAULT_MTLS_ENDPOINT
    assert ModelArmorClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ModelArmorClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ModelArmorClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ModelArmorClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ModelArmorClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ModelArmorClient._get_universe_domain(None, None) == ModelArmorClient._DEFAULT_UNIVERSE
...
def test_model_armor_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "modelarmor.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://modelarmor.googleapis.com"
        )
...
def test_model_armor_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ModelArmorAsyncClient, transports.ModelArmorGrpcAsyncIOTransport, "grpc_asyncio"),
        (ModelArmorClient, transports.ModelArmorRestTransport, "rest"),
    ],
)
@mock.patch.object(ModelArmorClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ModelArmorClient))
@mock.patch.object(ModelArmorAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ModelArmorAsyncClient))
def test_model_armor_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ModelArmorAsyncClient, transports.ModelArmorGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (ModelArmorClient, transports.ModelArmorGrpcTransport, "grpc", "false"),
        (ModelArmorAsyncClient, transports.ModelArmorGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_model_armor_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(ModelArmorClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ModelArmorClient))
@mock.patch.object(ModelArmorAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ModelArmorAsyncClient))
...
def test_model_armor_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ModelArmorAsyncClient, transports.ModelArmorGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (ModelArmorClient, transports.ModelArmorRestTransport, "rest", None),
    ],
)
def test_model_armor_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_model_armor_client_client_options_from_dict():
    with mock.patch("google.cloud.modelarmor_v1beta.services.model_armor.transports.ModelArmorGrpcTransport.__init__") as grpc_transport:
...
def test_model_armor_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
@pytest.mark.asyncio
async def test_list_templates_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_templates in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_templates] = mock_rpc
...
@pytest.mark.asyncio
async def test_list_templates_async(transport: str = "grpc_asyncio", request_type=service.ListTemplatesRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_list_templates_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_templates), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(service.ListTemplatesResponse())
        await client.list_templates(request)
...
async def test_list_templates_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_templates), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            service.ListTemplatesResponse(
                templates=[
...
async def test_list_templates_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_templates), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_templates(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
@pytest.mark.asyncio
async def test_get_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_template] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_template_async(transport: str = "grpc_asyncio", request_type=service.GetTemplateRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_create_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_template] = mock_rpc
...
@pytest.mark.asyncio
async def test_create_template_async(transport: str = "grpc_asyncio", request_type=service.CreateTemplateRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_update_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_template] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_template_async(transport: str = "grpc_asyncio", request_type=service.UpdateTemplateRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_delete_template_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_template in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_template] = mock_rpc
...
@pytest.mark.asyncio
async def test_delete_template_async(transport: str = "grpc_asyncio", request_type=service.DeleteTemplateRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_get_floor_setting(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_floor_setting), "__call__") as call:
        # Designate an appropriate return value for the call.
    ...
    assert response.integrated_services == [service.FloorSetting.IntegratedService.AI_PLATFORM]
...
def test_get_floor_setting_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_floor_setting] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_floor_setting_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_floor_setting in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_floor_setting] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_floor_setting_async(transport: str = "grpc_asyncio", request_type=service.GetFloorSettingRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_get_floor_setting_field_headers_async():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.get_floor_setting), "__call__") as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(service.FloorSetting())
        await client.get_floor_setting(request)
...
def test_update_floor_setting(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.update_floor_setting), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = service.FloorSetting(
            name="name_value",
            enable_floor_setting_enforcement=True,
...
def test_update_floor_setting_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_floor_setting in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_floor_setting] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_floor_setting_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_floor_setting in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_floor_setting] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_floor_setting_async(transport: str = "grpc_asyncio", request_type=service.UpdateFloorSettingRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_sanitize_user_prompt(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.sanitize_user_prompt), "__call__") as call:
        # Designate an appropriate return value for the call.
...
def test_sanitize_user_prompt_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.sanitize_user_prompt in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.sanitize_user_prompt] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_sanitize_user_prompt_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.sanitize_user_prompt in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.sanitize_user_prompt] = mock_rpc
...
@pytest.mark.asyncio
async def test_sanitize_user_prompt_async(transport: str = "grpc_asyncio", request_type=service.SanitizeUserPromptRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(type(client.transport.sanitize_user_prompt), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(service.SanitizeUserPromptResponse())
...
def test_sanitize_model_response(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.sanitize_model_response), "__call__") as call:
        # Designate an appropriate return value for the call.
...
def test_sanitize_model_response_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.sanitize_model_response in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.sanitize_model_response] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_sanitize_model_response_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelArmorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.sanitize_model_response in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.sanitize_model_response] = mock_rpc
...
@pytest.mark.asyncio
async def test_sanitize_model_response_async(transport: str = "grpc_asyncio", request_type=service.SanitizeModelResponseRequest):
    client = ModelArmorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(type(client.transport.sanitize_model_response), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(service.SanitizeModelResponseResponse())
...
def test_list_templates_rest_required_fields(request_type=service.ListTemplatesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_templates._get_unset_required_fields(jsonified_request)
...
def test_list_templates_rest_unset_required_fields():
    transport = transports.ModelArmorRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_templates_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v1beta/{parent=projects/*/locations/*}/templates" % client.transport._host, args[1])
...
def test_get_template_rest_required_fields(request_type=service.GetTemplateRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_template._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_template_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"name": "projects/sample1/locations/sample2/templates/sample3"}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{name=projects/*/locations/*/templates/*}" % client.transport._host, args[1])
...
def test_create_template_rest_required_fields(request_type=service.CreateTemplateRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_template._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_template_rest_required_fields(request_type=service.UpdateTemplateRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_template._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_template_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"template": {"name": "projects/sample1/locations/sample2/templates/sample3"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{template.name=projects/*/locations/*/templates/*}" % client.transport._host, args[1])
...
def test_delete_template_rest_required_fields(request_type=service.DeleteTemplateRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_template._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_floor_setting_rest_required_fields(request_type=service.GetFloorSettingRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_floor_setting._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_floor_setting_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{name=projects/*/locations/*/floorSetting}" % client.transport._host, args[1])
...
def test_update_floor_setting_rest_required_fields(request_type=service.UpdateFloorSettingRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_floor_setting._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_update_floor_setting_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {"floor_setting": {"name": "projects/sample1/locations/sample2/floorSetting"}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{floor_setting.name=projects/*/locations/*/floorSetting}" % client.transport._host, args[1])
...
def test_sanitize_user_prompt_rest_required_fields(request_type=service.SanitizeUserPromptRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).sanitize_user_prompt._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_sanitize_model_response_rest_required_fields(request_type=service.SanitizeModelResponseRequest):
    transport_class = transports.ModelArmorRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).sanitize_model_response._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = ModelArmorClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = ModelArmorClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = ModelArmorAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ModelArmorAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ModelArmorClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_templates_rest_bad_request(request_type=service.ListTemplatesRequest):
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_templates_rest_interceptors(null_interceptor):
    transport = transports.ModelArmorRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ModelArmorRestInterceptor(),
    )
    client = ModelArmorClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_list_templates") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = service.ListTemplatesResponse.to_json(service.ListTemplatesResponse())
...
def test_get_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_get_template") as post, mock.patch.object(
...
def test_create_template_rest_call_success(request_type):
    ...
    request_init["template"] = {
        ...
        "labels": {},
        "filter_config": {
            "rai_settings": {"rai_filters": [{"filter_type": 2, "confidence_level": 1}]},
            "sdp_settings": {
                "basic_config": {"filter_enforcement": 1},
                "advanced_config": {"inspect_template": "inspect_template_value", "deidentify_template": "deidentify_template_value"},
            },
            "pi_and_jailbreak_filter_settings": {"filter_enforcement": 1, "confidence_level": 1},
...
def test_create_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_create_template") as post, mock.patch.object(
...
def test_update_template_rest_bad_request(request_type=service.UpdateTemplateRequest):
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"template": {"name": "projects/sample1/locations/sample2/templates/sample3"}}
    request = request_type(**request_init)
...
def test_update_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_update_template") as post, mock.patch.object(
...
def test_delete_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "pre_delete_template") as pre:
        pre.assert_not_called()
...
def test_get_floor_setting_rest_bad_request(request_type=service.GetFloorSettingRequest):
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_floor_setting_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_get_floor_setting") as post, mock.patch.object(
...
def test_update_floor_setting_rest_bad_request(request_type=service.UpdateFloorSettingRequest):
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"floor_setting": {"name": "projects/sample1/locations/sample2/floorSetting"}}
    request = request_type(**request_init)
...
def test_update_floor_setting_rest_call_success(request_type):
    ...
    request_init["floor_setting"] = {
        ...
        "ai_platform_floor_setting": {"inspect_only": True, "inspect_and_block": True, "enable_cloud_logging": True},
        "floor_setting_metadata": {"multi_language_detection": {"enable_multi_language_detection": True}},
    }
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_update_floor_setting_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_update_floor_setting") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = service.UpdateFloorSettingRequest.pb(service.UpdateFloorSettingRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_sanitize_user_prompt_rest_bad_request(request_type=service.SanitizeUserPromptRequest):
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/templates/sample3"}
    request = request_type(**request_init)
...
def test_sanitize_user_prompt_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_sanitize_user_prompt") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = service.SanitizeUserPromptRequest.pb(service.SanitizeUserPromptRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = service.SanitizeUserPromptResponse.to_json(service.SanitizeUserPromptResponse())
        req.return_value.content = return_value
...
def test_sanitize_model_response_rest_bad_request(request_type=service.SanitizeModelResponseRequest):
    client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/templates/sample3"}
    request = request_type(**request_init)
...
def test_sanitize_model_response_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ModelArmorRestInterceptor, "post_sanitize_model_response") as post, mock.patch.object(
        transports.ModelArmorRestInterceptor, "post_sanitize_model_response_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = service.SanitizeModelResponseRequest.pb(service.SanitizeModelResponseRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = service.SanitizeModelResponseResponse.to_json(service.SanitizeModelResponseResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = service.SanitizeModelResponseResponse(), metadata
...
def test_model_armor_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.ModelArmorTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_model_armor_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.modelarmor_v1beta.services.model_armor.transports.ModelArmorTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.ModelArmorGrpcTransport, grpc_helpers), (transports.ModelArmorGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.ModelArmorGrpcTransport, transports.ModelArmorGrpcAsyncIOTransport])
...
def test_model_armor_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.ModelArmorRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_model_armor_host_no_port(transport_name):
    client = ModelArmorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="modelarmor.googleapis.com"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "modelarmor.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://modelarmor.googleapis.com"
...
def test_model_armor_host_with_port(transport_name):
    client = ModelArmorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="modelarmor.googleapis.com:8000"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "modelarmor.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://modelarmor.googleapis.com:8000"
    )
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.ModelArmorTransport, "_prep_wrapped_messages") as prep:
        client = ModelArmorClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = ModelArmorClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
