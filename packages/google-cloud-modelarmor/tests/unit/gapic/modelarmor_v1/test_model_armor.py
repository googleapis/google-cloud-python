...
def test_model_armor_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.modelarmor_v1.services.model_armor.transports.ModelArmorGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_get_floor_setting_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/{name=projects/*/locations/*/floorSetting}" % client.transport._host,
            args[1],
        )
...
def test_update_floor_setting_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/{floor_setting.name=projects/*/locations/*/floorSetting}"
            % client.transport._host,
            args[1],
        )
...
def test_model_armor_base_transport():
    ...
    with mock.patch(
        "google.cloud.modelarmor_v1.services.model_armor.transports.ModelArmorTransport.__init__"
    ) as Transport:
