...
from google.cloud.asset_v1p1beta1.services.asset_service import AssetServiceAsyncClient, AssetServiceClient, pagers, transports
from google.cloud.asset_v1p1beta1.types import asset_service, assets
...
def test_search_all_resources_rest_required_fields(request_type=asset_service.SearchAllResourcesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).search_all_resources._get_unset_required_fields(
        jsonified_request
    )
...
def test_search_all_iam_policies_rest_required_fields(request_type=asset_service.SearchAllIamPoliciesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).search_all_iam_policies._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_search_all_resources_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "post_search_all_resources") as post, mock.patch.object(
        transports.AssetServiceRestInterceptor, "post_search_all_resources_with_metadata"
...
def test_search_all_iam_policies_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "post_search_all_iam_policies") as post, mock.patch.object(
        transports.AssetServiceRestInterceptor, "post_search_all_iam_policies_with_metadata"
    ) as post_with_metadata, mock.patch.object(
