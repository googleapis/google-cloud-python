...
from google.cloud.asset_v1p2beta1.services.asset_service import AssetServiceAsyncClient, AssetServiceClient, transports
...
def test_asset_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "cloudasset.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudasset.googleapis.com"
        )
...
def test_asset_service_client_client_options_from_dict():
    with mock.patch("google.cloud.asset_v1p2beta1.services.asset_service.transports.AssetServiceGrpcTransport.__init__") as grpc_transport:
...
def test_create_feed_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v1p2beta1/{parent=*/*}/feeds" % client.transport._host, args[1])
...
def test_get_feed_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1p2beta1/{name=*/*/feeds/*}" % client.transport._host, args[1])
...
def test_update_feed_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1p2beta1/{feed.name=*/*/feeds/*}" % client.transport._host, args[1])
...
def test_transport_kind_grpc():
    transport = AssetServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = AssetServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = AssetServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = AssetServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_create_feed_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "post_create_feed") as post, mock.patch.object(
...
def test_get_feed_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "post_get_feed") as post, mock.patch.object(
...
def test_list_feeds_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "post_list_feeds") as post, mock.patch.object(
...
def test_update_feed_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "post_update_feed") as post, mock.patch.object(
...
def test_delete_feed_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AssetServiceRestInterceptor, "pre_delete_feed") as pre:
        pre.assert_not_called()
...
def test_asset_service_base_transport():
    ...
    with mock.patch("google.cloud.asset_v1p2beta1.services.asset_service.transports.AssetServiceTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.AssetServiceGrpcTransport, grpc_helpers), (transports.AssetServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_asset_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "cloudasset.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudasset.googleapis.com:8000"
    )
