...
def test__get_default_mtls_endpoint():
    ...
    assert (
        SystemPolicyV1Client._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        SystemPolicyV1Client._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        SystemPolicyV1Client._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                SystemPolicyV1Client._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                SystemPolicyV1Client._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(SystemPolicyV1Client),
)
@mock.patch.object(
    SystemPolicyV1AsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(SystemPolicyV1AsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = SystemPolicyV1Client._DEFAULT_UNIVERSE
    default_endpoint = SystemPolicyV1Client._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = SystemPolicyV1Client._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        SystemPolicyV1Client._get_api_endpoint(
    ...
    assert (
        SystemPolicyV1Client._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        SystemPolicyV1Client._get_api_endpoint(None, None, default_universe, "always")
        == SystemPolicyV1Client.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        SystemPolicyV1Client._get_api_endpoint(
    ...
    assert (
        SystemPolicyV1Client._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        SystemPolicyV1Client._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        SystemPolicyV1Client._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        SystemPolicyV1Client._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        SystemPolicyV1Client._get_universe_domain(None, None)
        == SystemPolicyV1Client._DEFAULT_UNIVERSE
    )
...
def test_system_policy_v1_client_from_service_account_info(
    client_class, transport_name
):
...
def test_system_policy_v1_client_from_service_account_file(
    client_class, transport_name
):
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(
...
def test_system_policy_v1_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_get_system_policy_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1/{name=locations/*/policy}" % client.transport._host, args[1]
        )
...
def test_transport_kind_grpc_asyncio():
    transport = SystemPolicyV1AsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = SystemPolicyV1Client.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_get_system_policy_rest_interceptors(null_interceptor):
    transport = transports.SystemPolicyV1RestTransport(
        ...
        interceptor=None
        if null_interceptor
        else transports.SystemPolicyV1RestInterceptor(),
    )
    client = SystemPolicyV1Client(transport=transport)
    ...
    with (
        ...
        mock.patch.object(
            transports.SystemPolicyV1RestInterceptor, "post_get_system_policy"
        ) as post,
        mock.patch.object(
            transports.SystemPolicyV1RestInterceptor,
