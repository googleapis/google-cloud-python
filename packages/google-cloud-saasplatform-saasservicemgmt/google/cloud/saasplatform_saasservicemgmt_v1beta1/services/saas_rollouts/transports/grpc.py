...
class SaasRolloutsGrpcTransport(SaasRolloutsTransport):
    """gRPC backend transport for SaasRollouts.
    ...
    def list_rollouts(
        ...
        if "list_rollouts" not in self._stubs:
            self._stubs["list_rollouts"] = self._logged_channel.unary_unary(
                ...
                request_serializer=rollouts_service.ListRolloutsRequest.serialize,
                response_deserializer=rollouts_service.ListRolloutsResponse.deserialize,
            )
    ...
    def get_rollout(
        ...
        if "get_rollout" not in self._stubs:
            self._stubs["get_rollout"] = self._logged_channel.unary_unary(
                ...
                request_serializer=rollouts_service.GetRolloutRequest.serialize,
                response_deserializer=rollouts_resources.Rollout.deserialize,
            )
    ...
    def create_rollout(
        ...
        if "create_rollout" not in self._stubs:
            self._stubs["create_rollout"] = self._logged_channel.unary_unary(
                ...
                request_serializer=rollouts_service.CreateRolloutRequest.serialize,
                response_deserializer=rollouts_resources.Rollout.deserialize,
            )
    ...
    def update_rollout(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "update_rollout" not in self._stubs:
            self._stubs["update_rollout"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/UpdateRollout",
                request_serializer=rollouts_service.UpdateRolloutRequest.serialize,
                response_deserializer=rollouts_resources.Rollout.deserialize,
            )
    ...
    def delete_rollout(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "delete_rollout" not in self._stubs:
            self._stubs["delete_rollout"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/DeleteRollout",
                request_serializer=rollouts_service.DeleteRolloutRequest.serialize,
    ...
    def list_rollout_kinds(
        ...
        Returns:
            Callable[[~.ListRolloutKindsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "list_rollout_kinds" not in self._stubs:
            self._stubs["list_rollout_kinds"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/ListRolloutKinds",
                request_serializer=rollouts_service.ListRolloutKindsRequest.serialize,
                response_deserializer=rollouts_service.ListRolloutKindsResponse.deserialize,
            )
    ...
    def get_rollout_kind(
        ...
        Returns:
            Callable[[~.GetRolloutKindRequest],
                    ~.RolloutKind]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_rollout_kind" not in self._stubs:
            self._stubs["get_rollout_kind"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/GetRolloutKind",
                request_serializer=rollouts_service.GetRolloutKindRequest.serialize,
                response_deserializer=rollouts_resources.RolloutKind.deserialize,
            )
    ...
    def create_rollout_kind(
        ...
        Returns:
            Callable[[~.CreateRolloutKindRequest],
                    ~.RolloutKind]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "create_rollout_kind" not in self._stubs:
            self._stubs["create_rollout_kind"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/CreateRolloutKind",
                request_serializer=rollouts_service.CreateRolloutKindRequest.serialize,
                response_deserializer=rollouts_resources.RolloutKind.deserialize,
            )
    ...
    def update_rollout_kind(
        ...
        Returns:
            Callable[[~.UpdateRolloutKindRequest],
                    ~.RolloutKind]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "update_rollout_kind" not in self._stubs:
            self._stubs["update_rollout_kind"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/UpdateRolloutKind",
                request_serializer=rollouts_service.UpdateRolloutKindRequest.serialize,
                response_deserializer=rollouts_resources.RolloutKind.deserialize,
            )
    ...
    def delete_rollout_kind(
        ...
        Returns:
            Callable[[~.DeleteRolloutKindRequest],
                    ~.Empty]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "delete_rollout_kind" not in self._stubs:
            self._stubs["delete_rollout_kind"] = self._logged_channel.unary_unary(
                "/google.cloud.saasplatform.saasservicemgmt.v1beta1.SaasRollouts/DeleteRolloutKind",
                request_serializer=rollouts_service.DeleteRolloutKindRequest.serialize,
                response_deserializer=empty_pb2.Empty.FromString,
            )
        return self._stubs["delete_rollout_kind"]
...
__all__ = ("SaasRolloutsGrpcTransport",)
