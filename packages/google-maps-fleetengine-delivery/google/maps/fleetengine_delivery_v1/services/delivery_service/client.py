...
class DeliveryServiceClient(metaclass=DeliveryServiceClientMeta):
    ...
    def parse_delivery_vehicle_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^providers/(?P<provider>.+?)/deliveryVehicles/(?P<vehicle>.+?)$", path)
    ...
    def parse_task_tracking_info_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^providers/(?P<provider>.+?)/taskTrackingInfo/(?P<tracking>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = DeliveryServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, DeliveryServiceTransport, Callable[..., DeliveryServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = DeliveryServiceClient._read_environment_variables()
        self._client_cert_source = DeliveryServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = DeliveryServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        self._api_endpoint = self._api_endpoint or DeliveryServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[DeliveryServiceTransport], Callable[..., DeliveryServiceTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `maps.fleetengine.delivery_v1.DeliveryServiceClient`.",
                    extra={
