...
from google.shopping.merchant_reports_v1alpha.services.report_service import ReportServiceAsyncClient, ReportServiceClient, pagers, transports
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert ReportServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert ReportServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                ReportServiceClient._use_client_cert_effective()
...
def test__get_api_endpoint():
    ...
    assert (
        ReportServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ReportServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_report_service_client_client_options_from_dict():
    with mock.patch(
        "google.shopping.merchant_reports_v1alpha.services.report_service.transports.ReportServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_search_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/reports/v1alpha/{parent=accounts/*}/reports:search" % client.transport._host, args[1])
...
def test_transport_kind_grpc():
    transport = ReportServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = ReportServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ReportServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ReportServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_search_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.ReportServiceRestInterceptor, "post_search") as post, mock.patch.object(
        transports.ReportServiceRestInterceptor, "post_search_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.ReportServiceRestInterceptor, "pre_search"
    ) as pre:
...
def test_report_service_base_transport():
    ...
    with mock.patch("google.shopping.merchant_reports_v1alpha.services.report_service.transports.ReportServiceTransport.__init__") as Transport:
...
def test_report_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.shopping.merchant_reports_v1alpha.services.report_service.transports.ReportServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
@pytest.mark.parametrize(
    ...
    [(transports.ReportServiceGrpcTransport, grpc_helpers), (transports.ReportServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
