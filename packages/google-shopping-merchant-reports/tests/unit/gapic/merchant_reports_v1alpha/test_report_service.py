...
from google.shopping.merchant_reports_v1alpha.services.report_service import (
...
@mock.patch.object(
    ReportServiceAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(ReportServiceAsyncClient),
)
...
@pytest.mark.parametrize(
    "client_class", [ReportServiceClient, ReportServiceAsyncClient]
)
@mock.patch.object(
    ReportServiceClient,
    "DEFAULT_ENDPOINT",
...
@mock.patch.object(
    ReportServiceAsyncClient,
    "DEFAULT_ENDPOINT",
    modify_default_endpoint(ReportServiceAsyncClient),
)
...
def test_report_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_search_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/reports/v1alpha/{parent=accounts/*}/reports:search"
...
def test_search_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.ReportServiceRestInterceptor, "post_search"
...
def test_report_service_base_transport():
    ...
    with mock.patch(
        "google.shopping.merchant_reports_v1alpha.services.report_service.transports.ReportServiceTransport.__init__"
    ) as Transport:
