...
from google.shopping.merchant_reports_v1beta.services.report_service import (
...
def test_report_service_client_client_options_from_dict():
    with mock.patch(
        "google.shopping.merchant_reports_v1beta.services.report_service.transports.ReportServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_search_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        req.return_value._content = reports.SearchResponse.to_json(
            reports.SearchResponse()
        )
...
def test_search_rest_bad_request(
    transport: str = "rest", request_type=reports.SearchRequest
):
    client = ReportServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_search_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/reports/v1beta/{parent=accounts/*}/reports:search"
...
def test_report_service_base_transport():
    ...
    with mock.patch(
        "google.shopping.merchant_reports_v1beta.services.report_service.transports.ReportServiceTransport.__init__"
    ) as Transport:
...
def test_report_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.shopping.merchant_reports_v1beta.services.report_service.transports.ReportServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
