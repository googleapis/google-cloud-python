...
def test__read_environment_variables():
    assert WebRiskServiceClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert WebRiskServiceClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert WebRiskServiceClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert WebRiskServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert WebRiskServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert WebRiskServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert WebRiskServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert WebRiskServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                WebRiskServiceClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert WebRiskServiceClient._get_client_cert_source(None, False) is None
    assert WebRiskServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert WebRiskServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert WebRiskServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert WebRiskServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(WebRiskServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebRiskServiceClient))
@mock.patch.object(WebRiskServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebRiskServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = WebRiskServiceClient._DEFAULT_UNIVERSE
    default_endpoint = WebRiskServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = WebRiskServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert WebRiskServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        WebRiskServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == WebRiskServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert WebRiskServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert WebRiskServiceClient._get_api_endpoint(None, None, default_universe, "always") == WebRiskServiceClient.DEFAULT_MTLS_ENDPOINT
    assert (
        WebRiskServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == WebRiskServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert WebRiskServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert WebRiskServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        WebRiskServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert WebRiskServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert WebRiskServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert WebRiskServiceClient._get_universe_domain(None, None) == WebRiskServiceClient._DEFAULT_UNIVERSE
...
def test_web_risk_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "webrisk.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://webrisk.googleapis.com"
        )
...
@mock.patch.object(WebRiskServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebRiskServiceClient))
@mock.patch.object(WebRiskServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WebRiskServiceAsyncClient))
def test_web_risk_service_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        (WebRiskServiceClient, transports.WebRiskServiceGrpcTransport, "grpc", grpc_helpers),
        (WebRiskServiceAsyncClient, transports.WebRiskServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_compute_threat_list_diff_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.compute_threat_list_diff in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.compute_threat_list_diff] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_compute_threat_list_diff_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.compute_threat_list_diff in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.compute_threat_list_diff] = mock_rpc
...
def test_search_uris_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.search_uris in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.search_uris] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_search_uris_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebRiskServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.search_uris in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.search_uris] = mock_rpc
...
def test_search_hashes_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.search_hashes in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.search_hashes] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_search_hashes_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebRiskServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.search_hashes in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.search_hashes] = mock_rpc
...
def test_create_submission_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WebRiskServiceClient(
        ...
        # Ensure method has been cached
        assert client._transport.create_submission in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_submission] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_submission_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebRiskServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_submission in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_submission] = mock_rpc
...
def test_submit_uri_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WebRiskServiceClient(
        ...
        # Ensure method has been cached
        assert client._transport.submit_uri in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.submit_uri] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_submit_uri_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WebRiskServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.submit_uri in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.submit_uri] = mock_rpc
...
def test_compute_threat_list_diff_rest_required_fields(request_type=webrisk.ComputeThreatListDiffRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).compute_threat_list_diff._get_unset_required_fields(
        jsonified_request
    )
...
def test_create_submission_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WebRiskServiceClient(
...
def test_submit_uri_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = WebRiskServiceClient(
...
def test_submit_uri_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=projects/*}/uris:submit" % client.transport._host, args[1])
...
def test_transport_kind_grpc():
    transport = WebRiskServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_submission_empty_call_grpc():
    client = WebRiskServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.create_submission), "__call__") as call:
        call.return_value = webrisk.Submission()
        client.create_submission(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = webrisk.CreateSubmissionRequest()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_submit_uri_empty_call_grpc():
    client = WebRiskServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.submit_uri), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.submit_uri(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = webrisk.SubmitUriRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = WebRiskServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = WebRiskServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_submission_empty_call_grpc_asyncio():
    client = WebRiskServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.create_submission), "__call__") as call:
        ...
        await client.create_submission(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_submit_uri_empty_call_grpc_asyncio():
    client = WebRiskServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.submit_uri), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.submit_uri(request=None)
...
def test_transport_kind_rest():
    transport = WebRiskServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_compute_threat_list_diff_rest_bad_request(request_type=webrisk.ComputeThreatListDiffRequest):
...
def test_compute_threat_list_diff_rest_interceptors(null_interceptor):
    transport = transports.WebRiskServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.WebRiskServiceRestInterceptor(),
    )
    client = WebRiskServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebRiskServiceRestInterceptor, "post_compute_threat_list_diff") as post, mock.patch.object(
        transports.WebRiskServiceRestInterceptor, "post_compute_threat_list_diff_with_metadata"
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = webrisk.ComputeThreatListDiffResponse.to_json(webrisk.ComputeThreatListDiffResponse())
        ...
        post_with_metadata.return_value = webrisk.ComputeThreatListDiffResponse(), metadata
...
def test_search_uris_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebRiskServiceRestInterceptor, "post_search_uris") as post, mock.patch.object(
...
def test_search_hashes_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebRiskServiceRestInterceptor, "post_search_hashes") as post, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = webrisk.SearchHashesResponse.to_json(webrisk.SearchHashesResponse())
        req.return_value.content = return_value
...
def test_create_submission_rest_bad_request(request_type=webrisk.CreateSubmissionRequest):
    client = WebRiskServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_create_submission_rest_call_success(request_type):
    client = WebRiskServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    for field, value in request_init["submission"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_create_submission_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WebRiskServiceRestInterceptor, "post_create_submission") as post, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = webrisk.Submission.to_json(webrisk.Submission())
        req.return_value.content = return_value
...
def test_submit_uri_rest_call_success(request_type):
    client = WebRiskServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_submission_empty_call_rest():
    client = WebRiskServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_submit_uri_empty_call_rest():
    client = WebRiskServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
@pytest.mark.parametrize(
    ...
    [(transports.WebRiskServiceGrpcTransport, grpc_helpers), (transports.WebRiskServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_web_risk_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "webrisk.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://webrisk.googleapis.com:8000"
    )
