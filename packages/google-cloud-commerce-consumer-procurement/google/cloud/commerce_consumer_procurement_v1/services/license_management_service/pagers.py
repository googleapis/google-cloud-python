...
class EnumerateLicensedUsersPager:
    ...
    :class:`google.cloud.commerce_consumer_procurement_v1.types.EnumerateLicensedUsersResponse` object, and
    provides an ``__iter__`` method to iterate through its
    ``licensed_users`` field.

    If there are more pages, the ``__iter__`` method will make additional
    ``EnumerateLicensedUsers`` requests and continue to iterate
    through the ``licensed_users`` field on the
    corresponding responses.

    All the usual :class:`google.cloud.commerce_consumer_procurement_v1.types.EnumerateLicensedUsersResponse`
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            request (google.cloud.commerce_consumer_procurement_v1.types.EnumerateLicensedUsersRequest):
                The initial request object.
            response (google.cloud.commerce_consumer_procurement_v1.types.EnumerateLicensedUsersResponse):
    ...
    def __iter__(self) -> Iterator[license_management_service.LicensedUser]:
        for page in self.pages:
            yield from page.licensed_users

    def __repr__(self) -> str:
        return "{0}<{1!r}>".format(self.__class__.__name__, self._response)


class EnumerateLicensedUsersAsyncPager:
    """A pager for iterating through ``enumerate_licensed_users`` requests.
    ...
    def __aiter__(self) -> AsyncIterator[license_management_service.LicensedUser]:
        async def async_generator():
            async for page in self.pages:
                for response in page.licensed_users:
                    yield response
