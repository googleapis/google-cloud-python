...
def test__get_default_mtls_endpoint():
    ...
    assert (
        LicenseManagementServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": ""}):
            with mock.patch.dict(os.environ, {"GOOGLE_API_CERTIFICATE_CONFIG": ""}):
                assert (
                    LicenseManagementServiceClient._use_client_cert_effective() is False
                )
...
async def test_enumerate_licensed_users_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.enumerate_licensed_users(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
