...
from google.shopping.merchant_accounts_v1.types import customerservice, phoneverificationstate
...
class BusinessInfo(proto.Message):
    r"""The ``BusinessInfo`` message contains essential information about a
    business.
    ...
    Attributes:
        ...
        address (google.type.postal_address_pb2.PostalAddress):
            ...
            ``address_lines``, ``postal_code``, ``administrative_area``
            and ``locality`` fields are supported. All other fields are
            ignored.
        ...
        phone_verification_state (google.shopping.merchant_accounts_v1.types.PhoneVerificationState):
            Output only. The phone verification state of
            the business.

            This field is a member of `oneof`_ ``_phone_verification_state``.
        customer_service (google.shopping.merchant_accounts_v1.types.CustomerService):
...
class UpdateBusinessInfoRequest(proto.Message):
    ...
    Attributes:
        business_info (google.shopping.merchant_accounts_v1.types.BusinessInfo):
