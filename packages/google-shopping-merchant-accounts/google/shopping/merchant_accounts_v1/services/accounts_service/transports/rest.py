    ...
    * Stripping extraneous information from responses
    ...
    .. code-block:: python
        class MyCustomAccountsServiceInterceptor(AccountsServiceRestInterceptor):
            ...
            def pre_create_test_account(self, request, metadata):
                logging.log(f"Received request: {request}")
                return request, metadata

            def post_create_test_account(self, response):
                logging.log(f"Received response: {response}")
                return response
    ...
    def pre_create_test_account(
        self,
        request: accounts.CreateTestAccountRequest,
        metadata: Sequence[Tuple[str, Union[str, bytes]]],
    ) -> Tuple[
        accounts.CreateTestAccountRequest, Sequence[Tuple[str, Union[str, bytes]]]
    ]:
        """Pre-rpc interceptor for create_test_account
    ...
    def post_create_test_account(self, response: accounts.Account) -> accounts.Account:
        """Post-rpc interceptor for create_test_account

        DEPRECATED. Please use the `post_create_test_account_with_metadata`
        interceptor instead.

        Override in a subclass to read or manipulate the response
        after it is returned by the AccountsService server but before
        it is returned to user code. This `post_create_test_account` interceptor runs
        before the `post_create_test_account_with_metadata` interceptor.
        """
        return response

    def post_create_test_account_with_metadata(
        self,
        response: accounts.Account,
        metadata: Sequence[Tuple[str, Union[str, bytes]]],
    ) -> Tuple[accounts.Account, Sequence[Tuple[str, Union[str, bytes]]]]:
        ...
        We recommend only using this `post_create_test_account_with_metadata`
        interceptor in new development instead of the `post_create_test_account` interceptor.
        When both interceptors are used, this `post_create_test_account_with_metadata` interceptor runs after the
        `post_create_test_account` interceptor. The (possibly modified) response returned by
        `post_create_test_account` will be passed to
        `post_create_test_account_with_metadata`.
        """
        return response, metadata
...
class AccountsServiceRestTransport(_BaseAccountsServiceRestTransport):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            interceptor (Optional[AccountsServiceRestInterceptor]): Interceptor used
    ...
    class _CreateTestAccount(
        _BaseAccountsServiceRestTransport._BaseCreateTestAccount,
        AccountsServiceRestStub,
    ):
        def __hash__(self):
            return hash("AccountsServiceRestTransport.CreateTestAccount")
        ...
        def __call__(
            ...
            *,
            ...
            r"""Call the create test account method over HTTP.
            ...
            http_options = _BaseAccountsServiceRestTransport._BaseCreateTestAccount._get_http_options()

            request, metadata = self._interceptor.pre_create_test_account(
                request, metadata
            )
            transcoded_request = _BaseAccountsServiceRestTransport._BaseCreateTestAccount._get_transcoded_request(
                http_options, request
            )

            body = _BaseAccountsServiceRestTransport._BaseCreateTestAccount._get_request_body_json(
                transcoded_request
            )

            # Jsonify the query params
            query_params = _BaseAccountsServiceRestTransport._BaseCreateTestAccount._get_query_params_json(
                transcoded_request
            )
            ...
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                ...
                _LOGGER.debug(
                    f"Sending request for google.shopping.merchant.accounts_v1.AccountsServiceClient.CreateTestAccount",
                    extra={
                        "serviceName": "google.shopping.merchant.accounts.v1.AccountsService",
                        "rpcName": "CreateTestAccount",
                        "httpRequest": http_request,
                        "metadata": http_request["headers"],
                    },
                )
            ...
            # Send the request
            response = AccountsServiceRestTransport._CreateTestAccount._get_response(
                self._host,
                metadata,
                query_params,
                self._session,
            ...
            resp = self._interceptor.post_create_test_account(resp)
            response_metadata = [(k, str(v)) for k, v in response.headers.items()]
            resp, _ = self._interceptor.post_create_test_account_with_metadata(
                resp, response_metadata
            )
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                logging.DEBUG
                ...
                _LOGGER.debug(
                    "Received response for google.shopping.merchant.accounts_v1.AccountsServiceClient.create_test_account",
                    extra={
                        "serviceName": "google.shopping.merchant.accounts.v1.AccountsService",
                        "rpcName": "CreateTestAccount",
                        "metadata": http_response["headers"],
    ...
    def create_test_account(
        ...
        # The return type is fine, but mypy isn't sophisticated enough to determine what's going on here.
        # In C++ this would require a dynamic_cast
        return self._CreateTestAccount(self._session, self._host, self._interceptor)  # type: ignore
