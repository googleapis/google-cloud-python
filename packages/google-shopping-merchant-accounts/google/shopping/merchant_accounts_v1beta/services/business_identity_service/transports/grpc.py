...
from .base import DEFAULT_CLIENT_INFO, BusinessIdentityServiceTransport
...
class BusinessIdentityServiceGrpcTransport(BusinessIdentityServiceTransport):
    ...
    def get_business_identity(
        ...
        r"""Return a callable for the get business identity method over gRPC.

        Retrieves the business identity of an account.

        Returns:
            Callable[[~.GetBusinessIdentityRequest],
                    ~.BusinessIdentity]:
        ...
        if "get_business_identity" not in self._stubs:
            self._stubs["get_business_identity"] = self._logged_channel.unary_unary(
                "/google.shopping.merchant.accounts.v1beta.BusinessIdentityService/GetBusinessIdentity",
                request_serializer=businessidentity.GetBusinessIdentityRequest.serialize,
                response_deserializer=businessidentity.BusinessIdentity.deserialize,
            )
    ...
    def update_business_identity(
        ...
        r"""Return a callable for the update business identity method over gRPC.

        Updates the business identity of an account.
        Executing this method requires admin access.

        Returns:
            Callable[[~.UpdateBusinessIdentityRequest],
                    ~.BusinessIdentity]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "update_business_identity" not in self._stubs:
            self._stubs["update_business_identity"] = self._logged_channel.unary_unary(
                "/google.shopping.merchant.accounts.v1beta.BusinessIdentityService/UpdateBusinessIdentity",
                request_serializer=businessidentity.UpdateBusinessIdentityRequest.serialize,
                response_deserializer=businessidentity.BusinessIdentity.deserialize,
            )
...
__all__ = ("BusinessIdentityServiceGrpcTransport",)
