...
from google.shopping.merchant_accounts_v1.services.terms_of_service_service import (
...
def test_terms_of_service_service_client_client_options_from_dict():
    with mock.patch(
        "google.shopping.merchant_accounts_v1.services.terms_of_service_service.transports.TermsOfServiceServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_get_terms_of_service_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/accounts/v1/{name=termsOfService/*}" % client.transport._host, args[1]
        )
...
def test_accept_terms_of_service_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/accounts/v1/{name=termsOfService/*}:accept" % client.transport._host,
            args[1],
        )
...
def test_accept_terms_of_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.TermsOfServiceServiceRestInterceptor, "post_accept_terms_of_service"
    ) as post, mock.patch.object(
        transports.TermsOfServiceServiceRestInterceptor,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = termsofservice.AcceptTermsOfServiceResponse.to_json(
            termsofservice.AcceptTermsOfServiceResponse()
        )
        req.return_value.content = return_value
...
def test_terms_of_service_service_base_transport():
    ...
    with mock.patch(
        "google.shopping.merchant_accounts_v1.services.terms_of_service_service.transports.TermsOfServiceServiceTransport.__init__"
    ) as Transport:
...
def test_terms_of_service_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.shopping.merchant_accounts_v1.services.terms_of_service_service.transports.TermsOfServiceServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
def test_terms_of_service_agreement_state_path():
    ...
    actual = TermsOfServiceServiceClient.terms_of_service_agreement_state_path(
        account, identifier
    )
    assert expected == actual
...
def test_parse_terms_of_service_agreement_state_path():
    ...
    # Check that the path construction is reversible.
    actual = TermsOfServiceServiceClient.parse_terms_of_service_agreement_state_path(
        path
    )
    assert expected == actual
