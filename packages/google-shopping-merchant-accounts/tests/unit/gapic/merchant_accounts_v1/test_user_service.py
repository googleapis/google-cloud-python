...
from google.shopping.merchant_accounts_v1.services.user_service import (
    UserServiceAsyncClient,
    UserServiceClient,
    pagers,
    transports,
)
from google.shopping.merchant_accounts_v1.types import accessright, user
...
def test_user_service_client_client_options_from_dict():
    with mock.patch(
        "google.shopping.merchant_accounts_v1.services.user_service.transports.UserServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
@pytest.mark.asyncio
async def test_verify_self_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.verify_self
            in client._client._transport._wrapped_methods
        )
...
@pytest.mark.asyncio
async def test_verify_self_async(
    transport: str = "grpc_asyncio", request_type=user.VerifySelfRequest
):
    client = UserServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
...
def test_get_user_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/accounts/v1/{name=accounts/*/users/*}" % client.transport._host, args[1]
        )
...
def test_create_user_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/accounts/v1/{parent=accounts/*}/users" % client.transport._host, args[1]
        )
...
def test_update_user_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/accounts/v1/{user.name=accounts/*/users/*}" % client.transport._host,
            args[1],
        )
...
def test_verify_self_rest_required_fields(request_type=user.VerifySelfRequest):
    ...
    unset_fields = transport_class(
        credentials=ga_credentials.AnonymousCredentials()
    ).verify_self._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_verify_self_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/accounts/v1/{account=accounts/*}/users/me:verifySelf"
...
def test_verify_self_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.UserServiceRestInterceptor, "post_verify_self"
    ) as post, mock.patch.object(
...
def test_user_service_base_transport():
    ...
    with mock.patch(
        "google.shopping.merchant_accounts_v1.services.user_service.transports.UserServiceTransport.__init__"
    ) as Transport:
