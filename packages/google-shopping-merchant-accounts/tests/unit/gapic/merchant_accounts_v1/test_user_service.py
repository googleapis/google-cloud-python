...
async def test_verify_self_async(request_type, transport: str = "grpc_asyncio"):
    client = UserServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_create_user_rest_required_fields(request_type=gsma_user.CreateUserRequest):
    ...
    default_values = getattr(
        transport_class._BaseCreateUser,
    ...
    assert not set(unset_fields) - set(("userId",))
...
def test_update_user_rest_required_fields(request_type=gsma_user.UpdateUserRequest):
    ...
    default_values = getattr(
        transport_class._BaseUpdateUser,
...
def test_list_users_rest_required_fields(request_type=user.ListUsersRequest):
    ...
    default_values = getattr(
        transport_class._BaseListUsers,
...
def test_verify_self_rest_required_fields(request_type=user.VerifySelfRequest):
    ...
    default_values = getattr(
        transport_class._BaseVerifySelf,
        "_BaseVerifySelf__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
