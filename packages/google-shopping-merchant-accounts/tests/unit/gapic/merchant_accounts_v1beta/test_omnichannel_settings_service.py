...
def test_update_omnichannel_setting_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/accounts/v1beta/{omnichannel_setting.name=accounts/*/omnichannelSettings/*}" % client.transport._host, args[1]
        )
...
def test_request_inventory_verification_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/accounts/v1beta/{name=accounts/*/omnichannelSettings/*}:requestInventoryVerification" % client.transport._host, args[1]
        )
