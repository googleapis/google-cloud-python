...
def test__read_environment_variables():
    assert AppConnectorsServiceClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert AppConnectorsServiceClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                AppConnectorsServiceClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert AppConnectorsServiceClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert AppConnectorsServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert AppConnectorsServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert AppConnectorsServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert AppConnectorsServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert AppConnectorsServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                AppConnectorsServiceClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert AppConnectorsServiceClient._get_client_cert_source(None, False) is None
    assert AppConnectorsServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert AppConnectorsServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert AppConnectorsServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert AppConnectorsServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(AppConnectorsServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceClient))
@mock.patch.object(AppConnectorsServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = AppConnectorsServiceClient._DEFAULT_UNIVERSE
    default_endpoint = AppConnectorsServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = AppConnectorsServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert AppConnectorsServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        AppConnectorsServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
        == AppConnectorsServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert AppConnectorsServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert AppConnectorsServiceClient._get_api_endpoint(None, None, default_universe, "always") == AppConnectorsServiceClient.DEFAULT_MTLS_ENDPOINT
    assert (
        AppConnectorsServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == AppConnectorsServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert AppConnectorsServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert AppConnectorsServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        AppConnectorsServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert AppConnectorsServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert AppConnectorsServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert AppConnectorsServiceClient._get_universe_domain(None, None) == AppConnectorsServiceClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize(
    ...
    [
        (AppConnectorsServiceClient, "grpc"),
        (AppConnectorsServiceAsyncClient, "grpc_asyncio"),
        (AppConnectorsServiceClient, "rest"),
    ],
)
def test_app_connectors_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "beyondcorp.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://beyondcorp.googleapis.com"
        )
...
@pytest.mark.parametrize(
    ...
    [
        (transports.AppConnectorsServiceGrpcTransport, "grpc"),
        (transports.AppConnectorsServiceGrpcAsyncIOTransport, "grpc_asyncio"),
        (transports.AppConnectorsServiceRestTransport, "rest"),
    ],
)
...
def test_app_connectors_service_client_get_transport_class():
    ...
    available_transports = [
        ...
        transports.AppConnectorsServiceRestTransport,
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (AppConnectorsServiceClient, transports.AppConnectorsServiceGrpcTransport, "grpc"),
        (AppConnectorsServiceAsyncClient, transports.AppConnectorsServiceGrpcAsyncIOTransport, "grpc_asyncio"),
        (AppConnectorsServiceClient, transports.AppConnectorsServiceRestTransport, "rest"),
    ],
)
@mock.patch.object(AppConnectorsServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceClient))
@mock.patch.object(AppConnectorsServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceAsyncClient))
...
@pytest.mark.parametrize(
    ...
    [
        (AppConnectorsServiceClient, transports.AppConnectorsServiceGrpcTransport, "grpc", "true"),
        (AppConnectorsServiceAsyncClient, transports.AppConnectorsServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (AppConnectorsServiceClient, transports.AppConnectorsServiceGrpcTransport, "grpc", "false"),
        (AppConnectorsServiceAsyncClient, transports.AppConnectorsServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
        (AppConnectorsServiceClient, transports.AppConnectorsServiceRestTransport, "rest", "true"),
        (AppConnectorsServiceClient, transports.AppConnectorsServiceRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(AppConnectorsServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceClient))
@mock.patch.object(AppConnectorsServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceAsyncClient))
...
@mock.patch.object(AppConnectorsServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceClient))
@mock.patch.object(AppConnectorsServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AppConnectorsServiceAsyncClient))
def test_app_connectors_service_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        (AppConnectorsServiceClient, transports.AppConnectorsServiceGrpcTransport, "grpc", grpc_helpers),
        (AppConnectorsServiceAsyncClient, transports.AppConnectorsServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (AppConnectorsServiceClient, transports.AppConnectorsServiceRestTransport, "rest", None),
    ],
)
...
def test_app_connectors_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.beyondcorp_appconnectors_v1.services.app_connectors_service.transports.AppConnectorsServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_list_app_connectors_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.list_app_connectors in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_app_connectors] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_app_connectors_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_app_connectors in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_app_connectors] = mock_rpc
...
async def test_list_app_connectors_flattened_async():
    ...
    with mock.patch.object(type(client.transport.list_app_connectors), "__call__") as call:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.list_app_connectors(
            parent="parent_value",
        )
...
async def test_list_app_connectors_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.list_app_connectors(
            app_connectors_service.ListAppConnectorsRequest(),
            parent="parent_value",
        )
...
def test_list_app_connectors_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_app_connectors), "__call__") as call:
        ...
        pager = client.list_app_connectors(request={}, retry=retry, timeout=timeout)
...
async def test_list_app_connectors_async_pager():
    ...
    with mock.patch.object(type(client.transport.list_app_connectors), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async_pager = await client.list_app_connectors(
            request={},
        )
...
async def test_list_app_connectors_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_app_connectors), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_app_connectors(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_app_connector(request_type, transport: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.get_app_connector), "__call__") as call:
        ...
        call.return_value = app_connectors_service.AppConnector(
            ...
            uid="uid_value",
            state=app_connectors_service.AppConnector.State.CREATING,
        )
...
def test_get_app_connector_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_get_app_connector_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_app_connector in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_app_connector] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_app_connector_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_app_connector in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_app_connector] = mock_rpc
...
async def test_get_app_connector_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.get_app_connector(
            app_connectors_service.GetAppConnectorRequest(),
            name="name_value",
        )
...
def test_create_app_connector_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_create_app_connector_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_app_connector in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_app_connector] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_app_connector_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_app_connector in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_app_connector] = mock_rpc
...
def test_create_app_connector_flattened():
    ...
    with mock.patch.object(type(client.transport.create_app_connector), "__call__") as call:
        ...
        client.create_app_connector(
            parent="parent_value",
            app_connector=app_connectors_service.AppConnector(name="name_value"),
            app_connector_id="app_connector_id_value",
        )
...
async def test_create_app_connector_flattened_async():
    ...
    with mock.patch.object(type(client.transport.create_app_connector), "__call__") as call:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.create_app_connector(
            parent="parent_value",
            app_connector=app_connectors_service.AppConnector(name="name_value"),
            app_connector_id="app_connector_id_value",
        )
...
async def test_create_app_connector_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.create_app_connector(
            app_connectors_service.CreateAppConnectorRequest(),
            parent="parent_value",
            app_connector=app_connectors_service.AppConnector(name="name_value"),
            app_connector_id="app_connector_id_value",
...
def test_update_app_connector_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_update_app_connector_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_app_connector in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_app_connector] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_app_connector_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_app_connector in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_app_connector] = mock_rpc
...
async def test_update_app_connector_flattened_async():
    ...
    with mock.patch.object(type(client.transport.update_app_connector), "__call__") as call:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.update_app_connector(
            app_connector=app_connectors_service.AppConnector(name="name_value"),
            update_mask=field_mask_pb2.FieldMask(paths=["paths_value"]),
        )
...
async def test_update_app_connector_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.update_app_connector(
            app_connectors_service.UpdateAppConnectorRequest(),
            app_connector=app_connectors_service.AppConnector(name="name_value"),
            update_mask=field_mask_pb2.FieldMask(paths=["paths_value"]),
        )
...
def test_delete_app_connector_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_delete_app_connector_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_app_connector in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_app_connector] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_app_connector_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_app_connector in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_app_connector] = mock_rpc
...
async def test_delete_app_connector_flattened_async():
    ...
    with mock.patch.object(type(client.transport.delete_app_connector), "__call__") as call:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.delete_app_connector(
            name="name_value",
        )
...
async def test_delete_app_connector_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.delete_app_connector(
            app_connectors_service.DeleteAppConnectorRequest(),
            name="name_value",
        )
...
def test_report_status_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_report_status_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.report_status in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.report_status] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_report_status_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.report_status in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.report_status] = mock_rpc
...
def test_report_status_flattened():
    ...
    with mock.patch.object(type(client.transport.report_status), "__call__") as call:
        ...
        client.report_status(
            app_connector="app_connector_value",
            resource_info=gcba_resource_info.ResourceInfo(id="id_value"),
        )
...
async def test_report_status_flattened_async():
    ...
    with mock.patch.object(type(client.transport.report_status), "__call__") as call:
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.report_status(
            app_connector="app_connector_value",
            resource_info=gcba_resource_info.ResourceInfo(id="id_value"),
        )
...
def test_list_app_connectors_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceClient(
...
def test_list_app_connectors_rest_required_fields(request_type=app_connectors_service.ListAppConnectorsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_app_connectors._get_unset_required_fields(
        jsonified_request
    )
...
def test_get_app_connector_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceClient(
...
def test_create_app_connector_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceClient(
...
def test_create_app_connector_rest_required_fields(request_type=app_connectors_service.CreateAppConnectorRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_app_connector._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_update_app_connector_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceClient(
...
def test_update_app_connector_rest_required_fields(request_type=app_connectors_service.UpdateAppConnectorRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_app_connector._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_delete_app_connector_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceClient(
...
def test_report_status_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = AppConnectorsServiceClient(
...
def test_credentials_transport_error():
    ...
    transport = transports.AppConnectorsServiceGrpcTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    with pytest.raises(ValueError):
        client = AppConnectorsServiceClient(
...
def test_transport_get_channel():
    ...
    transport = transports.AppConnectorsServiceGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
@pytest.mark.parametrize(
    ...
    [
        transports.AppConnectorsServiceGrpcTransport,
        transports.AppConnectorsServiceGrpcAsyncIOTransport,
        transports.AppConnectorsServiceRestTransport,
    ],
)
...
def test_transport_kind_grpc():
    transport = AppConnectorsServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_list_app_connectors_empty_call_grpc():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.list_app_connectors), "__call__") as call:
        call.return_value = app_connectors_service.ListAppConnectorsResponse()
        client.list_app_connectors(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_app_connector_empty_call_grpc():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.get_app_connector), "__call__") as call:
        call.return_value = app_connectors_service.AppConnector()
        client.get_app_connector(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_app_connector_empty_call_grpc():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.create_app_connector), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.create_app_connector(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_app_connector_empty_call_grpc():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.update_app_connector), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.update_app_connector(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_app_connector_empty_call_grpc():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.delete_app_connector), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.delete_app_connector(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_report_status_empty_call_grpc():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.report_status), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.report_status(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = AppConnectorsServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_app_connectors_empty_call_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    ...
    with mock.patch.object(type(client.transport.list_app_connectors), "__call__") as call:
        ...
        await client.list_app_connectors(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_app_connector_empty_call_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.get_app_connector), "__call__") as call:
        ...
        await client.get_app_connector(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_app_connector_empty_call_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.create_app_connector), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.create_app_connector(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_update_app_connector_empty_call_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.update_app_connector), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.update_app_connector(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_delete_app_connector_empty_call_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.delete_app_connector), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.delete_app_connector(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_report_status_empty_call_grpc_asyncio():
    client = AppConnectorsServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.report_status), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.report_status(request=None)
...
def test_transport_kind_rest():
    transport = AppConnectorsServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_app_connectors_rest_bad_request(request_type=app_connectors_service.ListAppConnectorsRequest):
...
def test_list_app_connectors_rest_call_success(request_type):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_app_connectors_rest_interceptors(null_interceptor):
    transport = transports.AppConnectorsServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.AppConnectorsServiceRestInterceptor(),
    )
    client = AppConnectorsServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AppConnectorsServiceRestInterceptor, "post_list_app_connectors") as post, mock.patch.object(
        transports.AppConnectorsServiceRestInterceptor, "post_list_app_connectors_with_metadata"
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = app_connectors_service.ListAppConnectorsResponse.to_json(app_connectors_service.ListAppConnectorsResponse())
        ...
        post_with_metadata.return_value = app_connectors_service.ListAppConnectorsResponse(), metadata
...
def test_get_app_connector_rest_bad_request(request_type=app_connectors_service.GetAppConnectorRequest):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_app_connector_rest_call_success(request_type):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_app_connector_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AppConnectorsServiceRestInterceptor, "post_get_app_connector") as post, mock.patch.object(
        transports.AppConnectorsServiceRestInterceptor, "post_get_app_connector_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = app_connectors_service.AppConnector.to_json(app_connectors_service.AppConnector())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = app_connectors_service.AppConnector(), metadata
...
def test_create_app_connector_rest_bad_request(request_type=app_connectors_service.CreateAppConnectorRequest):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_create_app_connector_rest_call_success(request_type):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    request_init["app_connector"] = {
        ...
        "uid": "uid_value",
        "state": 1,
        "principal_info": {"service_account": {"email": "email_value"}},
        "resource_info": {
            "id": "id_value",
            "status": 1,
            "resource": {"type_url": "type.googleapis.com/google.protobuf.Duration", "value": b"\x08\x0c\x10\xdb\x07"},
            "time": {},
            "sub": {},
        },
    }
    ...
    for field, value in request_init["app_connector"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_create_app_connector_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.AppConnectorsServiceRestInterceptor, "post_create_app_connector_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_update_app_connector_rest_bad_request(request_type=app_connectors_service.UpdateAppConnectorRequest):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_update_app_connector_rest_call_success(request_type):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")

    # send a request that will satisfy transcoding
    request_init = {"app_connector": {"name": "projects/sample1/locations/sample2/appConnectors/sample3"}}
    request_init["app_connector"] = {
        "name": "projects/sample1/locations/sample2/appConnectors/sample3",
        "create_time": {"seconds": 751, "nanos": 543},
        "update_time": {},
        "labels": {},
        "display_name": "display_name_value",
...
def test_update_app_connector_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.AppConnectorsServiceRestInterceptor, "post_update_app_connector_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_delete_app_connector_rest_bad_request(request_type=app_connectors_service.DeleteAppConnectorRequest):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/appConnectors/sample3"}
    request = request_type(**request_init)
...
def test_delete_app_connector_rest_call_success(request_type):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_delete_app_connector_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.AppConnectorsServiceRestInterceptor, "post_delete_app_connector_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_report_status_rest_bad_request(request_type=app_connectors_service.ReportStatusRequest):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_report_status_rest_call_success(request_type):
    client = AppConnectorsServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_report_status_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.AppConnectorsServiceRestInterceptor, "post_report_status_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_app_connectors_empty_call_rest():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_app_connector_empty_call_rest():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_app_connector_empty_call_rest():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_app_connector_empty_call_rest():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_app_connector_empty_call_rest():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_report_status_empty_call_rest():
    client = AppConnectorsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_app_connectors_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.beyondcorp_appconnectors_v1.services.app_connectors_service.transports.AppConnectorsServiceTransport.__init__"
    ) as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.AppConnectorsServiceGrpcTransport, grpc_helpers), (transports.AppConnectorsServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_app_connectors_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "beyondcorp.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://beyondcorp.googleapis.com:8000"
    )
...
def test_app_connectors_service_client_transport_session_collision(transport_name):
    ...
    client1 = AppConnectorsServiceClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = AppConnectorsServiceClient(
        credentials=creds2,
        transport=transport_name,
    )
...
def test_app_connector_path():
    ...
    location = "clam"
    app_connector = "whelk"
    expected = "projects/{project}/locations/{location}/appConnectors/{app_connector}".format(
        project=project,
        location=location,
...
async def test_get_location_field_headers_async():
    client = AppConnectorsServiceAsyncClient(credentials=async_anonymous_credentials())
...
@pytest.mark.parametrize(
    ...
    [
        (AppConnectorsServiceClient, transports.AppConnectorsServiceGrpcTransport),
        (AppConnectorsServiceAsyncClient, transports.AppConnectorsServiceGrpcAsyncIOTransport),
    ],
)
