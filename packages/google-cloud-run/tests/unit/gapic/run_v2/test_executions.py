...
from google.cloud.run_v2.services.executions import ExecutionsAsyncClient, ExecutionsClient, pagers, transports
...
def test_transport_kind_rest():
    transport = ExecutionsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_get_execution_rest_bad_request(request_type=execution.GetExecutionRequest):
    client = ExecutionsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_execution_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.ExecutionsRestInterceptor, "post_get_execution") as post, mock.patch.object(
...
def test_list_executions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ExecutionsRestInterceptor, "post_list_executions") as post, mock.patch.object(
...
def test_executions_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.ExecutionsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
