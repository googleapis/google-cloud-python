...
from google.cloud.run_v2.services.worker_pools import WorkerPoolsAsyncClient, WorkerPoolsClient, pagers, transports
from google.cloud.run_v2.types import condition, instance_split, k8s_min, vendor_settings
...
def test__get_default_mtls_endpoint():
    ...
    assert WorkerPoolsClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert WorkerPoolsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert WorkerPoolsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert WorkerPoolsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert WorkerPoolsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert WorkerPoolsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert WorkerPoolsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert WorkerPoolsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                WorkerPoolsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert WorkerPoolsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert WorkerPoolsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert WorkerPoolsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert WorkerPoolsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(WorkerPoolsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkerPoolsClient))
@mock.patch.object(WorkerPoolsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkerPoolsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = WorkerPoolsClient._DEFAULT_UNIVERSE
    default_endpoint = WorkerPoolsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = WorkerPoolsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert WorkerPoolsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert WorkerPoolsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == WorkerPoolsClient.DEFAULT_MTLS_ENDPOINT
    assert WorkerPoolsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert WorkerPoolsClient._get_api_endpoint(None, None, default_universe, "always") == WorkerPoolsClient.DEFAULT_MTLS_ENDPOINT
    assert WorkerPoolsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == WorkerPoolsClient.DEFAULT_MTLS_ENDPOINT
    assert WorkerPoolsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert WorkerPoolsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        WorkerPoolsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert WorkerPoolsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert WorkerPoolsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert WorkerPoolsClient._get_universe_domain(None, None) == WorkerPoolsClient._DEFAULT_UNIVERSE
...
def test_worker_pools_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WorkerPoolsAsyncClient, transports.WorkerPoolsGrpcAsyncIOTransport, "grpc_asyncio"),
        (WorkerPoolsClient, transports.WorkerPoolsRestTransport, "rest"),
    ],
)
@mock.patch.object(WorkerPoolsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkerPoolsClient))
@mock.patch.object(WorkerPoolsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(WorkerPoolsAsyncClient))
def test_worker_pools_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WorkerPoolsAsyncClient, transports.WorkerPoolsGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (WorkerPoolsClient, transports.WorkerPoolsGrpcTransport, "grpc", "false"),
        (WorkerPoolsAsyncClient, transports.WorkerPoolsGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_worker_pools_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@mock.patch.object(WorkerPoolsClient, "DEFAULT_ENDPOINT", modify_default_endpoint(WorkerPoolsClient))
@mock.patch.object(WorkerPoolsAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(WorkerPoolsAsyncClient))
...
def test_worker_pools_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (WorkerPoolsAsyncClient, transports.WorkerPoolsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (WorkerPoolsClient, transports.WorkerPoolsRestTransport, "rest", None),
    ],
)
def test_worker_pools_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_worker_pools_client_client_options_from_dict():
    with mock.patch("google.cloud.run_v2.services.worker_pools.transports.WorkerPoolsGrpcTransport.__init__") as grpc_transport:
...
def test_worker_pools_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_create_worker_pool_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.create_worker_pool in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_worker_pool] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_worker_pool_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.create_worker_pool in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_worker_pool] = mock_rpc
...
@pytest.mark.asyncio
async def test_create_worker_pool_async(transport: str = "grpc_asyncio", request_type=gcr_worker_pool.CreateWorkerPoolRequest):
    client = WorkerPoolsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_get_worker_pool_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkerPoolsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_worker_pool in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_worker_pool] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_worker_pool_async(transport: str = "grpc_asyncio", request_type=worker_pool.GetWorkerPoolRequest):
    client = WorkerPoolsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_get_worker_pool_flattened_async():
    ...
    with mock.patch.object(type(client.transport.get_worker_pool), "__call__") as call:
        ...
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(worker_pool.WorkerPool())
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
...
def test_list_worker_pools_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_worker_pools] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_worker_pools_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkerPoolsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_worker_pools in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_worker_pools] = mock_rpc
...
@pytest.mark.asyncio
async def test_list_worker_pools_async(transport: str = "grpc_asyncio", request_type=worker_pool.ListWorkerPoolsRequest):
    client = WorkerPoolsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_list_worker_pools_flattened_async():
    ...
    with mock.patch.object(type(client.transport.list_worker_pools), "__call__") as call:
        ...
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(worker_pool.ListWorkerPoolsResponse())
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
...
async def test_list_worker_pools_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_worker_pools), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            worker_pool.ListWorkerPoolsResponse(
                worker_pools=[
...
def test_update_worker_pool_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_worker_pool in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_worker_pool] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_worker_pool_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkerPoolsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_worker_pool in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_worker_pool] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_worker_pool_async(transport: str = "grpc_asyncio", request_type=gcr_worker_pool.UpdateWorkerPoolRequest):
    client = WorkerPoolsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_delete_worker_pool_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_worker_pool in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_worker_pool] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_worker_pool_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = WorkerPoolsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_worker_pool in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_worker_pool] = mock_rpc
...
@pytest.mark.asyncio
async def test_delete_worker_pool_async(transport: str = "grpc_asyncio", request_type=worker_pool.DeleteWorkerPoolRequest):
    client = WorkerPoolsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_create_worker_pool_rest_required_fields(request_type=gcr_worker_pool.CreateWorkerPoolRequest):
...
def test_create_worker_pool_rest_unset_required_fields():
    transport = transports.WorkerPoolsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_create_worker_pool_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/v2/{parent=projects/*/locations/*}/workerPools" % client.transport._host, args[1])
...
def test_get_worker_pool_rest_required_fields(request_type=worker_pool.GetWorkerPoolRequest):
    transport_class = transports.WorkerPoolsRestTransport
...
def test_get_worker_pool_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2/{name=projects/*/locations/*/workerPools/*}" % client.transport._host, args[1])
...
def test_list_worker_pools_rest_required_fields(request_type=worker_pool.ListWorkerPoolsRequest):
    transport_class = transports.WorkerPoolsRestTransport
...
def test_list_worker_pools_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(worker_pool.ListWorkerPoolsResponse.to_json(x) for x in response)
...
def test_update_worker_pool_rest_required_fields(request_type=gcr_worker_pool.UpdateWorkerPoolRequest):
    transport_class = transports.WorkerPoolsRestTransport
...
def test_update_worker_pool_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2/{worker_pool.name=projects/*/locations/*/workerPools/*}" % client.transport._host, args[1])
...
def test_delete_worker_pool_rest_required_fields(request_type=worker_pool.DeleteWorkerPoolRequest):
    transport_class = transports.WorkerPoolsRestTransport
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = WorkerPoolsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = WorkerPoolsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_create_worker_pool_routing_parameters_request_1_grpc():
    ...
    with mock.patch.object(type(client.transport.create_worker_pool), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.create_worker_pool(request={"parent": "projects/sample1/locations/sample2"})

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, kw = call.mock_calls[0]
        request_msg = gcr_worker_pool.CreateWorkerPoolRequest(**{"parent": "projects/sample1/locations/sample2"})
...
def test_get_worker_pool_routing_parameters_request_1_grpc():
    ...
    with mock.patch.object(type(client.transport.get_worker_pool), "__call__") as call:
        call.return_value = worker_pool.WorkerPool()
        client.get_worker_pool(request={"name": "projects/sample1/locations/sample2/sample3"})

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, kw = call.mock_calls[0]
        request_msg = worker_pool.GetWorkerPoolRequest(**{"name": "projects/sample1/locations/sample2/sample3"})
...
def test_list_worker_pools_routing_parameters_request_1_grpc():
    ...
    with mock.patch.object(type(client.transport.list_worker_pools), "__call__") as call:
        call.return_value = worker_pool.ListWorkerPoolsResponse()
        client.list_worker_pools(request={"parent": "projects/sample1/locations/sample2"})
...
def test_update_worker_pool_routing_parameters_request_1_grpc():
    ...
    with mock.patch.object(type(client.transport.update_worker_pool), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.update_worker_pool(request={"worker_pool": {"name": "projects/sample1/locations/sample2/sample3"}})

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, kw = call.mock_calls[0]
        request_msg = gcr_worker_pool.UpdateWorkerPoolRequest(**{"worker_pool": {"name": "projects/sample1/locations/sample2/sample3"}})
...
def test_delete_worker_pool_routing_parameters_request_1_grpc():
    ...
    with mock.patch.object(type(client.transport.delete_worker_pool), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.delete_worker_pool(request={"name": "projects/sample1/locations/sample2/sample3"})

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, kw = call.mock_calls[0]
        request_msg = worker_pool.DeleteWorkerPoolRequest(**{"name": "projects/sample1/locations/sample2/sample3"})
...
def test_transport_kind_grpc_asyncio():
    transport = WorkerPoolsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = WorkerPoolsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
async def test_create_worker_pool_routing_parameters_request_1_grpc_asyncio():
    ...
    with mock.patch.object(type(client.transport.create_worker_pool), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.create_worker_pool(request={"parent": "projects/sample1/locations/sample2"})
...
async def test_get_worker_pool_routing_parameters_request_1_grpc_asyncio():
    ...
    with mock.patch.object(type(client.transport.get_worker_pool), "__call__") as call:
        ...
        await client.get_worker_pool(request={"name": "projects/sample1/locations/sample2/sample3"})
...
async def test_list_worker_pools_routing_parameters_request_1_grpc_asyncio():
    ...
    with mock.patch.object(type(client.transport.list_worker_pools), "__call__") as call:
        ...
        await client.list_worker_pools(request={"parent": "projects/sample1/locations/sample2"})
...
async def test_update_worker_pool_routing_parameters_request_1_grpc_asyncio():
    ...
    with mock.patch.object(type(client.transport.update_worker_pool), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.update_worker_pool(request={"worker_pool": {"name": "projects/sample1/locations/sample2/sample3"}})
...
async def test_delete_worker_pool_routing_parameters_request_1_grpc_asyncio():
    ...
    with mock.patch.object(type(client.transport.delete_worker_pool), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.delete_worker_pool(request={"name": "projects/sample1/locations/sample2/sample3"})
...
def test_transport_kind_rest():
    transport = WorkerPoolsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_create_worker_pool_rest_bad_request(request_type=gcr_worker_pool.CreateWorkerPoolRequest):
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_create_worker_pool_rest_call_success(request_type):
    ...
    request_init["worker_pool"] = {
        ...
        "client_version": "client_version_value",
        "launch_stage": 6,
        "binary_authorization": {"use_default": True, "policy": "policy_value", "breakglass_justification": "breakglass_justification_value"},
        "template": {
            ...
            "vpc_access": {
                ...
                "egress": 1,
                "network_interfaces": [{"network": "network_value", "subnetwork": "subnetwork_value", "tags": ["tags_value1", "tags_value2"]}],
            },
            ...
            "containers": [
                {
                    "name": "name_value",
                    "image": "image_value",
                    "source_code": {"cloud_storage_source": {"bucket": "bucket_value", "object_": "object__value", "generation": 1068}},
                    ...
                    "env": [
                        {
                            "name": "name_value",
                            "value": "value_value",
                            "value_source": {"secret_key_ref": {"secret": "secret_value", "version": "version_value"}},
                        }
                    ],
                    "resources": {"limits": {}, "cpu_idle": True, "startup_cpu_boost": True},
                    "ports": [{"name": "name_value", "container_port": 1511}],
                    "volume_mounts": [{"name": "name_value", "mount_path": "mount_path_value", "sub_path": "sub_path_value"}],
                    ...
                    "liveness_probe": {
                        ...
                        "http_get": {"path": "path_value", "http_headers": [{"name": "name_value", "value": "value_value"}], "port": 453},
                        "tcp_socket": {"port": 453},
                    ...
                    "build_info": {"function_target": "function_target_value", "source_location": "source_location_value"},
                }
            ],
            "volumes": [
                {
                    ...
                    "secret": {
                        "secret": "secret_value",
                        "items": [{"path": "path_value", "version": "version_value", "mode": 421}],
                        "default_mode": 1257,
                    },
                    "cloud_sql_instance": {"instances": ["instances_value1", "instances_value2"]},
                    "empty_dir": {"medium": 1, "size_limit": "size_limit_value"},
                    "nfs": {"server": "server_value", "path": "path_value", "read_only": True},
                    "gcs": {"bucket": "bucket_value", "read_only": True, "mount_options": ["mount_options_value1", "mount_options_value2"]},
                }
            ],
        ...
        "latest_ready_revision": "latest_ready_revision_value",
        "latest_created_revision": "latest_created_revision_value",
        "instance_split_statuses": [{"type_": 1, "revision": "revision_value", "percent": 753}],
        "threat_detection_enabled": True,
...
def test_create_worker_pool_rest_interceptors(null_interceptor):
    transport = transports.WorkerPoolsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.WorkerPoolsRestInterceptor(),
    )
    client = WorkerPoolsClient(transport=transport)
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        pb_message = gcr_worker_pool.CreateWorkerPoolRequest.pb(gcr_worker_pool.CreateWorkerPoolRequest())
...
def test_get_worker_pool_rest_bad_request(request_type=worker_pool.GetWorkerPoolRequest):
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_get_worker_pool") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = worker_pool.GetWorkerPoolRequest.pb(worker_pool.GetWorkerPoolRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_list_worker_pools_rest_bad_request(request_type=worker_pool.ListWorkerPoolsRequest):
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_list_worker_pools_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_list_worker_pools") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = worker_pool.ListWorkerPoolsRequest.pb(worker_pool.ListWorkerPoolsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = worker_pool.ListWorkerPoolsResponse.to_json(worker_pool.ListWorkerPoolsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = worker_pool.ListWorkerPoolsResponse(), metadata
...
def test_update_worker_pool_rest_bad_request(request_type=gcr_worker_pool.UpdateWorkerPoolRequest):
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_update_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcr_worker_pool.UpdateWorkerPoolRequest.pb(gcr_worker_pool.UpdateWorkerPoolRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_worker_pool_rest_bad_request(request_type=worker_pool.DeleteWorkerPoolRequest):
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/workerPools/sample3"}
    request = request_type(**request_init)
...
def test_delete_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = worker_pool.DeleteWorkerPoolRequest.pb(worker_pool.DeleteWorkerPoolRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_get_iam_policy_rest_bad_request(request_type=iam_policy_pb2.GetIamPolicyRequest):
    client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"resource": "projects/sample1/locations/sample2/workerPools/sample3"}
    request = request_type(**request_init)
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_get_iam_policy") as post, mock.patch.object(
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_set_iam_policy") as post, mock.patch.object(
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
...
def test_worker_pools_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.WorkerPoolsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_worker_pools_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.run_v2.services.worker_pools.transports.WorkerPoolsTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.WorkerPoolsGrpcTransport, grpc_helpers), (transports.WorkerPoolsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.WorkerPoolsGrpcTransport, transports.WorkerPoolsGrpcAsyncIOTransport])
...
def test_worker_pools_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.WorkerPoolsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.WorkerPoolsTransport, "_prep_wrapped_messages") as prep:
        client = WorkerPoolsClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = WorkerPoolsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
