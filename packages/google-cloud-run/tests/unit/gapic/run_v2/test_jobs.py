...
from google.cloud.run_v2.services.jobs import JobsAsyncClient, JobsClient, pagers, transports
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert JobsClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert JobsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert JobsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                JobsClient._use_client_cert_effective()
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = JobsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = JobsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = JobsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = JobsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = JobsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = JobsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_create_job_rest_bad_request(request_type=gcr_job.CreateJobRequest):
    client = JobsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_create_job_rest_call_success(request_type):
    ...
    request_init["job"] = {
        ...
        "conditions": {},
        "execution_count": 1628,
        "latest_created_execution": {"name": "name_value", "create_time": {}, "completion_time": {}, "delete_time": {}, "completion_status": 1},
        "reconciling": True,
        "satisfies_pzs": True,
        "start_execution_token": "start_execution_token_value",
...
def test_get_job_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.JobsRestInterceptor, "post_get_job") as post, mock.patch.object(
...
def test_list_jobs_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.JobsRestInterceptor, "post_list_jobs") as post, mock.patch.object(
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.JobsRestInterceptor, "post_get_iam_policy") as post, mock.patch.object(
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.JobsRestInterceptor, "post_set_iam_policy") as post, mock.patch.object(
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.JobsRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
...
def test_jobs_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.JobsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.JobsGrpcTransport, grpc_helpers), (transports.JobsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_jobs_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.JobsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_ctx():
    ...
    for transport in transports:
        client = JobsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
