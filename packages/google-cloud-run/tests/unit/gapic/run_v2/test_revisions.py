...
async def test_list_revisions_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_revisions(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_revision_rest_required_fields(request_type=revision.GetRevisionRequest):
    ...
    default_values = getattr(
        transport_class._BaseGetRevision,
        "_BaseGetRevision__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
...
def test_list_revisions_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseListRevisions,
        "_BaseListRevisions__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_delete_revision_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseDeleteRevision,
        "_BaseDeleteRevision__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
