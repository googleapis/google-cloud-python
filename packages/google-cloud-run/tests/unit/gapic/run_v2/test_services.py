...
from google.cloud.run_v2.services.services import ServicesAsyncClient, ServicesClient, pagers, transports
...
def test_create_service_rest_call_success(request_type):
    ...
    request_init["service"] = {
        ...
        "traffic": [{"type_": 1, "revision": "revision_value", "percent": 753, "tag": "tag_value"}],
        "scaling": {"min_instance_count": 1920, "scaling_mode": 1, "max_instance_count": 1922, "manual_instance_count": 2234},
        "invoker_iam_disabled": True,
        "default_uri_disabled": True,
        "urls": ["urls_value1", "urls_value2"],
        "iap_enabled": True,
        "multi_region_settings": {"regions": ["regions_value1", "regions_value2"], "multi_region_id": "multi_region_id_value"},
        ...
        "latest_ready_revision": "latest_ready_revision_value",
        "latest_created_revision": "latest_created_revision_value",
        "traffic_statuses": [{"type_": 1, "revision": "revision_value", "percent": 753, "tag": "tag_value", "uri": "uri_value"}],
        "uri": "uri_value",
        "satisfies_pzs": True,
        "threat_detection_enabled": True,
        "build_config": {
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServicesRestInterceptor, "post_get_iam_policy") as post, mock.patch.object(
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServicesRestInterceptor, "post_set_iam_policy") as post, mock.patch.object(
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServicesRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
...
def test_build_worker_pool_path():
    ...
    actual = ServicesClient.build_worker_pool_path(project, location, worker_pool)
    assert expected == actual
...
def test_parse_build_worker_pool_path():
    ...
    path = ServicesClient.build_worker_pool_path(**expected)

    # Check that the path construction is reversible.
    actual = ServicesClient.parse_build_worker_pool_path(path)
    assert expected == actual
