...
from google.cloud.parallelstore_v1.services.parallelstore import ParallelstoreAsyncClient, ParallelstoreClient, pagers, transports
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert ParallelstoreClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert ParallelstoreClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                ParallelstoreClient._use_client_cert_effective()
...
def test__get_api_endpoint():
    ...
    assert (
        ParallelstoreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ParallelstoreClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_parallelstore_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "parallelstore.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://parallelstore.googleapis.com"
        )
...
@pytest.mark.parametrize("client_class", [ParallelstoreClient, ParallelstoreAsyncClient])
...
def test_parallelstore_client_client_options_from_dict():
    with mock.patch("google.cloud.parallelstore_v1.services.parallelstore.transports.ParallelstoreGrpcTransport.__init__") as grpc_transport:
        grpc_transport.return_value = None
        client = ParallelstoreClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
async def test_list_instances_async_pager():
    ...
    with mock.patch.object(type(client.transport.list_instances), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        assert all(isinstance(i, parallelstore.Instance) for i in responses)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = ParallelstoreClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = ParallelstoreClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = ParallelstoreClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = ParallelstoreAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ParallelstoreAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ParallelstoreClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_instances_rest_bad_request(request_type=parallelstore.ListInstancesRequest):
    client = ParallelstoreClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_instances_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.ParallelstoreRestInterceptor, "post_list_instances") as post, mock.patch.object(
...
def test_get_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ParallelstoreRestInterceptor, "post_get_instance") as post, mock.patch.object(
...
def test_parallelstore_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.ParallelstoreTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_parallelstore_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.parallelstore_v1.services.parallelstore.transports.ParallelstoreTransport.__init__") as Transport:
...
def test_parallelstore_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.cloud.parallelstore_v1.services.parallelstore.transports.ParallelstoreTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
@pytest.mark.parametrize(
    ...
    [(transports.ParallelstoreGrpcTransport, grpc_helpers), (transports.ParallelstoreGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_parallelstore_grpc_transport_client_cert_source_for_mtls(transport_class):
...
def test_parallelstore_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.ParallelstoreRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_parallelstore_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "parallelstore.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://parallelstore.googleapis.com:8000"
    )
...
@pytest.mark.parametrize("transport_class", [transports.ParallelstoreGrpcTransport, transports.ParallelstoreGrpcAsyncIOTransport])
def test_parallelstore_transport_channel_mtls_with_client_cert_source(transport_class):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ParallelstoreGrpcTransport, transports.ParallelstoreGrpcAsyncIOTransport])
def test_parallelstore_transport_channel_mtls_with_adc(transport_class):
    mock_ssl_cred = mock.Mock()
...
def test_address_path():
    ...
    region = "clam"
    address = "whelk"
    expected = "projects/{project}/regions/{region}/addresses/{address}".format(
        project=project,
        region=region,
        address=address,
    )
...
def test_get_location_field_headers():
    client = ParallelstoreClient(credentials=ga_credentials.AnonymousCredentials())
...
async def test_get_location_field_headers_async():
    client = ParallelstoreAsyncClient(credentials=async_anonymous_credentials())
...
def test_client_ctx():
    ...
    for transport in transports:
        client = ParallelstoreClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
