...
def test__get_default_mtls_endpoint():
    ...
    assert (
        CloudCatalogClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        CloudCatalogClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        CloudCatalogClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                CloudCatalogClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                CloudCatalogClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(CloudCatalogClient),
)
@mock.patch.object(
    CloudCatalogAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(CloudCatalogAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = CloudCatalogClient._DEFAULT_UNIVERSE
    default_endpoint = CloudCatalogClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = CloudCatalogClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        CloudCatalogClient._get_api_endpoint(
    ...
    assert (
        ...
        == CloudCatalogClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        CloudCatalogClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        CloudCatalogClient._get_api_endpoint(None, None, default_universe, "always")
        == CloudCatalogClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        CloudCatalogClient._get_api_endpoint(
    ...
    assert (
        CloudCatalogClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        CloudCatalogClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        CloudCatalogClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        CloudCatalogClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        CloudCatalogClient._get_universe_domain(None, None)
        == CloudCatalogClient._DEFAULT_UNIVERSE
    )
...
def test_transport_kind_grpc_asyncio():
    transport = CloudCatalogAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = CloudCatalogClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
