...
def test__read_environment_variables():
    assert CloudCatalogClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert CloudCatalogClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                CloudCatalogClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert CloudCatalogClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert CloudCatalogClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert CloudCatalogClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert CloudCatalogClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert CloudCatalogClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert CloudCatalogClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                CloudCatalogClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert CloudCatalogClient._get_client_cert_source(None, False) is None
    assert CloudCatalogClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert CloudCatalogClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert CloudCatalogClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert CloudCatalogClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(CloudCatalogClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudCatalogClient))
@mock.patch.object(CloudCatalogAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudCatalogAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = CloudCatalogClient._DEFAULT_UNIVERSE
    default_endpoint = CloudCatalogClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = CloudCatalogClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert CloudCatalogClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert CloudCatalogClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == CloudCatalogClient.DEFAULT_MTLS_ENDPOINT
    assert CloudCatalogClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert CloudCatalogClient._get_api_endpoint(None, None, default_universe, "always") == CloudCatalogClient.DEFAULT_MTLS_ENDPOINT
    assert CloudCatalogClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == CloudCatalogClient.DEFAULT_MTLS_ENDPOINT
    assert CloudCatalogClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert CloudCatalogClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        CloudCatalogClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert CloudCatalogClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert CloudCatalogClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert CloudCatalogClient._get_universe_domain(None, None) == CloudCatalogClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        CloudCatalogClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = CloudCatalogClient(credentials=cred)
...
def test_cloud_catalog_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "cloudbilling.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudbilling.googleapis.com"
        )
...
@mock.patch.object(CloudCatalogClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudCatalogClient))
@mock.patch.object(CloudCatalogAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudCatalogAsyncClient))
def test_cloud_catalog_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        (CloudCatalogClient, transports.CloudCatalogGrpcTransport, "grpc", grpc_helpers),
        (CloudCatalogAsyncClient, transports.CloudCatalogGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
async def test_list_services_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_services), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_services(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_transport_kind_grpc():
    transport = CloudCatalogClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = CloudCatalogAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = CloudCatalogAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = CloudCatalogClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_services_rest_bad_request(request_type=cloud_catalog.ListServicesRequest):
...
def test_list_services_rest_interceptors(null_interceptor):
    transport = transports.CloudCatalogRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.CloudCatalogRestInterceptor(),
    )
    client = CloudCatalogClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.CloudCatalogRestInterceptor, "post_list_services") as post, mock.patch.object(
        transports.CloudCatalogRestInterceptor, "post_list_services_with_metadata"
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_catalog.ListServicesResponse.to_json(cloud_catalog.ListServicesResponse())
        ...
        post_with_metadata.return_value = cloud_catalog.ListServicesResponse(), metadata
...
def test_list_skus_rest_bad_request(request_type=cloud_catalog.ListSkusRequest):
    client = CloudCatalogClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_list_skus_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.CloudCatalogRestInterceptor, "post_list_skus") as post, mock.patch.object(
        transports.CloudCatalogRestInterceptor, "post_list_skus_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_catalog.ListSkusResponse.to_json(cloud_catalog.ListSkusResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = cloud_catalog.ListSkusResponse(), metadata
...
@pytest.mark.parametrize(
    ...
    [(transports.CloudCatalogGrpcTransport, grpc_helpers), (transports.CloudCatalogGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_cloud_catalog_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "cloudbilling.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://cloudbilling.googleapis.com:8000"
    )
