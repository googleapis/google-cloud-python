...
class JobControllerClient(object):
    ...
    def list_jobs(
        ...
        Returns:
            ...
            An iterable of :class:`~google.cloud.dataproc_v1beta2.types.Job` instances.
    ...
    def update_job(
        ...
        Args:
            ...
            job (Union[dict, ~google.cloud.dataproc_v1beta2.types.Job]): Required. The changes to the job.
                ...
                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.cloud.dataproc_v1beta2.types.Job`
            update_mask (Union[dict, ~google.cloud.dataproc_v1beta2.types.FieldMask]): Required. Specifies the path, relative to Job, of the field to
                update. For example, to update the labels of a Job the update_mask
                parameter would be specified as labels, and the ``PATCH`` request body
                would specify the new value. Note: Currently, labels is the only field
                that can be updated.
