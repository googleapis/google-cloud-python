...
def test__get_default_mtls_endpoint():
    ...
    assert (
        JobControllerClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
async def test_submit_job_async(request_type, transport: str = "grpc_asyncio"):
    client = JobControllerAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_submit_job_as_operation_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(
            transports.JobControllerRestInterceptor,
