...
async def test_stop_cluster_async(request_type, transport: str = "grpc_asyncio"):
    client = ClusterControllerAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_start_cluster_async(request_type, transport: str = "grpc_asyncio"):
    client = ClusterControllerAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
async def test_diagnose_cluster_async(request_type, transport: str = "grpc_asyncio"):
    client = ClusterControllerAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_stop_cluster_rest_required_fields(request_type=clusters.StopClusterRequest):
    ...
    default_values = getattr(
        transport_class._BaseStopCluster,
        "_BaseStopCluster__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_start_cluster_rest_required_fields(request_type=clusters.StartClusterRequest):
    ...
    default_values = getattr(
        transport_class._BaseStartCluster,
        "_BaseStartCluster__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_diagnose_cluster_rest_required_fields(
    ...
    default_values = getattr(
        transport_class._BaseDiagnoseCluster,
        "_BaseDiagnoseCluster__REQUIRED_FIELDS_DEFAULT_VALUES",
        {},
    )
    unset_fields = {
        k: v for k, v in default_values.items() if k not in jsonified_request
...
def test_list_clusters_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = clusters.ListClustersResponse.to_json(
            clusters.ListClustersResponse()
        )
        req.return_value.content = return_value
