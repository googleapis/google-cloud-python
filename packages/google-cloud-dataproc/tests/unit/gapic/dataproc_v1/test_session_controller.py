...
def test__get_client_cert_source():
    ...
    assert (
        SessionControllerClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                SessionControllerClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(SessionControllerClient),
)
@mock.patch.object(
    SessionControllerAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(SessionControllerAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = SessionControllerClient._DEFAULT_UNIVERSE
    default_endpoint = SessionControllerClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = SessionControllerClient._DEFAULT_ENDPOINT_TEMPLATE.format(
    ...
    assert (
        SessionControllerClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        SessionControllerClient._get_api_endpoint(
    ...
    assert (
        SessionControllerClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        SessionControllerClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        SessionControllerClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        SessionControllerClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        SessionControllerClient._get_universe_domain(None, None)
        == SessionControllerClient._DEFAULT_UNIVERSE
    )
...
def test_terminate_session_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[client._transport.terminate_session] = (
            mock_rpc
        )
        request = {}
...
@pytest.mark.asyncio
async def test_terminate_session_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.terminate_session
            in client._client._transport._wrapped_methods
        )
...
def test_list_sessions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = sessions.ListSessionsResponse.to_json(
            sessions.ListSessionsResponse()
        )
        req.return_value.content = return_value
...
def test_terminate_session_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(
        ...
        transports.SessionControllerRestInterceptor,
