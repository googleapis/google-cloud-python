...
class ManagedKafkaClient(metaclass=ManagedKafkaClientMeta):
    ...
    def parse_acl_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/clusters/(?P<cluster>.+?)/acls/(?P<acl>.+?)$", path)
    ...
    def parse_consumer_group_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/clusters/(?P<cluster>.+?)/consumerGroups/(?P<consumer_group>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def parse_topic_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/clusters/(?P<cluster>.+?)/topics/(?P<topic>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = ManagedKafkaClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, ManagedKafkaTransport, Callable[..., ManagedKafkaTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = ManagedKafkaClient._read_environment_variables()
        self._client_cert_source = ManagedKafkaClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = ManagedKafkaClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[ManagedKafkaTransport], Callable[..., ManagedKafkaTransport]] = (
