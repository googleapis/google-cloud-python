...
class ManagedKafkaConnectClient(metaclass=ManagedKafkaConnectClientMeta):
    ...
    def parse_connect_cluster_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/connectClusters/(?P<connect_cluster>.+?)$", path)
    ...
    def parse_connector_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/connectClusters/(?P<connect_cluster>.+?)/connectors/(?P<connector>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = ManagedKafkaConnectClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(
        ...
        *,
        ...
        transport: Optional[Union[str, ManagedKafkaConnectTransport, Callable[..., ManagedKafkaConnectTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = ManagedKafkaConnectClient._read_environment_variables()
        self._client_cert_source = ManagedKafkaConnectClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = ManagedKafkaConnectClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        self._api_endpoint = self._api_endpoint or ManagedKafkaConnectClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[ManagedKafkaConnectTransport], Callable[..., ManagedKafkaConnectTransport]] = (
