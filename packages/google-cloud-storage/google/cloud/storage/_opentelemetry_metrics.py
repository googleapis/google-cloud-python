...
try:
    ...
    logger.debug(
        "OpenTelemetry metrics package is not installed. "
        "GCS client metrics are disabled."
    )
...
def get_common_attributes() -> Dict[str, Any]:
    """Returns standard GCS client attributes for metrics."""
    return {
...
def get_meter():
    ...
    if not HAS_OPENTELEMETRY_METRICS:
        return None
    return metrics.get_meter("google.cloud.storage", __version__)
