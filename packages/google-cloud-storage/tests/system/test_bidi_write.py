...
def make_blob(bucket_name, object_name, location_type, generation=None):
    ...
    if generation is not None:
        blob._properties["generation"] = generation
    return blob
...
async def test_appendable_upload_empty_object(grpc_client, bidi_bucket, bidi_location_type, finalize_on_close):
    async def _run():
        object_name = f"test_empty_{uuid.uuid4()}"
        blob = make_blob(bidi_bucket, object_name, bidi_location_type)
...
async def test_appendable_upload_bytes(grpc_client, bidi_bucket, bidi_location_type, flush_interval, finalize_on_close, object_size):
    async def _run():
        ...
        writer = AsyncAppendableObjectWriter.from_blob(
            client=grpc_client, blob=blob, writer_options=writer_options
        )
        await writer.open()
...
@pytest.mark.asyncio
async def test_explicit_flush(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
...
@pytest.mark.asyncio
async def test_appendable_blob_upload_takeover(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
        ...
        blob1 = make_blob(bidi_bucket, object_name, bidi_location_type)
        writer1 = AsyncAppendableObjectWriter.from_blob(grpc_client, blob1)
        await writer1.open()
        ...
        blob2 = make_blob(bidi_bucket, object_name, bidi_location_type, generation=generation)
        writer2 = AsyncAppendableObjectWriter.from_blob(grpc_client, blob2)
        await writer2.open()
...
@pytest.mark.asyncio
async def test_takeover_just_to_finalize(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
...
@pytest.mark.asyncio
async def test_explicit_finalize_with_correct_checksum(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
...
@pytest.mark.asyncio
async def test_explicit_finalize_with_incorrect_checksum_fails(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
...
@pytest.mark.asyncio
async def test_takeover_just_to_finalize_with_incorrect_checksum_fails(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
...
@pytest.mark.asyncio
async def test_takeover_and_append_with_correct_checksum_works(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
...
@pytest.mark.asyncio
async def test_takeover_and_append_with_incorrect_checksum_fails(grpc_client, bidi_bucket, bidi_location_type):
    async def _run():
