...
class DataprocMetastoreClient(metaclass=DataprocMetastoreClientMeta):
    ...
    def parse_metadata_import_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/services/(?P<service>.+?)/metadataImports/(?P<metadata_import>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def __init__(
        ...
        *,
        ...
        self._api_endpoint = self._api_endpoint or DataprocMetastoreClient._get_api_endpoint(
