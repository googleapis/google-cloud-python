    ...
    * Stripping extraneous information from responses
    ...
    .. code-block:: python
        class MyCustomRegionInstanceGroupManagersInterceptor(RegionInstanceGroupManagersRestInterceptor):
            ...
            def pre_adopt_instances(self, request, metadata):
                logging.log(f"Received request: {request}")
                return request, metadata

            def post_adopt_instances(self, response):
                logging.log(f"Received response: {response}")
                return response
    ...
    def pre_adopt_instances(
        ...
        """Pre-rpc interceptor for adopt_instances
    ...
    def post_adopt_instances(self, response: compute.Operation) -> compute.Operation:
        """Post-rpc interceptor for adopt_instances

        DEPRECATED. Please use the `post_adopt_instances_with_metadata`
        interceptor instead.

        Override in a subclass to read or manipulate the response
        after it is returned by the RegionInstanceGroupManagers server but before
        it is returned to user code. This `post_adopt_instances` interceptor runs
        before the `post_adopt_instances_with_metadata` interceptor.
        """
        return response

    def post_adopt_instances_with_metadata(
        self,
        response: compute.Operation,
        metadata: Sequence[Tuple[str, Union[str, bytes]]],
    ) -> Tuple[compute.Operation, Sequence[Tuple[str, Union[str, bytes]]]]:
        ...
        We recommend only using this `post_adopt_instances_with_metadata`
        interceptor in new development instead of the `post_adopt_instances` interceptor.
        When both interceptors are used, this `post_adopt_instances_with_metadata` interceptor runs after the
        `post_adopt_instances` interceptor. The (possibly modified) response returned by
        `post_adopt_instances` will be passed to
...
class RegionInstanceGroupManagersRestTransport(
    ...
    class _AdoptInstances(
        _BaseRegionInstanceGroupManagersRestTransport._BaseAdoptInstances,
        RegionInstanceGroupManagersRestStub,
    ):
        def __hash__(self):
            return hash("RegionInstanceGroupManagersRestTransport.AdoptInstances")
        ...
        def __call__(
            ...
            *,
            ...
            r"""Call the adopt instances method over HTTP.

            Args:
                request (~.compute.AdoptInstancesRegionInstanceGroupManagerRequest):
                    The request object. A request message for
                RegionInstanceGroupManagers.AdoptInstances.
                See the method description for details.
                retry (google.api_core.retry.Retry): Designation of what errors, if any,
                    should be retried.
                timeout (float): The timeout for this request.
            ...
            http_options = _BaseRegionInstanceGroupManagersRestTransport._BaseAdoptInstances._get_http_options()

            request, metadata = self._interceptor.pre_adopt_instances(request, metadata)
            transcoded_request = _BaseRegionInstanceGroupManagersRestTransport._BaseAdoptInstances._get_transcoded_request(
                http_options, request
            )

            body = _BaseRegionInstanceGroupManagersRestTransport._BaseAdoptInstances._get_request_body_json(
                transcoded_request
            )

            # Jsonify the query params
            query_params = _BaseRegionInstanceGroupManagersRestTransport._BaseAdoptInstances._get_query_params_json(
                transcoded_request
            )
            ...
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                ...
                _LOGGER.debug(
                    f"Sending request for google.cloud.compute_v1beta.RegionInstanceGroupManagersClient.AdoptInstances",
                    extra={
                        "serviceName": "google.cloud.compute.v1beta.RegionInstanceGroupManagers",
                        "rpcName": "AdoptInstances",
                        "httpRequest": http_request,
                        "metadata": http_request["headers"],
                    },
                )
            ...
            # Send the request
            response = (
                RegionInstanceGroupManagersRestTransport._AdoptInstances._get_response(
                    self._host,
                    metadata,
                    query_params,
                    self._session,
            ...
            resp = self._interceptor.post_adopt_instances(resp)
            response_metadata = [(k, str(v)) for k, v in response.headers.items()]
            resp, _ = self._interceptor.post_adopt_instances_with_metadata(
                resp, response_metadata
            )
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                logging.DEBUG
                ...
                _LOGGER.debug(
                    "Received response for google.cloud.compute_v1beta.RegionInstanceGroupManagersClient.adopt_instances",
                    extra={
                        "serviceName": "google.cloud.compute.v1beta.RegionInstanceGroupManagers",
                        "rpcName": "AdoptInstances",
                        "metadata": http_response["headers"],
    ...
    def adopt_instances(
        ...
        [compute.AdoptInstancesRegionInstanceGroupManagerRequest], compute.Operation
    ]:
        # The return type is fine, but mypy isn't sophisticated enough to determine what's going on here.
        # In C++ this would require a dynamic_cast
        return self._AdoptInstances(self._session, self._host, self._interceptor)  # type: ignore
