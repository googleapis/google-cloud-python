...
from google.cloud.compute_v1beta.services.region_target_tcp_proxies import RegionTargetTcpProxiesClient, pagers, transports
...
def test__read_environment_variables():
    assert RegionTargetTcpProxiesClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert RegionTargetTcpProxiesClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                RegionTargetTcpProxiesClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert RegionTargetTcpProxiesClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert RegionTargetTcpProxiesClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert RegionTargetTcpProxiesClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert RegionTargetTcpProxiesClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert RegionTargetTcpProxiesClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert RegionTargetTcpProxiesClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                RegionTargetTcpProxiesClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert RegionTargetTcpProxiesClient._get_client_cert_source(None, False) is None
    assert RegionTargetTcpProxiesClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert RegionTargetTcpProxiesClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert RegionTargetTcpProxiesClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert RegionTargetTcpProxiesClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(RegionTargetTcpProxiesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionTargetTcpProxiesClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = RegionTargetTcpProxiesClient._DEFAULT_UNIVERSE
    default_endpoint = RegionTargetTcpProxiesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = RegionTargetTcpProxiesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert RegionTargetTcpProxiesClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        RegionTargetTcpProxiesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
        == RegionTargetTcpProxiesClient.DEFAULT_MTLS_ENDPOINT
    )
    assert RegionTargetTcpProxiesClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert (
        RegionTargetTcpProxiesClient._get_api_endpoint(None, None, default_universe, "always") == RegionTargetTcpProxiesClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        RegionTargetTcpProxiesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == RegionTargetTcpProxiesClient.DEFAULT_MTLS_ENDPOINT
    )
    assert RegionTargetTcpProxiesClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert RegionTargetTcpProxiesClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        RegionTargetTcpProxiesClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert RegionTargetTcpProxiesClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert RegionTargetTcpProxiesClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert RegionTargetTcpProxiesClient._get_universe_domain(None, None) == RegionTargetTcpProxiesClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        RegionTargetTcpProxiesClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = RegionTargetTcpProxiesClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        (RegionTargetTcpProxiesClient, "rest"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (transports.RegionTargetTcpProxiesRestTransport, "rest"),
    ],
)
...
def test_region_target_tcp_proxies_client_get_transport_class():
    ...
    available_transports = [
        transports.RegionTargetTcpProxiesRestTransport,
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (RegionTargetTcpProxiesClient, transports.RegionTargetTcpProxiesRestTransport, "rest"),
    ],
)
@mock.patch.object(RegionTargetTcpProxiesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionTargetTcpProxiesClient))
...
@pytest.mark.parametrize(
    ...
    [
        (RegionTargetTcpProxiesClient, transports.RegionTargetTcpProxiesRestTransport, "rest", "true"),
        (RegionTargetTcpProxiesClient, transports.RegionTargetTcpProxiesRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(RegionTargetTcpProxiesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionTargetTcpProxiesClient))
...
@pytest.mark.parametrize("client_class", [RegionTargetTcpProxiesClient])
@mock.patch.object(RegionTargetTcpProxiesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionTargetTcpProxiesClient))
def test_region_target_tcp_proxies_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        (RegionTargetTcpProxiesClient, transports.RegionTargetTcpProxiesRestTransport, "rest", None),
    ],
)
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/compute/beta/projects/{project}/regions/{region}/targetTcpProxies/{target_tcp_proxy}" % client.transport._host, args[1]
        )
...
def test_delete_rest_flattened_error(transport: str = "rest"):
    ...
    with pytest.raises(ValueError):
        client.delete(
            compute.DeleteRegionTargetTcpProxyRequest(),
            project="project_value",
            region="region_value",
            target_tcp_proxy="target_tcp_proxy_value",
        )
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        mock_args = dict(
            project="project_value",
            region="region_value",
            target_tcp_proxy_resource=compute.TargetTcpProxy(creation_timestamp="creation_timestamp_value"),
        )
        mock_args.update(sample_request)
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/regions/{region}/targetTcpProxies" % client.transport._host, args[1])
...
def test_list_rest_flattened_error(transport: str = "rest"):
    ...
    with pytest.raises(ValueError):
        client.list(
            compute.ListRegionTargetTcpProxiesRequest(),
            project="project_value",
            region="region_value",
        )
...
def test_test_iam_permissions_rest_required_fields(request_type=compute.TestIamPermissionsRegionTargetTcpProxyRequest):
    transport_class = transports.RegionTargetTcpProxiesRestTransport
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/beta/projects/{project}/regions/{region}/targetTcpProxies/{resource}/testIamPermissions" % client.transport._host, args[1]
        )
...
def test_test_iam_permissions_rest_flattened_error(transport: str = "rest"):
    ...
    with pytest.raises(ValueError):
        client.test_iam_permissions(
            compute.TestIamPermissionsRegionTargetTcpProxyRequest(),
            project="project_value",
            region="region_value",
            resource="resource_value",
            test_permissions_request_resource=compute.TestPermissionsRequest(permissions=["permissions_value"]),
...
def test_credentials_transport_error():
    ...
    transport = transports.RegionTargetTcpProxiesRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    with pytest.raises(ValueError):
        client = RegionTargetTcpProxiesClient(
...
def test_transport_kind_rest():
    transport = RegionTargetTcpProxiesClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_delete_rest_bad_request(request_type=compute.DeleteRegionTargetTcpProxyRequest):
...
def test_delete_rest_interceptors(null_interceptor):
    transport = transports.RegionTargetTcpProxiesRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.RegionTargetTcpProxiesRestInterceptor(),
    )
    client = RegionTargetTcpProxiesClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionTargetTcpProxiesRestInterceptor, "post_delete") as post, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "post_delete_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "pre_delete"
    ) as pre:
...
def test_get_rest_bad_request(request_type=compute.GetRegionTargetTcpProxyRequest):
    client = RegionTargetTcpProxiesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "region": "sample2", "target_tcp_proxy": "sample3"}
    request = request_type(**request_init)
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionTargetTcpProxiesRestInterceptor, "post_get") as post, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "post_get_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "pre_get"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.TargetTcpProxy.to_json(compute.TargetTcpProxy())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.TargetTcpProxy(), metadata
...
def test_insert_rest_bad_request(request_type=compute.InsertRegionTargetTcpProxyRequest):
    client = RegionTargetTcpProxiesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_insert_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "region": "sample2"}
    request_init["target_tcp_proxy_resource"] = {
        "creation_timestamp": "creation_timestamp_value",
        "description": "description_value",
        "id": 205,
        "kind": "kind_value",
    ...
    for field, value in request_init["target_tcp_proxy_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionTargetTcpProxiesRestInterceptor, "post_insert") as post, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "post_insert_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "pre_insert"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_list_rest_bad_request(request_type=compute.ListRegionTargetTcpProxiesRequest):
    client = RegionTargetTcpProxiesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "region": "sample2"}
    request = request_type(**request_init)
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionTargetTcpProxiesRestInterceptor, "post_list") as post, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "post_list_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "pre_list"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.TargetTcpProxyList.to_json(compute.TargetTcpProxyList())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.TargetTcpProxyList(), metadata
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsRegionTargetTcpProxyRequest):
    client = RegionTargetTcpProxiesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_test_iam_permissions_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.TestIamPermissionsRegionTargetTcpProxyRequest.meta.fields["test_permissions_request_resource"]
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionTargetTcpProxiesRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "post_test_iam_permissions_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.RegionTargetTcpProxiesRestInterceptor, "pre_test_iam_permissions"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.TestIamPermissionsRegionTargetTcpProxyRequest.pb(compute.TestIamPermissionsRegionTargetTcpProxyRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        request = compute.TestIamPermissionsRegionTargetTcpProxyRequest()
        metadata = [
            ("key", "val"),
            ("cephalopod", "squid"),
        ]
...
def test_delete_unary_empty_call_rest():
    ...
    with mock.patch.object(type(client.transport.delete), "__call__") as call:
        ...
        request_msg = compute.DeleteRegionTargetTcpProxyRequest()
...
def test_get_empty_call_rest():
    ...
    with mock.patch.object(type(client.transport.get), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.GetRegionTargetTcpProxyRequest()
...
def test_insert_unary_empty_call_rest():
    ...
    with mock.patch.object(type(client.transport.insert), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.InsertRegionTargetTcpProxyRequest()
...
def test_list_empty_call_rest():
    ...
    with mock.patch.object(type(client.transport.list), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.ListRegionTargetTcpProxiesRequest()
...
def test_test_iam_permissions_empty_call_rest():
    ...
    with mock.patch.object(type(client.transport.test_iam_permissions), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.TestIamPermissionsRegionTargetTcpProxyRequest()
...
def test_region_target_tcp_proxies_base_transport():
    ...
    with mock.patch(
        "google.cloud.compute_v1beta.services.region_target_tcp_proxies.transports.RegionTargetTcpProxiesTransport.__init__"
    ) as Transport:
...
def test_region_target_tcp_proxies_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.cloud.compute_v1beta.services.region_target_tcp_proxies.transports.RegionTargetTcpProxiesTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
def test_region_target_tcp_proxies_client_transport_session_collision(transport_name):
    ...
    client1 = RegionTargetTcpProxiesClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = RegionTargetTcpProxiesClient(
        credentials=creds2,
        transport=transport_name,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (RegionTargetTcpProxiesClient, transports.RegionTargetTcpProxiesRestTransport),
    ],
)
