...
from google.cloud.compute_v1beta.services.region_commitments import RegionCommitmentsClient, pagers, transports
...
def test__read_environment_variables():
    assert RegionCommitmentsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert RegionCommitmentsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert RegionCommitmentsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert RegionCommitmentsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert RegionCommitmentsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert RegionCommitmentsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert RegionCommitmentsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                RegionCommitmentsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert RegionCommitmentsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert RegionCommitmentsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert RegionCommitmentsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert RegionCommitmentsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(RegionCommitmentsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionCommitmentsClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = RegionCommitmentsClient._DEFAULT_UNIVERSE
    default_endpoint = RegionCommitmentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = RegionCommitmentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert RegionCommitmentsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        RegionCommitmentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
        == RegionCommitmentsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert RegionCommitmentsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert RegionCommitmentsClient._get_api_endpoint(None, None, default_universe, "always") == RegionCommitmentsClient.DEFAULT_MTLS_ENDPOINT
    assert (
        RegionCommitmentsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == RegionCommitmentsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert RegionCommitmentsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert RegionCommitmentsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        RegionCommitmentsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert RegionCommitmentsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert RegionCommitmentsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert RegionCommitmentsClient._get_universe_domain(None, None) == RegionCommitmentsClient._DEFAULT_UNIVERSE
...
def test_region_commitments_client_from_service_account_info(client_class, transport_name):
...
def test_region_commitments_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize(
    ...
    [
        (RegionCommitmentsClient, transports.RegionCommitmentsRestTransport, "rest", None),
    ],
)
def test_region_commitments_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_aggregated_list_rest_required_fields(request_type=compute.AggregatedListRegionCommitmentsRequest):
...
def test_aggregated_list_rest_unset_required_fields():
    transport = transports.RegionCommitmentsRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_aggregated_list_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate("%s/compute/beta/projects/{project}/aggregated/commitments" % client.transport._host, args[1])
...
def test_get_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/compute/beta/projects/{project}/regions/{region}/commitments/{commitment}" % client.transport._host, args[1]
        )
...
def test_insert_rest_required_fields(request_type=compute.InsertRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/regions/{region}/commitments" % client.transport._host, args[1])
...
def test_insert_unary_rest_required_fields(request_type=compute.InsertRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
...
def test_test_iam_permissions_rest_required_fields(request_type=compute.TestIamPermissionsRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/beta/projects/{project}/regions/{region}/commitments/{resource}/testIamPermissions" % client.transport._host, args[1]
        )
...
def test_test_iam_permissions_rest_flattened_error(transport: str = "rest"):
    ...
    with pytest.raises(ValueError):
        client.test_iam_permissions(
            compute.TestIamPermissionsRegionCommitmentRequest(),
            project="project_value",
            region="region_value",
            resource="resource_value",
            test_permissions_request_resource=compute.TestPermissionsRequest(permissions=["permissions_value"]),
...
def test_update_rest_required_fields(request_type=compute.UpdateRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
...
def test_update_unary_rest_required_fields(request_type=compute.UpdateRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
...
def test_update_reservations_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = RegionCommitmentsClient(
        ...
        # Ensure method has been cached
        assert client._transport.update_reservations in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_reservations] = mock_rpc

        request = {}
        client.update_reservations(request)
...
def test_update_reservations_rest_required_fields(request_type=compute.UpdateReservationsRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_reservations._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            response = client.update_reservations(request)
...
def test_update_reservations_rest_unset_required_fields():
    ...
    unset_fields = transport.update_reservations._get_unset_required_fields({})
    assert set(unset_fields) == (
        set(("requestId",))
        & set(
            (
...
def test_update_reservations_rest_flattened():
    client = RegionCommitmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.update_reservations(**mock_args)
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/compute/beta/projects/{project}/regions/{region}/commitments/{commitment}/updateReservations" % client.transport._host, args[1]
        )


def test_update_reservations_rest_flattened_error(transport: str = "rest"):
    client = RegionCommitmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        client.update_reservations(
            compute.UpdateReservationsRegionCommitmentRequest(),
            project="project_value",
            region="region_value",
            commitment="commitment_value",
            region_commitments_update_reservations_request_resource=compute.RegionCommitmentsUpdateReservationsRequest(
...
def test_update_reservations_unary_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = RegionCommitmentsClient(
        ...
        request = {}
        client.update_reservations_unary(request)
...
def test_update_reservations_unary_rest_required_fields(request_type=compute.UpdateReservationsRegionCommitmentRequest):
    transport_class = transports.RegionCommitmentsRestTransport
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            response = client.update_reservations_unary(request)
...
def test_update_reservations_unary_rest_flattened():
    client = RegionCommitmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.update_reservations_unary(**mock_args)
...
def test_update_reservations_unary_rest_flattened_error(transport: str = "rest"):
    client = RegionCommitmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        client.update_reservations_unary(
            compute.UpdateReservationsRegionCommitmentRequest(),
            project="project_value",
            region="region_value",
            commitment="commitment_value",
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = RegionCommitmentsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_rest():
    transport = RegionCommitmentsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_aggregated_list_rest_bad_request(request_type=compute.AggregatedListRegionCommitmentsRequest):
    client = RegionCommitmentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    transport = transports.RegionCommitmentsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.RegionCommitmentsRestInterceptor(),
    )
    client = RegionCommitmentsClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_aggregated_list") as post, mock.patch.object(
        transports.RegionCommitmentsRestInterceptor, "post_aggregated_list_with_metadata"
        ...
        pb_message = compute.AggregatedListRegionCommitmentsRequest.pb(compute.AggregatedListRegionCommitmentsRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.CommitmentAggregatedList.to_json(compute.CommitmentAggregatedList())
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_get") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.GetRegionCommitmentRequest.pb(compute.GetRegionCommitmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.Commitment.to_json(compute.Commitment())
        req.return_value.content = return_value
...
def test_insert_rest_call_success(request_type):
    ...
    request_init["commitment_resource"] = {
        ...
        "description": "description_value",
        "end_timestamp": "end_timestamp_value",
        "existing_reservations": ["existing_reservations_value1", "existing_reservations_value2"],
        "id": 205,
        "kind": "kind_value",
        "license_resource": {"amount": 660, "cores_per_license": "cores_per_license_value", "license_": "license__value"},
        "merge_source_commitments": ["merge_source_commitments_value1", "merge_source_commitments_value2"],
        "name": "name_value",
        "plan": "plan_value",
        "region": "region_value",
        "reservations": [
            {
                "advanced_deployment_control": {"reservation_operational_mode": "reservation_operational_mode_value"},
                ...
                "id": 205,
                "kind": "kind_value",
                "linked_commitments": ["linked_commitments_value1", "linked_commitments_value2"],
                "name": "name_value",
                "protection_tier": "protection_tier_value",
                "reservation_mode": "reservation_mode_value",
                "reservation_sharing_policy": {"service_share_type": "service_share_type_value"},
                "resource_policies": {},
                "resource_status": {
                    "health_info": {"degraded_block_count": 2082, "health_status": "health_status_value", "healthy_block_count": 2017},
                    ...
                    "reservation_maintenance": {
                        ...
                        "upcoming_group_maintenance": {
                            ...
                            "maintenance_reasons": ["maintenance_reasons_value1", "maintenance_reasons_value2"],
                    ...
                    "specific_sku_allocation": {"source_instance_template_id": "source_instance_template_id_value", "utilizations": {}},
                },
                "satisfies_pzs": True,
                "scheduling_type": "scheduling_type_value",
                "self_link": "self_link_value",
        ...
        "resource_status": {"custom_term_eligibility_end_timestamp": "custom_term_eligibility_end_timestamp_value"},
        "resources": [{"accelerator_type": "accelerator_type_value", "amount": 660, "type_": "type__value"}],
        "self_link": "self_link_value",
        "split_source_commitment": "split_source_commitment_value",
        "start_timestamp": "start_timestamp_value",
        "status": "status_value",
    ...
    test_field = compute.InsertRegionCommitmentRequest.meta.fields["commitment_resource"]
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_insert") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.InsertRegionCommitmentRequest.pb(compute.InsertRegionCommitmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_list") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.ListRegionCommitmentsRequest.pb(compute.ListRegionCommitmentsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.CommitmentList.to_json(compute.CommitmentList())
        req.return_value.content = return_value
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsRegionCommitmentRequest):
    client = RegionCommitmentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_test_iam_permissions_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.TestIamPermissionsRegionCommitmentRequest.meta.fields["test_permissions_request_resource"]
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
        transports.RegionCommitmentsRestInterceptor, "post_test_iam_permissions_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.TestIamPermissionsRegionCommitmentRequest.pb(compute.TestIamPermissionsRegionCommitmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        request = compute.TestIamPermissionsRegionCommitmentRequest()
        metadata = [
            ("key", "val"),
            ("cephalopod", "squid"),
        ]
...
def test_update_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.UpdateRegionCommitmentRequest.meta.fields["commitment_resource"]
...
def test_update_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_update") as post, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.UpdateRegionCommitmentRequest.pb(compute.UpdateRegionCommitmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_reservations_rest_bad_request(request_type=compute.UpdateReservationsRegionCommitmentRequest):
    client = RegionCommitmentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"project": "sample1", "region": "sample2", "commitment": "sample3"}
    request = request_type(**request_init)
...
def test_update_reservations_rest_call_success(request_type):
    client = RegionCommitmentsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.UpdateReservationsRegionCommitmentRequest.meta.fields["region_commitments_update_reservations_request_resource"]
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["region_commitments_update_reservations_request_resource"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
    ...
    for subfield_to_delete in subfields_not_in_runtime:  # pragma: NO COVER
        ...
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init["region_commitments_update_reservations_request_resource"][field])):
                    del request_init["region_commitments_update_reservations_request_resource"][field][i][subfield]
            else:
                del request_init["region_commitments_update_reservations_request_resource"][field][subfield]
    request = request_type(**request_init)
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_update_reservations_rest_interceptors(null_interceptor):
    transport = transports.RegionCommitmentsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.RegionCommitmentsRestInterceptor(),
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.RegionCommitmentsRestInterceptor, "post_update_reservations") as post, mock.patch.object(
        transports.RegionCommitmentsRestInterceptor, "post_update_reservations_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.UpdateReservationsRegionCommitmentRequest.pb(compute.UpdateReservationsRegionCommitmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        request = compute.UpdateReservationsRegionCommitmentRequest()
        metadata = [
            ("key", "val"),
            ("cephalopod", "squid"),
        ]
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_reservations_unary_empty_call_rest():
    client = RegionCommitmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(type(client.transport.update_reservations), "__call__") as call:
        client.update_reservations_unary(request=None)
...
def test_region_commitments_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.RegionCommitmentsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_region_commitments_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.compute_v1beta.services.region_commitments.transports.RegionCommitmentsTransport.__init__") as Transport:
...
def test_region_commitments_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.RegionCommitmentsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_region_commitments_client_transport_session_collision(transport_name):
    ...
    assert session1 != session2
    session1 = client1.transport.update_reservations._session
    session2 = client2.transport.update_reservations._session
    assert session1 != session2
...
def test_client_ctx():
    ...
    for transport in transports:
        client = RegionCommitmentsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
