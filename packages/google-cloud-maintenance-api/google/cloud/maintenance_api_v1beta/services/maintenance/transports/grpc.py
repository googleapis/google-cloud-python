...
from .base import DEFAULT_CLIENT_INFO, MaintenanceTransport
...
class MaintenanceGrpcTransport(MaintenanceTransport):
    """gRPC backend transport for Maintenance.
    ...
    def summarize_maintenances(
        ...
        r"""Return a callable for the summarize maintenances method over gRPC.

        Retrieves the statistics of a specific maintenance.

        Returns:
            Callable[[~.SummarizeMaintenancesRequest],
        ...
        if "summarize_maintenances" not in self._stubs:
            self._stubs["summarize_maintenances"] = self._logged_channel.unary_unary(
                "/google.cloud.maintenance.api.v1beta.Maintenance/SummarizeMaintenances",
                request_serializer=maintenance_service.SummarizeMaintenancesRequest.serialize,
                response_deserializer=maintenance_service.SummarizeMaintenancesResponse.deserialize,
            )
        return self._stubs["summarize_maintenances"]
    ...
    def list_resource_maintenances(
        ...
        r"""Return a callable for the list resource maintenances method over gRPC.

        Retrieve a collection of resource maintenances.

        Returns:
            Callable[[~.ListResourceMaintenancesRequest],
        ...
        if "list_resource_maintenances" not in self._stubs:
            self._stubs["list_resource_maintenances"] = self._logged_channel.unary_unary(
    ...
    def get_resource_maintenance(self) -> Callable[[maintenance_service.GetResourceMaintenanceRequest], maintenance_service.ResourceMaintenance]:
        r"""Return a callable for the get resource maintenance method over gRPC.

        Retrieve a single resource maintenance.

        Returns:
            Callable[[~.GetResourceMaintenanceRequest],
                    ~.ResourceMaintenance]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_resource_maintenance" not in self._stubs:
            self._stubs["get_resource_maintenance"] = self._logged_channel.unary_unary(
                "/google.cloud.maintenance.api.v1beta.Maintenance/GetResourceMaintenance",
                request_serializer=maintenance_service.GetResourceMaintenanceRequest.serialize,
                response_deserializer=maintenance_service.ResourceMaintenance.deserialize,
            )
        return self._stubs["get_resource_maintenance"]
...
__all__ = ("MaintenanceGrpcTransport",)
