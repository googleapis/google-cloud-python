...
from google.cloud.appengine_admin_v1.services.domain_mappings import DomainMappingsAsyncClient, DomainMappingsClient, pagers, transports
...
def test__get_default_mtls_endpoint():
    ...
    assert DomainMappingsClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert DomainMappingsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert DomainMappingsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert DomainMappingsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert DomainMappingsClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert DomainMappingsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert DomainMappingsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert DomainMappingsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert DomainMappingsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert DomainMappingsClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert DomainMappingsClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                DomainMappingsClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert DomainMappingsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert DomainMappingsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert DomainMappingsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert DomainMappingsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(DomainMappingsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DomainMappingsClient))
@mock.patch.object(DomainMappingsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DomainMappingsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DomainMappingsClient._DEFAULT_UNIVERSE
    default_endpoint = DomainMappingsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = DomainMappingsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert DomainMappingsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        DomainMappingsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DomainMappingsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert DomainMappingsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert DomainMappingsClient._get_api_endpoint(None, None, default_universe, "always") == DomainMappingsClient.DEFAULT_MTLS_ENDPOINT
    assert (
        DomainMappingsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == DomainMappingsClient.DEFAULT_MTLS_ENDPOINT
    )
    assert DomainMappingsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert DomainMappingsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        DomainMappingsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert DomainMappingsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert DomainMappingsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert DomainMappingsClient._get_universe_domain(None, None) == DomainMappingsClient._DEFAULT_UNIVERSE
...
def test_domain_mappings_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "appengine.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://appengine.googleapis.com"
        )
...
def test_domain_mappings_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DomainMappingsAsyncClient, transports.DomainMappingsGrpcAsyncIOTransport, "grpc_asyncio"),
        (DomainMappingsClient, transports.DomainMappingsRestTransport, "rest"),
    ],
)
@mock.patch.object(DomainMappingsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DomainMappingsClient))
@mock.patch.object(DomainMappingsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DomainMappingsAsyncClient))
def test_domain_mappings_client_client_options(client_class, transport_class, transport_name):
...
def test_domain_mappings_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [DomainMappingsClient, DomainMappingsAsyncClient])
...
def test_domain_mappings_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize(
    ...
    [
        (DomainMappingsClient, transports.DomainMappingsGrpcTransport, "grpc", grpc_helpers),
        (DomainMappingsAsyncClient, transports.DomainMappingsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (DomainMappingsClient, transports.DomainMappingsRestTransport, "rest", None),
    ],
)
def test_domain_mappings_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_domain_mappings_client_client_options_from_dict():
    with mock.patch("google.cloud.appengine_admin_v1.services.domain_mappings.transports.DomainMappingsGrpcTransport.__init__") as grpc_transport:
        grpc_transport.return_value = None
        client = DomainMappingsClient(client_options={"api_endpoint": "squid.clam.whelk"})
...
def test_domain_mappings_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_list_domain_mappings(request_type, transport: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_domain_mappings), "__call__") as call:
...
def test_list_domain_mappings_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.list_domain_mappings in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_domain_mappings] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_domain_mappings_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_domain_mappings in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_domain_mappings] = mock_rpc
...
async def test_list_domain_mappings_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_domain_mappings), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            appengine.ListDomainMappingsResponse(
                domain_mappings=[
...
async def test_list_domain_mappings_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_domain_mappings), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_domain_mappings(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_domain_mapping_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_domain_mapping in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_domain_mapping] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_domain_mapping_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DomainMappingsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_domain_mapping in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_domain_mapping] = mock_rpc
...
def test_create_domain_mapping_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_domain_mapping in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_domain_mapping] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_domain_mapping_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DomainMappingsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_domain_mapping in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_domain_mapping] = mock_rpc
...
def test_update_domain_mapping_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_domain_mapping in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_domain_mapping] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_domain_mapping_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DomainMappingsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_domain_mapping in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_domain_mapping] = mock_rpc
...
def test_delete_domain_mapping(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.delete_domain_mapping), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name="operations/spam")
...
def test_delete_domain_mapping_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_domain_mapping in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_domain_mapping] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_domain_mapping_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DomainMappingsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_domain_mapping in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_domain_mapping] = mock_rpc
...
def test_list_domain_mappings_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(appengine.ListDomainMappingsResponse.to_json(x) for x in response)
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = DomainMappingsClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = DomainMappingsClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = DomainMappingsAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = DomainMappingsAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = DomainMappingsClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_domain_mappings_rest_bad_request(request_type=appengine.ListDomainMappingsRequest):
    client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_domain_mappings_rest_interceptors(null_interceptor):
    transport = transports.DomainMappingsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.DomainMappingsRestInterceptor(),
    )
    client = DomainMappingsClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DomainMappingsRestInterceptor, "post_list_domain_mappings") as post, mock.patch.object(
        transports.DomainMappingsRestInterceptor, "post_list_domain_mappings_with_metadata"
        ...
        pb_message = appengine.ListDomainMappingsRequest.pb(appengine.ListDomainMappingsRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = appengine.ListDomainMappingsResponse.to_json(appengine.ListDomainMappingsResponse())
        ...
        post_with_metadata.return_value = appengine.ListDomainMappingsResponse(), metadata
...
def test_get_domain_mapping_rest_bad_request(request_type=appengine.GetDomainMappingRequest):
    client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_domain_mapping_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DomainMappingsRestInterceptor, "post_get_domain_mapping") as post, mock.patch.object(
        transports.DomainMappingsRestInterceptor, "post_get_domain_mapping_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = appengine.GetDomainMappingRequest.pb(appengine.GetDomainMappingRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = domain_mapping.DomainMapping.to_json(domain_mapping.DomainMapping())
        req.return_value.content = return_value
...
def test_create_domain_mapping_rest_bad_request(request_type=appengine.CreateDomainMappingRequest):
    client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "apps/sample1"}
    request = request_type(**request_init)
...
def test_create_domain_mapping_rest_call_success(request_type):
    ...
    request_init["domain_mapping"] = {
        ...
        "resource_records": [{"name": "name_value", "rrdata": "rrdata_value", "type_": 1}],
    }
...
def test_create_domain_mapping_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DomainMappingsRestInterceptor, "post_create_domain_mapping_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = appengine.CreateDomainMappingRequest.pb(appengine.CreateDomainMappingRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_domain_mapping_rest_bad_request(request_type=appengine.UpdateDomainMappingRequest):
    client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "apps/sample1/domainMappings/sample2"}
    request = request_type(**request_init)
...
def test_update_domain_mapping_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DomainMappingsRestInterceptor, "post_update_domain_mapping_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = appengine.UpdateDomainMappingRequest.pb(appengine.UpdateDomainMappingRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_domain_mapping_rest_bad_request(request_type=appengine.DeleteDomainMappingRequest):
    client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "apps/sample1/domainMappings/sample2"}
    request = request_type(**request_init)
...
def test_delete_domain_mapping_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DomainMappingsRestInterceptor, "post_delete_domain_mapping_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = appengine.DeleteDomainMappingRequest.pb(appengine.DeleteDomainMappingRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_domain_mappings_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.DomainMappingsTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_domain_mappings_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.appengine_admin_v1.services.domain_mappings.transports.DomainMappingsTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.DomainMappingsGrpcTransport, grpc_helpers), (transports.DomainMappingsGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
@pytest.mark.parametrize("transport_class", [transports.DomainMappingsGrpcTransport, transports.DomainMappingsGrpcAsyncIOTransport])
...
def test_domain_mappings_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.DomainMappingsRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_domain_mappings_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "appengine.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://appengine.googleapis.com:8000"
    )
...
@pytest.mark.parametrize("transport_class", [transports.DomainMappingsGrpcTransport, transports.DomainMappingsGrpcAsyncIOTransport])
def test_domain_mappings_transport_channel_mtls_with_client_cert_source(transport_class):
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.DomainMappingsTransport, "_prep_wrapped_messages") as prep:
        client = DomainMappingsClient(
            credentials=ga_credentials.AnonymousCredentials(),
...
def test_client_ctx():
    ...
    for transport in transports:
        client = DomainMappingsClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
