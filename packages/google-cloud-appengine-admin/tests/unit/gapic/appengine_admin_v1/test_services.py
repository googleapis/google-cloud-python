...
from google.cloud.appengine_admin_v1.services.services import ServicesAsyncClient, ServicesClient, pagers, transports
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = ServicesClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = ServicesClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = ServicesClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_list_services_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.list_services), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = appengine.ListServicesRequest()
...
def test_get_service_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.get_service), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = appengine.GetServiceRequest()
...
def test_update_service_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.update_service), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = appengine.UpdateServiceRequest()
...
def test_delete_service_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.delete_service), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = appengine.DeleteServiceRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = ServicesAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = ServicesAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = ServicesClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_services_rest_bad_request(request_type=appengine.ListServicesRequest):
    client = ServicesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.ServicesRestInterceptor, "post_list_services") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = appengine.ListServicesResponse.to_json(appengine.ListServicesResponse())
...
def test_get_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.ServicesRestInterceptor, "post_get_service") as post, mock.patch.object(
...
def test_services_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.ServicesTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.ServicesGrpcTransport, grpc_helpers), (transports.ServicesGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_services_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.ServicesRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_ctx():
    ...
    for transport in transports:
        client = ServicesClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
