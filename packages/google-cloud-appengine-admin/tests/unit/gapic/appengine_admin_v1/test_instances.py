...
from google.cloud.appengine_admin_v1.services.instances import InstancesAsyncClient, InstancesClient, pagers, transports
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert InstancesClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert InstancesClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert InstancesClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert InstancesClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert InstancesClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(InstancesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstancesClient))
@mock.patch.object(InstancesAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstancesAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = InstancesClient._DEFAULT_UNIVERSE
    default_endpoint = InstancesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = InstancesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert InstancesClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert InstancesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == InstancesClient.DEFAULT_MTLS_ENDPOINT
    assert InstancesClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert InstancesClient._get_api_endpoint(None, None, default_universe, "always") == InstancesClient.DEFAULT_MTLS_ENDPOINT
    assert InstancesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == InstancesClient.DEFAULT_MTLS_ENDPOINT
    assert InstancesClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert InstancesClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        InstancesClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert InstancesClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert InstancesClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert InstancesClient._get_universe_domain(None, None) == InstancesClient._DEFAULT_UNIVERSE
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = InstancesClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (InstancesAsyncClient, transports.InstancesGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_instances_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_debug_instance_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(type(client.transport.debug_instance), "__call__") as call:
        call.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client.debug_instance(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_debug_instance_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = InstancesClient(
        ...
        # Ensure method has been cached
        assert client._transport.debug_instance in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.debug_instance] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_debug_instance_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = InstancesAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.debug_instance in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.debug_instance] = mock_rpc
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = InstancesClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = InstancesClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = InstancesClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = InstancesAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = InstancesAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = InstancesClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_instances_rest_bad_request(request_type=appengine.ListInstancesRequest):
    client = InstancesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_instances_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.InstancesRestInterceptor, "post_list_instances") as post, mock.patch.object(
        transports.InstancesRestInterceptor, "post_list_instances_with_metadata"
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = appengine.ListInstancesResponse.to_json(appengine.ListInstancesResponse())
        ...
        post_with_metadata.return_value = appengine.ListInstancesResponse(), metadata
...
def test_get_instance_rest_bad_request(request_type=appengine.GetInstanceRequest):
    client = InstancesClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "apps/sample1/services/sample2/versions/sample3/instances/sample4"}
    request = request_type(**request_init)
...
def test_get_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.InstancesRestInterceptor, "post_get_instance") as post, mock.patch.object(
        transports.InstancesRestInterceptor, "post_get_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_delete_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.InstancesRestInterceptor, "post_delete_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = appengine.DeleteInstanceRequest.pb(appengine.DeleteInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_debug_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.InstancesRestInterceptor, "post_debug_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_instances_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.InstancesTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")


def test_instances_base_transport():
    # Instantiate the base transport.
    with mock.patch("google.cloud.appengine_admin_v1.services.instances.transports.InstancesTransport.__init__") as Transport:
...
@pytest.mark.parametrize(
    ...
    [(transports.InstancesGrpcTransport, grpc_helpers), (transports.InstancesGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_instances_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.InstancesRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_client_ctx():
    ...
    for transport in transports:
        client = InstancesClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
