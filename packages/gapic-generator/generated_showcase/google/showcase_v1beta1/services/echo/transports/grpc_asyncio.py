...
from .grpc import EchoGrpcTransport
...
class EchoGrpcAsyncIOTransport(EchoTransport):
    """gRPC AsyncIO backend transport for Echo.
    ...
    def echo(self) -> Callable[
            [gs_echo.EchoRequest],
            Awaitable[gs_echo.EchoResponse]]:
    ...
    def echo_error_details(self) -> Callable[
        ...
        Returns:
            Callable[[~.EchoErrorDetailsRequest],
                    Awaitable[~.EchoErrorDetailsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def paged_expand_legacy_mapped(self) -> Callable[
        ...
        Returns:
            Callable[[~.PagedExpandRequest],
                    Awaitable[~.PagedExpandLegacyMappedResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def block(self) -> Callable[
            ...
            Awaitable[gs_echo.BlockResponse]]:
