...
def test__get_default_mtls_endpoint():
    ...
    assert EchoClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(
        ...
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert EchoClient._read_environment_variables() == (
            False,
            "auto",
            None,
        )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert EchoClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert EchoClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                EchoClient._use_client_cert_effective()
...
def test__setup_request_id():
    ...
    request = MockRequest()
    EchoClient._setup_request_id(request, "request_id", True)
    ...
    EchoClient._setup_request_id(request, "request_id", False)
    assert re.match(r"[a-f0-9]{8}-?[a-f0-9]{4}-?4[a-f0-9]{3}-?[89ab][a-f0-9]{3}-?[a-f0-9]{12}", request.request_id)
