...
def test_cloud_redis_async_client_otel_channel_injection_enabled():
    mock_interceptor = mock.Mock()
    mock_obs = mock.Mock()
    ...
    with (
        ...
        mock.patch.object(
            transports.CloudRedisGrpcAsyncIOTransport, "__init__", return_value=None
        ) as patched_transport_init,
    ):
...
def test_cloud_redis_async_client_otel_channel_injection_disabled():
    mock_obs = mock.Mock()
...
def test_cloud_redis_grpc_asyncio_transport_custom_channel_interceptors():
    mock_interceptor = mock.Mock()
