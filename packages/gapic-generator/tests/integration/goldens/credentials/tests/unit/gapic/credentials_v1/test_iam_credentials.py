...
def test_iam_credentials_async_client_otel_channel_injection_enabled():
    mock_interceptor = mock.Mock()
    mock_obs = mock.Mock()
    ...
    with (
        ...
        mock.patch.object(
            transports.IAMCredentialsGrpcAsyncIOTransport, "__init__", return_value=None
        ) as patched_transport_init,
    ):
...
def test_iam_credentials_async_client_otel_channel_injection_disabled():
    mock_obs = mock.Mock()
...
def test_iam_credentials_grpc_asyncio_transport_custom_channel_interceptors():
    mock_interceptor = mock.Mock()
...
def test_iam_credentials_base_transport_wrap_method():
    mock_wrap = mock.Mock()
    with mock.patch("google.api_core.gapic_v1.method.wrap_method", mock_wrap):
        options = client_options.ClientOptions()
        with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.iam.credentials_v1.services.iam_credentials.transports.IAMCredentialsTransport._prep_wrapped_messages') as prep:
            adc.return_value = (ga_credentials.AnonymousCredentials(), None)
            transport = transports.IAMCredentialsTransport(client_options=options)
