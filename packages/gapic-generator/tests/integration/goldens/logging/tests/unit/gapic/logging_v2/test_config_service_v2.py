...
def test_config_service_v2_client_otel_channel_injection_enabled():
    mock_obs = mock.Mock()
    mock_obs.is_otel_capabilities_enabled.return_value = True
    with (
        mock.patch(
            "google.cloud.logging_v2.services.config_service_v2.client._observability",
            mock_obs,
        ),
        mock.patch.object(
...
def test_config_service_v2_client_otel_channel_injection_disabled():
    mock_obs = mock.Mock()
...
def test_config_service_v2_async_client_otel_channel_injection_enabled():
    mock_interceptor = mock.Mock()
    mock_obs = mock.Mock()
    ...
    with (
        ...
        mock.patch.object(
            transports.ConfigServiceV2GrpcAsyncIOTransport, "__init__", return_value=None
        ) as patched_transport_init,
    ):
...
def test_config_service_v2_async_client_otel_channel_injection_disabled():
    mock_obs = mock.Mock()
...
def test_config_service_v2_grpc_asyncio_transport_custom_channel_interceptors():
    mock_interceptor = mock.Mock()
...
def test_config_service_v2_base_transport_wrap_method():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method", mock_wrap):
        options = client_options.ClientOptions()
        with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.logging_v2.services.config_service_v2.transports.ConfigServiceV2Transport._prep_wrapped_messages') as prep:
            adc.return_value = (ga_credentials.AnonymousCredentials(), None)
            transport = transports.ConfigServiceV2Transport(client_options=options)
