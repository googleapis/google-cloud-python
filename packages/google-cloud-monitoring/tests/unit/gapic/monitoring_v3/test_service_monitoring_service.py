...
def test__get_default_mtls_endpoint():
    ...
    assert (
        ServiceMonitoringServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": ""}):
            with mock.patch.dict(os.environ, {"GOOGLE_API_CERTIFICATE_CONFIG": ""}):
                assert (
                    ServiceMonitoringServiceClient._use_client_cert_effective() is False
                )
...
def test_transport_kind_grpc_asyncio():
    transport = ServiceMonitoringServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
