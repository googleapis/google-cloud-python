...
def test__get_default_mtls_endpoint():
    ...
    assert (
        QueryServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        QueryServiceClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        QueryServiceClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                QueryServiceClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                QueryServiceClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(QueryServiceClient),
)
@mock.patch.object(
    QueryServiceAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(QueryServiceAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = QueryServiceClient._DEFAULT_UNIVERSE
    default_endpoint = QueryServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = QueryServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        QueryServiceClient._get_api_endpoint(
    ...
    assert (
        ...
        == QueryServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        QueryServiceClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        QueryServiceClient._get_api_endpoint(None, None, default_universe, "always")
        == QueryServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        QueryServiceClient._get_api_endpoint(
    ...
    assert (
        QueryServiceClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        QueryServiceClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        QueryServiceClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        QueryServiceClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        QueryServiceClient._get_universe_domain(None, None)
        == QueryServiceClient._DEFAULT_UNIVERSE
    )
...
def test_query_time_series_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[client._transport.query_time_series] = (
            mock_rpc
        )
        request = {}
...
@pytest.mark.asyncio
async def test_query_time_series_async_use_cached_wrapped_rpc(
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.query_time_series
...
async def test_query_time_series_async(request_type, transport: str = "grpc_asyncio"):
    client = QueryServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_transport_kind_grpc_asyncio():
    transport = QueryServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
