...
from google.cloud.recommender_v1beta1.types import insight, recommendation, recommender_service
...
class ListRecommendersAsyncPager:
    ...
    @property
    async def pages(self) -> AsyncIterator[recommender_service.ListRecommendersResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
...
class ListInsightTypesAsyncPager:
    ...
    @property
    async def pages(self) -> AsyncIterator[recommender_service.ListInsightTypesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
