...
class RecommenderClient(metaclass=RecommenderClientMeta):
    ...
    def parse_recommendation_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/recommenders/(?P<recommender>.+?)/recommendations/(?P<recommendation>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def get_mtls_endpoint_and_cert_source(cls, client_options: Optional[client_options_lib.ClientOptions] = None):
        ...
        use_client_cert = RecommenderClient._use_client_cert_effective()
        ...
        *,
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.recommender_v1beta1.RecommenderClient`.",
                    extra={
