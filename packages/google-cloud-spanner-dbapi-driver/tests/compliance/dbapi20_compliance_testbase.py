...
class DBAPI20ComplianceTestBase(unittest.TestCase):
    ...
    def _simple_queries(self, cur):
        ...
        _ = cur.fetchall()
        self.assertTrue(cur.rowcount in (-1, len(self.sql_factory.names_table1)))
    ...
    def _parametized_queries(self, cur):
        ...
        # DML
        cur.execute(
            self.sql_factory.stmt_dml_insert_table2("101, 'Moms Lasagna', 1, True, ''")
        )
        ...
        if self.driver.paramstyle == "qmark":
            ...
            cur.execute(
                self.sql_factory.stmt_dml_insert_table2(
                    "102, :item_name, 1, True, 'thi%%s :may ca%%(u)se? troub:1e'"
                ),
        ...
        self.assertEqual(
            ...
            "incorrectly. Got=%s, Expected=%s" % (repr(rows[0][4]), repr(trouble)),
        )
        self.assertEqual(
            rows[1][4],
            trouble,
            "cursor.fetchall retrieved incorrect data, or data inserted "
            "incorrectly. Got=%s, Expected=%s" % (repr(rows[1][4]), repr(trouble)),
        )
    ...
    def test_exceptions(self):
        ...
        self.assertTrue(issubclass(self.errors.InterfaceError, self.errors.Error))
        self.assertTrue(issubclass(self.errors.DatabaseError, self.errors.Error))
        self.assertTrue(issubclass(self.errors.DataError, self.errors.DatabaseError))
        self.assertTrue(
    ...
    def test_fetchmany(self):
        ...
        try:
            ...
            cur.execute(self.sql_factory.stmt_ddl_create_table2)
            cur.execute(self.sql_factory.stmt_dql_select_cols_table2("item_name"))
    ...
    def test_nextset(self):
        ...
        try:
            ...
            try:
                ...
                s = cur.nextset()
                self.assertEqual(s, True, "Has more return sets, should return True")
            finally:
    ...
    def test_no_nextset(self):
        ...
        try:
            ...
            try:
                ...
                self.assertEqual(len(rows), 1)
                s = cur.nextset()
                self.assertEqual(s, None, "No more return sets, should return None")
            finally:
                self.help_nextset_tearDown(cur)
