...
def test__get_api_endpoint():
    ...
    assert (
        TenantServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == TenantServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
async def test_list_tenants_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_tenants), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_tenants(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_transport_kind_grpc():
    transport = TenantServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = TenantServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = TenantServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = TenantServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_create_tenant_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.TenantServiceRestInterceptor, "post_create_tenant") as post, mock.patch.object(
...
def test_get_tenant_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TenantServiceRestInterceptor, "post_get_tenant") as post, mock.patch.object(
...
def test_update_tenant_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TenantServiceRestInterceptor, "post_update_tenant") as post, mock.patch.object(
...
def test_delete_tenant_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TenantServiceRestInterceptor, "pre_delete_tenant") as pre:
        pre.assert_not_called()
...
def test_list_tenants_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.TenantServiceRestInterceptor, "post_list_tenants") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.TenantServiceGrpcTransport, grpc_helpers), (transports.TenantServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
