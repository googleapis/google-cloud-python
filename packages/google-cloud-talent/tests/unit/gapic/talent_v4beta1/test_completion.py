...
def test__get_default_mtls_endpoint():
    ...
    assert (
        CompletionClient._get_default_mtls_endpoint(custom_endpoint) == custom_endpoint
    )
...
def test__get_client_cert_source():
    ...
    assert (
        CompletionClient._get_client_cert_source(mock_provided_cert_source, False)
        is None
    )
    assert (
        CompletionClient._get_client_cert_source(mock_provided_cert_source, True)
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                CompletionClient._get_client_cert_source(None, True)
                is mock_default_cert_source
            )
            assert (
                CompletionClient._get_client_cert_source(
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(CompletionClient),
)
@mock.patch.object(
    CompletionAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(CompletionAsyncClient),
)
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = CompletionClient._DEFAULT_UNIVERSE
    default_endpoint = CompletionClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=default_universe
    )
    mock_universe = "bar.com"
    mock_endpoint = CompletionClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        CompletionClient._get_api_endpoint(
    ...
    assert (
        ...
        == CompletionClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        CompletionClient._get_api_endpoint(None, None, default_universe, "auto")
        == default_endpoint
    )
    assert (
        CompletionClient._get_api_endpoint(None, None, default_universe, "always")
        == CompletionClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        CompletionClient._get_api_endpoint(
    ...
    assert (
        CompletionClient._get_api_endpoint(None, None, mock_universe, "never")
        == mock_endpoint
    )
    assert (
        CompletionClient._get_api_endpoint(None, None, default_universe, "never")
        == default_endpoint
    )
...
def test__get_universe_domain():
    ...
    assert (
        CompletionClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        CompletionClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        CompletionClient._get_universe_domain(None, None)
        == CompletionClient._DEFAULT_UNIVERSE
    )
...
def test_completion_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.talent_v4beta1.services.completion.transports.CompletionGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_completion_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_transport_kind_grpc_asyncio():
    transport = CompletionAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = CompletionClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_completion_base_transport():
    ...
    with mock.patch(
        "google.cloud.talent_v4beta1.services.completion.transports.CompletionTransport.__init__"
    ) as Transport:
