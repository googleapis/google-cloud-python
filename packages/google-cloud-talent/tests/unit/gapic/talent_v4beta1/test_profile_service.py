...
@pytest.mark.parametrize(
    ...
    [
        (ProfileServiceClient, "grpc"),
        (ProfileServiceAsyncClient, "grpc_asyncio"),
    ],
)
def test_profile_service_client_from_service_account_info(client_class, transport_name):
...
def test_profile_service_client_service_account_always_use_jwt(
    transport_class, transport_name
):
    with mock.patch.object(
...
def test_profile_service_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(
...
def test_profile_service_client_get_mtls_endpoint_and_cert_source(client_class):
...
def test_profile_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_transport_kind(transport_name):
    transport = ProfileServiceClient.get_transport_class(transport_name)(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
def test_profile_service_host_no_port(transport_name):
    client = ProfileServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_profile_service_host_with_port(transport_name):
    client = ProfileServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.parametrize(
    ...
    [
        (ProfileServiceClient, transports.ProfileServiceGrpcTransport),
        (ProfileServiceAsyncClient, transports.ProfileServiceGrpcAsyncIOTransport),
    ],
)
