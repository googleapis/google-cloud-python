...
TableSpec = _reflection.GeneratedProtocolMessageType(
    ...
    (_message.Message,),
    {
        ...
        "__doc__": """A specification of a relational table. The table’s schema is
  represented via its child column specs. It is pre-populated as part of
  ImportData by schema inference algorithm, the version of which is a
  required parameter of ImportData InputConfig. Note: While working with
  ...
  Attributes:
      ...
      time_column_spec_id:
          ...
          dataset’s ml_use_column_spec_id is not set. Used to split rows
          into TRAIN, VALIDATE and TEST sets such that oldest rows go to
          TRAIN set, newest to TEST, and those in between to VALIDATE.
          Required type: TIMESTAMP. If both this column and
          ml_use_column are not set, then ML use of all rows will be
