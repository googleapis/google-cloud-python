...
class SolarGrpcTransport(SolarTransport):
    """gRPC backend transport for Solar.
    ...
    def get_data_layers(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_data_layers" not in self._stubs:
            self._stubs["get_data_layers"] = self._logged_channel.unary_unary(
                "/google.maps.solar.v1.Solar/GetDataLayers",
                request_serializer=solar_service.GetDataLayersRequest.serialize,
                response_deserializer=solar_service.DataLayers.deserialize,
            )
    ...
    def get_geo_tiff(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_geo_tiff" not in self._stubs:
            self._stubs["get_geo_tiff"] = self._logged_channel.unary_unary(
                "/google.maps.solar.v1.Solar/GetGeoTiff",
                request_serializer=solar_service.GetGeoTiffRequest.serialize,
                response_deserializer=httpbody_pb2.HttpBody.FromString,
            )
        return self._stubs["get_geo_tiff"]
