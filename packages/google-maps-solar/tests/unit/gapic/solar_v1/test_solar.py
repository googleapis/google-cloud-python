...
from google.maps.solar_v1.services.solar import SolarAsyncClient, SolarClient, transports
...
def test_solar_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "solar.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://solar.googleapis.com"
        )
...
def test_find_closest_building_insights(request_type, transport: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.find_closest_building_insights), "__call__") as call:
...
def test_find_closest_building_insights_rest_required_fields(request_type=solar_service.FindClosestBuildingInsightsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).find_closest_building_insights._get_unset_required_fields(
        jsonified_request
    )
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = SolarClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = SolarClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = SolarClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
...
def test_transport_kind_grpc_asyncio():
    transport = SolarAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = SolarAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = SolarClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_find_closest_building_insights_rest_bad_request(request_type=solar_service.FindClosestBuildingInsightsRequest):
    client = SolarClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_find_closest_building_insights_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.SolarRestInterceptor, "post_find_closest_building_insights") as post, mock.patch.object(
        transports.SolarRestInterceptor, "post_find_closest_building_insights_with_metadata"
...
def test_get_data_layers_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SolarRestInterceptor, "post_get_data_layers") as post, mock.patch.object(
...
def test_get_geo_tiff_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.SolarRestInterceptor, "post_get_geo_tiff") as post, mock.patch.object(
...
def test_solar_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.SolarTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.SolarGrpcTransport, grpc_helpers), (transports.SolarGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_solar_http_transport_client_cert_source_for_mtls():
    ...
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.SolarRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_solar_host_no_port(transport_name):
    ...
    assert client.transport._host == ("solar.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://solar.googleapis.com")
...
def test_solar_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "solar.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://solar.googleapis.com:8000"
    )
...
def test_client_ctx():
    ...
    for transport in transports:
        client = SolarClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
