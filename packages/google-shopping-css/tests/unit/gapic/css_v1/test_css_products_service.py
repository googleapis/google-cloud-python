...
def test__get_default_mtls_endpoint():
    ...
    assert (
        CssProductsServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
...
@pytest.mark.asyncio
async def test_get_css_product_async_use_cached_wrapped_rpc(
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.get_css_product
...
def test_list_css_products_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[client._transport.list_css_products] = (
            mock_rpc
        )
        request = {}
...
@pytest.mark.asyncio
async def test_list_css_products_async_use_cached_wrapped_rpc(
    transport: str = "grpc_asyncio",
):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.list_css_products
            in client._client._transport._wrapped_methods
        )
...
async def test_list_css_products_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_css_products(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_css_product_rest_interceptors(null_interceptor):
    ...
    with (
        ...
        mock.patch.object(
            transports.CssProductsServiceRestInterceptor,
