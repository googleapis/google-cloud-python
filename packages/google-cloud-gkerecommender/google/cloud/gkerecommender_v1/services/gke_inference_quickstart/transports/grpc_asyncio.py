...
from .base import DEFAULT_CLIENT_INFO, GkeInferenceQuickstartTransport
...
class GkeInferenceQuickstartGrpcAsyncIOTransport(GkeInferenceQuickstartTransport):
    """gRPC AsyncIO backend transport for GkeInferenceQuickstart.
    ...
    @property
    def fetch_models(self) -> Callable[[gkerecommender.FetchModelsRequest], Awaitable[gkerecommender.FetchModelsResponse]]:
        ...
        Returns:
            Callable[[~.FetchModelsRequest],
                    Awaitable[~.FetchModelsResponse]]:
        ...
        if "fetch_models" not in self._stubs:
            self._stubs["fetch_models"] = self._logged_channel.unary_unary(
                "/google.cloud.gkerecommender.v1.GkeInferenceQuickstart/FetchModels",
                request_serializer=gkerecommender.FetchModelsRequest.serialize,
                response_deserializer=gkerecommender.FetchModelsResponse.deserialize,
            )
        return self._stubs["fetch_models"]

    @property
    def fetch_model_servers(self) -> Callable[[gkerecommender.FetchModelServersRequest], Awaitable[gkerecommender.FetchModelServersResponse]]:
        ...
        Returns:
            Callable[[~.FetchModelServersRequest],
                    Awaitable[~.FetchModelServersResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "fetch_model_servers" not in self._stubs:
            self._stubs["fetch_model_servers"] = self._logged_channel.unary_unary(
                "/google.cloud.gkerecommender.v1.GkeInferenceQuickstart/FetchModelServers",
                request_serializer=gkerecommender.FetchModelServersRequest.serialize,
                response_deserializer=gkerecommender.FetchModelServersResponse.deserialize,
            )
        return self._stubs["fetch_model_servers"]

    @property
    def fetch_model_server_versions(
        self,
    ) -> Callable[[gkerecommender.FetchModelServerVersionsRequest], Awaitable[gkerecommender.FetchModelServerVersionsResponse]]:
        ...
        Returns:
            Callable[[~.FetchModelServerVersionsRequest],
                    Awaitable[~.FetchModelServerVersionsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    @property
    def fetch_profiles(self) -> Callable[[gkerecommender.FetchProfilesRequest], Awaitable[gkerecommender.FetchProfilesResponse]]:
        ...
        Returns:
            Callable[[~.FetchProfilesRequest],
                    Awaitable[~.FetchProfilesResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "fetch_profiles" not in self._stubs:
            self._stubs["fetch_profiles"] = self._logged_channel.unary_unary(
                "/google.cloud.gkerecommender.v1.GkeInferenceQuickstart/FetchProfiles",
                request_serializer=gkerecommender.FetchProfilesRequest.serialize,
                response_deserializer=gkerecommender.FetchProfilesResponse.deserialize,
            )
        return self._stubs["fetch_profiles"]

    @property
    def generate_optimized_manifest(
        self,
    ) -> Callable[[gkerecommender.GenerateOptimizedManifestRequest], Awaitable[gkerecommender.GenerateOptimizedManifestResponse]]:
        ...
        Returns:
            Callable[[~.GenerateOptimizedManifestRequest],
                    Awaitable[~.GenerateOptimizedManifestResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def fetch_benchmarking_data(
        self,
    ) -> Callable[[gkerecommender.FetchBenchmarkingDataRequest], Awaitable[gkerecommender.FetchBenchmarkingDataResponse]]:
        ...
        Returns:
            Callable[[~.FetchBenchmarkingDataRequest],
                    Awaitable[~.FetchBenchmarkingDataResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "fetch_benchmarking_data" not in self._stubs:
            self._stubs["fetch_benchmarking_data"] = self._logged_channel.unary_unary(
                "/google.cloud.gkerecommender.v1.GkeInferenceQuickstart/FetchBenchmarkingData",
                request_serializer=gkerecommender.FetchBenchmarkingDataRequest.serialize,
                response_deserializer=gkerecommender.FetchBenchmarkingDataResponse.deserialize,
            )
        return self._stubs["fetch_benchmarking_data"]

    def _prep_wrapped_messages(self, client_info):
        """Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.fetch_models: self._wrap_method(
                self.fetch_models,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.fetch_model_servers: self._wrap_method(
                self.fetch_model_servers,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.fetch_model_server_versions: self._wrap_method(
                self.fetch_model_server_versions,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.fetch_profiles: self._wrap_method(
                self.fetch_profiles,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.generate_optimized_manifest: self._wrap_method(
                self.generate_optimized_manifest,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.fetch_benchmarking_data: self._wrap_method(
...
__all__ = ("GkeInferenceQuickstartGrpcAsyncIOTransport",)
