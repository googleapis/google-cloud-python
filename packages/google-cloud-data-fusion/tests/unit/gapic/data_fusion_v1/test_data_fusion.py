...
from google.cloud.data_fusion_v1.services.data_fusion import DataFusionAsyncClient, DataFusionClient, pagers, transports
...
def test__read_environment_variables():
    assert DataFusionClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert DataFusionClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            with pytest.raises(ValueError) as excinfo:
                DataFusionClient._read_environment_variables()
            assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"
        else:
            assert DataFusionClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert DataFusionClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert DataFusionClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert DataFusionClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert DataFusionClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert DataFusionClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                DataFusionClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert DataFusionClient._get_client_cert_source(None, False) is None
    assert DataFusionClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert DataFusionClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert DataFusionClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert DataFusionClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(DataFusionClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionClient))
@mock.patch.object(DataFusionAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DataFusionClient._DEFAULT_UNIVERSE
    default_endpoint = DataFusionClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = DataFusionClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert DataFusionClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert DataFusionClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DataFusionClient.DEFAULT_MTLS_ENDPOINT
    assert DataFusionClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert DataFusionClient._get_api_endpoint(None, None, default_universe, "always") == DataFusionClient.DEFAULT_MTLS_ENDPOINT
    assert DataFusionClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == DataFusionClient.DEFAULT_MTLS_ENDPOINT
    assert DataFusionClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert DataFusionClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        DataFusionClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert DataFusionClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert DataFusionClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert DataFusionClient._get_universe_domain(None, None) == DataFusionClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        DataFusionClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = DataFusionClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        (DataFusionClient, "grpc"),
        (DataFusionAsyncClient, "grpc_asyncio"),
        (DataFusionClient, "rest"),
    ],
)
def test_data_fusion_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "datafusion.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://datafusion.googleapis.com"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (transports.DataFusionRestTransport, "rest"),
    ],
)
def test_data_fusion_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_data_fusion_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DataFusionClient, transports.DataFusionRestTransport, "rest"),
    ],
)
@mock.patch.object(DataFusionClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionClient))
@mock.patch.object(DataFusionAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionAsyncClient))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DataFusionClient, transports.DataFusionRestTransport, "rest", "true"),
        (DataFusionClient, transports.DataFusionRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(DataFusionClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionClient))
@mock.patch.object(DataFusionAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionAsyncClient))
...
@mock.patch.object(DataFusionClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionClient))
@mock.patch.object(DataFusionAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DataFusionAsyncClient))
def test_data_fusion_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DataFusionAsyncClient, transports.DataFusionGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (DataFusionClient, transports.DataFusionRestTransport, "rest", None),
    ],
)
def test_data_fusion_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_data_fusion_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_list_available_versions(request_type, transport: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_available_versions), "__call__") as call:
...
def test_list_available_versions_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(type(client.transport.list_available_versions), "__call__") as call:
        ...
        client.list_available_versions(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_list_available_versions_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.list_available_versions in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_available_versions] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_available_versions_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_available_versions in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_available_versions] = mock_rpc
...
def test_list_available_versions_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.list_available_versions), "__call__") as call:
        ...
        pager = client.list_available_versions(request={}, retry=retry, timeout=timeout)
...
async def test_list_available_versions_async_pager():
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(type(client.transport.list_available_versions), "__call__", new_callable=mock.AsyncMock) as call:
        # Set the response to a series of pages.
        call.side_effect = (
            datafusion.ListAvailableVersionsResponse(
                available_versions=[
...
async def test_list_available_versions_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_available_versions), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_available_versions(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_get_instance(request_type, transport: str = "grpc"):
    ...
    assert response.disabled_reason == [datafusion.Instance.DisabledReason.KMS_KEY_ISSUE]
...
def test_restart_instance_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.restart_instance] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_restart_instance_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DataFusionAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.restart_instance in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.restart_instance] = mock_rpc
...
def test_list_available_versions_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = DataFusionClient(
...
def test_list_available_versions_rest_required_fields(request_type=datafusion.ListAvailableVersionsRequest):
    transport_class = transports.DataFusionRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_available_versions._get_unset_required_fields(
        jsonified_request
    )
    ...
    return_value = datafusion.ListAvailableVersionsResponse()
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            return_value = datafusion.ListAvailableVersionsResponse.pb(return_value)
...
def test_list_available_versions_rest_unset_required_fields():
    transport = transports.DataFusionRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.list_available_versions._get_unset_required_fields({})
...
def test_list_available_versions_rest_flattened():
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        client.list_available_versions(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=projects/*/locations/*}/versions" % client.transport._host, args[1])


def test_list_available_versions_rest_flattened_error(transport: str = "rest"):
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_available_versions_rest_pager(transport: str = "rest"):
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        response = tuple(datafusion.ListAvailableVersionsResponse.to_json(x) for x in response)
        ...
        pager = client.list_available_versions(request=sample_request)

        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, datafusion.Version) for i in results)

        pages = list(client.list_available_versions(request=sample_request).pages)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_instances_rest_required_fields(request_type=datafusion.ListInstancesRequest):
    transport_class = transports.DataFusionRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = datafusion.ListInstancesResponse()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, "request") as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = datafusion.ListInstancesResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_list_instances_rest_pager(transport: str = "rest"):
    ...
    with mock.patch.object(Session, "request") as req:
        ...
        # Wrap the values into proper Response objs
        response = tuple(datafusion.ListInstancesResponse.to_json(x) for x in response)
        return_values = tuple(Response() for i in response)
        for return_val, response_val in zip(return_values, response):
            return_val._content = response_val.encode("UTF-8")
            return_val.status_code = 200
...
def test_get_instance_rest_required_fields(request_type=datafusion.GetInstanceRequest):
    transport_class = transports.DataFusionRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = datafusion.Instance()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, "request") as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, "transcode") as transcode:
            ...
            # Convert return value to protobuf type
            return_value = datafusion.Instance.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_create_instance_rest_required_fields(request_type=datafusion.CreateInstanceRequest):
    transport_class = transports.DataFusionRestTransport
...
def test_delete_instance_rest_required_fields(request_type=datafusion.DeleteInstanceRequest):
    transport_class = transports.DataFusionRestTransport
...
def test_update_instance_rest_required_fields(request_type=datafusion.UpdateInstanceRequest):
    transport_class = transports.DataFusionRestTransport
...
def test_restart_instance_rest_required_fields(request_type=datafusion.RestartInstanceRequest):
    transport_class = transports.DataFusionRestTransport
...
def test_credentials_transport_error():
    ...
    options.api_key = "api_key"
    with pytest.raises(ValueError):
        client = DataFusionClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc():
    transport = DataFusionClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "grpc"


def test_initialize_client_w_grpc():
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="grpc")
    assert client is not None


# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_available_versions_empty_call_grpc():
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.list_available_versions), "__call__") as call:
        call.return_value = datafusion.ListAvailableVersionsResponse()
        client.list_available_versions(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.ListAvailableVersionsRequest()
...
def test_list_instances_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.list_instances), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.ListInstancesRequest()
...
def test_get_instance_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.get_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.GetInstanceRequest()
...
def test_create_instance_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.create_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.CreateInstanceRequest()
...
def test_delete_instance_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.delete_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.DeleteInstanceRequest()
...
def test_update_instance_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.update_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.UpdateInstanceRequest()
...
def test_restart_instance_empty_call_grpc():
    ...
    with mock.patch.object(type(client.transport.restart_instance), "__call__") as call:
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = datafusion.RestartInstanceRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = DataFusionAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = DataFusionAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_available_versions_empty_call_grpc_asyncio():
    client = DataFusionAsyncClient(
        credentials=async_anonymous_credentials(),
    ...
    with mock.patch.object(type(client.transport.list_available_versions), "__call__") as call:
        ...
        await client.list_available_versions(request=None)
...
def test_transport_kind_rest():
    transport = DataFusionClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_list_available_versions_rest_bad_request(request_type=datafusion.ListAvailableVersionsRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_available_versions_rest_call_success(request_type):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_available_versions_rest_interceptors(null_interceptor):
    transport = transports.DataFusionRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.DataFusionRestInterceptor(),
    )
    client = DataFusionClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DataFusionRestInterceptor, "post_list_available_versions") as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_list_available_versions_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_list_available_versions"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.ListAvailableVersionsRequest.pb(datafusion.ListAvailableVersionsRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = datafusion.ListAvailableVersionsResponse.to_json(datafusion.ListAvailableVersionsResponse())
        ...
        post.return_value = datafusion.ListAvailableVersionsResponse()
        post_with_metadata.return_value = datafusion.ListAvailableVersionsResponse(), metadata
...
def test_list_instances_rest_bad_request(request_type=datafusion.ListInstancesRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_list_instances_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DataFusionRestInterceptor, "post_list_instances") as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_list_instances_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_list_instances"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.ListInstancesRequest.pb(datafusion.ListInstancesRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = datafusion.ListInstancesResponse.to_json(datafusion.ListInstancesResponse())
        req.return_value.content = return_value
        ...
        pre.return_value = request, metadata
        post.return_value = datafusion.ListInstancesResponse()
        post_with_metadata.return_value = datafusion.ListInstancesResponse(), metadata
...
def test_get_instance_rest_bad_request(request_type=datafusion.GetInstanceRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_get_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.DataFusionRestInterceptor, "post_get_instance") as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_get_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_get_instance"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.GetInstanceRequest.pb(datafusion.GetInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = datafusion.Instance.to_json(datafusion.Instance())
        req.return_value.content = return_value
        ...
        pre.return_value = request, metadata
        post.return_value = datafusion.Instance()
        post_with_metadata.return_value = datafusion.Instance(), metadata
...
def test_create_instance_rest_bad_request(request_type=datafusion.CreateInstanceRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"parent": "projects/sample1/locations/sample2"}
    request = request_type(**request_init)
...
def test_create_instance_rest_call_success(request_type):
    ...
    request_init["instance"] = {
        ...
        "enable_stackdriver_monitoring": True,
        "private_instance": True,
        "network_config": {"network": "network_value", "ip_allocation": "ip_allocation_value"},
        "labels": {},
        "options": {},
        ...
        "available_version": [
            {
                ...
                "available_features": ["available_features_value1", "available_features_value2"],
                "type_": 1,
            }
        ],
        "api_endpoint": "api_endpoint_value",
        "gcs_bucket": "gcs_bucket_value",
        "accelerators": [{"accelerator_type": 1, "state": 1}],
        "p4_service_account": "p4_service_account_value",
        "tenant_project_id": "tenant_project_id_value",
        "dataproc_service_account": "dataproc_service_account_value",
        "enable_rbac": True,
    ...
    test_field = datafusion.CreateInstanceRequest.meta.fields["instance"]
...
def test_create_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DataFusionRestInterceptor, "post_create_instance"
    ) as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_create_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_create_instance"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.CreateInstanceRequest.pb(datafusion.CreateInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_instance_rest_bad_request(request_type=datafusion.DeleteInstanceRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/instances/sample3"}
    request = request_type(**request_init)
...
def test_delete_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DataFusionRestInterceptor, "post_delete_instance"
    ) as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_delete_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_delete_instance"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.DeleteInstanceRequest.pb(datafusion.DeleteInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_instance_rest_bad_request(request_type=datafusion.UpdateInstanceRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_update_instance_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = datafusion.UpdateInstanceRequest.meta.fields["instance"]
...
def test_update_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DataFusionRestInterceptor, "post_update_instance"
    ) as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_update_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_update_instance"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.UpdateInstanceRequest.pb(datafusion.UpdateInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_restart_instance_rest_bad_request(request_type=datafusion.RestartInstanceRequest):
    client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
    request_init = {"name": "projects/sample1/locations/sample2/instances/sample3"}
    request = request_type(**request_init)
...
def test_restart_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.DataFusionRestInterceptor, "post_restart_instance"
    ) as post, mock.patch.object(
        transports.DataFusionRestInterceptor, "post_restart_instance_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.DataFusionRestInterceptor, "pre_restart_instance"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = datafusion.RestartInstanceRequest.pb(datafusion.RestartInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_available_versions_empty_call_rest():
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_data_fusion_rest_lro_client():
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_data_fusion_base_transport_error():
    ...
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.DataFusionTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file="credentials.json")
...
@pytest.mark.parametrize(
    ...
    [(transports.DataFusionGrpcTransport, grpc_helpers), (transports.DataFusionGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_data_fusion_http_transport_client_cert_source_for_mtls():
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch("google.auth.transport.requests.AuthorizedSession.configure_mtls_channel") as mock_configure_mtls_channel:
        transports.DataFusionRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
...
def test_data_fusion_host_no_port(transport_name):
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_data_fusion_host_with_port(transport_name):
    client = DataFusionClient(
        credentials=ga_credentials.AnonymousCredentials(),
    ...
    assert client.transport._host == (
        "datafusion.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://datafusion.googleapis.com:8000"
    )
...
def test_data_fusion_client_transport_session_collision(transport_name):
    creds1 = ga_credentials.AnonymousCredentials()
    creds2 = ga_credentials.AnonymousCredentials()
    client1 = DataFusionClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = DataFusionClient(
        credentials=creds2,
        transport=transport_name,
    )
    session1 = client1.transport.list_available_versions._session
    session2 = client2.transport.list_available_versions._session
...
def test_client_ctx():
    ...
    for transport in transports:
        client = DataFusionClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
