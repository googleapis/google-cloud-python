...
from google.cloud.apigeeconnect_v1.services.tether.transports.base import (
...
@pytest.mark.parametrize("client_class", [TetherClient, TetherAsyncClient,])
def test_tether_client_service_account_always_use_jwt(client_class):
    with mock.patch.object(
...
def test_tether_client_service_account_always_use_jwt_true(
    transport_class, transport_name
):
    with mock.patch.object(
        service_account.Credentials, "with_always_use_jwt_access", create=True
...
def test_egress(transport: str = "grpc", request_type=tether.EgressResponse):
    client = TetherClient(
...
def test_egress_from_dict():
    test_egress(request_type=dict)
...
def test_tether_host_with_port():
    client = TetherClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(
