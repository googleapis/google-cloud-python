...
def test__read_environment_variables():
    assert FoldersClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert FoldersClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert FoldersClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert FoldersClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert FoldersClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert FoldersClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert FoldersClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert FoldersClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                FoldersClient._use_client_cert_effective()
...
def test__get_client_cert_source():
    ...
    assert FoldersClient._get_client_cert_source(None, False) is None
    assert FoldersClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert FoldersClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert FoldersClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert FoldersClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(FoldersClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(FoldersClient))
@mock.patch.object(FoldersAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(FoldersAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = FoldersClient._DEFAULT_UNIVERSE
    default_endpoint = FoldersClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = FoldersClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert FoldersClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert FoldersClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == FoldersClient.DEFAULT_MTLS_ENDPOINT
    assert FoldersClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert FoldersClient._get_api_endpoint(None, None, default_universe, "always") == FoldersClient.DEFAULT_MTLS_ENDPOINT
    assert FoldersClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == FoldersClient.DEFAULT_MTLS_ENDPOINT
    assert FoldersClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert FoldersClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        FoldersClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert FoldersClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert FoldersClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert FoldersClient._get_universe_domain(None, None) == FoldersClient._DEFAULT_UNIVERSE
...
@mock.patch.object(FoldersClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(FoldersClient))
@mock.patch.object(FoldersAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(FoldersAsyncClient))
def test_folders_client_client_api_endpoint(client_class):
...
def test_search_folders_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_search_folders_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.search_folders in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.search_folders] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_search_folders_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = FoldersAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.search_folders in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.search_folders] = mock_rpc
...
def test_search_folders_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.search_folders), "__call__") as call:
        ...
        retry = retries.Retry()
        timeout = 5
        pager = client.search_folders(request={}, retry=retry, timeout=timeout)
...
def test_update_folder_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_update_folder_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_folder in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_folder] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_folder_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = FoldersAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_folder in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_folder] = mock_rpc
...
def test_move_folder_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_move_folder_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.move_folder in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.move_folder] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_move_folder_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = FoldersAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.move_folder in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.move_folder] = mock_rpc
...
def test_undelete_folder_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_undelete_folder_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.undelete_folder in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.undelete_folder] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_undelete_folder_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = FoldersAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.undelete_folder in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.undelete_folder] = mock_rpc
...
def test_get_folder_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_list_folders_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_search_folders_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_create_folder_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_update_folder_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_move_folder_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_delete_folder_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_undelete_folder_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = FoldersClient(
...
def test_transport_kind_grpc():
    transport = FoldersClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_search_folders_empty_call_grpc():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.search_folders), "__call__") as call:
        call.return_value = folders.SearchFoldersResponse()
        client.search_folders(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_folder_empty_call_grpc():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.update_folder), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.update_folder(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_move_folder_empty_call_grpc():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.move_folder), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.move_folder(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_undelete_folder_empty_call_grpc():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.undelete_folder), "__call__") as call:
        call.return_value = operations_pb2.Operation(name="operations/op")
        client.undelete_folder(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = FoldersAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = FoldersAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_search_folders_empty_call_grpc_asyncio():
    client = FoldersAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.search_folders), "__call__") as call:
        ...
        await client.search_folders(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_update_folder_empty_call_grpc_asyncio():
    client = FoldersAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.update_folder), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.update_folder(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_move_folder_empty_call_grpc_asyncio():
    client = FoldersAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.move_folder), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.move_folder(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_undelete_folder_empty_call_grpc_asyncio():
    client = FoldersAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.undelete_folder), "__call__") as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name="operations/spam"))
        await client.undelete_folder(request=None)
...
def test_transport_kind_rest():
    transport = FoldersClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_get_folder_rest_bad_request(request_type=folders.GetFolderRequest):
...
def test_get_folder_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_folder_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
    ...
    ) as transcode, mock.patch.object(transports.FoldersRestInterceptor, "post_get_folder") as post, mock.patch.object(
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = folders.Folder.to_json(folders.Folder())
...
def test_list_folders_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_list_folders_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FoldersRestInterceptor, "post_list_folders") as post, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = folders.ListFoldersResponse.to_json(folders.ListFoldersResponse())
        req.return_value.content = return_value
...
def test_search_folders_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_search_folders_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FoldersRestInterceptor, "post_search_folders") as post, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = folders.SearchFoldersResponse.to_json(folders.SearchFoldersResponse())
        req.return_value.content = return_value
...
def test_create_folder_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    ...
    for field, value in request_init["folder"].items():  # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_update_folder_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")

    # send a request that will satisfy transcoding
    request_init = {"folder": {"name": "folders/sample1"}}
    request_init["folder"] = {
        "name": "folders/sample1",
        "parent": "parent_value",
        "display_name": "display_name_value",
        "state": 1,
        "create_time": {"seconds": 751, "nanos": 543},
...
def test_move_folder_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_delete_folder_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_undelete_folder_rest_call_success(request_type):
    client = FoldersClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FoldersRestInterceptor, "post_get_iam_policy") as post, mock.patch.object(
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FoldersRestInterceptor, "post_set_iam_policy") as post, mock.patch.object(
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.FoldersRestInterceptor, "post_test_iam_permissions") as post, mock.patch.object(
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_search_folders_empty_call_rest():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_update_folder_empty_call_rest():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_move_folder_empty_call_rest():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_undelete_folder_empty_call_rest():
    client = FoldersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
@pytest.mark.parametrize(
    ...
    [(transports.FoldersGrpcTransport, grpc_helpers), (transports.FoldersGrpcAsyncIOTransport, grpc_helpers_async)],
)
