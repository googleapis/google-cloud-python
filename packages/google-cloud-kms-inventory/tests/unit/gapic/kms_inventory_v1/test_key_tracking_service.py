...
def test__read_environment_variables():
    assert KeyTrackingServiceClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert KeyTrackingServiceClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert KeyTrackingServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert KeyTrackingServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert KeyTrackingServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert KeyTrackingServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert KeyTrackingServiceClient._use_client_cert_effective() is False
...
def test__get_client_cert_source():
    ...
    assert KeyTrackingServiceClient._get_client_cert_source(None, False) is None
    assert KeyTrackingServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert KeyTrackingServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch("google.auth.transport.mtls.has_default_client_cert_source", return_value=True):
        with mock.patch("google.auth.transport.mtls.default_client_cert_source", return_value=mock_default_cert_source):
            assert KeyTrackingServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert KeyTrackingServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source


@mock.patch.object(KeyTrackingServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceClient))
@mock.patch.object(KeyTrackingServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = KeyTrackingServiceClient._DEFAULT_UNIVERSE
    default_endpoint = KeyTrackingServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = KeyTrackingServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert KeyTrackingServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert (
        KeyTrackingServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
        == KeyTrackingServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert KeyTrackingServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert KeyTrackingServiceClient._get_api_endpoint(None, None, default_universe, "always") == KeyTrackingServiceClient.DEFAULT_MTLS_ENDPOINT
    assert (
        KeyTrackingServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == KeyTrackingServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert KeyTrackingServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert KeyTrackingServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        KeyTrackingServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert KeyTrackingServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert KeyTrackingServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert KeyTrackingServiceClient._get_universe_domain(None, None) == KeyTrackingServiceClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        KeyTrackingServiceClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = KeyTrackingServiceClient(credentials=cred)
...
@pytest.mark.parametrize(
    ...
    [
        (KeyTrackingServiceClient, "grpc"),
        (KeyTrackingServiceAsyncClient, "grpc_asyncio"),
        (KeyTrackingServiceClient, "rest"),
    ],
)
def test_key_tracking_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "kmsinventory.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://kmsinventory.googleapis.com"
        )
...
@pytest.mark.parametrize(
    ...
    [
        (transports.KeyTrackingServiceGrpcTransport, "grpc"),
        (transports.KeyTrackingServiceGrpcAsyncIOTransport, "grpc_asyncio"),
...
def test_key_tracking_service_client_get_transport_class():
    ...
    available_transports = [
        transports.KeyTrackingServiceGrpcTransport,
        transports.KeyTrackingServiceRestTransport,
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (KeyTrackingServiceClient, transports.KeyTrackingServiceGrpcTransport, "grpc"),
        (KeyTrackingServiceAsyncClient, transports.KeyTrackingServiceGrpcAsyncIOTransport, "grpc_asyncio"),
        (KeyTrackingServiceClient, transports.KeyTrackingServiceRestTransport, "rest"),
    ],
)
@mock.patch.object(KeyTrackingServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceClient))
@mock.patch.object(KeyTrackingServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceAsyncClient))
...
@pytest.mark.parametrize(
    ...
    [
        (KeyTrackingServiceClient, transports.KeyTrackingServiceGrpcTransport, "grpc", "true"),
        (KeyTrackingServiceAsyncClient, transports.KeyTrackingServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
        (KeyTrackingServiceClient, transports.KeyTrackingServiceGrpcTransport, "grpc", "false"),
        (KeyTrackingServiceAsyncClient, transports.KeyTrackingServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
        (KeyTrackingServiceClient, transports.KeyTrackingServiceRestTransport, "rest", "true"),
        (KeyTrackingServiceClient, transports.KeyTrackingServiceRestTransport, "rest", "false"),
    ],
)
@mock.patch.object(KeyTrackingServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceClient))
@mock.patch.object(KeyTrackingServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceAsyncClient))
...
@mock.patch.object(KeyTrackingServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceClient))
@mock.patch.object(KeyTrackingServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(KeyTrackingServiceAsyncClient))
def test_key_tracking_service_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize(
    ...
    [
        (KeyTrackingServiceClient, transports.KeyTrackingServiceGrpcTransport, "grpc", grpc_helpers),
        (KeyTrackingServiceAsyncClient, transports.KeyTrackingServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
        (KeyTrackingServiceClient, transports.KeyTrackingServiceRestTransport, "rest", None),
    ],
)
...
def test_key_tracking_service_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.kms_inventory_v1.services.key_tracking_service.transports.KeyTrackingServiceGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_get_protected_resources_summary_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.get_protected_resources_summary in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_protected_resources_summary] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_protected_resources_summary_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.get_protected_resources_summary in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_protected_resources_summary] = mock_rpc
...
def test_search_protected_resources_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.search_protected_resources in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo"  # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.search_protected_resources] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_search_protected_resources_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = KeyTrackingServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.search_protected_resources in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.search_protected_resources] = mock_rpc
...
async def test_search_protected_resources_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.search_protected_resources(
            key_tracking_service.SearchProtectedResourcesRequest(),
            scope="scope_value",
            crypto_key="crypto_key_value",
        )
...
def test_search_protected_resources_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(type(client.transport.search_protected_resources), "__call__") as call:
        ...
        retry = retries.Retry()
        timeout = 5
        expected_metadata = tuple(expected_metadata) + (gapic_v1.routing_header.to_grpc_metadata((("scope", ""),)),)
        pager = client.search_protected_resources(request={}, retry=retry, timeout=timeout)
...
async def test_search_protected_resources_async_pager():
    ...
    with mock.patch.object(type(client.transport.search_protected_resources), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async_pager = await client.search_protected_resources(
            request={},
        )
...
async def test_search_protected_resources_async_pages():
    ...
    with mock.patch.object(type(client.transport.search_protected_resources), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.search_protected_resources(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token


def test_get_protected_resources_summary_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = KeyTrackingServiceClient(
...
def test_get_protected_resources_summary_rest_required_fields(request_type=key_tracking_service.GetProtectedResourcesSummaryRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_protected_resources_summary._get_unset_required_fields(
        jsonified_request
    )
...
def test_get_protected_resources_summary_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1/{name=projects/*/locations/*/keyRings/*/cryptoKeys/**}/protectedResourcesSummary" % client.transport._host, args[1]
        )
...
def test_search_protected_resources_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = KeyTrackingServiceClient(
...
def test_search_protected_resources_rest_required_fields(request_type=key_tracking_service.SearchProtectedResourcesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).search_protected_resources._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_credentials_transport_error():
    ...
    transport = transports.KeyTrackingServiceGrpcTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    with pytest.raises(ValueError):
        client = KeyTrackingServiceClient(
...
def test_transport_get_channel():
    ...
    transport = transports.KeyTrackingServiceGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
@pytest.mark.parametrize(
    ...
    [
        transports.KeyTrackingServiceGrpcTransport,
        transports.KeyTrackingServiceGrpcAsyncIOTransport,
        transports.KeyTrackingServiceRestTransport,
    ],
)
...
def test_transport_kind_grpc():
    transport = KeyTrackingServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_get_protected_resources_summary_empty_call_grpc():
    client = KeyTrackingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.get_protected_resources_summary), "__call__") as call:
        call.return_value = key_tracking_service.ProtectedResourcesSummary()
        client.get_protected_resources_summary(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_search_protected_resources_empty_call_grpc():
    client = KeyTrackingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(type(client.transport.search_protected_resources), "__call__") as call:
        call.return_value = key_tracking_service.SearchProtectedResourcesResponse()
        client.search_protected_resources(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = KeyTrackingServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = KeyTrackingServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_protected_resources_summary_empty_call_grpc_asyncio():
    client = KeyTrackingServiceAsyncClient(
        credentials=async_anonymous_credentials(),
    ...
    with mock.patch.object(type(client.transport.get_protected_resources_summary), "__call__") as call:
        ...
        await client.get_protected_resources_summary(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_search_protected_resources_empty_call_grpc_asyncio():
    client = KeyTrackingServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(type(client.transport.search_protected_resources), "__call__") as call:
        ...
        await client.search_protected_resources(request=None)
...
def test_transport_kind_rest():
    transport = KeyTrackingServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == "rest"


def test_get_protected_resources_summary_rest_bad_request(request_type=key_tracking_service.GetProtectedResourcesSummaryRequest):
...
def test_get_protected_resources_summary_rest_call_success(request_type):
    client = KeyTrackingServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_get_protected_resources_summary_rest_interceptors(null_interceptor):
    transport = transports.KeyTrackingServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.KeyTrackingServiceRestInterceptor(),
    )
    client = KeyTrackingServiceClient(transport=transport)
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        ...
        transports.KeyTrackingServiceRestInterceptor, "post_get_protected_resources_summary"
    ) as post, mock.patch.object(
        transports.KeyTrackingServiceRestInterceptor, "post_get_protected_resources_summary_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.KeyTrackingServiceRestInterceptor, "pre_get_protected_resources_summary"
    ) as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = key_tracking_service.ProtectedResourcesSummary.to_json(key_tracking_service.ProtectedResourcesSummary())
        ...
        post_with_metadata.return_value = key_tracking_service.ProtectedResourcesSummary(), metadata
...
def test_search_protected_resources_rest_bad_request(request_type=key_tracking_service.SearchProtectedResourcesRequest):
    client = KeyTrackingServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
    # send a request that will satisfy transcoding
...
def test_search_protected_resources_rest_call_success(request_type):
    client = KeyTrackingServiceClient(credentials=ga_credentials.AnonymousCredentials(), transport="rest")
...
def test_search_protected_resources_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.KeyTrackingServiceRestInterceptor, "post_search_protected_resources") as post, mock.patch.object(
        transports.KeyTrackingServiceRestInterceptor, "post_search_protected_resources_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        transports.KeyTrackingServiceRestInterceptor, "pre_search_protected_resources"
    ) as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = key_tracking_service.SearchProtectedResourcesResponse.to_json(key_tracking_service.SearchProtectedResourcesResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = key_tracking_service.SearchProtectedResourcesResponse(), metadata
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_protected_resources_summary_empty_call_rest():
    client = KeyTrackingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_key_tracking_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, "load_credentials_from_file", autospec=True) as load_creds, mock.patch(
        "google.cloud.kms_inventory_v1.services.key_tracking_service.transports.KeyTrackingServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
@pytest.mark.parametrize(
    ...
    [(transports.KeyTrackingServiceGrpcTransport, grpc_helpers), (transports.KeyTrackingServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_key_tracking_service_host_no_port(transport_name):
    client = KeyTrackingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="kmsinventory.googleapis.com"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "kmsinventory.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://kmsinventory.googleapis.com"
...
def test_key_tracking_service_host_with_port(transport_name):
    client = KeyTrackingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint="kmsinventory.googleapis.com:8000"),
        transport=transport_name,
    )
    assert client.transport._host == (
        "kmsinventory.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://kmsinventory.googleapis.com:8000"
    )
...
def test_key_tracking_service_client_transport_session_collision(transport_name):
    ...
    client1 = KeyTrackingServiceClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = KeyTrackingServiceClient(
        credentials=creds2,
        transport=transport_name,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (KeyTrackingServiceClient, transports.KeyTrackingServiceGrpcTransport),
        (KeyTrackingServiceAsyncClient, transports.KeyTrackingServiceGrpcAsyncIOTransport),
    ],
)
