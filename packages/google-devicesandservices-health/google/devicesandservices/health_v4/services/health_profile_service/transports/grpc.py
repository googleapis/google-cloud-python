...
from .base import DEFAULT_CLIENT_INFO, HealthProfileServiceTransport
...
class HealthProfileServiceGrpcTransport(HealthProfileServiceTransport):
    """gRPC backend transport for HealthProfileService.
    ...
    def get_profile(
        ...
        if "get_profile" not in self._stubs:
            self._stubs["get_profile"] = self._logged_channel.unary_unary(
                "/google.devicesandservices.health.v4.HealthProfileService/GetProfile",
                request_serializer=health_profile.GetProfileRequest.serialize,
                response_deserializer=health_profile.Profile.deserialize,
            )
    ...
    def update_profile(
        ...
        if "update_profile" not in self._stubs:
            self._stubs["update_profile"] = self._logged_channel.unary_unary(
                "/google.devicesandservices.health.v4.HealthProfileService/UpdateProfile",
                request_serializer=health_profile.UpdateProfileRequest.serialize,
                response_deserializer=health_profile.Profile.deserialize,
            )
    ...
    def get_settings(
        ...
        if "get_settings" not in self._stubs:
            self._stubs["get_settings"] = self._logged_channel.unary_unary(
                ...
                request_serializer=health_profile.GetSettingsRequest.serialize,
                response_deserializer=health_profile.Settings.deserialize,
            )
    ...
    def update_settings(
        ...
        if "update_settings" not in self._stubs:
            self._stubs["update_settings"] = self._logged_channel.unary_unary(
                ...
                request_serializer=health_profile.UpdateSettingsRequest.serialize,
                response_deserializer=health_profile.Settings.deserialize,
            )
    ...
    def get_identity(
        ...
        r"""Return a callable for the get identity method over gRPC.
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_identity" not in self._stubs:
            self._stubs["get_identity"] = self._logged_channel.unary_unary(
                "/google.devicesandservices.health.v4.HealthProfileService/GetIdentity",
                request_serializer=health_profile.GetIdentityRequest.serialize,
                response_deserializer=health_profile.Identity.deserialize,
            )
        return self._stubs["get_identity"]

    @property
    def get_irn_profile(
        self,
    ) -> Callable[[health_profile.GetIrnProfileRequest], health_profile.IrnProfile]:
        r"""Return a callable for the get irn profile method over gRPC.

        Returns user's IRN Profile details.

        Returns:
            Callable[[~.GetIrnProfileRequest],
                    ~.IrnProfile]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_irn_profile" not in self._stubs:
            self._stubs["get_irn_profile"] = self._logged_channel.unary_unary(
                "/google.devicesandservices.health.v4.HealthProfileService/GetIrnProfile",
                request_serializer=health_profile.GetIrnProfileRequest.serialize,
                response_deserializer=health_profile.IrnProfile.deserialize,
            )
        return self._stubs["get_irn_profile"]

    @property
    def get_paired_device(
        self,
    ) -> Callable[[health_profile.GetPairedDeviceRequest], health_profile.PairedDevice]:
        r"""Return a callable for the get paired device method over gRPC.

        Returns user's Device.

        Returns:
            Callable[[~.GetPairedDeviceRequest],
                    ~.PairedDevice]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_paired_device" not in self._stubs:
            self._stubs["get_paired_device"] = self._logged_channel.unary_unary(
                "/google.devicesandservices.health.v4.HealthProfileService/GetPairedDevice",
                request_serializer=health_profile.GetPairedDeviceRequest.serialize,
                response_deserializer=health_profile.PairedDevice.deserialize,
            )
        return self._stubs["get_paired_device"]
    ...
    def list_paired_devices(
        ...
        r"""Return a callable for the list paired devices method over gRPC.

        Returns the user's list of paired 1P trackers and
        smartwatches.

        Returns:
            Callable[[~.ListPairedDevicesRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "list_paired_devices" not in self._stubs:
            self._stubs["list_paired_devices"] = self._logged_channel.unary_unary(
                "/google.devicesandservices.health.v4.HealthProfileService/ListPairedDevices",
                request_serializer=health_profile.ListPairedDevicesRequest.serialize,
                response_deserializer=health_profile.ListPairedDevicesResponse.deserialize,
            )
        return self._stubs["list_paired_devices"]
...
__all__ = ("HealthProfileServiceGrpcTransport",)
