...
from .base import DEFAULT_CLIENT_INFO, SqlDataServiceTransport
...
class SqlDataServiceGrpcTransport(SqlDataServiceTransport):
    """gRPC backend transport for SqlDataService.
    ...
    def stream_sql_data(
        ...
        [cloud_sql_data.StreamSqlDataRequest], cloud_sql_data.StreamSqlDataResponse
    ]:
        r"""Return a callable for the stream sql data method over gRPC.

        ``StreamSqlData`` establishes a bidirectional stream to a Cloud
        ...
        if "stream_sql_data" not in self._stubs:
            self._stubs["stream_sql_data"] = self._logged_channel.stream_stream(
                "/google.cloud.sql.v1beta4.SqlDataService/StreamSqlData",
                request_serializer=cloud_sql_data.StreamSqlDataRequest.serialize,
                response_deserializer=cloud_sql_data.StreamSqlDataResponse.deserialize,
            )
        return self._stubs["stream_sql_data"]
...
__all__ = ("SqlDataServiceGrpcTransport",)
