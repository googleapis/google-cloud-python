...
from .base import DEFAULT_CLIENT_INFO, SSERealmServiceTransport
...
class SSERealmServiceGrpcTransport(SSERealmServiceTransport):
    """gRPC backend transport for SSERealmService.
    ...
    def list_sac_realms(
        ...
        if "list_sac_realms" not in self._stubs:
            self._stubs["list_sac_realms"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/ListSACRealms",
                request_serializer=sse_realm.ListSACRealmsRequest.serialize,
                response_deserializer=sse_realm.ListSACRealmsResponse.deserialize,
            )
    ...
    def get_sac_realm(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_sac_realm" not in self._stubs:
            self._stubs["get_sac_realm"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/GetSACRealm",
                request_serializer=sse_realm.GetSACRealmRequest.serialize,
                response_deserializer=sse_realm.SACRealm.deserialize,
            )
    ...
    def create_sac_realm(
        ...
        Returns:
            Callable[[~.CreateSACRealmRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "create_sac_realm" not in self._stubs:
            self._stubs["create_sac_realm"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/CreateSACRealm",
                request_serializer=sse_realm.CreateSACRealmRequest.serialize,
    ...
    def delete_sac_realm(
        ...
        Returns:
            Callable[[~.DeleteSACRealmRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "delete_sac_realm" not in self._stubs:
            self._stubs["delete_sac_realm"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/DeleteSACRealm",
                request_serializer=sse_realm.DeleteSACRealmRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
    ...
    def list_sac_attachments(
        ...
        Returns:
            Callable[[~.ListSACAttachmentsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "list_sac_attachments" not in self._stubs:
            self._stubs["list_sac_attachments"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/ListSACAttachments",
                request_serializer=sse_realm.ListSACAttachmentsRequest.serialize,
                response_deserializer=sse_realm.ListSACAttachmentsResponse.deserialize,
            )
    ...
    def get_sac_attachment(
        ...
        Returns:
            Callable[[~.GetSACAttachmentRequest],
                    ~.SACAttachment]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_sac_attachment" not in self._stubs:
            self._stubs["get_sac_attachment"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/GetSACAttachment",
                request_serializer=sse_realm.GetSACAttachmentRequest.serialize,
                response_deserializer=sse_realm.SACAttachment.deserialize,
            )
    ...
    def create_sac_attachment(
        ...
        Returns:
            Callable[[~.CreateSACAttachmentRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "create_sac_attachment" not in self._stubs:
            self._stubs["create_sac_attachment"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/CreateSACAttachment",
                request_serializer=sse_realm.CreateSACAttachmentRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
    ...
    def delete_sac_attachment(
        ...
        Returns:
            Callable[[~.DeleteSACAttachmentRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "delete_sac_attachment" not in self._stubs:
            self._stubs["delete_sac_attachment"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/DeleteSACAttachment",
                request_serializer=sse_realm.DeleteSACAttachmentRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
    ...
    def list_partner_sse_realms(
        ...
        Returns:
            Callable[[~.ListPartnerSSERealmsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "list_partner_sse_realms" not in self._stubs:
            self._stubs["list_partner_sse_realms"] = self._logged_channel.unary_unary(
                ...
                response_deserializer=sse_realm.ListPartnerSSERealmsResponse.deserialize,
            )
    ...
    def get_partner_sse_realm(
        ...
        Returns:
            Callable[[~.GetPartnerSSERealmRequest],
                    ~.PartnerSSERealm]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "get_partner_sse_realm" not in self._stubs:
            self._stubs["get_partner_sse_realm"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/GetPartnerSSERealm",
                request_serializer=sse_realm.GetPartnerSSERealmRequest.serialize,
                response_deserializer=sse_realm.PartnerSSERealm.deserialize,
            )
    ...
    def create_partner_sse_realm(
        ...
        Returns:
            Callable[[~.CreatePartnerSSERealmRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "create_partner_sse_realm" not in self._stubs:
            self._stubs["create_partner_sse_realm"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/CreatePartnerSSERealm",
                request_serializer=sse_realm.CreatePartnerSSERealmRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
    ...
    def delete_partner_sse_realm(
        ...
        Returns:
            Callable[[~.DeletePartnerSSERealmRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "delete_partner_sse_realm" not in self._stubs:
            self._stubs["delete_partner_sse_realm"] = self._logged_channel.unary_unary(
                "/google.cloud.networksecurity.v1alpha1.SSERealmService/DeletePartnerSSERealm",
                request_serializer=sse_realm.DeletePartnerSSERealmRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["delete_partner_sse_realm"]
...
__all__ = ("SSERealmServiceGrpcTransport",)
