...
class SecurityProfile(proto.Message):
    ...
    Attributes:
        threat_prevention_profile (google.cloud.network_security_v1.types.ThreatPreventionProfile):
            The threat prevention configuration for the
            SecurityProfile.

            This field is a member of `oneof`_ ``profile``.
        custom_mirroring_profile (google.cloud.network_security_v1.types.CustomMirroringProfile):
            The custom Packet Mirroring v2 configuration
            for the SecurityProfile.

            This field is a member of `oneof`_ ``profile``.
        custom_intercept_profile (google.cloud.network_security_v1.types.CustomInterceptProfile):
            The custom TPPI configuration for the
            SecurityProfile.

            This field is a member of `oneof`_ ``profile``.
        url_filtering_profile (google.cloud.network_security_v1.types.UrlFilteringProfile):
        ...
        labels (MutableMapping[str, str]):
            Optional. Labels as key value pairs.
        type_ (google.cloud.network_security_v1.types.SecurityProfile.ProfileType):
