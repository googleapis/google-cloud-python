...
def sample_get_sac_attachment():
    # Create a client
    client = network_security_v1.SSERealmServiceClient()

    # Initialize request argument(s)
    request = network_security_v1.GetSACAttachmentRequest(
        name="name_value",
    )

    # Make the request
    response = client.get_sac_attachment(request=request)
