...
def test__get_default_mtls_endpoint():
    ...
    assert (
        RegionalInventoryServiceClient._get_default_mtls_endpoint(custom_endpoint)
        == custom_endpoint
    )
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": ""}):
            with mock.patch.dict(os.environ, {"GOOGLE_API_CERTIFICATE_CONFIG": ""}):
                assert (
                    RegionalInventoryServiceClient._use_client_cert_effective() is False
                )
...
def test_list_regional_inventories_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/inventories/v1/{parent=accounts/*/products/*}/regionalInventories"
...
def test_delete_regional_inventory_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/inventories/v1/{name=accounts/*/products/*/regionalInventories/*}"
            % client.transport._host,
            args[1],
        )
