...
def test__get_api_endpoint():
    ...
    assert (
        LocalInventoryServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        LocalInventoryServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == LocalInventoryServiceClient.DEFAULT_MTLS_ENDPOINT
    )
...
async def test_list_local_inventories_async_pages():
    ...
    with mock.patch.object(type(client.transport.list_local_inventories), "__call__", new_callable=mock.AsyncMock) as call:
        ...
        async for page_ in (await client.list_local_inventories(request={})).pages:  # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ["abc", "def", "ghi", ""]):
            assert page_.raw_page.next_page_token == token
...
def test_list_local_inventories_rest_required_fields(request_type=localinventory.ListLocalInventoriesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_local_inventories._get_unset_required_fields(
        jsonified_request
    )
...
def test_insert_local_inventory_rest_required_fields(request_type=localinventory.InsertLocalInventoryRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).insert_local_inventory._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_delete_local_inventory_rest_required_fields(request_type=localinventory.DeleteLocalInventoryRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_local_inventory._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_transport_kind_grpc():
    transport = LocalInventoryServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = LocalInventoryServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = LocalInventoryServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = LocalInventoryServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_list_local_inventories_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "post_list_local_inventories") as post, mock.patch.object(
        transports.LocalInventoryServiceRestInterceptor, "post_list_local_inventories_with_metadata"
...
def test_insert_local_inventory_rest_call_success(request_type):
    ...
    request_init["local_inventory"] = {
        ...
        "sale_price": {},
        "sale_price_effective_date": {"start_time": {"seconds": 751, "nanos": 543}, "end_time": {}},
        "availability": "availability_value",
        "quantity": 895,
        "pickup_method": "pickup_method_value",
        "pickup_sla": "pickup_sla_value",
        "instore_product_location": "instore_product_location_value",
        "custom_attributes": [{"name": "name_value", "value": "value_value", "group_values": {}}],
    }
...
def test_insert_local_inventory_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "post_insert_local_inventory") as post, mock.patch.object(
        transports.LocalInventoryServiceRestInterceptor, "post_insert_local_inventory_with_metadata"
    ) as post_with_metadata, mock.patch.object(
...
def test_delete_local_inventory_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "pre_delete_local_inventory") as pre:
        pre.assert_not_called()
...
@pytest.mark.parametrize(
    ...
    [(transports.LocalInventoryServiceGrpcTransport, grpc_helpers), (transports.LocalInventoryServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_local_inventory_path():
    ...
    product = "clam"
    store_code = "whelk"
    expected = "accounts/{account}/products/{product}/localInventories/{store_code}".format(
        account=account,
        product=product,
