...
#
# Snippet for UpdateQueryTemplate
...
def sample_update_query_template():
    # Create a client
    client = bigquery_analyticshub_v1.AnalyticsHubServiceClient()

    # Initialize request argument(s)
    query_template = bigquery_analyticshub_v1.QueryTemplate()
    query_template.display_name = "display_name_value"

    request = bigquery_analyticshub_v1.UpdateQueryTemplateRequest(
        query_template=query_template,
    )

    # Make the request
    response = client.update_query_template(request=request)
