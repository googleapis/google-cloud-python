...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
            ...
            assert BatchServiceClient._read_environment_variables() == (
                False,
                "auto",
                None,
            )
...
def test_use_client_cert_effective():
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=True):
            assert BatchServiceClient._use_client_cert_effective() is True
    ...
    if hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch("google.auth.transport.mtls.should_use_client_cert", return_value=False):
            assert BatchServiceClient._use_client_cert_effective() is False
    ...
    if not hasattr(google.auth.transport.mtls, "should_use_client_cert"):
        with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "unsupported"}):
            with pytest.raises(ValueError):
                BatchServiceClient._use_client_cert_effective()
...
def test_batch_service_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "batch.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://batch.googleapis.com"
        )
...
def test_transport_kind_grpc():
    transport = BatchServiceClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = BatchServiceAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = BatchServiceAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = BatchServiceClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_create_job_rest_call_success(request_type):
    ...
    request_init["job"] = {
                    ...
                    "runnables": [
                        {
                            ...
                            "environment": {
                                "variables": {},
                                "secret_variables": {},
                                "encrypted_variables": {"key_name": "key_name_value", "cipher_text": "cipher_text_value"},
                            },
                            "timeout": {"seconds": 751, "nanos": 543},
                            "labels": {},
                        }
                    ],
                    "compute_resource": {"cpu_milli": 958, "memory_mib": 1072, "boot_disk_mib": 1365},
                    "max_run_duration": {},
                    "max_retry_count": 1635,
                    "lifecycle_policies": [{"action": 1, "action_condition": {"exit_codes": [1064, 1065]}}],
                    "environments": {},
                    "volumes": [
                        {
                            "nfs": {"server": "server_value", "remote_path": "remote_path_value"},
                            "gcs": {"remote_path": "remote_path_value"},
                            "device_name": "device_name_value",
                            "mount_path": "mount_path_value",
                            "mount_options": ["mount_options_value1", "mount_options_value2"],
                        }
                    ],
                    "environment": {},
                },
        ...
        "allocation_policy": {
            "location": {"allowed_locations": ["allowed_locations_value1", "allowed_locations_value2"]},
            "instances": [
                {
                    "policy": {
                        ...
                        "accelerators": [
                            {"type_": "type__value", "count": 553, "install_gpu_drivers": True, "driver_version": "driver_version_value"}
                        ],
                        "boot_disk": {
                            "image": "image_value",
                        ...
                        "disks": [{"new_disk": {}, "existing_disk": "existing_disk_value", "device_name": "device_name_value"}],
            ...
            "service_account": {"email": "email_value", "scopes": ["scopes_value1", "scopes_value2"]},
            "labels": {},
            "network": {"network_interfaces": [{"network": "network_value", "subnetwork": "subnetwork_value", "no_external_ip_address": True}]},
        ...
        "create_time": {},
        "update_time": {},
        "logs_policy": {"destination": 1, "logs_path": "logs_path_value", "cloud_logging_option": {"use_generic_task_monitored_resource": True}},
        "notifications": [{"pubsub_topic": "pubsub_topic_value", "message": {"type_": 1, "new_job_state": 1, "new_task_state": 1}}],
    }
...
def test_create_job_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.BatchServiceRestInterceptor, "post_create_job") as post, mock.patch.object(
...
def test_get_job_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BatchServiceRestInterceptor, "post_get_job") as post, mock.patch.object(
...
def test_list_jobs_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BatchServiceRestInterceptor, "post_list_jobs") as post, mock.patch.object(
...
def test_get_task_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BatchServiceRestInterceptor, "post_get_task") as post, mock.patch.object(
...
def test_list_tasks_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.BatchServiceRestInterceptor, "post_list_tasks") as post, mock.patch.object(
...
@pytest.mark.parametrize(
    ...
    [(transports.BatchServiceGrpcTransport, grpc_helpers), (transports.BatchServiceGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_batch_service_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "batch.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://batch.googleapis.com:8000"
    )
