...
class BatchServiceClient(metaclass=BatchServiceClientMeta):
    ...
    @staticmethod
    def resource_allowance_path(
        project: str,
        location: str,
        resource_allowance: str,
    ) -> str:
        """Returns a fully-qualified resource_allowance string."""
        return "projects/{project}/locations/{location}/resourceAllowances/{resource_allowance}".format(
            project=project,
            location=location,
    ...
    def parse_task_path(path: str) -> Dict[str, str]:
        ...
        m = re.match(
            r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/jobs/(?P<job>.+?)/taskGroups/(?P<task_group>.+?)/tasks/(?P<task>.+?)$", path
        )
        return m.groupdict() if m else {}
    ...
    def update_resource_allowance(
        ...
        *,
        ...
        # Certain fields should be provided within the metadata header;
        # add these here.
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((("resource_allowance.name", request.resource_allowance.name),)),)
