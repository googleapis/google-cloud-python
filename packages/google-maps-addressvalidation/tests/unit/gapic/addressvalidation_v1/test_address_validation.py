...
def test__get_api_endpoint():
    ...
    assert (
        AddressValidationClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto")
    ...
    assert (
        AddressValidationClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always")
        == AddressValidationClient.DEFAULT_MTLS_ENDPOINT
    )
...
def test_address_validation_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, "from_service_account_info") as factory:
        ...
        assert client.transport._host == (
            "addressvalidation.googleapis.com:443" if transport_name in ["grpc", "grpc_asyncio"] else "https://addressvalidation.googleapis.com"
        )
...
async def test_provide_validation_feedback_async(
    transport: str = "grpc_asyncio", request_type=address_validation_service.ProvideValidationFeedbackRequest
):
    client = AddressValidationAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
...
def test_provide_validation_feedback_rest_required_fields(request_type=address_validation_service.ProvideValidationFeedbackRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).provide_validation_feedback._get_unset_required_fields(
        jsonified_request
    )
    jsonified_request.update(unset_fields)
...
def test_transport_kind_grpc():
    transport = AddressValidationClient.get_transport_class("grpc")(credentials=ga_credentials.AnonymousCredentials())
...
def test_transport_kind_grpc_asyncio():
    transport = AddressValidationAsyncClient.get_transport_class("grpc_asyncio")(credentials=async_anonymous_credentials())
    assert transport.kind == "grpc_asyncio"


def test_initialize_client_w_grpc_asyncio():
    client = AddressValidationAsyncClient(credentials=async_anonymous_credentials(), transport="grpc_asyncio")
    assert client is not None
...
def test_transport_kind_rest():
    transport = AddressValidationClient.get_transport_class("rest")(credentials=ga_credentials.AnonymousCredentials())
...
def test_validate_address_rest_interceptors(null_interceptor):
    ...
    ) as transcode, mock.patch.object(transports.AddressValidationRestInterceptor, "post_validate_address") as post, mock.patch.object(
        transports.AddressValidationRestInterceptor, "post_validate_address_with_metadata"
...
def test_provide_validation_feedback_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, mock.patch.object(
        path_template, "transcode"
    ) as transcode, mock.patch.object(transports.AddressValidationRestInterceptor, "post_provide_validation_feedback") as post, mock.patch.object(
        transports.AddressValidationRestInterceptor, "post_provide_validation_feedback_with_metadata"
    ) as post_with_metadata, mock.patch.object(
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = address_validation_service.ProvideValidationFeedbackResponse.to_json(
...
@pytest.mark.parametrize(
    ...
    [(transports.AddressValidationGrpcTransport, grpc_helpers), (transports.AddressValidationGrpcAsyncIOTransport, grpc_helpers_async)],
)
...
def test_address_validation_host_with_port(transport_name):
    ...
    assert client.transport._host == (
        "addressvalidation.googleapis.com:8000" if transport_name in ["grpc", "grpc_asyncio"] else "https://addressvalidation.googleapis.com:8000"
    )
