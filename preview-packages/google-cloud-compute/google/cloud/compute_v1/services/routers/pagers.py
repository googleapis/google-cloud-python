...
class ListNamedSetsPager:
    ...
    This class thinly wraps an initial
    :class:`google.cloud.compute_v1.types.RoutersListNamedSets` object, and
    provides an ``__iter__`` method to iterate through its
    ``result`` field.
    ...
    All the usual :class:`google.cloud.compute_v1.types.RoutersListNamedSets`
    attributes are available on the pager. If multiple requests are made, only
    the most recent response is retained, and thus used for attribute lookup.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            method (Callable): The method that was originally called, and
                which instantiated this pager.
            request (google.cloud.compute_v1.types.ListNamedSetsRoutersRequest):
                The initial request object.
            response (google.cloud.compute_v1.types.RoutersListNamedSets):
                The initial response object.
            retry (google.api_core.retry.Retry): Designation of what errors,
                if any, should be retried.
            timeout (float): The timeout for this request.
