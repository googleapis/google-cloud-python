...
def _copy_files_needed_for_post_processing(
    ...
    header_line = """# DO NOT EDIT THIS FILE OUTSIDE OF `.librarian/generator-input`
# The source of truth for this file is .librarian/generator-input\n"""
    ...
    if Path(source_dir).exists():
        ...
        # Walk through the destination directory to add `header_line`
        # to files in `.librarian/generator-input`.
        for root, _, files in os.walk(source_dir):
            for file_name in files:
                # Skip JSON files which do not support comments.
                if file_name.endswith(".json"):
                    continue

                rel_path = os.path.relpath(root, source_dir)
                dest_file_path = Path(destination_dir) / rel_path / file_name

                with open(dest_file_path, "r", encoding="utf-8") as f:
                    lines = f.readlines()

                line_index = 0
                # Skip the license header which is a contiguous block of comments.
                # We keep advancing as long as the line starts with '#'.
                while line_index < len(lines) and lines[line_index].strip().startswith(
                    "#"
                ):
                    line_index += 1

                # Insert the new header at the calculated index.
