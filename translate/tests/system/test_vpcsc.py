...
class TestVPCServiceControl(object):
  ...
  def setup(self):
    ...
    self._parent_inside = self._client.location_path(PROJECT_INSIDE,
                                                     'us-central1')
    self._parent_outside = self._client.location_path(PROJECT_OUTSIDE,
                                                      'us-central1')
  ...
  def test_create_glossary(self):
    ...
    def make_glossary(project_id):
      return {
          'name':
              'projects/{0}/locations/us-central1/glossaries/fake_glossary'
              .format(project_id),
          'language_codes_set': {
          ...
          'input_config': {
              'gcs_source': {
                  'input_uri': 'gs://fake-bucket/fake_glossary.csv'
              }
          }
      }
  ...
  def test_list_glossaries(self):
    ...
    delayed_inside = lambda: self._client.list_glossaries(self._parent_inside
                                                         )._next_page()
    delayed_outside = lambda: self._client.list_glossaries(self._parent_outside
                                                          )._next_page()
    TestVPCServiceControl._do_test(delayed_inside, delayed_outside)
  ...
  def test_get_glossary(self):
    delayed_inside = lambda: self._client.get_glossary(self.
                                                       _glossary_name_inside)
    delayed_outside = lambda: self._client.get_glossary(self.
                                                        _glossary_name_outside)
    TestVPCServiceControl._do_test(delayed_inside, delayed_outside)
  ...
  def test_delete_glossary(self):
    delayed_inside = lambda: self._client.delete_glossary(self.
                                                          _glossary_name_inside)
  ...
  def test_batch_translate_text(self):
    ...
    output_config = {
        'gcs_destination': {
            'output_uri_prefix': 'gs://fake-bucket/output/'
        }
    }
    delayed_inside = lambda: self._client.batch_translate_text(
        self._parent_inside, source_language_code, target_language_codes,
        input_configs, output_config)
    delayed_outside = lambda: self._client.batch_translate_text(
        self._parent_outside, source_language_code, target_language_codes,
        input_configs, output_config)
    TestVPCServiceControl._do_test(delayed_inside, delayed_outside)
