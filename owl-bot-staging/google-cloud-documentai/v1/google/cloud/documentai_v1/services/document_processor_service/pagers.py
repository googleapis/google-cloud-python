...
class ListEvaluationsPager:
    ...
    This class thinly wraps an initial
    :class:`google.cloud.documentai_v1.types.ListEvaluationsResponse` object, and
    provides an ``__iter__`` method to iterate through its
    ``evaluations`` field.
    ...
    All the usual :class:`google.cloud.documentai_v1.types.ListEvaluationsResponse`
    attributes are available on the pager. If multiple requests are made, only
    the most recent response is retained, and thus used for attribute lookup.
    """
    def __init__(self,
        ...
        Args:
            method (Callable): The method that was originally called, and
                which instantiated this pager.
            request (google.cloud.documentai_v1.types.ListEvaluationsRequest):
                The initial request object.
            response (google.cloud.documentai_v1.types.ListEvaluationsResponse):
                The initial response object.
            metadata (Sequence[Tuple[str, str]]): Strings which should be
                sent along with the request as metadata.
        """
