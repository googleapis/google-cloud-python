...
from google.cloud.documentai_v1beta3.services.document_service import DocumentServiceAsyncClient
...
from google.cloud.documentai_v1beta3.services.document_service import transports
...
def test_update_dataset_flattened():
    ...
    with mock.patch.object(
        ...
        client.update_dataset(
            dataset=gcd_dataset.Dataset(gcs_managed_config=gcd_dataset.Dataset.GCSManagedConfig(gcs_prefix=document_io.GcsPrefix(gcs_uri_prefix='gcs_uri_prefix_value'))),
...
def test_list_documents_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, document_service.DocumentMetadata)
                   for i in results)
...
def test_batch_delete_documents(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.batch_delete_documents),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name='operations/spam')
...
def test_get_dataset_schema(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_dataset_schema),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_update_dataset_schema(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.update_dataset_schema),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = dataset.DatasetSchema(
            name='name_value',
...
def test_update_dataset_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_update_dataset") as post, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_update_dataset_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "pre_update_dataset") as pre:
...
def test_batch_delete_documents_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_batch_delete_documents") as post, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_batch_delete_documents_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "pre_batch_delete_documents") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_dataset_schema_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_get_dataset_schema") as post, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_get_dataset_schema_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "pre_get_dataset_schema") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_update_dataset_schema_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_update_dataset_schema") as post, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "post_update_dataset_schema_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.DocumentServiceRestInterceptor, "pre_update_dataset_schema") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
