...
class JobControllerClient(metaclass=JobControllerClientMeta):
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = JobControllerClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = JobControllerClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = JobControllerClient._DEFAULT_UNIVERSE
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, JobControllerTransport, Callable[..., JobControllerTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = JobControllerClient._read_environment_variables()
        self._client_cert_source = JobControllerClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = JobControllerClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, JobControllerTransport)
        if transport_provided:
            ...
            self._transport = cast(JobControllerTransport, transport)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[JobControllerTransport], Callable[..., JobControllerTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.dataproc_v1.JobControllerClient`.",
                    extra = {
    ...
    def list_jobs(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [project_id, region, filter]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
