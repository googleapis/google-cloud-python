...
def test_ingest_context_references_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2/{conversation=projects/*/locations/*/conversations/*}:ingestContextReferences" % client.transport._host, args[1])
...
def test_create_conversation_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1'}
    request_init["conversation"] = {'name': 'name_value', 'lifecycle_state': 1, 'conversation_profile': 'conversation_profile_value', 'phone_number': {'country_code': 1294, 'phone_number': 'phone_number_value'}, 'start_time': {'seconds': 751, 'nanos': 543}, 'end_time': {}, 'conversation_stage': 1, 'telephony_connection_info': {'dialed_number': 'dialed_number_value', 'sdp': 'sdp_value', 'sip_headers': [{'name': 'name_value', 'value': 'value_value'}], 'extra_mime_contents': [{'mime_type': 'mime_type_value', 'content': b'content_blob'}]}, 'ingested_context_references': {}}
