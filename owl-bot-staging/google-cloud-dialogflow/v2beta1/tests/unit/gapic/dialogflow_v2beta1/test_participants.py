...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ParticipantsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ParticipantsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ParticipantsClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert ParticipantsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ParticipantsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ParticipantsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ParticipantsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ParticipantsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParticipantsClient))
@mock.patch.object(ParticipantsAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParticipantsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ParticipantsClient._DEFAULT_UNIVERSE
    default_endpoint = ParticipantsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ParticipantsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ParticipantsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ParticipantsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ParticipantsClient.DEFAULT_MTLS_ENDPOINT
    assert ParticipantsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ParticipantsClient._get_api_endpoint(None, None, default_universe, "always") == ParticipantsClient.DEFAULT_MTLS_ENDPOINT
    assert ParticipantsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ParticipantsClient.DEFAULT_MTLS_ENDPOINT
    assert ParticipantsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ParticipantsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ParticipantsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ParticipantsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ParticipantsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ParticipantsClient._get_universe_domain(None, None) == ParticipantsClient._DEFAULT_UNIVERSE
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = ParticipantsClient(credentials=cred)
...
def test_participants_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class", [
    ParticipantsClient, ParticipantsAsyncClient
])
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ParticipantsClient, transports.ParticipantsGrpcTransport, "grpc", grpc_helpers),
    (ParticipantsAsyncClient, transports.ParticipantsGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (ParticipantsClient, transports.ParticipantsRestTransport, "rest", None),
])
def test_participants_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_participants_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_create_participant_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.create_participant in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_participant] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_participant_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.create_participant in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_participant] = mock_rpc
...
def test_update_participant_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.update_participant in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.update_participant] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_update_participant_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_participant in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_participant] = mock_rpc
...
@pytest.mark.asyncio
async def test_analyze_content_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.analyze_content in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.analyze_content] = mock_rpc
...
def test_streaming_analyze_content_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.streaming_analyze_content in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.streaming_analyze_content] = mock_rpc
        request = [{}]
...
@pytest.mark.asyncio
async def test_streaming_analyze_content_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.streaming_analyze_content in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.streaming_analyze_content] = mock_rpc
...
def test_suggest_articles_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.suggest_articles] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_suggest_articles_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.suggest_articles in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.suggest_articles] = mock_rpc
...
def test_suggest_faq_answers_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.suggest_faq_answers in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.suggest_faq_answers] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_suggest_faq_answers_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.suggest_faq_answers in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.suggest_faq_answers] = mock_rpc
...
def test_suggest_smart_replies_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.suggest_smart_replies in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.suggest_smart_replies] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_suggest_smart_replies_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.suggest_smart_replies in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.suggest_smart_replies] = mock_rpc
...
def test_suggest_knowledge_assist(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.suggest_knowledge_assist),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_suggest_knowledge_assist_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.suggest_knowledge_assist in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.suggest_knowledge_assist] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_suggest_knowledge_assist_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.suggest_knowledge_assist in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.suggest_knowledge_assist] = mock_rpc
...
@pytest.mark.asyncio
async def test_suggest_knowledge_assist_async(transport: str = 'grpc_asyncio', request_type=participant.SuggestKnowledgeAssistRequest):
    client = ParticipantsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(participant.SuggestKnowledgeAssistResponse(
            latest_message='latest_message_value',
            context_size=1311,
        ))
...
def test_list_suggestions_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_suggestions] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_suggestions_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_suggestions in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_suggestions] = mock_rpc
...
def test_compile_suggestion_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.compile_suggestion in client._transport._wrapped_methods
...
@pytest.mark.asyncio
async def test_compile_suggestion_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ParticipantsAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.compile_suggestion in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.compile_suggestion] = mock_rpc
...
def test_create_participant_rest_required_fields(request_type=gcd_participant.CreateParticipantRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_participant._get_unset_required_fields(jsonified_request)
...
def test_create_participant_rest_unset_required_fields():
    transport = transports.ParticipantsRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.create_participant._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("parent", "participant", )))
...
def test_create_participant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v2beta1/{parent=projects/*/conversations/*}/participants" % client.transport._host, args[1])
...
def test_get_participant_rest_required_fields(request_type=participant.GetParticipantRequest):
    transport_class = transports.ParticipantsRestTransport
...
def test_get_participant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {'name': 'projects/sample1/conversations/sample2/participants/sample3'}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{name=projects/*/conversations/*/participants/*}" % client.transport._host, args[1])
...
def test_list_participants_rest_required_fields(request_type=participant.ListParticipantsRequest):
    transport_class = transports.ParticipantsRestTransport
...
def test_list_participants_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        response = tuple(participant.ListParticipantsResponse.to_json(x) for x in response)
...
def test_update_participant_rest_required_fields(request_type=gcd_participant.UpdateParticipantRequest):
    transport_class = transports.ParticipantsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_participant._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_participant_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("updateMask", )) & set(("participant", "updateMask", )))
...
def test_update_participant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {'participant': {'name': 'projects/sample1/conversations/sample2/participants/sample3'}}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{participant.name=projects/*/conversations/*/participants/*}" % client.transport._host, args[1])
...
def test_analyze_content_rest_required_fields(request_type=gcd_participant.AnalyzeContentRequest):
    transport_class = transports.ParticipantsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).analyze_content._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_analyze_content_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {'participant': 'projects/sample1/conversations/sample2/participants/sample3'}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{participant=projects/*/conversations/*/participants/*}:analyzeContent" % client.transport._host, args[1])
...
def test_suggest_articles_rest_required_fields(request_type=participant.SuggestArticlesRequest):
    transport_class = transports.ParticipantsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).suggest_articles._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_suggest_articles_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {'parent': 'projects/sample1/conversations/sample2/participants/sample3'}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{parent=projects/*/conversations/*/participants/*}/suggestions:suggestArticles" % client.transport._host, args[1])
...
def test_suggest_faq_answers_rest_required_fields(request_type=participant.SuggestFaqAnswersRequest):
    transport_class = transports.ParticipantsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).suggest_faq_answers._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_suggest_faq_answers_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{parent=projects/*/conversations/*/participants/*}/suggestions:suggestFaqAnswers" % client.transport._host, args[1])
...
def test_suggest_smart_replies_rest_required_fields(request_type=participant.SuggestSmartRepliesRequest):
    transport_class = transports.ParticipantsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).suggest_smart_replies._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_suggest_smart_replies_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2beta1/{parent=projects/*/conversations/*/participants/*}/suggestions:suggestSmartReplies" % client.transport._host, args[1])
...
def test_suggest_knowledge_assist_rest_required_fields(request_type=participant.SuggestKnowledgeAssistRequest):
    transport_class = transports.ParticipantsRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).suggest_knowledge_assist._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_list_suggestions_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # Wrap the values into proper Response objs
        response = tuple(participant.ListSuggestionsResponse.to_json(x) for x in response)
        return_values = tuple(Response() for i in response)
        for return_val, response_val in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_suggestions_empty_call_grpc():
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = participant.ListSuggestionsResponse()
        client.list_suggestions(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = participant.ListSuggestionsRequest()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_compile_suggestion_empty_call_grpc():
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = participant.CompileSuggestionResponse()
        client.compile_suggestion(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = participant.CompileSuggestionRequest()
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_suggestions_empty_call_grpc_asyncio():
    client = ParticipantsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.list_suggestions(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_compile_suggestion_empty_call_grpc_asyncio():
    client = ParticipantsAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.compile_suggestion(request=None)
...
def test_create_participant_rest_bad_request(request_type=gcd_participant.CreateParticipantRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_create_participant_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/conversations/sample2'}
    request_init["participant"] = {'name': 'name_value', 'role': 1, 'obfuscated_external_user_id': 'obfuscated_external_user_id_value', 'documents_metadata_filters': {}}
...
def test_create_participant_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_create_participant") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_create_participant_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_create_participant") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_participant.CreateParticipantRequest.pb(gcd_participant.CreateParticipantRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcd_participant.Participant.to_json(gcd_participant.Participant())
        ...
        post_with_metadata.return_value = gcd_participant.Participant(), metadata

        client.create_participant(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_get_participant_rest_bad_request(request_type=participant.GetParticipantRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    # send a request that will satisfy transcoding
    request_init = {'name': 'projects/sample1/conversations/sample2/participants/sample3'}
    request = request_type(**request_init)
...
def test_get_participant_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_get_participant") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_get_participant_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_get_participant") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.GetParticipantRequest.pb(participant.GetParticipantRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        post_with_metadata.return_value = participant.Participant(), metadata
...
def test_list_participants_rest_bad_request(request_type=participant.ListParticipantsRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_participants_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_list_participants") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_list_participants_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_list_participants") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.ListParticipantsRequest.pb(participant.ListParticipantsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.ListParticipantsResponse.to_json(participant.ListParticipantsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.ListParticipantsResponse(), metadata
...
def test_update_participant_rest_bad_request(request_type=gcd_participant.UpdateParticipantRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    # send a request that will satisfy transcoding
    request_init = {'participant': {'name': 'projects/sample1/conversations/sample2/participants/sample3'}}
    request = request_type(**request_init)
...
def test_update_participant_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'participant': {'name': 'projects/sample1/conversations/sample2/participants/sample3'}}
    request_init["participant"] = {'name': 'projects/sample1/conversations/sample2/participants/sample3', 'role': 1, 'obfuscated_external_user_id': 'obfuscated_external_user_id_value', 'documents_metadata_filters': {}}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_update_participant_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_update_participant") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_update_participant_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_update_participant") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_participant.UpdateParticipantRequest.pb(gcd_participant.UpdateParticipantRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.update_participant(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_analyze_content_rest_bad_request(request_type=gcd_participant.AnalyzeContentRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    # send a request that will satisfy transcoding
    request_init = {'participant': 'projects/sample1/conversations/sample2/participants/sample3'}
    request = request_type(**request_init)
...
def test_analyze_content_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_analyze_content") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_analyze_content_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_analyze_content") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = gcd_participant.AnalyzeContentRequest.pb(gcd_participant.AnalyzeContentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcd_participant.AnalyzeContentResponse.to_json(gcd_participant.AnalyzeContentResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = gcd_participant.AnalyzeContentResponse(), metadata

        client.analyze_content(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_suggest_articles_rest_bad_request(request_type=participant.SuggestArticlesRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/conversations/sample2/participants/sample3'}
    request = request_type(**request_init)
...
def test_suggest_articles_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_articles") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_articles_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_suggest_articles") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.SuggestArticlesRequest.pb(participant.SuggestArticlesRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.SuggestArticlesResponse.to_json(participant.SuggestArticlesResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.SuggestArticlesResponse(), metadata

        client.suggest_articles(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_suggest_faq_answers_rest_bad_request(request_type=participant.SuggestFaqAnswersRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_suggest_faq_answers_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_faq_answers") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_faq_answers_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_suggest_faq_answers") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.SuggestFaqAnswersRequest.pb(participant.SuggestFaqAnswersRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.SuggestFaqAnswersResponse.to_json(participant.SuggestFaqAnswersResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.SuggestFaqAnswersResponse(), metadata

        client.suggest_faq_answers(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_suggest_smart_replies_rest_bad_request(request_type=participant.SuggestSmartRepliesRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_suggest_smart_replies_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_smart_replies") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_smart_replies_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_suggest_smart_replies") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.SuggestSmartRepliesRequest.pb(participant.SuggestSmartRepliesRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.SuggestSmartRepliesResponse.to_json(participant.SuggestSmartRepliesResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.SuggestSmartRepliesResponse(), metadata

        client.suggest_smart_replies(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_suggest_knowledge_assist_rest_bad_request(request_type=participant.SuggestKnowledgeAssistRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_suggest_knowledge_assist_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_knowledge_assist") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_suggest_knowledge_assist_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_suggest_knowledge_assist") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.SuggestKnowledgeAssistRequest.pb(participant.SuggestKnowledgeAssistRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.SuggestKnowledgeAssistResponse.to_json(participant.SuggestKnowledgeAssistResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.SuggestKnowledgeAssistResponse(), metadata

        client.suggest_knowledge_assist(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_list_suggestions_rest_bad_request(request_type=participant.ListSuggestionsRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_suggestions_rest_call_success(request_type):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_suggestions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_list_suggestions") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_list_suggestions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_list_suggestions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.ListSuggestionsRequest.pb(participant.ListSuggestionsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.ListSuggestionsResponse.to_json(participant.ListSuggestionsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.ListSuggestionsResponse(), metadata

        client.list_suggestions(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_compile_suggestion_rest_bad_request(request_type=participant.CompileSuggestionRequest):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_compile_suggestion_rest_call_success(request_type):
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_compile_suggestion_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_compile_suggestion") as post, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "post_compile_suggestion_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParticipantsRestInterceptor, "pre_compile_suggestion") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = participant.CompileSuggestionRequest.pb(participant.CompileSuggestionRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = participant.CompileSuggestionResponse.to_json(participant.CompileSuggestionResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = participant.CompileSuggestionResponse(), metadata

        client.compile_suggestion(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_suggestions_empty_call_rest():
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_compile_suggestion_empty_call_rest():
    client = ParticipantsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_participants_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.dialogflow_v2beta1.services.participants.transports.ParticipantsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_participants_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.dialogflow_v2beta1.services.participants.transports.ParticipantsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ParticipantsTransport()
        adc.assert_called_once()
