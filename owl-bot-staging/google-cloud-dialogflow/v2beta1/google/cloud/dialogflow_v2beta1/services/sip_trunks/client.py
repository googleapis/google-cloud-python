...
from .transports.base import SipTrunksTransport, DEFAULT_CLIENT_INFO
...
class SipTrunksClient(metaclass=SipTrunksClientMeta):
    ...
    @staticmethod
    def sip_trunk_path(project: str,location: str,siptrunk: str,) -> str:
        """Returns a fully-qualified sip_trunk string."""
        return "projects/{project}/locations/{location}/sipTrunks/{siptrunk}".format(project=project, location=location, siptrunk=siptrunk, )

    @staticmethod
    def parse_sip_trunk_path(path: str) -> Dict[str,str]:
        """Parses a sip_trunk path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/sipTrunks/(?P<siptrunk>.+?)$", path)
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = SipTrunksClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, SipTrunksTransport, Callable[..., SipTrunksTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = SipTrunksClient._read_environment_variables()
        self._client_cert_source = SipTrunksClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = SipTrunksClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[SipTrunksTransport], Callable[..., SipTrunksTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.dialogflow_v2beta1.SipTrunksClient`.",
                    extra = {
