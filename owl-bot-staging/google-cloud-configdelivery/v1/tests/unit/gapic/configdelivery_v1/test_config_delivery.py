...
from google.cloud.configdelivery_v1.services.config_delivery import ConfigDeliveryAsyncClient
from google.cloud.configdelivery_v1.services.config_delivery import ConfigDeliveryClient
from google.cloud.configdelivery_v1.services.config_delivery import pagers
from google.cloud.configdelivery_v1.services.config_delivery import transports
...
def test_update_variant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{variant.name=projects/*/locations/*/resourceBundles/*/releases/*/variants/*}" % client.transport._host, args[1])
...
def test_config_delivery_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.configdelivery_v1.services.config_delivery.transports.ConfigDeliveryTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_config_delivery_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.configdelivery_v1.services.config_delivery.transports.ConfigDeliveryTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ConfigDeliveryTransport()
        adc.assert_called_once()
