...
from .base import ImageServiceTransport, DEFAULT_CLIENT_INFO
...
class ImageServiceGrpcAsyncIOTransport(ImageServiceTransport):
    """gRPC AsyncIO backend transport for ImageService.
    ...
    @property
    def generate_product_image_background(self) -> Callable[
            [image.GenerateProductImageBackgroundRequest],
            Awaitable[image.GenerateProductImageBackgroundResponse]]:
        r"""Return a callable for the generate product image
        background method over gRPC.

        GenerateProductImageBackground generates a new image
        ...
        Returns:
            Callable[[~.GenerateProductImageBackgroundRequest],
                    Awaitable[~.GenerateProductImageBackgroundResponse]]:
    ...
    @property
    def remove_product_image_background(self) -> Callable[
            [image.RemoveProductImageBackgroundRequest],
            Awaitable[image.RemoveProductImageBackgroundResponse]]:
        r"""Return a callable for the remove product image
        background method over gRPC.

        RemoveProductImageBackground generates a new image
        where the background of the original image is removed.

        Returns:
            Callable[[~.RemoveProductImageBackgroundRequest],
                    Awaitable[~.RemoveProductImageBackgroundResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    @property
    def upscale_product_image(self) -> Callable[
            [image.UpscaleProductImageRequest],
            Awaitable[image.UpscaleProductImageResponse]]:
        ...
        Returns:
            Callable[[~.UpscaleProductImageRequest],
                    Awaitable[~.UpscaleProductImageResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'upscale_product_image' not in self._stubs:
            self._stubs['upscale_product_image'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.productstudio.v1alpha.ImageService/UpscaleProductImage',
                request_serializer=image.UpscaleProductImageRequest.serialize,
                response_deserializer=image.UpscaleProductImageResponse.deserialize,
            )
        return self._stubs['upscale_product_image']

    def _prep_wrapped_messages(self, client_info):
        """ Precompute the wrapped methods, overriding the base class method to use async wrappers."""
        self._wrapped_methods = {
            self.generate_product_image_background: self._wrap_method(
                self.generate_product_image_background,
                default_timeout=None,
                client_info=client_info,
            ),
            self.remove_product_image_background: self._wrap_method(
                self.remove_product_image_background,
                default_timeout=None,
                client_info=client_info,
            ),
            self.upscale_product_image: self._wrap_method(
                self.upscale_product_image,
                default_timeout=None,
                client_info=client_info,
            ),
        }
