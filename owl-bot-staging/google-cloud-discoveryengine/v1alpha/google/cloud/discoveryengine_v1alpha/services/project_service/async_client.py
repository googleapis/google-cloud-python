...
class ProjectServiceAsyncClient:
    ...
    get_transport_class = functools.partial(type(ProjectServiceClient).get_transport_class, type(ProjectServiceClient))
    ...
    async def report_consent_change(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        has_flattened_params = any([consent_change_action, project, service_term_id, service_term_version])
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of "
                             "the individual field arguments should be set.")
