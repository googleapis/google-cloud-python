...
async def sample_update_cmek_config():
    # Create a client
    client = discoveryengine_v1.CmekConfigServiceAsyncClient()

    # Initialize request argument(s)
    config = discoveryengine_v1.CmekConfig()
    config.name = "name_value"

    request = discoveryengine_v1.UpdateCmekConfigRequest(
        config=config,
    )

    # Make the request
    operation = client.update_cmek_config(request=request)
