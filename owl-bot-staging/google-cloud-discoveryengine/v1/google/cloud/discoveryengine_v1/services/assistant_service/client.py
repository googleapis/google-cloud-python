...
class AssistantServiceClient(metaclass=AssistantServiceClientMeta):
    ...
    @staticmethod
    def assistant_path(project: str,location: str,collection: str,engine: str,assistant: str,) -> str:
        """Returns a fully-qualified assistant string."""
        return "projects/{project}/locations/{location}/collections/{collection}/engines/{engine}/assistants/{assistant}".format(project=project, location=location, collection=collection, engine=engine, assistant=assistant, )

    @staticmethod
    def parse_assistant_path(path: str) -> Dict[str,str]:
        """Parses a assistant path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/collections/(?P<collection>.+?)/engines/(?P<engine>.+?)/assistants/(?P<assistant>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = AssistantServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, AssistantServiceTransport, Callable[..., AssistantServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = AssistantServiceClient._read_environment_variables()
        self._client_cert_source = AssistantServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = AssistantServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        self._api_endpoint = (self._api_endpoint or
            AssistantServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[AssistantServiceTransport], Callable[..., AssistantServiceTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.discoveryengine_v1.AssistantServiceClient`.",
                    extra = {
