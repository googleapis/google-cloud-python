    ...
    * Stripping extraneous information from responses
    ...
    .. code-block:: python
        class MyCustomGroundedGenerationServiceInterceptor(GroundedGenerationServiceRestInterceptor):
            ...
            def pre_generate_grounded_content(self, request, metadata):
                logging.log(f"Received request: {request}")
                return request, metadata

            def post_generate_grounded_content(self, response):
                logging.log(f"Received response: {response}")
                return response
    ...
    def post_generate_grounded_content(self, response: grounded_generation_service.GenerateGroundedContentResponse) -> grounded_generation_service.GenerateGroundedContentResponse:
        ...
        Override in a subclass to read or manipulate the response
        after it is returned by the GroundedGenerationService server but before
        it is returned to user code. This `post_generate_grounded_content` interceptor runs
        before the `post_generate_grounded_content_with_metadata` interceptor.
        """
        return response
    ...
    def post_generate_grounded_content_with_metadata(self, response: grounded_generation_service.GenerateGroundedContentResponse, metadata: Sequence[Tuple[str, Union[str, bytes]]]) -> Tuple[grounded_generation_service.GenerateGroundedContentResponse, Sequence[Tuple[str, Union[str, bytes]]]]:
        ...
        We recommend only using this `post_generate_grounded_content_with_metadata`
        interceptor in new development instead of the `post_generate_grounded_content` interceptor.
        When both interceptors are used, this `post_generate_grounded_content_with_metadata` interceptor runs after the
        `post_generate_grounded_content` interceptor. The (possibly modified) response returned by
        `post_generate_grounded_content` will be passed to
        `post_generate_grounded_content_with_metadata`.
        """
        return response, metadata
