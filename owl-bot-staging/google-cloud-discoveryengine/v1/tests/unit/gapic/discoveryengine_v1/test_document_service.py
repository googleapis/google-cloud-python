...
from google.cloud.discoveryengine_v1.services.document_service import DocumentServiceAsyncClient
...
def test_update_document_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{document.name=projects/*/locations/*/dataStores/*/branches/*/documents/*}" % client.transport._host, args[1])
...
def test_batch_get_documents_metadata_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=projects/*/locations/*/dataStores/*/branches/*}/batchGetDocumentsMetadata" % client.transport._host, args[1])
...
def test_create_document_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/locations/sample2/dataStores/sample3/branches/sample4'}
    request_init["document"] = {'struct_data': {'fields': {}}, 'json_data': 'json_data_value', 'name': 'name_value', 'id': 'id_value', 'schema_id': 'schema_id_value', 'content': {'raw_bytes': b'raw_bytes_blob', 'uri': 'uri_value', 'mime_type': 'mime_type_value'}, 'parent_document_id': 'parent_document_id_value', 'derived_struct_data': {}, 'acl_info': {'readers': [{'principals': [{'user_id': 'user_id_value', 'group_id': 'group_id_value', 'external_entity_id': 'external_entity_id_value'}], 'idp_wide': True}]}, 'index_time': {'seconds': 751, 'nanos': 543}, 'index_status': {'index_time': {}, 'error_samples': [{'code': 411, 'message': 'message_value', 'details': [{'type_url': 'type.googleapis.com/google.protobuf.Duration', 'value': b'\x08\x0c\x10\xdb\x07'}]}], 'pending_message': 'pending_message_value'}}
...
def test_document_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.discoveryengine_v1.services.document_service.transports.DocumentServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_document_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.discoveryengine_v1.services.document_service.transports.DocumentServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.DocumentServiceTransport()
        adc.assert_called_once()
