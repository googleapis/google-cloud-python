...
class RecommendationServiceClient(metaclass=RecommendationServiceClientMeta):
    ...
    @staticmethod
    def engine_path(project: str,location: str,collection: str,engine: str,) -> str:
        """Returns a fully-qualified engine string."""
        return "projects/{project}/locations/{location}/collections/{collection}/engines/{engine}".format(project=project, location=location, collection=collection, engine=engine, )

    @staticmethod
    def parse_engine_path(path: str) -> Dict[str,str]:
        """Parses a engine path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/collections/(?P<collection>.+?)/engines/(?P<engine>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = RecommendationServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = RecommendationServiceClient._DEFAULT_UNIVERSE
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            RecommendationServiceClient._compare_universes(self.universe_domain, self.transport._credentials))
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, RecommendationServiceTransport, Callable[..., RecommendationServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = RecommendationServiceClient._read_environment_variables()
        self._client_cert_source = RecommendationServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = RecommendationServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if transport_provided:
            ...
            self._transport = cast(RecommendationServiceTransport, transport)
            self._api_endpoint = self._transport.host

        self._api_endpoint = (self._api_endpoint or
            RecommendationServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[RecommendationServiceTransport], Callable[..., RecommendationServiceTransport]] = (
    ...
    def recommend(self,
            request: Optional[Union[recommendation_service.RecommendRequest, dict]] = None,
            *,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: Union[float, object] = gapic_v1.method.DEFAULT,
            metadata: Sequence[Tuple[str, str]] = (),
            ) -> recommendation_service.RecommendResponse:
        r"""Makes a recommendation, which requires a contextual
        user event.
        ...
        Returns:
            google.cloud.discoveryengine_v1beta.types.RecommendResponse:
                Response message for Recommend
                method.
        ...
        if not isinstance(request, recommendation_service.RecommendRequest):
            request = recommendation_service.RecommendRequest(request)

        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.recommend]
