...
def test_generate_attached_cluster_agent_token_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = attached_service.GenerateAttachedClusterAgentTokenResponse.to_json(attached_service.GenerateAttachedClusterAgentTokenResponse())
...
def test_generate_attached_cluster_agent_token_rest_bad_request(transport: str = 'rest', request_type=attached_service.GenerateAttachedClusterAgentTokenRequest):
    client = AttachedClustersClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
