...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert BudgetServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert BudgetServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert BudgetServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert BudgetServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert BudgetServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert BudgetServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert BudgetServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(BudgetServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BudgetServiceClient))
@mock.patch.object(BudgetServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BudgetServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = BudgetServiceClient._DEFAULT_UNIVERSE
    default_endpoint = BudgetServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = BudgetServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert BudgetServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert BudgetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == BudgetServiceClient.DEFAULT_MTLS_ENDPOINT
    assert BudgetServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert BudgetServiceClient._get_api_endpoint(None, None, default_universe, "always") == BudgetServiceClient.DEFAULT_MTLS_ENDPOINT
    assert BudgetServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == BudgetServiceClient.DEFAULT_MTLS_ENDPOINT
    assert BudgetServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert BudgetServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        BudgetServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert BudgetServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert BudgetServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert BudgetServiceClient._get_universe_domain(None, None) == BudgetServiceClient._DEFAULT_UNIVERSE
...
def test_budget_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class", [
    BudgetServiceClient, BudgetServiceAsyncClient
])
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (BudgetServiceClient, transports.BudgetServiceGrpcTransport, "grpc", grpc_helpers),
    (BudgetServiceAsyncClient, transports.BudgetServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (BudgetServiceClient, transports.BudgetServiceRestTransport, "rest", None),
])
def test_budget_service_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_budget_service_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
@pytest.mark.asyncio
async def test_create_budget_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.create_budget in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_budget] = mock_rpc
...
@pytest.mark.asyncio
async def test_update_budget_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BudgetServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.update_budget in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.update_budget] = mock_rpc
...
async def test_get_budget_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.get_budget in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_budget] = mock_rpc
...
@pytest.mark.asyncio
async def test_list_budgets_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BudgetServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_budgets in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_budgets] = mock_rpc
...
@pytest.mark.asyncio
async def test_delete_budget_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BudgetServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_budget in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_budget] = mock_rpc
...
def test_create_budget_rest_required_fields(request_type=budget_service.CreateBudgetRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_budget._get_unset_required_fields(jsonified_request)
...
def test_create_budget_rest_unset_required_fields():
    transport = transports.BudgetServiceRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.create_budget._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("parent", "budget", )))
...
def test_create_budget_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/{parent=billingAccounts/*}/budgets" % client.transport._host, args[1])
...
def test_update_budget_rest_required_fields(request_type=budget_service.UpdateBudgetRequest):
    transport_class = transports.BudgetServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_budget._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_budget_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{budget.name=billingAccounts/*/budgets/*}" % client.transport._host, args[1])
...
def test_get_budget_rest_required_fields(request_type=budget_service.GetBudgetRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_budget._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_budget_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=billingAccounts/*/budgets/*}" % client.transport._host, args[1])
...
def test_list_budgets_rest_required_fields(request_type=budget_service.ListBudgetsRequest):
    transport_class = transports.BudgetServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_budgets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_budgets._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("page_size", "page_token", "scope", ))
    jsonified_request.update(unset_fields)
...
def test_list_budgets_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("pageSize", "pageToken", "scope", )) & set(("parent", )))
...
def test_list_budgets_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        response = tuple(budget_service.ListBudgetsResponse.to_json(x) for x in response)
...
def test_delete_budget_rest_required_fields(request_type=budget_service.DeleteBudgetRequest):
    transport_class = transports.BudgetServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_budget._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_create_budget_rest_bad_request(request_type=budget_service.CreateBudgetRequest):
    client = BudgetServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_create_budget_rest_interceptors(null_interceptor):
    transport = transports.BudgetServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.BudgetServiceRestInterceptor(),
        )
    client = BudgetServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_create_budget") as post, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_create_budget_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "pre_create_budget") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = budget_service.CreateBudgetRequest.pb(budget_service.CreateBudgetRequest())
        ...
        client.create_budget(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_update_budget_rest_bad_request(request_type=budget_service.UpdateBudgetRequest):
    client = BudgetServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_update_budget_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'budget': {'name': 'billingAccounts/sample1/budgets/sample2'}}
    request_init["budget"] = {'name': 'billingAccounts/sample1/budgets/sample2', 'display_name': 'display_name_value', 'budget_filter': {'projects': ['projects_value1', 'projects_value2'], 'resource_ancestors': ['resource_ancestors_value1', 'resource_ancestors_value2'], 'credit_types': ['credit_types_value1', 'credit_types_value2'], 'credit_types_treatment': 1, 'services': ['services_value1', 'services_value2'], 'subaccounts': ['subaccounts_value1', 'subaccounts_value2'], 'labels': {}, 'calendar_period': 1, 'custom_period': {'start_date': {'year': 433, 'month': 550, 'day': 318}, 'end_date': {}}}, 'amount': {'specified_amount': {'currency_code': 'currency_code_value', 'units': 563, 'nanos': 543}, 'last_period_amount': {}}, 'threshold_rules': [{'threshold_percent': 0.1821, 'spend_basis': 1}], 'notifications_rule': {'pubsub_topic': 'pubsub_topic_value', 'schema_version': 'schema_version_value', 'monitoring_notification_channels': ['monitoring_notification_channels_value1', 'monitoring_notification_channels_value2'], 'disable_default_iam_recipients': True, 'enable_project_level_recipients': True}, 'etag': 'etag_value'}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_update_budget_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_update_budget") as post, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_update_budget_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "pre_update_budget") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = budget_service.UpdateBudgetRequest.pb(budget_service.UpdateBudgetRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.update_budget(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_get_budget_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_get_budget") as post, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_get_budget_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "pre_get_budget") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = budget_service.GetBudgetRequest.pb(budget_service.GetBudgetRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.get_budget(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_list_budgets_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_list_budgets") as post, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "post_list_budgets_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "pre_list_budgets") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = budget_service.ListBudgetsRequest.pb(budget_service.ListBudgetsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = budget_service.ListBudgetsResponse.to_json(budget_service.ListBudgetsResponse())
        req.return_value.content = return_value
        ...
        client.list_budgets(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_delete_budget_rest_bad_request(request_type=budget_service.DeleteBudgetRequest):
    client = BudgetServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_budget_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.BudgetServiceRestInterceptor, "pre_delete_budget") as pre:
        pre.assert_not_called()
        pb_message = budget_service.DeleteBudgetRequest.pb(budget_service.DeleteBudgetRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.delete_budget(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_budget_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.billing.budgets_v1.services.budget_service.transports.BudgetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_budget_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.billing.budgets_v1.services.budget_service.transports.BudgetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.BudgetServiceTransport()
        adc.assert_called_once()
