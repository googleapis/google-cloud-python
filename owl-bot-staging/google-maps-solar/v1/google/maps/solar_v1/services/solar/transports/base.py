...
class SolarTransport(abc.ABC):
    ...
    DEFAULT_HOST: str = 'solar.googleapis.com'
    def __init__(
            self, *,
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.find_closest_building_insights: gapic_v1.method.wrap_method(
                self.find_closest_building_insights,
...
initial=1.0,maximum=10.0,multiplier=1.3,                    predicate=retries.if_exception_type(
            ...
            self.get_data_layers: gapic_v1.method.wrap_method(
...
initial=1.0,maximum=10.0,multiplier=1.3,                    predicate=retries.if_exception_type(
            ...
            self.get_geo_tiff: gapic_v1.method.wrap_method(
                self.get_geo_tiff,
                default_retry=retries.Retry(
initial=1.0,maximum=10.0,multiplier=1.3,                    predicate=retries.if_exception_type(
                        core_exceptions.ServiceUnavailable,
    ...
    @property
    def find_closest_building_insights(self) -> Callable[
            [solar_service.FindClosestBuildingInsightsRequest],
            Union[
                solar_service.BuildingInsights,
                Awaitable[solar_service.BuildingInsights]
            ]]:
        raise NotImplementedError()

    @property
    def get_data_layers(self) -> Callable[
            [solar_service.GetDataLayersRequest],
            Union[
                solar_service.DataLayers,
                Awaitable[solar_service.DataLayers]
            ]]:
        raise NotImplementedError()

    @property
    def get_geo_tiff(self) -> Callable[
