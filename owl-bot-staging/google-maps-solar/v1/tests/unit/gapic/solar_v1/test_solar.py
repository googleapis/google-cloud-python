...
from google.maps.solar_v1.services.solar import SolarAsyncClient
from google.maps.solar_v1.services.solar import SolarClient
from google.maps.solar_v1.services.solar import transports
...
def test__get_default_mtls_endpoint():
    ...
    assert SolarClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert SolarClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert SolarClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__get_client_cert_source():
    ...
    assert SolarClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert SolarClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert SolarClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(SolarClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SolarClient))
@mock.patch.object(SolarAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SolarAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = SolarClient._DEFAULT_UNIVERSE
    default_endpoint = SolarClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = SolarClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert SolarClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert SolarClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == SolarClient.DEFAULT_MTLS_ENDPOINT
    assert SolarClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert SolarClient._get_api_endpoint(None, None, default_universe, "always") == SolarClient.DEFAULT_MTLS_ENDPOINT
    assert SolarClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == SolarClient.DEFAULT_MTLS_ENDPOINT
    assert SolarClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert SolarClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        SolarClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert SolarClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert SolarClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = SolarClient(credentials=cred)
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    ...
    (SolarAsyncClient, transports.SolarGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (SolarClient, transports.SolarGrpcTransport, "grpc", "false"),
    (SolarAsyncClient, transports.SolarGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_solar_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [
    SolarClient, SolarAsyncClient
])
@mock.patch.object(SolarClient, "DEFAULT_ENDPOINT", modify_default_endpoint(SolarClient))
@mock.patch.object(SolarAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(SolarAsyncClient))
...
def test_solar_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    ...
    (SolarAsyncClient, transports.SolarGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (SolarClient, transports.SolarRestTransport, "rest", None),
])
def test_solar_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_solar_client_client_options_from_dict():
    with mock.patch('google.maps.solar_v1.services.solar.transports.SolarGrpcTransport.__init__') as grpc_transport:
...
def test_solar_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_find_closest_building_insights(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.find_closest_building_insights),
            '__call__') as call:
...
def test_find_closest_building_insights_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.find_closest_building_insights in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.find_closest_building_insights] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_find_closest_building_insights_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.find_closest_building_insights in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.find_closest_building_insights] = mock_rpc
...
@pytest.mark.asyncio
async def test_find_closest_building_insights_async(transport: str = 'grpc_asyncio', request_type=solar_service.FindClosestBuildingInsightsRequest):
    client = SolarAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(solar_service.BuildingInsights(
            name='name_value',
            postal_code='postal_code_value',
            administrative_area='administrative_area_value',
            statistical_area='statistical_area_value',
...
def test_get_data_layers(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_data_layers),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_get_data_layers_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = SolarAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_data_layers in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_data_layers] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_data_layers_async(transport: str = 'grpc_asyncio', request_type=solar_service.GetDataLayersRequest):
    client = SolarAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(solar_service.DataLayers(
            dsm_url='dsm_url_value',
            rgb_url='rgb_url_value',
            mask_url='mask_url_value',
            annual_flux_url='annual_flux_url_value',
...
def test_get_geo_tiff(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_geo_tiff),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_get_geo_tiff_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = SolarAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_geo_tiff in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_geo_tiff] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_geo_tiff_async(transport: str = 'grpc_asyncio', request_type=solar_service.GetGeoTiffRequest):
    client = SolarAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_find_closest_building_insights_rest_required_fields(request_type=solar_service.FindClosestBuildingInsightsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).find_closest_building_insights._get_unset_required_fields(jsonified_request)
    ...
    assert not set(unset_fields) - set(("exact_quality_required", "experiments", "location", "required_quality", ))
    jsonified_request.update(unset_fields)
...
def test_find_closest_building_insights_rest_unset_required_fields():
    transport = transports.SolarRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.find_closest_building_insights._get_unset_required_fields({})
    assert set(unset_fields) == (set(("exactQualityRequired", "experiments", "location", "requiredQuality", )) & set(("location", )))
...
def test_get_data_layers_rest_required_fields(request_type=solar_service.GetDataLayersRequest):
    transport_class = transports.SolarRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_data_layers._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_data_layers._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("exact_quality_required", "experiments", "location", "pixel_size_meters", "radius_meters", "required_quality", "view", ))
    jsonified_request.update(unset_fields)
...
def test_get_data_layers_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("exactQualityRequired", "experiments", "location", "pixelSizeMeters", "radiusMeters", "requiredQuality", "view", )) & set(("location", "radiusMeters", )))
...
def test_get_geo_tiff_rest_required_fields(request_type=solar_service.GetGeoTiffRequest):
    transport_class = transports.SolarRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_geo_tiff._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_find_closest_building_insights_empty_call_grpc():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = solar_service.BuildingInsights()
        client.find_closest_building_insights(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_data_layers_empty_call_grpc():
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = solar_service.DataLayers()
        client.get_data_layers(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_geo_tiff_empty_call_grpc():
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = httpbody_pb2.HttpBody()
        client.get_geo_tiff(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = SolarAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_find_closest_building_insights_empty_call_grpc_asyncio():
    client = SolarAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.find_closest_building_insights(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_data_layers_empty_call_grpc_asyncio():
    client = SolarAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_data_layers(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_geo_tiff_empty_call_grpc_asyncio():
    client = SolarAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_geo_tiff(request=None)
...
def test_transport_kind_rest():
    transport = SolarClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
    assert transport.kind == "rest"


def test_find_closest_building_insights_rest_bad_request(request_type=solar_service.FindClosestBuildingInsightsRequest):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_find_closest_building_insights_rest_call_success(request_type):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_find_closest_building_insights_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.SolarRestInterceptor, "post_find_closest_building_insights") as post, \
        mock.patch.object(transports.SolarRestInterceptor, "post_find_closest_building_insights_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SolarRestInterceptor, "pre_find_closest_building_insights") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = solar_service.FindClosestBuildingInsightsRequest.pb(solar_service.FindClosestBuildingInsightsRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = solar_service.BuildingInsights.to_json(solar_service.BuildingInsights())
        ...
        post_with_metadata.return_value = solar_service.BuildingInsights(), metadata

        client.find_closest_building_insights(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_get_data_layers_rest_bad_request(request_type=solar_service.GetDataLayersRequest):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_data_layers_rest_call_success(request_type):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_data_layers_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.SolarRestInterceptor, "post_get_data_layers") as post, \
        mock.patch.object(transports.SolarRestInterceptor, "post_get_data_layers_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SolarRestInterceptor, "pre_get_data_layers") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = solar_service.GetDataLayersRequest.pb(solar_service.GetDataLayersRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        post_with_metadata.return_value = solar_service.DataLayers(), metadata

        client.get_data_layers(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_get_geo_tiff_rest_bad_request(request_type=solar_service.GetGeoTiffRequest):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_geo_tiff_rest_call_success(request_type):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_geo_tiff_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.SolarRestInterceptor, "post_get_geo_tiff_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SolarRestInterceptor, "pre_get_geo_tiff") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = solar_service.GetGeoTiffRequest.pb(solar_service.GetGeoTiffRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.get_geo_tiff(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_find_closest_building_insights_empty_call_rest():
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_data_layers_empty_call_rest():
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_geo_tiff_empty_call_rest():
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_solar_base_transport():
    ...
    with mock.patch('google.maps.solar_v1.services.solar.transports.SolarTransport.__init__') as Transport:
...
def test_solar_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.maps.solar_v1.services.solar.transports.SolarTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_solar_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.maps.solar_v1.services.solar.transports.SolarTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.SolarTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.SolarGrpcTransport, transports.SolarGrpcAsyncIOTransport])
def test_solar_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_solar_host_no_port(transport_name):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='solar.googleapis.com'),
...
def test_solar_host_with_port(transport_name):
    client = SolarClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='solar.googleapis.com:8000'),
        transport=transport_name,
    )
    assert client.transport._host == (
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.SolarGrpcTransport, transports.SolarGrpcAsyncIOTransport])
def test_solar_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
