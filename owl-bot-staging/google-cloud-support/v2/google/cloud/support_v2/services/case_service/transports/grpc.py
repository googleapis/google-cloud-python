...
from .base import CaseServiceTransport, DEFAULT_CLIENT_INFO
...
class CaseServiceGrpcTransport(CaseServiceTransport):
    ...
    @property
    def search_cases(self) -> Callable[
    ...
    @property
    def escalate_case(self) -> Callable[
            [case_service.EscalateCaseRequest],
            case.Case]:
        r"""Return a callable for the escalate case method over gRPC.

        Escalate a case, starting the Google Cloud Support escalation
        management process.
    ...
    @property
    def search_case_classifications(self) -> Callable[
        ...
        if 'search_case_classifications' not in self._stubs:
            self._stubs['search_case_classifications'] = self._logged_channel.unary_unary(
