...
@pytest.mark.parametrize("client_class", [
    SpeechClient, SpeechAsyncClient
])
...
async def test_recognize_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        client._client._transport._wrapped_methods[client._client._transport.recognize] = mock_object
...
def test_long_running_recognize_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = SpeechClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.asyncio
async def test_long_running_recognize_empty_call_async():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = SpeechAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        class AwaitableMock(mock.AsyncMock):
            def __await__(self):
                ...
                return iter([])
        mock_object = AwaitableMock()
        client._client._transport._wrapped_methods[client._client._transport.long_running_recognize] = mock_object
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        class AwaitableMock(mock.AsyncMock):
            def __await__(self):
                ...
                return iter([])
        mock_object = AwaitableMock()
        client._client._transport._wrapped_methods[client._client._transport.streaming_recognize] = mock_object
...
def test_recognize_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("config", "audio", )))
...
def test_recognize_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value._content = cloud_speech.RecognizeResponse.to_json(cloud_speech.RecognizeResponse())
        ...
        client.recognize(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_recognize_rest_bad_request(transport: str = 'rest', request_type=cloud_speech.RecognizeRequest):
    client = SpeechClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_recognize_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/speech:recognize" % client.transport._host, args[1])
...
def test_long_running_recognize_rest(request_type):
    client = SpeechClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_long_running_recognize_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = SpeechClient(
...
def test_long_running_recognize_rest_required_fields(request_type=cloud_speech.LongRunningRecognizeRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).long_running_recognize._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_long_running_recognize_rest_interceptors(null_interceptor):
    transport = transports.SpeechRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.SpeechRestInterceptor(),
        )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.SpeechRestInterceptor, "post_long_running_recognize") as post, \
         mock.patch.object(transports.SpeechRestInterceptor, "pre_long_running_recognize") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        client.long_running_recognize(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_long_running_recognize_rest_bad_request(transport: str = 'rest', request_type=cloud_speech.LongRunningRecognizeRequest):
    client = SpeechClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_long_running_recognize_rest_flattened():
    client = SpeechClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        client.long_running_recognize(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/speech:longrunningrecognize" % client.transport._host, args[1])
...
def test_long_running_recognize_rest_error():
    client = SpeechClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport='rest'
    )
...
def test_speech_client_transport_session_collision(transport_name):
    ...
    session1 = client1.transport.long_running_recognize._session
    session2 = client2.transport.long_running_recognize._session
    assert session1 != session2
