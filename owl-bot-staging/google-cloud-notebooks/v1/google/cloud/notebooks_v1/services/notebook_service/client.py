...
class NotebookServiceClient(metaclass=NotebookServiceClientMeta):
    ...
    def execution_path(project: str,location: str,execution: str,) -> str:
        ...
        return "projects/{project}/location/{location}/executions/{execution}".format(project=project, location=location, execution=execution, )

    @staticmethod
    def parse_execution_path(path: str) -> Dict[str,str]:
        """Parses a execution path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/location/(?P<location>.+?)/executions/(?P<execution>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def schedule_path(project: str,location: str,schedule: str,) -> str:
        ...
        return "projects/{project}/location/{location}/schedules/{schedule}".format(project=project, location=location, schedule=schedule, )

    @staticmethod
    def parse_schedule_path(path: str) -> Dict[str,str]:
        """Parses a schedule path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/location/(?P<location>.+?)/schedules/(?P<schedule>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def __init__(self, *,
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.notebooks_v1.NotebookServiceClient`.",
                    extra = {
    ...
    def update_instance_metadata_items(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.update_instance_metadata_items]
    ...
    def diagnose_instance(self,
        ...
        Args:
            request (Union[google.cloud.notebooks_v1.types.DiagnoseInstanceRequest, dict]):
                The request object. Request for creating a notebook
