...
class IdentityAwareProxyAdminServiceClient(metaclass=IdentityAwareProxyAdminServiceClientMeta):
    ...
    def tunnel_dest_group_path(project: str,location: str,dest_group: str,) -> str:
        ...
        return "projects/{project}/iap_tunnel/locations/{location}/destGroups/{dest_group}".format(project=project, location=location, dest_group=dest_group, )

    @staticmethod
    def parse_tunnel_dest_group_path(path: str) -> Dict[str,str]:
        """Parses a tunnel_dest_group path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/iap_tunnel/locations/(?P<location>.+?)/destGroups/(?P<dest_group>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def tunnel_location_path(project: str,location: str,) -> str:
        """Returns a fully-qualified tunnel_location string."""
        return "projects/{project}/iap_tunnel/locations/{location}".format(project=project, location=location, )

    @staticmethod
    def parse_tunnel_location_path(path: str) -> Dict[str,str]:
        """Parses a tunnel_location path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/iap_tunnel/locations/(?P<location>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = IdentityAwareProxyAdminServiceClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = IdentityAwareProxyAdminServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = IdentityAwareProxyAdminServiceClient._DEFAULT_UNIVERSE
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, IdentityAwareProxyAdminServiceTransport, Callable[..., IdentityAwareProxyAdminServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = IdentityAwareProxyAdminServiceClient._read_environment_variables()
        self._client_cert_source = IdentityAwareProxyAdminServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = IdentityAwareProxyAdminServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, IdentityAwareProxyAdminServiceTransport)
        if transport_provided:
            ...
            self._transport = cast(IdentityAwareProxyAdminServiceTransport, transport)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[IdentityAwareProxyAdminServiceTransport], Callable[..., IdentityAwareProxyAdminServiceTransport]] = (
                IdentityAwareProxyAdminServiceClient.get_transport_class(transport)
                if isinstance(transport, str) or transport is None
                else cast(Callable[..., IdentityAwareProxyAdminServiceTransport], transport)
            )
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.iap_v1.IdentityAwareProxyAdminServiceClient`.",
                    extra = {
    ...
    def validate_iap_attribute_expression(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.validate_iap_attribute_expression]
    ...
    def list_tunnel_dest_groups(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.list_tunnel_dest_groups]
        ...
        response = pagers.ListTunnelDestGroupsPager(
            method=rpc,
            request=request,
            response=response,
            retry=retry,
    ...
    def create_tunnel_dest_group(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.create_tunnel_dest_group]
    ...
    def get_tunnel_dest_group(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.get_tunnel_dest_group]
    ...
    def delete_tunnel_dest_group(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.delete_tunnel_dest_group]
    ...
    def update_tunnel_dest_group(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.update_tunnel_dest_group]
