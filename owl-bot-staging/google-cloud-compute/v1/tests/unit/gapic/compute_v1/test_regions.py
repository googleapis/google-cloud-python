...
def test_get_rest_unset_required_fields():
    transport = transports.RegionsRestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)
...
def test_get_rest_bad_request(transport: str = 'rest', request_type=compute.GetRegionRequest):
    client = RegionsClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_list_rest_bad_request(transport: str = 'rest', request_type=compute.ListRegionsRequest):
    client = RegionsClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_list_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert all(isinstance(i, compute.Region)
                for i in results)
...
def test_regions_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.regions.transports.RegionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_regions_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.regions.transports.RegionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (_AnonymousCredentialsWithUniverseDomain(), None)
        transport = transports.RegionsTransport()
        adc.assert_called_once()
