...
def test_set_cloud_armor_tier_rest_bad_request(transport: str = 'rest', request_type=compute.SetCloudArmorTierProjectRequest):
    client = ProjectsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_set_cloud_armor_tier_unary_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        client.set_cloud_armor_tier_unary(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_set_cloud_armor_tier_unary_rest_bad_request(transport: str = 'rest', request_type=compute.SetCloudArmorTierProjectRequest):
    client = ProjectsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
