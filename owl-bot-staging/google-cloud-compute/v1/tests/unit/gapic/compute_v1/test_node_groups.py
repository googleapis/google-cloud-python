...
def test__read_environment_variables():
    assert NodeGroupsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert NodeGroupsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert NodeGroupsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert NodeGroupsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert NodeGroupsClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert NodeGroupsClient._get_client_cert_source(None, False) is None
    assert NodeGroupsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert NodeGroupsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert NodeGroupsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert NodeGroupsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(NodeGroupsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(NodeGroupsClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = NodeGroupsClient._DEFAULT_UNIVERSE
    default_endpoint = NodeGroupsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = NodeGroupsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert NodeGroupsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert NodeGroupsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == NodeGroupsClient.DEFAULT_MTLS_ENDPOINT
    assert NodeGroupsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert NodeGroupsClient._get_api_endpoint(None, None, default_universe, "always") == NodeGroupsClient.DEFAULT_MTLS_ENDPOINT
    assert NodeGroupsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == NodeGroupsClient.DEFAULT_MTLS_ENDPOINT
    assert NodeGroupsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert NodeGroupsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        NodeGroupsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert NodeGroupsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert NodeGroupsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert NodeGroupsClient._get_universe_domain(None, None) == NodeGroupsClient._DEFAULT_UNIVERSE
...
@mock.patch.object(NodeGroupsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(NodeGroupsClient))
def test_node_groups_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (NodeGroupsClient, transports.NodeGroupsRestTransport, "rest", None),
])
...
def test_add_nodes_rest_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.add_nodes in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.add_nodes] = mock_rpc
...
def test_delete_nodes_rest_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_nodes in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_nodes] = mock_rpc
...
def test_perform_maintenance_rest_required_fields(request_type=compute.PerformMaintenanceNodeGroupRequest):
    transport_class = transports.NodeGroupsRestTransport
...
def test_perform_maintenance_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("requestId", )) & set(("nodeGroup", "nodeGroupsPerformMaintenanceRequestResource", "project", "zone", )))
...
def test_perform_maintenance_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        mock_args = dict(
            ...
            zone='zone_value',
            node_group='node_group_value',
            node_groups_perform_maintenance_request_resource=compute.NodeGroupsPerformMaintenanceRequest(nodes=['nodes_value']),
        )
        mock_args.update(sample_request)
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/v1/projects/{project}/zones/{zone}/nodeGroups/{node_group}/performMaintenance" % client.transport._host, args[1])
...
def test_perform_maintenance_unary_rest_required_fields(request_type=compute.PerformMaintenanceNodeGroupRequest):
    transport_class = transports.NodeGroupsRestTransport
...
def test_set_node_template_rest_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.set_node_template] = mock_rpc
...
def test_simulate_maintenance_event_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        mock_args = dict(
            ...
            zone='zone_value',
            node_group='node_group_value',
            node_groups_simulate_maintenance_event_request_resource=compute.NodeGroupsSimulateMaintenanceEventRequest(nodes=['nodes_value']),
        )
        mock_args.update(sample_request)
...
def test_add_nodes_rest_call_success(request_type):
    ...
    for field, value in request_init["node_groups_add_nodes_request_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_add_nodes_rest_interceptors(null_interceptor):
    transport = transports.NodeGroupsRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.NodeGroupsRestInterceptor(),
        )
    client = NodeGroupsClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_add_nodes") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_add_nodes_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_add_nodes") as pre:
...
def test_aggregated_list_rest_bad_request(request_type=compute.AggregatedListNodeGroupsRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_aggregated_list") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_aggregated_list_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_aggregated_list") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.NodeGroupAggregatedList.to_json(compute.NodeGroupAggregatedList())
        req.return_value.content = return_value
...
def test_delete_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_delete") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_delete_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_delete") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_delete_nodes_rest_bad_request(request_type=compute.DeleteNodesNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_nodes_rest_call_success(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["node_groups_delete_nodes_request_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_delete_nodes_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_delete_nodes") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_delete_nodes_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_delete_nodes") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_get") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_get_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_get") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_iam_policy_rest_bad_request(request_type=compute.GetIamPolicyNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_get_iam_policy") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_get_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_get_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_insert_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'project': 'sample1', 'zone': 'sample2'}
    request_init["node_group_resource"] = {'autoscaling_policy': {'max_nodes': 958, 'min_nodes': 956, 'mode': 'mode_value'}, 'creation_timestamp': 'creation_timestamp_value', 'description': 'description_value', 'fingerprint': 'fingerprint_value', 'id': 205, 'kind': 'kind_value', 'location_hint': 'location_hint_value', 'maintenance_interval': 'maintenance_interval_value', 'maintenance_policy': 'maintenance_policy_value', 'maintenance_window': {'maintenance_duration': {'nanos': 543, 'seconds': 751}, 'start_time': 'start_time_value'}, 'name': 'name_value', 'node_template': 'node_template_value', 'self_link': 'self_link_value', 'share_settings': {'project_map': {}, 'share_type': 'share_type_value'}, 'size': 443, 'status': 'status_value', 'zone': 'zone_value'}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_insert") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_insert_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_insert") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_list") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_list_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_list") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_list_nodes_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_list_nodes") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_list_nodes_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_list_nodes") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.NodeGroupsListNodes.to_json(compute.NodeGroupsListNodes())
        req.return_value.content = return_value
...
def test_patch_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_patch") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_patch_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_patch") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_perform_maintenance_rest_bad_request(request_type=compute.PerformMaintenanceNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_perform_maintenance_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'project': 'sample1', 'zone': 'sample2', 'node_group': 'sample3'}
    request_init["node_groups_perform_maintenance_request_resource"] = {'nodes': ['nodes_value1', 'nodes_value2'], 'start_time': 'start_time_value'}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748

    # Determine if the message type is proto-plus or protobuf
    test_field = compute.PerformMaintenanceNodeGroupRequest.meta.fields["node_groups_perform_maintenance_request_resource"]
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["node_groups_perform_maintenance_request_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
    ...
    for subfield_to_delete in subfields_not_in_runtime: # pragma: NO COVER
        ...
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init["node_groups_perform_maintenance_request_resource"][field])):
                    del request_init["node_groups_perform_maintenance_request_resource"][field][i][subfield]
            else:
                del request_init["node_groups_perform_maintenance_request_resource"][field][subfield]
    request = request_type(**request_init)
...
def test_perform_maintenance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_perform_maintenance") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_perform_maintenance_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_perform_maintenance") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.PerformMaintenanceNodeGroupRequest.pb(compute.PerformMaintenanceNodeGroupRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_set_iam_policy_rest_bad_request(request_type=compute.SetIamPolicyNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_set_iam_policy") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_set_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_set_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_set_node_template_rest_bad_request(request_type=compute.SetNodeTemplateNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_set_node_template_rest_call_success(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["node_groups_set_node_template_request_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_set_node_template_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_set_node_template") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_set_node_template_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_set_node_template") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_simulate_maintenance_event_rest_bad_request(request_type=compute.SimulateMaintenanceEventNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_simulate_maintenance_event_rest_call_success(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    # Add `# pragma: NO COVER` because this test code will not run if all subfields are present at runtime
    for field, value in request_init["node_groups_simulate_maintenance_event_request_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_simulate_maintenance_event_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_simulate_maintenance_event") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_simulate_maintenance_event_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_simulate_maintenance_event") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.NodeGroupsRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_add_nodes_unary_empty_call_rest():
    ...
    with mock.patch.object(
            type(client.transport.add_nodes),
            '__call__') as call:
...
def test_delete_nodes_unary_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
            type(client.transport.delete_nodes),
            '__call__') as call:
...
def test_set_node_template_unary_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
            type(client.transport.set_node_template),
            '__call__') as call:
...
def test_node_groups_client_transport_session_collision(transport_name):
    ...
    client1 = NodeGroupsClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = NodeGroupsClient(
        credentials=creds2,
        transport=transport_name,
    )
...
@pytest.mark.parametrize("client_class,transport_class", [
    (NodeGroupsClient, transports.NodeGroupsRestTransport),
])
