...
def test_perform_maintenance_rest_bad_request(transport: str = 'rest', request_type=compute.PerformMaintenanceNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_perform_maintenance_unary_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        client.perform_maintenance_unary(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_perform_maintenance_unary_rest_bad_request(transport: str = 'rest', request_type=compute.PerformMaintenanceNodeGroupRequest):
    client = NodeGroupsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
