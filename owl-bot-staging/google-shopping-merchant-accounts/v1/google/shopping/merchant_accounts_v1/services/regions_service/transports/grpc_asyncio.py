...
from .base import RegionsServiceTransport, DEFAULT_CLIENT_INFO
...
class RegionsServiceGrpcAsyncIOTransport(RegionsServiceTransport):
    """gRPC AsyncIO backend transport for RegionsService.
    ...
    @property
    def get_region(self) -> Callable[
            [regions.GetRegionRequest],
            Awaitable[regions.Region]]:
        r"""Return a callable for the get region method over gRPC.

        Retrieves a region defined in your Merchant Center
        account.

        Returns:
            Callable[[~.GetRegionRequest],
                    Awaitable[~.Region]]:
        ...
        if 'get_region' not in self._stubs:
            self._stubs['get_region'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.RegionsService/GetRegion',
                request_serializer=regions.GetRegionRequest.serialize,
                response_deserializer=regions.Region.deserialize,
            )
        return self._stubs['get_region']

    @property
    def create_region(self) -> Callable[
            [regions.CreateRegionRequest],
            Awaitable[regions.Region]]:
        r"""Return a callable for the create region method over gRPC.

        Creates a region definition in your Merchant Center
        account. Executing this method requires admin access.

        Returns:
            Callable[[~.CreateRegionRequest],
                    Awaitable[~.Region]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'create_region' not in self._stubs:
            self._stubs['create_region'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.RegionsService/CreateRegion',
                request_serializer=regions.CreateRegionRequest.serialize,
                response_deserializer=regions.Region.deserialize,
            )
        return self._stubs['create_region']

    @property
    def batch_create_regions(self) -> Callable[
        ...
        Returns:
            Callable[[~.BatchCreateRegionsRequest],
                    Awaitable[~.BatchCreateRegionsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    @property
    def update_region(self) -> Callable[
            [regions.UpdateRegionRequest],
            Awaitable[regions.Region]]:
        r"""Return a callable for the update region method over gRPC.

        Updates a region definition in your Merchant Center
        account. Executing this method requires admin access.

        Returns:
            Callable[[~.UpdateRegionRequest],
                    Awaitable[~.Region]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'update_region' not in self._stubs:
            self._stubs['update_region'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.RegionsService/UpdateRegion',
                request_serializer=regions.UpdateRegionRequest.serialize,
                response_deserializer=regions.Region.deserialize,
            )
        return self._stubs['update_region']

    @property
    def batch_update_regions(self) -> Callable[
        ...
        Returns:
            Callable[[~.BatchUpdateRegionsRequest],
                    Awaitable[~.BatchUpdateRegionsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    @property
    def delete_region(self) -> Callable[
            [regions.DeleteRegionRequest],
            Awaitable[empty_pb2.Empty]]:
        r"""Return a callable for the delete region method over gRPC.

        Deletes a region definition from your Merchant Center
        account. Executing this method requires admin access.

        Returns:
            Callable[[~.DeleteRegionRequest],
                    Awaitable[~.Empty]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'delete_region' not in self._stubs:
            self._stubs['delete_region'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.RegionsService/DeleteRegion',
                request_serializer=regions.DeleteRegionRequest.serialize,
                response_deserializer=empty_pb2.Empty.FromString,
            )
        return self._stubs['delete_region']

    @property
    def batch_delete_regions(self) -> Callable[
    ...
    def list_regions(self) -> Callable[
            ...
            Awaitable[regions.ListRegionsResponse]]:
        ...
        if 'list_regions' not in self._stubs:
            self._stubs['list_regions'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.RegionsService/ListRegions',
                request_serializer=regions.ListRegionsRequest.serialize,
                response_deserializer=regions.ListRegionsResponse.deserialize,
            )
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.get_region: self._wrap_method(
                self.get_region,
                default_timeout=None,
                client_info=client_info,
            ),
            self.create_region: self._wrap_method(
                self.create_region,
                default_timeout=None,
                client_info=client_info,
            ),
            self.batch_create_regions: self._wrap_method(
                self.batch_create_regions,
                default_timeout=None,
                client_info=client_info,
            ),
            self.update_region: self._wrap_method(
                self.update_region,
                default_timeout=None,
                client_info=client_info,
            ),
            self.batch_update_regions: self._wrap_method(
                self.batch_update_regions,
                default_timeout=None,
                client_info=client_info,
            ),
            self.delete_region: self._wrap_method(
                self.delete_region,
                default_timeout=None,
                client_info=client_info,
            ),
            self.batch_delete_regions: self._wrap_method(
                self.batch_delete_regions,
                default_timeout=None,
                client_info=client_info,
            ),
            self.list_regions: self._wrap_method(
                self.list_regions,
                default_timeout=None,
                client_info=client_info,
            ),
