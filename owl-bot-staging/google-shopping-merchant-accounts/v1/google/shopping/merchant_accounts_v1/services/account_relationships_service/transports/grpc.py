...
from .base import AccountRelationshipsServiceTransport, DEFAULT_CLIENT_INFO
...
class AccountRelationshipsServiceGrpcTransport(AccountRelationshipsServiceTransport):
    """gRPC backend transport for AccountRelationshipsService.
    ...
    @property
    def get_account_relationship(self) -> Callable[
            [accountrelationships.GetAccountRelationshipRequest],
            accountrelationships.AccountRelationship]:
        r"""Return a callable for the get account relationship method over gRPC.

        Retrieve an account relationship.

        Returns:
            Callable[[~.GetAccountRelationshipRequest],
                    ~.AccountRelationship]:
        ...
        if 'get_account_relationship' not in self._stubs:
            self._stubs['get_account_relationship'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.AccountRelationshipsService/GetAccountRelationship',
                request_serializer=accountrelationships.GetAccountRelationshipRequest.serialize,
                response_deserializer=accountrelationships.AccountRelationship.deserialize,
            )
        return self._stubs['get_account_relationship']

    @property
    def update_account_relationship(self) -> Callable[
            [accountrelationships.UpdateAccountRelationshipRequest],
            accountrelationships.AccountRelationship]:
        r"""Return a callable for the update account relationship method over gRPC.

        Updates the account relationship. Executing this
        method requires admin access.

        Returns:
            Callable[[~.UpdateAccountRelationshipRequest],
                    ~.AccountRelationship]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'update_account_relationship' not in self._stubs:
            self._stubs['update_account_relationship'] = self._logged_channel.unary_unary(
    ...
    def list_account_relationships(self) -> Callable[
        ...
        if 'list_account_relationships' not in self._stubs:
            self._stubs['list_account_relationships'] = self._logged_channel.unary_unary(
