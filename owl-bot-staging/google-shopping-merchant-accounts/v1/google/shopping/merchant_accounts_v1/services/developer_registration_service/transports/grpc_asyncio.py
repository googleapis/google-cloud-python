...
class DeveloperRegistrationServiceGrpcAsyncIOTransport(DeveloperRegistrationServiceTransport):
    ...
    @property
    def register_gcp(self) -> Callable[
        ...
        Returns:
            Callable[[~.RegisterGcpRequest],
                    Awaitable[~.DeveloperRegistration]]:
        ...
        if 'register_gcp' not in self._stubs:
            self._stubs['register_gcp'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.DeveloperRegistrationService/RegisterGcp',
                request_serializer=developerregistration.RegisterGcpRequest.serialize,
                response_deserializer=developerregistration.DeveloperRegistration.deserialize,
            )
        return self._stubs['register_gcp']

    @property
    def get_developer_registration(self) -> Callable[
        ...
        Returns:
            Callable[[~.GetDeveloperRegistrationRequest],
                    Awaitable[~.DeveloperRegistration]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
    ...
    @property
    def unregister_gcp(self) -> Callable[
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'unregister_gcp' not in self._stubs:
            self._stubs['unregister_gcp'] = self._logged_channel.unary_unary(
                '/google.shopping.merchant.accounts.v1.DeveloperRegistrationService/UnregisterGcp',
                request_serializer=developerregistration.UnregisterGcpRequest.serialize,
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.register_gcp: self._wrap_method(
                self.register_gcp,
                default_timeout=None,
                client_info=client_info,
            ),
            self.get_developer_registration: self._wrap_method(
