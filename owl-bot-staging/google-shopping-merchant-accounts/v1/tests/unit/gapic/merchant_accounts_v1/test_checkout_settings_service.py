...
from google.shopping.merchant_accounts_v1.services.checkout_settings_service import CheckoutSettingsServiceAsyncClient
from google.shopping.merchant_accounts_v1.services.checkout_settings_service import CheckoutSettingsServiceClient
from google.shopping.merchant_accounts_v1.services.checkout_settings_service import transports
...
def test_checkout_settings_service_client_client_options_from_dict():
    with mock.patch('google.shopping.merchant_accounts_v1.services.checkout_settings_service.transports.CheckoutSettingsServiceGrpcTransport.__init__') as grpc_transport:
...
def test_update_checkout_settings_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/accounts/v1/{checkout_settings.name=accounts/*/programs/*/checkoutSettings}" % client.transport._host, args[1])
...
def test_checkout_settings_service_base_transport():
    ...
    with mock.patch('google.shopping.merchant_accounts_v1.services.checkout_settings_service.transports.CheckoutSettingsServiceTransport.__init__') as Transport:
...
def test_checkout_settings_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.shopping.merchant_accounts_v1.services.checkout_settings_service.transports.CheckoutSettingsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_checkout_settings_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.shopping.merchant_accounts_v1.services.checkout_settings_service.transports.CheckoutSettingsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CheckoutSettingsServiceTransport()
        adc.assert_called_once()
