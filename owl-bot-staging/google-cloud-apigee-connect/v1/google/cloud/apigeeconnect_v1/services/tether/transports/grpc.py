...
from .base import TetherTransport, DEFAULT_CLIENT_INFO
...
class TetherGrpcTransport(TetherTransport):
    """gRPC backend transport for Tether.
    ...
    def __init__(self, *,
        ...
        Args:
            host (Optional[str]):
                 The hostname to connect to (default: 'apigeeconnect.googleapis.com').
    ...
    def egress(self) -> Callable[
        ...
        if 'egress' not in self._stubs:
            self._stubs['egress'] = self._logged_channel.stream_stream(
                '/google.cloud.apigeeconnect.v1.Tether/Egress',
                request_serializer=tether.EgressResponse.serialize,
                response_deserializer=tether.EgressRequest.deserialize,
            )
