...
from .base import KeyTrackingServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import KeyTrackingServiceGrpcTransport


class KeyTrackingServiceGrpcAsyncIOTransport(KeyTrackingServiceTransport):
    """gRPC AsyncIO backend transport for KeyTrackingService.
    ...
    @property
    def get_protected_resources_summary(self) -> Callable[
        ...
        Returns:
            Callable[[~.GetProtectedResourcesSummaryRequest],
                    Awaitable[~.ProtectedResourcesSummary]]:
        ...
        if 'get_protected_resources_summary' not in self._stubs:
            self._stubs['get_protected_resources_summary'] = self.grpc_channel.unary_unary(
    ...
    @property
    def search_protected_resources(self) -> Callable[
        ...
        Returns:
            Callable[[~.SearchProtectedResourcesRequest],
                    Awaitable[~.SearchProtectedResourcesResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
