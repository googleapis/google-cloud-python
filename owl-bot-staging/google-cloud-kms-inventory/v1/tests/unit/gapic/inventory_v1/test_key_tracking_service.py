...
from google.cloud.kms.inventory_v1.services.key_tracking_service import KeyTrackingServiceAsyncClient
from google.cloud.kms.inventory_v1.services.key_tracking_service import KeyTrackingServiceClient
from google.cloud.kms.inventory_v1.services.key_tracking_service import pagers
from google.cloud.kms.inventory_v1.services.key_tracking_service import transports
...
@pytest.mark.parametrize("client_class", [
    KeyTrackingServiceClient, KeyTrackingServiceAsyncClient
])
@mock.patch.object(KeyTrackingServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(KeyTrackingServiceClient))
@mock.patch.object(KeyTrackingServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(KeyTrackingServiceAsyncClient))
...
def test_key_tracking_service_client_client_options_from_dict():
    with mock.patch('google.cloud.kms.inventory_v1.services.key_tracking_service.transports.KeyTrackingServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_protected_resources_summary(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.get_protected_resources_summary),
            '__call__') as call:
...
def test_search_protected_resources_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, key_tracking_service.ProtectedResource)
                   for i in results)
...
def test_get_protected_resources_summary_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        pb_return_value = key_tracking_service.ProtectedResourcesSummary.pb(return_value)
...
def test_get_protected_resources_summary_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.KeyTrackingServiceRestInterceptor, "post_get_protected_resources_summary") as post, \
         mock.patch.object(transports.KeyTrackingServiceRestInterceptor, "pre_get_protected_resources_summary") as pre:
...
def test_search_protected_resources_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = key_tracking_service.SearchProtectedResourcesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_search_protected_resources_rest_required_fields(request_type=key_tracking_service.SearchProtectedResourcesRequest):
    ...
    assert not set(unset_fields) - set(("crypto_key", "page_size", "page_token", ))
    jsonified_request.update(unset_fields)
...
def test_search_protected_resources_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("cryptoKey", "pageSize", "pageToken", )) & set(("scope", "cryptoKey", )))
...
def test_search_protected_resources_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.KeyTrackingServiceRestInterceptor, "post_search_protected_resources") as post, \
         mock.patch.object(transports.KeyTrackingServiceRestInterceptor, "pre_search_protected_resources") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_key_tracking_service_base_transport():
    ...
    with mock.patch('google.cloud.kms.inventory_v1.services.key_tracking_service.transports.KeyTrackingServiceTransport.__init__') as Transport:
...
def test_key_tracking_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.kms.inventory_v1.services.key_tracking_service.transports.KeyTrackingServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_key_tracking_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.kms.inventory_v1.services.key_tracking_service.transports.KeyTrackingServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.KeyTrackingServiceTransport()
        adc.assert_called_once()
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.KeyTrackingServiceGrpcTransport, transports.KeyTrackingServiceGrpcAsyncIOTransport])
def test_key_tracking_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
