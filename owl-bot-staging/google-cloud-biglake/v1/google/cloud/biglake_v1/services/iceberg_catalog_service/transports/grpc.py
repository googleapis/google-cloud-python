...
from .base import IcebergCatalogServiceTransport, DEFAULT_CLIENT_INFO
...
class IcebergCatalogServiceGrpcTransport(IcebergCatalogServiceTransport):
    ...
    @property
    def get_iceberg_catalog(self) -> Callable[
            [iceberg_rest_catalog.GetIcebergCatalogRequest],
            iceberg_rest_catalog.IcebergCatalog]:
        r"""Return a callable for the get iceberg catalog method over gRPC.

        Returns the Iceberg REST Catalog configuration
        options.

        Returns:
            Callable[[~.GetIcebergCatalogRequest],
                    ~.IcebergCatalog]:
        ...
        if 'get_iceberg_catalog' not in self._stubs:
            self._stubs['get_iceberg_catalog'] = self._logged_channel.unary_unary(
                '/google.cloud.biglake.v1.IcebergCatalogService/GetIcebergCatalog',
                request_serializer=iceberg_rest_catalog.GetIcebergCatalogRequest.serialize,
                response_deserializer=iceberg_rest_catalog.IcebergCatalog.deserialize,
            )
        return self._stubs['get_iceberg_catalog']

    @property
    def list_iceberg_catalogs(self) -> Callable[
            [iceberg_rest_catalog.ListIcebergCatalogsRequest],
            iceberg_rest_catalog.ListIcebergCatalogsResponse]:
        r"""Return a callable for the list iceberg catalogs method over gRPC.

        Lists the Iceberg REST Catalogs.

        Returns:
            Callable[[~.ListIcebergCatalogsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'list_iceberg_catalogs' not in self._stubs:
            self._stubs['list_iceberg_catalogs'] = self._logged_channel.unary_unary(
                '/google.cloud.biglake.v1.IcebergCatalogService/ListIcebergCatalogs',
                request_serializer=iceberg_rest_catalog.ListIcebergCatalogsRequest.serialize,
                response_deserializer=iceberg_rest_catalog.ListIcebergCatalogsResponse.deserialize,
            )
        return self._stubs['list_iceberg_catalogs']

    @property
    def update_iceberg_catalog(self) -> Callable[
            [iceberg_rest_catalog.UpdateIcebergCatalogRequest],
            iceberg_rest_catalog.IcebergCatalog]:
        r"""Return a callable for the update iceberg catalog method over gRPC.

        Update the Iceberg REST Catalog configuration
        options.

        Returns:
            Callable[[~.UpdateIcebergCatalogRequest],
                    ~.IcebergCatalog]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'update_iceberg_catalog' not in self._stubs:
            self._stubs['update_iceberg_catalog'] = self._logged_channel.unary_unary(
                '/google.cloud.biglake.v1.IcebergCatalogService/UpdateIcebergCatalog',
                request_serializer=iceberg_rest_catalog.UpdateIcebergCatalogRequest.serialize,
                response_deserializer=iceberg_rest_catalog.IcebergCatalog.deserialize,
            )
        return self._stubs['update_iceberg_catalog']

    @property
    def create_iceberg_catalog(self) -> Callable[
        ...
        Returns:
            Callable[[~.CreateIcebergCatalogRequest],
                    ~.IcebergCatalog]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'create_iceberg_catalog' not in self._stubs:
            self._stubs['create_iceberg_catalog'] = self._logged_channel.unary_unary(
                '/google.cloud.biglake.v1.IcebergCatalogService/CreateIcebergCatalog',
                request_serializer=iceberg_rest_catalog.CreateIcebergCatalogRequest.serialize,
                response_deserializer=iceberg_rest_catalog.IcebergCatalog.deserialize,
            )
        return self._stubs['create_iceberg_catalog']

    @property
    def failover_iceberg_catalog(self) -> Callable[
            [iceberg_rest_catalog.FailoverIcebergCatalogRequest],
            iceberg_rest_catalog.FailoverIcebergCatalogResponse]:
        r"""Return a callable for the failover iceberg catalog method over gRPC.

        Failover the catalog to a new primary replica region.

        Returns:
            Callable[[~.FailoverIcebergCatalogRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'failover_iceberg_catalog' not in self._stubs:
            self._stubs['failover_iceberg_catalog'] = self._logged_channel.unary_unary(
                '/google.cloud.biglake.v1.IcebergCatalogService/FailoverIcebergCatalog',
                request_serializer=iceberg_rest_catalog.FailoverIcebergCatalogRequest.serialize,
                response_deserializer=iceberg_rest_catalog.FailoverIcebergCatalogResponse.deserialize,
            )
        return self._stubs['failover_iceberg_catalog']
...
__all__ = (
    'IcebergCatalogServiceGrpcTransport',
)
