...
class MetastoreServiceClient(metaclass=MetastoreServiceClientMeta):
    ...
    @staticmethod
    def database_path(project: str,location: str,catalog: str,database: str,) -> str:
        """Returns a fully-qualified database string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/databases/{database}".format(project=project, location=location, catalog=catalog, database=database, )

    @staticmethod
    def parse_database_path(path: str) -> Dict[str,str]:
        """Parses a database path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/databases/(?P<database>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def lock_path(project: str,location: str,catalog: str,database: str,lock: str,) -> str:
        """Returns a fully-qualified lock string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/databases/{database}/locks/{lock}".format(project=project, location=location, catalog=catalog, database=database, lock=lock, )

    @staticmethod
    def parse_lock_path(path: str) -> Dict[str,str]:
        """Parses a lock path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/databases/(?P<database>.+?)/locks/(?P<lock>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def table_path(project: str,location: str,catalog: str,database: str,table: str,) -> str:
        """Returns a fully-qualified table string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/databases/{database}/tables/{table}".format(project=project, location=location, catalog=catalog, database=database, table=table, )

    @staticmethod
    def parse_table_path(path: str) -> Dict[str,str]:
        """Parses a table path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/databases/(?P<database>.+?)/tables/(?P<table>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = MetastoreServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, MetastoreServiceTransport, Callable[..., MetastoreServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = MetastoreServiceClient._read_environment_variables()
        self._client_cert_source = MetastoreServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = MetastoreServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[MetastoreServiceTransport], Callable[..., MetastoreServiceTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.bigquery.biglake_v1alpha1.MetastoreServiceClient`.",
                    extra = {
    ...
    def create_catalog(self,
        ...
        flattened_params = [parent, catalog, catalog_id]
    ...
    def create_table(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [parent, table, table_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
    ...
    def rename_table(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [name, new_name]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
    ...
    def create_lock(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.create_lock]
    ...
    def delete_lock(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.delete_lock]
    ...
    def check_lock(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.check_lock]
    ...
    def list_locks(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.list_locks]
        ...
        # This method is paged; wrap the response in a pager, which provides
        # an `__iter__` convenience method.
        response = pagers.ListLocksPager(
            method=rpc,
            request=request,
            response=response,
            retry=retry,
