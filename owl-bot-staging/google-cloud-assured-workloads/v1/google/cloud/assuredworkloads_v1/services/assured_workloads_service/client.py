...
class AssuredWorkloadsServiceClient(metaclass=AssuredWorkloadsServiceClientMeta):
    ...
    _DEFAULT_ENDPOINT_TEMPLATE = "assuredworkloads.{UNIVERSE_DOMAIN}"
    ...
    @staticmethod
    def violation_path(organization: str,location: str,workload: str,violation: str,) -> str:
        """Returns a fully-qualified violation string."""
        return "organizations/{organization}/locations/{location}/workloads/{workload}/violations/{violation}".format(organization=organization, location=location, workload=workload, violation=violation, )

    @staticmethod
    def parse_violation_path(path: str) -> Dict[str,str]:
        """Parses a violation path into its component segments."""
        m = re.match(r"^organizations/(?P<organization>.+?)/locations/(?P<location>.+?)/workloads/(?P<workload>.+?)/violations/(?P<violation>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def workload_path(organization: str,location: str,workload: str,) -> str:
        """Returns a fully-qualified workload string."""
        return "organizations/{organization}/locations/{location}/workloads/{workload}".format(organization=organization, location=location, workload=workload, )

    @staticmethod
    def parse_workload_path(path: str) -> Dict[str,str]:
        """Parses a workload path into its component segments."""
        m = re.match(r"^organizations/(?P<organization>.+?)/locations/(?P<location>.+?)/workloads/(?P<workload>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = AssuredWorkloadsServiceClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = AssuredWorkloadsServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = AssuredWorkloadsServiceClient._DEFAULT_UNIVERSE
    ...
    def _compare_universes(client_universe: str,
        ...
        default_universe = AssuredWorkloadsServiceClient._DEFAULT_UNIVERSE
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            AssuredWorkloadsServiceClient._compare_universes(self.universe_domain, self.transport._credentials))
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, AssuredWorkloadsServiceTransport, Callable[..., AssuredWorkloadsServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = AssuredWorkloadsServiceClient._read_environment_variables()
        self._client_cert_source = AssuredWorkloadsServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = AssuredWorkloadsServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, AssuredWorkloadsServiceTransport)
        ...
        self._api_endpoint = (self._api_endpoint or
            AssuredWorkloadsServiceClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[AssuredWorkloadsServiceTransport], Callable[..., AssuredWorkloadsServiceTransport]] = (
    ...
    def restrict_allowed_resources(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.restrict_allowed_resources]
    ...
    def list_violations(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.list_violations]
        ...
        # This method is paged; wrap the response in a pager, which provides
        # an `__iter__` convenience method.
        response = pagers.ListViolationsPager(
            method=rpc,
            request=request,
            response=response,
            retry=retry,
    ...
    def get_violation(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.get_violation]
    ...
    def acknowledge_violation(self,
            request: Optional[Union[assuredworkloads.AcknowledgeViolationRequest, dict]] = None,
            *,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: Union[float, object] = gapic_v1.method.DEFAULT,
            metadata: Sequence[Tuple[str, str]] = (),
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.acknowledge_violation]
