...
class LineageClient(metaclass=LineageClientMeta):
    ...
    @staticmethod
    def lineage_event_path(project: str,location: str,process: str,run: str,lineage_event: str,) -> str:
        """Returns a fully-qualified lineage_event string."""
        return "projects/{project}/locations/{location}/processes/{process}/runs/{run}/lineageEvents/{lineage_event}".format(project=project, location=location, process=process, run=run, lineage_event=lineage_event, )

    @staticmethod
    def parse_lineage_event_path(path: str) -> Dict[str,str]:
        """Parses a lineage_event path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/processes/(?P<process>.+?)/runs/(?P<run>.+?)/lineageEvents/(?P<lineage_event>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def process_path(project: str,location: str,process: str,) -> str:
        """Returns a fully-qualified process string."""
        return "projects/{project}/locations/{location}/processes/{process}".format(project=project, location=location, process=process, )
    ...
    @staticmethod
    def run_path(project: str,location: str,process: str,run: str,) -> str:
        """Returns a fully-qualified run string."""
        return "projects/{project}/locations/{location}/processes/{process}/runs/{run}".format(project=project, location=location, process=process, run=run, )
    ...
    def batch_search_link_processes(self,
