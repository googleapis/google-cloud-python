...
from .base import DataPolicyServiceTransport, DEFAULT_CLIENT_INFO
...
class DataPolicyServiceGrpcAsyncIOTransport(DataPolicyServiceTransport):
    ...
    def rename_data_policy(self) -> Callable[
            ...
            Awaitable[datapolicy.DataPolicy]]:
        r"""Return a callable for the rename data policy method over gRPC.

        Renames the id (display name) of the specified data
        policy.

        Returns:
            Callable[[~.RenameDataPolicyRequest],
                    Awaitable[~.DataPolicy]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'rename_data_policy' not in self._stubs:
            self._stubs['rename_data_policy'] = self._logged_channel.unary_unary(
                '/google.cloud.bigquery.datapolicies.v1.DataPolicyService/RenameDataPolicy',
                request_serializer=datapolicy.RenameDataPolicyRequest.serialize,
                response_deserializer=datapolicy.DataPolicy.deserialize,
            )
        return self._stubs['rename_data_policy']
