...
from .transports.base import SpacesServiceTransport, DEFAULT_CLIENT_INFO
...
class SpacesServiceAsyncClient:
    ...
    get_transport_class = SpacesServiceClient.get_transport_class
    ...
    def __init__(self, *,
        ...
        if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
            _LOGGER.debug(
                "Created client `google.apps.meet_v2beta.SpacesServiceAsyncClient`.",
                extra = {
    ...
    async def connect_active_conference(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._client._transport._wrapped_methods[self._client._transport.connect_active_conference]
    ...
    async def end_active_conference(self,
    ...
    async def create_member(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        has_flattened_params = any([parent, member])
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of "
                             "the individual field arguments should be set.")
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._client._transport._wrapped_methods[self._client._transport.create_member]
