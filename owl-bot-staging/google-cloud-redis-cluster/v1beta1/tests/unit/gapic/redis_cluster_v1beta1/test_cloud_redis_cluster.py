...
from google.cloud.redis_cluster_v1beta1.services.cloud_redis_cluster import CloudRedisClusterAsyncClient
...
from google.cloud.redis_cluster_v1beta1.services.cloud_redis_cluster import transports
...
def test_reschedule_cluster_maintenance_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta1/{name=projects/*/locations/*/clusters/*}:rescheduleClusterMaintenance" % client.transport._host, args[1])
