...
from google.cloud.redis_cluster_v1.services.cloud_redis_cluster import CloudRedisClusterAsyncClient
from google.cloud.redis_cluster_v1.services.cloud_redis_cluster import CloudRedisClusterClient
from google.cloud.redis_cluster_v1.services.cloud_redis_cluster import pagers
from google.cloud.redis_cluster_v1.services.cloud_redis_cluster import transports
...
def test__read_environment_variables():
    assert CloudRedisClusterClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert CloudRedisClusterClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            CloudRedisClusterClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert CloudRedisClusterClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert CloudRedisClusterClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert CloudRedisClusterClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert CloudRedisClusterClient._get_client_cert_source(None, False) is None
    assert CloudRedisClusterClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert CloudRedisClusterClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert CloudRedisClusterClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert CloudRedisClusterClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(CloudRedisClusterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterClient))
@mock.patch.object(CloudRedisClusterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = CloudRedisClusterClient._DEFAULT_UNIVERSE
    default_endpoint = CloudRedisClusterClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = CloudRedisClusterClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert CloudRedisClusterClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert CloudRedisClusterClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == CloudRedisClusterClient.DEFAULT_MTLS_ENDPOINT
    assert CloudRedisClusterClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert CloudRedisClusterClient._get_api_endpoint(None, None, default_universe, "always") == CloudRedisClusterClient.DEFAULT_MTLS_ENDPOINT
    assert CloudRedisClusterClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == CloudRedisClusterClient.DEFAULT_MTLS_ENDPOINT
    assert CloudRedisClusterClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert CloudRedisClusterClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        CloudRedisClusterClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert CloudRedisClusterClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert CloudRedisClusterClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert CloudRedisClusterClient._get_universe_domain(None, None) == CloudRedisClusterClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize("client_class,transport_name", [
    (CloudRedisClusterClient, "grpc"),
    (CloudRedisClusterAsyncClient, "grpc_asyncio"),
    (CloudRedisClusterClient, "rest"),
])
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.CloudRedisClusterGrpcTransport, "grpc"),
    (transports.CloudRedisClusterGrpcAsyncIOTransport, "grpc_asyncio"),
    (transports.CloudRedisClusterRestTransport, "rest"),
])
...
def test_cloud_redis_cluster_client_get_transport_class():
    ...
    available_transports = [
        transports.CloudRedisClusterGrpcTransport,
        transports.CloudRedisClusterRestTransport,
    ]
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (CloudRedisClusterClient, transports.CloudRedisClusterGrpcTransport, "grpc"),
    (CloudRedisClusterAsyncClient, transports.CloudRedisClusterGrpcAsyncIOTransport, "grpc_asyncio"),
    (CloudRedisClusterClient, transports.CloudRedisClusterRestTransport, "rest"),
])
@mock.patch.object(CloudRedisClusterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterClient))
@mock.patch.object(CloudRedisClusterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (CloudRedisClusterClient, transports.CloudRedisClusterGrpcTransport, "grpc", "true"),
    (CloudRedisClusterAsyncClient, transports.CloudRedisClusterGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (CloudRedisClusterClient, transports.CloudRedisClusterGrpcTransport, "grpc", "false"),
    (CloudRedisClusterAsyncClient, transports.CloudRedisClusterGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (CloudRedisClusterClient, transports.CloudRedisClusterRestTransport, "rest", "true"),
    (CloudRedisClusterClient, transports.CloudRedisClusterRestTransport, "rest", "false"),
])
@mock.patch.object(CloudRedisClusterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterClient))
@mock.patch.object(CloudRedisClusterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterAsyncClient))
...
@pytest.mark.parametrize("client_class", [
    CloudRedisClusterClient, CloudRedisClusterAsyncClient
])
...
@mock.patch.object(CloudRedisClusterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterClient))
@mock.patch.object(CloudRedisClusterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(CloudRedisClusterAsyncClient))
def test_cloud_redis_cluster_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (CloudRedisClusterClient, transports.CloudRedisClusterGrpcTransport, "grpc", grpc_helpers),
    (CloudRedisClusterAsyncClient, transports.CloudRedisClusterGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (CloudRedisClusterClient, transports.CloudRedisClusterRestTransport, "rest", None),
])
...
def test_cloud_redis_cluster_client_client_options_from_dict():
    with mock.patch('google.cloud.redis_cluster_v1.services.cloud_redis_cluster.transports.CloudRedisClusterGrpcTransport.__init__') as grpc_transport:
...
def test_list_clusters_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, cloud_redis_cluster.Cluster)
                   for i in results)
...
def test_update_cluster_flattened():
    ...
    with mock.patch.object(
        ...
        client.update_cluster(
            cluster=cloud_redis_cluster.Cluster(gcs_source=cloud_redis_cluster.Cluster.GcsBackupSource(uris=['uris_value'])),
        ...
        _, args, _ = call.mock_calls[0]
        arg = args[0].cluster
        mock_val = cloud_redis_cluster.Cluster(gcs_source=cloud_redis_cluster.Cluster.GcsBackupSource(uris=['uris_value']))
        assert arg == mock_val
...
def test_get_cluster_certificate_authority(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_cluster_certificate_authority),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_get_cluster_certificate_authority_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_cluster_certificate_authority in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_cluster_certificate_authority] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_cluster_certificate_authority_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = CloudRedisClusterAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_cluster_certificate_authority in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_cluster_certificate_authority] = mock_rpc
...
@pytest.mark.asyncio
async def test_get_cluster_certificate_authority_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.GetClusterCertificateAuthorityRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_reschedule_cluster_maintenance(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.reschedule_cluster_maintenance),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.reschedule_cluster_maintenance(request)
...
def test_reschedule_cluster_maintenance_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_reschedule_cluster_maintenance_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.reschedule_cluster_maintenance in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.reschedule_cluster_maintenance] = mock_rpc
        request = {}
        client.reschedule_cluster_maintenance(request)
...
@pytest.mark.asyncio
async def test_reschedule_cluster_maintenance_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = CloudRedisClusterAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.reschedule_cluster_maintenance in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.reschedule_cluster_maintenance] = mock_rpc

        request = {}
        await client.reschedule_cluster_maintenance(request)
...
@pytest.mark.asyncio
async def test_reschedule_cluster_maintenance_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.RescheduleClusterMaintenanceRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
        ...
        response = await client.reschedule_cluster_maintenance(request)
...
@pytest.mark.asyncio
async def test_reschedule_cluster_maintenance_field_headers_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_reschedule_cluster_maintenance_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    ...
    with mock.patch.object(
        ...
        client.reschedule_cluster_maintenance(
            name='name_value',
            reschedule_type=cloud_redis_cluster.RescheduleClusterMaintenanceRequest.RescheduleType.IMMEDIATE,
...
def test_reschedule_cluster_maintenance_flattened_error():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
@pytest.mark.asyncio
async def test_reschedule_cluster_maintenance_flattened_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.reschedule_cluster_maintenance(
            name='name_value',
            reschedule_type=cloud_redis_cluster.RescheduleClusterMaintenanceRequest.RescheduleType.IMMEDIATE,
            schedule_time=timestamp_pb2.Timestamp(seconds=751),
        )
...
@pytest.mark.asyncio
async def test_reschedule_cluster_maintenance_flattened_error_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.reschedule_cluster_maintenance(
            cloud_redis_cluster.RescheduleClusterMaintenanceRequest(),
            name='name_value',
            reschedule_type=cloud_redis_cluster.RescheduleClusterMaintenanceRequest.RescheduleType.IMMEDIATE,
            schedule_time=timestamp_pb2.Timestamp(seconds=751),
...
def test_list_backup_collections(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.list_backup_collections),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = cloud_redis_cluster.ListBackupCollectionsResponse(
            next_page_token='next_page_token_value',
            unreachable=['unreachable_value'],
        )
        response = client.list_backup_collections(request)
...
def test_list_backup_collections_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.asyncio
async def test_list_backup_collections_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = CloudRedisClusterAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_backup_collections in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_backup_collections] = mock_rpc
...
@pytest.mark.asyncio
async def test_list_backup_collections_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.ListBackupCollectionsRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(cloud_redis_cluster.ListBackupCollectionsResponse(
            next_page_token='next_page_token_value',
            unreachable=['unreachable_value'],
        ))
        response = await client.list_backup_collections(request)
...
@pytest.mark.asyncio
async def test_list_backup_collections_field_headers_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_list_backup_collections_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
def test_list_backup_collections_flattened_error():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
@pytest.mark.asyncio
async def test_list_backup_collections_flattened_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.list_backup_collections(
            parent='parent_value',
        )
...
@pytest.mark.asyncio
async def test_list_backup_collections_flattened_error_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.list_backup_collections(
            cloud_redis_cluster.ListBackupCollectionsRequest(),
            parent='parent_value',
        )


def test_list_backup_collections_pager(transport_name: str = "grpc"):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport_name,
    )
    ...
    with mock.patch.object(
        ...
        pager = client.list_backup_collections(request={}, retry=retry, timeout=timeout)
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, cloud_redis_cluster.BackupCollection)
                   for i in results)
def test_list_backup_collections_pages(transport_name: str = "grpc"):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport_name,
    )
    ...
    with mock.patch.object(
        ...
        pages = list(client.list_backup_collections(request={}).pages)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_backup_collections_async_pager():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_backup_collections(request={},)
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager: # pragma: no branch
            responses.append(response)
...
@pytest.mark.asyncio
async def test_list_backup_collections_async_pages():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_get_backup_collection(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_backup_collection),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = cloud_redis_cluster.BackupCollection(
            name='name_value',
        ...
        response = client.get_backup_collection(request)
    ...
    # Establish that the response is the type that we expect.
    assert isinstance(response, cloud_redis_cluster.BackupCollection)
    assert response.name == 'name_value'
...
def test_get_backup_collection_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.asyncio
async def test_get_backup_collection_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = CloudRedisClusterAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_backup_collection in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_backup_collection] = mock_rpc

        request = {}
        await client.get_backup_collection(request)
...
@pytest.mark.asyncio
async def test_get_backup_collection_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.GetBackupCollectionRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(cloud_redis_cluster.BackupCollection(
            name='name_value',
            cluster_uid='cluster_uid_value',
            cluster='cluster_value',
            kms_key='kms_key_value',
        ...
        response = await client.get_backup_collection(request)
...
@pytest.mark.asyncio
async def test_get_backup_collection_field_headers_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_get_backup_collection_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
def test_get_backup_collection_flattened_error():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
@pytest.mark.asyncio
async def test_get_backup_collection_flattened_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.get_backup_collection(
            name='name_value',
        )
...
@pytest.mark.asyncio
async def test_get_backup_collection_flattened_error_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.get_backup_collection(
            cloud_redis_cluster.GetBackupCollectionRequest(),
            name='name_value',
        )
...
def test_list_backups(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = cloud_redis_cluster.ListBackupsResponse(
            next_page_token='next_page_token_value',
            unreachable=['unreachable_value'],
        )
...
@pytest.mark.asyncio
async def test_list_backups_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.ListBackupsRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(cloud_redis_cluster.ListBackupsResponse(
            next_page_token='next_page_token_value',
            unreachable=['unreachable_value'],
        ))
...
def test_list_backups_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, cloud_redis_cluster.Backup)
                   for i in results)
...
def test_get_backup(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = cloud_redis_cluster.Backup(
            name='name_value',
            cluster='cluster_value',
            cluster_uid='cluster_uid_value',
            total_size_bytes=1732,
            engine_version='engine_version_value',
            node_type=cloud_redis_cluster.NodeType.REDIS_SHARED_CORE_NANO,
            replica_count=1384,
            shard_count=1178,
            backup_type=cloud_redis_cluster.Backup.BackupType.ON_DEMAND,
            state=cloud_redis_cluster.Backup.State.CREATING,
            uid='uid_value',
        )
    ...
    # Establish that the response is the type that we expect.
    assert isinstance(response, cloud_redis_cluster.Backup)
    assert response.name == 'name_value'
    assert response.cluster == 'cluster_value'
    assert response.cluster_uid == 'cluster_uid_value'
    assert response.total_size_bytes == 1732
...
@pytest.mark.asyncio
async def test_get_backup_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.GetBackupRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(cloud_redis_cluster.Backup(
            name='name_value',
            cluster='cluster_value',
            cluster_uid='cluster_uid_value',
            total_size_bytes=1732,
...
@pytest.mark.asyncio
async def test_delete_backup_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.DeleteBackupRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_export_backup(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.export_backup),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.export_backup(request)
...
def test_export_backup_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_export_backup_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.export_backup in client._transport._wrapped_methods
...
@pytest.mark.asyncio
async def test_export_backup_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = CloudRedisClusterAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.export_backup in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.export_backup] = mock_rpc

        request = {}
        await client.export_backup(request)
...
@pytest.mark.asyncio
async def test_export_backup_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.ExportBackupRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
        ...
        response = await client.export_backup(request)
...
@pytest.mark.asyncio
async def test_export_backup_field_headers_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_backup_cluster(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.backup_cluster),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.backup_cluster(request)
...
def test_backup_cluster_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_backup_cluster_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.backup_cluster in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.backup_cluster] = mock_rpc
        request = {}
        client.backup_cluster(request)
...
@pytest.mark.asyncio
async def test_backup_cluster_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = CloudRedisClusterAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.backup_cluster in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.backup_cluster] = mock_rpc

        request = {}
        await client.backup_cluster(request)
...
@pytest.mark.asyncio
async def test_backup_cluster_async(transport: str = 'grpc_asyncio', request_type=cloud_redis_cluster.BackupClusterRequest):
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
        ...
        response = await client.backup_cluster(request)
...
@pytest.mark.asyncio
async def test_backup_cluster_field_headers_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_backup_cluster_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
def test_backup_cluster_flattened_error():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
    )
...
@pytest.mark.asyncio
async def test_backup_cluster_flattened_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.backup_cluster(
            name='name_value',
        )
...
@pytest.mark.asyncio
async def test_backup_cluster_flattened_error_async():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_update_cluster_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{cluster.name=projects/*/locations/*/clusters/*}" % client.transport._host, args[1])
...
def test_get_cluster_certificate_authority_rest_required_fields(request_type=cloud_redis_cluster.GetClusterCertificateAuthorityRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_cluster_certificate_authority._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_cluster_certificate_authority_rest_unset_required_fields():
    ...
    unset_fields = transport.get_cluster_certificate_authority._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("name", )))
...
def test_get_cluster_certificate_authority_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        # get arguments that satisfy an http rule for this method
        sample_request = {'name': 'projects/sample1/locations/sample2/clusters/sample3/certificateAuthority'}
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=projects/*/locations/*/clusters/*/certificateAuthority}" % client.transport._host, args[1])


def test_get_cluster_certificate_authority_rest_flattened_error(transport: str = 'rest'):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_reschedule_cluster_maintenance_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = CloudRedisClusterClient(
...
def test_reschedule_cluster_maintenance_rest_required_fields(request_type=cloud_redis_cluster.RescheduleClusterMaintenanceRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).reschedule_cluster_maintenance._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_reschedule_cluster_maintenance_rest_unset_required_fields():
    transport = transports.CloudRedisClusterRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.reschedule_cluster_maintenance._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("name", "rescheduleType", )))


def test_reschedule_cluster_maintenance_rest_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        client.reschedule_cluster_maintenance(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=projects/*/locations/*/clusters/*}:rescheduleClusterMaintenance" % client.transport._host, args[1])


def test_reschedule_cluster_maintenance_rest_flattened_error(transport: str = 'rest'):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_backup_collections_rest_required_fields(request_type=cloud_redis_cluster.ListBackupCollectionsRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_backup_collections._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    ...
    # Designate an appropriate value for the returned response.
    return_value = cloud_redis_cluster.ListBackupCollectionsResponse()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, 'request') as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            # Convert return value to protobuf type
            return_value = cloud_redis_cluster.ListBackupCollectionsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_list_backup_collections_rest_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        client.list_backup_collections(**mock_args)
...
def test_list_backup_collections_rest_pager(transport: str = 'rest'):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # Wrap the values into proper Response objs
        response = tuple(cloud_redis_cluster.ListBackupCollectionsResponse.to_json(x) for x in response)
        return_values = tuple(Response() for i in response)
        for return_val, response_val in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        ...
        pages = list(client.list_backup_collections(request=sample_request).pages)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token


def test_get_backup_collection_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = CloudRedisClusterClient(
...
def test_get_backup_collection_rest_required_fields(request_type=cloud_redis_cluster.GetBackupCollectionRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_backup_collection._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    ...
    # Designate an appropriate value for the returned response.
    return_value = cloud_redis_cluster.BackupCollection()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, 'request') as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            # Convert return value to protobuf type
            return_value = cloud_redis_cluster.BackupCollection.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_get_backup_collection_rest_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        client.get_backup_collection(**mock_args)
...
def test_list_backups_rest_required_fields(request_type=cloud_redis_cluster.ListBackupsRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = cloud_redis_cluster.ListBackupsResponse()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, 'request') as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            # Convert return value to protobuf type
            return_value = cloud_redis_cluster.ListBackupsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_list_backups_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        # Wrap the values into proper Response objs
        response = tuple(cloud_redis_cluster.ListBackupsResponse.to_json(x) for x in response)
        return_values = tuple(Response() for i in response)
        for return_val, response_val in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
...
def test_get_backup_rest_required_fields(request_type=cloud_redis_cluster.GetBackupRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = cloud_redis_cluster.Backup()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, 'request') as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            # Convert return value to protobuf type
            return_value = cloud_redis_cluster.Backup.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_delete_backup_rest_required_fields(request_type=cloud_redis_cluster.DeleteBackupRequest):
    transport_class = transports.CloudRedisClusterRestTransport
...
def test_export_backup_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = CloudRedisClusterClient(
...
def test_export_backup_rest_required_fields(request_type=cloud_redis_cluster.ExportBackupRequest):
    transport_class = transports.CloudRedisClusterRestTransport
...
def test_backup_cluster_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = CloudRedisClusterClient(
...
def test_backup_cluster_rest_required_fields(request_type=cloud_redis_cluster.BackupClusterRequest):
    transport_class = transports.CloudRedisClusterRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).backup_cluster._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_backup_cluster_rest_unset_required_fields():
    transport = transports.CloudRedisClusterRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.backup_cluster._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("name", )))


def test_backup_cluster_rest_flattened():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        client.backup_cluster(**mock_args)

        # Establish that the underlying call was made with the expected
        # request object values.
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=projects/*/locations/*/clusters/*}:backup" % client.transport._host, args[1])


def test_backup_cluster_rest_flattened_error(transport: str = 'rest'):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_transport_get_channel():
    ...
    transport = transports.CloudRedisClusterGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
@pytest.mark.parametrize("transport_class", [
    transports.CloudRedisClusterGrpcTransport,
    transports.CloudRedisClusterGrpcAsyncIOTransport,
    transports.CloudRedisClusterRestTransport,
])
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_cluster_certificate_authority_empty_call_grpc():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = cloud_redis_cluster.CertificateAuthority()
        client.get_cluster_certificate_authority(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_reschedule_cluster_maintenance_empty_call_grpc():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.reschedule_cluster_maintenance(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_backup_collections_empty_call_grpc():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_backup_collection_empty_call_grpc():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_export_backup_empty_call_grpc():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.export_backup(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_backup_cluster_empty_call_grpc():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.backup_cluster(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = CloudRedisClusterAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_cluster_certificate_authority_empty_call_grpc_asyncio():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_cluster_certificate_authority(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_reschedule_cluster_maintenance_empty_call_grpc_asyncio():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.reschedule_cluster_maintenance(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_backup_collections_empty_call_grpc_asyncio():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.list_backup_collections(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_backup_collection_empty_call_grpc_asyncio():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_backup_collection(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_export_backup_empty_call_grpc_asyncio():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.export_backup(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_backup_cluster_empty_call_grpc_asyncio():
    client = CloudRedisClusterAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.backup_cluster(request=None)
...
def test_transport_kind_rest():
    transport = CloudRedisClusterClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
    assert transport.kind == "rest"


def test_list_clusters_rest_bad_request(request_type=cloud_redis_cluster.ListClustersRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_clusters_rest_interceptors(null_interceptor):
    transport = transports.CloudRedisClusterRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.CloudRedisClusterRestInterceptor(),
        )
    client = CloudRedisClusterClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_list_clusters") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_list_clusters_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_list_clusters") as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.ListClustersResponse.to_json(cloud_redis_cluster.ListClustersResponse())
        ...
        post_with_metadata.return_value = cloud_redis_cluster.ListClustersResponse(), metadata
...
def test_get_cluster_rest_bad_request(request_type=cloud_redis_cluster.GetClusterRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_cluster_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_cluster") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_cluster_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_get_cluster") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.Cluster.to_json(cloud_redis_cluster.Cluster())
        req.return_value.content = return_value
...
def test_update_cluster_rest_bad_request(request_type=cloud_redis_cluster.UpdateClusterRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_update_cluster_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'cluster': {'name': 'projects/sample1/locations/sample2/clusters/sample3'}}
    request_init["cluster"] = {'gcs_source': {'uris': ['uris_value1', 'uris_value2']}, 'managed_backup_source': {'backup': 'backup_value'}, 'name': 'projects/sample1/locations/sample2/clusters/sample3', 'create_time': {'seconds': 751, 'nanos': 543}, 'state': 1, 'uid': 'uid_value', 'replica_count': 1384, 'authorization_mode': 1, 'transit_encryption_mode': 1, 'size_gb': 739, 'shard_count': 1178, 'psc_configs': [{'network': 'network_value'}], 'discovery_endpoints': [{'address': 'address_value', 'port': 453, 'psc_config': {}}], 'psc_connections': [{'psc_connection_id': 'psc_connection_id_value', 'address': 'address_value', 'forwarding_rule': 'forwarding_rule_value', 'project_id': 'project_id_value', 'network': 'network_value', 'service_attachment': 'service_attachment_value', 'psc_connection_status': 1, 'connection_type': 1}], 'state_info': {'update_info': {'target_shard_count': 1920, 'target_replica_count': 2126}}, 'node_type': 1, 'persistence_config': {'mode': 1, 'rdb_config': {'rdb_snapshot_period': 1, 'rdb_snapshot_start_time': {}}, 'aof_config': {'append_fsync': 1}}, 'redis_configs': {}, 'precise_size_gb': 0.15810000000000002, 'zone_distribution_config': {'mode': 1, 'zone': 'zone_value'}, 'cross_cluster_replication_config': {'cluster_role': 1, 'primary_cluster': {'cluster': 'cluster_value', 'uid': 'uid_value'}, 'secondary_clusters': {}, 'update_time': {}, 'membership': {'primary_cluster': {}, 'secondary_clusters': {}}}, 'deletion_protection_enabled': True, 'maintenance_policy': {'create_time': {}, 'update_time': {}, 'weekly_maintenance_window': [{'day': 1, 'start_time': {'hours': 561, 'minutes': 773, 'seconds': 751, 'nanos': 543}}]}, 'maintenance_schedule': {'start_time': {}, 'end_time': {}}, 'psc_service_attachments': [{'service_attachment': 'service_attachment_value', 'connection_type': 1}], 'cluster_endpoints': [{'connections': [{'psc_auto_connection': {'psc_connection_id': 'psc_connection_id_value', 'address': 'address_value', 'forwarding_rule': 'forwarding_rule_value', 'project_id': 'project_id_value', 'network': 'network_value', 'service_attachment': 'service_attachment_value', 'psc_connection_status': 1, 'connection_type': 1}, 'psc_connection': {}}]}], 'backup_collection': 'backup_collection_value', 'kms_key': 'kms_key_value', 'automated_backup_config': {'fixed_frequency_schedule': {'start_time': {}}, 'automated_backup_mode': 1, 'retention': {'seconds': 751, 'nanos': 543}}, 'encryption_info': {'encryption_type': 1, 'kms_key_versions': ['kms_key_versions_value1', 'kms_key_versions_value2'], 'kms_key_primary_state': 1, 'last_update_time': {}}}
...
def test_update_cluster_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_update_cluster") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_update_cluster_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_update_cluster") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_delete_cluster_rest_bad_request(request_type=cloud_redis_cluster.DeleteClusterRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_cluster_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_delete_cluster") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_delete_cluster_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_delete_cluster") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_create_cluster_rest_bad_request(request_type=cloud_redis_cluster.CreateClusterRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_create_cluster_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_create_cluster") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_create_cluster_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_create_cluster") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_cluster_certificate_authority_rest_bad_request(request_type=cloud_redis_cluster.GetClusterCertificateAuthorityRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    # send a request that will satisfy transcoding
    request_init = {'name': 'projects/sample1/locations/sample2/clusters/sample3/certificateAuthority'}
    request = request_type(**request_init)
...
def test_get_cluster_certificate_authority_rest_call_success(request_type):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_cluster_certificate_authority_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_cluster_certificate_authority") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_cluster_certificate_authority_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_get_cluster_certificate_authority") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.GetClusterCertificateAuthorityRequest.pb(cloud_redis_cluster.GetClusterCertificateAuthorityRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.CertificateAuthority.to_json(cloud_redis_cluster.CertificateAuthority())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = cloud_redis_cluster.CertificateAuthority(), metadata

        client.get_cluster_certificate_authority(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_reschedule_cluster_maintenance_rest_bad_request(request_type=cloud_redis_cluster.RescheduleClusterMaintenanceRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_reschedule_cluster_maintenance_rest_call_success(request_type):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_reschedule_cluster_maintenance_rest_interceptors(null_interceptor):
    transport = transports.CloudRedisClusterRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.CloudRedisClusterRestInterceptor(),
        )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_reschedule_cluster_maintenance") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_reschedule_cluster_maintenance_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_reschedule_cluster_maintenance") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.RescheduleClusterMaintenanceRequest.pb(cloud_redis_cluster.RescheduleClusterMaintenanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.reschedule_cluster_maintenance(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_list_backup_collections_rest_bad_request(request_type=cloud_redis_cluster.ListBackupCollectionsRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_backup_collections_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_list_backup_collections") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_list_backup_collections_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_list_backup_collections") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.ListBackupCollectionsRequest.pb(cloud_redis_cluster.ListBackupCollectionsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.ListBackupCollectionsResponse.to_json(cloud_redis_cluster.ListBackupCollectionsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = cloud_redis_cluster.ListBackupCollectionsResponse(), metadata

        client.list_backup_collections(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_get_backup_collection_rest_bad_request(request_type=cloud_redis_cluster.GetBackupCollectionRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_backup_collection_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_backup_collection") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_backup_collection_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_get_backup_collection") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.GetBackupCollectionRequest.pb(cloud_redis_cluster.GetBackupCollectionRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.BackupCollection.to_json(cloud_redis_cluster.BackupCollection())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = cloud_redis_cluster.BackupCollection(), metadata

        client.get_backup_collection(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_list_backups_rest_bad_request(request_type=cloud_redis_cluster.ListBackupsRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_backups_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_list_backups") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_list_backups_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_list_backups") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.ListBackupsRequest.pb(cloud_redis_cluster.ListBackupsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.ListBackupsResponse.to_json(cloud_redis_cluster.ListBackupsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = cloud_redis_cluster.ListBackupsResponse(), metadata
...
def test_get_backup_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_backup") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_get_backup_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_get_backup") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.GetBackupRequest.pb(cloud_redis_cluster.GetBackupRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = cloud_redis_cluster.Backup.to_json(cloud_redis_cluster.Backup())
        req.return_value.content = return_value
...
def test_delete_backup_rest_bad_request(request_type=cloud_redis_cluster.DeleteBackupRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_backup_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_delete_backup") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_delete_backup_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_delete_backup") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.DeleteBackupRequest.pb(cloud_redis_cluster.DeleteBackupRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_export_backup_rest_bad_request(request_type=cloud_redis_cluster.ExportBackupRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_export_backup_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_export_backup") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_export_backup_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_export_backup") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.ExportBackupRequest.pb(cloud_redis_cluster.ExportBackupRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.export_backup(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_backup_cluster_rest_bad_request(request_type=cloud_redis_cluster.BackupClusterRequest):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_backup_cluster_rest_call_success(request_type):
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_backup_cluster_rest_interceptors(null_interceptor):
    transport = transports.CloudRedisClusterRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.CloudRedisClusterRestInterceptor(),
        )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_backup_cluster") as post, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "post_backup_cluster_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CloudRedisClusterRestInterceptor, "pre_backup_cluster") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = cloud_redis_cluster.BackupClusterRequest.pb(cloud_redis_cluster.BackupClusterRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.backup_cluster(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_cluster_certificate_authority_empty_call_rest():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_reschedule_cluster_maintenance_empty_call_rest():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_backup_collections_empty_call_rest():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_backup_collection_empty_call_rest():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_export_backup_empty_call_rest():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_backup_cluster_empty_call_rest():
    client = CloudRedisClusterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_cloud_redis_cluster_base_transport():
    ...
    with mock.patch('google.cloud.redis_cluster_v1.services.cloud_redis_cluster.transports.CloudRedisClusterTransport.__init__') as Transport:
...
def test_cloud_redis_cluster_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.redis_cluster_v1.services.cloud_redis_cluster.transports.CloudRedisClusterTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_cloud_redis_cluster_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.redis_cluster_v1.services.cloud_redis_cluster.transports.CloudRedisClusterTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CloudRedisClusterTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize(
    ...
    [
        (transports.CloudRedisClusterGrpcTransport, grpc_helpers),
        (transports.CloudRedisClusterGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
...
def test_cloud_redis_cluster_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_cloud_redis_cluster_client_transport_session_collision(transport_name):
    ...
    client1 = CloudRedisClusterClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = CloudRedisClusterClient(
        credentials=creds2,
        transport=transport_name,
    )
    ...
    assert session1 != session2
    session1 = client1.transport.reschedule_cluster_maintenance._session
    session2 = client2.transport.reschedule_cluster_maintenance._session
    assert session1 != session2
    session1 = client1.transport.list_backup_collections._session
    session2 = client2.transport.list_backup_collections._session
    assert session1 != session2
    session1 = client1.transport.get_backup_collection._session
    session2 = client2.transport.get_backup_collection._session
    assert session1 != session2
    ...
    assert session1 != session2
    session1 = client1.transport.export_backup._session
    session2 = client2.transport.export_backup._session
    assert session1 != session2
    session1 = client1.transport.backup_cluster._session
    session2 = client2.transport.backup_cluster._session
    assert session1 != session2
...
@pytest.mark.parametrize("transport_class", [transports.CloudRedisClusterGrpcTransport, transports.CloudRedisClusterGrpcAsyncIOTransport])
def test_cloud_redis_cluster_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.CloudRedisClusterGrpcTransport, transports.CloudRedisClusterGrpcAsyncIOTransport])
def test_cloud_redis_cluster_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_backup_path():
    ...
    backup = "octopus"
    expected = "projects/{project}/locations/{location}/backupCollections/{backup_collection}/backups/{backup}".format(project=project, location=location, backup_collection=backup_collection, backup=backup, )
    actual = CloudRedisClusterClient.backup_path(project, location, backup_collection, backup)
...
def test_parse_backup_path():
    ...
    actual = CloudRedisClusterClient.parse_backup_path(path)
    assert expected == actual
...
def test_backup_collection_path():
    ...
    expected = "projects/{project}/locations/{location}/backupCollections/{backup_collection}".format(project=project, location=location, backup_collection=backup_collection, )
    actual = CloudRedisClusterClient.backup_collection_path(project, location, backup_collection)
    assert expected == actual
...
def test_parse_backup_collection_path():
    ...
    # Check that the path construction is reversible.
    actual = CloudRedisClusterClient.parse_backup_collection_path(path)
    assert expected == actual
...
def test_certificate_authority_path():
    ...
    location = "octopus"
    cluster = "oyster"
    expected = "projects/{project}/locations/{location}/clusters/{cluster}/certificateAuthority".format(project=project, location=location, cluster=cluster, )
    actual = CloudRedisClusterClient.certificate_authority_path(project, location, cluster)
    assert expected == actual
...
def test_crypto_key_path():
    ...
    actual = CloudRedisClusterClient.crypto_key_path(project, location, key_ring, crypto_key)
    assert expected == actual
...
def test_parse_crypto_key_path():
    ...
    # Check that the path construction is reversible.
    actual = CloudRedisClusterClient.parse_crypto_key_path(path)
    assert expected == actual
...
def test_crypto_key_version_path():
    ...
    actual = CloudRedisClusterClient.crypto_key_version_path(project, location, key_ring, crypto_key, crypto_key_version)
    assert expected == actual
...
def test_parse_crypto_key_version_path():
    ...
    # Check that the path construction is reversible.
    actual = CloudRedisClusterClient.parse_crypto_key_version_path(path)
    assert expected == actual

def test_forwarding_rule_path():
    project = "winkle"
    region = "nautilus"
    forwarding_rule = "scallop"
    ...
    actual = CloudRedisClusterClient.forwarding_rule_path(project, region, forwarding_rule)
    assert expected == actual
...
def test_parse_forwarding_rule_path():
    expected = {
        "project": "abalone",
        "region": "squid",
        "forwarding_rule": "clam",
    }
    path = CloudRedisClusterClient.forwarding_rule_path(**expected)

    # Check that the path construction is reversible.
    actual = CloudRedisClusterClient.parse_forwarding_rule_path(path)
    assert expected == actual
...
def test_network_path():
    ...
    actual = CloudRedisClusterClient.network_path(project, network)
    assert expected == actual
...
def test_parse_network_path():
    ...
    # Check that the path construction is reversible.
    actual = CloudRedisClusterClient.parse_network_path(path)
    assert expected == actual
...
def test_service_attachment_path():
    ...
    expected = "projects/{project}/regions/{region}/serviceAttachments/{service_attachment}".format(project=project, region=region, service_attachment=service_attachment, )
    actual = CloudRedisClusterClient.service_attachment_path(project, region, service_attachment)
    assert expected == actual
...
def test_parse_service_attachment_path():
    ...
    # Check that the path construction is reversible.
    actual = CloudRedisClusterClient.parse_service_attachment_path(path)
    assert expected == actual
...
@pytest.mark.parametrize("client_class,transport_class", [
    (CloudRedisClusterClient, transports.CloudRedisClusterGrpcTransport),
    (CloudRedisClusterAsyncClient, transports.CloudRedisClusterGrpcAsyncIOTransport),
])
