...
from .base import CloudControlsPartnerMonitoringTransport, DEFAULT_CLIENT_INFO
...
class CloudControlsPartnerMonitoringGrpcAsyncIOTransport(CloudControlsPartnerMonitoringTransport):
    """gRPC AsyncIO backend transport for CloudControlsPartnerMonitoring.
    ...
    def list_violations(self) -> Callable[
            ...
            Awaitable[violations.ListViolationsResponse]]:
        r"""Return a callable for the list violations method over gRPC.

        Lists Violations for a workload Callers may also choose to read
        ...
        Returns:
            Callable[[~.ListViolationsRequest],
                    Awaitable[~.ListViolationsResponse]]:
        ...
        if 'list_violations' not in self._stubs:
            self._stubs['list_violations'] = self._logged_channel.unary_unary(
                '/google.cloud.cloudcontrolspartner.v1beta.CloudControlsPartnerMonitoring/ListViolations',
                request_serializer=violations.ListViolationsRequest.serialize,
                response_deserializer=violations.ListViolationsResponse.deserialize,
            )
    ...
    def get_violation(self) -> Callable[
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'get_violation' not in self._stubs:
            self._stubs['get_violation'] = self._logged_channel.unary_unary(
                '/google.cloud.cloudcontrolspartner.v1beta.CloudControlsPartnerMonitoring/GetViolation',
                request_serializer=violations.GetViolationRequest.serialize,
                response_deserializer=violations.Violation.deserialize,
            )
        return self._stubs['get_violation']
