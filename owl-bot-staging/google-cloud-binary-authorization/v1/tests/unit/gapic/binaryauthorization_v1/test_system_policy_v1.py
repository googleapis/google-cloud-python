...
from google.cloud.binaryauthorization_v1.services.system_policy_v1 import SystemPolicyV1AsyncClient
from google.cloud.binaryauthorization_v1.services.system_policy_v1 import SystemPolicyV1Client
...
def test__get_api_endpoint():
    ...
    assert SystemPolicyV1Client._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == SystemPolicyV1Client.DEFAULT_MTLS_ENDPOINT
    ...
    assert SystemPolicyV1Client._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == SystemPolicyV1Client.DEFAULT_MTLS_ENDPOINT
...
@pytest.mark.parametrize("client_class", [
    SystemPolicyV1Client, SystemPolicyV1AsyncClient
])
...
def test_get_system_policy(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.get_system_policy),
            '__call__') as call:
...
def test_get_system_policy_rest_required_fields(request_type=service.GetSystemPolicyRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).get_system_policy._get_unset_required_fields(jsonified_request)
...
def test_get_system_policy_rest_unset_required_fields():
    transport = transports.SystemPolicyV1RestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)
...
def test_get_system_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.SystemPolicyV1RestInterceptor, "post_get_system_policy") as post, \
         mock.patch.object(transports.SystemPolicyV1RestInterceptor, "pre_get_system_policy") as pre:
...
def test_system_policy_v1_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.SystemPolicyV1GrpcTransport, transports.SystemPolicyV1GrpcAsyncIOTransport])
def test_system_policy_v1_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
