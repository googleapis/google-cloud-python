...
from google.cloud.run_v2.services.worker_pools import WorkerPoolsAsyncClient
from google.cloud.run_v2.services.worker_pools import WorkerPoolsClient
from google.cloud.run_v2.services.worker_pools import pagers
from google.cloud.run_v2.services.worker_pools import transports
...
@pytest.mark.parametrize("client_class", [
    WorkerPoolsClient, WorkerPoolsAsyncClient
])
...
def test_list_worker_pools_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, worker_pool.WorkerPool)
                   for i in results)
...
def test_update_worker_pool_rest_required_fields(request_type=gcr_worker_pool.UpdateWorkerPoolRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_worker_pool._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("allow_missing", "force_new_revision", "update_mask", "validate_only", ))
    jsonified_request.update(unset_fields)
...
def test_update_worker_pool_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("allowMissing", "forceNewRevision", "updateMask", "validateOnly", )) & set(("workerPool", )))
...
def test_create_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_create_worker_pool") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_create_worker_pool_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_create_worker_pool") as pre:
...
def test_get_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_get_worker_pool") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_get_worker_pool_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_get_worker_pool") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_list_worker_pools_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_list_worker_pools") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_list_worker_pools_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_list_worker_pools") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_update_worker_pool_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'worker_pool': {'name': 'projects/sample1/locations/sample2/workerPools/sample3'}}
    request_init["worker_pool"] = {'name': 'projects/sample1/locations/sample2/workerPools/sample3', 'description': 'description_value', 'uid': 'uid_value', 'generation': 1068, 'labels': {}, 'annotations': {}, 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'delete_time': {}, 'expire_time': {}, 'creator': 'creator_value', 'last_modifier': 'last_modifier_value', 'client': 'client_value', 'client_version': 'client_version_value', 'launch_stage': 6, 'binary_authorization': {'use_default': True, 'policy': 'policy_value', 'breakglass_justification': 'breakglass_justification_value'}, 'template': {'revision': 'revision_value', 'labels': {}, 'annotations': {}, 'vpc_access': {'connector': 'connector_value', 'egress': 1, 'network_interfaces': [{'network': 'network_value', 'subnetwork': 'subnetwork_value', 'tags': ['tags_value1', 'tags_value2']}]}, 'service_account': 'service_account_value', 'containers': [{'name': 'name_value', 'image': 'image_value', 'command': ['command_value1', 'command_value2'], 'args': ['args_value1', 'args_value2'], 'env': [{'name': 'name_value', 'value': 'value_value', 'value_source': {'secret_key_ref': {'secret': 'secret_value', 'version': 'version_value'}}}], 'resources': {'limits': {}, 'cpu_idle': True, 'startup_cpu_boost': True}, 'ports': [{'name': 'name_value', 'container_port': 1511}], 'volume_mounts': [{'name': 'name_value', 'mount_path': 'mount_path_value'}], 'working_dir': 'working_dir_value', 'liveness_probe': {'initial_delay_seconds': 2214, 'timeout_seconds': 1621, 'period_seconds': 1489, 'failure_threshold': 1812, 'http_get': {'path': 'path_value', 'http_headers': [{'name': 'name_value', 'value': 'value_value'}], 'port': 453}, 'tcp_socket': {'port': 453}, 'grpc': {'port': 453, 'service': 'service_value'}}, 'startup_probe': {}, 'depends_on': ['depends_on_value1', 'depends_on_value2'], 'base_image_uri': 'base_image_uri_value', 'build_info': {'function_target': 'function_target_value', 'source_location': 'source_location_value'}}], 'volumes': [{'name': 'name_value', 'secret': {'secret': 'secret_value', 'items': [{'path': 'path_value', 'version': 'version_value', 'mode': 421}], 'default_mode': 1257}, 'cloud_sql_instance': {'instances': ['instances_value1', 'instances_value2']}, 'empty_dir': {'medium': 1, 'size_limit': 'size_limit_value'}, 'nfs': {'server': 'server_value', 'path': 'path_value', 'read_only': True}, 'gcs': {'bucket': 'bucket_value', 'read_only': True, 'mount_options': ['mount_options_value1', 'mount_options_value2']}}], 'encryption_key': 'encryption_key_value', 'service_mesh': {'mesh': 'mesh_value'}, 'encryption_key_revocation_action': 1, 'encryption_key_shutdown_duration': {'seconds': 751, 'nanos': 543}, 'node_selector': {'accelerator': 'accelerator_value'}}, 'instance_splits': [{'type_': 1, 'revision': 'revision_value', 'percent': 753}], 'scaling': {'manual_instance_count': 2234}, 'observed_generation': 2021, 'terminal_condition': {'type_': 'type__value', 'state': 1, 'message': 'message_value', 'last_transition_time': {}, 'severity': 1, 'reason': 1, 'revision_reason': 1, 'execution_reason': 1}, 'conditions': {}, 'latest_ready_revision': 'latest_ready_revision_value', 'latest_created_revision': 'latest_created_revision_value', 'instance_split_statuses': [{'type_': 1, 'revision': 'revision_value', 'percent': 753}], 'custom_audiences': ['custom_audiences_value1', 'custom_audiences_value2'], 'satisfies_pzs': True, 'reconciling': True, 'etag': 'etag_value'}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_update_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_update_worker_pool") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_update_worker_pool_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_update_worker_pool") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_delete_worker_pool_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_delete_worker_pool") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_delete_worker_pool_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_delete_worker_pool") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_get_iam_policy") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_get_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_get_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_set_iam_policy") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_set_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_set_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkerPoolsRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_worker_pools_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.run_v2.services.worker_pools.transports.WorkerPoolsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_worker_pools_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.run_v2.services.worker_pools.transports.WorkerPoolsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.WorkerPoolsTransport()
        adc.assert_called_once()
...
def test_worker_pools_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.WorkerPoolsGrpcTransport, transports.WorkerPoolsGrpcAsyncIOTransport])
def test_worker_pools_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.WorkerPoolsGrpcTransport, transports.WorkerPoolsGrpcAsyncIOTransport])
def test_worker_pools_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
