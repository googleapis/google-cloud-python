...
def test_get_task_rest_bad_request(transport: str = 'rest', request_type=task.GetTaskRequest):
    client = TasksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_tasks_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = task.ListTasksResponse.to_json(task.ListTasksResponse())
...
def test_list_tasks_rest_bad_request(transport: str = 'rest', request_type=task.ListTasksRequest):
    client = TasksClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
