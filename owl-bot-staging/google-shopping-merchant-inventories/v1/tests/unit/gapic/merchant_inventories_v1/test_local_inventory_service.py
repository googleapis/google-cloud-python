...
from google.shopping.merchant_inventories_v1.services.local_inventory_service import LocalInventoryServiceAsyncClient
from google.shopping.merchant_inventories_v1.services.local_inventory_service import LocalInventoryServiceClient
from google.shopping.merchant_inventories_v1.services.local_inventory_service import pagers
from google.shopping.merchant_inventories_v1.services.local_inventory_service import transports
...
def test__read_environment_variables():
    assert LocalInventoryServiceClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert LocalInventoryServiceClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert LocalInventoryServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert LocalInventoryServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert LocalInventoryServiceClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert LocalInventoryServiceClient._get_client_cert_source(None, False) is None
    assert LocalInventoryServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert LocalInventoryServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert LocalInventoryServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert LocalInventoryServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(LocalInventoryServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceClient))
@mock.patch.object(LocalInventoryServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = LocalInventoryServiceClient._DEFAULT_UNIVERSE
    default_endpoint = LocalInventoryServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = LocalInventoryServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert LocalInventoryServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert LocalInventoryServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == LocalInventoryServiceClient.DEFAULT_MTLS_ENDPOINT
    assert LocalInventoryServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert LocalInventoryServiceClient._get_api_endpoint(None, None, default_universe, "always") == LocalInventoryServiceClient.DEFAULT_MTLS_ENDPOINT
    assert LocalInventoryServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == LocalInventoryServiceClient.DEFAULT_MTLS_ENDPOINT
    assert LocalInventoryServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert LocalInventoryServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        LocalInventoryServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert LocalInventoryServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert LocalInventoryServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert LocalInventoryServiceClient._get_universe_domain(None, None) == LocalInventoryServiceClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        LocalInventoryServiceClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = LocalInventoryServiceClient(credentials=cred)
...
@pytest.mark.parametrize("client_class,transport_name", [
    (LocalInventoryServiceClient, "grpc"),
    (LocalInventoryServiceAsyncClient, "grpc_asyncio"),
    (LocalInventoryServiceClient, "rest"),
])
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.LocalInventoryServiceGrpcTransport, "grpc"),
    (transports.LocalInventoryServiceGrpcAsyncIOTransport, "grpc_asyncio"),
    (transports.LocalInventoryServiceRestTransport, "rest"),
])
...
def test_local_inventory_service_client_get_transport_class():
    ...
    available_transports = [
        transports.LocalInventoryServiceGrpcTransport,
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (LocalInventoryServiceClient, transports.LocalInventoryServiceGrpcTransport, "grpc"),
    (LocalInventoryServiceAsyncClient, transports.LocalInventoryServiceGrpcAsyncIOTransport, "grpc_asyncio"),
    (LocalInventoryServiceClient, transports.LocalInventoryServiceRestTransport, "rest"),
])
@mock.patch.object(LocalInventoryServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceClient))
@mock.patch.object(LocalInventoryServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (LocalInventoryServiceClient, transports.LocalInventoryServiceGrpcTransport, "grpc", "true"),
    (LocalInventoryServiceAsyncClient, transports.LocalInventoryServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (LocalInventoryServiceClient, transports.LocalInventoryServiceGrpcTransport, "grpc", "false"),
    (LocalInventoryServiceAsyncClient, transports.LocalInventoryServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (LocalInventoryServiceClient, transports.LocalInventoryServiceRestTransport, "rest", "true"),
    (LocalInventoryServiceClient, transports.LocalInventoryServiceRestTransport, "rest", "false"),
])
@mock.patch.object(LocalInventoryServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceClient))
@mock.patch.object(LocalInventoryServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceAsyncClient))
...
@pytest.mark.parametrize("client_class", [
    LocalInventoryServiceClient, LocalInventoryServiceAsyncClient
])
...
@mock.patch.object(LocalInventoryServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceClient))
@mock.patch.object(LocalInventoryServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(LocalInventoryServiceAsyncClient))
def test_local_inventory_service_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (LocalInventoryServiceClient, transports.LocalInventoryServiceGrpcTransport, "grpc", grpc_helpers),
    (LocalInventoryServiceAsyncClient, transports.LocalInventoryServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (LocalInventoryServiceClient, transports.LocalInventoryServiceRestTransport, "rest", None),
])
...
def test_local_inventory_service_client_client_options_from_dict():
    with mock.patch('google.shopping.merchant_inventories_v1.services.local_inventory_service.transports.LocalInventoryServiceGrpcTransport.__init__') as grpc_transport:
...
def test_list_local_inventories(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.list_local_inventories),
            '__call__') as call:
...
def test_list_local_inventories_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.list_local_inventories in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_local_inventories] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_local_inventories_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.list_local_inventories in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_local_inventories] = mock_rpc
...
async def test_list_local_inventories_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.list_local_inventories(
            localinventory.ListLocalInventoriesRequest(),
            parent='parent_value',
        )
...
def test_list_local_inventories_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        pager = client.list_local_inventories(request={}, retry=retry, timeout=timeout)
        ...
        assert all(isinstance(i, localinventory.LocalInventory)
                   for i in results)
...
def test_insert_local_inventory(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.insert_local_inventory),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_insert_local_inventory_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.insert_local_inventory in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.insert_local_inventory] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_insert_local_inventory_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = LocalInventoryServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.insert_local_inventory in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.insert_local_inventory] = mock_rpc
...
def test_delete_local_inventory(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.delete_local_inventory),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_delete_local_inventory_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.delete_local_inventory in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.delete_local_inventory] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_delete_local_inventory_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = LocalInventoryServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.delete_local_inventory in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.delete_local_inventory] = mock_rpc
...
async def test_delete_local_inventory_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.delete_local_inventory(
            localinventory.DeleteLocalInventoryRequest(),
            name='name_value',
        )
...
def test_list_local_inventories_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/inventories/v1/{parent=accounts/*/products/*}/localInventories" % client.transport._host, args[1])
...
def test_delete_local_inventory_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/inventories/v1/{name=accounts/*/products/*/localInventories/*}" % client.transport._host, args[1])
...
def test_credentials_transport_error():
    ...
    transport = transports.LocalInventoryServiceGrpcTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    with pytest.raises(ValueError):
        client = LocalInventoryServiceClient(
...
def test_transport_get_channel():
    ...
    transport = transports.LocalInventoryServiceGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
@pytest.mark.parametrize("transport_class", [
    transports.LocalInventoryServiceGrpcTransport,
    transports.LocalInventoryServiceGrpcAsyncIOTransport,
    transports.LocalInventoryServiceRestTransport,
])
...
def test_list_local_inventories_empty_call_grpc():
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = localinventory.ListLocalInventoriesResponse()
        client.list_local_inventories(request=None)
...
def test_insert_local_inventory_empty_call_grpc():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = localinventory.LocalInventory()
        client.insert_local_inventory(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_local_inventory_empty_call_grpc():
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = None
        client.delete_local_inventory(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = LocalInventoryServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_local_inventories_empty_call_grpc_asyncio():
    client = LocalInventoryServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.list_local_inventories(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_insert_local_inventory_empty_call_grpc_asyncio():
    client = LocalInventoryServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.insert_local_inventory(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_delete_local_inventory_empty_call_grpc_asyncio():
    client = LocalInventoryServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.delete_local_inventory(request=None)
...
def test_transport_kind_rest():
    transport = LocalInventoryServiceClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_list_local_inventories_rest_call_success(request_type):
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_local_inventories_rest_interceptors(null_interceptor):
    transport = transports.LocalInventoryServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.LocalInventoryServiceRestInterceptor(),
        )
    client = LocalInventoryServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "post_list_local_inventories") as post, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "post_list_local_inventories_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "pre_list_local_inventories") as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = localinventory.ListLocalInventoriesResponse.to_json(localinventory.ListLocalInventoriesResponse())
        ...
        post_with_metadata.return_value = localinventory.ListLocalInventoriesResponse(), metadata
...
def test_insert_local_inventory_rest_bad_request(request_type=localinventory.InsertLocalInventoryRequest):
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_insert_local_inventory_rest_call_success(request_type):
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    ...
    for field, value in request_init["local_inventory"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_insert_local_inventory_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "post_insert_local_inventory") as post, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "post_insert_local_inventory_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "pre_insert_local_inventory") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = localinventory.LocalInventory.to_json(localinventory.LocalInventory())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = localinventory.LocalInventory(), metadata
...
def test_delete_local_inventory_rest_bad_request(request_type=localinventory.DeleteLocalInventoryRequest):
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_local_inventory_rest_call_success(request_type):
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_local_inventory_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.LocalInventoryServiceRestInterceptor, "pre_delete_local_inventory") as pre:
        pre.assert_not_called()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_local_inventories_empty_call_rest():
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_insert_local_inventory_empty_call_rest():
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_delete_local_inventory_empty_call_rest():
    client = LocalInventoryServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_local_inventory_service_base_transport():
    ...
    with mock.patch('google.shopping.merchant_inventories_v1.services.local_inventory_service.transports.LocalInventoryServiceTransport.__init__') as Transport:
...
def test_local_inventory_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.shopping.merchant_inventories_v1.services.local_inventory_service.transports.LocalInventoryServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_local_inventory_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.shopping.merchant_inventories_v1.services.local_inventory_service.transports.LocalInventoryServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.LocalInventoryServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize(
    ...
    [
        (transports.LocalInventoryServiceGrpcTransport, grpc_helpers),
        (transports.LocalInventoryServiceGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
...
def test_local_inventory_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_local_inventory_service_client_transport_session_collision(transport_name):
    ...
    client1 = LocalInventoryServiceClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = LocalInventoryServiceClient(
        credentials=creds2,
        transport=transport_name,
    )
...
@pytest.mark.parametrize("transport_class", [transports.LocalInventoryServiceGrpcTransport, transports.LocalInventoryServiceGrpcAsyncIOTransport])
def test_local_inventory_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.LocalInventoryServiceGrpcTransport, transports.LocalInventoryServiceGrpcAsyncIOTransport])
def test_local_inventory_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
@pytest.mark.parametrize("client_class,transport_class", [
    (LocalInventoryServiceClient, transports.LocalInventoryServiceGrpcTransport),
    (LocalInventoryServiceAsyncClient, transports.LocalInventoryServiceGrpcAsyncIOTransport),
])
