...
from google.shopping.merchant_reports_v1beta.services.report_service import ReportServiceAsyncClient
from google.shopping.merchant_reports_v1beta.services.report_service import ReportServiceClient
from google.shopping.merchant_reports_v1beta.services.report_service import pagers
from google.shopping.merchant_reports_v1beta.services.report_service import transports
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ReportServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ReportServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ReportServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert ReportServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ReportServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ReportServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ReportServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ReportServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReportServiceClient))
@mock.patch.object(ReportServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ReportServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ReportServiceClient._DEFAULT_UNIVERSE
    default_endpoint = ReportServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ReportServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ReportServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ReportServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ReportServiceClient.DEFAULT_MTLS_ENDPOINT
    assert ReportServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ReportServiceClient._get_api_endpoint(None, None, default_universe, "always") == ReportServiceClient.DEFAULT_MTLS_ENDPOINT
    ...
    assert ReportServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ReportServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ReportServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ReportServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ReportServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ReportServiceClient._get_universe_domain(None, None) == ReportServiceClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize("client_class,transport_name", [
    (ReportServiceClient, "grpc"),
    (ReportServiceAsyncClient, "grpc_asyncio"),
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.ReportServiceGrpcTransport, "grpc"),
    (transports.ReportServiceGrpcAsyncIOTransport, "grpc_asyncio"),
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (ReportServiceClient, transports.ReportServiceGrpcTransport, "grpc"),
    (ReportServiceAsyncClient, transports.ReportServiceGrpcAsyncIOTransport, "grpc_asyncio"),
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (ReportServiceClient, transports.ReportServiceGrpcTransport, "grpc", "true"),
    (ReportServiceAsyncClient, transports.ReportServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (ReportServiceClient, transports.ReportServiceGrpcTransport, "grpc", "false"),
    (ReportServiceAsyncClient, transports.ReportServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
@pytest.mark.parametrize("client_class", [
    ReportServiceClient, ReportServiceAsyncClient
])
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ReportServiceClient, transports.ReportServiceGrpcTransport, "grpc", grpc_helpers),
    (ReportServiceAsyncClient, transports.ReportServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_search_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, reports.ReportRow)
                   for i in results)
...
def test_credentials_transport_error():
    ...
    transport = transports.ReportServiceGrpcTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    with pytest.raises(ValueError):
        client = ReportServiceClient(
...
def test_transport_get_channel():
    ...
    transport = transports.ReportServiceGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
@pytest.mark.parametrize("transport_class", [
    transports.ReportServiceGrpcTransport,
    transports.ReportServiceGrpcAsyncIOTransport,
    transports.ReportServiceRestTransport,
])
...
def test_search_empty_call_grpc():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = reports.SearchRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = ReportServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_search_rest_bad_request(request_type=reports.SearchRequest):
    client = ReportServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_search_rest_interceptors(null_interceptor):
    transport = transports.ReportServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ReportServiceRestInterceptor(),
        )
    client = ReportServiceClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_search") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_search_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_search") as pre:
        ...
        post_with_metadata.return_value = reports.SearchResponse(), metadata
...
@pytest.mark.parametrize(
    ...
    [
        (transports.ReportServiceGrpcTransport, grpc_helpers),
        (transports.ReportServiceGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
...
def test_report_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.ReportServiceGrpcTransport, transports.ReportServiceGrpcAsyncIOTransport])
def test_report_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ReportServiceGrpcTransport, transports.ReportServiceGrpcAsyncIOTransport])
def test_report_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
@pytest.mark.parametrize("client_class,transport_class", [
    (ReportServiceClient, transports.ReportServiceGrpcTransport),
    (ReportServiceAsyncClient, transports.ReportServiceGrpcAsyncIOTransport),
])
