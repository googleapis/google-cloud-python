...
from .base import ReportServiceTransport, DEFAULT_CLIENT_INFO as BASE_DEFAULT_CLIENT_INFO
...
class ReportServiceRestTransport(ReportServiceTransport):
    ...
    class _Search(ReportServiceRestStub):
        def __hash__(self):
            return hash("Search")
        ...
        def __call__(self,
            ...
            http_options: List[Dict[str, str]] = [{
                'method': 'post',
                'uri': '/reports/v1beta/{parent=accounts/*}/reports:search',
                'body': '*',
            },
            ]
            request, metadata = self._interceptor.pre_search(request, metadata)
            pb_request = reports.SearchRequest.pb(request)
