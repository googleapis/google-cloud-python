...
from google.cloud.parallelstore_v1beta.services.parallelstore import ParallelstoreAsyncClient
from google.cloud.parallelstore_v1beta.services.parallelstore import ParallelstoreClient
from google.cloud.parallelstore_v1beta.services.parallelstore import pagers
from google.cloud.parallelstore_v1beta.services.parallelstore import transports
...
def test__get_default_mtls_endpoint():
    ...
    assert ParallelstoreClient._get_default_mtls_endpoint(None) is None
    assert ParallelstoreClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert ParallelstoreClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert ParallelstoreClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert ParallelstoreClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert ParallelstoreClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi

def test__read_environment_variables():
    assert ParallelstoreClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert ParallelstoreClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ParallelstoreClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ParallelstoreClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ParallelstoreClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert ParallelstoreClient._get_client_cert_source(None, False) is None
    assert ParallelstoreClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ParallelstoreClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ParallelstoreClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ParallelstoreClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ParallelstoreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreClient))
@mock.patch.object(ParallelstoreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ParallelstoreClient._DEFAULT_UNIVERSE
    default_endpoint = ParallelstoreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ParallelstoreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ParallelstoreClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ParallelstoreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ParallelstoreClient.DEFAULT_MTLS_ENDPOINT
    assert ParallelstoreClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ParallelstoreClient._get_api_endpoint(None, None, default_universe, "always") == ParallelstoreClient.DEFAULT_MTLS_ENDPOINT
    assert ParallelstoreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ParallelstoreClient.DEFAULT_MTLS_ENDPOINT
    assert ParallelstoreClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ParallelstoreClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ParallelstoreClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ParallelstoreClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ParallelstoreClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ParallelstoreClient._get_universe_domain(None, None) == ParallelstoreClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize("client_class,transport_name", [
    (ParallelstoreClient, "grpc"),
    (ParallelstoreAsyncClient, "grpc_asyncio"),
    (ParallelstoreClient, "rest"),
])
def test_parallelstore_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, 'from_service_account_info') as factory:
        ...
        assert client.transport._host == (
            'parallelstore.googleapis.com:443'
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.ParallelstoreGrpcTransport, "grpc"),
    (transports.ParallelstoreGrpcAsyncIOTransport, "grpc_asyncio"),
    (transports.ParallelstoreRestTransport, "rest"),
])
def test_parallelstore_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_parallelstore_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
def test_parallelstore_client_get_transport_class():
    transport = ParallelstoreClient.get_transport_class()
    available_transports = [
        transports.ParallelstoreGrpcTransport,
        transports.ParallelstoreRestTransport,
    ]
    assert transport in available_transports

    transport = ParallelstoreClient.get_transport_class("grpc")
    assert transport == transports.ParallelstoreGrpcTransport


@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (ParallelstoreClient, transports.ParallelstoreGrpcTransport, "grpc"),
    (ParallelstoreAsyncClient, transports.ParallelstoreGrpcAsyncIOTransport, "grpc_asyncio"),
    (ParallelstoreClient, transports.ParallelstoreRestTransport, "rest"),
])
@mock.patch.object(ParallelstoreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreClient))
@mock.patch.object(ParallelstoreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreAsyncClient))
def test_parallelstore_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(ParallelstoreClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (ParallelstoreClient, transports.ParallelstoreGrpcTransport, "grpc", "true"),
    (ParallelstoreAsyncClient, transports.ParallelstoreGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (ParallelstoreClient, transports.ParallelstoreGrpcTransport, "grpc", "false"),
    (ParallelstoreAsyncClient, transports.ParallelstoreGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (ParallelstoreClient, transports.ParallelstoreRestTransport, "rest", "true"),
    (ParallelstoreClient, transports.ParallelstoreRestTransport, "rest", "false"),
])
@mock.patch.object(ParallelstoreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreClient))
@mock.patch.object(ParallelstoreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreAsyncClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_parallelstore_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [
    ParallelstoreClient, ParallelstoreAsyncClient
])
@mock.patch.object(ParallelstoreClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ParallelstoreClient))
@mock.patch.object(ParallelstoreAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ParallelstoreAsyncClient))
def test_parallelstore_client_get_mtls_endpoint_and_cert_source(client_class):
    mock_client_cert_source = mock.Mock()
...
@mock.patch.object(ParallelstoreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreClient))
@mock.patch.object(ParallelstoreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ParallelstoreAsyncClient))
def test_parallelstore_client_client_api_endpoint(client_class):
...
def test_parallelstore_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ParallelstoreClient, transports.ParallelstoreGrpcTransport, "grpc", grpc_helpers),
    (ParallelstoreAsyncClient, transports.ParallelstoreGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (ParallelstoreClient, transports.ParallelstoreRestTransport, "rest", None),
])
def test_parallelstore_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_parallelstore_client_client_options_from_dict():
    with mock.patch('google.cloud.parallelstore_v1beta.services.parallelstore.transports.ParallelstoreGrpcTransport.__init__') as grpc_transport:
...
def test_parallelstore_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
    ...
    with mock.patch.object(
        ...
        create_channel.assert_called_with(
            ...
            scopes=None,
            default_host="parallelstore.googleapis.com",
...
def test_list_instances(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = parallelstore.ListInstancesResponse(
...
@pytest.mark.asyncio
async def test_list_instances_async(transport: str = 'grpc_asyncio', request_type=parallelstore.ListInstancesRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(parallelstore.ListInstancesResponse(
            next_page_token='next_page_token_value',
            unreachable=['unreachable_value'],
        ))
...
def test_list_instances_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, parallelstore.Instance)
                   for i in results)
...
def test_get_instance(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = parallelstore.Instance(
            name='name_value',
    ...
    assert response.file_stripe_level == parallelstore.FileStripeLevel.FILE_STRIPE_LEVEL_MIN
    assert response.directory_stripe_level == parallelstore.DirectoryStripeLevel.DIRECTORY_STRIPE_LEVEL_MIN
...
@pytest.mark.asyncio
async def test_get_instance_async(transport: str = 'grpc_asyncio', request_type=parallelstore.GetInstanceRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(parallelstore.Instance(
            name='name_value',
            description='description_value',
            state=parallelstore.Instance.State.CREATING,
            capacity_gib=1247,
...
@pytest.mark.asyncio
async def test_create_instance_async(transport: str = 'grpc_asyncio', request_type=parallelstore.CreateInstanceRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_create_instance_flattened():
    ...
    with mock.patch.object(
        ...
        client.create_instance(
            parent='parent_value',
            instance=parallelstore.Instance(name='name_value'),
            instance_id='instance_id_value',
        )
...
@pytest.mark.asyncio
async def test_update_instance_async(transport: str = 'grpc_asyncio', request_type=parallelstore.UpdateInstanceRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_delete_instance_async(transport: str = 'grpc_asyncio', request_type=parallelstore.DeleteInstanceRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_import_data_async(transport: str = 'grpc_asyncio', request_type=parallelstore.ImportDataRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_export_data_async(transport: str = 'grpc_asyncio', request_type=parallelstore.ExportDataRequest):
    client = ParallelstoreAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
...
def test_list_instances_rest_required_fields(request_type=parallelstore.ListInstancesRequest):
    ...
    return_value = parallelstore.ListInstancesResponse()
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            return_value = parallelstore.ListInstancesResponse.pb(return_value)
...
def test_list_instances_rest_unset_required_fields():
    transport = transports.ParallelstoreRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_list_instances_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1beta/{parent=projects/*/locations/*}/instances" % client.transport._host, args[1])
...
def test_list_instances_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        response = tuple(parallelstore.ListInstancesResponse.to_json(x) for x in response)
...
def test_get_instance_rest_required_fields(request_type=parallelstore.GetInstanceRequest):
    transport_class = transports.ParallelstoreRestTransport
    ...
    # Designate an appropriate value for the returned response.
    return_value = parallelstore.Instance()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, 'request') as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
...
def test_get_instance_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{name=projects/*/locations/*/instances/*}" % client.transport._host, args[1])
...
def test_create_instance_rest_required_fields(request_type=parallelstore.CreateInstanceRequest):
    transport_class = transports.ParallelstoreRestTransport
...
def test_update_instance_rest_required_fields(request_type=parallelstore.UpdateInstanceRequest):
    transport_class = transports.ParallelstoreRestTransport
...
def test_update_instance_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("requestId", "updateMask", )) & set(("updateMask", "instance", )))
...
def test_update_instance_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{instance.name=projects/*/locations/*/instances/*}" % client.transport._host, args[1])
...
def test_delete_instance_rest_required_fields(request_type=parallelstore.DeleteInstanceRequest):
    transport_class = transports.ParallelstoreRestTransport
...
def test_transport_get_channel():
    ...
    transport = transports.ParallelstoreGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
def test_transport_kind_grpc_asyncio():
    transport = ParallelstoreAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = ParallelstoreClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
    assert transport.kind == "rest"


def test_list_instances_rest_bad_request(request_type=parallelstore.ListInstancesRequest):
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_instances_rest_interceptors(null_interceptor):
    transport = transports.ParallelstoreRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ParallelstoreRestInterceptor(),
        )
    client = ParallelstoreClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_list_instances") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_list_instances_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_list_instances") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.ListInstancesRequest.pb(parallelstore.ListInstancesRequest())
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = parallelstore.ListInstancesResponse.to_json(parallelstore.ListInstancesResponse())
        ...
        post_with_metadata.return_value = parallelstore.ListInstancesResponse(), metadata
...
def test_get_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_get_instance") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_get_instance_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_get_instance") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.GetInstanceRequest.pb(parallelstore.GetInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_create_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_create_instance") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_create_instance_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_create_instance") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.CreateInstanceRequest.pb(parallelstore.CreateInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_instance_rest_bad_request(request_type=parallelstore.UpdateInstanceRequest):
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_update_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_update_instance") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_update_instance_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_update_instance") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.UpdateInstanceRequest.pb(parallelstore.UpdateInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_delete_instance_rest_bad_request(request_type=parallelstore.DeleteInstanceRequest):
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_instance_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_delete_instance") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_delete_instance_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_delete_instance") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.DeleteInstanceRequest.pb(parallelstore.DeleteInstanceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_import_data_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_import_data") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_import_data_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_import_data") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.ImportDataRequest.pb(parallelstore.ImportDataRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_export_data_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_export_data") as post, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "post_export_data_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ParallelstoreRestInterceptor, "pre_export_data") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = parallelstore.ExportDataRequest.pb(parallelstore.ExportDataRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_parallelstore_rest_lro_client():
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_parallelstore_base_transport_error():
    # Passing both a credentials object and credentials_file should raise an error
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.ParallelstoreTransport(
            credentials=ga_credentials.AnonymousCredentials(),
            credentials_file="credentials.json"
        )


def test_parallelstore_base_transport():
    # Instantiate the base transport.
    with mock.patch('google.cloud.parallelstore_v1beta.services.parallelstore.transports.ParallelstoreTransport.__init__') as Transport:
...
def test_parallelstore_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.parallelstore_v1beta.services.parallelstore.transports.ParallelstoreTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_parallelstore_base_transport_with_adc():
    # Test the default credentials are used if credentials and credentials_file are None.
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.parallelstore_v1beta.services.parallelstore.transports.ParallelstoreTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ParallelstoreTransport()
        adc.assert_called_once()
...
def test_parallelstore_auth_adc():
...
def test_parallelstore_transport_auth_adc(transport_class):
...
def test_parallelstore_transport_auth_gdch_credentials(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.ParallelstoreGrpcTransport, grpc_helpers),
        (transports.ParallelstoreGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
def test_parallelstore_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
@pytest.mark.parametrize("transport_class", [transports.ParallelstoreGrpcTransport, transports.ParallelstoreGrpcAsyncIOTransport])
def test_parallelstore_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_parallelstore_http_transport_client_cert_source_for_mtls():
    cred = ga_credentials.AnonymousCredentials()
...
def test_parallelstore_host_no_port(transport_name):
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='parallelstore.googleapis.com'),
         transport=transport_name,
    )
    assert client.transport._host == (
        'parallelstore.googleapis.com:443'
        if transport_name in ['grpc', 'grpc_asyncio']
        else 'https://parallelstore.googleapis.com'
    )
...
def test_parallelstore_host_with_port(transport_name):
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='parallelstore.googleapis.com:8000'),
        transport=transport_name,
    )
    assert client.transport._host == (
        'parallelstore.googleapis.com:8000'
        if transport_name in ['grpc', 'grpc_asyncio']
        else 'https://parallelstore.googleapis.com:8000'
    )

@pytest.mark.parametrize("transport_name", [
    "rest",
])
def test_parallelstore_client_transport_session_collision(transport_name):
    creds1 = ga_credentials.AnonymousCredentials()
    creds2 = ga_credentials.AnonymousCredentials()
    client1 = ParallelstoreClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = ParallelstoreClient(
        credentials=creds2,
        transport=transport_name,
    )
...
def test_parallelstore_grpc_asyncio_transport_channel():
...
@pytest.mark.parametrize("transport_class", [transports.ParallelstoreGrpcTransport, transports.ParallelstoreGrpcAsyncIOTransport])
def test_parallelstore_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ParallelstoreGrpcTransport, transports.ParallelstoreGrpcAsyncIOTransport])
def test_parallelstore_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_parallelstore_grpc_lro_client():
    client = ParallelstoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport='grpc',
    )
...
def test_parallelstore_grpc_lro_async_client():
    client = ParallelstoreAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport='grpc_asyncio',
    )
...
def test_address_path():
    ...
    region = "clam"
    address = "whelk"
    expected = "projects/{project}/regions/{region}/addresses/{address}".format(project=project, region=region, address=address, )
    actual = ParallelstoreClient.address_path(project, region, address)
    assert expected == actual


def test_parse_address_path():
    expected = {
        "project": "octopus",
        "region": "oyster",
        "address": "nudibranch",
    }
    path = ParallelstoreClient.address_path(**expected)

    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_address_path(path)
    assert expected == actual
...
def test_instance_path():
    ...
    actual = ParallelstoreClient.instance_path(project, location, instance)
    assert expected == actual
...
def test_parse_instance_path():
    ...
    path = ParallelstoreClient.instance_path(**expected)

    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_instance_path(path)
    assert expected == actual
...
def test_network_path():
    ...
    actual = ParallelstoreClient.network_path(project, network)
    assert expected == actual
...
def test_parse_network_path():
    ...
    path = ParallelstoreClient.network_path(**expected)

    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_network_path(path)
    assert expected == actual
...
def test_common_billing_account_path():
    ...
    actual = ParallelstoreClient.common_billing_account_path(billing_account)
    assert expected == actual
...
def test_parse_common_billing_account_path():
    ...
    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_common_billing_account_path(path)
    assert expected == actual

def test_common_folder_path():
    folder = "scallop"
    expected = "folders/{folder}".format(folder=folder, )
    actual = ParallelstoreClient.common_folder_path(folder)
    assert expected == actual
...
def test_parse_common_folder_path():
    ...
    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_common_folder_path(path)
    assert expected == actual

def test_common_organization_path():
    organization = "squid"
    expected = "organizations/{organization}".format(organization=organization, )
    actual = ParallelstoreClient.common_organization_path(organization)
    assert expected == actual
...
def test_parse_common_organization_path():
    ...
    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_common_organization_path(path)
    assert expected == actual

def test_common_project_path():
    project = "whelk"
    expected = "projects/{project}".format(project=project, )
    actual = ParallelstoreClient.common_project_path(project)
    assert expected == actual
...
def test_parse_common_project_path():
    ...
    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_common_project_path(path)
    assert expected == actual
...
def test_common_location_path():
    ...
    actual = ParallelstoreClient.common_location_path(project, location)
    assert expected == actual
...
def test_parse_common_location_path():
    ...
    # Check that the path construction is reversible.
    actual = ParallelstoreClient.parse_common_location_path(path)
    assert expected == actual
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.ParallelstoreTransport, '_prep_wrapped_messages') as prep:
        client = ParallelstoreClient(
            credentials=ga_credentials.AnonymousCredentials(),
    ...
    with mock.patch.object(transports.ParallelstoreTransport, '_prep_wrapped_messages') as prep:
        transport_class = ParallelstoreClient.get_transport_class()
        transport = transport_class(
            credentials=ga_credentials.AnonymousCredentials(),
            client_info=client_info,
        )
...
@pytest.mark.parametrize("client_class,transport_class", [
    (ParallelstoreClient, transports.ParallelstoreGrpcTransport),
    (ParallelstoreAsyncClient, transports.ParallelstoreGrpcAsyncIOTransport),
])
