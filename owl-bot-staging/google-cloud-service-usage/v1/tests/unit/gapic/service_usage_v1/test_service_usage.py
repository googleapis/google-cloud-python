...
def test__read_environment_variables():
    assert ServiceUsageClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert ServiceUsageClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            ServiceUsageClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ServiceUsageClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ServiceUsageClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ServiceUsageClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert ServiceUsageClient._get_client_cert_source(None, False) is None
    assert ServiceUsageClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ServiceUsageClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ServiceUsageClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ServiceUsageClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ServiceUsageClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ServiceUsageClient))
@mock.patch.object(ServiceUsageAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ServiceUsageAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ServiceUsageClient._DEFAULT_UNIVERSE
    default_endpoint = ServiceUsageClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ServiceUsageClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ServiceUsageClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ServiceUsageClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ServiceUsageClient.DEFAULT_MTLS_ENDPOINT
    assert ServiceUsageClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ServiceUsageClient._get_api_endpoint(None, None, default_universe, "always") == ServiceUsageClient.DEFAULT_MTLS_ENDPOINT
    assert ServiceUsageClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ServiceUsageClient.DEFAULT_MTLS_ENDPOINT
    assert ServiceUsageClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ServiceUsageClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ServiceUsageClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ServiceUsageClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ServiceUsageClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ServiceUsageClient._get_universe_domain(None, None) == ServiceUsageClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize("client_class", [
    ServiceUsageClient, ServiceUsageAsyncClient
])
...
@mock.patch.object(ServiceUsageClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ServiceUsageClient))
@mock.patch.object(ServiceUsageAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ServiceUsageAsyncClient))
def test_service_usage_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ServiceUsageClient, transports.ServiceUsageGrpcTransport, "grpc", grpc_helpers),
    (ServiceUsageAsyncClient, transports.ServiceUsageGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_enable_service_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(
        ...
        client.enable_service(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_enable_service_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.enable_service in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.enable_service] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_enable_service_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.enable_service in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.enable_service] = mock_rpc
...
def test_disable_service_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client.disable_service(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_disable_service_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
        ...
        # Ensure method has been cached
        assert client._transport.disable_service in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.disable_service] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_disable_service_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ServiceUsageAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.disable_service in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.disable_service] = mock_rpc
...
def test_batch_enable_services_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client.batch_enable_services(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_batch_enable_services_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
        ...
        # Ensure method has been cached
        assert client._transport.batch_enable_services in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.batch_enable_services] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_batch_enable_services_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ServiceUsageAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.batch_enable_services in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.batch_enable_services] = mock_rpc
...
def test_batch_get_services(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.batch_get_services),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_batch_get_services_non_empty_request_with_auto_populated_field():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client.batch_get_services(request=request)
        call.assert_called()
        _, args, _ = call.mock_calls[0]
...
def test_batch_get_services_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
        ...
        # Ensure method has been cached
        assert client._transport.batch_get_services in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.batch_get_services] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_batch_get_services_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ServiceUsageAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.batch_get_services in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.batch_get_services] = mock_rpc
...
def test_enable_service_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
...
def test_disable_service_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
...
def test_batch_enable_services_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
...
def test_batch_get_services_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ServiceUsageClient(
...
def test_enable_service_empty_call_grpc():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.enable_service(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_disable_service_empty_call_grpc():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.disable_service(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = serviceusage.DisableServiceRequest()
...
def test_get_service_empty_call_grpc():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = serviceusage.GetServiceRequest()
...
def test_list_services_empty_call_grpc():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = serviceusage.ListServicesRequest()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_batch_enable_services_empty_call_grpc():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.batch_enable_services(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = serviceusage.BatchEnableServicesRequest()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_batch_get_services_empty_call_grpc():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = serviceusage.BatchGetServicesResponse()
        client.batch_get_services(request=None)

        # Establish that the underlying stub method was called.
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = serviceusage.BatchGetServicesRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = ServiceUsageAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_enable_service_empty_call_grpc_asyncio():
    client = ServiceUsageAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.enable_service(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_disable_service_empty_call_grpc_asyncio():
    client = ServiceUsageAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.disable_service(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_batch_enable_services_empty_call_grpc_asyncio():
    client = ServiceUsageAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.batch_enable_services(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_batch_get_services_empty_call_grpc_asyncio():
    client = ServiceUsageAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.batch_get_services(request=None)
...
def test_transport_kind_rest():
    transport = ServiceUsageClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
    assert transport.kind == "rest"


def test_enable_service_rest_bad_request(request_type=serviceusage.EnableServiceRequest):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_enable_service_rest_interceptors(null_interceptor):
    transport = transports.ServiceUsageRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ServiceUsageRestInterceptor(),
        )
    client = ServiceUsageClient(transport=transport)
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_enable_service") as post, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_enable_service_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "pre_enable_service") as pre:
...
def test_disable_service_rest_bad_request(request_type=serviceusage.DisableServiceRequest):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_disable_service_rest_call_success(request_type):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_disable_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_disable_service") as post, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_disable_service_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "pre_disable_service") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_get_service") as post, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_get_service_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "pre_get_service") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = resources.Service.to_json(resources.Service())
        req.return_value.content = return_value
...
def test_list_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_list_services") as post, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_list_services_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "pre_list_services") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = serviceusage.ListServicesResponse.to_json(serviceusage.ListServicesResponse())
        req.return_value.content = return_value
...
def test_batch_enable_services_rest_bad_request(request_type=serviceusage.BatchEnableServicesRequest):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_batch_enable_services_rest_call_success(request_type):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_batch_enable_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_batch_enable_services") as post, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_batch_enable_services_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "pre_batch_enable_services") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_batch_get_services_rest_bad_request(request_type=serviceusage.BatchGetServicesRequest):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_batch_get_services_rest_call_success(request_type):
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_batch_get_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_batch_get_services") as post, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "post_batch_get_services_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceUsageRestInterceptor, "pre_batch_get_services") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = serviceusage.BatchGetServicesResponse.to_json(serviceusage.BatchGetServicesResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = serviceusage.BatchGetServicesResponse(), metadata
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_enable_service_empty_call_rest():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_disable_service_empty_call_rest():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_batch_enable_services_empty_call_rest():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_batch_get_services_empty_call_rest():
    client = ServiceUsageClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
