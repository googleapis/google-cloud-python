...
class ListAggregateProductStatusesPager:
    """A pager for iterating through ``list_aggregate_product_statuses`` requests.

    This class thinly wraps an initial
    :class:`google.shopping.merchant_issueresolution_v1beta.types.ListAggregateProductStatusesResponse` object, and
    provides an ``__iter__`` method to iterate through its
    ``aggregate_product_statuses`` field.

    If there are more pages, the ``__iter__`` method will make additional
    ``ListAggregateProductStatuses`` requests and continue to iterate
    through the ``aggregate_product_statuses`` field on the
    corresponding responses.

    All the usual :class:`google.shopping.merchant_issueresolution_v1beta.types.ListAggregateProductStatusesResponse`
    ...
    def __init__(self,
        ...
        Args:
            ...
            request (google.shopping.merchant_issueresolution_v1beta.types.ListAggregateProductStatusesRequest):
                The initial request object.
            response (google.shopping.merchant_issueresolution_v1beta.types.ListAggregateProductStatusesResponse):
    ...
    def __iter__(self) -> Iterator[aggregateproductstatuses.AggregateProductStatus]:
        for page in self.pages:
            yield from page.aggregate_product_statuses

    def __repr__(self) -> str:
        return '{0}<{1!r}>'.format(self.__class__.__name__, self._response)


class ListAggregateProductStatusesAsyncPager:
    """A pager for iterating through ``list_aggregate_product_statuses`` requests.
