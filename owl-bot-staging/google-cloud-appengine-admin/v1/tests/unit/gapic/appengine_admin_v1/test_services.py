...
def test_list_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value._content = appengine.ListServicesResponse.to_json(appengine.ListServicesResponse())
...
def test_list_services_rest_bad_request(transport: str = 'rest', request_type=appengine.ListServicesRequest):
    client = ServicesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_service_rest_bad_request(transport: str = 'rest', request_type=appengine.GetServiceRequest):
    client = ServicesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_service_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'name': 'apps/sample1/services/sample2'}
    request_init["service"] = {'name': 'name_value', 'id': 'id_value', 'split': {'shard_by': 1, 'allocations': {}}, 'labels': {}, 'network_settings': {'ingress_traffic_allowed': 1}}
...
def test_update_service_rest_bad_request(transport: str = 'rest', request_type=appengine.UpdateServiceRequest):
    client = ServicesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_service_rest_bad_request(transport: str = 'rest', request_type=appengine.DeleteServiceRequest):
    client = ServicesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_services_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.appengine_admin_v1.services.services.transports.ServicesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_services_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.appengine_admin_v1.services.services.transports.ServicesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ServicesTransport()
        adc.assert_called_once()
