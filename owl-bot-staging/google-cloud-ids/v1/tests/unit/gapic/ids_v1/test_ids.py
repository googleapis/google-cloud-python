...
def test_list_endpoints_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, ids.Endpoint)
                   for i in results)
...
def test_create_endpoint_rest_required_fields(request_type=ids.CreateEndpointRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_endpoint._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("endpoint_id", "request_id", ))
    jsonified_request.update(unset_fields)
...
def test_create_endpoint_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("endpointId", "requestId", )) & set(("parent", "endpointId", "endpoint", )))
...
def test_transport_kind_grpc_asyncio():
    transport = IDSAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
def test_transport_kind_rest():
    transport = IDSClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_list_endpoints_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.IDSRestInterceptor, "post_list_endpoints_with_metadata") as post_with_metadata, \
...
def test_get_endpoint_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.IDSRestInterceptor, "post_get_endpoint_with_metadata") as post_with_metadata, \
...
def test_create_endpoint_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init["endpoint"] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'labels': {}, 'network': 'network_value', 'endpoint_forwarding_rule': 'endpoint_forwarding_rule_value', 'endpoint_ip': 'endpoint_ip_value', 'description': 'description_value', 'severity': 1, 'state': 1, 'traffic_logs': True}
...
def test_create_endpoint_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.IDSRestInterceptor, "post_create_endpoint") as post, \
        mock.patch.object(transports.IDSRestInterceptor, "post_create_endpoint_with_metadata") as post_with_metadata, \
...
def test_delete_endpoint_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.IDSRestInterceptor, "post_delete_endpoint") as post, \
        mock.patch.object(transports.IDSRestInterceptor, "post_delete_endpoint_with_metadata") as post_with_metadata, \
...
def test_ids_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.ids_v1.services.ids.transports.IDSTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_ids_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.ids_v1.services.ids.transports.IDSTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.IDSTransport()
        adc.assert_called_once()
...
def test_ids_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.IDSGrpcTransport, transports.IDSGrpcAsyncIOTransport])
def test_ids_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.IDSGrpcTransport, transports.IDSGrpcAsyncIOTransport])
def test_ids_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
