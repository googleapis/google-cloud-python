...
from .transports.grpc import DeploymentsGrpcTransport
from .transports.grpc_asyncio import DeploymentsGrpcAsyncIOTransport
from .transports.rest import DeploymentsRestTransport


class DeploymentsClientMeta(type):
    """Metaclass for the Deployments client.

    This provides class-level methods for building and retrieving
    support objects (e.g. transport) without polluting the client instance
    objects.
    """
    _transport_registry = OrderedDict()  # type: Dict[str, Type[DeploymentsTransport]]
    _transport_registry["grpc"] = DeploymentsGrpcTransport
    _transport_registry["grpc_asyncio"] = DeploymentsGrpcAsyncIOTransport
    _transport_registry["rest"] = DeploymentsRestTransport
...
class DeploymentsClient(metaclass=DeploymentsClientMeta):
    ...
    def from_service_account_info(cls, info: dict, *args, **kwargs):
        ...
        Returns:
            DeploymentsClient: The constructed client.
        """
    ...
    def transport(self) -> DeploymentsTransport:
        ...
        Returns:
            DeploymentsTransport: The transport used by the client
                instance.
        """
        return self._transport

    @staticmethod
    def deployment_path(project: str,location: str,agent: str,environment: str,deployment: str,) -> str:
        """Returns a fully-qualified deployment string."""
        return "projects/{project}/locations/{location}/agents/{agent}/environments/{environment}/deployments/{deployment}".format(project=project, location=location, agent=agent, environment=environment, deployment=deployment, )

    @staticmethod
    def parse_deployment_path(path: str) -> Dict[str,str]:
        """Parses a deployment path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/agents/(?P<agent>.+?)/environments/(?P<environment>.+?)/deployments/(?P<deployment>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def experiment_path(project: str,location: str,agent: str,environment: str,experiment: str,) -> str:
        """Returns a fully-qualified experiment string."""
        return "projects/{project}/locations/{location}/agents/{agent}/environments/{environment}/experiments/{experiment}".format(project=project, location=location, agent=agent, environment=environment, experiment=experiment, )

    @staticmethod
    def parse_experiment_path(path: str) -> Dict[str,str]:
        """Parses a experiment path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/agents/(?P<agent>.+?)/environments/(?P<environment>.+?)/experiments/(?P<experiment>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            _default_universe = DeploymentsClient._DEFAULT_UNIVERSE
            if universe_domain != _default_universe:
                raise MutualTLSChannelError(f"mTLS is not supported in any universe other than {_default_universe}.")
            api_endpoint = DeploymentsClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = DeploymentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = DeploymentsClient._DEFAULT_UNIVERSE
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            DeploymentsClient._compare_universes(self.universe_domain, self.transport._credentials))
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, DeploymentsTransport, Callable[..., DeploymentsTransport]]] = None,
        ...
        Args:
            ...
            transport (Optional[Union[str,DeploymentsTransport,Callable[..., DeploymentsTransport]]]):
                The transport to use, or a Callable that constructs and returns a new transport.
                If a Callable is given, it will be called with the same set of initialization
                arguments as used in the DeploymentsTransport constructor.
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = DeploymentsClient._read_environment_variables()
        self._client_cert_source = DeploymentsClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = DeploymentsClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, DeploymentsTransport)
        if transport_provided:
            # transport is a DeploymentsTransport instance.
            ...
            self._transport = cast(DeploymentsTransport, transport)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[DeploymentsTransport], Callable[..., DeploymentsTransport]] = (
    ...
    def list_deployments(self,
            request: Optional[Union[deployment.ListDeploymentsRequest, dict]] = None,
            *,
        ...
        if not isinstance(request, deployment.ListDeploymentsRequest):
    ...
    def get_deployment(self,
            request: Optional[Union[deployment.GetDeploymentRequest, dict]] = None,
            *,
        ...
        # - Use the request object if provided (there's no risk of modifying the input as
        #   there are no flattened fields), or create one.
        if not isinstance(request, deployment.GetDeploymentRequest):
