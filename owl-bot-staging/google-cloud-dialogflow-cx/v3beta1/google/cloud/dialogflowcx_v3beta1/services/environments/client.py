...
class EnvironmentsClient(metaclass=EnvironmentsClientMeta):
    ...
    @staticmethod
    def continuous_test_result_path(project: str,location: str,agent: str,environment: str,continuous_test_result: str,) -> str:
        """Returns a fully-qualified continuous_test_result string."""
        return "projects/{project}/locations/{location}/agents/{agent}/environments/{environment}/continuousTestResults/{continuous_test_result}".format(project=project, location=location, agent=agent, environment=environment, continuous_test_result=continuous_test_result, )
    ...
    def parse_continuous_test_result_path(path: str) -> Dict[str,str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/agents/(?P<agent>.+?)/environments/(?P<environment>.+?)/continuousTestResults/(?P<continuous_test_result>.+?)$", path)
