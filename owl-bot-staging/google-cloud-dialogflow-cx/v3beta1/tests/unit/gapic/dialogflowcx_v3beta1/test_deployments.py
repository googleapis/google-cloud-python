...
from google.cloud.dialogflowcx_v3beta1.services.deployments import DeploymentsAsyncClient
...
def test_list_deployments_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, deployment.Deployment)
                   for i in results)
...
def test_list_deployments_rest_unset_required_fields():
    transport = transports.DeploymentsRestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)
...
def test_list_deployments_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.DeploymentsRestInterceptor, "post_list_deployments") as post, \
         mock.patch.object(transports.DeploymentsRestInterceptor, "pre_list_deployments") as pre:
        ...
        req.return_value._content = deployment.ListDeploymentsResponse.to_json(deployment.ListDeploymentsResponse())
...
def test_list_deployments_rest_bad_request(transport: str = 'rest', request_type=deployment.ListDeploymentsRequest):
    client = DeploymentsClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_list_deployments_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v3beta1/{parent=projects/*/locations/*/agents/*/environments/*}/deployments" % client.transport._host, args[1])
...
def test_get_deployment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.DeploymentsRestInterceptor, "post_get_deployment") as post, \
         mock.patch.object(transports.DeploymentsRestInterceptor, "pre_get_deployment") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = deployment.Deployment.to_json(deployment.Deployment())
...
def test_get_deployment_rest_bad_request(transport: str = 'rest', request_type=deployment.GetDeploymentRequest):
    client = DeploymentsClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_get_deployment_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v3beta1/{name=projects/*/locations/*/agents/*/environments/*/deployments/*}" % client.transport._host, args[1])
...
def test_deployments_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.dialogflowcx_v3beta1.services.deployments.transports.DeploymentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_deployments_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.dialogflowcx_v3beta1.services.deployments.transports.DeploymentsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (_AnonymousCredentialsWithUniverseDomain(), None)
        transport = transports.DeploymentsTransport()
        adc.assert_called_once()
...
def test_deployments_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.DeploymentsGrpcTransport, transports.DeploymentsGrpcAsyncIOTransport])
def test_deployments_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.DeploymentsGrpcTransport, transports.DeploymentsGrpcAsyncIOTransport])
def test_deployments_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_deployment_path():
    ...
    environment = "octopus"
    deployment = "oyster"
    expected = "projects/{project}/locations/{location}/agents/{agent}/environments/{environment}/deployments/{deployment}".format(project=project, location=location, agent=agent, environment=environment, deployment=deployment, )
