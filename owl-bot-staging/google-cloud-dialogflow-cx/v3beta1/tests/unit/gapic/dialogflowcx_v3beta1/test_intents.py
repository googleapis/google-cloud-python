...
def test_import_intents(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.import_intents),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_export_intents(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.export_intents),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = operations_pb2.Operation(name='operations/spam')
...
def test_import_intents_rest_required_fields(request_type=intent.ImportIntentsRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).import_intents._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_import_intents_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.IntentsRestInterceptor, "post_import_intents") as post, \
         mock.patch.object(transports.IntentsRestInterceptor, "pre_import_intents") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        client.import_intents(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_import_intents_rest_bad_request(transport: str = 'rest', request_type=intent.ImportIntentsRequest):
    client = IntentsClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_export_intents_rest_required_fields(request_type=intent.ExportIntentsRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).export_intents._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_export_intents_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(operation.Operation, "_set_result_from_operation"), \
         mock.patch.object(transports.IntentsRestInterceptor, "post_export_intents") as post, \
         mock.patch.object(transports.IntentsRestInterceptor, "pre_export_intents") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        client.export_intents(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_export_intents_rest_bad_request(transport: str = 'rest', request_type=intent.ExportIntentsRequest):
    client = IntentsClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
