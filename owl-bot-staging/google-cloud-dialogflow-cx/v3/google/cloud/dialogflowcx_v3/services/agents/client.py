...
class AgentsClient(metaclass=AgentsClientMeta):
    ...
    @staticmethod
    def agent_generative_settings_path(project: str,location: str,agent: str,) -> str:
        """Returns a fully-qualified agent_generative_settings string."""
        return "projects/{project}/locations/{location}/agents/{agent}/generativeSettings".format(project=project, location=location, agent=agent, )
    ...
    def parse_agent_generative_settings_path(path: str) -> Dict[str,str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/agents/(?P<agent>.+?)/generativeSettings$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = AgentsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, AgentsTransport, Callable[..., AgentsTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = AgentsClient._read_environment_variables()
        self._client_cert_source = AgentsClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = AgentsClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[AgentsTransport], Callable[..., AgentsTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.dialogflow.cx_v3.AgentsClient`.",
                    extra = {
    ...
    def get_generative_settings(self,
        ...
        Args:
            request (Union[google.cloud.dialogflowcx_v3.types.GetGenerativeSettingsRequest, dict]):
    ...
    def update_generative_settings(self,
            request: Optional[Union[agent.UpdateGenerativeSettingsRequest, dict]] = None,
            *,
            generative_settings: Optional[gcdc_generative_settings.GenerativeSettings] = None,
            update_mask: Optional[field_mask_pb2.FieldMask] = None,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: Union[float, object] = gapic_v1.method.DEFAULT,
            metadata: Sequence[Tuple[str, Union[str, bytes]]] = (),
        ...
        Args:
            request (Union[google.cloud.dialogflowcx_v3.types.UpdateGenerativeSettingsRequest, dict]):
                The request object. Request for
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [generative_settings, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.update_generative_settings]
