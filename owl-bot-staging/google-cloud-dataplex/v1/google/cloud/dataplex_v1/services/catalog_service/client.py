...
class CatalogServiceClient(metaclass=CatalogServiceClientMeta):
    ...
    @staticmethod
    def aspect_type_path(project: str,location: str,aspect_type: str,) -> str:
        """Returns a fully-qualified aspect_type string."""
        return "projects/{project}/locations/{location}/aspectTypes/{aspect_type}".format(project=project, location=location, aspect_type=aspect_type, )

    @staticmethod
    def parse_aspect_type_path(path: str) -> Dict[str,str]:
        """Parses a aspect_type path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/aspectTypes/(?P<aspect_type>.+?)$", path)
    ...
    @staticmethod
    def entry_link_path(project: str,location: str,entry_group: str,entry_link: str,) -> str:
        """Returns a fully-qualified entry_link string."""
        return "projects/{project}/locations/{location}/entryGroups/{entry_group}/entryLinks/{entry_link}".format(project=project, location=location, entry_group=entry_group, entry_link=entry_link, )

    @staticmethod
    def parse_entry_link_path(path: str) -> Dict[str,str]:
        """Parses a entry_link path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/entryGroups/(?P<entry_group>.+?)/entryLinks/(?P<entry_link>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def entry_type_path(project: str,location: str,entry_type: str,) -> str:
        """Returns a fully-qualified entry_type string."""
        return "projects/{project}/locations/{location}/entryTypes/{entry_type}".format(project=project, location=location, entry_type=entry_type, )

    @staticmethod
    def parse_entry_type_path(path: str) -> Dict[str,str]:
        """Parses a entry_type path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/entryTypes/(?P<entry_type>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    @staticmethod
    def metadata_job_path(project: str,location: str,metadataJob: str,) -> str:
        """Returns a fully-qualified metadata_job string."""
        return "projects/{project}/locations/{location}/metadataJobs/{metadataJob}".format(project=project, location=location, metadataJob=metadataJob, )

    @staticmethod
    def parse_metadata_job_path(path: str) -> Dict[str,str]:
        """Parses a metadata_job path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/metadataJobs/(?P<metadataJob>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = CatalogServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, CatalogServiceTransport, Callable[..., CatalogServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = CatalogServiceClient._read_environment_variables()
        self._client_cert_source = CatalogServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = CatalogServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[CatalogServiceTransport], Callable[..., CatalogServiceTransport]] = (
