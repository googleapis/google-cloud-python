...
class DataTaxonomyServiceAsyncClient:
    ...
    get_transport_class = functools.partial(type(DataTaxonomyServiceClient).get_transport_class, type(DataTaxonomyServiceClient))
    ...
    async def create_data_attribute_binding(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        has_flattened_params = any([parent, data_attribute_binding, data_attribute_binding_id])
        if request is not None and has_flattened_params:
            raise ValueError("If the `request` argument is set, then none of "
                             "the individual field arguments should be set.")
