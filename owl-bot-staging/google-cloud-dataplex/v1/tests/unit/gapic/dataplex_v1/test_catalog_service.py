...
def test_search_entries_rest_required_fields(request_type=catalog.SearchEntriesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).search_entries._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("order_by", "page_size", "page_token", "query", "scope", ))
    jsonified_request.update(unset_fields)
...
def test_search_entries_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("orderBy", "pageSize", "pageToken", "query", "scope", )) & set(("name", "query", )))
