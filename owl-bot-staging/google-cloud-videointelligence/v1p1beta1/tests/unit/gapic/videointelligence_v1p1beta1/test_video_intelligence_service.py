...
@mock.patch.object(VideoIntelligenceServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(VideoIntelligenceServiceAsyncClient))
def test__get_api_endpoint():
    ...
    assert VideoIntelligenceServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == VideoIntelligenceServiceClient.DEFAULT_MTLS_ENDPOINT
    assert VideoIntelligenceServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert VideoIntelligenceServiceClient._get_api_endpoint(None, None, default_universe, "always") == VideoIntelligenceServiceClient.DEFAULT_MTLS_ENDPOINT
    assert VideoIntelligenceServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == VideoIntelligenceServiceClient.DEFAULT_MTLS_ENDPOINT
...
def test_annotate_video_rest_required_fields(request_type=video_intelligence.AnnotateVideoRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).annotate_video._get_unset_required_fields(jsonified_request)
...
def test_annotate_video_rest_unset_required_fields():
    transport = transports.VideoIntelligenceServiceRestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)
...
def test_annotate_video_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.VideoIntelligenceServiceRestInterceptor, "post_annotate_video") as post, \
         mock.patch.object(transports.VideoIntelligenceServiceRestInterceptor, "pre_annotate_video") as pre:
...
def test_annotate_video_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1p1beta1/videos:annotate" % client.transport._host, args[1])
...
def test_video_intelligence_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.videointelligence_v1p1beta1.services.video_intelligence_service.transports.VideoIntelligenceServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_video_intelligence_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.videointelligence_v1p1beta1.services.video_intelligence_service.transports.VideoIntelligenceServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (_AnonymousCredentialsWithUniverseDomain(), None)
        transport = transports.VideoIntelligenceServiceTransport()
        adc.assert_called_once()
