...
from google.ai.generativelanguage_v1beta2.services.text_service import TextServiceAsyncClient
from google.ai.generativelanguage_v1beta2.services.text_service import TextServiceClient
from google.ai.generativelanguage_v1beta2.services.text_service import transports
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert TextServiceClient._read_environment_variables() == (True, "auto", None)
...
@pytest.mark.parametrize("client_class", [
    TextServiceClient, TextServiceAsyncClient
])
...
def test_generate_text(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.generate_text),
            '__call__') as call:
...
def test_embed_text(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.embed_text),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_generate_text_empty_call_grpc():
    client = TextServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = text_service.GenerateTextResponse()
        client.generate_text(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_embed_text_empty_call_grpc():
    client = TextServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = text_service.EmbedTextResponse()
        client.embed_text(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_generate_text_empty_call_grpc_asyncio():
    client = TextServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_embed_text_empty_call_grpc_asyncio():
    client = TextServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
...
def test_generate_text_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.TextServiceRestInterceptor, "post_generate_text") as post, \
        mock.patch.object(transports.TextServiceRestInterceptor, "post_generate_text_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.TextServiceRestInterceptor, "pre_generate_text") as pre:
...
def test_embed_text_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.TextServiceRestInterceptor, "post_embed_text") as post, \
        mock.patch.object(transports.TextServiceRestInterceptor, "post_embed_text_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.TextServiceRestInterceptor, "pre_embed_text") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_generate_text_empty_call_rest():
    client = TextServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_embed_text_empty_call_rest():
    client = TextServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_text_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.TextServiceGrpcTransport, transports.TextServiceGrpcAsyncIOTransport])
def test_text_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.TextServiceGrpcTransport, transports.TextServiceGrpcAsyncIOTransport])
def test_text_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
