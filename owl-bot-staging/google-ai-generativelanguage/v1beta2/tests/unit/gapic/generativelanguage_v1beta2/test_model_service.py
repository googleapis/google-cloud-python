...
from google.ai.generativelanguage_v1beta2.services.model_service import ModelServiceAsyncClient
from google.ai.generativelanguage_v1beta2.services.model_service import ModelServiceClient
from google.ai.generativelanguage_v1beta2.services.model_service import pagers
from google.ai.generativelanguage_v1beta2.services.model_service import transports
from google.ai.generativelanguage_v1beta2.types import model
from google.ai.generativelanguage_v1beta2.types import model_service
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ModelServiceClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ModelServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ModelServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert ModelServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ModelServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ModelServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ModelServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ModelServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ModelServiceClient))
@mock.patch.object(ModelServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ModelServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ModelServiceClient._DEFAULT_UNIVERSE
    default_endpoint = ModelServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ModelServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ModelServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ModelServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ModelServiceClient.DEFAULT_MTLS_ENDPOINT
    assert ModelServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ModelServiceClient._get_api_endpoint(None, None, default_universe, "always") == ModelServiceClient.DEFAULT_MTLS_ENDPOINT
    ...
    assert ModelServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ModelServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ModelServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ModelServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ModelServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ModelServiceClient._get_universe_domain(None, None) == ModelServiceClient._DEFAULT_UNIVERSE
...
def test_model_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ModelServiceClient, transports.ModelServiceGrpcTransport, "grpc", grpc_helpers),
    (ModelServiceAsyncClient, transports.ModelServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (ModelServiceClient, transports.ModelServiceRestTransport, "rest", None),
])
def test_model_service_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_model_service_client_client_options_from_dict():
    with mock.patch('google.ai.generativelanguage_v1beta2.services.model_service.transports.ModelServiceGrpcTransport.__init__') as grpc_transport:
...
def test_model_service_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_get_model(request_type, transport: str = 'grpc'):
    ...
    assert response.base_model_id == 'base_model_id_value'
    assert response.version == 'version_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.input_token_limit == 1838
    assert response.output_token_limit == 1967
    assert response.supported_generation_methods == ['supported_generation_methods_value']
...
async def test_get_model_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.get_model in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_model] = mock_rpc
...
@pytest.mark.asyncio
async def test_list_models_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ModelServiceAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_models in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_models] = mock_rpc
...
def test_list_models_flattened():
    ...
    with mock.patch.object(
        ...
        assert len(call.mock_calls) == 1
        _, args, _ = call.mock_calls[0]
        arg = args[0].page_size
        mock_val = 951
        assert arg == mock_val
        arg = args[0].page_token
        mock_val = 'page_token_value'
        assert arg == mock_val
...
def test_get_model_rest_required_fields(request_type=model_service.GetModelRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_model._get_unset_required_fields(jsonified_request)
...
def test_get_model_rest_unset_required_fields():
    transport = transports.ModelServiceRestTransport(credentials=ga_credentials.AnonymousCredentials)
...
def test_get_model_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        sample_request = {'name': 'models/sample1'}
        ...
        assert path_template.validate("%s/v1beta2/{name=models/*}" % client.transport._host, args[1])
...
def test_list_models_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta2/models" % client.transport._host, args[1])
...
def test_get_model_empty_call_grpc():
    client = ModelServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = model_service.GetModelRequest()
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_models_empty_call_grpc():
    client = ModelServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = model_service.ListModelsRequest()

        assert args[0] == request_msg


def test_transport_kind_grpc_asyncio():
    transport = ModelServiceAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_model_empty_call_grpc_asyncio():
    client = ModelServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_model(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_models_empty_call_grpc_asyncio():
    client = ModelServiceAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.list_models(request=None)
...
def test_get_model_rest_bad_request(request_type=model_service.GetModelRequest):
    ...
    request_init = {'name': 'models/sample1'}
    request = request_type(**request_init)
...
def test_get_model_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ModelServiceRestInterceptor, "post_get_model") as post, \
        mock.patch.object(transports.ModelServiceRestInterceptor, "post_get_model_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ModelServiceRestInterceptor, "pre_get_model") as pre:
        ...
        client.get_model(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_list_models_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ModelServiceRestInterceptor, "post_list_models") as post, \
        mock.patch.object(transports.ModelServiceRestInterceptor, "post_list_models_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ModelServiceRestInterceptor, "pre_list_models") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = model_service.ListModelsRequest.pb(model_service.ListModelsRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = model_service.ListModelsResponse.to_json(model_service.ListModelsResponse())
        req.return_value.content = return_value
        ...
        client.list_models(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_model_service_base_transport():
    ...
    with mock.patch('google.ai.generativelanguage_v1beta2.services.model_service.transports.ModelServiceTransport.__init__') as Transport:
...
def test_model_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ai.generativelanguage_v1beta2.services.model_service.transports.ModelServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_model_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ai.generativelanguage_v1beta2.services.model_service.transports.ModelServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ModelServiceTransport()
        adc.assert_called_once()
...
def test_model_path():
    ...
    actual = ModelServiceClient.model_path(model)
