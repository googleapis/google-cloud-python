...
def test_generative_service_client_client_options_from_dict():
    with mock.patch('google.ai.generativelanguage_v1beta.services.generative_service.transports.GenerativeServiceGrpcTransport.__init__') as grpc_transport:
...
def test_generate_answer_rest_required_fields(request_type=generative_service.GenerateAnswerRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).generate_answer._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_generate_answer_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = generative_service.GenerateAnswerResponse.to_json(generative_service.GenerateAnswerResponse())
...
def test_generate_answer_rest_bad_request(transport: str = 'rest', request_type=generative_service.GenerateAnswerRequest):
    client = GenerativeServiceClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_generative_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ai.generativelanguage_v1beta.services.generative_service.transports.GenerativeServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_generative_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ai.generativelanguage_v1beta.services.generative_service.transports.GenerativeServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (_AnonymousCredentialsWithUniverseDomain(), None)
        transport = transports.GenerativeServiceTransport()
        adc.assert_called_once()
