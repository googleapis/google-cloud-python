...
from google.ai.generativelanguage_v1beta.services.discuss_service import DiscussServiceAsyncClient
from google.ai.generativelanguage_v1beta.services.discuss_service import DiscussServiceClient
from google.ai.generativelanguage_v1beta.services.discuss_service import transports
...
def test_discuss_service_client_client_options_from_dict():
    with mock.patch('google.ai.generativelanguage_v1beta.services.discuss_service.transports.DiscussServiceGrpcTransport.__init__') as grpc_transport:
...
def test_generate_message_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1beta/{model=models/*}:generateMessage" % client.transport._host, args[1])
...
def test_count_message_tokens_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{model=models/*}:countMessageTokens" % client.transport._host, args[1])
...
def test_discuss_service_base_transport():
    ...
    with mock.patch('google.ai.generativelanguage_v1beta.services.discuss_service.transports.DiscussServiceTransport.__init__') as Transport:
...
def test_discuss_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ai.generativelanguage_v1beta.services.discuss_service.transports.DiscussServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_discuss_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ai.generativelanguage_v1beta.services.discuss_service.transports.DiscussServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.DiscussServiceTransport()
        adc.assert_called_once()
