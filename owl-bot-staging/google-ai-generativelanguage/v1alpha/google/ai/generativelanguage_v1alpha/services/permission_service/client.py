...
from .transports.base import PermissionServiceTransport, DEFAULT_CLIENT_INFO
...
class PermissionServiceClient(metaclass=PermissionServiceClientMeta):
    ...
    def permission_path(tuned_model: str,permission: str,) -> str:
        ...
        return "tunedModels/{tuned_model}/permissions/{permission}".format(tuned_model=tuned_model, permission=permission, )

    @staticmethod
    def parse_permission_path(path: str) -> Dict[str,str]:
        """Parses a permission path into its component segments."""
        m = re.match(r"^tunedModels/(?P<tuned_model>.+?)/permissions/(?P<permission>.+?)$", path)
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = PermissionServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, PermissionServiceTransport, Callable[..., PermissionServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = PermissionServiceClient._read_environment_variables()
        self._client_cert_source = PermissionServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = PermissionServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[PermissionServiceTransport], Callable[..., PermissionServiceTransport]] = (
