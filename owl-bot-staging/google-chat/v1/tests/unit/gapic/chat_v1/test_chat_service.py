...
from google.apps.chat_v1.services.chat_service import ChatServiceAsyncClient
from google.apps.chat_v1.services.chat_service import ChatServiceClient
...
from google.apps.chat_v1.types import action_status
...
from google.apps.chat_v1.types import widgets
...
def test__get_default_mtls_endpoint():
    ...
    assert ChatServiceClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert ChatServiceClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert ChatServiceClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert ChatServiceClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ChatServiceClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ChatServiceClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert ChatServiceClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ChatServiceClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ChatServiceClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ChatServiceClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ChatServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ChatServiceClient))
@mock.patch.object(ChatServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ChatServiceAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ChatServiceClient._DEFAULT_UNIVERSE
    default_endpoint = ChatServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ChatServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ChatServiceClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ChatServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ChatServiceClient.DEFAULT_MTLS_ENDPOINT
    assert ChatServiceClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ChatServiceClient._get_api_endpoint(None, None, default_universe, "always") == ChatServiceClient.DEFAULT_MTLS_ENDPOINT
    assert ChatServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ChatServiceClient.DEFAULT_MTLS_ENDPOINT
    assert ChatServiceClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ChatServiceClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ChatServiceClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ChatServiceClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ChatServiceClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ChatServiceClient._get_universe_domain(None, None) == ChatServiceClient._DEFAULT_UNIVERSE
...
def test_chat_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (ChatServiceClient, transports.ChatServiceGrpcTransport, "grpc"),
    (ChatServiceAsyncClient, transports.ChatServiceGrpcAsyncIOTransport, "grpc_asyncio"),
    (ChatServiceClient, transports.ChatServiceRestTransport, "rest"),
])
@mock.patch.object(ChatServiceClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ChatServiceClient))
@mock.patch.object(ChatServiceAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ChatServiceAsyncClient))
def test_chat_service_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    ...
    (ChatServiceAsyncClient, transports.ChatServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (ChatServiceClient, transports.ChatServiceGrpcTransport, "grpc", "false"),
    (ChatServiceAsyncClient, transports.ChatServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_chat_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [
    ChatServiceClient, ChatServiceAsyncClient
])
@mock.patch.object(ChatServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ChatServiceClient))
@mock.patch.object(ChatServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ChatServiceAsyncClient))
...
def test_chat_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    ...
    (ChatServiceAsyncClient, transports.ChatServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (ChatServiceClient, transports.ChatServiceRestTransport, "rest", None),
])
def test_chat_service_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_chat_service_client_client_options_from_dict():
    with mock.patch('google.apps.chat_v1.services.chat_service.transports.ChatServiceGrpcTransport.__init__') as grpc_transport:
...
def test_chat_service_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_create_message(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.create_message),
            '__call__') as call:
...
@pytest.mark.asyncio
async def test_create_message_async(transport: str = 'grpc_asyncio', request_type=gc_message.CreateMessageRequest):
...
@pytest.mark.asyncio
async def test_list_messages_async(transport: str = 'grpc_asyncio', request_type=message.ListMessagesRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_messages_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, message.Message)
                   for i in results)
...
def test_get_message(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.get_message),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_get_message_async(transport: str = 'grpc_asyncio', request_type=message.GetMessageRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_message(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.update_message),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = gc_message.Message(
            name='name_value',
...
@pytest.mark.asyncio
async def test_update_message_async(transport: str = 'grpc_asyncio', request_type=gc_message.UpdateMessageRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_message(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.delete_message),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_delete_message_async(transport: str = 'grpc_asyncio', request_type=message.DeleteMessageRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_get_attachment_async(transport: str = 'grpc_asyncio', request_type=attachment.GetAttachmentRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_upload_attachment(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.upload_attachment),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_upload_attachment_async(transport: str = 'grpc_asyncio', request_type=attachment.UploadAttachmentRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_spaces(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.list_spaces),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_list_spaces_async(transport: str = 'grpc_asyncio', request_type=space.ListSpacesRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(space.ListSpacesResponse(
            next_page_token='next_page_token_value',
        ))
...
def test_list_spaces_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, space.Space)
                   for i in results)
...
async def test_list_spaces_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_spaces(request={},)
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager: # pragma: no branch
            responses.append(response)
...
def test_get_space(request_type, transport: str = 'grpc'):
    ...
    assert response.space_threading_state == space.Space.SpaceThreadingState.THREADED_MESSAGES
...
@pytest.mark.asyncio
async def test_get_space_async(transport: str = 'grpc_asyncio', request_type=space.GetSpaceRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_space(request_type, transport: str = 'grpc'):
    ...
    assert response.display_name == 'display_name_value'
    assert response.external_user_allowed is True
    assert response.space_threading_state == gc_space.Space.SpaceThreadingState.THREADED_MESSAGES
    assert response.space_history_state == history_state.HistoryState.HISTORY_OFF
    assert response.import_mode is True
    assert response.admin_installed is True
...
@pytest.mark.asyncio
async def test_create_space_async(transport: str = 'grpc_asyncio', request_type=gc_space.CreateSpaceRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_set_up_space(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.set_up_space),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = space.Space(
            name='name_value',
...
@pytest.mark.asyncio
async def test_set_up_space_async(transport: str = 'grpc_asyncio', request_type=space_setup.SetUpSpaceRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_update_space_async(transport: str = 'grpc_asyncio', request_type=gc_space.UpdateSpaceRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_space(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.delete_space),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = None
...
@pytest.mark.asyncio
async def test_delete_space_async(transport: str = 'grpc_asyncio', request_type=space.DeleteSpaceRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_complete_import_space(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.complete_import_space),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_complete_import_space_async(transport: str = 'grpc_asyncio', request_type=space.CompleteImportSpaceRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(space.CompleteImportSpaceResponse(
        ))
...
def test_find_direct_message(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.find_direct_message),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = space.Space(
            name='name_value',
...
@pytest.mark.asyncio
async def test_find_direct_message_async(transport: str = 'grpc_asyncio', request_type=space.FindDirectMessageRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_create_membership_async(transport: str = 'grpc_asyncio', request_type=gc_membership.CreateMembershipRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(gc_membership.Membership(
            name='name_value',
            state=gc_membership.Membership.MembershipState.JOINED,
            role=gc_membership.Membership.MembershipRole.ROLE_MEMBER,
        ))
...
def test_create_reaction(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.create_reaction),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_create_reaction_async(transport: str = 'grpc_asyncio', request_type=gc_reaction.CreateReactionRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(gc_reaction.Reaction(
            name='name_value',
        ))
...
def test_list_reactions(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.list_reactions),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_list_reactions_async(transport: str = 'grpc_asyncio', request_type=reaction.ListReactionsRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_reactions_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, reaction.Reaction)
                   for i in results)
...
async def test_list_reactions_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_reactions(request={},)
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager: # pragma: no branch
            responses.append(response)
...
def test_delete_reaction(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.delete_reaction),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = None
...
@pytest.mark.asyncio
async def test_delete_reaction_async(transport: str = 'grpc_asyncio', request_type=reaction.DeleteReactionRequest):
    client = ChatServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_message_rest_required_fields(request_type=gc_message.CreateMessageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_message._get_unset_required_fields(jsonified_request)
    ...
    assert not set(unset_fields) - set(("message_id", "message_reply_option", "request_id", "thread_key", ))
    jsonified_request.update(unset_fields)
...
def test_create_message_rest_unset_required_fields():
    transport = transports.ChatServiceRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.create_message._get_unset_required_fields({})
    assert set(unset_fields) == (set(("messageId", "messageReplyOption", "requestId", "threadKey", )) & set(("parent", "message", )))
...
def test_create_message_rest_interceptors(null_interceptor):
    transport = transports.ChatServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ChatServiceRestInterceptor(),
        )
    client = ChatServiceClient(transport=transport)
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_create_message") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_create_message") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = gc_message.CreateMessageRequest.pb(gc_message.CreateMessageRequest())
        ...
        client.create_message(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_create_message_rest_bad_request(transport: str = 'rest', request_type=gc_message.CreateMessageRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_message_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/{parent=spaces/*}/messages" % client.transport._host, args[1])
...
def test_list_messages_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_list_messages") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_list_messages") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = message.ListMessagesResponse.to_json(message.ListMessagesResponse())
...
def test_list_messages_rest_bad_request(transport: str = 'rest', request_type=message.ListMessagesRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_memberships_rest_required_fields(request_type=membership.ListMembershipsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_memberships._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("filter", "page_size", "page_token", "show_groups", "show_invited", ))
    jsonified_request.update(unset_fields)
...
def test_list_memberships_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("filter", "pageSize", "pageToken", "showGroups", "showInvited", )) & set(("parent", )))
...
def test_list_memberships_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_list_memberships") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_list_memberships") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_list_memberships_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=spaces/*}/members" % client.transport._host, args[1])
...
def test_get_membership_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_get_membership") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_get_membership") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_get_membership_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=spaces/*/members/*}" % client.transport._host, args[1])
...
def test_get_message_rest_required_fields(request_type=message.GetMessageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_message._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_get_message_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_get_message") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_get_message") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        client.get_message(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_get_message_rest_bad_request(transport: str = 'rest', request_type=message.GetMessageRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_message_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=spaces/*/messages/*}" % client.transport._host, args[1])
...
def test_update_message_rest(request_type):
    ...
    request_init["message"] = {'name': 'spaces/sample1/messages/sample2', 'sender': {'name': 'name_value', 'display_name': 'display_name_value', 'domain_id': 'domain_id_value', 'type_': 1, 'is_anonymous': True}, 'create_time': {'seconds': 751, 'nanos': 543}, 'last_update_time': {}, 'delete_time': {}, 'text': 'text_value', 'formatted_text': 'formatted_text_value', 'cards': [{'header': {'title': 'title_value', 'subtitle': 'subtitle_value', 'image_style': 1, 'image_url': 'image_url_value'}, 'sections': [{'header': 'header_value', 'widgets': [{'text_paragraph': {'text': 'text_value'}, 'image': {'image_url': 'image_url_value', 'on_click': {'action': {'action_method_name': 'action_method_name_value', 'parameters': [{'key': 'key_value', 'value': 'value_value'}]}, 'open_link': {'url': 'url_value'}}, 'aspect_ratio': 0.1278}, 'key_value': {'icon': 1, 'icon_url': 'icon_url_value', 'top_label': 'top_label_value', 'content': 'content_value', 'content_multiline': True, 'bottom_label': 'bottom_label_value', 'on_click': {}, 'button': {'text_button': {'text': 'text_value', 'on_click': {}}, 'image_button': {'icon': 1, 'icon_url': 'icon_url_value', 'on_click': {}, 'name': 'name_value'}}}, 'buttons': {}}]}], 'card_actions': [{'action_label': 'action_label_value', 'on_click': {}}], 'name': 'name_value'}], 'cards_v2': [{'card_id': 'card_id_value', 'card': {'header': {'title': 'title_value', 'subtitle': 'subtitle_value', 'image_type': 1, 'image_url': 'image_url_value', 'image_alt_text': 'image_alt_text_value'}, 'sections': [{'header': 'header_value', 'widgets': [{'text_paragraph': {'text': 'text_value'}, 'image': {'image_url': 'image_url_value', 'on_click': {'action': {'function': 'function_value', 'parameters': [{'key': 'key_value', 'value': 'value_value'}], 'load_indicator': 1, 'persist_values': True, 'interaction': 1}, 'open_link': {'url': 'url_value', 'open_as': 1, 'on_close': 1}, 'open_dynamic_link_action': {}, 'card': {}}, 'alt_text': 'alt_text_value'}, 'decorated_text': {'icon': {'known_icon': 'known_icon_value', 'icon_url': 'icon_url_value', 'alt_text': 'alt_text_value', 'image_type': 1}, 'start_icon': {}, 'top_label': 'top_label_value', 'text': 'text_value', 'wrap_text': True, 'bottom_label': 'bottom_label_value', 'on_click': {}, 'button': {'text': 'text_value', 'icon': {}, 'color': {'red': 0.315, 'green': 0.529, 'blue': 0.424, 'alpha': {'value': 0.541}}, 'on_click': {}, 'disabled': True, 'alt_text': 'alt_text_value'}, 'switch_control': {'name': 'name_value', 'value': 'value_value', 'selected': True, 'on_change_action': {}, 'control_type': 1}, 'end_icon': {}}, 'button_list': {'buttons': {}}, 'text_input': {'name': 'name_value', 'label': 'label_value', 'hint_text': 'hint_text_value', 'value': 'value_value', 'type_': 1, 'on_change_action': {}, 'initial_suggestions': {'items': [{'text': 'text_value'}]}, 'auto_complete_action': {}, 'placeholder_text': 'placeholder_text_value'}, 'selection_input': {'name': 'name_value', 'label': 'label_value', 'type_': 1, 'items': [{'text': 'text_value', 'value': 'value_value', 'selected': True, 'start_icon_uri': 'start_icon_uri_value', 'bottom_text': 'bottom_text_value'}], 'on_change_action': {}, 'multi_select_max_selected_items': 3288, 'multi_select_min_query_length': 3107, 'external_data_source': {}, 'platform_data_source': {'common_data_source': 1}}, 'date_time_picker': {'name': 'name_value', 'label': 'label_value', 'type_': 1, 'value_ms_epoch': 1482, 'timezone_offset_date': 2126, 'on_change_action': {}}, 'divider': {}, 'grid': {'title': 'title_value', 'items': [{'id': 'id_value', 'image': {'image_uri': 'image_uri_value', 'alt_text': 'alt_text_value', 'crop_style': {'type_': 1, 'aspect_ratio': 0.1278}, 'border_style': {'type_': 1, 'stroke_color': {}, 'corner_radius': 1392}}, 'title': 'title_value', 'subtitle': 'subtitle_value', 'layout': 1}], 'border_style': {}, 'column_count': 1302, 'on_click': {}}, 'columns': {'column_items': [{'horizontal_size_style': 1, 'horizontal_alignment': 1, 'vertical_alignment': 1, 'widgets': [{'text_paragraph': {}, 'image': {}, 'decorated_text': {}, 'button_list': {}, 'text_input': {}, 'selection_input': {}, 'date_time_picker': {}}]}]}, 'horizontal_alignment': 1}], 'collapsible': True, 'uncollapsible_widgets_count': 2891}], 'section_divider_style': 1, 'card_actions': [{'action_label': 'action_label_value', 'on_click': {}}], 'name': 'name_value', 'fixed_footer': {'primary_button': {}, 'secondary_button': {}}, 'display_style': 1, 'peek_card_header': {}}}], 'annotations': [{'type_': 1, 'start_index': 1189, 'length': 642, 'user_mention': {'user': {}, 'type_': 1}, 'slash_command': {'bot': {}, 'type_': 1, 'command_name': 'command_name_value', 'command_id': 1035, 'triggers_dialog': True}}], 'thread': {'name': 'name_value', 'thread_key': 'thread_key_value'}, 'space': {'name': 'name_value', 'type_': 1, 'space_type': 1, 'single_user_bot_dm': True, 'threaded': True, 'display_name': 'display_name_value', 'external_user_allowed': True, 'space_threading_state': 2, 'space_details': {'description': 'description_value', 'guidelines': 'guidelines_value'}, 'space_history_state': 1, 'import_mode': True, 'create_time': {}, 'admin_installed': True}, 'fallback_text': 'fallback_text_value', 'action_response': {'type_': 1, 'url': 'url_value', 'dialog_action': {'dialog': {'body': {}}, 'action_status': {'status_code': 1, 'user_facing_message': 'user_facing_message_value'}}, 'updated_widget': {'suggestions': {'items': {}}, 'widget': 'widget_value'}}, 'argument_text': 'argument_text_value', 'slash_command': {'command_id': 1035}, 'attachment': [{'name': 'name_value', 'content_name': 'content_name_value', 'content_type': 'content_type_value', 'attachment_data_ref': {'resource_name': 'resource_name_value', 'attachment_upload_token': 'attachment_upload_token_value'}, 'drive_data_ref': {'drive_file_id': 'drive_file_id_value'}, 'thumbnail_uri': 'thumbnail_uri_value', 'download_uri': 'download_uri_value', 'source': 1}], 'matched_url': {'url': 'url_value'}, 'thread_reply': True, 'client_assigned_message_id': 'client_assigned_message_id_value', 'emoji_reaction_summaries': [{'emoji': {'unicode': 'unicode_value', 'custom_emoji': {'uid': 'uid_value'}}, 'reaction_count': 1501}], 'private_message_viewer': {}, 'deletion_metadata': {'deletion_type': 1}, 'quoted_message_metadata': {'name': 'name_value', 'last_update_time': {}}, 'attached_gifs': [{'uri': 'uri_value'}]}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_update_message_rest_required_fields(request_type=gc_message.UpdateMessageRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_message._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_update_message_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("allowMissing", "updateMask", )) & set(("message", )))
...
def test_update_message_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_update_message") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_update_message") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = gc_message.UpdateMessageRequest.pb(gc_message.UpdateMessageRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.update_message(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_update_message_rest_bad_request(transport: str = 'rest', request_type=gc_message.UpdateMessageRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_message_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{message.name=spaces/*/messages/*}" % client.transport._host, args[1])
...
def test_delete_message_rest_required_fields(request_type=message.DeleteMessageRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_message._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_delete_message_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_delete_message") as pre:
        pre.assert_not_called()
        ...
        client.delete_message(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()


def test_delete_message_rest_bad_request(transport: str = 'rest', request_type=message.DeleteMessageRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_attachment_rest_required_fields(request_type=attachment.GetAttachmentRequest):
    transport_class = transports.ChatServiceRestTransport
...
def test_get_attachment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_get_attachment") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_get_attachment") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = attachment.GetAttachmentRequest.pb(attachment.GetAttachmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = attachment.Attachment.to_json(attachment.Attachment())
...
def test_get_attachment_rest_bad_request(transport: str = 'rest', request_type=attachment.GetAttachmentRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_attachment_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=spaces/*/messages/*/attachments/*}" % client.transport._host, args[1])
...
def test_upload_attachment_rest_required_fields(request_type=attachment.UploadAttachmentRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).upload_attachment._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_upload_attachment_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("parent", "filename", )))
...
def test_upload_attachment_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_upload_attachment") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_upload_attachment") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = attachment.UploadAttachmentRequest.pb(attachment.UploadAttachmentRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = attachment.UploadAttachmentResponse.to_json(attachment.UploadAttachmentResponse())
        ...
        client.upload_attachment(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_upload_attachment_rest_bad_request(transport: str = 'rest', request_type=attachment.UploadAttachmentRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_spaces_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_list_spaces") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_list_spaces") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = space.ListSpacesResponse.to_json(space.ListSpacesResponse())
        ...
        client.list_spaces(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_list_spaces_rest_bad_request(transport: str = 'rest', request_type=space.ListSpacesRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_space_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_get_space") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_get_space") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_get_space_rest_bad_request(transport: str = 'rest', request_type=space.GetSpaceRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_space_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=spaces/*}" % client.transport._host, args[1])
...
def test_create_space_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {}
    request_init["space"] = {'name': 'name_value', 'type_': 1, 'space_type': 1, 'single_user_bot_dm': True, 'threaded': True, 'display_name': 'display_name_value', 'external_user_allowed': True, 'space_threading_state': 2, 'space_details': {'description': 'description_value', 'guidelines': 'guidelines_value'}, 'space_history_state': 1, 'import_mode': True, 'create_time': {'seconds': 751, 'nanos': 543}, 'admin_installed': True}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_create_space_rest_required_fields(request_type=gc_space.CreateSpaceRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_space._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_create_space_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_create_space") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_create_space") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_create_space_rest_bad_request(transport: str = 'rest', request_type=gc_space.CreateSpaceRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_set_up_space_rest_required_fields(request_type=space_setup.SetUpSpaceRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).set_up_space._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_set_up_space_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_set_up_space") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_set_up_space") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        client.set_up_space(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_set_up_space_rest_bad_request(transport: str = 'rest', request_type=space_setup.SetUpSpaceRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_space_rest(request_type):
    ...
    request_init["space"] = {'name': 'spaces/sample1', 'type_': 1, 'space_type': 1, 'single_user_bot_dm': True, 'threaded': True, 'display_name': 'display_name_value', 'external_user_allowed': True, 'space_threading_state': 2, 'space_details': {'description': 'description_value', 'guidelines': 'guidelines_value'}, 'space_history_state': 1, 'import_mode': True, 'create_time': {'seconds': 751, 'nanos': 543}, 'admin_installed': True}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_update_space_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_update_space") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_update_space") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_update_space_rest_bad_request(transport: str = 'rest', request_type=gc_space.UpdateSpaceRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_space_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{space.name=spaces/*}" % client.transport._host, args[1])
...
def test_delete_space_rest_required_fields(request_type=space.DeleteSpaceRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_space._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_delete_space_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_delete_space") as pre:
        pre.assert_not_called()
        ...
        client.delete_space(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()


def test_delete_space_rest_bad_request(transport: str = 'rest', request_type=space.DeleteSpaceRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_complete_import_space_rest_required_fields(request_type=space.CompleteImportSpaceRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).complete_import_space._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_complete_import_space_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_complete_import_space") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_complete_import_space") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = space.CompleteImportSpaceRequest.pb(space.CompleteImportSpaceRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = space.CompleteImportSpaceResponse.to_json(space.CompleteImportSpaceResponse())
        ...
        client.complete_import_space(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_complete_import_space_rest_bad_request(transport: str = 'rest', request_type=space.CompleteImportSpaceRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_find_direct_message_rest_required_fields(request_type=space.FindDirectMessageRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).find_direct_message._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_find_direct_message_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_find_direct_message") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_find_direct_message") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        client.find_direct_message(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_find_direct_message_rest_bad_request(transport: str = 'rest', request_type=space.FindDirectMessageRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_membership_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'spaces/sample1'}
    request_init["membership"] = {'name': 'name_value', 'state': 1, 'role': 1, 'member': {'name': 'name_value', 'display_name': 'display_name_value', 'domain_id': 'domain_id_value', 'type_': 1, 'is_anonymous': True}, 'group_member': {'name': 'name_value'}, 'create_time': {'seconds': 751, 'nanos': 543}, 'delete_time': {}}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_create_membership_rest_required_fields(request_type=gc_membership.CreateMembershipRequest):
    transport_class = transports.ChatServiceRestTransport
...
def test_create_membership_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("parent", "membership", )))
...
def test_create_membership_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_create_membership") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_create_membership") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = gc_membership.CreateMembershipRequest.pb(gc_membership.CreateMembershipRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = gc_membership.Membership.to_json(gc_membership.Membership())
...
def test_create_membership_rest_bad_request(transport: str = 'rest', request_type=gc_membership.CreateMembershipRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_membership_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_delete_membership") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_delete_membership") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_create_reaction_rest(request_type):
    ...
    request_init["reaction"] = {'name': 'name_value', 'user': {'name': 'name_value', 'display_name': 'display_name_value', 'domain_id': 'domain_id_value', 'type_': 1, 'is_anonymous': True}, 'emoji': {'unicode': 'unicode_value', 'custom_emoji': {'uid': 'uid_value'}}}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_create_reaction_rest_required_fields(request_type=gc_reaction.CreateReactionRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_reaction._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_create_reaction_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("parent", "reaction", )))
...
def test_create_reaction_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_create_reaction") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_create_reaction") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = gc_reaction.CreateReactionRequest.pb(gc_reaction.CreateReactionRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        client.create_reaction(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_create_reaction_rest_bad_request(transport: str = 'rest', request_type=gc_reaction.CreateReactionRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_reaction_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=spaces/*/messages/*}/reactions" % client.transport._host, args[1])
...
def test_list_reactions_rest_required_fields(request_type=reaction.ListReactionsRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_reactions._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_list_reactions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "post_list_reactions") as post, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_list_reactions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = reaction.ListReactionsResponse.to_json(reaction.ListReactionsResponse())
        ...
        client.list_reactions(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_list_reactions_rest_bad_request(transport: str = 'rest', request_type=reaction.ListReactionsRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_reaction_rest_required_fields(request_type=reaction.DeleteReactionRequest):
    transport_class = transports.ChatServiceRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_reaction._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_delete_reaction_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.ChatServiceRestInterceptor, "pre_delete_reaction") as pre:
        pre.assert_not_called()
        ...
        client.delete_reaction(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()


def test_delete_reaction_rest_bad_request(transport: str = 'rest', request_type=reaction.DeleteReactionRequest):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_reaction_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{name=spaces/*/messages/*/reactions/*}" % client.transport._host, args[1])
...
def test_chat_service_base_transport():
    ...
    with mock.patch('google.apps.chat_v1.services.chat_service.transports.ChatServiceTransport.__init__') as Transport:
...
def test_chat_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.apps.chat_v1.services.chat_service.transports.ChatServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_chat_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.apps.chat_v1.services.chat_service.transports.ChatServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ChatServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.ChatServiceGrpcTransport, transports.ChatServiceGrpcAsyncIOTransport])
def test_chat_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_chat_service_host_with_port(transport_name):
    client = ChatServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='chat.googleapis.com:8000'),
        transport=transport_name,
    )
    assert client.transport._host == (
...
@pytest.mark.parametrize("transport_class", [transports.ChatServiceGrpcTransport, transports.ChatServiceGrpcAsyncIOTransport])
def test_chat_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ChatServiceGrpcTransport, transports.ChatServiceGrpcAsyncIOTransport])
def test_chat_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_attachment_path():
    ...
    message = "clam"
    attachment = "whelk"
    expected = "spaces/{space}/messages/{message}/attachments/{attachment}".format(space=space, message=message, attachment=attachment, )
...
def test_message_path():
    space = "scallop"
    message = "abalone"
    expected = "spaces/{space}/messages/{message}".format(space=space, message=message, )
...
def test_quoted_message_metadata_path():
    ...
    expected = "spaces/{space}/messages/{message}/quotedMessageMetadata/{quoted_message_metadata}".format(space=space, message=message, quoted_message_metadata=quoted_message_metadata, )
    actual = ChatServiceClient.quoted_message_metadata_path(space, message, quoted_message_metadata)
    assert expected == actual
...
def test_reaction_path():
    ...
    message = "nautilus"
    reaction = "scallop"
    expected = "spaces/{space}/messages/{message}/reactions/{reaction}".format(space=space, message=message, reaction=reaction, )
...
def test_thread_path():
    space = "oyster"
    thread = "nudibranch"
    expected = "spaces/{space}/threads/{thread}".format(space=space, thread=thread, )
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.ChatServiceTransport, '_prep_wrapped_messages') as prep:
        client = ChatServiceClient(
            credentials=ga_credentials.AnonymousCredentials(),
