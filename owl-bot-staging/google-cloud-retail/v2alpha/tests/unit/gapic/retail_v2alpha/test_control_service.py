...
from google.cloud.retail_v2alpha.services.control_service import ControlServiceAsyncClient
from google.cloud.retail_v2alpha.services.control_service import ControlServiceClient
from google.cloud.retail_v2alpha.services.control_service import pagers
from google.cloud.retail_v2alpha.services.control_service import transports
...
def test_control_service_client_client_options_from_dict():
    with mock.patch('google.cloud.retail_v2alpha.services.control_service.transports.ControlServiceGrpcTransport.__init__') as grpc_transport:
...
def test_create_control_flattened():
    ...
    with mock.patch.object(
        ...
        client.create_control(
            parent='parent_value',
            control=gcr_control.Control(facet_spec=search_service.SearchRequest.FacetSpec(facet_key=search_service.SearchRequest.FacetSpec.FacetKey(key='key_value'))),
            control_id='control_id_value',
        )
        ...
        assert arg == mock_val
        arg = args[0].control
        mock_val = gcr_control.Control(facet_spec=search_service.SearchRequest.FacetSpec(facet_key=search_service.SearchRequest.FacetSpec.FacetKey(key='key_value')))
        assert arg == mock_val
...
def test_create_control_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v2alpha/{parent=projects/*/locations/*/catalogs/*}/controls" % client.transport._host, args[1])
...
def test_delete_control_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2alpha/{name=projects/*/locations/*/catalogs/*/controls/*}" % client.transport._host, args[1])
...
def test_update_control_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v2alpha/{control.name=projects/*/locations/*/catalogs/*/controls/*}" % client.transport._host, args[1])
...
def test_create_control_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = gcr_control.Control.to_json(gcr_control.Control())
        ...
        post_with_metadata.return_value = gcr_control.Control(), metadata
...
def test_update_control_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'control': {'name': 'projects/sample1/locations/sample2/catalogs/sample3/controls/sample4'}}
    request_init["control"] = {'facet_spec': {'facet_key': {'key': 'key_value', 'intervals': [{'minimum': 0.764, 'exclusive_minimum': 0.18430000000000002, 'maximum': 0.766, 'exclusive_maximum': 0.1845}], 'restricted_values': ['restricted_values_value1', 'restricted_values_value2'], 'prefixes': ['prefixes_value1', 'prefixes_value2'], 'contains': ['contains_value1', 'contains_value2'], 'case_insensitive': True, 'order_by': 'order_by_value', 'query': 'query_value', 'return_min_max': True}, 'limit': 543, 'excluded_filter_keys': ['excluded_filter_keys_value1', 'excluded_filter_keys_value2'], 'enable_dynamic_position': True}, 'rule': {'boost_action': {'boost': 0.551, 'products_filter': 'products_filter_value'}, 'redirect_action': {'redirect_uri': 'redirect_uri_value'}, 'oneway_synonyms_action': {'query_terms': ['query_terms_value1', 'query_terms_value2'], 'synonyms': ['synonyms_value1', 'synonyms_value2'], 'oneway_terms': ['oneway_terms_value1', 'oneway_terms_value2']}, 'do_not_associate_action': {'query_terms': ['query_terms_value1', 'query_terms_value2'], 'do_not_associate_terms': ['do_not_associate_terms_value1', 'do_not_associate_terms_value2'], 'terms': ['terms_value1', 'terms_value2']}, 'replacement_action': {'query_terms': ['query_terms_value1', 'query_terms_value2'], 'replacement_term': 'replacement_term_value', 'term': 'term_value'}, 'ignore_action': {'ignore_terms': ['ignore_terms_value1', 'ignore_terms_value2']}, 'filter_action': {'filter': 'filter_value'}, 'twoway_synonyms_action': {'synonyms': ['synonyms_value1', 'synonyms_value2']}, 'force_return_facet_action': {'facet_position_adjustments': [{'attribute_name': 'attribute_name_value', 'position': 885}]}, 'remove_facet_action': {'attribute_names': ['attribute_names_value1', 'attribute_names_value2']}, 'pin_action': {'pin_map': {}}, 'condition': {'query_terms': [{'value': 'value_value', 'full_match': True}], 'active_time_range': [{'start_time': {'seconds': 751, 'nanos': 543}, 'end_time': {}}], 'page_categories': ['page_categories_value1', 'page_categories_value2']}}, 'name': 'projects/sample1/locations/sample2/catalogs/sample3/controls/sample4', 'display_name': 'display_name_value', 'associated_serving_config_ids': ['associated_serving_config_ids_value1', 'associated_serving_config_ids_value2'], 'solution_types': [1], 'search_solution_use_case': [1]}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_control_service_base_transport():
    ...
    with mock.patch('google.cloud.retail_v2alpha.services.control_service.transports.ControlServiceTransport.__init__') as Transport:
...
def test_control_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.retail_v2alpha.services.control_service.transports.ControlServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_control_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.retail_v2alpha.services.control_service.transports.ControlServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ControlServiceTransport()
        adc.assert_called_once()
