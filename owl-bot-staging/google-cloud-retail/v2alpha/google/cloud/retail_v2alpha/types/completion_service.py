...
class CompleteQueryRequest(proto.Message):
    ...
    Attributes:
        ...
        dataset (str):
            ...
            Retail based on user events. If left empty, completions will
            be fetched from the ``user-data`` dataset.
        ...
        enable_attribute_suggestions (bool):
            If true, attribute suggestions are enabled and provided in
            the response.
        ...
        entity (str):
            ...
            to get per-entity autocomplete results. This field will be
            applied to ``completion_results`` only. It has no effect on
            the ``attribute_results``. Also, this entity should be
            limited to 256 characters, if too long, it will be truncated
            to 256 characters in both generation and serving time, and
            may lead to mis-match. To ensure it works, please set the
