...
class ControlServiceClient(metaclass=ControlServiceClientMeta):
    ...
    @staticmethod
    def control_path(project: str,location: str,catalog: str,control: str,) -> str:
        """Returns a fully-qualified control string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/controls/{control}".format(project=project, location=location, catalog=catalog, control=control, )

    @staticmethod
    def parse_control_path(path: str) -> Dict[str,str]:
        """Parses a control path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/controls/(?P<control>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = ControlServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            ControlServiceClient._compare_universes(self.universe_domain, self.transport._credentials))
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, ControlServiceTransport, Callable[..., ControlServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = ControlServiceClient._read_environment_variables()
        self._client_cert_source = ControlServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = ControlServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[ControlServiceTransport], Callable[..., ControlServiceTransport]] = (
