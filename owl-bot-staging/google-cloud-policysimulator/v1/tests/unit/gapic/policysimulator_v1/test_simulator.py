...
from google.cloud.policysimulator_v1.services.simulator import SimulatorAsyncClient
...
from google.cloud.policysimulator_v1.services.simulator import transports
...
def test__read_environment_variables():
    assert SimulatorClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert SimulatorClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            SimulatorClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert SimulatorClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert SimulatorClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert SimulatorClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert SimulatorClient._get_client_cert_source(None, False) is None
    assert SimulatorClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert SimulatorClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert SimulatorClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert SimulatorClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(SimulatorClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorClient))
@mock.patch.object(SimulatorAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = SimulatorClient._DEFAULT_UNIVERSE
    default_endpoint = SimulatorClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = SimulatorClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert SimulatorClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert SimulatorClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == SimulatorClient.DEFAULT_MTLS_ENDPOINT
    assert SimulatorClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert SimulatorClient._get_api_endpoint(None, None, default_universe, "always") == SimulatorClient.DEFAULT_MTLS_ENDPOINT
    assert SimulatorClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == SimulatorClient.DEFAULT_MTLS_ENDPOINT
    assert SimulatorClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert SimulatorClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        SimulatorClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert SimulatorClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert SimulatorClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert SimulatorClient._get_universe_domain(None, None) == SimulatorClient._DEFAULT_UNIVERSE
...
@pytest.mark.parametrize("client_class,transport_name", [
    (SimulatorClient, "grpc"),
    (SimulatorAsyncClient, "grpc_asyncio"),
    (SimulatorClient, "rest"),
])
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.SimulatorGrpcTransport, "grpc"),
    (transports.SimulatorGrpcAsyncIOTransport, "grpc_asyncio"),
    (transports.SimulatorRestTransport, "rest"),
])
...
def test_simulator_client_get_transport_class():
    ...
    available_transports = [
        transports.SimulatorGrpcTransport,
        transports.SimulatorRestTransport,
    ]
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (SimulatorClient, transports.SimulatorGrpcTransport, "grpc"),
    (SimulatorAsyncClient, transports.SimulatorGrpcAsyncIOTransport, "grpc_asyncio"),
    (SimulatorClient, transports.SimulatorRestTransport, "rest"),
])
@mock.patch.object(SimulatorClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorClient))
@mock.patch.object(SimulatorAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (SimulatorClient, transports.SimulatorGrpcTransport, "grpc", "true"),
    (SimulatorAsyncClient, transports.SimulatorGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (SimulatorClient, transports.SimulatorGrpcTransport, "grpc", "false"),
    (SimulatorAsyncClient, transports.SimulatorGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (SimulatorClient, transports.SimulatorRestTransport, "rest", "true"),
    (SimulatorClient, transports.SimulatorRestTransport, "rest", "false"),
])
@mock.patch.object(SimulatorClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorClient))
@mock.patch.object(SimulatorAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorAsyncClient))
...
@pytest.mark.parametrize("client_class", [
    SimulatorClient, SimulatorAsyncClient
])
...
@mock.patch.object(SimulatorClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorClient))
@mock.patch.object(SimulatorAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(SimulatorAsyncClient))
def test_simulator_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (SimulatorClient, transports.SimulatorGrpcTransport, "grpc", grpc_helpers),
    (SimulatorAsyncClient, transports.SimulatorGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (SimulatorClient, transports.SimulatorRestTransport, "rest", None),
])
...
def test_get_replay(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.get_replay),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = simulator.Replay(
            name='name_value',
            state=simulator.Replay.State.PENDING,
        )
...
def test_get_replay_non_empty_request_with_auto_populated_field():
...
def test_get_replay_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.get_replay in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_replay] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_replay_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.get_replay in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_replay] = mock_rpc
...
async def test_get_replay_flattened_async():
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.get_replay(
            name='name_value',
        )
...
def test_create_replay(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.create_replay),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_create_replay_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_create_replay_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.create_replay in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_replay] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_replay_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = SimulatorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.create_replay in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_replay] = mock_rpc
...
def test_create_replay_flattened():
    ...
    with mock.patch.object(
        ...
        client.create_replay(
            parent='parent_value',
            replay=simulator.Replay(name='name_value'),
        )
...
async def test_create_replay_flattened_async():
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.create_replay(
            parent='parent_value',
            replay=simulator.Replay(name='name_value'),
        )
...
def test_list_replay_results(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.list_replay_results),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_list_replay_results_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_list_replay_results_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.list_replay_results in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.list_replay_results] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_list_replay_results_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = SimulatorAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.list_replay_results in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.list_replay_results] = mock_rpc
...
async def test_list_replay_results_flattened_async():
    ...
    with mock.patch.object(
        ...
        # Call the method with a truthy value for each flattened field,
        # using the keyword arguments to the method.
        response = await client.list_replay_results(
            parent='parent_value',
        )
...
async def test_list_replay_results_flattened_error_async():
    ...
    # fields is an error.
    with pytest.raises(ValueError):
        await client.list_replay_results(
            simulator.ListReplayResultsRequest(),
            parent='parent_value',
        )
...
def test_list_replay_results_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        pager = client.list_replay_results(request={}, retry=retry, timeout=timeout)
        ...
        assert all(isinstance(i, simulator.ReplayResult)
                   for i in results)
...
def test_get_replay_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = SimulatorClient(
...
def test_create_replay_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = SimulatorClient(
...
def test_list_replay_results_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = SimulatorClient(
...
def test_credentials_transport_error():
    ...
    transport = transports.SimulatorGrpcTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    with pytest.raises(ValueError):
        client = SimulatorClient(
...
def test_transport_get_channel():
    ...
    transport = transports.SimulatorGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
def test_get_replay_empty_call_grpc():
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = simulator.Replay()
        client.get_replay(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_replay_empty_call_grpc():
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_replay(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_replay_results_empty_call_grpc():
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = simulator.ListReplayResultsResponse()
        client.list_replay_results(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = SimulatorAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_replay_empty_call_grpc_asyncio():
    client = SimulatorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_replay(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_replay_empty_call_grpc_asyncio():
    client = SimulatorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.create_replay(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_list_replay_results_empty_call_grpc_asyncio():
    client = SimulatorAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.list_replay_results(request=None)
...
def test_transport_kind_rest():
    transport = SimulatorClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
    assert transport.kind == "rest"


def test_get_replay_rest_bad_request(request_type=simulator.GetReplayRequest):
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_replay_rest_call_success(request_type):
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_replay_rest_interceptors(null_interceptor):
    transport = transports.SimulatorRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.SimulatorRestInterceptor(),
        )
    client = SimulatorClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.SimulatorRestInterceptor, "post_get_replay") as post, \
        mock.patch.object(transports.SimulatorRestInterceptor, "post_get_replay_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SimulatorRestInterceptor, "pre_get_replay") as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = simulator.Replay.to_json(simulator.Replay())
...
def test_create_replay_rest_call_success(request_type):
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    ...
    for field, value in request_init["replay"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_create_replay_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.SimulatorRestInterceptor, "post_create_replay") as post, \
        mock.patch.object(transports.SimulatorRestInterceptor, "post_create_replay_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SimulatorRestInterceptor, "pre_create_replay") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_list_replay_results_rest_bad_request(request_type=simulator.ListReplayResultsRequest):
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_replay_results_rest_call_success(request_type):
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_replay_results_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.SimulatorRestInterceptor, "post_list_replay_results") as post, \
        mock.patch.object(transports.SimulatorRestInterceptor, "post_list_replay_results_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.SimulatorRestInterceptor, "pre_list_replay_results") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = simulator.ListReplayResultsResponse.to_json(simulator.ListReplayResultsResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = simulator.ListReplayResultsResponse(), metadata
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_replay_empty_call_rest():
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_create_replay_empty_call_rest():
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_list_replay_results_empty_call_rest():
    client = SimulatorClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (transports.SimulatorGrpcTransport, grpc_helpers),
        (transports.SimulatorGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
...
def test_simulator_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.SimulatorGrpcTransport, transports.SimulatorGrpcAsyncIOTransport])
def test_simulator_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.SimulatorGrpcTransport, transports.SimulatorGrpcAsyncIOTransport])
def test_simulator_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
@pytest.mark.parametrize("client_class,transport_class", [
    (SimulatorClient, transports.SimulatorGrpcTransport),
    (SimulatorAsyncClient, transports.SimulatorGrpcAsyncIOTransport),
])
