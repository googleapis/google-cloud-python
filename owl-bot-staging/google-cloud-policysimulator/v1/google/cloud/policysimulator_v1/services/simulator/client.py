...
class SimulatorClient(metaclass=SimulatorClientMeta):
    ...
    @staticmethod
    def replay_path(project: str,location: str,replay: str,) -> str:
        """Returns a fully-qualified replay string."""
        return "projects/{project}/locations/{location}/replays/{replay}".format(project=project, location=location, replay=replay, )

    @staticmethod
    def parse_replay_path(path: str) -> Dict[str,str]:
        """Parses a replay path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/replays/(?P<replay>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def replay_result_path(project: str,location: str,replay: str,replay_result: str,) -> str:
        """Returns a fully-qualified replay_result string."""
        return "projects/{project}/locations/{location}/replays/{replay}/results/{replay_result}".format(project=project, location=location, replay=replay, replay_result=replay_result, )

    @staticmethod
    def parse_replay_result_path(path: str) -> Dict[str,str]:
        """Parses a replay_result path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/replays/(?P<replay>.+?)/results/(?P<replay_result>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = SimulatorClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = SimulatorClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = SimulatorClient._DEFAULT_UNIVERSE
    ...
    def _compare_universes(client_universe: str,
        ...
        default_universe = SimulatorClient._DEFAULT_UNIVERSE
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            SimulatorClient._compare_universes(self.universe_domain, self.transport._credentials))
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, SimulatorTransport, Callable[..., SimulatorTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = SimulatorClient._read_environment_variables()
        self._client_cert_source = SimulatorClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = SimulatorClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, SimulatorTransport)
        if transport_provided:
            ...
            self._transport = cast(SimulatorTransport, transport)
            self._api_endpoint = self._transport.host

        self._api_endpoint = (self._api_endpoint or
            SimulatorClient._get_api_endpoint(
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[SimulatorTransport], Callable[..., SimulatorTransport]] = (
