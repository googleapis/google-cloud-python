...
def test__read_environment_variables():
    assert ContainerAnalysisClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert ContainerAnalysisClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            ContainerAnalysisClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert ContainerAnalysisClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert ContainerAnalysisClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert ContainerAnalysisClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert ContainerAnalysisClient._get_client_cert_source(None, False) is None
    assert ContainerAnalysisClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert ContainerAnalysisClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert ContainerAnalysisClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert ContainerAnalysisClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(ContainerAnalysisClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisClient))
@mock.patch.object(ContainerAnalysisAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = ContainerAnalysisClient._DEFAULT_UNIVERSE
    default_endpoint = ContainerAnalysisClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = ContainerAnalysisClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert ContainerAnalysisClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert ContainerAnalysisClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ContainerAnalysisClient.DEFAULT_MTLS_ENDPOINT
    assert ContainerAnalysisClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert ContainerAnalysisClient._get_api_endpoint(None, None, default_universe, "always") == ContainerAnalysisClient.DEFAULT_MTLS_ENDPOINT
    assert ContainerAnalysisClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ContainerAnalysisClient.DEFAULT_MTLS_ENDPOINT
    assert ContainerAnalysisClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert ContainerAnalysisClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        ContainerAnalysisClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert ContainerAnalysisClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert ContainerAnalysisClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert ContainerAnalysisClient._get_universe_domain(None, None) == ContainerAnalysisClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        ContainerAnalysisClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = ContainerAnalysisClient(credentials=cred)
...
@pytest.mark.parametrize("transport_class,transport_name", [
    ...
    (transports.ContainerAnalysisRestTransport, "rest"),
])
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (ContainerAnalysisClient, transports.ContainerAnalysisRestTransport, "rest"),
])
@mock.patch.object(ContainerAnalysisClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisClient))
@mock.patch.object(ContainerAnalysisAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    ...
    (ContainerAnalysisClient, transports.ContainerAnalysisRestTransport, "rest", "true"),
    (ContainerAnalysisClient, transports.ContainerAnalysisRestTransport, "rest", "false"),
])
@mock.patch.object(ContainerAnalysisClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisClient))
@mock.patch.object(ContainerAnalysisAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisAsyncClient))
...
@pytest.mark.parametrize("client_class", [
    ContainerAnalysisClient, ContainerAnalysisAsyncClient
])
...
@mock.patch.object(ContainerAnalysisClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisClient))
@mock.patch.object(ContainerAnalysisAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(ContainerAnalysisAsyncClient))
def test_container_analysis_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ContainerAnalysisClient, transports.ContainerAnalysisGrpcTransport, "grpc", grpc_helpers),
    (ContainerAnalysisAsyncClient, transports.ContainerAnalysisGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (ContainerAnalysisClient, transports.ContainerAnalysisRestTransport, "rest", None),
])
...
def test_get_vulnerability_occurrences_summary_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_get_vulnerability_occurrences_summary_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_vulnerability_occurrences_summary in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_vulnerability_occurrences_summary] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_vulnerability_occurrences_summary_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ContainerAnalysisAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_vulnerability_occurrences_summary in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_vulnerability_occurrences_summary] = mock_rpc
...
def test_export_sbom(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.export_sbom),
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = containeranalysis.ExportSBOMResponse(
            discovery_occurrence='discovery_occurrence_value',
        )
        response = client.export_sbom(request)
...
def test_export_sbom_non_empty_request_with_auto_populated_field():
    # This test is a coverage failsafe to make sure that UUID4 fields are
    # automatically populated, according to AIP-4235, with non-empty requests.
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_export_sbom_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.export_sbom in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.export_sbom] = mock_rpc
        request = {}
        client.export_sbom(request)
...
@pytest.mark.asyncio
async def test_export_sbom_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = ContainerAnalysisAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.export_sbom in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.export_sbom] = mock_rpc

        request = {}
        await client.export_sbom(request)
...
@pytest.mark.asyncio
async def test_export_sbom_async(transport: str = 'grpc_asyncio', request_type=containeranalysis.ExportSBOMRequest):
    client = ContainerAnalysisAsyncClient(
        credentials=async_anonymous_credentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(containeranalysis.ExportSBOMResponse(
            discovery_occurrence='discovery_occurrence_value',
        ))
        response = await client.export_sbom(request)
...
@pytest.mark.asyncio
async def test_export_sbom_field_headers_async():
    client = ContainerAnalysisAsyncClient(
        credentials=async_anonymous_credentials(),
    )
...
def test_get_vulnerability_occurrences_summary_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ContainerAnalysisClient(
...
def test_export_sbom_rest_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = ContainerAnalysisClient(
...
def test_export_sbom_rest_required_fields(request_type=containeranalysis.ExportSBOMRequest):
    transport_class = transports.ContainerAnalysisRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).export_sbom._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    ...
    # Designate an appropriate value for the returned response.
    return_value = containeranalysis.ExportSBOMResponse()
    # Mock the http request call within the method and fake a response.
    with mock.patch.object(Session, 'request') as req:
        # We need to mock transcode() because providing default values
        # for required fields will fail the real version if the http_options
        ...
        with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            # Convert return value to protobuf type
            return_value = containeranalysis.ExportSBOMResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
...
def test_export_sbom_rest_unset_required_fields():
    ...
    unset_fields = transport.export_sbom._get_unset_required_fields({})
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_vulnerability_occurrences_summary_empty_call_grpc():
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = containeranalysis.VulnerabilityOccurrencesSummary()
        client.get_vulnerability_occurrences_summary(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_export_sbom_empty_call_grpc():
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = containeranalysis.ExportSBOMResponse()
        client.export_sbom(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = ContainerAnalysisAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_get_vulnerability_occurrences_summary_empty_call_grpc_asyncio():
    client = ContainerAnalysisAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.get_vulnerability_occurrences_summary(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_export_sbom_empty_call_grpc_asyncio():
    client = ContainerAnalysisAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.export_sbom(request=None)
...
def test_transport_kind_rest():
    transport = ContainerAnalysisClient.get_transport_class("rest")(
        credentials=ga_credentials.AnonymousCredentials()
    )
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    transport = transports.ContainerAnalysisRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ContainerAnalysisRestInterceptor(),
        )
    client = ContainerAnalysisClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_set_iam_policy") as post, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_set_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "pre_set_iam_policy") as pre:
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_get_iam_policy") as post, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_get_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "pre_get_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_vulnerability_occurrences_summary_rest_bad_request(request_type=containeranalysis.GetVulnerabilityOccurrencesSummaryRequest):
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_vulnerability_occurrences_summary_rest_call_success(request_type):
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_vulnerability_occurrences_summary_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_get_vulnerability_occurrences_summary") as post, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_get_vulnerability_occurrences_summary_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "pre_get_vulnerability_occurrences_summary") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = containeranalysis.VulnerabilityOccurrencesSummary.to_json(containeranalysis.VulnerabilityOccurrencesSummary())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = containeranalysis.VulnerabilityOccurrencesSummary(), metadata
...
def test_export_sbom_rest_bad_request(request_type=containeranalysis.ExportSBOMRequest):
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
    # send a request that will satisfy transcoding
    request_init = {'name': 'projects/sample1/resources/sample2'}
    request = request_type(**request_init)
...
def test_export_sbom_rest_call_success(request_type):
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
@pytest.mark.parametrize("null_interceptor", [True, False])
def test_export_sbom_rest_interceptors(null_interceptor):
    transport = transports.ContainerAnalysisRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.ContainerAnalysisRestInterceptor(),
        )
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_export_sbom") as post, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "post_export_sbom_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ContainerAnalysisRestInterceptor, "pre_export_sbom") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = containeranalysis.ExportSBOMRequest.pb(containeranalysis.ExportSBOMRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = containeranalysis.ExportSBOMResponse.to_json(containeranalysis.ExportSBOMResponse())
        req.return_value.content = return_value
        ...
        pre.return_value = request, metadata
        post.return_value = containeranalysis.ExportSBOMResponse()
        post_with_metadata.return_value = containeranalysis.ExportSBOMResponse(), metadata

        client.export_sbom(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_get_vulnerability_occurrences_summary_empty_call_rest():
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_export_sbom_empty_call_rest():
    client = ContainerAnalysisClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest",
    )
...
def test_container_analysis_client_transport_session_collision(transport_name):
    ...
    client1 = ContainerAnalysisClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = ContainerAnalysisClient(
        credentials=creds2,
        transport=transport_name,
    )
    ...
    assert session1 != session2
    session1 = client1.transport.export_sbom._session
    session2 = client2.transport.export_sbom._session
    assert session1 != session2
...
@pytest.mark.parametrize("client_class,transport_class", [
    ...
    (ContainerAnalysisAsyncClient, transports.ContainerAnalysisGrpcAsyncIOTransport),
])
