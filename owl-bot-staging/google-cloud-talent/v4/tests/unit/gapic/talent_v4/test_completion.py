...
def test_complete_query_rest(request_type):
    ...
    request_init = {'tenant': 'projects/sample1/tenants/sample2'}
...
def test_complete_query_rest_required_fields(request_type=completion_service.CompleteQueryRequest):
    ...
    jsonified_request["tenant"] = 'tenant_value'
    ...
    assert "tenant" in jsonified_request
    assert jsonified_request["tenant"] == 'tenant_value'
    assert "query" in jsonified_request
...
def test_complete_query_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("company", "languageCodes", "pageSize", "query", "scope", "type", )) & set(("tenant", "query", "pageSize", )))
...
def test_completion_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.talent_v4.services.completion.transports.CompletionTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_completion_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.talent_v4.services.completion.transports.CompletionTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CompletionTransport()
        adc.assert_called_once()
