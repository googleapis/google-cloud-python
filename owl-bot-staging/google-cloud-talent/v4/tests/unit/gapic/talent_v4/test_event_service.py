...
@pytest.mark.parametrize("client_class", [
    EventServiceClient, EventServiceAsyncClient
])
...
def test_create_client_event_rest(request_type):
    ...
    request_init["client_event"] = {'request_id': 'request_id_value', 'event_id': 'event_id_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'job_event': {'type_': 1, 'jobs': ['jobs_value1', 'jobs_value2']}, 'event_notes': 'event_notes_value'}
...
def test_create_client_event_rest_required_fields(request_type=event_service.CreateClientEventRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).create_client_event._get_unset_required_fields(jsonified_request)
...
def test_create_client_event_rest_unset_required_fields():
    transport = transports.EventServiceRestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)
...
def test_create_client_event_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.EventServiceRestInterceptor, "post_create_client_event") as post, \
         mock.patch.object(transports.EventServiceRestInterceptor, "pre_create_client_event") as pre:
...
def test_event_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.talent_v4.services.event_service.transports.EventServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_event_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.talent_v4.services.event_service.transports.EventServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (_AnonymousCredentialsWithUniverseDomain(), None)
        transport = transports.EventServiceTransport()
        adc.assert_called_once()
