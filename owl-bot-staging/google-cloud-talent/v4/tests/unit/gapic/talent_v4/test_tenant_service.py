...
def test_create_tenant_rest(request_type):
    ...
    test_field = tenant_service.CreateTenantRequest.meta.fields["tenant"]
    ...
    for field, value in request_init["tenant"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
    ...
    for subfield_to_delete in subfields_not_in_runtime: # pragma: NO COVER
        ...
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init["tenant"][field])):
                    del request_init["tenant"][field][i][subfield]
            else:
                del request_init["tenant"][field][subfield]
...
def test_create_tenant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v4/{parent=projects/*}/tenants" % client.transport._host, args[1])
...
def test_get_tenant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v4/{name=projects/*/tenants/*}" % client.transport._host, args[1])
...
def test_update_tenant_rest(request_type):
    ...
    request_init["tenant"] = {'name': 'projects/sample1/tenants/sample2', 'external_id': 'external_id_value'}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748

    # Determine if the message type is proto-plus or protobuf
    test_field = tenant_service.UpdateTenantRequest.meta.fields["tenant"]
...
def test_update_tenant_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v4/{tenant.name=projects/*/tenants/*}" % client.transport._host, args[1])
