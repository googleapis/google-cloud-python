...
@pytest.mark.parametrize("client_class", [
    CompletionClient, CompletionAsyncClient
])
...
def test_complete_query_rest_required_fields(request_type=completion_service.CompleteQueryRequest):
    ...
    assert not set(unset_fields) - set(("company", "language_codes", "page_size", "query", "scope", "type_", ))
    jsonified_request.update(unset_fields)
...
def test_complete_query_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("company", "languageCodes", "pageSize", "query", "scope", "type", )) & set(("parent", "query", "pageSize", )))
...
def test_complete_query_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.CompletionRestInterceptor, "post_complete_query") as post, \
        mock.patch.object(transports.CompletionRestInterceptor, "post_complete_query_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.CompletionRestInterceptor, "pre_complete_query") as pre:
...
def test_completion_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.talent_v4beta1.services.completion.transports.CompletionTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_completion_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.talent_v4beta1.services.completion.transports.CompletionTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CompletionTransport()
        adc.assert_called_once()
