...
@pytest.mark.parametrize("client_class", [
    CloudCatalogClient, CloudCatalogAsyncClient
])
...
def test_list_services_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.CloudCatalogRestInterceptor, "post_list_services") as post, \
         mock.patch.object(transports.CloudCatalogRestInterceptor, "pre_list_services") as pre:
...
def test_list_skus_rest_required_fields(request_type=cloud_catalog.ListSkusRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).list_skus._get_unset_required_fields(jsonified_request)
...
def test_list_skus_rest_unset_required_fields():
    transport = transports.CloudCatalogRestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)
...
def test_list_skus_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.CloudCatalogRestInterceptor, "post_list_skus") as post, \
         mock.patch.object(transports.CloudCatalogRestInterceptor, "pre_list_skus") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_cloud_catalog_transport_auth_adc(transport_class):
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        ...
        adc.assert_called_once_with(
            scopes=["1", "2"],
            default_scopes=(                'https://www.googleapis.com/auth/cloud-billing',                'https://www.googleapis.com/auth/cloud-billing.readonly',                'https://www.googleapis.com/auth/cloud-platform',),
            quota_project_id="octopus",
        )
