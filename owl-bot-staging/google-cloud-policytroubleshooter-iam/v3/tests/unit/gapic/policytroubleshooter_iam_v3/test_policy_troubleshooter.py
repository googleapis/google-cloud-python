...
from google.cloud.policytroubleshooter_iam_v3.services.policy_troubleshooter import transports
...
def test__read_environment_variables():
    assert PolicyTroubleshooterClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert PolicyTroubleshooterClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            PolicyTroubleshooterClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert PolicyTroubleshooterClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert PolicyTroubleshooterClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert PolicyTroubleshooterClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert PolicyTroubleshooterClient._get_client_cert_source(None, False) is None
    assert PolicyTroubleshooterClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert PolicyTroubleshooterClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert PolicyTroubleshooterClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert PolicyTroubleshooterClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(PolicyTroubleshooterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterClient))
@mock.patch.object(PolicyTroubleshooterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = PolicyTroubleshooterClient._DEFAULT_UNIVERSE
    default_endpoint = PolicyTroubleshooterClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = PolicyTroubleshooterClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert PolicyTroubleshooterClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert PolicyTroubleshooterClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == PolicyTroubleshooterClient.DEFAULT_MTLS_ENDPOINT
    assert PolicyTroubleshooterClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert PolicyTroubleshooterClient._get_api_endpoint(None, None, default_universe, "always") == PolicyTroubleshooterClient.DEFAULT_MTLS_ENDPOINT
    assert PolicyTroubleshooterClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == PolicyTroubleshooterClient.DEFAULT_MTLS_ENDPOINT
    assert PolicyTroubleshooterClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert PolicyTroubleshooterClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        PolicyTroubleshooterClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert PolicyTroubleshooterClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert PolicyTroubleshooterClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert PolicyTroubleshooterClient._get_universe_domain(None, None) == PolicyTroubleshooterClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        PolicyTroubleshooterClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = PolicyTroubleshooterClient(credentials=cred)
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterGrpcTransport, "grpc"),
    (PolicyTroubleshooterAsyncClient, transports.PolicyTroubleshooterGrpcAsyncIOTransport, "grpc_asyncio"),
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterRestTransport, "rest"),
])
@mock.patch.object(PolicyTroubleshooterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterClient))
@mock.patch.object(PolicyTroubleshooterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterGrpcTransport, "grpc", "true"),
    (PolicyTroubleshooterAsyncClient, transports.PolicyTroubleshooterGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterGrpcTransport, "grpc", "false"),
    (PolicyTroubleshooterAsyncClient, transports.PolicyTroubleshooterGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterRestTransport, "rest", "true"),
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterRestTransport, "rest", "false"),
])
@mock.patch.object(PolicyTroubleshooterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterClient))
@mock.patch.object(PolicyTroubleshooterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterAsyncClient))
...
@pytest.mark.parametrize("client_class", [
    PolicyTroubleshooterClient, PolicyTroubleshooterAsyncClient
])
...
@mock.patch.object(PolicyTroubleshooterClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterClient))
@mock.patch.object(PolicyTroubleshooterAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(PolicyTroubleshooterAsyncClient))
def test_policy_troubleshooter_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterGrpcTransport, "grpc", grpc_helpers),
    (PolicyTroubleshooterAsyncClient, transports.PolicyTroubleshooterGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (PolicyTroubleshooterClient, transports.PolicyTroubleshooterRestTransport, "rest", None),
])
...
async def test_troubleshoot_iam_policy_async(transport: str = 'grpc_asyncio', request_type=troubleshooter.TroubleshootIamPolicyRequest):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(troubleshooter.TroubleshootIamPolicyResponse(
            overall_access_state=troubleshooter.TroubleshootIamPolicyResponse.OverallAccessState.CAN_ACCESS,
        ))
...
def test_troubleshoot_iam_policy_rest_bad_request(request_type=troubleshooter.TroubleshootIamPolicyRequest):
    client = PolicyTroubleshooterClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_troubleshoot_iam_policy_rest_interceptors(null_interceptor):
    transport = transports.PolicyTroubleshooterRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.PolicyTroubleshooterRestInterceptor(),
        )
    client = PolicyTroubleshooterClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.PolicyTroubleshooterRestInterceptor, "post_troubleshoot_iam_policy") as post, \
        mock.patch.object(transports.PolicyTroubleshooterRestInterceptor, "post_troubleshoot_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.PolicyTroubleshooterRestInterceptor, "pre_troubleshoot_iam_policy") as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = troubleshooter.TroubleshootIamPolicyResponse.to_json(troubleshooter.TroubleshootIamPolicyResponse())
        ...
        post_with_metadata.return_value = troubleshooter.TroubleshootIamPolicyResponse(), metadata
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.PolicyTroubleshooterGrpcTransport, transports.PolicyTroubleshooterGrpcAsyncIOTransport])
def test_policy_troubleshooter_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
@pytest.mark.parametrize("client_class,transport_class", [
    ...
    (PolicyTroubleshooterAsyncClient, transports.PolicyTroubleshooterGrpcAsyncIOTransport),
])
