...
class QuotaServiceClient(metaclass=QuotaServiceClientMeta):
    ...
    @staticmethod
    def quota_group_path(account: str,quota_group: str,) -> str:
        """Returns a fully-qualified quota_group string."""
        return "accounts/{account}/quotaGroups/{quota_group}".format(account=account, quota_group=quota_group, )

    @staticmethod
    def parse_quota_group_path(path: str) -> Dict[str,str]:
        """Parses a quota_group path into its component segments."""
        m = re.match(r"^accounts/(?P<account>.+?)/quotaGroups/(?P<quota_group>.+?)$", path)
