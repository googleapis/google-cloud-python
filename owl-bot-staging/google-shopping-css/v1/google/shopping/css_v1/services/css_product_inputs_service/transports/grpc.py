...
class CssProductInputsServiceGrpcTransport(CssProductInputsServiceTransport):
    """gRPC backend transport for CssProductInputsService.
    ...
    @property
    def insert_css_product_input(self) -> Callable[
            [css_product_inputs.InsertCssProductInputRequest],
            css_product_inputs.CssProductInput]:
        r"""Return a callable for the insert css product input method over gRPC.
        ...
        Returns:
            Callable[[~.InsertCssProductInputRequest],
                    ~.CssProductInput]:
    ...
    @property
    def update_css_product_input(self) -> Callable[
    ...
    @property
    def delete_css_product_input(self) -> Callable[
            [css_product_inputs.DeleteCssProductInputRequest],
            empty_pb2.Empty]:
        r"""Return a callable for the delete css product input method over gRPC.

        Deletes a CSS Product input from your CSS Center
        account.
        After a delete it may take several minutes until the
        input is no longer available.

        Returns:
            Callable[[~.DeleteCssProductInputRequest],
                    ~.Empty]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
