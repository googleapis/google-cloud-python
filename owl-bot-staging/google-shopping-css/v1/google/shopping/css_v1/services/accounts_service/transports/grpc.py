...
class AccountsServiceGrpcTransport(AccountsServiceTransport):
    ...
    @property
    def list_child_accounts(self) -> Callable[
            [accounts.ListChildAccountsRequest],
            accounts.ListChildAccountsResponse]:
        r"""Return a callable for the list child accounts method over gRPC.

        Lists all the accounts under the specified CSS
        account ID, and optionally filters by label ID and
        account name.

        Returns:
            Callable[[~.ListChildAccountsRequest],
        ...
        if 'list_child_accounts' not in self._stubs:
            self._stubs['list_child_accounts'] = self._logged_channel.unary_unary(
                '/google.shopping.css.v1.AccountsService/ListChildAccounts',
                request_serializer=accounts.ListChildAccountsRequest.serialize,
                response_deserializer=accounts.ListChildAccountsResponse.deserialize,
            )
        return self._stubs['list_child_accounts']
    ...
    @property
    def update_labels(self) -> Callable[
            [accounts.UpdateAccountLabelsRequest],
            accounts.Account]:
        r"""Return a callable for the update labels method over gRPC.

        Updates labels assigned to CSS/MC accounts by a CSS
        domain.

        Returns:
            Callable[[~.UpdateAccountLabelsRequest],
                    ~.Account]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'update_labels' not in self._stubs:
            self._stubs['update_labels'] = self._logged_channel.unary_unary(
                '/google.shopping.css.v1.AccountsService/UpdateLabels',
                request_serializer=accounts.UpdateAccountLabelsRequest.serialize,
                response_deserializer=accounts.Account.deserialize,
            )
        return self._stubs['update_labels']
