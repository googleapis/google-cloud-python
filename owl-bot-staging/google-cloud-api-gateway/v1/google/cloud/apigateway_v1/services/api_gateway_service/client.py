...
class ApiGatewayServiceClient(metaclass=ApiGatewayServiceClientMeta):
    ...
    _DEFAULT_ENDPOINT_TEMPLATE = "apigateway.{UNIVERSE_DOMAIN}"
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = ApiGatewayServiceClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = ApiGatewayServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def _get_universe_domain(client_universe_domain: Optional[str], universe_domain_env: Optional[str]) -> str:
        ...
        """
        universe_domain = ApiGatewayServiceClient._DEFAULT_UNIVERSE
    ...
    def __init__(self, *,
            ...
            transport: Optional[Union[str, ApiGatewayServiceTransport, Callable[..., ApiGatewayServiceTransport]]] = None,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = ApiGatewayServiceClient._read_environment_variables()
        self._client_cert_source = ApiGatewayServiceClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = ApiGatewayServiceClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        transport_provided = isinstance(transport, ApiGatewayServiceTransport)
        if transport_provided:
            ...
            self._transport = cast(ApiGatewayServiceTransport, transport)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[ApiGatewayServiceTransport], Callable[..., ApiGatewayServiceTransport]] = (
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(std_logging.DEBUG):  # pragma: NO COVER
                _LOGGER.debug(
                    "Created client `google.cloud.apigateway_v1.ApiGatewayServiceClient`.",
                    extra = {
    ...
    def create_api_config(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [parent, api_config, api_config_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
    ...
    def update_api_config(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [api_config, update_mask]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
