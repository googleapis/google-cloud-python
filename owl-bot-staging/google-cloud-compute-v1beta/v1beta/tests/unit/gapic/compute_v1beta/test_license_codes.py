...
from google.cloud.compute_v1beta.services.license_codes import LicenseCodesClient
from google.cloud.compute_v1beta.services.license_codes import transports
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/licenseCodes/{resource}/testIamPermissions" % client.transport._host, args[1])
...
def test_license_codes_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1beta.services.license_codes.transports.LicenseCodesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_license_codes_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1beta.services.license_codes.transports.LicenseCodesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.LicenseCodesTransport()
        adc.assert_called_once()
