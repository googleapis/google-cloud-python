...
from google.cloud.compute_v1beta.services.instance_templates import InstanceTemplatesClient
from google.cloud.compute_v1beta.services.instance_templates import pagers
from google.cloud.compute_v1beta.services.instance_templates import transports
...
def test__read_environment_variables():
    assert InstanceTemplatesClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert InstanceTemplatesClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            InstanceTemplatesClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert InstanceTemplatesClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert InstanceTemplatesClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert InstanceTemplatesClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert InstanceTemplatesClient._get_client_cert_source(None, False) is None
    assert InstanceTemplatesClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert InstanceTemplatesClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert InstanceTemplatesClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert InstanceTemplatesClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(InstanceTemplatesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstanceTemplatesClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = InstanceTemplatesClient._DEFAULT_UNIVERSE
    default_endpoint = InstanceTemplatesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = InstanceTemplatesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert InstanceTemplatesClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert InstanceTemplatesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == InstanceTemplatesClient.DEFAULT_MTLS_ENDPOINT
    assert InstanceTemplatesClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert InstanceTemplatesClient._get_api_endpoint(None, None, default_universe, "always") == InstanceTemplatesClient.DEFAULT_MTLS_ENDPOINT
    assert InstanceTemplatesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == InstanceTemplatesClient.DEFAULT_MTLS_ENDPOINT
    assert InstanceTemplatesClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert InstanceTemplatesClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        InstanceTemplatesClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert InstanceTemplatesClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert InstanceTemplatesClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert InstanceTemplatesClient._get_universe_domain(None, None) == InstanceTemplatesClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        InstanceTemplatesClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = InstanceTemplatesClient(credentials=cred)
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (InstanceTemplatesClient, transports.InstanceTemplatesRestTransport, "rest", "true"),
    (InstanceTemplatesClient, transports.InstanceTemplatesRestTransport, "rest", "false"),
])
@mock.patch.object(InstanceTemplatesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstanceTemplatesClient))
...
@mock.patch.object(InstanceTemplatesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(InstanceTemplatesClient))
def test_instance_templates_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (InstanceTemplatesClient, transports.InstanceTemplatesRestTransport, "rest", None),
])
...
def test_aggregated_list_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/compute/beta/projects/{project}/aggregated/instanceTemplates" % client.transport._host, args[1])
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/instanceTemplates/{instance_template}" % client.transport._host, args[1])
...
def test_get_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("view", )) & set(("instanceTemplate", "project", )))
...
def test_get_iam_policy_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/instanceTemplates/{resource}/getIamPolicy" % client.transport._host, args[1])
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        mock_args = dict(
            project='project_value',
            instance_template_resource=compute.InstanceTemplate(creation_timestamp='creation_timestamp_value'),
        )
        mock_args.update(sample_request)
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/instanceTemplates" % client.transport._host, args[1])
...
def test_list_rest_required_fields(request_type=compute.ListInstanceTemplatesRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("filter", "max_results", "order_by", "page_token", "return_partial_success", "view", ))
    jsonified_request.update(unset_fields)
...
def test_list_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("filter", "maxResults", "orderBy", "pageToken", "returnPartialSuccess", "view", )) & set(("project", )))
...
def test_list_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, compute.InstanceTemplate)
                for i in results)
...
def test_set_iam_policy_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/instanceTemplates/{resource}/setIamPolicy" % client.transport._host, args[1])
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/instanceTemplates/{resource}/testIamPermissions" % client.transport._host, args[1])
...
def test_aggregated_list_rest_bad_request(request_type=compute.AggregatedListInstanceTemplatesRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_aggregated_list_rest_interceptors(null_interceptor):
    transport = transports.InstanceTemplatesRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.InstanceTemplatesRestInterceptor(),
        )
    client = InstanceTemplatesClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_aggregated_list") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_aggregated_list_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_aggregated_list") as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceTemplateAggregatedList.to_json(compute.InstanceTemplateAggregatedList())
        ...
        post_with_metadata.return_value = compute.InstanceTemplateAggregatedList(), metadata
...
def test_delete_rest_bad_request(request_type=compute.DeleteInstanceTemplateRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_delete") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_delete_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_delete") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_get_rest_bad_request(request_type=compute.GetInstanceTemplateRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_get") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_get_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_get") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceTemplate.to_json(compute.InstanceTemplate())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.InstanceTemplate(), metadata
...
def test_get_iam_policy_rest_bad_request(request_type=compute.GetIamPolicyInstanceTemplateRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_get_iam_policy") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_get_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_get_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_insert_rest_bad_request(request_type=compute.InsertInstanceTemplateRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_insert_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'project': 'sample1'}
    request_init["instance_template_resource"] = {'creation_timestamp': 'creation_timestamp_value', 'description': 'description_value', 'id': 205, 'kind': 'kind_value', 'name': 'name_value', 'properties': {'advanced_machine_features': {'enable_nested_virtualization': True, 'enable_uefi_networking': True, 'performance_monitoring_unit': 'performance_monitoring_unit_value', 'threads_per_core': 1689, 'turbo_mode': 'turbo_mode_value', 'visible_core_count': 1918}, 'can_ip_forward': True, 'confidential_instance_config': {'confidential_instance_type': 'confidential_instance_type_value', 'enable_confidential_compute': True}, 'description': 'description_value', 'disks': [{'architecture': 'architecture_value', 'auto_delete': True, 'boot': True, 'device_name': 'device_name_value', 'disk_encryption_key': {'kms_key_name': 'kms_key_name_value', 'kms_key_service_account': 'kms_key_service_account_value', 'raw_key': 'raw_key_value', 'rsa_encrypted_key': 'rsa_encrypted_key_value', 'sha256': 'sha256_value'}, 'disk_size_gb': 1261, 'force_attach': True, 'guest_os_features': [{'type_': 'type__value'}], 'index': 536, 'initialize_params': {'architecture': 'architecture_value', 'description': 'description_value', 'disk_name': 'disk_name_value', 'disk_size_gb': 1261, 'disk_type': 'disk_type_value', 'enable_confidential_compute': True, 'guest_os_features': {}, 'labels': {}, 'licenses': ['licenses_value1', 'licenses_value2'], 'multi_writer': True, 'on_update_action': 'on_update_action_value', 'provisioned_iops': 1740, 'provisioned_throughput': 2411, 'replica_zones': ['replica_zones_value1', 'replica_zones_value2'], 'resource_manager_tags': {}, 'resource_policies': ['resource_policies_value1', 'resource_policies_value2'], 'source_image': 'source_image_value', 'source_image_encryption_key': {}, 'source_instant_snapshot': 'source_instant_snapshot_value', 'source_snapshot': 'source_snapshot_value', 'source_snapshot_encryption_key': {}, 'storage_pool': 'storage_pool_value'}, 'interface': 'interface_value', 'kind': 'kind_value', 'licenses': ['licenses_value1', 'licenses_value2'], 'locked': True, 'mode': 'mode_value', 'saved_state': 'saved_state_value', 'shielded_instance_initial_state': {'dbs': [{'content': 'content_value', 'file_type': 'file_type_value'}], 'dbxs': {}, 'keks': {}, 'pk': {}}, 'source': 'source_value', 'type_': 'type__value', 'user_licenses': ['user_licenses_value1', 'user_licenses_value2']}], 'display_device': {'enable_display': True}, 'guest_accelerators': [{'accelerator_count': 1805, 'accelerator_type': 'accelerator_type_value'}], 'key_revocation_action_type': 'key_revocation_action_type_value', 'labels': {}, 'machine_type': 'machine_type_value', 'metadata': {'fingerprint': 'fingerprint_value', 'items': [{'key': 'key_value', 'value': 'value_value'}], 'kind': 'kind_value'}, 'min_cpu_platform': 'min_cpu_platform_value', 'network_interfaces': [{'access_configs': [{'external_ipv6': 'external_ipv6_value', 'external_ipv6_prefix_length': 2837, 'kind': 'kind_value', 'name': 'name_value', 'nat_i_p': 'nat_i_p_value', 'network_tier': 'network_tier_value', 'public_ptr_domain_name': 'public_ptr_domain_name_value', 'security_policy': 'security_policy_value', 'set_public_ptr': True, 'type_': 'type__value'}], 'alias_ip_ranges': [{'ip_cidr_range': 'ip_cidr_range_value', 'subnetwork_range_name': 'subnetwork_range_name_value'}], 'fingerprint': 'fingerprint_value', 'igmp_query': 'igmp_query_value', 'internal_ipv6_prefix_length': 2831, 'ipv6_access_configs': {}, 'ipv6_access_type': 'ipv6_access_type_value', 'ipv6_address': 'ipv6_address_value', 'kind': 'kind_value', 'name': 'name_value', 'network': 'network_value', 'network_attachment': 'network_attachment_value', 'network_i_p': 'network_i_p_value', 'nic_type': 'nic_type_value', 'parent_nic_name': 'parent_nic_name_value', 'queue_count': 1197, 'stack_type': 'stack_type_value', 'subnetwork': 'subnetwork_value', 'vlan': 433}], 'network_performance_config': {'total_egress_bandwidth_tier': 'total_egress_bandwidth_tier_value'}, 'partner_metadata': {}, 'post_key_revocation_action_type': 'post_key_revocation_action_type_value', 'private_ipv6_google_access': 'private_ipv6_google_access_value', 'reservation_affinity': {'consume_reservation_type': 'consume_reservation_type_value', 'key': 'key_value', 'values': ['values_value1', 'values_value2']}, 'resource_manager_tags': {}, 'resource_policies': ['resource_policies_value1', 'resource_policies_value2'], 'scheduling': {'automatic_restart': True, 'availability_domain': 2002, 'graceful_shutdown': {'enabled': True, 'max_duration': {'nanos': 543, 'seconds': 751}}, 'host_error_timeout_seconds': 2811, 'instance_termination_action': 'instance_termination_action_value', 'local_ssd_recovery_timeout': {}, 'location_hint': 'location_hint_value', 'maintenance_freeze_duration_hours': 3512, 'maintenance_interval': 'maintenance_interval_value', 'max_run_duration': {}, 'min_node_cpus': 1379, 'node_affinities': [{'key': 'key_value', 'operator': 'operator_value', 'values': ['values_value1', 'values_value2']}], 'on_host_maintenance': 'on_host_maintenance_value', 'on_instance_stop_action': {'discard_local_ssd': True}, 'preemptible': True, 'provisioning_model': 'provisioning_model_value', 'skip_guest_os_shutdown': True, 'termination_time': 'termination_time_value'}, 'service_accounts': [{'email': 'email_value', 'scopes': ['scopes_value1', 'scopes_value2']}], 'shielded_instance_config': {'enable_integrity_monitoring': True, 'enable_secure_boot': True, 'enable_vtpm': True}, 'shielded_vm_config': {'enable_integrity_monitoring': True, 'enable_secure_boot': True, 'enable_vtpm': True}, 'tags': {'fingerprint': 'fingerprint_value', 'items': ['items_value1', 'items_value2']}}, 'region': 'region_value', 'self_link': 'self_link_value', 'source_instance': 'source_instance_value', 'source_instance_params': {'disk_configs': [{'auto_delete': True, 'custom_image': 'custom_image_value', 'device_name': 'device_name_value', 'instantiate_from': 'instantiate_from_value'}]}}
    ...
    for field, value in request_init["instance_template_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_insert") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_insert_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_insert") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_list_rest_bad_request(request_type=compute.ListInstanceTemplatesRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_list") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_list_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_list") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.InstanceTemplateList.to_json(compute.InstanceTemplateList())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.InstanceTemplateList(), metadata
...
def test_set_iam_policy_rest_bad_request(request_type=compute.SetIamPolicyInstanceTemplateRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_set_iam_policy_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_set_iam_policy") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_set_iam_policy_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_set_iam_policy") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsInstanceTemplateRequest):
    client = InstanceTemplatesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.InstanceTemplatesRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_aggregated_list_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        request_msg = compute.AggregatedListInstanceTemplatesRequest()
...
def test_delete_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.DeleteInstanceTemplateRequest()
...
def test_get_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.GetInstanceTemplateRequest()
...
def test_get_iam_policy_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.GetIamPolicyInstanceTemplateRequest()
...
def test_insert_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.InsertInstanceTemplateRequest()
...
def test_list_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.ListInstanceTemplatesRequest()
...
def test_set_iam_policy_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.SetIamPolicyInstanceTemplateRequest()
...
def test_test_iam_permissions_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.TestIamPermissionsInstanceTemplateRequest()
...
def test_instance_templates_base_transport():
    ...
    with mock.patch('google.cloud.compute_v1beta.services.instance_templates.transports.InstanceTemplatesTransport.__init__') as Transport:
...
def test_instance_templates_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1beta.services.instance_templates.transports.InstanceTemplatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_instance_templates_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1beta.services.instance_templates.transports.InstanceTemplatesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.InstanceTemplatesTransport()
        adc.assert_called_once()
...
def test_instance_templates_client_transport_session_collision(transport_name):
    ...
    client1 = InstanceTemplatesClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = InstanceTemplatesClient(
        credentials=creds2,
        transport=transport_name,
    )
