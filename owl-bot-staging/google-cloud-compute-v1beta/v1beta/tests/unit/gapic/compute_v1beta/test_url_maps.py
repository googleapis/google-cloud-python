...
from google.cloud.compute_v1beta.services.url_maps import UrlMapsClient
from google.cloud.compute_v1beta.services.url_maps import pagers
from google.cloud.compute_v1beta.services.url_maps import transports
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/urlMaps/{resource}/testIamPermissions" % client.transport._host, args[1])
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.UrlMapsRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.UrlMapsRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.UrlMapsRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_url_maps_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1beta.services.url_maps.transports.UrlMapsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_url_maps_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1beta.services.url_maps.transports.UrlMapsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.UrlMapsTransport()
        adc.assert_called_once()
