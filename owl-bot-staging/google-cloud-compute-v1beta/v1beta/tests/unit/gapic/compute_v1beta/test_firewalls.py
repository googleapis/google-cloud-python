...
from google.cloud.compute_v1beta.services.firewalls import FirewallsClient
from google.cloud.compute_v1beta.services.firewalls import pagers
from google.cloud.compute_v1beta.services.firewalls import transports
...
def test__read_environment_variables():
    assert FirewallsClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert FirewallsClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert FirewallsClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert FirewallsClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert FirewallsClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert FirewallsClient._get_client_cert_source(None, False) is None
    assert FirewallsClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert FirewallsClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert FirewallsClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert FirewallsClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(FirewallsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(FirewallsClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = FirewallsClient._DEFAULT_UNIVERSE
    default_endpoint = FirewallsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = FirewallsClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert FirewallsClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert FirewallsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == FirewallsClient.DEFAULT_MTLS_ENDPOINT
    assert FirewallsClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert FirewallsClient._get_api_endpoint(None, None, default_universe, "always") == FirewallsClient.DEFAULT_MTLS_ENDPOINT
    assert FirewallsClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == FirewallsClient.DEFAULT_MTLS_ENDPOINT
    assert FirewallsClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert FirewallsClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        FirewallsClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert FirewallsClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert FirewallsClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert FirewallsClient._get_universe_domain(None, None) == FirewallsClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        FirewallsClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = FirewallsClient(credentials=cred)
...
@mock.patch.object(FirewallsClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(FirewallsClient))
def test_firewalls_client_client_api_endpoint(client_class):
...
def test_delete_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/compute/beta/projects/{project}/global/firewalls/{firewall}" % client.transport._host, args[1])
...
def test_insert_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/firewalls" % client.transport._host, args[1])
...
def test_test_iam_permissions_rest_required_fields(request_type=compute.TestIamPermissionsFirewallRequest):
    transport_class = transports.FirewallsRestTransport
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/global/firewalls/{resource}/testIamPermissions" % client.transport._host, args[1])
...
def test_delete_rest_bad_request(request_type=compute.DeleteFirewallRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_delete_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.FirewallsRestInterceptor, "post_delete_with_metadata") as post_with_metadata, \
...
def test_get_rest_bad_request(request_type=compute.GetFirewallRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_get_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.FirewallsRestInterceptor, "post_get_with_metadata") as post_with_metadata, \
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.Firewall.to_json(compute.Firewall())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.Firewall(), metadata
...
def test_insert_rest_bad_request(request_type=compute.InsertFirewallRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_insert_rest_call_success(request_type):
    ...
    for field, value in request_init["firewall_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_insert_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.FirewallsRestInterceptor, "post_insert_with_metadata") as post_with_metadata, \
...
def test_list_rest_bad_request(request_type=compute.ListFirewallsRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_list_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.FirewallsRestInterceptor, "post_list_with_metadata") as post_with_metadata, \
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = compute.FirewallList.to_json(compute.FirewallList())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = compute.FirewallList(), metadata
...
def test_patch_rest_bad_request(request_type=compute.PatchFirewallRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_patch_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.FirewallsRestInterceptor, "post_patch_with_metadata") as post_with_metadata, \
...
def test_test_iam_permissions_rest_bad_request(request_type=compute.TestIamPermissionsFirewallRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_test_iam_permissions_rest_call_success(request_type):
    ...
    # Determine if the message type is proto-plus or protobuf
    test_field = compute.TestIamPermissionsFirewallRequest.meta.fields["test_permissions_request_resource"]
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.FirewallsRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.FirewallsRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.FirewallsRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        pb_message = compute.TestIamPermissionsFirewallRequest.pb(compute.TestIamPermissionsFirewallRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
...
def test_update_rest_bad_request(request_type=compute.UpdateFirewallRequest):
    client = FirewallsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_update_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.FirewallsRestInterceptor, "post_update_with_metadata") as post_with_metadata, \
...
def test_delete_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        request_msg = compute.DeleteFirewallRequest()
...
def test_get_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.GetFirewallRequest()
...
def test_insert_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.InsertFirewallRequest()
...
def test_list_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.ListFirewallsRequest()
...
def test_patch_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.PatchFirewallRequest()
...
def test_test_iam_permissions_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.TestIamPermissionsFirewallRequest()
...
def test_update_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        request_msg = compute.UpdateFirewallRequest()
...
def test_firewalls_base_transport():
    ...
    with mock.patch('google.cloud.compute_v1beta.services.firewalls.transports.FirewallsTransport.__init__') as Transport:
...
def test_firewalls_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1beta.services.firewalls.transports.FirewallsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_firewalls_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1beta.services.firewalls.transports.FirewallsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FirewallsTransport()
        adc.assert_called_once()
