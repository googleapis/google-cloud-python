...
from google.cloud.compute_v1beta.services.region_url_maps import RegionUrlMapsClient
from google.cloud.compute_v1beta.services.region_url_maps import pagers
from google.cloud.compute_v1beta.services.region_url_maps import transports
...
def test_invalidate_cache_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("requestId", )) & set(("cacheInvalidationRuleResource", "project", "region", "urlMap", )))
...
def test_invalidate_cache_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/regions/{region}/urlMaps/{url_map}/invalidateCache" % client.transport._host, args[1])
...
def test_test_iam_permissions_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/regions/{region}/urlMaps/{resource}/testIamPermissions" % client.transport._host, args[1])
...
def test_validate_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/compute/beta/projects/{project}/regions/{region}/urlMaps/{url_map}/validate" % client.transport._host, args[1])
...
def test_invalidate_cache_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.RegionUrlMapsRestInterceptor, "post_invalidate_cache") as post, \
        mock.patch.object(transports.RegionUrlMapsRestInterceptor, "post_invalidate_cache_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.RegionUrlMapsRestInterceptor, "pre_invalidate_cache") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_test_iam_permissions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.RegionUrlMapsRestInterceptor, "post_test_iam_permissions") as post, \
        mock.patch.object(transports.RegionUrlMapsRestInterceptor, "post_test_iam_permissions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.RegionUrlMapsRestInterceptor, "pre_test_iam_permissions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_region_url_maps_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1beta.services.region_url_maps.transports.RegionUrlMapsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_region_url_maps_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1beta.services.region_url_maps.transports.RegionUrlMapsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.RegionUrlMapsTransport()
        adc.assert_called_once()
