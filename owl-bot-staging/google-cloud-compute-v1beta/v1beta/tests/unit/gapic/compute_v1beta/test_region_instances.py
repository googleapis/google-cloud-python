...
from google.cloud.compute_v1beta.services.region_instances import RegionInstancesClient
from google.cloud.compute_v1beta.services.region_instances import transports
...
def test__read_environment_variables():
    assert RegionInstancesClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert RegionInstancesClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert RegionInstancesClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert RegionInstancesClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert RegionInstancesClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert RegionInstancesClient._get_client_cert_source(None, False) is None
    assert RegionInstancesClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert RegionInstancesClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert RegionInstancesClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert RegionInstancesClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(RegionInstancesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionInstancesClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = RegionInstancesClient._DEFAULT_UNIVERSE
    default_endpoint = RegionInstancesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = RegionInstancesClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert RegionInstancesClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert RegionInstancesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == RegionInstancesClient.DEFAULT_MTLS_ENDPOINT
    assert RegionInstancesClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert RegionInstancesClient._get_api_endpoint(None, None, default_universe, "always") == RegionInstancesClient.DEFAULT_MTLS_ENDPOINT
    assert RegionInstancesClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == RegionInstancesClient.DEFAULT_MTLS_ENDPOINT
    assert RegionInstancesClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert RegionInstancesClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        RegionInstancesClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert RegionInstancesClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert RegionInstancesClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert RegionInstancesClient._get_universe_domain(None, None) == RegionInstancesClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        RegionInstancesClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = RegionInstancesClient(credentials=cred)
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (RegionInstancesClient, transports.RegionInstancesRestTransport, "rest", "true"),
    (RegionInstancesClient, transports.RegionInstancesRestTransport, "rest", "false"),
])
@mock.patch.object(RegionInstancesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionInstancesClient))
...
@mock.patch.object(RegionInstancesClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(RegionInstancesClient))
def test_region_instances_client_client_api_endpoint(client_class):
...
def test_bulk_insert_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/compute/beta/projects/{project}/regions/{region}/instances/bulkInsert" % client.transport._host, args[1])
...
def test_bulk_insert_rest_bad_request(request_type=compute.BulkInsertRegionInstanceRequest):
    client = RegionInstancesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_bulk_insert_rest_call_success(request_type):
    ...
    for field, value in request_init["bulk_insert_instance_resource_resource"].items(): # pragma: NO COVER
        result = None
        is_repeated = False
        # For repeated fields
...
def test_bulk_insert_rest_interceptors(null_interceptor):
    transport = transports.RegionInstancesRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.RegionInstancesRestInterceptor(),
        )
    client = RegionInstancesClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.RegionInstancesRestInterceptor, "post_bulk_insert") as post, \
        mock.patch.object(transports.RegionInstancesRestInterceptor, "post_bulk_insert_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.RegionInstancesRestInterceptor, "pre_bulk_insert") as pre:
...
def test_bulk_insert_unary_empty_call_rest():
    ...
    with mock.patch.object(
        ...
        request_msg = compute.BulkInsertRegionInstanceRequest()
...
def test_region_instances_base_transport():
    ...
    with mock.patch('google.cloud.compute_v1beta.services.region_instances.transports.RegionInstancesTransport.__init__') as Transport:
...
def test_region_instances_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1beta.services.region_instances.transports.RegionInstancesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_region_instances_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1beta.services.region_instances.transports.RegionInstancesTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.RegionInstancesTransport()
        adc.assert_called_once()
...
def test_region_instances_client_transport_session_collision(transport_name):
    ...
    client1 = RegionInstancesClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = RegionInstancesClient(
        credentials=creds2,
        transport=transport_name,
    )
