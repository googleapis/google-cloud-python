...
class SaasRolloutsGrpcAsyncIOTransport(SaasRolloutsTransport):
    """gRPC AsyncIO backend transport for SaasRollouts.
    ...
    def get_rollout(self) -> Callable[
            ...
            Awaitable[rollouts_resources.Rollout]]:
    ...
    @property
    def update_rollout(self) -> Callable[
    ...
    @property
    def delete_rollout(self) -> Callable[
    ...
    @property
    def list_rollout_kinds(self) -> Callable[
        ...
        Returns:
            Callable[[~.ListRolloutKindsRequest],
                    Awaitable[~.ListRolloutKindsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    @property
    def get_rollout_kind(self) -> Callable[
            [rollouts_service.GetRolloutKindRequest],
            Awaitable[rollouts_resources.RolloutKind]]:
    ...
    @property
    def create_rollout_kind(self) -> Callable[
    ...
    @property
    def update_rollout_kind(self) -> Callable[
    ...
    @property
    def delete_rollout_kind(self) -> Callable[
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            ...
            self.update_rollout: self._wrap_method(
                self.update_rollout,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.delete_rollout: self._wrap_method(
                self.delete_rollout,
                default_retry=retries.AsyncRetry(
                    initial=1.0,
                    maximum=10.0,
            ...
            self.list_rollout_kinds: self._wrap_method(
            ...
            self.get_rollout_kind: self._wrap_method(
            ...
            self.create_rollout_kind: self._wrap_method(
                self.create_rollout_kind,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.update_rollout_kind: self._wrap_method(
                self.update_rollout_kind,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.delete_rollout_kind: self._wrap_method(
