...
from google.ads.admanager_v1.types import line_item_service
from google.longrunning import operations_pb2  # type: ignore

from .base import LineItemServiceTransport, DEFAULT_CLIENT_INFO as BASE_DEFAULT_CLIENT_INFO
...
class LineItemServiceRestInterceptor:
    ...
    * Stripping extraneous information from responses
    ...
    """
    def pre_get_line_item(self, request: line_item_service.GetLineItemRequest, metadata: Sequence[Tuple[str, str]]) -> Tuple[line_item_service.GetLineItemRequest, Sequence[Tuple[str, str]]]:
    ...
    def post_get_line_item(self, response: line_item_service.LineItem) -> line_item_service.LineItem:
        ...
        """
        return response
    def pre_list_line_items(self, request: line_item_service.ListLineItemsRequest, metadata: Sequence[Tuple[str, str]]) -> Tuple[line_item_service.ListLineItemsRequest, Sequence[Tuple[str, str]]]:
    ...
    def post_list_line_items(self, response: line_item_service.ListLineItemsResponse) -> line_item_service.ListLineItemsResponse:
...
class LineItemServiceRestTransport(LineItemServiceTransport):
    """REST backend transport for LineItemService.
    ...
    class _GetLineItem(LineItemServiceRestStub):
        def __hash__(self):
            return hash("GetLineItem")
        ...
        def __call__(self,
            ...
            http_options: List[Dict[str, str]] = [{
                'method': 'get',
                'uri': '/v1/{name=networks/*/orders/*/lineItems/*}',
            },
            ]
            ...
            # Return the response
            resp = line_item_service.LineItem()
            pb_resp = line_item_service.LineItem.pb(resp)

            json_format.Parse(response.content, pb_resp, ignore_unknown_fields=True)
            resp = self._interceptor.post_get_line_item(resp)
            return resp

    class _ListLineItems(LineItemServiceRestStub):
        def __hash__(self):
            return hash("ListLineItems")
        ...
        def __call__(self,
            ...
            http_options: List[Dict[str, str]] = [{
                'method': 'get',
                'uri': '/v1/{parent=networks/*/orders/*}/lineItems',
            },
            ]
    ...
    @property
    def get_line_item(self) -> Callable[
    ...
    @property
    def list_line_items(self) -> Callable[
    ...
    class _GetOperation(LineItemServiceRestStub):
        def __call__(self,
