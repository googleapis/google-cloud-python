...
class ReportServiceClient(metaclass=ReportServiceClientMeta):
    ...
    @staticmethod
    def report_path(network_code: str,report: str,) -> str:
        """Returns a fully-qualified report string."""
        return "networks/{network_code}/reports/{report}".format(network_code=network_code, report=report, )

    @staticmethod
    def parse_report_path(path: str) -> Dict[str,str]:
        """Parses a report path into its component segments."""
        m = re.match(r"^networks/(?P<network_code>.+?)/reports/(?P<report>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            ReportServiceClient._compare_universes(self.universe_domain, self.transport._credentials))
    ...
    def fetch_report_result_rows(self,
            request: Optional[Union[report_service.FetchReportResultRowsRequest, dict]] = None,
            *,
            name: Optional[str] = None,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
            timeout: Union[float, object] = gapic_v1.method.DEFAULT,
