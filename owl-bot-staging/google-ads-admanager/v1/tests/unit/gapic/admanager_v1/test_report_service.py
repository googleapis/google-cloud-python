...
from google.ads.admanager_v1.services.report_service import ReportServiceClient
from google.ads.admanager_v1.services.report_service import pagers
from google.ads.admanager_v1.services.report_service import transports
...
def test_list_reports_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert all(isinstance(i, report_messages.Report)
                for i in results)
...
def test_create_report_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("parent", "report", )))
...
def test_update_report_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("updateMask", )) & set(("report", "updateMask", )))
...
def test_fetch_report_result_rows_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, report_messages.Report.DataTable.Row)
                for i in results)
...
def test_get_report_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_get_report") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_get_report_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_get_report") as pre:
...
def test_list_reports_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_list_reports") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_list_reports_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_list_reports") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_create_report_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_create_report") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_create_report_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_create_report") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_update_report_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_update_report") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_update_report_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_update_report") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        client.update_report(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_run_report_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_run_report") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_run_report_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_run_report") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_fetch_report_result_rows_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_fetch_report_result_rows") as post, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "post_fetch_report_result_rows_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ReportServiceRestInterceptor, "pre_fetch_report_result_rows") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        client.fetch_report_result_rows(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_get_report_empty_call_rest():
    ...
    with mock.patch.object(
            type(client.transport.get_report),
            '__call__') as call:
...
def test_create_report_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
            type(client.transport.create_report),
            '__call__') as call:
...
def test_update_report_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
            type(client.transport.update_report),
            '__call__') as call:
...
def test_fetch_report_result_rows_empty_call_rest():
    ...
    # Mock the actual call, and fake the request.
    with mock.patch.object(
            type(client.transport.fetch_report_result_rows),
            '__call__') as call:
...
def test_report_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.admanager_v1.services.report_service.transports.ReportServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_report_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.admanager_v1.services.report_service.transports.ReportServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ReportServiceTransport()
        adc.assert_called_once()
...
def test_report_path():
    network_code = "whelk"
    report = "octopus"
    expected = "networks/{network_code}/reports/{report}".format(network_code=network_code, report=report, )
