...
def test_export_saved_report_rest_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.export_saved_report in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.export_saved_report] = mock_rpc
...
def test_export_saved_report_rest_required_fields(request_type=report_service.ExportSavedReportRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).export_saved_report._get_unset_required_fields(jsonified_request)
...
def test_export_saved_report_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.ReportServiceRestInterceptor, "post_export_saved_report") as post, \
         mock.patch.object(transports.ReportServiceRestInterceptor, "pre_export_saved_report") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = report_service.ExportSavedReportRequest.pb(report_service.ExportSavedReportRequest())
        ...
        client.export_saved_report(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_export_saved_report_rest_bad_request(transport: str = 'rest', request_type=report_service.ExportSavedReportRequest):
    client = ReportServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_export_saved_report_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/{report=networks/*/reports/*}:exportSavedReport" % client.transport._host, args[1])
