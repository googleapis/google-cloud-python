...
from google.ads.admanager_v1.services.team_service import TeamServiceClient
from google.ads.admanager_v1.services.team_service import pagers
from google.ads.admanager_v1.services.team_service import transports
...
def test_get_team_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.TeamServiceRestInterceptor, "post_get_team") as post, \
        ...
        client.get_team(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_get_team_rest_bad_request(transport: str = 'rest', request_type=team_service.GetTeamRequest):
    client = TeamServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_teams_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TeamServiceRestInterceptor, "post_list_teams") as post, \
         mock.patch.object(transports.TeamServiceRestInterceptor, "pre_list_teams") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = team_service.ListTeamsResponse.to_json(team_service.ListTeamsResponse())
        ...
        client.list_teams(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_list_teams_rest_bad_request(transport: str = 'rest', request_type=team_service.ListTeamsRequest):
    client = TeamServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_teams_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert all(isinstance(i, team_service.Team)
                for i in results)
...
def test_team_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.admanager_v1.services.team_service.transports.TeamServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_team_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.admanager_v1.services.team_service.transports.TeamServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.TeamServiceTransport()
        adc.assert_called_once()
