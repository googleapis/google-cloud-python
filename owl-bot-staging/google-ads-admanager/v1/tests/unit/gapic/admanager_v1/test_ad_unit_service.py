...
def test_get_ad_unit_rest(request_type):
    ...
    assert response.smart_size_mode == ad_unit_service.SmartSizeModeEnum.SmartSizeMode.NONE
    assert response.applied_adsense_enabled == ad_unit_enums.AppliedAdsenseEnabledEnum.AppliedAdsenseEnabled.TRUE
...
def test_get_ad_unit_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value._content = ad_unit_service.AdUnit.to_json(ad_unit_service.AdUnit())
...
def test_list_ad_units_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert all(isinstance(i, ad_unit_service.AdUnit)
                for i in results)
