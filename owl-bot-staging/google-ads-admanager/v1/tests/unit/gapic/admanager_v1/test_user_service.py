...
from google.ads.admanager_v1.services.user_service import UserServiceClient
from google.ads.admanager_v1.services.user_service import pagers
from google.ads.admanager_v1.services.user_service import transports
...
def test_user_service_client_get_transport_class():
    ...
    assert transport == transports.UserServiceRestTransport
...
def test_get_user_rest(request_type):
    ...
    request_init = {'name': 'networks/sample1/users/sample2'}
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        return_value = user_service.User(
              ...
              orders_ui_local_time_zone='orders_ui_local_time_zone_value',
        )
    ...
    assert response.orders_ui_local_time_zone == 'orders_ui_local_time_zone_value'
...
def test_get_user_rest_required_fields(request_type=user_service.GetUserRequest):
...
def test_get_user_rest_bad_request(transport: str = 'rest', request_type=user_service.GetUserRequest):
    client = UserServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_user_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/{name=networks/*/users/*}" % client.transport._host, args[1])
...
def test_list_users_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = user_service.ListUsersResponse.to_json(user_service.ListUsersResponse())
...
def test_list_users_rest_bad_request(transport: str = 'rest', request_type=user_service.ListUsersRequest):
    client = UserServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_users_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/{parent=networks/*}/users" % client.transport._host, args[1])
...
def test_list_users_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert all(isinstance(i, user_service.User)
                for i in results)
...
def test_user_service_base_transport():
    ...
    with mock.patch('google.ads.admanager_v1.services.user_service.transports.UserServiceTransport.__init__') as Transport:
...
def test_user_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.admanager_v1.services.user_service.transports.UserServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_user_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.admanager_v1.services.user_service.transports.UserServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.UserServiceTransport()
        adc.assert_called_once()
...
def test_role_path():
    network_code = "whelk"
    role = "octopus"
    expected = "networks/{network_code}/roles/{role}".format(network_code=network_code, role=role, )
    actual = UserServiceClient.role_path(network_code, role)
    assert expected == actual
...
def test_parse_role_path():
    ...
    # Check that the path construction is reversible.
    actual = UserServiceClient.parse_role_path(path)
    assert expected == actual

def test_user_path():
    network_code = "cuttlefish"
    user = "mussel"
    expected = "networks/{network_code}/users/{user}".format(network_code=network_code, user=user, )
    actual = UserServiceClient.user_path(network_code, user)
    assert expected == actual
...
@pytest.mark.parametrize("client_class,transport_class", [
    (UserServiceClient, transports.UserServiceRestTransport),
])
