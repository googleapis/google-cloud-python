...
class AlphaAnalyticsDataClient(metaclass=AlphaAnalyticsDataClientMeta):
    ...
    @staticmethod
    def audience_list_path(propertyId: str,audienceListId: str,) -> str:
        """Returns a fully-qualified audience_list string."""
        return "properties/{propertyId}/audienceLists/{audienceListId}".format(propertyId=propertyId, audienceListId=audienceListId, )

    @staticmethod
    def parse_audience_list_path(path: str) -> Dict[str,str]:
        """Parses a audience_list path into its component segments."""
        m = re.match(r"^properties/(?P<propertyId>.+?)/audienceLists/(?P<audienceListId>.+?)$", path)
