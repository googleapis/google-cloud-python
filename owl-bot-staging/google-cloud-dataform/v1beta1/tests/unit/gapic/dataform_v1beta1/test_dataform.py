...
def test_create_workspace_rest(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    for field, value in request_init["workspace"].items():
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_create_compilation_result_rest(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    for field, value in request_init["compilation_result"].items():
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
...
def test_create_workflow_invocation_rest(request_type):
    ...
    # For each item in the sample request, create a list of sub fields which are not present at runtime
    for field, value in request_init["workflow_invocation"].items():
        result = None
        is_repeated = False
        # For repeated fields
        if isinstance(value, list) and len(value):
