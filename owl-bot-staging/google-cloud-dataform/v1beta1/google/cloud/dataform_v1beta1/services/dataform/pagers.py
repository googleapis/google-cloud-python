...
class QueryDirectoryContentsPager:
    ...
    def __iter__(self) -> Iterator[dataform.QueryDirectoryContentsResponse.DirectoryEntry]:
        for page in self.pages:
...
class QueryDirectoryContentsAsyncPager:
    ...
    async def pages(self) -> AsyncIterator[dataform.QueryDirectoryContentsResponse]:
        ...
        while self._response.next_page_token:
            ...
            self._response = await self._method(self._request, metadata=self._metadata)
            yield self._response
    def __aiter__(self) -> AsyncIterator[dataform.QueryDirectoryContentsResponse.DirectoryEntry]:
        async def async_generator():
            async for page in self.pages:
