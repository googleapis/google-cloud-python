...
async def test_list_workflow_revisions_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_workflow_revisions(request={},)
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager: # pragma: no branch
            responses.append(response)
...
def test_list_workflow_revisions_rest_required_fields(request_type=workflows.ListWorkflowRevisionsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_workflow_revisions._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_create_workflow_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init["workflow"] = {'name': 'name_value', 'description': 'description_value', 'state': 1, 'revision_id': 'revision_id_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'revision_create_time': {}, 'labels': {}, 'service_account': 'service_account_value', 'source_contents': 'source_contents_value', 'crypto_key_name': 'crypto_key_name_value', 'state_error': {'details': 'details_value', 'type_': 1}, 'call_log_level': 1, 'user_env_vars': {}, 'execution_history_level': 1, 'all_kms_keys': ['all_kms_keys_value1', 'all_kms_keys_value2'], 'all_kms_keys_versions': ['all_kms_keys_versions_value1', 'all_kms_keys_versions_value2'], 'crypto_key_version': 'crypto_key_version_value', 'tags': {}}
...
def test_update_workflow_rest_call_success(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'workflow': {'name': 'projects/sample1/locations/sample2/workflows/sample3'}}
    request_init["workflow"] = {'name': 'projects/sample1/locations/sample2/workflows/sample3', 'description': 'description_value', 'state': 1, 'revision_id': 'revision_id_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'revision_create_time': {}, 'labels': {}, 'service_account': 'service_account_value', 'source_contents': 'source_contents_value', 'crypto_key_name': 'crypto_key_name_value', 'state_error': {'details': 'details_value', 'type_': 1}, 'call_log_level': 1, 'user_env_vars': {}, 'execution_history_level': 1, 'all_kms_keys': ['all_kms_keys_value1', 'all_kms_keys_value2'], 'all_kms_keys_versions': ['all_kms_keys_versions_value1', 'all_kms_keys_versions_value2'], 'crypto_key_version': 'crypto_key_version_value', 'tags': {}}
    # The version of a generated dependency at test runtime may differ from the version used during generation.
    # Delete any fields which are not present in the current runtime dependency
    # See https://github.com/googleapis/gapic-generator-python/issues/1748
...
def test_list_workflow_revisions_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.WorkflowsRestInterceptor, "post_list_workflow_revisions") as post, \
        mock.patch.object(transports.WorkflowsRestInterceptor, "post_list_workflow_revisions_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.WorkflowsRestInterceptor, "pre_list_workflow_revisions") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        client.list_workflow_revisions(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
...
def test_crypto_key_path():
    ...
    keyRing = "whelk"
    cryptoKey = "octopus"
    expected = "projects/{project}/locations/{location}/keyRings/{keyRing}/cryptoKeys/{cryptoKey}".format(project=project, location=location, keyRing=keyRing, cryptoKey=cryptoKey, )
...
def test_crypto_key_version_path():
    ...
    expected = "projects/{project}/locations/{location}/keyRings/{keyRing}/cryptoKeys/{cryptoKey}/cryptoKeyVersions/{cryptoKeyVersion}".format(project=project, location=location, keyRing=keyRing, cryptoKey=cryptoKey, cryptoKeyVersion=cryptoKeyVersion, )
