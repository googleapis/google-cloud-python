...
class WorkflowsClient(metaclass=WorkflowsClientMeta):
    ...
    @staticmethod
    def crypto_key_path(project: str,location: str,keyRing: str,cryptoKey: str,) -> str:
        """Returns a fully-qualified crypto_key string."""
        return "projects/{project}/locations/{location}/keyRings/{keyRing}/cryptoKeys/{cryptoKey}".format(project=project, location=location, keyRing=keyRing, cryptoKey=cryptoKey, )

    @staticmethod
    def parse_crypto_key_path(path: str) -> Dict[str,str]:
        """Parses a crypto_key path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/keyRings/(?P<keyRing>.+?)/cryptoKeys/(?P<cryptoKey>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def crypto_key_version_path(project: str,location: str,keyRing: str,cryptoKey: str,cryptoKeyVersion: str,) -> str:
        """Returns a fully-qualified crypto_key_version string."""
        return "projects/{project}/locations/{location}/keyRings/{keyRing}/cryptoKeys/{cryptoKey}/cryptoKeyVersions/{cryptoKeyVersion}".format(project=project, location=location, keyRing=keyRing, cryptoKey=cryptoKey, cryptoKeyVersion=cryptoKeyVersion, )

    @staticmethod
    def parse_crypto_key_version_path(path: str) -> Dict[str,str]:
        """Parses a crypto_key_version path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/keyRings/(?P<keyRing>.+?)/cryptoKeys/(?P<cryptoKey>.+?)/cryptoKeyVersions/(?P<cryptoKeyVersion>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def list_workflow_revisions(self,
