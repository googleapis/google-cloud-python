...
def test__get_api_endpoint():
    ...
    assert LookupServiceClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == LookupServiceClient.DEFAULT_MTLS_ENDPOINT
...
@pytest.mark.parametrize("client_class", [
    LookupServiceClient, LookupServiceAsyncClient
])
...
def test_resolve_service_rest_required_fields(request_type=lookup_service.ResolveServiceRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).resolve_service._get_unset_required_fields(jsonified_request)
...
def test_resolve_service_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.LookupServiceRestInterceptor, "post_resolve_service") as post, \
         mock.patch.object(transports.LookupServiceRestInterceptor, "pre_resolve_service") as pre:
