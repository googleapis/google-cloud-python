...
class StorageControlClient(metaclass=StorageControlClientMeta):
    ...
    @staticmethod
    def managed_folder_path(project: str,bucket: str,managedFolder: str,) -> str:
        """Returns a fully-qualified managed_folder string."""
        return "projects/{project}/buckets/{bucket}/managedFolders/{managedFolder}".format(project=project, bucket=bucket, managedFolder=managedFolder, )

    @staticmethod
    def parse_managed_folder_path(path: str) -> Dict[str,str]:
        """Parses a managed_folder path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/buckets/(?P<bucket>.+?)/managedFolders/(?P<managedFolder>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def _validate_universe_domain(self):
        ...
        self._is_universe_domain_valid = (self._is_universe_domain_valid or
            StorageControlClient._compare_universes(self.universe_domain, self.transport._credentials))
