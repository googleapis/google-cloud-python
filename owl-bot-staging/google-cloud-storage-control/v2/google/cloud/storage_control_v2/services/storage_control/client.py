    ...
    def create_anywhere_cache(self,
        ...
        .. code-block:: python
            ...
            def sample_create_anywhere_cache():
                ...
                request = storage_control_v2.CreateAnywhereCacheRequest(
                    parent="parent_value",
                    anywhere_cache_id="anywhere_cache_id_value",
                )
        ...
        Args:
            ...
            anywhere_cache_id (str):
                Required. System generated Anywhere
                Cache ID.

                This corresponds to the ``anywhere_cache_id`` field
                on the ``request`` instance; if ``request`` is provided, this
                should not be set.
            retry (google.api_core.retry.Retry): Designation of what errors, if any,
                should be retried.
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [parent, anywhere_cache, anywhere_cache_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
        ...
        if not isinstance(request, storage_control.CreateAnywhereCacheRequest):
            ...
            if anywhere_cache_id is not None:
                request.anywhere_cache_id = anywhere_cache_id
