...
from google.maps.addressvalidation_v1.services.address_validation import AddressValidationAsyncClient
from google.maps.addressvalidation_v1.services.address_validation import AddressValidationClient
from google.maps.addressvalidation_v1.services.address_validation import transports
...
def test__read_environment_variables():
    assert AddressValidationClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert AddressValidationClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "Unsupported"}):
        with pytest.raises(ValueError) as excinfo:
            AddressValidationClient._read_environment_variables()
    assert str(excinfo.value) == "Environment variable `GOOGLE_API_USE_CLIENT_CERTIFICATE` must be either `true` or `false`"

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert AddressValidationClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert AddressValidationClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert AddressValidationClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert AddressValidationClient._get_client_cert_source(None, False) is None
    assert AddressValidationClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert AddressValidationClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert AddressValidationClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert AddressValidationClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(AddressValidationClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationClient))
@mock.patch.object(AddressValidationAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = AddressValidationClient._DEFAULT_UNIVERSE
    default_endpoint = AddressValidationClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = AddressValidationClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert AddressValidationClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert AddressValidationClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == AddressValidationClient.DEFAULT_MTLS_ENDPOINT
    assert AddressValidationClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert AddressValidationClient._get_api_endpoint(None, None, default_universe, "always") == AddressValidationClient.DEFAULT_MTLS_ENDPOINT
    assert AddressValidationClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == AddressValidationClient.DEFAULT_MTLS_ENDPOINT
    assert AddressValidationClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert AddressValidationClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        AddressValidationClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert AddressValidationClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert AddressValidationClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert AddressValidationClient._get_universe_domain(None, None) == AddressValidationClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        AddressValidationClient._get_universe_domain("", None)
...
def test__add_cred_info_for_auth_errors(error_code, cred_info_json, show_cred_info):
    ...
    client = AddressValidationClient(credentials=cred)
...
@pytest.mark.parametrize("client_class,transport_name", [
    (AddressValidationClient, "grpc"),
    (AddressValidationAsyncClient, "grpc_asyncio"),
    (AddressValidationClient, "rest"),
])
def test_address_validation_client_from_service_account_info(client_class, transport_name):
    ...
    with mock.patch.object(service_account.Credentials, 'from_service_account_info') as factory:
        ...
        assert client.transport._host == (
            'addressvalidation.googleapis.com:443'
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.AddressValidationGrpcTransport, "grpc"),
    (transports.AddressValidationGrpcAsyncIOTransport, "grpc_asyncio"),
    (transports.AddressValidationRestTransport, "rest"),
])
...
def test_address_validation_client_get_transport_class():
    ...
    available_transports = [
        transports.AddressValidationGrpcTransport,
        transports.AddressValidationRestTransport,
    ]
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (AddressValidationClient, transports.AddressValidationGrpcTransport, "grpc"),
    (AddressValidationAsyncClient, transports.AddressValidationGrpcAsyncIOTransport, "grpc_asyncio"),
    (AddressValidationClient, transports.AddressValidationRestTransport, "rest"),
])
@mock.patch.object(AddressValidationClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationClient))
@mock.patch.object(AddressValidationAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (AddressValidationClient, transports.AddressValidationGrpcTransport, "grpc", "true"),
    (AddressValidationAsyncClient, transports.AddressValidationGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (AddressValidationClient, transports.AddressValidationGrpcTransport, "grpc", "false"),
    (AddressValidationAsyncClient, transports.AddressValidationGrpcAsyncIOTransport, "grpc_asyncio", "false"),
    (AddressValidationClient, transports.AddressValidationRestTransport, "rest", "true"),
    (AddressValidationClient, transports.AddressValidationRestTransport, "rest", "false"),
])
@mock.patch.object(AddressValidationClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationClient))
@mock.patch.object(AddressValidationAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationAsyncClient))
...
@pytest.mark.parametrize("client_class", [
    AddressValidationClient, AddressValidationAsyncClient
])
...
@mock.patch.object(AddressValidationClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationClient))
@mock.patch.object(AddressValidationAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(AddressValidationAsyncClient))
def test_address_validation_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (AddressValidationClient, transports.AddressValidationGrpcTransport, "grpc", grpc_helpers),
    (AddressValidationAsyncClient, transports.AddressValidationGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (AddressValidationClient, transports.AddressValidationRestTransport, "rest", None),
])
...
def test_address_validation_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    ...
    with mock.patch.object(
        ...
        create_channel.assert_called_with(
            ...
            scopes=None,
            default_host="addressvalidation.googleapis.com",
...
def test_validate_address(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
            type(client.transport.validate_address),
            '__call__') as call:
...
def test_validate_address_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.validate_address in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.validate_address] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_validate_address_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.validate_address in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.validate_address] = mock_rpc

        request = {}
        await client.validate_address(request)
...
async def test_validate_address_async(transport: str = 'grpc_asyncio', request_type=address_validation_service.ValidateAddressRequest):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(address_validation_service.ValidateAddressResponse(
            response_id='response_id_value',
        ))
...
def test_provide_validation_feedback(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.provide_validation_feedback),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_provide_validation_feedback_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.provide_validation_feedback in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.provide_validation_feedback] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_provide_validation_feedback_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = AddressValidationAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.provide_validation_feedback in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.provide_validation_feedback] = mock_rpc

        request = {}
        await client.provide_validation_feedback(request)
...
def test_transport_get_channel():
    ...
    transport = transports.AddressValidationGrpcAsyncIOTransport(
        credentials=ga_credentials.AnonymousCredentials(),
    )
    channel = transport.grpc_channel
    assert channel
...
@pytest.mark.parametrize("transport_class", [
    transports.AddressValidationGrpcTransport,
    transports.AddressValidationGrpcAsyncIOTransport,
    transports.AddressValidationRestTransport,
])
...
def test_validate_address_rest_bad_request(request_type=address_validation_service.ValidateAddressRequest):
    client = AddressValidationClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_validate_address_rest_interceptors(null_interceptor):
    transport = transports.AddressValidationRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.AddressValidationRestInterceptor(),
        )
    client = AddressValidationClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.AddressValidationRestInterceptor, "post_validate_address") as post, \
        mock.patch.object(transports.AddressValidationRestInterceptor, "post_validate_address_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.AddressValidationRestInterceptor, "pre_validate_address") as pre:
        ...
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = address_validation_service.ValidateAddressResponse.to_json(address_validation_service.ValidateAddressResponse())
        ...
        post_with_metadata.return_value = address_validation_service.ValidateAddressResponse(), metadata
...
def test_provide_validation_feedback_rest_bad_request(request_type=address_validation_service.ProvideValidationFeedbackRequest):
    client = AddressValidationClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_provide_validation_feedback_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.AddressValidationRestInterceptor, "post_provide_validation_feedback") as post, \
        mock.patch.object(transports.AddressValidationRestInterceptor, "post_provide_validation_feedback_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.AddressValidationRestInterceptor, "pre_provide_validation_feedback") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.headers = {"header-1": "value-1", "header-2": "value-2"}
        return_value = address_validation_service.ProvideValidationFeedbackResponse.to_json(address_validation_service.ProvideValidationFeedbackResponse())
        req.return_value.content = return_value
        ...
        post_with_metadata.return_value = address_validation_service.ProvideValidationFeedbackResponse(), metadata
...
@pytest.mark.parametrize(
    ...
    [
        (transports.AddressValidationGrpcTransport, grpc_helpers),
        (transports.AddressValidationGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
...
def test_address_validation_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_address_validation_host_no_port(transport_name):
    client = AddressValidationClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='addressvalidation.googleapis.com'),
         transport=transport_name,
    )
    assert client.transport._host == (
        'addressvalidation.googleapis.com:443'
        if transport_name in ['grpc', 'grpc_asyncio']
        else 'https://addressvalidation.googleapis.com'
    )
...
def test_address_validation_host_with_port(transport_name):
    client = AddressValidationClient(
        credentials=ga_credentials.AnonymousCredentials(),
        client_options=client_options.ClientOptions(api_endpoint='addressvalidation.googleapis.com:8000'),
        transport=transport_name,
    )
    assert client.transport._host == (
        'addressvalidation.googleapis.com:8000'
        if transport_name in ['grpc', 'grpc_asyncio']
        else 'https://addressvalidation.googleapis.com:8000'
    )
...
def test_address_validation_client_transport_session_collision(transport_name):
    ...
    client1 = AddressValidationClient(
        credentials=creds1,
        transport=transport_name,
    )
    client2 = AddressValidationClient(
        credentials=creds2,
        transport=transport_name,
    )
...
@pytest.mark.parametrize("transport_class", [transports.AddressValidationGrpcTransport, transports.AddressValidationGrpcAsyncIOTransport])
def test_address_validation_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.AddressValidationGrpcTransport, transports.AddressValidationGrpcAsyncIOTransport])
def test_address_validation_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
@pytest.mark.parametrize("client_class,transport_class", [
    (AddressValidationClient, transports.AddressValidationGrpcTransport),
    (AddressValidationAsyncClient, transports.AddressValidationGrpcAsyncIOTransport),
])
