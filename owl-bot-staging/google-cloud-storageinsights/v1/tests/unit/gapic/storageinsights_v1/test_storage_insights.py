...
async def test_list_report_configs_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_report_configs(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_get_report_config(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = storageinsights.ReportConfig(
            ...
            csv_options=storageinsights.CSVOptions(record_separator='record_separator_value'),
            object_metadata_report_options=storageinsights.ObjectMetadataReportOptions(metadata_fields=['metadata_fields_value']),
        )
...
async def test_list_report_details_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_report_details(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_get_report_detail(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = storageinsights.ReportDetail(
            name='name_value',
            report_names=['report_names_value'],
        )
    ...
    assert response.name == 'name_value'
    assert response.report_names == ['report_names_value']
