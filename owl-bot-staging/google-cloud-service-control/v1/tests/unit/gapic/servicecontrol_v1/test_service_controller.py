...
def test__get_api_endpoint():
    ...
    assert ServiceControllerClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == ServiceControllerClient.DEFAULT_MTLS_ENDPOINT
    ...
    assert ServiceControllerClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == ServiceControllerClient.DEFAULT_MTLS_ENDPOINT
...
@pytest.mark.parametrize("client_class", [
    ServiceControllerClient, ServiceControllerAsyncClient
])
...
def test_service_controller_client_client_options_from_dict():
    with mock.patch('google.cloud.servicecontrol_v1.services.service_controller.transports.ServiceControllerGrpcTransport.__init__') as grpc_transport:
...
def test_check_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        mock.patch.object(transports.ServiceControllerRestInterceptor, "post_check") as post, \
        mock.patch.object(transports.ServiceControllerRestInterceptor, "post_check_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceControllerRestInterceptor, "pre_check") as pre:
...
def test_report_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.ServiceControllerRestInterceptor, "post_report") as post, \
        mock.patch.object(transports.ServiceControllerRestInterceptor, "post_report_with_metadata") as post_with_metadata, \
        mock.patch.object(transports.ServiceControllerRestInterceptor, "pre_report") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        post_with_metadata.assert_not_called()
...
def test_service_controller_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.servicecontrol_v1.services.service_controller.transports.ServiceControllerTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_service_controller_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.servicecontrol_v1.services.service_controller.transports.ServiceControllerTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ServiceControllerTransport()
        adc.assert_called_once()
...
def test_service_controller_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ServiceControllerGrpcTransport, transports.ServiceControllerGrpcAsyncIOTransport])
def test_service_controller_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
