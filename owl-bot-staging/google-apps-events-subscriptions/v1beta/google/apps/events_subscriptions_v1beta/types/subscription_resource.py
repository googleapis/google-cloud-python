...
class Subscription(proto.Message):
    ...
    Attributes:
        drive_options (google.apps.events_subscriptions_v1beta.types.Subscription.DriveOptions):
            Optional. Features that are supported only
            for subscriptions on Drive resources.

            This field is a member of `oneof`_ ``subscription_options``.
        ...
        event_types (MutableSequence[str]):
            Required. Unordered list. Input for creating a subscription.
            Otherwise, output only. One or more types of events to
            receive about the target resource. Formatted according to
        ...
        payload_options (google.apps.events_subscriptions_v1beta.types.PayloadOptions):
            Optional. Options about what data to include
            in the event payload. Only supported for Google
            Chat events.
        notification_endpoint (google.apps.events_subscriptions_v1beta.types.NotificationEndpoint):
            Required. Immutable. The endpoint where the
            subscription delivers events, such as a Pub/Sub
            topic.
        state (google.apps.events_subscriptions_v1beta.types.Subscription.State):
            ...
            endpoint.
        suspension_reason (google.apps.events_subscriptions_v1beta.types.Subscription.ErrorType):
    ...
    class DriveOptions(proto.Message):
        r"""Additional supported options for serving Drive events.

        Attributes:
            include_descendants (bool):
                Optional. Immutable. Whether to include all the descendant
                Drive files in the list of events. If false, the
                subscription will be created for the single file/drive that
                is specified as the
                ...
                [``target_resource``][google.apps.events.subscriptions.v1beta.Subscription.target_resource]
                is either a
                `Drive <https://developers.google.com/drive/api/reference/rest/v3/drives>`__
                or a `Drive
                file <https://developers.google.com/drive/api/reference/rest/v3/files>`__
                that has mimeType set to
    ...
    drive_options: DriveOptions = proto.Field(
        proto.MESSAGE,
        number=20,
        oneof='subscription_options',
...
class NotificationEndpoint(proto.Message):
    ...
    Attributes:
        pubsub_topic (str):
            Immutable. The Pub/Sub topic that receives events for the
            subscription.
            ...
            keys <https://cloud.google.com/pubsub/docs/ordering>`__ for
            the benefit of sequential events. If the Cloud Pub/Sub topic
            has a `message storage
            policy <https://cloud.google.com/pubsub/docs/resource-location-restriction#exceptions>`__
            configured to exclude the nearest Google Cloud region,
            publishing events with ordering keys will fail.

            When the topic receives events, the events are encoded as
            Pub/Sub messages. For details, see the `Google Cloud Pub/Sub
