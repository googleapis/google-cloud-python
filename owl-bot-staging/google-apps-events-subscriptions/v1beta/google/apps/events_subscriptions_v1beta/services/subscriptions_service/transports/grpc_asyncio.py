...
from google.apps.events_subscriptions_v1beta.types import subscription_resource
from google.apps.events_subscriptions_v1beta.types import subscriptions_service
from google.longrunning import operations_pb2 # type: ignore
from .base import SubscriptionsServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import SubscriptionsServiceGrpcTransport
...
class SubscriptionsServiceGrpcAsyncIOTransport(SubscriptionsServiceTransport):
    """gRPC AsyncIO backend transport for SubscriptionsService.
    ...
    def create_subscription(self) -> Callable[
        ...
        if 'create_subscription' not in self._stubs:
            self._stubs['create_subscription'] = self._logged_channel.unary_unary(
                ...
                request_serializer=subscriptions_service.CreateSubscriptionRequest.serialize,
    ...
    def delete_subscription(self) -> Callable[
        ...
        if 'delete_subscription' not in self._stubs:
            self._stubs['delete_subscription'] = self._logged_channel.unary_unary(
                ...
                request_serializer=subscriptions_service.DeleteSubscriptionRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
    ...
    def get_subscription(self) -> Callable[
            ...
            Awaitable[subscription_resource.Subscription]]:
        ...
        if 'get_subscription' not in self._stubs:
            self._stubs['get_subscription'] = self._logged_channel.unary_unary(
                ...
                request_serializer=subscriptions_service.GetSubscriptionRequest.serialize,
                response_deserializer=subscription_resource.Subscription.deserialize,
            )
    ...
    def list_subscriptions(self) -> Callable[
        ...
        if 'list_subscriptions' not in self._stubs:
            self._stubs['list_subscriptions'] = self._logged_channel.unary_unary(
                ...
                request_serializer=subscriptions_service.ListSubscriptionsRequest.serialize,
                response_deserializer=subscriptions_service.ListSubscriptionsResponse.deserialize,
            )
    ...
    def update_subscription(self) -> Callable[
        ...
        if 'update_subscription' not in self._stubs:
            self._stubs['update_subscription'] = self._logged_channel.unary_unary(
                ...
                request_serializer=subscriptions_service.UpdateSubscriptionRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
    ...
    def reactivate_subscription(self) -> Callable[
            ...
            Awaitable[operations_pb2.Operation]]:
        r"""Return a callable for the reactivate subscription method over gRPC.

        Reactivates a suspended Google Workspace subscription.

        This method resets your subscription's ``State`` field to
        ``ACTIVE``. Before you use this method, you must fix the error
        that suspended the subscription. This method will ignore or
        reject any subscription that isn't currently in a suspended
        state. To learn how to use this method, see `Reactivate a Google
        Workspace
        subscription <https://developers.google.com/workspace/events/guides/reactivate-subscription>`__.

        Returns:
            Callable[[~.ReactivateSubscriptionRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'reactivate_subscription' not in self._stubs:
            self._stubs['reactivate_subscription'] = self._logged_channel.unary_unary(
                '/google.apps.events.subscriptions.v1beta.SubscriptionsService/ReactivateSubscription',
                request_serializer=subscriptions_service.ReactivateSubscriptionRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs['reactivate_subscription']
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            ...
            self.reactivate_subscription: self._wrap_method(
                self.reactivate_subscription,
                default_timeout=60.0,
                client_info=client_info,
            ),
...
__all__ = (
    'SubscriptionsServiceGrpcAsyncIOTransport',
)
