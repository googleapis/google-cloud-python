...
async def test_list_delivery_pipelines_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_delivery_pipelines(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_get_delivery_pipeline(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = cloud_deploy.DeliveryPipeline(
            ...
            etag='etag_value',
            suspended=True,
            serial_pipeline=cloud_deploy.SerialPipeline(stages=[cloud_deploy.Stage(target_id='target_id_value')]),
        )
...
async def test_list_targets_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_targets(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_job_runs_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_job_runs(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_create_delivery_pipeline_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init["delivery_pipeline"] = {'name': 'name_value', 'uid': 'uid_value', 'description': 'description_value', 'annotations': {}, 'labels': {}, 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'serial_pipeline': {'stages': [{'target_id': 'target_id_value', 'profiles': ['profiles_value1', 'profiles_value2'], 'strategy': {'standard': {'verify': True}, 'canary': {'runtime_config': {'kubernetes': {'gateway_service_mesh': {'http_route': 'http_route_value', 'service': 'service_value', 'deployment': 'deployment_value'}, 'service_networking': {'service': 'service_value', 'deployment': 'deployment_value', 'disable_pod_overprovisioning': True}}, 'cloud_run': {'automatic_traffic_control': True}}, 'canary_deployment': {'percentages': [1170, 1171], 'verify': True}, 'custom_canary_deployment': {'phase_configs': [{'phase_id': 'phase_id_value', 'percentage': 1054, 'profiles': ['profiles_value1', 'profiles_value2'], 'verify': True}]}}}, 'deploy_parameters': [{'values': {}, 'match_target_labels': {}}]}]}, 'condition': {'pipeline_ready_condition': {'status': True, 'update_time': {}}, 'targets_present_condition': {'status': True, 'missing_targets': ['missing_targets_value1', 'missing_targets_value2'], 'update_time': {}}, 'targets_type_condition': {'status': True, 'error_details': 'error_details_value'}}, 'etag': 'etag_value', 'suspended': True}
    request = request_type(**request_init)
