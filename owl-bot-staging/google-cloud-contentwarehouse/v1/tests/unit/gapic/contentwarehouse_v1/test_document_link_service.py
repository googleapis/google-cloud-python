...
async def test_list_linked_sources_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_linked_sources(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_list_linked_targets_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        pb_return_value = document_link_service.ListLinkedTargetsResponse.pb(return_value)
...
def test_list_linked_sources_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = document_link_service.ListLinkedSourcesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_create_document_link_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = document_link_service.DocumentLink.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
