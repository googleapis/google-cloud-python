...
class LfpInventoryServiceGrpcAsyncIOTransport(LfpInventoryServiceTransport):
    ...
    @property
    def insert_lfp_inventory(self) -> Callable[
            [lfpinventory.InsertLfpInventoryRequest],
            Awaitable[lfpinventory.LfpInventory]]:
        r"""Return a callable for the insert lfp inventory method over gRPC.

        Inserts a ``LfpInventory`` resource for the given target
        merchant account. If the resource already exists, it will be
        replaced. The inventory automatically expires after 30 days.

        Returns:
            Callable[[~.InsertLfpInventoryRequest],
                    Awaitable[~.LfpInventory]]:
        ...
        if 'insert_lfp_inventory' not in self._stubs:
            self._stubs['insert_lfp_inventory'] = self._logged_channel.unary_unary(
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.insert_lfp_inventory: self._wrap_method(
