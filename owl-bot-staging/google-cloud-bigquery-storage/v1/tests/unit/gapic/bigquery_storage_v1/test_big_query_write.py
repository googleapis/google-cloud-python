...
def test__read_environment_variables():
    assert BigQueryWriteClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert BigQueryWriteClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert BigQueryWriteClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert BigQueryWriteClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert BigQueryWriteClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert BigQueryWriteClient._get_client_cert_source(None, False) is None
    assert BigQueryWriteClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert BigQueryWriteClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert BigQueryWriteClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert BigQueryWriteClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(BigQueryWriteClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryWriteClient))
@mock.patch.object(BigQueryWriteAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryWriteAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = BigQueryWriteClient._DEFAULT_UNIVERSE
    default_endpoint = BigQueryWriteClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = BigQueryWriteClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert BigQueryWriteClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert BigQueryWriteClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == BigQueryWriteClient.DEFAULT_MTLS_ENDPOINT
    assert BigQueryWriteClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert BigQueryWriteClient._get_api_endpoint(None, None, default_universe, "always") == BigQueryWriteClient.DEFAULT_MTLS_ENDPOINT
    assert BigQueryWriteClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == BigQueryWriteClient.DEFAULT_MTLS_ENDPOINT
    assert BigQueryWriteClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert BigQueryWriteClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        BigQueryWriteClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert BigQueryWriteClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert BigQueryWriteClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert BigQueryWriteClient._get_universe_domain(None, None) == BigQueryWriteClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        BigQueryWriteClient._get_universe_domain("", None)
...
@pytest.mark.parametrize("client_class,transport_name", [
    (BigQueryWriteClient, "grpc"),
    (BigQueryWriteAsyncClient, "grpc_asyncio"),
])
def test_big_query_write_client_from_service_account_info(client_class, transport_name):
...
def test_big_query_write_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_big_query_write_client_from_service_account_file(client_class, transport_name):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class", [
    BigQueryWriteClient, BigQueryWriteAsyncClient
])
...
def test_big_query_write_client_get_mtls_endpoint_and_cert_source(client_class):
    mock_client_cert_source = mock.Mock()
...
@mock.patch.object(BigQueryWriteClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryWriteClient))
@mock.patch.object(BigQueryWriteAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryWriteAsyncClient))
def test_big_query_write_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (BigQueryWriteClient, transports.BigQueryWriteGrpcTransport, "grpc", grpc_helpers),
    (BigQueryWriteAsyncClient, transports.BigQueryWriteGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
def test_big_query_write_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_big_query_write_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_create_write_stream_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.create_write_stream in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.create_write_stream] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_create_write_stream_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.create_write_stream in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_write_stream] = mock_rpc
...
def test_append_rows_use_cached_wrapped_rpc():
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        client = BigQueryWriteClient(
        ...
        # Ensure method has been cached
        assert client._transport.append_rows in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.append_rows] = mock_rpc
        request = [{}]
        client.append_rows(request)
...
@pytest.mark.asyncio
async def test_append_rows_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BigQueryWriteAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.append_rows in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.append_rows] = mock_rpc

        request = [{}]
        await client.append_rows(request)
...
def test_get_write_stream_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.get_write_stream in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.get_write_stream] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_get_write_stream_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BigQueryWriteAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.get_write_stream in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.get_write_stream] = mock_rpc
...
def test_finalize_write_stream_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.finalize_write_stream in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.finalize_write_stream] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_finalize_write_stream_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BigQueryWriteAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.finalize_write_stream in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.finalize_write_stream] = mock_rpc
...
def test_batch_commit_write_streams_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.batch_commit_write_streams in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.batch_commit_write_streams] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_batch_commit_write_streams_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BigQueryWriteAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.batch_commit_write_streams in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.batch_commit_write_streams] = mock_rpc
...
def test_flush_rows_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.flush_rows in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.flush_rows] = mock_rpc
        request = {}
...
async def test_flush_rows_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.flush_rows in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.flush_rows] = mock_rpc
...
def test_big_query_write_base_transport():
    ...
    with mock.patch('google.cloud.bigquery_storage_v1.services.big_query_write.transports.BigQueryWriteTransport.__init__') as Transport:
...
def test_big_query_write_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.bigquery_storage_v1.services.big_query_write.transports.BigQueryWriteTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_big_query_write_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.bigquery_storage_v1.services.big_query_write.transports.BigQueryWriteTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.BigQueryWriteTransport()
        adc.assert_called_once()
...
def test_big_query_write_transport_auth_gdch_credentials(transport_class):
...
def test_big_query_write_host_no_port(transport_name):
    client = BigQueryWriteClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_big_query_write_host_with_port(transport_name):
    client = BigQueryWriteClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
@pytest.mark.parametrize("client_class,transport_class", [
    ...
    (BigQueryWriteAsyncClient, transports.BigQueryWriteGrpcAsyncIOTransport),
])
