...
def test__read_environment_variables():
    assert BigQueryReadClient._read_environment_variables() == (False, "auto", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_CLIENT_CERTIFICATE": "true"}):
        assert BigQueryReadClient._read_environment_variables() == (True, "auto", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "never"}):
        assert BigQueryReadClient._read_environment_variables() == (False, "never", None)

    with mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "always"}):
        assert BigQueryReadClient._read_environment_variables() == (False, "always", None)
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert BigQueryReadClient._read_environment_variables() == (False, "auto", "foo.com")

def test__get_client_cert_source():
    mock_provided_cert_source = mock.Mock()
    mock_default_cert_source = mock.Mock()

    assert BigQueryReadClient._get_client_cert_source(None, False) is None
    assert BigQueryReadClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert BigQueryReadClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert BigQueryReadClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert BigQueryReadClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(BigQueryReadClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryReadClient))
@mock.patch.object(BigQueryReadAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryReadAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = BigQueryReadClient._DEFAULT_UNIVERSE
    default_endpoint = BigQueryReadClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = BigQueryReadClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert BigQueryReadClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert BigQueryReadClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == BigQueryReadClient.DEFAULT_MTLS_ENDPOINT
    assert BigQueryReadClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert BigQueryReadClient._get_api_endpoint(None, None, default_universe, "always") == BigQueryReadClient.DEFAULT_MTLS_ENDPOINT
    assert BigQueryReadClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == BigQueryReadClient.DEFAULT_MTLS_ENDPOINT
    assert BigQueryReadClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert BigQueryReadClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        BigQueryReadClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert BigQueryReadClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert BigQueryReadClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert BigQueryReadClient._get_universe_domain(None, None) == BigQueryReadClient._DEFAULT_UNIVERSE

    with pytest.raises(ValueError) as excinfo:
        BigQueryReadClient._get_universe_domain("", None)
...
def test_big_query_read_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class", [
    BigQueryReadClient, BigQueryReadAsyncClient
])
...
def test_big_query_read_client_get_mtls_endpoint_and_cert_source(client_class):
    mock_client_cert_source = mock.Mock()
...
@mock.patch.object(BigQueryReadClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryReadClient))
@mock.patch.object(BigQueryReadAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(BigQueryReadAsyncClient))
def test_big_query_read_client_client_api_endpoint(client_class):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (BigQueryReadClient, transports.BigQueryReadGrpcTransport, "grpc", grpc_helpers),
    (BigQueryReadAsyncClient, transports.BigQueryReadGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
def test_big_query_read_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_big_query_read_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
def test_create_read_session_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        assert client._transport.create_read_session in client._transport._wrapped_methods
...
@pytest.mark.asyncio
async def test_create_read_session_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.create_read_session in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.create_read_session] = mock_rpc
...
async def test_read_rows_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.read_rows in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.read_rows] = mock_rpc
...
def test_split_read_stream_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.split_read_stream in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.split_read_stream] = mock_rpc
        request = {}
...
@pytest.mark.asyncio
async def test_split_read_stream_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = BigQueryReadAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.split_read_stream in client._client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.AsyncMock()
        mock_rpc.return_value = mock.Mock()
        client._client._transport._wrapped_methods[client._client._transport.split_read_stream] = mock_rpc
...
def test_create_read_session_empty_call_grpc():
    client = BigQueryReadClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = stream.ReadSession()
        client.create_read_session(request=None)
...
# This test is a coverage failsafe to make sure that totally empty calls,
# i.e. request == None and no flattened fields passed, work.
def test_split_read_stream_empty_call_grpc():
    client = BigQueryReadClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="grpc",
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        call.return_value = storage.SplitReadStreamResponse()
        client.split_read_stream(request=None)
...
def test_transport_kind_grpc_asyncio():
    transport = BigQueryReadAsyncClient.get_transport_class("grpc_asyncio")(
        credentials=async_anonymous_credentials()
    )
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_create_read_session_empty_call_grpc_asyncio():
    client = BigQueryReadAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.create_read_session(request=None)
...
# i.e. request == None and no flattened fields passed, work.
@pytest.mark.asyncio
async def test_split_read_stream_empty_call_grpc_asyncio():
    client = BigQueryReadAsyncClient(
        credentials=async_anonymous_credentials(),
        transport="grpc_asyncio",
    )
    ...
    with mock.patch.object(
        ...
        await client.split_read_stream(request=None)
...
def test_big_query_read_transport_auth_gdch_credentials(transport_class):
...
@pytest.mark.parametrize("client_class,transport_class", [
    (BigQueryReadClient, transports.BigQueryReadGrpcTransport),
    (BigQueryReadAsyncClient, transports.BigQueryReadGrpcAsyncIOTransport),
])
