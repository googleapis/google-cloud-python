...
class PublisherGrpcAsyncIOTransport(PublisherTransport):
    ...
    def publish_channel_connection_events(self) -> Callable[
            ...
            Awaitable[publisher.PublishChannelConnectionEventsResponse]]:
        r"""Return a callable for the publish channel connection
        events method over gRPC.

        Publish events to a ChannelConnection in a partner's
        project.

        Returns:
            Callable[[~.PublishChannelConnectionEventsRequest],
                    Awaitable[~.PublishChannelConnectionEventsResponse]]:
        ...
        if 'publish_channel_connection_events' not in self._stubs:
            self._stubs['publish_channel_connection_events'] = self.grpc_channel.unary_unary(
    ...
    def publish_events(self) -> Callable[
            ...
            Awaitable[publisher.PublishEventsResponse]]:
        ...
        Returns:
            Callable[[~.PublishEventsRequest],
                    Awaitable[~.PublishEventsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.publish_channel_connection_events: gapic_v1.method_async.wrap_method(
                self.publish_channel_connection_events,
                default_timeout=60.0,
                client_info=client_info,
            ),
            self.publish_events: gapic_v1.method_async.wrap_method(
                self.publish_events,
