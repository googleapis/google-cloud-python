...
class ListCloudControlDeploymentsAsyncPager:
    ...
    """
    def __init__(self,
            method: Callable[..., Awaitable[deployment.ListCloudControlDeploymentsResponse]],
            request: deployment.ListCloudControlDeploymentsRequest,
            response: deployment.ListCloudControlDeploymentsResponse,
            *,
            retry: OptionalAsyncRetry = gapic_v1.method.DEFAULT,
    ...
    @property
    async def pages(self) -> AsyncIterator[deployment.ListCloudControlDeploymentsResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
