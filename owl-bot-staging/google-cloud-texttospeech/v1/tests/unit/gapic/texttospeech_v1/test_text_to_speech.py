...
@pytest.mark.parametrize("client_class", [
    TextToSpeechClient, TextToSpeechAsyncClient
])
...
def test_list_voices_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         ...
         mock.patch.object(transports.TextToSpeechRestInterceptor, "post_list_voices") as post, \
         mock.patch.object(transports.TextToSpeechRestInterceptor, "pre_list_voices") as pre:
        ...
        req.return_value._content = cloud_tts.ListVoicesResponse.to_json(cloud_tts.ListVoicesResponse())
        ...
        client.list_voices(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_list_voices_rest_bad_request(transport: str = 'rest', request_type=cloud_tts.ListVoicesRequest):
    client = TextToSpeechClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_list_voices_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/voices" % client.transport._host, args[1])
...
def test_synthesize_speech_rest_required_fields(request_type=cloud_tts.SynthesizeSpeechRequest):
    ...
    unset_fields = transport_class(credentials=_AnonymousCredentialsWithUniverseDomain()).synthesize_speech._get_unset_required_fields(jsonified_request)
...
def test_synthesize_speech_rest_unset_required_fields():
    transport = transports.TextToSpeechRestTransport(credentials=_AnonymousCredentialsWithUniverseDomain)

    unset_fields = transport.synthesize_speech._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("input", "voice", "audioConfig", )))
...
def test_synthesize_speech_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TextToSpeechRestInterceptor, "post_synthesize_speech") as post, \
         mock.patch.object(transports.TextToSpeechRestInterceptor, "pre_synthesize_speech") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = cloud_tts.SynthesizeSpeechResponse.to_json(cloud_tts.SynthesizeSpeechResponse())
        ...
        client.synthesize_speech(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_synthesize_speech_rest_bad_request(transport: str = 'rest', request_type=cloud_tts.SynthesizeSpeechRequest):
    client = TextToSpeechClient(
        credentials=_AnonymousCredentialsWithUniverseDomain(),
        transport=transport,
    )
...
def test_synthesize_speech_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/text:synthesize" % client.transport._host, args[1])
...
def test_text_to_speech_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.texttospeech_v1.services.text_to_speech.transports.TextToSpeechTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_text_to_speech_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.texttospeech_v1.services.text_to_speech.transports.TextToSpeechTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (_AnonymousCredentialsWithUniverseDomain(), None)
        transport = transports.TextToSpeechTransport()
        adc.assert_called_once()
