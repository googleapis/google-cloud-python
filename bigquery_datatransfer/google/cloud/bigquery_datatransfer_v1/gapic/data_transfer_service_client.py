...
class DataTransferServiceClient(object):
    ...
    def schedule_transfer_runs(
        ...
        request = datatransfer_pb2.ScheduleTransferRunsRequest(
            parent=parent, start_time=start_time, end_time=end_time, labels=labels
        )
        if metadata is None:
            metadata = []
        metadata = list(metadata)
    ...
    def enable_data_transfer_service(
        ...
        request = datatransfer_pb2.EnableDataTransferServiceRequest(name=name)
        if metadata is None:
            metadata = []
        metadata = list(metadata)
        try:
    ...
    def is_data_transfer_service_enabled(
        ...
        Returns:
            A :class:`~google.cloud.bigquery_datatransfer_v1.types.IsDataTransferServiceEnabledResponse` instance.
        ...
        if "is_data_transfer_service_enabled" not in self._inner_api_calls:
            self._inner_api_calls[
                "is_data_transfer_service_enabled"
            ] = google.api_core.gapic_v1.method.wrap_method(
        ...
        request = datatransfer_pb2.IsDataTransferServiceEnabledRequest(name=name)
        if metadata is None:
            metadata = []
        metadata = list(metadata)
        try:
