...
class TestConnection(unittest2.TestCase):
    ...
    def test_generate_signed_url_w_expiration_int(self):
        ...
        def _generate(*args, **kw):
            _called_with.append((args, kw))

        with _Monkey(credentials, generate_signed_url=_generate):
            _ = conn.generate_signed_url(RESOURCE, 1000)

        self.assertEqual(len(_called_with), 1)
        args, kw = _called_with[0]

        self.assertTrue(args[0] is creds)
        self.assertEqual(args[1], ENDPOINT)
        self.assertEqual(args[2], RESOURCE)
