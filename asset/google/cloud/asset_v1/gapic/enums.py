...
class BasicLevel(object):
    class ConditionCombiningFunction(enum.IntEnum):
        """
        Options for how the ``conditions`` list should be combined to determine
        if this ``AccessLevel`` is applied. Default is AND.

        Attributes:
          AND (int): All ``Conditions`` must be true for the ``BasicLevel`` to be true.
          OR (int): If at least one ``Condition`` is true, then the ``BasicLevel`` is true.
        """
...
class Policy(object):
    class ListPolicy(object):
        class AllValues(enum.IntEnum):
            ...
            ``DENY,``\ allowed\_values\ ``and``\ denied\_values\ ``must be unset. Setting this to``\ ALL\_VALUES\_UNSPECIFIED\ ``allows for setting``\ allowed\_values\ ``and``\ denied\_values\`.

            Attributes:
              ALL_VALUES_UNSPECIFIED (int): Indicates that allowed\_values or denied\_values must be set.
