...
STATIC_MODEL='test_online_model_do_not_delete'
...
class TestSystemTablesClient(object):
    ...
    def test_create_delete_dataset(self):
        ...
        assert dataset.name == client.get_dataset(
    ...
    def test_import_data(self):
        ...
        dataset = client.create_dataset(display_name)
        op = client.import_data(dataset=dataset,
    ...
    def test_set_column_nullable(self):
        client = automl_v1beta1.TablesClient(project=PROJECT, region=REGION)
        dataset = self.ensure_dataset_ready(client)
        client.update_column_spec(dataset=dataset,
                column_spec_display_name='POutcome', nullable=True)
        columns = {c.display_name: c
                for c in client.list_column_specs(dataset=dataset)}
    ...
    def test_set_weight_column(self):
        client = automl_v1beta1.TablesClient(project=PROJECT, region=REGION)
        dataset = self.ensure_dataset_ready(client)
        client.set_weight_column(dataset=dataset,
                column_spec_display_name='Duration')
        columns = {c.display_name: c
                for c in client.list_column_specs(dataset=dataset)}
        dataset = client.get_dataset(dataset_name=dataset.name)
        metadata = dataset.tables_dataset_metadata
    ...
    def test_set_weight_and_target_column(self):
        ...
        dataset = self.ensure_dataset_ready(client)
        client.set_weight_column(dataset=dataset,
                column_spec_display_name='Day')
        client.set_target_column(dataset=dataset,
                column_spec_display_name='Campaign')
        columns = {c.display_name: c
                for c in client.list_column_specs(dataset=dataset)}
        dataset = client.get_dataset(dataset_name=dataset.name)
        metadata = dataset.tables_dataset_metadata
    ...
    def test_create_delete_model(self):
        ...
        display_name = _id('t_cr_dl')
        op = client.create_model(display_name, dataset=dataset,
    ...
    def ensure_model_ready(self, client):
        ...
        client.update_column_spec(dataset=dataset,
                column_spec_display_name='POutcome', nullable=True)
        op = client.create_model(STATIC_MODEL, dataset=dataset,
                train_budget_milli_node_hours=1000)
        op.result()
        return client.get_model(model_display_name=STATIC_MODEL)
