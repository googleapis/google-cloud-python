...
class TestVPCServiceControlV3(object):
    ...
    def test_delete_metric_descriptor(self):
        client = monitoring_v3.MetricServiceClient()
        name_inside = client.metric_descriptor_path(PROJECT_INSIDE, 'mock_metric_descriptor')
        delayed_inside = lambda: client.delete_metric_descriptor(name_inside)
        name_outside = client.metric_descriptor_path(PROJECT_OUTSIDE, 'mock_metric_descriptor')
    ...
    def test_get_monitored_resource_descriptor(self):
        client = monitoring_v3.MetricServiceClient()
        name_inside = client.monitored_resource_descriptor_path(PROJECT_INSIDE, 'mock_monitored_resource_descriptor')
        delayed_inside = lambda: client.get_monitored_resource_descriptor(name_inside)
        name_outside = client.monitored_resource_descriptor_path(PROJECT_OUTSIDE, 'mock_monitored_resource_descriptor')
    ...
    def test_list_monitored_resource_descriptors(self):
        ...
        name_outside = client.project_path(PROJECT_OUTSIDE)
        delayed_outside = lambda: client.list_monitored_resource_descriptors(name_outside)
        TestVPCServiceControlV3._do_test(delayed_inside, delayed_outside)
    ...
    def test_list_time_series(self):
        client = monitoring_v3.MetricServiceClient()
        name_inside = client.project_path(PROJECT_INSIDE)
        delayed_inside = lambda: client.list_time_series(name_inside, '', {}, enums.ListTimeSeriesRequest.TimeSeriesView.FULL)
        name_outside = client.project_path(PROJECT_OUTSIDE)
        delayed_outside = lambda: client.list_time_series(name_outside, '', {}, enums.ListTimeSeriesRequest.TimeSeriesView.FULL)
        TestVPCServiceControlV3._do_test(delayed_inside, delayed_outside)
